 ...
 * in the database
 */
class Ultimatum_Dismissable_Notice_Ajax {
	...
	public function __construct( $notice_name, $notice_type = self::FOR_USER ) {
		...
		add_action( 'mcms_ajax_mcmsseo_dismiss_' . $notice_name, array( $this, 'dismiss_notice' ) );
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'mcmsseo-dismiss-' . $this->notice_name );
	...
	private function save_dismissed() {
		if ( $this->notice_type === self::FOR_SITE ) {
			update_option( 'mcmsseo_dismiss_' . $this->notice_name, 1 );

			return;
		}

		if ( $this->notice_type === self::FOR_NETWORK ) {
			update_site_option( 'mcmsseo_dismiss_' . $this->notice_name, 1 );

			return;
		}

		update_user_meta( get_current_user_id(), 'mcmsseo_dismiss_' . $this->notice_name, 1 );
	}
}
