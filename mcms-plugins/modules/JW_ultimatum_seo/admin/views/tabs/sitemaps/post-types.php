...
echo '<h2>' . esc_html__( 'Post types sitemap settings', 'mandarincms-seo' ) . '</h2>';
 ...
 */
$post_types = apply_filters( 'mcmsseo_sitemaps_supported_post_types', get_post_types( array( 'public' => true ), 'objects' ) );
if ( is_array( $post_types ) && $post_types !== array() ) {
	foreach ( $post_types as $pt ) {
		 ...
		 *
		 * @api  Ultimatum_Form  $yform  The Ultimatum_Form object
		 * @api  Object      $pt     The post type
		 */
		do_action( 'mcmsseo_admin_page_sitemap_post_types', $yform, $pt );
	}
}
