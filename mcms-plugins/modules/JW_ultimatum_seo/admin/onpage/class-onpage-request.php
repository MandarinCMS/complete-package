 ...
 */
class MCMSSEO_OnPage_Request {

	/**
	 * @var string The endpoint where the request will be send to.
	 */
	private $onpage_endpoint = 'https://indexability.ultimatum.onpage.org/';
	...
	protected function get_remote( $target_url, $parameters = array() ) {
		$parameters = array_merge( array(
			'url'          => $target_url,
			'mcms_version'   => $GLOBALS['mcms_version'],
			'yseo_version' => MCMSSEO_VERSION,
		), $parameters );
		...
		if ( $response_code === 200 ) {
			$response_body  = mcms_remote_retrieve_body( $response );
	...
	public function get_response() {
		_deprecated_function( 'MCMSSEO_OnPage_Request::get_response', 'MCMSSEO 3.1.2', 'MCMSSEO_OnPage_Request::do_request' );
