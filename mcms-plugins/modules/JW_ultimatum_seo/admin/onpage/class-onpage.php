 ...
 */
class MCMSSEO_OnPage {

	/**
	 * The name of the user meta key for storing the dismissed status.
	 */
	const USER_META_KEY = 'mcmsseo_dismiss_onpage';

	/**
	 * @var MCMSSEO_OnPage_Option The OnPage.org option class.
	 */
	...
	public function __construct() {
		...
		if ( ! ( defined( 'DOING_AJAX' ) && DOING_AJAX === true ) ) {
			$this->onpage_option = new MCMSSEO_OnPage_Option();
	...
	/**
	 * The hooks to run on module activation
	 */
	...
	public function fetch_from_onpage() {
		if ( $this->onpage_option->should_be_fetched() && false !== ( $new_status = $this->request_indexability() ) ) {
			...
			if ( $old_status !== $new_status && $new_status !== MCMSSEO_OnPage_Option::CANNOT_FETCH ) {
				$this->notify_admins();
			}
	...
	protected function request_indexability() {
		...
		$request  = new MCMSSEO_OnPage_Request();
		...
		return MCMSSEO_OnPage_Option::CANNOT_FETCH;
	}
	...
	protected function should_show_notice() {
		...
		if ( MCMSSEO_Utils::is_development_mode() || ( '0' === get_option( 'blog_public' ) ) ) {
			return false;
		}

		return $this->onpage_option->get_status() === MCMSSEO_OnPage_Option::IS_NOT_INDEXABLE;
	}
	...
	protected function notify_admins() {
		 ...
		 */
		delete_metadata( 'user', 0, MCMSSEO_OnPage::USER_META_KEY, '', true );
	}
	...
	private function set_hooks() {
		...
		register_activation_hook( MCMSSEO_FILE, array( $this, 'activate_hooks' ) );
		...
		add_action( 'mcmsseo_onpage_fetch', array( $this, 'fetch_from_onpage' ) );
	}
	...
	private function set_cron() {
		if ( ! mcms_next_scheduled( 'mcmsseo_onpage_fetch' ) ) {
			mcms_schedule_event( time(), 'weekly', 'mcmsseo_onpage_fetch' );
		}
	}
	...
	private function catch_redo_listener() {
		if ( filter_input( INPUT_GET, 'mcmsseo-redo-onpage' ) === '1' ) {
