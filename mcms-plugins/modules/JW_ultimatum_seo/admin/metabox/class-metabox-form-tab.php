 ...
 */
class MCMSSEO_Metabox_Form_Tab implements MCMSSEO_Metabox_Tab {
	...
	public function link() {
		...
		$html = '<li class="%1$s%2$s"><a class="mcmsseo_tablink%3$s" href="#mcmsseo_%1$s"%4$s%5$s>%6$s</a></li>';

		if ( $this->single ) {
			$html = '<li class="%1$s%2$s"><span class="mcmsseo_tablink%3$s"%4$s%5$s>%6$s</span></li>';
		}
	...
	public function content() {
		return sprintf(
			'<div id="mcmsseo_%1$s" class="mcmsseotab %1$s"><table class="form-table">%2$s</table></div>',
			esc_attr( $this->name ),
			$this->content
		);
	}
