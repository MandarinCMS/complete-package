 ...
 */
class MCMSSEO_Bulk_Title_Editor_List_Table extends MCMSSEO_Bulk_List_Table {
	...
	protected $settings = array(
		'singular' => 'mcmsseo_bulk_title',
		'plural'   => 'mcmsseo_bulk_titles',
		'ajax'     => true,
	);
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'col_existing_ultimatum_seo_title' => sprintf( __( 'Existing %1$s Title', 'mandarincms-seo' ), 'Ultimatum SEO' ),
			/* translators: %1$s expands to Ultimatum SEO */
			'col_new_ultimatum_seo_title'      => sprintf( __( 'New %1$s Title', 'mandarincms-seo' ), 'Ultimatum SEO' ),
		);
	...
	protected function parse_page_specific_column( $column_name, $record, $attributes ) {
		...
		switch ( $column_name ) {
			case 'col_existing_ultimatum_seo_title':
				// TODO inconsistent echo/return behavior R.
				echo $this->parse_meta_data_field( $record->ID, $attributes );
				break;

			case 'col_new_ultimatum_seo_title':
				return sprintf(
					'<input type="text" id="%1$s" name="%1$s" class="mcmsseo-new-title" data-id="%2$s" aria-labelledby="col_new_ultimatum_seo_title" />',
					'mcmsseo-new-title-' . $record->ID,
					$record->ID
				);
				break;
		}
