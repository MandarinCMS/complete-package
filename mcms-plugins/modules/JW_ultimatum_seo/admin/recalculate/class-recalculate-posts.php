 ...
 */
class MCMSSEO_Recalculate_Posts extends MCMSSEO_Recalculate {
	...
	/**
	 * Get the posts from the database by doing a MCMS_Query.
	 *
	 ...
	 *
	 * @param MCMS_Post $item The post for which to build the analyzer data.
	 *
	 * @return array
	 */
	protected function item_to_response( $item ) {
		$focus_keyword = MCMSSEO_Meta::get_value( 'focuskw', $item->ID );
		 ...
		 * @param MCMS_Post $item The Post object the content comes from.
		 */
		$content = apply_filters( 'mcmsseo_post_content_for_recalculation', $content, $item );
		...
		return array(
			...
			'pageTitle'     => apply_filters( 'mcmsseo_title', mcmsseo_replace_vars( $this->get_title( $item->ID, $item->post_type ), $item ) ),
			'meta'          => apply_filters( 'mcmsseo_metadesc', mcmsseo_replace_vars( $this->get_meta_description( $item->ID, $item->post_type ), $item ) ),
			'keyword_usage' => array(
				$focus_keyword => MCMSSEO_Meta::keyword_usage( $focus_keyword, $item->ID ),
			),
		);
	}
	...
	private function get_title( $post_id, $post_type ) {
		$title = MCMSSEO_Meta::get_value( 'title', $post_id );
		if ( '' !== $title ) {
			return $title;
		}
	...
	private function get_meta_description( $post_id, $post_type ) {
		$meta_description = MCMSSEO_Meta::get_value( 'metadesc', $post_id );
		if ( '' !== $meta_description ) {
	 ...
	 *
	 * @param MCMS_Post $item The post item to check for a featured image.
	 *
