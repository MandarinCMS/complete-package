 ...
 */
class MCMSSEO_Admin_Help_Panel {
	...
	public function get_button_html() {
		...
		return sprintf(
			' <button type="button" class="ultimatum_help ultimatum-help-button dashicons" id="%1$s-help-toggle" aria-expanded="false" aria-controls="%1$s-help"><span class="screen-reader-text">%2$s</span></button>',
			esc_attr( $this->id ),
	...
	public function get_panel_html() {
		...
		if ( 'has-wrapper' === $this->wrapper ) {
			$wrapper_start = '<div class="ultimatum-seo-help-container">';
			$wrapper_end   = '</div>';
		}

		return sprintf(
			'%1$s<p id="%2$s-help" class="ultimatum-help-panel">%3$s</p>%4$s',
			$wrapper_start,
			esc_attr( $this->id ),
			$this->help_content,
			$wrapper_end
