...
/**
 * Class MCMSSEO_Import_External
 *
 * Class with functionality to import Ultimatum SEO settings from other modules
 */
class MCMSSEO_Import_External {
	...
	public function import_headspace() {
		...
		MCMSSEO_Meta::replace_meta( '_headspace_description', MCMSSEO_Meta::$meta_prefix . 'metadesc', $this->replace );
		MCMSSEO_Meta::replace_meta( '_headspace_keywords', MCMSSEO_Meta::$meta_prefix . 'metakeywords', $this->replace );
		MCMSSEO_Meta::replace_meta( '_headspace_page_title', MCMSSEO_Meta::$meta_prefix . 'title', $this->replace );
		 ...
		 */
		MCMSSEO_Meta::replace_meta( '_headspace_noindex', MCMSSEO_Meta::$meta_prefix . 'meta-robots-noindex', $this->replace );
		MCMSSEO_Meta::replace_meta( '_headspace_nofollow', MCMSSEO_Meta::$meta_prefix . 'meta-robots-nofollow', $this->replace );
		 ...
		 */
		$posts = $mcmsdb->get_results( "SELECT ID FROM $mcmsdb->posts" );
		if ( is_array( $posts ) && $posts !== array() ) {
			foreach ( $posts as $post ) {
				...
				MCMSSEO_Meta::set_value( 'meta-robots-adv', $robotsmeta_adv, $post->ID );
			}
		}
		...
		$this->set_msg( __( 'HeadSpace2 data successfully imported', 'mandarincms-seo' ) );
	}
}
