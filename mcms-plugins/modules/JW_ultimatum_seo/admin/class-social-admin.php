...
/**
 * This class adds the Social tab to the Ultimatum SEO metabox and makes sure the settings are saved.
 */
class MCMSSEO_Social_Admin extends MCMSSEO_Metabox {
	...
	public function __construct() {
		...
		add_filter( 'mcmsseo_save_metaboxes', array( $this, 'save_meta_boxes' ), 10, 1 );
		add_action( 'mcmsseo_save_compare_data', array( $this, 'og_data_compare' ), 10, 1 );
	}
	...
	public static function translate_meta_boxes() {
		...
		$title_text       = __( 'If you don\'t want to use the post title for sharing the post on %s but instead want another title there, write it here.', 'mandarincms-seo' );
		/* translators: %s expands to the social network's name */
		$description_text = __( 'If you don\'t want to use the meta description for sharing the post on %s but want another description there, write it here.', 'mandarincms-seo' );
		/* translators: %s expands to the social network's name */
		$image_text       = __( 'If you want to override the image used on %s for this post, upload / choose an image here.', 'mandarincms-seo' );
		/* translators: %1$s expands to the social network, %2$s to the recommended image size */
		$image_size_text  = __( 'The recommended image size for %1$s is %2$s pixels.', 'mandarincms-seo' );

		$options = MCMSSEO_Options::get_option( 'mcmsseo_social' );

		$social_networks = array(
			'opengraph'  => __( 'Facebook', 'mandarincms-seo' ),
			'twitter'    => __( 'Twitter', 'mandarincms-seo' ),
		);
		...
		$recommended_image_sizes = array(
			...
			'opengraph'   => sprintf( __( '%1$s by %2$s', 'mandarincms-seo' ), '1200', '630' ), // Source: https://developers.facebook.com/docs/sharing/best-practices#images.
			/* translators: %1$s expands to the image recommended width, %2$s to its height. */
			'twitter'     => sprintf( __( '%1$s by %2$s', 'mandarincms-seo' ), '1024', '512' ),
		);

		foreach ( $social_networks as $network => $label ) {
			if ( true === $options[ $network ] ) {

				self::$meta_fields['social'][ $network . '-title' ]['title']       = sprintf( __( '%s Title', 'mandarincms-seo' ), $label );
				self::$meta_fields['social'][ $network . '-title' ]['description'] = sprintf( $title_text, $label );

				self::$meta_fields['social'][ $network . '-description' ]['title']       = sprintf( __( '%s Description', 'mandarincms-seo' ), $label );
				self::$meta_fields['social'][ $network . '-description' ]['description'] = sprintf( $description_text, $label );

				self::$meta_fields['social'][ $network . '-image' ]['title']       = sprintf( __( '%s Image', 'mandarincms-seo' ), $label );
	 ...
	 *
	 * @return MCMSSEO_Metabox_Tab_Section
	 */
	public function get_meta_section() {
		...
		if ( $this->options['opengraph'] === true ) {
			$tabs[] = new MCMSSEO_Metabox_Form_Tab(
				...
				'<span class="screen-reader-text">' . __( 'Facebook / Open Graph metadata', 'mandarincms-seo' ) . '</span><span class="dashicons dashicons-facebook-alt"></span>',
				array(
					'link_aria_label' => __( 'Facebook / Open Graph metadata', 'mandarincms-seo' ),
					'link_class'      => 'ultimatum-tooltip ultimatum-tooltip-se',
					'single'          => $single,
				)
			);
		}
		...
		if ( $this->options['twitter'] === true ) {
			$tabs[] = new MCMSSEO_Metabox_Form_Tab(
				...
				'<span class="screen-reader-text">' . __( 'Twitter metadata', 'mandarincms-seo' ) . '</span><span class="dashicons dashicons-twitter"></span>',
				array(
					'link_aria_label' => __( 'Twitter metadata', 'mandarincms-seo' ),
					'link_class'      => 'ultimatum-tooltip ultimatum-tooltip-se',
					'single'          => $single,
				)
			);
		...
		return new MCMSSEO_Metabox_Tab_Section(
			'social',
			'<span class="screen-reader-text">' . __( 'Social', 'mandarincms-seo' ) . '</span><span class="dashicons dashicons-share"></span>',
			$tabs,
			array(
				'link_aria_label' => __( 'Social', 'mandarincms-seo' ),
	 ...
	 *
	 * @param MCMS_Post $post Post instance.
	 */
	...
	public function tab_header() {
		_deprecated_function( 'MCMSSEO_Social_Admin::tab_header', 'MCMSSEO 3.0' );
	...
	public function tab_content() {
		_deprecated_function( 'MCMSSEO_Social_Admin::tab_content', 'MCMSSEO 3.0' );
