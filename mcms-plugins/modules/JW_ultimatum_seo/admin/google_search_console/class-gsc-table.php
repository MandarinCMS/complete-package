...
/**
 * Class MCMSSEO_GSC_Table
 */
class MCMSSEO_GSC_Table extends MCMS_List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'url'            => __( 'URL', 'mandarincms-seo' ),
			'last_crawled'   => __( 'Last crawled', 'mandarincms-seo' ),
			'first_detected' => __( 'First detected', 'mandarincms-seo' ),
			'response_code'  => __( 'Response code', 'mandarincms-seo' ),
		);
	...
	protected function get_bulk_actions() {
		return array(
			'mark_as_fixed' => __( 'Mark as fixed', 'mandarincms-seo' ),
		);
	}
	...
	protected function column_cb( $item ) {
		return sprintf(
			'<input type="checkbox" name="mcmsseo_crawl_issues[]" value="%s" />', $item['url']
		);
	}
	...
	protected function column_url( $item ) {
		...
		if ( $this->can_create_redirect() ) {
			...
			$actions['create_redirect'] = '<a href="#TB_inline?width=600&height=' . $this->modal_heights[ $modal_height ] . '&inlineId=redirect-' . md5( $item['url'] ) . '" class="thickbox mcmsseo-open-gsc-redirect-modal aria-button-if-js">' . __( 'Create redirect', 'mandarincms-seo' ) . '</a>';
		}

		$actions['view']        = '<a href="' . home_url( $item['url'] ) . '" target="_blank">' . __( 'View', 'mandarincms-seo' ) . '</a>';
		$actions['markasfixed'] = '<a href="javascript:mcmsseo_mark_as_fixed(\'' . urlencode( $item['url'] ) . '\');">' . __( 'Mark as fixed', 'mandarincms-seo' ) . '</a>';
	...
	private function modal_box( $url ) {
		...
		require MCMSSEO_PATH . '/admin/google_search_console/views/gsc-create-redirect.php';
	...
	private function modal_box_type( $url, &$current_redirect ) {
		if ( defined( 'MCMSSEO_PREMIUM_FILE' ) && class_exists( 'MCMSSEO_Redirect_Manager' ) ) {
	...
	private function show_fields( $platform ) {
		echo "<input type='hidden' name='mcmsseo_gsc_nonce' value='" . mcms_create_nonce( 'mcmsseo_gsc_nonce' ) . "' />";
