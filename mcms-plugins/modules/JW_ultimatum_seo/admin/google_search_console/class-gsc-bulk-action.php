...
class MCMSSEO_GSC_Bulk_Action {
	...
	public function __construct() {
		if ( mcms_verify_nonce( filter_input( INPUT_POST, 'mcmsseo_gsc_nonce' ), 'mcmsseo_gsc_nonce' ) ) {
	...
	private function handle_bulk_action() {
		if ( $bulk_action = $this->determine_bulk_action() ) {
			...
			mcms_redirect( filter_input( INPUT_POST, '_mcms_http_referer' ) );
			exit;
		}
	}
	...
	private function posted_issues() {
		if ( $issues = filter_input( INPUT_POST, 'mcmsseo_crawl_issues', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY ) ) {
			return $issues;
		}
