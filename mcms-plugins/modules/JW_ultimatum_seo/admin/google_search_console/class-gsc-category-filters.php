...
/**
 * Class MCMSSEO_GSC_Category_Filters
 *
...
class MCMSSEO_GSC_Category_Filters {
	...
	private function get_current_category() {
		...
		if ( ! empty( $this->category_counts ) ) {
			...
			mcms_redirect( add_query_arg( 'category', $current_category ) );
			exit;
		}
	}
	...
	private function set_filter_values() {
		$this->set_filter_value( 'access_denied', __( 'Access denied', 'mandarincms-seo' ), __( 'Server requires authentication or is blocking Googlebot from accessing the site.', 'mandarincms-seo' ), __( 'Show information about errors in category \'Access Denied\'', 'mandarincms-seo' ) );
		$this->set_filter_value( 'faulty_redirects', __( 'Faulty redirects', 'mandarincms-seo' ) );
		$this->set_filter_value( 'not_followed',__( 'Not followed', 'mandarincms-seo' ) );
		$this->set_filter_value( 'not_found', __( 'Not found', 'mandarincms-seo' ), __( 'URL points to a non-existent page.', 'mandarincms-seo' ), __( 'Show information about errors in category \'Not Found\'', 'mandarincms-seo' ) );
		$this->set_filter_value( 'other', __( 'Other', 'mandarincms-seo' ), __( 'Google was unable to crawl this URL due to an undetermined issue.', 'mandarincms-seo' ), __( 'Show information about errors in category \'Other\'', 'mandarincms-seo' ) );
		/* Translators: %1$s: expands to '<code>robots.txt</code>'. */
		$this->set_filter_value( 'roboted', __( 'Blocked', 'mandarincms-seo' ), sprintf( __( 'Googlebot could access your site, but certain URLs are blocked for Googlebot in your %1$s file. This block could either be for all Googlebots or even specifically for Googlebot-mobile.', 'mandarincms-seo' ), '<code>robots.txt</code>' ), __( 'Show information about errors in category \'Blocked\'', 'mandarincms-seo' ) );
		$this->set_filter_value( 'server_error', __( 'Server Error', 'mandarincms-seo' ), __( 'Request timed out or site is blocking Google.', 'mandarincms-seo' ), __( 'Show information about errors in category \'Server\'', 'mandarincms-seo' ) );
		$this->set_filter_value( 'soft_404', __( 'Soft 404', 'mandarincms-seo' ), __( "The target URL doesn't exist, but your server is not returning a 404 (file not found) error.", 'mandarincms-seo' ), __( 'Show information about errors in category \'Soft 404\'', 'mandarincms-seo' ) );
	}
	...
	private function create_view_link( $category, $count ) {
		...
		if ( $this->filter_values[ $category ]['description'] !== '' ) {
			$help        = new MCMSSEO_Admin_Help_Panel( $category, $this->filter_values[ $category ]['help-button'], $this->filter_values[ $category ]['description'], 'has-wrapper' );
