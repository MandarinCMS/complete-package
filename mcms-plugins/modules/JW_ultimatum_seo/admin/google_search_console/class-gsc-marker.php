...
class MCMSSEO_GSC_Marker {
	...
	private function get_result() {
		if ( $this->can_be_marked_as_fixed() ) {
			$service = new MCMSSEO_GSC_Service( MCMSSEO_GSC_Settings::get_profile() );
	...
	private function set_crawl_issues() {
		...
		if ( $this->platform && $this->category ) {
			$this->crawl_issues = new MCMSSEO_GSC_Issues( $this->platform, $this->category );
	 ...
	 *
	 * @param MCMSSEO_GSC_Service $service Service object instance.
	 *
	 * @return bool
	 */
	private function send_mark_as_fixed( MCMSSEO_GSC_Service $service ) {
	 ...
	 * @param MCMSSEO_GSC_Service $service Service object instance.
	 */
	private function update_issue_count( MCMSSEO_GSC_Service $service ) {
		$counts  = new MCMSSEO_GSC_Count( $service );
