...
/**
 * Class MCMSSEO_GSC_Count
 */
class MCMSSEO_GSC_Count {

	// The last checked timestamp.
	const OPTION_CI_LAST_FETCH = 'mcmsseo_gsc_last_fetch';

	// The option name where the issues counts are saved.
	const OPTION_CI_COUNTS     = 'mcmsseo_gsc_issues_counts';
	 ...
	 */
	public function __construct( MCMSSEO_GSC_Service $service ) {
	...
	public function fetch_counts() {
		if ( MCMSSEO_GSC_Settings::get_profile() && $this->get_last_fetch() <= strtotime( '-12 hours' ) ) {
	...
	/**
	 * Parsing the received counts from the API and map the keys to module friendly values
	 *
	...
	private function parse_counts( array $fetched_counts ) {
		...
		foreach ( $fetched_counts as $platform_name => $categories ) {
			$new_platform = MCMSSEO_GSC_Mapper::platform_from_api( $platform_name );

			foreach ( $categories as $category_name => $category ) {
				$new_category = MCMSSEO_GSC_Mapper::category_from_api( $category_name );
	...
	private function list_category_issues( array $counts, $platform, $category ) {
		...
		if ( array_key_exists( $category, $counts ) && $counts[ $category ]['count'] > 0 && $counts[ $category ]['last_fetch'] <= strtotime( '-12 hours' ) ) {
			if ( $issues = $this->service->fetch_category_issues( MCMSSEO_GSC_Mapper::platform_to_api( $platform ), MCMSSEO_GSC_Mapper::category_to_api( $category ) ) ) {
				$this->issues = $issues;
			}
