...
class MCMSSEO_GSC_Issues {
	...
	public function __construct( $platform, $category, $fetched_issues = false ) {
		$this->option_name = strtolower( 'mcmsseo-gsc-issues-' . $platform . '-' . $category );
	...
	private function issue_compare( &$crawl_issues, $issue ) {
		$issue->pageUrl = MCMSSEO_Utils::format_url( (string) $issue->pageUrl );
	...
	/**
	 * The fetched issue from the API will be parsed as an MCMSSEO_Crawl_Issue object. After initializing the issue as an
	 * object, the object will be returned
	 *
	 * @param stdClass $issue Issue data object.
	 *
	 * @return MCMSSEO_GSC_Issue
	 */
	private function create_issue( $issue ) {
		return new MCMSSEO_GSC_Issue(
			$issue->pageUrl,
	 ...
	 *
	 * @param MCMSSEO_GSC_Issue $crawl_issue Issue object instance.
	 *
	 * @return array()
	 */
	private function get_issue( MCMSSEO_GSC_Issue $crawl_issue ) {
	...
	private function set_current_issues() {
		if ( ! empty( $this->issues ) ) {
			$this->current_issues = mcms_list_pluck( $this->issues, 'url' );
		}
	}
