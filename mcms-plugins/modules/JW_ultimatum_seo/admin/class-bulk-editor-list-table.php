 ...
 */
class MCMSSEO_Bulk_List_Table extends MCMS_List_Table {
	...
	function __construct() {
		...
		$this->nonce    = mcms_create_nonce( 'bulk-editor-table' );
	...
	private function verify_nonce() {
		if ( $this->should_verify_nonce() && ! mcms_verify_nonce( filter_input( INPUT_GET, 'nonce' ), 'bulk-editor-table' ) ) {
			Ultimatum_Notification_Center::get()->add_notification(
				new Ultimatum_Notification(
					__( 'You are not allowed to access this page.', 'mandarincms-seo' ),
					array( 'type' => Ultimatum_Notification::ERROR )
				)
			);
	...
	function display_tablenav( $which ) {
		...
		<div class="tablenav <?php echo esc_attr( $which ); ?>">
			...
			<form id="posts-filter" action="" method="get">
				...
				<input type="hidden" name="page" value="mcmsseo_tools"/>
	 ...
	 *
	 * @return string $subquery The subquery, which should always be used in $mcmsdb->prepare(), passing the current user_id in as the first parameter.
	 */
	...
	function get_views() {
		...
		$status_links['all'] = '<a href="' . esc_url( admin_url( 'admin.php?page=mcmsseo_tools&tool=bulk-editor' . $this->page_url ) ) . '"' . $class . '>' . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts', 'mandarincms-seo' ), number_format_i18n( $total_posts ) ) . '</a>';
		...
		if ( is_array( $post_stati ) && $post_stati !== array() ) {
			foreach ( $post_stati as $status ) {
				...
				$status_links[ $status_name ] = '<a href="' . esc_url( add_query_arg( array( 'post_status' => $status_name ), admin_url( 'admin.php?page=mcmsseo_tools&tool=bulk-editor' . $this->page_url ) ) ) . '"' . $class . '>' . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
			}
		}
		unset( $post_stati, $status, $status_name, $total, $class );

		$trashed_posts = $mcmsdb->get_var(
			"
					SELECT COUNT(ID) FROM {$subquery}
		...
		$status_links['trash'] = '<a href="' . esc_url( admin_url( 'admin.php?page=mcmsseo_tools&tool=bulk-editor&post_status=trash' . $this->page_url ) ) . '"' . $class . '>' . sprintf( _nx( 'Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', $trashed_posts, 'posts', 'mandarincms-seo' ), number_format_i18n( $trashed_posts ) ) . '</a>';
	...
	function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			...
			if ( is_array( $post_types ) && $post_types !== array() ) {
				...
				$options = '<option value="-1">' . __( 'Show All Post Types', 'mandarincms-seo' ) . '</option>';
				...
				printf(
					...
					__( 'Filter by post type', 'mandarincms-seo' )
				);
				echo sprintf( '<select name="post_type_filter" id="post-type-filter-%2$s">%1$s</select>', $options, $instance_type );
				submit_button( __( 'Filter', 'mandarincms-seo' ), 'button', false, false, array( 'id' => 'post-query-submit' ) );
				echo '</div>';
			}
		}
	}
	...
	protected function set_pagination( $total_items ) {
		...
		$per_page = $this->get_items_per_page( 'mcmsseo_posts_per_page', 10 );
	 ...
	 * Parsing the title.
	 *
	 * @param MCMS_Post $rec Post object.
	 *
	 * @return string
	 */
	protected function parse_page_title_column( $rec ) {

		$title = empty( $rec->post_title ) ? __( '(no title)', 'mandarincms-seo' ) : $rec->post_title;

		$return = sprintf( '<strong>%1$s</strong>', stripslashes( mcms_strip_all_tags( $title ) ) );
		...
		if ( $can_edit_post && 'trash' !== $rec->post_status ) {
			$actions['edit'] = sprintf(
				...
				esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;', 'mandarincms-seo' ), $title ) ),
				__( 'Edit', 'mandarincms-seo' )
			);
		}
		...
		if ( $post_type_object->public ) {
			if ( in_array( $rec->post_status, array( 'pending', 'draft', 'future' ) ) ) {
				if ( $can_edit_post ) {
					$actions['view'] = sprintf(
						...
						/* translators: %s: post title */
						esc_attr( sprintf( __( 'Preview &#8220;%s&#8221;', 'mandarincms-seo' ), $title ) ),
						__( 'Preview', 'mandarincms-seo' )
					);
				}
			}
			elseif ( 'trash' !== $rec->post_status ) {
				$actions['view'] = sprintf(
					...
					/* translators: %s: post title */
					esc_attr( sprintf( __( 'View &#8220;%s&#8221;', 'mandarincms-seo' ), $title ) ),
					__( 'View', 'mandarincms-seo' )
				);
			}
		}
	...
	protected function parse_column( $column_name, $rec ) {
		...
		switch ( $column_name ) {
			...
			case 'col_row_action':
				$column_value = sprintf( '<a href="#" role="button" class="mcmsseo-save" data-id="%1$s">Save</a> <span aria-hidden="true">|</span> <a href="#" role="button" class="mcmsseo-save-all">Save All</a>', $rec->ID );
				break;
		}
	...
	protected function parse_meta_data_field( $record_id, $attributes, $values = false ) {
		...
		$meta_key   = MCMSSEO_Meta::$meta_prefix . $this->target_db_field;
		...
		return sprintf( '<td %2$s id="mcmsseo-existing-%4$s-%3$s">%1$s</td>', $meta_value, $attributes, $record_id, $this->target_db_field );
	}
	...
	protected function get_meta_data_result( $post_ids ) {
		...
		$meta_data = $mcmsdb->get_results(
			"
				 	SELECT *
				 	FROM {$mcmsdb->postmeta}
				 	WHERE post_id IN({$post_ids}) && meta_key = '" . MCMSSEO_Meta::$meta_prefix . $this->target_db_field . "'
				"
		);
	...
	protected function merge_columns( $columns = array() ) {
		$columns = array_merge(
			array(
				'col_page_title'  => __( 'MCMS Page Title', 'mandarincms-seo' ),
				'col_post_type'   => __( 'Post Type', 'mandarincms-seo' ),
				'col_post_status' => __( 'Post Status', 'mandarincms-seo' ),
				'col_post_date'   => __( 'Publication date', 'mandarincms-seo' ),
				'col_page_slug'   => __( 'Page URL/Slug', 'mandarincms-seo' ),
			),
			$columns
		);

		$columns['col_row_action'] = __( 'Action', 'mandarincms-seo' );
