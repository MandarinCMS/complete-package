 ...
 */
class Ultimatum_Notification_Center {

	/** Option name to store notifications on */
	const STORAGE_KEY = 'ultimatum_notifications';

	/** @var \Ultimatum_Notification_Center The singleton instance of this object */
	private static $instance = null;

	/** @var $notifications Ultimatum_Notification[] */
	...
	private function __construct() {
		...
		add_action( 'mcms_ajax_ultimatum_get_notifications', array( $this, 'ajax_get_notifications' ) );

		add_action( 'mcmsseo_deactivate', array( $this, 'deactivate_hook' ) );
	 ...
	 * Singleton getter
	 *
	 * @return Ultimatum_Notification_Center
	 */
	...
	public static function ajax_dismiss_notification() {
		...
		if ( false === ( $notification instanceof Ultimatum_Notification ) ) {

			// Permit legacy.
			$notification = new Ultimatum_Notification( '', array(
				'id'            => $notification_id,
	 ...
	 *
	 * @param Ultimatum_Notification $notification The notification to check for dismissal.
	 * @param null|int           $user_id      User ID to check on.
	 *
	 * @return bool
	 */
	public static function is_notification_dismissed( Ultimatum_Notification $notification, $user_id = null ) {
	 ...
	 *
	 * @param string|Ultimatum_Notification $notification Notification to check dismissal of.
	 * @param string                    $meta_value   Value to set the meta value to if dismissed.
	 *
	 * @return bool True if dismissed.
	 */
	public static function maybe_dismiss_notification( Ultimatum_Notification $notification, $meta_value = 'seen' ) {
		...
		if ( false === mcms_verify_nonce( $user_nonce, $notification_id ) ) {
			return false;
		}
	 ...
	 * the message again.
	 *
	 * @param string|Ultimatum_Notification $notification Notification to clear the dismissal of.
	 *
	 * @return bool
	 */
	public function clear_dismissal( $notification ) {

		if ( $notification instanceof Ultimatum_Notification ) {
			$dismissal_key = $notification->get_dismissal_key();
		}
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification object instance.
	 */
	public function add_notification( Ultimatum_Notification $notification ) {
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification to remove.
	 * @param bool               $resolve Resolve as fixed.
	 */
	public function remove_notification( Ultimatum_Notification $notification, $resolve = true ) {
	 ...
	 *
	 * @return array|Ultimatum_Notification[] Sorted Notifications
	 */
	...
	/**
	 * Remove storage when the module is deactivated
	 */
	 ...
	 *
	 * @return array|Ultimatum_Notification[] Registered notifications.
	 */
	 ...
	 *
	 * @return array Ultimatum_Notification[] Notifications
	 */
	 ...
	 *
	 * @param Ultimatum_Notification $a Compare with B.
	 * @param Ultimatum_Notification $b Compare with A.
	 *
	 * @return int 1, 0 or -1 for sorting offset.
	 */
	private function sort_notifications( Ultimatum_Notification $a, Ultimatum_Notification $b ) {

		$a_type = $a->get_type();
		$b_type = $b->get_type();

		if ( $a_type === $b_type ) {
			return MCMSSEO_Utils::calc( $b->get_priority(), 'compare', $a->get_priority() );
		}
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification to dismiss.
	 * @param string             $meta_value   Value to save in the dismissal.
	 *
	 * @return bool
	 */
	private static function dismiss_notification( Ultimatum_Notification $notification, $meta_value = 'seen' ) {
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification to test for persistent.
	 *
	 * @since 3.2
	 *
	 * @return bool
	 */
	private function filter_persistent_notifications( Ultimatum_Notification $notification ) {
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification to check.
	 *
	 * @return bool
	 */
	private function filter_dismissed_notifications( Ultimatum_Notification $notification ) {
	 ...
	 *
	 * @param Ultimatum_Notification $notification Notification to convert.
	 *
	 * @since 3.2
	 *
	 * @return array
	 */
	private function notification_to_array( Ultimatum_Notification $notification ) {
	 ...
	 * @return bool
	 */
	private function filter_notification_current_user( Ultimatum_Notification $notification ) {
