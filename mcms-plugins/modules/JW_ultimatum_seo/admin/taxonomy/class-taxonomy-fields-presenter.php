...
/**
 * Class MCMSSEO_Taxonomy_Presenter
 */
class MCMSSEO_Taxonomy_Fields_Presenter {
	...
	public function __construct( $term ) {
		$this->tax_meta = MCMSSEO_Taxonomy_Meta::get_term_meta( (int) $term->term_id, $term->taxonomy );
	}
	...
	public function html( array $fields ) {
		...
		foreach ( $fields as $field_name => $field_configuration ) {
			$content .= $this->form_row( 'mcmsseo_' . $field_name, $field_configuration );
		}
		return $content;
	}
	...
	private function form_row( $field_name, array $field_configuration ) {
		...
		$help             = new MCMSSEO_Admin_Help_Panel( $field_name, $help_button_text, $help_content );
	...
	private function get_field( $field_type, $field_name, $field_value, array $options ) {
		...
		switch ( $field_type ) {
			...
			case 'snippetpreview':
				$field .= '<div id="mcmsseo_snippet" class="mcmsseosnippet"></div>';
				break;
			case 'pageanalysis' :
				$field .= '<div id="pageanalysis">';
				$field .= '<section class="ultimatum-section" id="mcmsseo-pageanalysis-section">';
				$field .= '<h3 class="ultimatum-section__heading ultimatum-section__heading-icon ultimatum-section__heading-icon-list">' . __( 'Analysis', 'mandarincms-seo' ) .'</h3>';
				$field .= '<div id="mcmsseo_analysis"></div>';
				$field .= '</section>';
				$field .= '</div>';
				break;
			case 'focuskeyword':
				$field .= '<div id="mcmsseofocuskeyword">';
				$field .= '<section class="ultimatum-section" id="mcmsseo-focuskeyword-section">';
				$field .= '<h3 class="ultimatum-section__heading ultimatum-section__heading-icon ultimatum-section__heading-icon-key">' . __( 'Focus keyword', 'mandarincms-seo' ) . '</h3>';
				$field .= '<label for="' . $field_name . '" class="screen-reader-text">' . __( 'Enter a focus keyword', 'mandarincms-seo' ) . '</label>';
			...
			case 'upload' :
				...
				$field .= '<input id="' . $field_name . '_button" class="mcmsseo_image_upload_button button" type="button" value="' . esc_attr__( 'Upload Image', 'mandarincms-seo' ) . '" />';
				break;
			case 'select' :
	 ...
	 * @param MCMSSEO_Admin_Help_Panel $help        The help panel to render in this row.
	 * @param string                 $field       The html for the field.
	 *
	 * @return string
	 */
	private function parse_row( $label, MCMSSEO_Admin_Help_Panel $help, $field ) {
		if ( $label !== '' || $help !== '' ) {
	 ...
	 * @param MCMSSEO_Admin_Help_Panel $help         The help button.
	 *
	 * @return string
	 */
	private function parse_section_row( $content, $esc_form_key, MCMSSEO_Admin_Help_Panel $help ) {
		$html = '<tr><td>';
		$html .= $content;
		$html .= '<div class="mcmsseo_hidden" id="help-ultimatum-'. $esc_form_key. '">' . $help->get_button_html() . $help->get_panel_html() . '</div>';
		$html .= '</td></tr>';
		return $html;
	}
}
