...
class MCMSSEO_Taxonomy {
	 ...
	 *
	 * @param stdClass|MCMS_Term $term Term to show the edit boxes for.
	 */
	public function term_metabox( $term ) {
		$metabox = new MCMSSEO_Taxonomy_Metabox( $this->taxonomy, $term );
		$metabox->display();
	}
	...
	public function admin_enqueue_scripts() {
		...
		if (
			...
			mcms_enqueue_media(); // Enqueue files needed for upload functionality.
			...
			mcms_localize_script( MCMSSEO_Admin_Asset_Manager::PREFIX . 'term-scraper', 'mcmsseoTermScraperL10n', $this->localize_term_scraper_script() );
			...
			mcms_localize_script( MCMSSEO_Admin_Asset_Manager::PREFIX . 'admin-media', 'mcmsseoMediaL10n', array(
	...
	public function update_term( $term_id, $tt_id, $taxonomy ) {
		...
		$new_meta_data = array();
		foreach ( MCMSSEO_Taxonomy_Meta::$defaults_per_term as $key => $default ) {
		...
		// Saving the values.
		MCMSSEO_Taxonomy_Meta::set_values( $term_id, $taxonomy, $new_meta_data );
	}
	...
	public function is_meta_value_disabled( $key ) {
		if ( 'mcmsseo_linkdex' === $key && ! $this->analysis_seo->is_enabled() ) {
			return true;
		}

		if ( 'mcmsseo_content_score' === $key && ! $this->analysis_readability->is_enabled() ) {
			return true;
		}
	...
	public function custom_category_descriptions_allow_html() {
		...
		foreach ( $filters as $filter ) {
			remove_filter( $filter, 'mcms_filter_kses' );
		}
		remove_filter( 'term_description', 'mcms_kses_data' );
	}

	/**
	 * Output the MandarinCMS editor.
	 */
	...
	public function localize_term_scraper_script() {
		...
		$term_formatter = new MCMSSEO_Metabox_Formatter(
			new MCMSSEO_Term_Metabox_Formatter( $taxonomy, $term, MCMSSEO_Options::get_option( 'mcmsseo_titles' ) )
		);
	...
	private function get_replace_vars() {
		...
		foreach ( $vars_to_cache as $var ) {
			$cached_replacement_vars[ $var ] = mcmsseo_replace_vars( '%%' . $var . '%%', $term );
		}
	 ...
	 * Needs a hook that runs before the description field. Prior to MCMS version 4.5 we need to use edit_form as
	 * term_edit_form_top was introduced in MCMS 4.5. This can be removed after <4.5 is no longer supported.
	 *
	 * @return {void}
	 */
	private function insert_description_field_editor() {
		if ( version_compare( $GLOBALS['mcms_version'], '4.5', '<' ) ) {
	...
	public static function get_title_template( $term ) {
		_deprecated_function( 'MCMSSEO_Taxonomy::get_title_template', 'MCMSSEO 3.2', 'MCMSSEO_Term_Scraper::get_title_template' );
	...
	public static function get_metadesc_template( $term ) {
		_deprecated_function( 'MCMSSEO_Taxonomy::get_metadesc_template', 'MCMSSEO 3.2', 'MCMSSEO_Term_Scraper::get_metadesc_template' );
	...
	public function translate_meta_options() {
		_deprecated_function( 'MCMSSEO_Taxonomy::translate_meta_options', 'MCMSSEO 3.2', 'MCMSSEO_Taxonomy_Settings_Fields::translate_meta_options' );
	}
}
