...
/**
 * This class parses all the values for the social tab in the Ultimatum SEO settings metabox
 */
class MCMSSEO_Taxonomy_Social_Fields extends MCMSSEO_Taxonomy_Fields {

	/**
	 * Setting the class properties
	 *
	 * @param stdClass|MCMS_Term $term    The current taxonomy.
	...
	public function get_by_network( $network ) {
		...
		return array(
			$settings['network'] . '-title'       => $this->get_field_config(
				...
				sprintf( __( '%s Title', 'mandarincms-seo' ), $settings['label'] ),
				/* translators: %1$s expands to the social network name */
				sprintf( esc_html__( 'If you don\'t want to use the title for sharing on %1$s but instead want another title there, write it here.', 'mandarincms-seo' ), $settings['label'] ),
				'text',
				array( 'class' => 'large-text' )
			),
			$settings['network'] . '-description' => $this->get_field_config(
				/* translators: %s expands to the social network name */
				sprintf( __( '%s Description', 'mandarincms-seo' ), $settings['label'] ),
				/* translators: %1$s expands to the social network name */
				sprintf( esc_html__( 'If you don\'t want to use the meta description for sharing on %1$s but want another description there, write it here.', 'mandarincms-seo' ), $settings['label'] ),
				'textarea'
			),
			$settings['network'] . '-image'       => $this->get_field_config(
				/* translators: %s expands to the social network name */
				sprintf( __( '%s Image', 'mandarincms-seo' ), $settings['label'] ),
				/* translators: %1$s expands to the social network name */
				sprintf( esc_html__( 'If you want to use an image for sharing on %1$s, you can upload / choose an image or add the image URL here.', 'mandarincms-seo' ), $settings['label'] ) . '<br />' .
				/* translators: %1$s expands to the social network name, %2$s expands to the image size */
				sprintf( __( 'The recommended image size for %1$s is %2$s pixels.', 'mandarincms-seo' ), $settings['label'], $settings['size'] ),
				'upload'
			),
		);
	}
	...
	private function get_social_networks() {
		$social_networks = array(
			...
			'opengraph' => $this->social_network( 'opengraph', __( 'Facebook', 'mandarincms-seo' ), sprintf(
				/* translators: %1$s expands to the image recommended width, %2$s to its height. */
				__( '%1$s by %2$s', 'mandarincms-seo' ), '1200', '630'
			) ),
			'twitter'   => $this->social_network( 'twitter', __( 'Twitter', 'mandarincms-seo' ), sprintf(
				/* translators: %1$s expands to the image recommended width, %2$s to its height. */
				__( '%1$s by %2$s', 'mandarincms-seo' ), '1024', '512'
			) ),
		);
