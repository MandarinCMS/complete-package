 ...
 */
class MCMSSEO_Taxonomy_Metabox {
	...
	/**
	 * @var MCMSSEO_Taxonomy_Fields_Presenter
	 */
	...
	public function __construct( $taxonomy, $term ) {
		...
		$this->taxonomy_tab_content = new MCMSSEO_Taxonomy_Fields_Presenter( $this->term );
	...
	/**
	 * Shows the Ultimatum SEO metabox for the term.
	 */
	public function display() {
		...
		printf( '<div id="mcmsseo_meta" class="postbox mcmsseo-taxonomy-metabox-postbox"><h2><span>%1$s</span></h2>', $product_title );

		echo '<div class="inside">';

		$helpcenter_tab = new MCMSSEO_Option_Tab( 'tax-metabox', 'Meta box',
			array( 'video_url' => 'https://jiiworks.net/metabox-taxonomy-screencast' ) );

		$helpcenter = new MCMSSEO_Help_Center( 'tax-metabox', $helpcenter_tab );
	...
	private function get_content_meta_section() {
		$taxonomy_content_fields = new MCMSSEO_Taxonomy_Content_Fields( $this->term );
	...
	private function get_settings_meta_section() {
		$taxonomy_settings_fields = new MCMSSEO_Taxonomy_Settings_Fields( $this->term );
		$content = $this->taxonomy_tab_content->html( $taxonomy_settings_fields->get() );

		$tab = new MCMSSEO_Metabox_Form_Tab(
			'settings',
			$content,
			__( 'Settings', 'mandarincms-seo' ),
			array(
				'single' => true,
			)
		);
		...
		return new MCMSSEO_Metabox_Tab_Section(
			'settings',
			'<span class="screen-reader-text">' . __( 'Settings', 'mandarincms-seo' ) . '</span><span class="dashicons dashicons-admin-generic"></span>',
			array( $tab ),
			array(
				'link_aria_label' => __( 'Settings', 'mandarincms-seo' ),
				'link_class'      => 'ultimatum-tooltip ultimatum-tooltip-e',
			)
		);
	}
	...
	private function get_social_meta_section() {
		...
		$taxonomy_social_fields = new MCMSSEO_Taxonomy_Social_Fields( $this->term );
...
SVG;
	...
	public function template_generic_tab() {
		...
		if ( ! mcms_script_is( MCMSSEO_Admin_Asset_Manager::PREFIX . 'term-scraper' ) ) {
			return;
		}
