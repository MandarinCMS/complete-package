 ...
 */
class MCMSSEO_Taxonomy_Columns {
	...
	/**
	 * MCMSSEO_Taxonomy_Columns constructor.
	 */
	...
	public function add_columns( array $columns ) {
		...
		foreach ( $columns as $column_name => $column_value ) {
			...
			if ( $column_name === 'description' && $this->analysis_seo->is_enabled() ) {
				$new_columns['mcmsseo_score'] = __( 'SEO', 'mandarincms-seo' );
			}

			if ( $column_name === 'description' && $this->analysis_readability->is_enabled() ) {
				$new_columns['mcmsseo_score_readability'] = __( 'Readability', 'mandarincms-seo' );
			}
		}
	...
	public function parse_column( $content, $column_name, $term_id ) {
		...
		switch ( $column_name ) {
			...
			case 'mcmsseo_score_readability':
	...
	private function get_score_value( $term_id ) {
		...
		if ( ! $this->is_indexable( $term ) ) {
			return $this->create_score_icon(
				new MCMSSEO_Rank( MCMSSEO_Rank::NO_INDEX ),
				__( 'Term is set to noindex.', 'mandarincms-seo' )
			);
		}

		// When there is a focus key word.
		if ( $focus_keyword = $this->get_focus_keyword( $term ) ) {
			$score = (int) MCMSSEO_Taxonomy_Meta::get_term_meta( $term_id, $this->taxonomy, 'linkdex' );
		...
		// Default icon.
		return $this->create_score_icon(
			new MCMSSEO_Rank( MCMSSEO_Rank::NO_FOCUS ),
			__( 'Focus keyword not set.', 'mandarincms-seo' )
		);
	}
	...
	private function get_score_readability_value( $term_id ) {
		$score = (int) MCMSSEO_Taxonomy_Meta::get_term_meta( $term_id, $this->taxonomy, 'content_score' );
		$rank = MCMSSEO_Rank::from_numeric_score( $score );
	...
	private function is_indexable( $term ) {
		...
		$no_index = MCMSSEO_Taxonomy_Meta::get_term_meta( $term->term_id, $this->taxonomy, 'noindex' );
	 ...
	 *
	 * @param stdClass|MCMS_Term $term The current term.
	 *
	 * @return string
	 */
	private function get_focus_keyword( $term ) {
		if ( $focus_keyword = MCMSSEO_Taxonomy_Meta::get_term_meta( 'focuskw', $term->term_id, $term->taxonomy ) ) {
