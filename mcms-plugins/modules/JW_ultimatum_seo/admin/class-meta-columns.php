...
class MCMSSEO_Meta_Columns {
	...
	/**
	 * Adds the column headings for the SEO module for edit posts / pages overview
	 *
	...
	public function column_heading( $columns ) {
		...
		if ( $this->analysis_seo->is_enabled() ) {
			$added_columns['mcmsseo-score'] = __( 'SEO', 'mandarincms-seo' );
		}
		if ( $this->analysis_readability->is_enabled() ) {
			$added_columns['mcmsseo-score-readability'] = __( 'Readability', 'mandarincms-seo' );
		}
		$added_columns['mcmsseo-title']    = __( 'SEO Title', 'mandarincms-seo' );
		$added_columns['mcmsseo-metadesc'] = __( 'Meta Desc.', 'mandarincms-seo' );

		if ( $this->analysis_seo->is_enabled() ) {
			$added_columns['mcmsseo-focuskw']  = __( 'Focus KW', 'mandarincms-seo' );
		}
	...
	public function column_content( $column_name, $post_id ) {
		...
		switch ( $column_name ) {
			case 'mcmsseo-score' :
				...
				break;
			case 'mcmsseo-score-readability':
				echo $this->parse_column_score_readability( $post_id );
				break;
			case 'mcmsseo-title' :
				echo esc_html( apply_filters( 'mcmsseo_title', mcmsseo_replace_vars( $this->page_title( $post_id ), get_post( $post_id, ARRAY_A ) ) ) );
				break;
			case 'mcmsseo-metadesc' :
				$metadesc_val = apply_filters( 'mcmsseo_metadesc', mcmsseo_replace_vars( MCMSSEO_Meta::get_value( 'metadesc', $post_id ), get_post( $post_id, ARRAY_A ) ) );
				$metadesc = ( '' === $metadesc_val ) ? '<span aria-hidden="true">&#8212;</span><span class="screen-reader-text">' . __( 'Meta description not set.', 'mandarincms-seo' ) . '</span>' : esc_html( $metadesc_val );
				echo $metadesc;
				break;
			case 'mcmsseo-focuskw' :
				$focuskw_val = MCMSSEO_Meta::get_value( 'focuskw', $post_id );
				$focuskw = ( '' === $focuskw_val ) ? '<span aria-hidden="true">&#8212;</span><span class="screen-reader-text">' . __( 'Focus keyword not set.', 'mandarincms-seo' ) . '</span>' : esc_html( $focuskw_val );
				echo $focuskw;
				break;
		}
	}
	...
	public function column_sort( $columns ) {
		...
		$columns['mcmsseo-metadesc'] = 'mcmsseo-metadesc';

		if ( $this->analysis_seo->is_enabled() ) {
			$columns['mcmsseo-focuskw'] = 'mcmsseo-focuskw';
		}
	...
	public function column_hidden( $result, $option, $user ) {
		...
		if ( ! $user->has_prop( $prefix . $option ) && ! $user->has_prop( $option ) ) {
			...
			array_push( $result, 'mcmsseo-title', 'mcmsseo-metadesc' );

			if ( $this->analysis_seo->is_enabled() ) {
				array_push( $result, 'mcmsseo-focuskw' );
			}
		}
	...
	public function posts_filter_dropdown() {
		...
		echo '
			<label class="screen-reader-text" for="mcmsseo-filter">' . __( 'Filter by SEO Score', 'mandarincms-seo' ) . '</label>
			<select name="seo_filter" id="mcmsseo-filter">
				<option value="">', __( 'All SEO Scores', 'mandarincms-seo' ), '</option>';
	...
	public function column_sort_orderby( $vars ) {
		if ( $seo_filter = filter_input( INPUT_GET, 'seo_filter' ) ) {
			$rank = new MCMSSEO_Rank( $seo_filter );

			if ( MCMSSEO_Rank::NO_FOCUS === $seo_filter || MCMSSEO_Rank::NO_INDEX === $seo_filter ) {
		...
		if ( $seo_kw_filter = filter_input( INPUT_GET, 'seo_kw_filter' ) ) {
			$vars = array_merge(
				$vars, array(
					...
					'meta_key'   => MCMSSEO_Meta::$meta_prefix . 'focuskw',
	...
	private function filter_scored( $low, $high ) {
		...
		return array(
			'meta_query' => array(
				'relation' => 'AND',
				array(
					'key'     => MCMSSEO_Meta::$meta_prefix . 'linkdex',
				...
				array(
					'relation' => 'OR',
					array(
						'key'     => MCMSSEO_Meta::$meta_prefix . 'meta-robots-noindex',
						'compare' => 'NOT EXISTS',
					),
					array(
						'key'     => MCMSSEO_Meta::$meta_prefix . 'meta-robots-noindex',
	...
	private function filter_order_by( $order_by ) {
		switch ( $order_by ) {
			case 'mcmsseo-metadesc' :
				return  array(
					'meta_key' => MCMSSEO_Meta::$meta_prefix . 'metadesc',
					'orderby'  => 'meta_value',
				);
				break;
			case 'mcmsseo-focuskw' :
	...
	private function parse_column_score( $post_id ) {
		if ( '1' === MCMSSEO_Meta::get_value( 'meta-robots-noindex', $post_id ) ) {
			$rank  = new MCMSSEO_Rank( MCMSSEO_Rank::NO_INDEX );
			$title = __( 'Post is set to noindex.', 'mandarincms-seo' );
			MCMSSEO_Meta::set_value( 'linkdex', 0, $post_id );
		}
		elseif ( '' === MCMSSEO_Meta::get_value( 'focuskw', $post_id ) ) {
			$rank  = new MCMSSEO_Rank( MCMSSEO_Rank::NO_FOCUS );
			$title = __( 'Focus keyword not set.', 'mandarincms-seo' );
		}
		else {
			$score = (int) MCMSSEO_Meta::get_value( 'linkdex', $post_id );
			$rank  = MCMSSEO_Rank::from_numeric_score( $score );
	...
	private function parse_column_score_readability( $post_id ) {
		$score = (int) MCMSSEO_Meta::get_value( 'content_score', $post_id );
		$rank = MCMSSEO_Rank::from_numeric_score( $score );
	...
	private function page_title( $post_id ) {
		$fixed_title = MCMSSEO_Meta::get_value( 'title', $post_id );
		if ( $fixed_title !== '' ) {
			return $fixed_title;
		}
		...
		if ( is_object( $post ) && ( isset( $options[ 'title-' . $post->post_type ] ) && $options[ 'title-' . $post->post_type ] !== '' ) ) {
			...
			return mcmsseo_replace_vars( $title_template, $post );
		}

		return mcmsseo_replace_vars( '%%title%%', $post );
	}

	/**
	 * @param MCMSSEO_Rank $rank The rank this indicator should have.
	...
	private function render_score_indicator( $rank, $title = '' ) {
		...
		return '<div aria-hidden="true" title="' . esc_attr( $title ) . '" class="mcmsseo-score-icon ' . esc_attr( $rank->get_css_class() ) . '"></div><span class="screen-reader-text">' . $title . '</span>';
	}
	...
	public function seo_score_posts_where( $where ) {
		...
		_deprecated_function( 'MCMSSEO_Metabox_Columns::seo_score_posts_where', '3.5' );

		global $mcmsdb;

		/* Find the two mutually exclusive noindex clauses which should be changed from AND to OR relation */
		$find = '`([\s]+AND[\s]+)((?:' . $mcmsdb->prefix . 'postmeta|mt[0-9]|mt1)\.post_id IS NULL[\s]+)AND([\s]+\([\s]*(?:' . $mcmsdb->prefix . 'postmeta|mt[0-9])\.meta_key = \'' . MCMSSEO_Meta::$meta_prefix . 'meta-robots-noindex\' AND CAST\([^\)]+\)[^\)]+\))`';
