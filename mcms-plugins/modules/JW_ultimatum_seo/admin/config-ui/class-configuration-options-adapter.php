...
/**
 * Class MCMSSEO_Configuration_Options_Adapter
 *
 * Convert Configuration settings to MCMSSEO Options
 *
 * @since 3.6
 */
class MCMSSEO_Configuration_Options_Adapter {

	const OPTION_TYPE_WORDPRESS = 'mandarincms';
	const OPTION_TYPE_YOAST = 'ultimatum';
	...
	/**
	 * Add a lookup for a MandarinCMS native option
	 *
	 ...
	 */
	public function add_mandarincms_lookup( $class_name, $option ) {

		if ( ! is_string( $option ) ) {
			throw new InvalidArgumentException( 'MandarinCMS option must be a string.' );
		}
	...
	/**
	 * Add a lookup for a Ultimatum option
	 *
	 ...
	 * @throws InvalidArgumentException Thrown when invalid input is provided.
	 */
	public function add_ultimatum_lookup( $class_name, $option, $key ) {

		$test = MCMSSEO_Options::get_option( $option );
		if ( is_null( $test ) ) {
			/* translators: %1$s resolves to the option name passed to the lookup registration */
			throw new InvalidArgumentException( sprintf( __( 'Ultimatum option %1$s not found.', 'mandarincms-seo' ), $option ) );
		}
	 ...
	 * @return mixed
	 */
	public function get( MCMSSEO_Config_Field $field ) {
		...
		switch ( $type ) {
			...
			case self::OPTION_TYPE_YOAST:
				$group = MCMSSEO_Options::get_option( $option[0] );
	 ...
	 *
	 * @param MCMSSEO_Config_Field $field Field to use for lookup.
	 * @param mixed              $value Value to save to the lookup of the field.
	 *
	 * @return bool
	 */
	public function set( MCMSSEO_Config_Field $field, $value ) {
		$identifier = $field->get_identifier();
		...
		switch ( $type ) {
			...
			case self::OPTION_TYPE_YOAST:
				...
				$saved = MCMSSEO_Options::get_option( $option[0] );
