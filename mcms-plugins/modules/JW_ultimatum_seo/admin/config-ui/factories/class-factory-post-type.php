...
/**
 * Class MCMSSEO_Config_Factory_Post_Type
 */
class MCMSSEO_Config_Factory_Post_Type {

	/** @var MCMSSEO_Config_Field_Choice_Post_Type[] List of fields */
	protected static $fields = array();

	/**
	 * @return MCMSSEO_Config_Field_Choice_Post_Type[] List of fields.
	 */
		...
		if ( empty( self::$fields ) ) {
			...
			if ( ! empty( $post_types ) ) {
				foreach ( $post_types as $post_type => $post_type_object ) {
					...
					$field = new MCMSSEO_Config_Field_Choice_Post_Type( $post_type, $label );
	 ...
	 * @param MCMSSEO_Config_Field $field     Field that corresponds to the post type.
	 */
	private function add_custom_properties( $post_type, $field ) {
		if ( 'attachment' === $post_type ) {
			$field->set_property( 'explanation', __( 'MandarinCMS automatically generates an URL for each media item in the library. Enabling this will allow for google to index the generated URL.', 'mandarincms-seo' ) );
		}
	}
