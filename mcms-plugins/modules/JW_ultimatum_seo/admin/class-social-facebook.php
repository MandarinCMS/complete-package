 ...
 */
class Ultimatum_Social_Facebook {
	...
	/**
	 * @var Ultimatum_Social_Facebook_Form
	 */
	private $form;
	...
	public function __construct() {
		$this->options = get_option( 'mcmsseo_social' );

		$this->get_listener();

		$this->form = new Ultimatum_Social_Facebook_Form();
	}
	...
	private function get_response_body( $type ) {
		switch ( $type ) {
			case 'not_present':
				$return = "<p class='notice-error notice'><span>" . __( 'Please make sure both fields are filled.', 'mandarincms-seo' ) . '</span></p>';
				break;
			case 'invalid_format':
				$return = "<p class='notice-error notice'><span>" . __( 'Your input contains invalid characters. Please make sure both fields are filled in correctly.', 'mandarincms-seo' ) . '</span></p>';
				break;
			case 'already_exists':
				$return = "<p class='notice-error notice'><span>" . __( 'This Facebook user has already been added as an admin.', 'mandarincms-seo' ) . '</span></p>';
				break;
			default:
				$return = '';
				break;
	...
	private function delete_admin( $delfbadmin ) {
		...
		if ( isset( $this->options['fb_admins'][ $admin_id ] ) ) {
			...
			$this->save_options();
			$this->success_notice( sprintf( __( 'Successfully removed admin %s', 'mandarincms-seo' ), $fbadmin ) );
	...
	private function clear_all() {
		...
		$this->options['fb_admins'] = MCMSSEO_Options::get_default( 'mcmsseo_social', 'fb_admins' );

		$this->save_options();
		$this->success_notice( __( 'Successfully cleared all Facebook Data', 'mandarincms-seo' ) );
	...
	private function success_notice( $notice_text ) {
		add_settings_error( 'ultimatum_mcmsseo_social_options', 'success', $notice_text, 'updated' );
	}
	...
	private function verify_nonce( $nonce_name ) {
		if ( mcms_verify_nonce( filter_input( INPUT_GET, 'nonce' ), $nonce_name ) != 1 ) {
	...
	private function save_options() {
		update_option( 'mcmsseo_social', $this->options );
	}
}
 ...
 */
class Ultimatum_Social_Facebook_Form {
	 ...
	 */
	private $admin_url = 'admin.php?page=mcmsseo_social';
	...
	private function form_head() {
		echo '<h2>' . esc_html__( 'Facebook Insights and Admins', 'mandarincms-seo' ) . '</h2>';
		echo '<p>', sprintf(
			/* translators: %1$s and %2$s expand to a link to Facebook Insights */
			esc_html__( 'To be able to access %1$sFacebook Insights%2$s for your site, you need to specify a Facebook Admin. This can be a user. If you have an app for your site, you could use that as well.', 'mandarincms-seo' ),
			'<a target="_blank" href="https://www.facebook.com/insights">',
			'</a>'
		);
		echo ' ';
		/* translators: %1$s and %2$s expand to a link to the Ultimatum Knowledge Base */
		printf( __( 'More info can be found %1$son our knowledge base%2$s.', 'mandarincms-seo' ), '<a target="_blank" href="http://kb.jiiworks.net/article/254-gaining-access-to-facebook-insights">', '</a>' );
		echo '</p>';
	...
	private function form_thickbox() {
		...
		echo "<div class='form-wrap mcmsseo_content_wrapper mcmsseo-add-fb-admin-form-wrap'>";
		echo '<p>';
		/* translators: %1$s and %2$s expand to a link to Facebook Insights */
		printf( __( 'To be able to access %1$sFacebook Insights%2$s, you need to add a user here. The name is used for reference only, the ID is used for verification.', 'mandarincms-seo' ), '<a target="_blank" href="https://www.facebook.com/insights">', '</a>' );
		echo '</p>';
		echo '<p>';
		/* translators: %1$s and %2$s expand to a link to the Ultimatum Knowledge Base */
		printf( __( 'If you don\'t know where to find the needed ID, see %1$sthis knowledge base article%2$s.', 'mandarincms-seo' ), '<a target="_blank" href="http://kb.jiiworks.net/article/254-gaining-access-to-facebook-insights">', '</a>' );
		echo '</p>';
		echo '<div class="form-field form-required">';
		echo '<label for="fb_admin_name">' . __( 'Admin\'s name:', 'mandarincms-seo' ) . '</label>';
		echo '<input type="text" id="fb_admin_name" name="fb_admin_name" value="" maxlength="255" />';
		echo '</div>';
		echo '<div class="form-field form-required">';
		echo '<label for="fb_admin_id">' . __( 'Admin\'s Facebook user ID:', 'mandarincms-seo' ) . '</label>';
		echo '<input type="text" id="fb_admin_id" name="fb_admin_id" value="" maxlength="255"  />';
		echo '</div>';
		echo "<p class='submit'>";
		echo '<input type="hidden" name="fb_admin_nonce" value="' . mcms_create_nonce( 'mcmsseo_fb_admin_nonce' ) . '" />';
		echo '<input type="submit" value="' . __( 'Add Facebook admin', 'mandarincms-seo' ) . '" class="button button-primary" onclick="javascript:mcmsseo_add_fb_admin();" />';
		echo '</p>';
		echo '</div>';
		echo '</div>';
	...
	private function manage_user_admin() {
		$button_text = __( 'Add Facebook admin', 'mandarincms-seo' );
		$nonce       = false;
		$class_attr  = ' class="hidden"';

		if ( is_array( $this->options['fb_admins'] ) && $this->options['fb_admins'] !== array() ) {
			$nonce       = $this->get_delete_nonce();
			$button_text = __( 'Add Another Facebook Admin', 'mandarincms-seo' );
			$class_attr  = '';
		}

		echo "<div id='connected_fb_admins'{$class_attr}>";
		echo '<p>' . __( 'Currently connected Facebook admins:', 'mandarincms-seo' ) . '</p>';
	...
	private function manage_app_as_admin() {
		...
		Ultimatum_Form::get_instance()->textinput( 'fbadminapp', __( 'Facebook App ID', 'mandarincms-seo' ) );
			...
			$this->add_button(
				array(
					'url'   => add_query_arg( array(
						'nonce'      => mcms_create_nonce( 'fbclearall' ),
						'fbclearall' => 'true',
					), admin_url( $this->admin_url . '#top#facebook' ) ),
					'value' => __( 'Clear all Facebook Data', 'mandarincms-seo' ),
				)
			);
		}
	...
	private function get_delete_nonce() {
		return mcms_create_nonce( 'delfbadmin' );
	}
}
