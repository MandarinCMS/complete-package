...
class Ultimatum_Notification {
	 ...
	 * -  mcmsseo_page_only: Only display on mcmsseo page or on every page.
	 *
	...
	public function get_nonce() {
		if ( $this->options['id'] && empty( $this->options['nonce'] ) ) {
			$this->options['nonce'] = mcms_create_nonce( $this->options['id'] );
		}
	...
	public function match_capabilities() {
		 ...
		 * @param Ultimatum_Notification $notification The notification object.
		 *
		 * @return array of capabilities or empty for no restrictions.
		 */
		$capabilities = apply_filters( 'mcmsseo_notification_capabilities', $this->options['capabilities'], $this );
		 ...
		 */
		$capability_check = apply_filters( 'mcmsseo_notification_capability_check', $this->options['capability_check'], $this );
	...
	public function __toString() {
		...
		// Maintain MandarinCMS visualisation of alerts when they are not persistent.
		if ( ! $this->is_persistent() ) {
			$classes[] = 'notice';
		...
		return '<div ' . implode( ' ', $attributes ) . '>' . mcmsautop( $this->message ) . '</div>' . PHP_EOL;
	}
	...
	public function get_json() {
		...
		return mcms_json_encode( $this->options['data_json'] );
	}
	...
	private function normalize_options( $options ) {
		$options = mcms_parse_args( $options, $this->defaults );
