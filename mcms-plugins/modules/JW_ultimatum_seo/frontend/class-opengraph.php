...
class MCMSSEO_OpenGraph {
	...
	public function __construct() {
		$this->options = MCMSSEO_Options::get_option( 'mcmsseo_social' );
		...
		if ( isset( $GLOBALS['fb_ver'] ) || class_exists( 'Facebook_Loader', false ) ) {
			...
			add_action( 'mcmsseo_opengraph', array( $this, 'locale' ), 1 );
			add_action( 'mcmsseo_opengraph', array( $this, 'type' ), 5 );
			add_action( 'mcmsseo_opengraph', array( $this, 'og_title' ), 10 );
			add_action( 'mcmsseo_opengraph', array( $this, 'site_owner' ), 20 );
			add_action( 'mcmsseo_opengraph', array( $this, 'description' ), 11 );
			add_action( 'mcmsseo_opengraph', array( $this, 'url' ), 12 );
			add_action( 'mcmsseo_opengraph', array( $this, 'site_name' ), 13 );
			add_action( 'mcmsseo_opengraph', array( $this, 'website_facebook' ), 14 );
			if ( is_singular() && ! is_front_page() ) {
				add_action( 'mcmsseo_opengraph', array( $this, 'article_author_facebook' ), 15 );
				add_action( 'mcmsseo_opengraph', array( $this, 'tags' ), 16 );
				add_action( 'mcmsseo_opengraph', array( $this, 'category' ), 17 );
				add_action( 'mcmsseo_opengraph', array( $this, 'publish_date' ), 19 );
			}

			add_action( 'mcmsseo_opengraph', array( $this, 'image' ), 30 );
		}
		add_filter( 'jetpack_enable_open_graph', '__return_false' );
		add_action( 'mcmsseo_head', array( $this, 'opengraph' ), 30 );
	}
	...
	public function opengraph() {
		mcms_reset_query();
		/**
		 * Action: 'mcmsseo_opengraph' - Hook to add all Facebook OpenGraph output to so they're close together.
		 */
		do_action( 'mcmsseo_opengraph' );
	}
	...
	public function og_tag( $property, $content ) {
		...
		/**
		 * Filter: 'mcmsseo_og_' . $og_property - Allow developers to change the content of specific OG meta tags.
		 *
		 * @api string $content The content of the property
		 */
		$content = apply_filters( 'mcmsseo_og_' . $og_property, $content );
		if ( empty( $content ) ) {
			return false;
		}
	...
	/**
	 * Filter the Facebook modules metadata
	 *
	...
	public function facebook_filter( $meta_tags ) {
		...
		// Filter the locale too because the Facebook module locale code is not as good as ours.
	...
	public function article_author_facebook() {
		...
		/**
		 * Filter: 'mcmsseo_opengraph_author_facebook' - Allow developers to filter the Ultimatum SEO post authors facebook profile URL
		 *
		 * @api bool|string $unsigned The Facebook author URL, return false to disable
		 */
		$facebook = apply_filters( 'mcmsseo_opengraph_author_facebook', get_the_author_meta( 'facebook', $GLOBALS['post']->post_author ) );
	...
	public function site_owner() {
		if ( isset( $this->options['fbadminapp'] ) && $this->options['fbadminapp'] != 0 ) {
			...
			/**
			 * Filter: 'mcmsseo_opengraph_admin' - Allow developer to filter the fb:admins string put out by Ultimatum SEO
			 *
			 * @api string $adminstr The admin string
			 */
			$adminstr = apply_filters( 'mcmsseo_opengraph_admin', $adminstr );
	...
	public function og_title( $echo = true ) {
		...
		$frontend      = MCMSSEO_Frontend::get_instance();
		...
		if ( is_singular() || $is_posts_page ) {
			...
			$title   = MCMSSEO_Meta::get_value( 'opengraph-title', $post_id );

			if ( $title === '' ) {
				$title = $frontend->title( '' );
			}
			else {
				// Replace Ultimatum SEO Variables.
				$title = mcmsseo_replace_vars( $title, $post );
			}
		}
		...
		elseif ( is_category() || is_tax() || is_tag() ) {
			$title = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'opengraph-title' );
			if ( $title === '' ) {
				$title = $frontend->title( '' );
			}
			else {
				// Replace Ultimatum SEO Variables.
				$title = mcmsseo_replace_vars( $title, $GLOBALS['mcms_query']->get_queried_object() );
			}
		}
		else {
			$title = $frontend->title( '' );
		...
		/**
		 * Filter: 'mcmsseo_opengraph_title' - Allow changing the title specifically for OpenGraph
		 *
		 * @api string $unsigned The title string
		 */
		$title = trim( apply_filters( 'mcmsseo_opengraph_title', $title ) );
	...
	public function url() {
		/**
		 * Filter: 'mcmsseo_opengraph_url' - Allow changing the OpenGraph URL
		 *
		 * @api string $unsigned Canonical URL
		 */
		$url = apply_filters( 'mcmsseo_opengraph_url', MCMSSEO_Frontend::get_instance()->canonical( false ) );
	...
	public function locale( $echo = true ) {
		/**
		 * Filter: 'mcmsseo_locale' - Allow changing the locale output
		 *
		 * @api string $unsigned Locale string
		 */
		$locale = apply_filters( 'mcmsseo_locale', get_locale() );

		// Catch some weird locales served out by MCMS that are not easily doubled up.
		$fix_locales = array(
			'ca' => 'ca_ES',
			'en' => 'en_US',
			'el' => 'el_GR',
	...
	public function type( $echo = true ) {
		...
		elseif ( is_singular() ) {
			...
			// This'll usually only be changed by modules right now.
			$type = MCMSSEO_Meta::get_value( 'og_type' );
		...
		/**
		 * Filter: 'mcmsseo_opengraph_type' - Allow changing the OpenGraph type of the page
		 *
		 * @api string $type The OpenGraph type string.
		 */
		$type = apply_filters( 'mcmsseo_opengraph_type', $type );
	...
	/**
	 * Create new MCMSSEO_OpenGraph_Image class and get the images to set the og:image
	 *
	 * @param string|boolean $image Optional image URL.
	 */
	public function image( $image = false ) {
		$opengraph_images = new MCMSSEO_OpenGraph_Image( $this->options, $image );
	...
	/**
	 * Fallback method for modules using image_output
	 *
	...
	public function description( $echo = true ) {
		...
		if ( is_front_page() ) {
			if ( isset( $this->options['og_frontpage_desc'] ) && $this->options['og_frontpage_desc'] !== '' ) {
				$ogdesc = mcmsseo_replace_vars( $this->options['og_frontpage_desc'], null );
			}
			else {
		...
		if ( is_singular() || $is_posts_page ) {
			$post_id = ( $is_posts_page ) ? get_option( 'page_for_posts' ) : get_the_ID();
			$post    = get_post( $post_id );
			$ogdesc  = MCMSSEO_Meta::get_value( 'opengraph-description', $post_id );

			// Replace Ultimatum SEO Variables.
			$ogdesc = mcmsseo_replace_vars( $ogdesc, $post );
		...
		if ( is_category() || is_tag() || is_tax() ) {
			$ogdesc = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'opengraph-description' );
			if ( $ogdesc === '' ) {
				$ogdesc = $frontend->metadesc( false );
			}
			...
			if ( $ogdesc === '' ) {
				$ogdesc = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'desc' );
			}
		}
		...
		/**
		 * Filter: 'mcmsseo_opengraph_desc' - Allow changing the OpenGraph description
		 *
		 * @api string $ogdesc The description string.
		 */
		$ogdesc = trim( apply_filters( 'mcmsseo_opengraph_desc', $ogdesc ) );
	...
	public function site_name() {
		/**
		 * Filter: 'mcmsseo_opengraph_site_name' - Allow changing the OpenGraph site name
		 *
		 * @api string $unsigned Blog name string
		 */
		$name = apply_filters( 'mcmsseo_opengraph_site_name', get_bloginfo( 'name' ) );
	...
	public function tags() {
		...
		$tags = get_the_tags();
		if ( ! is_mcms_error( $tags ) && ( is_array( $tags ) && $tags !== array() ) ) {
	...
	public function category() {
		...
		if ( ! is_mcms_error( $terms ) && ( is_array( $terms ) && $terms !== array() ) ) {
	...
	public function publish_date() {
		...
		if ( ! is_singular( 'post' ) ) {
			/**
			 * Filter: 'mcmsseo_opengraph_show_publish_date' - Allow showing publication date for other post types
			 *
			 * @api bool $unsigned Whether or not to show publish date
			 *
			 * @param string $post_type The current URL's post type.
			 */
			if ( false === apply_filters( 'mcmsseo_opengraph_show_publish_date', false, get_post_type() ) ) {
				return false;
			}
		}
...
class MCMSSEO_OpenGraph_Image {
	...
	private function get_opengraph_image_post( $post_id = 0 ) {
		$ogimg = MCMSSEO_Meta::get_value( 'opengraph-image', $post_id );
		if ( $ogimg !== '' ) {
	...
	private function get_opengraph_image_taxonomy() {
		if ( ( $ogimg = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'opengraph-image' ) ) !== '' ) {
			$this->add_image( $ogimg );
		}
	}
	...
	private function get_featured_image( $post_id ) {
		...
		if ( has_post_thumbnail( $post_id ) ) {
			/**
			 * Filter: 'mcmsseo_opengraph_image_size' - Allow changing the image size used for OpenGraph sharing
			 *
			 * @api string $unsigned Size string
			 */
			$thumb = mcms_get_attachment_image_src( get_post_thumbnail_id( $post_id ), apply_filters( 'mcmsseo_opengraph_image_size', 'original' ) );
	...
	private function get_attachment_page_image( $post_id ) {
		if ( get_post_type( $post_id ) === 'attachment' ) {
			...
			switch ( $mime_type ) {
				...
				case 'image/png':
				case 'image/gif':
					return $this->add_image( mcms_get_attachment_url( $post_id ) );
			}
		}
	...
	/**
	 * Filter: 'mcmsseo_pre_analysis_post_content' - Allow filtering the content before analysis
	 *
	...
	private function get_content_images( $post ) {
		$content = apply_filters( 'mcmsseo_pre_analysis_post_content', $post->post_content, $post );
	 ...
	 *
	 * @param array $img_data mcms_get_attachment_image_src: url, width, height, icon.
	 *
	 * @return bool
	 */
	...
	private function add_image( $img ) {
		...
		// Filter: 'mcmsseo_opengraph_image' - Allow changing the OpenGraph image.
		$img = trim( apply_filters( 'mcmsseo_opengraph_image', $img ) );
		...
		if ( MCMSSEO_Utils::is_url_relative( $img ) === true ) {
	...
	private function get_relative_path( $img ) {
		...
		// If it's a relative URL, it's relative to the domain, not necessarily to the MandarinCMS install, we
