...
/**
 * Main frontend class for Ultimatum SEO, responsible for the SEO output as well as removing
 * default MandarinCMS output.
 */
class MCMSSEO_Frontend {
	...
	/**
	 * @var array Holds the modules options.
	 */
	 ...
	 * @var array
	 */
	private $required_options = array( 'mcmsseo', 'mcmsseo_rss', 'mcmsseo_social', 'mcmsseo_permalinks', 'mcmsseo_titles' );
	...
	protected function __construct() {
		...
		$this->options = MCMSSEO_Options::get_options( $this->required_options );

		add_action( 'mcms_head', array( $this, 'front_page_specific_init' ), 0 );
		add_action( 'mcms_head', array( $this, 'head' ), 1 );

		// The head function here calls action mcmsseo_head, to which we hook all our functionality.
		add_action( 'mcmsseo_head', array( $this, 'debug_marker' ), 2 );
		add_action( 'mcmsseo_head', array( $this, 'metadesc' ), 6 );
		add_action( 'mcmsseo_head', array( $this, 'robots' ), 10 );
		add_action( 'mcmsseo_head', array( $this, 'metakeywords' ), 11 );
		add_action( 'mcmsseo_head', array( $this, 'canonical' ), 20 );
		add_action( 'mcmsseo_head', array( $this, 'adjacent_rel_links' ), 21 );
		add_action( 'mcmsseo_head', array( $this, 'publisher' ), 22 );

		// Remove actions that we will handle through our mcmsseo_head call, and probably change the output of.
		remove_action( 'mcms_head', 'rel_canonical' );
		remove_action( 'mcms_head', 'index_rel_link' );
		remove_action( 'mcms_head', 'start_post_rel_link' );
		remove_action( 'mcms_head', 'adjacent_posts_rel_link_mcms_head' );
		remove_action( 'mcms_head', 'noindex', 1 );

		// When using MCMS 4.4, just use the new hook.
		add_filter( 'pre_get_document_title', array( $this, 'title' ), 15 );
		add_filter( 'mcms_title', array( $this, 'title' ), 15, 3 );

		add_filter( 'thematic_doctitle', array( $this, 'title' ), 15 );

		add_action( 'mcms', array( $this, 'page_redirect' ), 99 );
		...
		// Fix the WooMySkins woo_title() output.
		...
		if ( $this->options['disable-date'] === true ||
			...
			add_action( 'mcms', array( $this, 'archive_redirect' ) );
		}
		...
		// For MandarinCMS functions below 4.4.
		if ( ! current_myskin_supports( 'title-tag' ) && $this->options['forcerewritetitle'] === true ) {
			add_action( 'template_redirect', array( $this, 'force_rewrite_output_buffer' ), 99999 );
			add_action( 'mcms_footer', array( $this, 'flush_cache' ), - 1 );
		}

		if ( $this->options['title_test'] > 0 ) {
			add_filter( 'mcmsseo_title', array( $this, 'title_test_helper' ) );
		}

		$primary_category = new MCMSSEO_Frontend_Primary_Category();
	...
	public function front_page_specific_init() {
		...
		new MCMSSEO_JSON_LD;
		add_action( 'mcmsseo_head', array( $this, 'webmaster_tools_authentication' ), 90 );
	}
	...
	public function get_content_title( $object = null ) {
		if ( is_null( $object ) ) {
			$object = $GLOBALS['mcms_query']->get_queried_object();
		}

		if ( is_object( $object ) ) {
			$title = MCMSSEO_Meta::get_value( 'title', $object->ID );

			if ( $title !== '' ) {
				return mcmsseo_replace_vars( $title, $object );
			}
		...
		return $this->get_title_from_options( 'title-404-mcmsseo' );
	}
	...
	public function get_taxonomy_title() {
		...
		$title = MCMSSEO_Taxonomy_Meta::get_term_meta( $object, $object->taxonomy, 'title' );
	...
	public function get_author_title() {
		...
		$title     = trim( get_the_author_meta( 'mcmsseo_title', $author_id ) );

		if ( $title !== '' ) {
			return mcmsseo_replace_vars( $title, array() );
		}

		return $this->get_title_from_options( 'title-author-mcmsseo' );
	}

	/**
	 * Simple function to use to pull data from $options.
	 *
	 * All titles pulled from options will be run through the mcmsseo_replace_vars function.
	 *
	...
	public function get_title_from_options( $index, $var_source = array() ) {
		if ( ! isset( $this->options[ $index ] ) || $this->options[ $index ] === '' ) {
			if ( is_singular() ) {
				return mcmsseo_replace_vars( '%%title%% %%sep%% %%sitename%%', $var_source );
			}
			else {
				return '';
			}
			...
			return mcmsseo_replace_vars( $this->options[ $index ], $var_source );
		}
	}
	 ...
	 * mcms_title call on singular pages).
	 *
	...
	public function add_paging_to_title( $sep, $seplocation, $title ) {
		...
		if ( ! empty( $mcms_query->query_vars['paged'] ) && $mcms_query->query_vars['paged'] > 1 ) {
			return $this->add_to_title( $sep, $seplocation, $title, $mcms_query->query_vars['paged'] . '/' . $mcms_query->max_num_pages );
		}
	 ...
	 * @param string $separator          Separator determined in myskin (unused).
	...
	private function generate_title( $title, $separator_location ) {
		...
		$separator = mcmsseo_replace_vars( '%%sep%%', array() );
		...
		elseif ( $this->is_home_posts_page() ) {
			$title = $this->get_title_from_options( 'title-home-mcmsseo' );
		}
		...
		elseif ( is_search() ) {
			$title = $this->get_title_from_options( 'title-search-mcmsseo' );

			if ( ! is_string( $title ) || '' === $title ) {
				$title_part = sprintf( __( 'Search for "%s"', 'mandarincms-seo' ), esc_html( get_search_query() ) );
			}
		}
		elseif ( is_category() || is_tag() || is_tax() ) {
			...
			if ( ! is_string( $title ) || '' === $title ) {
				...
				elseif ( is_tag() ) {
					...
					if ( $title_part === '' ) {
						$term       = $GLOBALS['mcms_query']->get_queried_object();
		...
		elseif ( is_archive() ) {
			$title = $this->get_title_from_options( 'title-archive-mcmsseo' );

			// @todo [JRF => Ultimatum] Should these not use the archive default if no title found ?
			// MCMSSEO_Options::get_default( 'mcmsseo_titles', 'title-archive-mcmsseo' )
			// Replacement would be needed!
			if ( empty( $title ) ) {
				if ( is_month() ) {
					$title_part = sprintf( __( '%s Archives', 'mandarincms-seo' ), single_month_title( ' ', false ) );
				}
				elseif ( is_year() ) {
					$title_part = sprintf( __( '%s Archives', 'mandarincms-seo' ), get_query_var( 'year' ) );
				}
				elseif ( is_day() ) {
					$title_part = sprintf( __( '%s Archives', 'mandarincms-seo' ), get_the_date() );
				}
				else {
					$title_part = __( 'Archives', 'mandarincms-seo' );
				}
			}
		}
		elseif ( is_404() ) {

			$title = $this->get_title_from_options( 'title-404-mcmsseo' );

			// @todo [JRF => Ultimatum] Should these not use the 404 default if no title found ?
			// MCMSSEO_Options::get_default( 'mcmsseo_titles', 'title-404-mcmsseo' )
			// Replacement would be needed!
			if ( empty( $title ) ) {
				$title_part = __( 'Page not found', 'mandarincms-seo' );
			}
		}
		else {
		...
		/**
		 * Filter: 'mcmsseo_title' - Allow changing the Ultimatum SEO <title> output
		 *
		 * @api string $title The page title being put out.
		 */

		return esc_html( strip_tags( stripslashes( apply_filters( 'mcmsseo_title', $title ) ) ) );
	}
	 ...
	 * @return string
	 */
	function force_mcms_title() {
		global $mcms_query;
		$old_mcms_query = null;

		if ( ! $mcms_query->is_main_query() ) {
			$old_mcms_query = $mcms_query;
			mcms_reset_query();
		}

		$title = $this->title( '' );

		if ( ! empty( $old_mcms_query ) ) {
			$GLOBALS['mcms_query'] = $old_mcms_query;
			unset( $old_mcms_query );
		}
	...
	public function debug_marker( $echo = true ) {
		$marker = sprintf(
			'<!-- This site is optimized with the ' . $this->head_product_name() . '%1$s - https://jiiworks.net/mandarincms/modules/seo/ -->',
			/**
			 * Filter: 'mcmsseo_hide_version' - can be used to hide the Ultimatum SEO version in the debug marker (only available in Ultimatum SEO)
			 *
			 * @api bool
			 */
			( ( apply_filters( 'mcmsseo_hide_version', false ) && $this->is_premium() ) ? '' : ' v' . MCMSSEO_VERSION )
		);
	...
	/**
	 * Main wrapper function attached to mcms_head. This combines all the output on the frontend of the Ultimatum SEO module.
	 */
	public function head() {
		...
		/**
		 * Action: 'mcmsseo_head' - Allow other modules to output inside the Ultimatum SEO section of the head section.
		 */
		do_action( 'mcmsseo_head' );
	...
	public function robots() {
		...
		if ( is_singular() && is_object( $post ) ) {
			...
			elseif ( is_tax() || is_tag() || is_category() ) {
				$term = $mcms_query->get_queried_object();
				...
				$term_meta = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'noindex' );
			...
			elseif (
				( is_author() && $this->options['noindex-author-mcmsseo'] === true ) ||
				( is_date() && $this->options['noindex-archive-mcmsseo'] === true )
			) {
				$robots['index'] = 'noindex';
			}
			elseif ( is_home() ) {
				if ( get_query_var( 'paged' ) > 1 && $this->options['noindex-subpages-mcmsseo'] === true ) {
					$robots['index'] = 'noindex';
				}
			...
			$is_paged         = isset( $mcms_query->query_vars['paged'] ) && ( $mcms_query->query_vars['paged'] && $mcms_query->query_vars['paged'] > 1 );
			$noindex_subpages = $this->options['noindex-subpages-mcmsseo'] === true;
		...
		// Force override to respect the MCMS settings.
		...
		/**
		 * Filter: 'mcmsseo_robots' - Allows filtering of the meta robots output of Ultimatum SEO
		 *
		 * @api string $robotsstr The meta robots directives to be echoed.
		 */
		$robotsstr = apply_filters( 'mcmsseo_robots', $robotsstr );
	...
	public function robots_for_single_post( $robots, $post_id = 0 ) {
		$noindex = MCMSSEO_Meta::get_value( 'meta-robots-noindex', $post_id );
		if ( $noindex === '1' ) {
			$robots['index'] = 'noindex';
		}
		elseif ( $noindex === '2' ) {
		...
		if ( MCMSSEO_Meta::get_value( 'meta-robots-nofollow', $post_id ) === '1' ) {
			$robots['follow'] = 'nofollow';
		}

		$meta_robots_adv = MCMSSEO_Meta::get_value( 'meta-robots-adv', $post_id );
	...
	private function generate_canonical() {
		...
		if ( is_singular() ) {
			...
			elseif ( is_front_page() ) {
				$canonical = MCMSSEO_Utils::home_url();
			}
			elseif ( $this->is_posts_page() ) {

				$posts_page_id = get_option( 'page_for_posts' );
				$canonical     = MCMSSEO_Meta::get_value( 'canonical', $posts_page_id );
			...
			elseif ( is_tax() || is_tag() || is_category() ) {
				...
				if ( ! empty( $term ) && ! $this->is_multiple_terms_query() ) {
					...
					$canonical_override = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'canonical' );
					$term_link          = get_term_link( $term, $term->taxonomy );

					if ( ! is_mcms_error( $term_link ) ) {
						$canonical = $term_link;
					}
				}
			}
			...
			if ( $canonical && get_query_var( 'paged' ) > 1 ) {
				global $mcms_rewrite;
				if ( ! $mcms_rewrite->using_permalinks() ) {
					if ( is_front_page() ) {
					...
					if ( is_front_page() ) {
						$canonical = MCMSSEO_Sitemaps_Router::get_base_url( '' );
					}
					$canonical = user_trailingslashit( trailingslashit( $canonical ) . trailingslashit( $mcms_rewrite->pagination_base ) . get_query_var( 'paged' ) );
				}
			}
		}
		...
		if ( is_string( $canonical ) && $canonical !== '' ) {
			...
			if ( MCMSSEO_Utils::is_url_relative( $canonical ) === true ) {
		...
		/**
		 * Filter: 'mcmsseo_canonical' - Allow filtering of the canonical URL put out by Ultimatum SEO
		 *
		 * @api string $canonical The canonical URL
		 */
		$this->canonical = apply_filters( 'mcmsseo_canonical', $canonical );
	}
	...
	public function adjacent_rel_links() {
		...
		/**
		 * Filter 'mcmsseo_genesis_force_adjacent_rel_home' - Allows devs to allow echoing rel="next" / rel="prev" by Ultimatum SEO on Genesis installs
		 *
		 * @api bool $unsigned Whether or not to rel=next / rel=prev
		 */
		if ( is_home() && function_exists( 'genesis' ) && apply_filters( 'mcmsseo_genesis_force_adjacent_rel_home', false ) === false ) {
			return;
		}
		...
		if ( ! is_singular() ) {
			...
			if ( is_string( $url ) && $url !== '' ) {
				...
				if ( is_front_page() ) {
					$url = MCMSSEO_Sitemaps_Router::get_base_url( '' );
				}
				...
				if ( $paged < $mcms_query->max_num_pages ) {
			...
			$numpages = 0;
			if ( isset( $mcms_query->post->post_content ) ) {
				$numpages = ( substr_count( $mcms_query->post->post_content, '<!--nextpage-->' ) + 1 );
			}
			if ( $numpages > 1 ) {
				...
				$url = get_permalink( $mcms_query->post->ID );
		...
		if ( ! $mcms_rewrite->using_permalinks() ) {
			...
			if ( $page > 1 ) {
				...
				if ( $incl_pagination_base ) {
					$base = trailingslashit( $mcms_rewrite->pagination_base );
				}
				$url = user_trailingslashit( trailingslashit( $url ) . $base . $page );
			}
		}
		/**
		 * Filter: 'mcmsseo_' . $rel . '_rel_link' - Allow changing link rel output by Ultimatum SEO
		 *
		 * @api string $unsigned The full `<link` element.
		 */
		$link = apply_filters( 'mcmsseo_' . $rel . '_rel_link', '<link rel="' . esc_attr( $rel ) . '" href="' . esc_url( $url ) . "\" />\n" );
	...
	public function metakeywords() {
		...
		if ( is_singular() ) {
			$keywords = MCMSSEO_Meta::get_value( 'metakeywords' );
			if ( $keywords === '' && ( is_object( $post ) && ( ( isset( $this->options[ 'metakey-' . $post->post_type ] ) && $this->options[ 'metakey-' . $post->post_type ] !== '' ) ) ) ) {
				$keywords = mcmsseo_replace_vars( $this->options[ 'metakey-' . $post->post_type ], $post );
			}
		}
		else {
			if ( $this->is_home_posts_page() && $this->options['metakey-home-mcmsseo'] !== '' ) {
				$keywords = mcmsseo_replace_vars( $this->options['metakey-home-mcmsseo'], array() );
			}
			...
			elseif ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_object( $term ) ) {
					$keywords = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'metakey' );
					if ( ( ! is_string( $keywords ) || $keywords === '' ) && ( isset( $this->options[ 'metakey-tax-' . $term->taxonomy ] ) && $this->options[ 'metakey-tax-' . $term->taxonomy ] !== '' ) ) {
						$keywords = mcmsseo_replace_vars( $this->options[ 'metakey-tax-' . $term->taxonomy ], $term );
					}
				}
			}
			elseif ( is_author() ) {
				...
				if ( ! $keywords && $this->options['metakey-author-mcmsseo'] !== '' ) {
					$keywords = mcmsseo_replace_vars( $this->options['metakey-author-mcmsseo'], $mcms_query->get_queried_object() );
				}
			}
			elseif ( is_post_type_archive() ) {
				$post_type = get_query_var( 'post_type' );
				...
				if ( isset( $this->options[ 'metakey-ptarchive-' . $post_type ] ) && $this->options[ 'metakey-ptarchive-' . $post_type ] !== '' ) {
					$keywords = mcmsseo_replace_vars( $this->options[ 'metakey-ptarchive-' . $post_type ], $mcms_query->get_queried_object() );
				}
			}
		}

		$keywords = apply_filters( 'mcmsseo_metakey', trim( $keywords ) ); // TODO Make deprecated.

		/**
		 * Filter: 'mcmsseo_metakeywords' - Allow changing the Ultimatum SEO meta keywords
		 *
		 * @api string $keywords The meta keywords to be echoed.
		 */
		$keywords = apply_filters( 'mcmsseo_metakeywords', trim( $keywords ) ); // More appropriately named.
	...
	public function metadesc( $echo = true ) {
		...
		if ( $echo !== false ) {
			...
			elseif ( current_user_can( 'manage_options' ) && is_singular() ) {
				echo '<!-- ', __( 'Admin only notice: this page doesn\'t show a meta description because it doesn\'t have one, either write it for this page specifically or go into the SEO -> Titles menu and set up a template.', 'mandarincms-seo' ), ' -->', "\n";
			}
		}
		else {
	...
	private function generate_metadesc() {
		...
		if ( is_singular() ) {
			...
			$metadesc_override = MCMSSEO_Meta::get_value( 'metadesc' );
		}
		else {
			if ( is_search() ) {
				$metadesc = '';
			...
			elseif ( $this->is_home_posts_page() ) {
				$template = $this->options['metadesc-home-mcmsseo'];
				$term     = array();
			...
			elseif ( $this->is_posts_page() ) {
				$metadesc = MCMSSEO_Meta::get_value( 'metadesc', get_option( 'page_for_posts' ) );
				if ( ( $metadesc === '' && $post_type !== '' ) && isset( $this->options[ 'metadesc-' . $post_type ] ) ) {
			...
			elseif ( $this->is_home_static_page() ) {
				$metadesc = MCMSSEO_Meta::get_value( 'metadesc' );
				if ( ( $metadesc === '' && $post_type !== '' ) && isset( $this->options[ 'metadesc-' . $post_type ] ) ) {
					$template = $this->options[ 'metadesc-' . $post_type ];
				}
			}
			elseif ( is_category() || is_tag() || is_tax() ) {
				$term              = $mcms_query->get_queried_object();
				$metadesc_override = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'desc' );
			...
			elseif ( is_author() ) {
				$author_id = get_query_var( 'author' );
				$metadesc  = get_the_author_meta( 'mcmsseo_metadesc', $author_id );
				if ( ( ! is_string( $metadesc ) || $metadesc === '' ) && '' !== $this->options['metadesc-author-mcmsseo'] ) {
					$template = $this->options['metadesc-author-mcmsseo'];
				}
			}
			elseif ( is_post_type_archive() ) {
				$post_type = get_query_var( 'post_type' );
			...
			elseif ( is_archive() ) {
				$template = $this->options['metadesc-archive-mcmsseo'];
			}
		...
		$metadesc = mcmsseo_replace_vars( $metadesc, $post_data );

		/**
		 * Filter: 'mcmsseo_metadesc' - Allow changing the Ultimatum SEO meta description sentence.
		 *
		 * @api string $metadesc The description sentence.
		 */
		$this->metadesc = apply_filters( 'mcmsseo_metadesc', trim( $metadesc ) );
	}
	...
	function page_redirect() {
		if ( is_singular() ) {
			...
			$redir = MCMSSEO_Meta::get_value( 'redirect', $post->ID );
			if ( $redir !== '' ) {
				mcms_redirect( $redir, 301 );
				exit;
			}
		}
	...
	function archive_redirect() {
		...
		if (
			( $this->options['disable-date'] === true && $mcms_query->is_date ) ||
			( $this->options['disable-author'] === true && $mcms_query->is_author ) ||
			( $this->options['disable-post_format'] === true && $mcms_query->is_tax( 'post_format' ) )
		) {
			mcms_safe_redirect( get_bloginfo( 'url' ), 301 );
			exit;
		}
	...
	function attachment_redirect() {
		...
		if ( is_attachment() && ( ( is_object( $post ) && isset( $post->post_parent ) ) && ( is_numeric( $post->post_parent ) && $post->post_parent != 0 ) ) ) {
			mcms_safe_redirect( get_permalink( $post->post_parent ), 301 );
			exit;
		}
	...
	public function clean_permalink() {
		...
		if ( is_singular() ) {
			global $post;
			if ( empty( $post ) ) {
				$post = $mcms_query->get_queried_object();
			}
		...
		if ( ! empty( $properurl ) && $mcms_query->query_vars['paged'] != 0 && $mcms_query->post_count != 0 ) {
			if ( is_search() && ! empty( $s ) ) {
				$properurl = get_bloginfo( 'url' ) . '/page/' . $mcms_query->query_vars['paged'] . '/?s=' . $s;
			}
			else {
				$properurl = user_trailingslashit( trailingslashit( $properurl ) . 'page/' . $mcms_query->query_vars['paged'] );
			}
		}

		// Prevent cleaning out the MCMS Subscription managers interface for everyone.
		if ( isset( $_GET['mcms-subscription-manager'] ) ) {
			$properurl = '';
		}

		/**
		 * Filter: 'mcmsseo_whitelist_permalink_vars' - Allow modules to register their own variables not to clean
		 *
		 * @api array $unsigned Array of permalink variables _not_ to clean. Empty by default.
		 */
		$whitelisted_extravars = apply_filters( 'mcmsseo_whitelist_permalink_vars', array() );
		...
		if ( ! empty( $properurl ) && $cururl != $properurl ) {
			mcms_safe_redirect( $properurl, 301 );
			exit;
		}
	}
	...
	function rss_replace_vars( $content ) {
		...
		/**
		 * Allow the developer to determine whether or not to follow the links in the bits Ultimatum SEO adds to the RSS feed, defaults to true.
		 *
	...
	function embed_rss( $content, $context = 'full' ) {
		...
		/**
		 * Filter: 'mcmsseo_include_rss_footer' - Allow the RSS footer to be dynamically shown/hidden.
		 *
		 ...
		 */
		if ( ! apply_filters( 'mcmsseo_include_rss_footer', true, $context ) ) {
			return $content;
		}
		...
		if ( is_feed() ) {
			...
			if ( $this->options['rssbefore'] !== '' ) {
				$before = mcmsautop( $this->rss_replace_vars( $this->options['rssbefore'] ) );
			}
			if ( $this->options['rssafter'] !== '' ) {
				$after = mcmsautop( $this->rss_replace_vars( $this->options['rssafter'] ) );
			}
			if ( $before !== '' || $after !== '' ) {
				if ( ( isset( $context ) && $context === 'excerpt' ) && trim( $content ) !== '' ) {
					$content = mcmsautop( $content );
				}
	...
	function title_test_helper( $title ) {
		$mcmsseo_titles = get_option( 'mcmsseo_titles' );

		$mcmsseo_titles['title_test'] ++;
		update_option( 'mcmsseo_titles', $mcmsseo_titles );

		// Prevent this setting from being on forever when something breaks, as it breaks caching.
		if ( $mcmsseo_titles['title_test'] > 5 ) {
			$mcmsseo_titles['title_test'] = 0;
			update_option( 'mcmsseo_titles', $mcmsseo_titles );

			remove_filter( 'mcmsseo_title', array( $this, 'title_test_helper' ) );
		...
		if ( $_SERVER['HTTP_USER_AGENT'] === "MandarinCMS/{$GLOBALS['mcms_version']}; " . get_bloginfo( 'url' ) . ' - Ultimatum' ) {
			return 'This is a Ultimatum Test Title';
		}
	...
	/**
	 * Check if this module is the premium version of MCMSSEO
	 *
	 * @return bool
	 */
	private function is_premium() {
		return file_exists( MCMSSEO_PATH . 'premium/' );
	}
	 ...
	 *
	 * @param MCMS_Post $post The post object with the values.
	 *
	 * @return string
	 */
	private function get_keywords( $post ) {
		$keywords        = MCMSSEO_Meta::get_value( 'metakeywords', $post->ID );
		...
		if ( $keywords === '' && ( is_object( $post ) && ( isset( $this->options[ $option_meta_key ] ) && $this->options[ $option_meta_key ] !== '' ) ) ) {
			$keywords = mcmsseo_replace_vars( $this->options[ $option_meta_key ], $post );
		}
	...
	protected function is_multiple_terms_query() {
		...
		$term          = get_queried_object();
		$queried_terms = $mcms_query->tax_query->queried_terms;
