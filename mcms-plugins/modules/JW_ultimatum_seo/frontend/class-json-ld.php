...
class MCMSSEO_JSON_LD {
	...
	public function __construct() {
		...
		add_action( 'mcmsseo_head', array( $this, 'json_ld' ), 90 );
		add_action( 'mcmsseo_json_ld', array( $this, 'website' ), 10 );
		add_action( 'mcmsseo_json_ld', array( $this, 'organization_or_person' ), 20 );
	}
	...
	private function output( $context ) {
		/**
		 * Filter: 'mcmsseo_json_ld_output' - Allows filtering of the JSON+LD output
		 *
		 ...
		 */
		$this->data = apply_filters( 'mcmsseo_json_ld_output', $this->data, $context );

		if ( is_array( $this->data ) && ! empty( $this->data ) ) {
			$json_data = mcms_json_encode( $this->data );
	...
	private function prepare_organization_person_markup() {
		...
		$this->data = array(
			'@context' => 'http://schema.org',
			'@type'    => '',
			'url'      => MCMSSEO_Frontend::get_instance()->canonical( false, true ),
	...
	private function get_home_url() {
		/**
		 * Filter: 'mcmsseo_json_home_url' - Allows filtering of the home URL for Ultimatum SEO's JSON+LD output
		 *
		 * @api unsigned string
		 */
		return apply_filters( 'mcmsseo_json_home_url', MCMSSEO_Utils::home_url() );
	}

	/**
	 * Returns an alternate name if one was specified in the Ultimatum SEO settings
	 */
	...
	private function internal_search_section() {
		/**
		 * Filter: 'disable_mcmsseo_json_ld_search' - Allow disabling of the json+ld output
		 *
		 * @api bool $display_search Whether or not to display json+ld search on the frontend
		 */
		if ( ! apply_filters( 'disable_mcmsseo_json_ld_search', false ) ) {
			/**
			 * Filter: 'mcmsseo_json_ld_search_url' - Allows filtering of the search URL for Ultimatum SEO
			 *
			 * @api string $search_url The search URL for this site with a `{search_term_string}` variable.
			 */
			$search_url = apply_filters( 'mcmsseo_json_ld_search_url', $this->get_home_url() . '?s={search_term_string}' );
	...
	/**
	 * Returns the website name either from Ultimatum SEO's options or from the site settings
	 *
	 * @since 2.1
	 *
	 * @return string
	 ...
	 *
	 * @deprecated 2.1
	 * @deprecated use MCMSSEO_JSON_LD::website()
	 */
	public function internal_search() {
		_deprecated_function( __METHOD__, 'MCMSSEO 2.1', 'MCMSSEO_JSON_LD::website()' );
