...
class MCMSSEO_Twitter {
	...
	public function twitter() {
		...
		/**
		 * Action: 'mcmsseo_twitter' - Hook to add all Ultimatum SEO Twitter output to so they're close together.
		 */
		do_action( 'mcmsseo_twitter' );
	}

	/**
	 * Display the Twitter card type.
	 *
	 * This defaults to summary but can be filtered using the <code>mcmsseo_twitter_card_type</code> filter.
	 *
	 * @link https://dev.twitter.com/docs/cards
	 */
	...
	private function determine_card_type() {
		...
		/**
		 * Filter: 'mcmsseo_twitter_card_type' - Allow changing the Twitter Card type as output in the Twitter card by Ultimatum SEO
		 *
		 * @api string $unsigned The type string
		 */
		$this->type = apply_filters( 'mcmsseo_twitter_card_type', $this->type );
	}
	...
	private function output_metatag( $name, $value, $escaped = false ) {
		...
		/**
		 * Filter: 'mcmsseo_twitter_metatag_key' - Make the Twitter metatag key filterable
		 *
		 * @api string $key The Twitter metatag key
		 */
		$metatag_key = apply_filters( 'mcmsseo_twitter_metatag_key', 'name' );
	...
	protected function description() {
		...
		elseif ( MCMSSEO_Frontend::get_instance()->is_posts_page() ) {
		...
		/**
		 * Filter: 'mcmsseo_twitter_description' - Allow changing the Twitter description as output in the Twitter card by Ultimatum SEO
		 *
		 * @api string $twitter The description string
		 */
		$meta_desc = apply_filters( 'mcmsseo_twitter_description', $meta_desc );
	...
	private function single_description( $post_id = 0 ) {
		$meta_desc = trim( MCMSSEO_Meta::get_value( 'twitter-description', $post_id ) );
	...
	private function taxonomy_description() {
		$meta_desc = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'twitter-description' );
	...
	private function fallback_description() {
		return trim( MCMSSEO_Frontend::get_instance()->metadesc( false ) );
	}
	...
	protected function title() {
		...
		/**
		 * Filter: 'mcmsseo_twitter_title' - Allow changing the Twitter title as output in the Twitter card by Ultimatum SEO
		 *
		 * @api string $twitter The title string
		 */
		$title = apply_filters( 'mcmsseo_twitter_title', $title );
	...
	private function single_title( $post_id = 0 ) {
		$title = MCMSSEO_Meta::get_value( 'twitter-title', $post_id );
		if ( ! is_string( $title ) || $title === '' ) {
	...
	private function taxonomy_title() {
		$title = MCMSSEO_Taxonomy_Meta::get_meta_without_term( 'twitter-title' );
	...
	private function fallback_title() {
		return MCMSSEO_Frontend::get_instance()->title( '' );
	}
	...
	protected function site_twitter() {
		/**
		 * Filter: 'mcmsseo_twitter_site' - Allow changing the Twitter site account as output in the Twitter card by Ultimatum SEO
		 *
		 * @api string $unsigned Twitter site account string
		 */
		$site = apply_filters( 'mcmsseo_twitter_site', $this->options['twitter_site'] );
	 ...
	 *
	 * Solves issues with filters returning urls and myskin's/other modules also adding a user meta
	...
	private function taxonomy_image_output() {
		foreach ( array( 'twitter-image', 'opengraph-image' ) as $tag ) {
			$img = MCMSSEO_Taxonomy_Meta::get_meta_without_term( $tag );
			if ( $img !== '' ) {
	...
	protected function image_output( $img, $tag = false ) {
		...
		if ( $tag ) {
			_deprecated_argument( __METHOD__, 'MCMSSEO 2.4' );
		}

		/**
		 * Filter: 'mcmsseo_twitter_image' - Allow changing the Twitter Card image
		 *
		 * @api string $img Image URL string
		 */
		$img = apply_filters( 'mcmsseo_twitter_image', $img );

		if ( MCMSSEO_Utils::is_url_relative( $img ) === true && $img[0] === '/' ) {
	...
	private function image_from_meta_values_output( $post_id = 0 ) {
		foreach ( array( 'twitter-image', 'opengraph-image' ) as $tag ) {
			$img = MCMSSEO_Meta::get_value( $tag, $post_id );
			if ( $img !== '' ) {
				$this->image_output( $img );
	...
	private function image_of_attachment_page_output( $attachment_id ) {
		if ( get_post_type( $attachment_id ) === 'attachment' ) {
			...
			switch ( $mime_type ) {
				...
				case 'image/png':
				case 'image/gif':
					$this->image_output( mcms_get_attachment_url( $attachment_id ) );
					return true;
			}
		}
	...
	private function image_thumbnail_output( $post_id = 0 ) {
		...
		if ( function_exists( 'has_post_thumbnail' ) && has_post_thumbnail( $post_id ) ) {
			/**
			 * Filter: 'mcmsseo_twitter_image_size' - Allow changing the Twitter Card image size
			 *
			 * @api string $featured_img Image size string
			 */
			$featured_img = mcms_get_attachment_image_src( get_post_thumbnail_id( $post_id ), apply_filters( 'mcmsseo_twitter_image_size', 'full' ) );
	...
	protected function author() {
		...
		/**
		 * Filter: 'mcmsseo_twitter_creator_account' - Allow changing the Twitter account as output in the Twitter card by Ultimatum SEO
		 *
		 * @api string $twitter The twitter account name string
		 */
		$twitter = apply_filters( 'mcmsseo_twitter_creator_account', $twitter );
	...
	protected function site_domain() {
		_deprecated_function( __METHOD__, 'MCMSSEO 3.0' );
	}
} /* End of class */
