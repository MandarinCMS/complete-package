...
class MCMSSEO_Breadcrumbs {
	...
	/**
	 * @var    array    MCMSSEO options array from get_all()
	 */
	private $options;
	...
	/**
	 * @var string    Ultimatum SEO breadcrumb separator
	 */
	private $separator = '';

	/**
	 * @var string    HTML wrapper element for the Ultimatum SEO breadcrumbs output
	 */
	...
	private function __construct() {
		$this->options        = MCMSSEO_Options::get_options( array( 'mcmsseo_titles', 'mcmsseo_internallinks', 'mcmsseo_xml' ) );
	...
	/**
	 * Filter: 'mcmsseo_breadcrumb_single_link_wrapper' - Allows developer to change or wrap each breadcrumb element
	 *
	 * @api string $element
	 */
	private function filter_element() {
		$this->element = esc_attr( apply_filters( 'mcmsseo_breadcrumb_single_link_wrapper', $this->element ) );
	}

	/**
	 * Filter: 'mcmsseo_breadcrumb_separator' - Allow (myskin) developer to change the Ultimatum SEO breadcrumb separator.
	 *
	 * @api string $breadcrumbs_sep Breadcrumbs separator
	 */
	private function filter_separator() {
		$separator       = apply_filters( 'mcmsseo_breadcrumb_separator', $this->options['breadcrumbs-sep'] );
		$this->separator = ' ' . $separator . ' ';
	}

	/**
	 * Filter: 'mcmsseo_breadcrumb_output_wrapper' - Allow changing the HTML wrapper element for the Ultimatum SEO breadcrumbs output
	 *
	 * @api string $wrapper The wrapper element
	 */
	private function filter_wrapper() {
		$wrapper = apply_filters( 'mcmsseo_breadcrumb_output_wrapper', $this->wrapper );
	...
	private function get_post_ancestors() {
		...
		/**
		 * Filter: Allow changing the ancestors for the Ultimatum SEO breadcrumbs output
		 *
		 * @api array $ancestors Ancestors
		 */
		$ancestors = apply_filters( 'mcms_seo_get_bc_ancestors', $ancestors );

		if ( ! is_array( $ancestors ) ) {
			trigger_error( 'The return value for the "mcms_seo_get_bc_ancestors" filter should be an array.', E_USER_WARNING );
	...
	private function set_crumbs() {
		...
		elseif ( is_singular() ) {
			...
			if ( is_post_type_archive() ) {
				$post_type = $mcms_query->get( 'post_type' );
			...
			elseif ( is_author() ) {
				$user = $mcms_query->get_queried_object();
		...
		/**
		 * Filter: 'mcmsseo_breadcrumb_links' - Allow the developer to filter the Ultimatum SEO breadcrumb links, add to them, change order, etc.
		 *
		 * @api array $crumbs The crumbs array
		 */
		$this->crumbs = apply_filters( 'mcmsseo_breadcrumb_links', $this->crumbs );
		...
		if ( isset( $this->options[ 'post_types-' . $this->post->post_type . '-maintax' ] ) && $this->options[ 'post_types-' . $this->post->post_type . '-maintax' ] != '0' ) {
			...
			if ( isset( $this->post->ID ) ) {
				...
				if ( is_array( $terms ) && $terms !== array() ) {
					...
					$primary_term = new MCMSSEO_Primary_Term( $main_tax, $this->post->ID );
	...
	private function add_linked_month_year_crumb() {
		$this->add_predefined_crumb(
			$GLOBALS['mcms_locale']->get_month( get_query_var( 'monthnum' ) ) . ' ' . get_query_var( 'year' ),
	...
	private function get_link_info_for_id( $id ) {
		...
		$link['text'] = MCMSSEO_Meta::get_value( 'bctitle', $id );
		if ( $link['text'] === '' ) {
			$link['text'] = strip_tags( get_the_title( $id ) );
		}

		/**
		 * Filter: 'mcms_seo_get_bc_title' - Allow developer to filter the Ultimatum SEO Breadcrumb title.
		 *
		 * @api string $link_text The Breadcrumb title text
		 *
		 * @param int $link_id The post ID
		 */
		$link['text'] = apply_filters( 'mcms_seo_get_bc_title', $link['text'], $id );
	...
	private function get_link_info_for_term( $term ) {
		...
		$bctitle = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'bctitle' );
	 ...
	 *                     MCMSSEO -> Internal Links options page.
	...
	private function crumb_to_link( $link, $i ) {
		...
		/**
		 * Filter: 'mcmsseo_breadcrumb_single_link' - Allow changing of each link being put out by the Ultimatum SEO breadcrumbs class
		 *
		...
		return apply_filters( 'mcmsseo_breadcrumb_single_link', $link_output, $link );
	}
	...
	private function wrap_breadcrumb() {
		if ( is_string( $this->output ) && $this->output !== '' ) {
			...
			/**
			 * Filter: 'mcmsseo_breadcrumb_output' - Allow changing the HTML output of the Ultimatum SEO breadcrumbs class
			 *
			 * @api string $unsigned HTML output
			 */
			$output = apply_filters( 'mcmsseo_breadcrumb_output', $output );
	...
	/**
	 * Filter: 'mcmsseo_breadcrumb_output_id' - Allow changing the HTML ID on the Ultimatum SEO breadcrumbs wrapper element
	 *
	 * @api string $unsigned ID to add to the wrapper element
	 */
	private function get_output_id() {
		$id = apply_filters( 'mcmsseo_breadcrumb_output_id', '' );
		if ( is_string( $id ) && '' !== $id ) {
			$id = ' id="' . esc_attr( $id ) . '"';
		}
	...
	/**
	 * Filter: 'mcmsseo_breadcrumb_output_class' - Allow changing the HTML class on the Ultimatum SEO breadcrumbs wrapper element
	 *
	 * @api string $unsigned class to add to the wrapper element
	 */
	private function get_output_class() {
		$class = apply_filters( 'mcmsseo_breadcrumb_output_class', '' );
	...
	/**
	 * Wrapper function for the breadcrumb so it can be output for the supported myskins.
	 *
	 * @deprecated 1.5.0
	 */
	public function breadcrumb_output() {
		_deprecated_function( __METHOD__, '1.5.0', 'ultimatum_breadcrumb' );
		self::breadcrumb( '<div id="mcmsseobreadcrumb">', '</div>' );
	}
	...
	public function create_breadcrumbs_string( $links, $wrapper = 'span', $element = 'span' ) {
		_deprecated_function( __METHOD__, 'MCMSSEO 1.5.2.3', 'ultimatum_breadcrumbs' );
	}
}
