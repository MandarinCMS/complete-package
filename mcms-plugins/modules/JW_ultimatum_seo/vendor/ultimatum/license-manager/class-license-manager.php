...
if ( ! interface_exists( 'iUltimatum_License_Manager', false ) ) {

	interface iUltimatum_License_Manager {
...
if ( ! class_exists( 'Ultimatum_License_Manager', false ) ) {

	/**
	 * Class Ultimatum_License_Manager
	 *
	 * @todo Maybe create a license class that contains key and option
	 * @todo Not sure if Ultimatum_License_Manager is a good name for this class, it's more managing the product (module or myskin)
	 */
	abstract class Ultimatum_License_Manager implements iUltimatum_License_Manager {
		...
		/**
		 * @var Ultimatum_License The license
		 */
		protected $product;
		...
		/**
		 * @var bool Boolean indicating whether this module is network activated
		 */
		 ...
		 * Constructor
		 *
		 * @param Ultimatum_Product $product
		 */
		public function __construct( Ultimatum_Product $product ) {
		...
		public function setup_hooks() {
			...
			// setup item type (module|myskin) specific hooks
			$this->specific_hooks();
		 ...
		 * - External requests are blocked through MCMS_HTTP_BLOCK_EXTERNAL
		 */
		public function display_admin_notices() {
			...
			// show notice if external requests are blocked through the MCMS_HTTP_BLOCK_EXTERNAL constant
			if ( defined( "MCMS_HTTP_BLOCK_EXTERNAL" ) && MCMS_HTTP_BLOCK_EXTERNAL === true ) {

				// check if our API endpoint is in the allowed hosts
				$host = parse_url( $this->product->get_api_url(), PHP_URL_HOST );

				if ( ! defined( "MCMS_ACCESSIBLE_HOSTS" ) || stristr( MCMS_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="error">
						<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', $this->product->get_text_domain() ), $this->product->get_item_name(), '<strong>' . $host . '</strong>', '<code>MCMS_ACCESSIBLE_HOSTS</code>' ); ?></p>
					</div>
				<?php
				}
		...
		protected function call_license_api( $action ) {
			...
			$request = new Ultimatum_API_Request( $url );

			if ( $request->is_valid() !== true ) {
				$this->set_notice( sprintf( __( "Request error: \"%s\" (%scommon license notices%s)", $this->product->get_text_domain() ), $request->get_error_message(), '<a href="http://kb.jiiworks.net/article/13-license-activation-notices">', '</a>' ), false );
			}
		...
		protected function get_options() {
			...
			$this->options = mcms_parse_args( $options, $defaults );
		...
		/**
		 * Output the script containing the UltimatumLicenseManager JS Object
		 *
		...
		private function check_api_host_availability() {
			$mcms_http = new MCMS_Http();
			if ( $mcms_http->block_request( $this->product->get_api_url() ) === false ) {
				return true;
			}
