...
mcms_nonce_field( $nonce_name, $nonce_name ); ?>
<table class="form-table ultimatum-license-form">
	<tbody>
		<tr valign="top">
		...
		<tr valign="top">
			...
			<td class="ultimatum-license-toggler">

				<?php if( $this->license_is_valid() ) { ?>
					<button name="<?php echo esc_attr( $action_name ); ?>" type="submit" class="button-secondary ultimatum-license-deactivate" value="deactivate"><?php echo esc_html_e( 'Deactivate License', $product->get_text_domain() ); ?></button> &nbsp;
					<small><?php _e( '(deactivate your license so you can activate it on another MandarinCMS site)', $product->get_text_domain() ); ?></small>
				<?php } else {

					if( $this->get_license_key() !== '') { ?>
						<button name="<?php echo esc_attr( $action_name ); ?>" type="submit" class="button-secondary ultimatum-license-activate" value="activate" /><?php echo esc_html_e('Activate License', $product->get_text_domain() ); ?></button> &nbsp;
					<?php } else {
		...
		<tr valign="top">
			...
			<td>
				<input name="<?php echo esc_attr( $key_name ); ?>" type="text" class="regular-text ultimatum-license-key-field <?php if( $obfuscate ) { ?>ultimatum-license-obfuscate<?php } ?>" value="<?php echo esc_attr( $visible_license_key ); ?>" placeholder="<?php echo esc_attr( sprintf( __( 'Paste your %s license key here..', $product->get_text_domain() ), $product->get_item_name() ) ); ?>" <?php if( $readonly ) { echo 'readonly="readonly"'; } ?> />
