...
if ( ! class_exists( "Ultimatum_Update_Manager", false ) ) {

	class Ultimatum_Update_Manager {
		 ...
		 * @param Ultimatum_License_Manager $license_manager The License Manager.
		 */
		public function __construct( Ultimatum_Product $product, $license_manager ) {
		...
		/**
		 * If the update check returned a MCMS_Error, show it to the user
		 */
		...
		private function call_remote_api() {
			...
			$request = new Ultimatum_API_Request( $this->product->get_api_url(), $request_params );
			...
			if ( isset( $response->license_check ) && ! empty( $response->license_check ) && $response->license_check != 'valid' ) {
				...
				$this->error_message = __( "This site has not been activated properly on jiiworks.net and thus cannot check for future updates. Please activate your site with a valid license key.", $this->product->get_text_domain() );
				add_action( 'admin_notices', array( $this, 'show_update_error' ) );
			}
