...
if ( class_exists( 'Ultimatum_License_Manager' ) && ! class_exists( "Ultimatum_Module_License_Manager", false ) ) {

	class Ultimatum_Module_License_Manager extends Ultimatum_License_Manager {
		...
		public function __construct( Ultimatum_Product $product ) {
			...
			// Check if module is network activated. We should use site(wide) options in that case.
			if( is_admin() && is_multisite() ) {
				...
				$this->is_network_activated = is_module_active_for_network( $product->get_file() );
			}
		}

		/**
		 * Setup auto updater for modules
		 */
		public function setup_auto_updater() {
			if ( $this->license_is_valid() ) {
				// setup auto updater
				require_once( dirname( __FILE__ ) . '/class-update-manager.php' );
				require_once( dirname( __FILE__ ) . '/class-module-update-manager.php' );
				new Ultimatum_Module_Update_Manager( $this->product, $this );
			}
		}
		...
		public function specific_hooks() {
			...
			// deactivate the license remotely on module deactivation
