...
if ( ! class_exists( "Ultimatum_Product", false ) ) {

	/**
	 * Class Ultimatum_Product
	 *
	...
	class Ultimatum_Product {
		...
		/**
		 * @var string The myskin slug or module file
		 */
		protected $slug;
		...
		/**
		 * @var string Relative file path to the module.
		 */
		protected $file;
		...
		/**
		 * Ultimatum_Product constructor.
		 *
		 * @param string $api_url          The URL of the shop running the EDD API.
		 * @param string $item_name        The item name in the EDD shop.
		 * @param string $slug             The slug of the module, for shiny updates this needs to be a valid HTML id.
		 ...
		 * @param string $file             The relative file path to the module.
		 * @param int    $product_id       The ID of the product in the backend system.
		 */
		public function __construct( $api_url, $item_name, $slug, $version, $item_url = '', $license_page_url = '#', $text_domain = 'ultimatum', $author = 'Ultimatum', $file = '', $product_id = 0 ) {
		...
		public function set_license_page_url( $license_page_url ) {
			...
			if ( is_admin() && is_multisite() ) {
				...
				if ( is_module_active_for_network( $this->get_slug() ) ) {
		...
		/**
		 * Returns the file path relative to the modules folder
		 *
		 * @return string
		 */
		public function get_file() {
			 ...
			 *
			 * We used to pass the file to the slug field, but this isn't supported with the shiny updates in MandarinCMS.
			 * MandarinCMS uses the slug in the HTML as an ID and a slash isn't a valid
			 */
			return empty( $this->file ) ? $this->slug : $this->file;
		}

		/**
		 * Sets the file path relative to the modules folder
		 *
		 * @param string $file Relative file path to the module.
		 */
