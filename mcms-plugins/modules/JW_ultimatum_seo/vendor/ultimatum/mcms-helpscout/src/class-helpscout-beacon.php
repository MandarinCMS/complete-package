 ...
 */
class Ultimatum_HelpScout_Beacon {
	...
	/**
	 * @var Ultimatum_HelpScout_Beacon_Identifier
	 */
	protected $identifier;

	/**
	 * @var Ultimatum_HelpScout_Beacon_Setting[]
	 */
	protected $settings;
	 ...
	 * @param Ultimatum_HelpScout_Beacon_Setting[] $settings     Suggestions for the admin pages.
	...
	/**
	 * @param Ultimatum_HelpScout_Beacon_Setting $setting Setting to add to the helpscout beacon.
	 */
	public function add_setting( Ultimatum_HelpScout_Beacon_Setting $setting ) {
	...
	public function output_beacon_js() {
		...
		$data = mcms_json_encode( $this->localize_beacon() );

		echo '<script type="text/javascript">';
		require dirname( __FILE__ ) . '/ultimatum-seo-helpscout-beacon.js.php';
		echo '</script>';
	}
	...
	private function get_instructions() {
		$return  = __( "Please explain what you're trying to find or do. If something isn't working, please explain what you expect to happen. If you can make a screenshot, please attach it.", 'mandarincms-seo-premium' );
		$return .= ' ';
		$return .= __( 'Note: submitting this form also sends us debug info about your server.', 'mandarincms-seo-premium' );
	...
	private function get_translations() {
		return array(
			'searchLabel'               => __( 'What can we help you with?', 'mandarincms-seo-premium' ),
			'searchErrorLabel'          => __( 'Your search timed out. Please double-check your internet connection and try again.', 'mandarincms-seo-premium' ),
			'noResultsLabel'            => __( 'No results found for', 'mandarincms-seo-premium' ),
			'contactLabel'              => __( 'Send a Message', 'mandarincms-seo-premium' ),
			'attachFileLabel'           => __( 'Attach a file', 'mandarincms-seo-premium' ),
			'attachFileError'           => __( 'The maximum file size is 10mb', 'mandarincms-seo-premium' ),
			'nameLabel'                 => __( 'Your Name', 'mandarincms-seo-premium' ),
			'nameError'                 => __( 'Please enter your name', 'mandarincms-seo-premium' ),
			'emailLabel'                => __( 'Email address', 'mandarincms-seo-premium' ),
			'emailError'                => __( 'Please enter a valid email address', 'mandarincms-seo-premium' ),
			'topicLabel'                => __( 'Select a topic', 'mandarincms-seo-premium' ),
			'topicError'                => __( 'Please select a topic from the list', 'mandarincms-seo-premium' ),
			'subjectLabel'              => __( 'Subject', 'mandarincms-seo-premium' ),
			'subjectError'              => __( 'Please enter a subject', 'mandarincms-seo-premium' ),
			'messageLabel'              => __( 'How can we help you?', 'mandarincms-seo-premium' ),
			'messageError'              => __( 'Please enter a message', 'mandarincms-seo-premium' ),
			'sendLabel'                 => __( 'Send', 'mandarincms-seo-premium' ),
			'contactSuccessLabel'       => __( 'Message sent, thank you!', 'mandarincms-seo-premium' ),
			'contactSuccessDescription' => __( 'Someone from Team Ultimatum will get back to you soon, normally within a couple of hours.', 'mandarincms-seo-premium' ),
		);
	}

	/**
	 * @param Ultimatum_Product[] $products The products to build the cache key for.
	 *
	 * @return string
	 */
	...
	private function get_identify() {
		...
		if ( ! $identify_data ) {
			$identifier = new Ultimatum_HelpScout_Beacon_Identifier( $this->get_products( $this->current_page ) );
