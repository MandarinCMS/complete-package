 ...
 *
 */
class Ultimatum_Google_ServiceResource {
  ...
  /** @var Ultimatum_Google_Service $service */
  private $service;
   ...
   * @param $name
   * @param $arguments
   * @return Ultimatum_Google_HttpRequest|array
   * @throws Ultimatum_Google_Exception
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new Ultimatum_Google_Exception("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new Ultimatum_Google_Exception("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new Ultimatum_Google_Exception("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = Ultimatum_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = Ultimatum_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new Ultimatum_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = Ultimatum_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = Ultimatum_Google_Client::$auth->sign($httpRequest);
    if (Ultimatum_Google_Client::$useBatch) {
      return $httpRequest;
    }

    // Terminate immediately if this is a resumable request.
    if (isset($parameters['uploadType']['value'])
        && Ultimatum_Google_MediaFileUpload::UPLOAD_RESUMABLE_TYPE == $parameters['uploadType']['value']) {
      $contentTypeHeader = array();
      if (isset($contentType) && $contentType) {
        $contentTypeHeader['content-type'] = $contentType;
      }
    ...
    return Ultimatum_Google_REST::execute($httpRequest);
  }
