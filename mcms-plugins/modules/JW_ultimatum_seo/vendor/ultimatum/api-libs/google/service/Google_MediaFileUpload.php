 ...
 *
 */
class Ultimatum_Google_MediaFileUpload {
  ...
  public function nextChunk(Ultimatum_Google_HttpRequest $req, $chunk=false) {
    ...
    $httpRequest = new Ultimatum_Google_HttpRequest($this->resumeUri, 'PUT', $headers, $chunk);
    $response = Ultimatum_Google_Client::$io->authenticatedRequest($httpRequest);
    ...
    if (308 == $code) {
      ...
      return Ultimatum_Google_REST::decodeHttpResponse($response);
    }
  }

  private function getResumeUri(Ultimatum_Google_HttpRequest $httpRequest) {
    $result = null;
    $body = $httpRequest->getPostBody();
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        'content-type' => 'application/json; charset=UTF-8',
        'content-length' => Ultimatum_Google_Utils::getStrLen($body),
    ...
    $response = Ultimatum_Google_Client::$io->makeRequest($httpRequest);
    $location = $response->getResponseHeader('location');
    $code = $response->getResponseHttpCode();
    if (200 == $code && true == $location) {
      return $location;
    }
    throw new Ultimatum_Google_Exception("Failed to start the resumable upload");
  }
}