...
class Ultimatum_Google_REST {
   ...
   *
   * @param Ultimatum_Google_HttpRequest $req
   * @return array decoded result
   * @throws Ultimatum_Google_ServiceException on server side error (ie: not authenticated,
   *  invalid or malformed post body, invalid url)
   */
  static public function execute(Ultimatum_Google_HttpRequest $req) {
    $httpRequest = Ultimatum_Google_Client::$io->makeRequest($req);
   ...
   * @static
   * @throws Ultimatum_Google_ServiceException
   * @param Ultimatum_Google_HttpRequest $response The http response to be decoded.
   * @return mixed|null
   */
  public static function decodeHttpResponse($response) {
    ...
    if ((intVal($code)) >= 300) {
      ...
      throw new Ultimatum_Google_ServiceException($err, $code, null, $decoded['error']['errors']);
    }
    ...
    if ($code != '204') {
      ...
      if ($decoded === null || $decoded === "") {
        throw new Ultimatum_Google_ServiceException("Invalid json in service response: $body");
      }
    }
    return $decoded;
  }
