...
class ultimatum_i18n {
	...
	/**
	 * Will contain the percentage translated for the module translation project in the locale
	 *
	 * @access private
	 * @var int
	 */
	...
	/**
	 * This is where you decide where to display the messages and where you set the module specific variables.
	 *
	 * @access private
	 *
	 * @param array $args
	...
	private function hide_promo() {
		$hide_promo = get_transient( 'ultimatum_i18n_' . $this->project_slug . '_promo_hide' );
		if ( ! $hide_promo ) {
			if ( filter_input( INPUT_GET, 'remove_i18n_promo', FILTER_VALIDATE_INT ) === 1 ) {
				// No expiration time, so this would normally not expire, but it wouldn't be copied to other sites etc.
				set_transient( 'ultimatum_i18n_' . $this->project_slug . '_promo_hide', true );
				$hide_promo = true;
			}
		}
	...
	private function promo_message() {
		...
		if ( $this->translation_exists && $this->translation_loaded && $this->percent_translated < 90 ) {
			$message = __( 'As you can see, there is a translation of this module in %1$s. This translation is currently %3$d%% complete. We need your help to make it complete and to fix any errors. Please register at %4$s to help complete the translation to %1$s!', $this->textdomain );
		} else if ( ! $this->translation_loaded && $this->translation_exists ) {
			$message = __( 'You\'re using MandarinCMS in %1$s. While %2$s has been translated to %1$s for %3$d%%, it\'s not been shipped with the module yet. You can help! Register at %4$s to help complete the translation to %1$s!', $this->textdomain );
		} else if ( ! $this->translation_exists ) {
			$message = __( 'You\'re using MandarinCMS in a language we don\'t support yet. We\'d love for %2$s to be translated in that language too, but unfortunately, it isn\'t right now. You can change that! Register at %4$s to help translate it!', $this->textdomain );
		}

		$registration_link = sprintf( '<a href="%1$s">%2$s</a>', esc_url( $this->register_url ), esc_html( $this->glotpress_name ) );
		$message           = sprintf( $message, esc_html( $this->locale_name ), esc_html( $this->module_name ), $this->percent_translated, $registration_link );
	...
	public function promo() {
		...
		if ( $message ) {
			...
			echo '<h2>' . sprintf( __( 'Translation of %s', $this->textdomain ), $this->module_name ) . '</h2>';
	...
	private function find_or_initialize_translation_details() {
		$set = get_transient( 'ultimatum_i18n_' . $this->project_slug . '_' . $this->locale );

		if ( ! $set ) {
			$set = $this->retrieve_translation_details();
			set_transient( 'ultimatum_i18n_' . $this->project_slug . '_' . $this->locale, $set, DAY_IN_SECONDS );
		}
	...
	/**
	 * Retrieve the translation details from Ultimatum Translate
	 *
	 * @access private
	 *
	 * @return object|null
	...
	private function retrieve_translation_details() {
		...
		$resp = mcms_remote_get( $api_url );
		$body = mcms_remote_retrieve_body( $resp );
		unset( $resp );

		if ( $body ) {
			$body = json_decode( $body );
			foreach ( $body->translation_sets as $set ) {
				if ( ! property_exists( $set, 'mcms_locale' ) ) {
					continue;
				}

				if ( $this->locale == $set->mcms_locale ) {
					return $set;
				}
			}
		}
	...
	/**
	 * Set the needed private variables based on the results from Ultimatum Translate
	 *
