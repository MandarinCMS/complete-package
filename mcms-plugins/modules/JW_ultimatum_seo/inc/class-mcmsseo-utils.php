...
/**
 * Group of utility methods for use by MCMSSEO
...
class MCMSSEO_Utils {
	...
	public static function allow_system_file_edit() {
		...
		/**
		 * Filter: 'mcmsseo_allow_system_file_edit' - Allow developers to change whether the editing of
		 * .htaccess and robots.txt is allowed
		 *
		 * @api bool $allowed Whether file editing is allowed
		 */

		return apply_filters( 'mcmsseo_allow_system_file_edit', $allowed );
	}
	...
	public static function localize_console_notices() {
		...
		mcms_localize_script( MCMSSEO_Admin_Asset_Manager::PREFIX . 'admin-global-script', 'mcmsseoConsoleNotifications', array_values( self::$console_notifications ) );
	}
	...
	/**
	 * First strip out registered and enclosing shortcodes using native MandarinCMS strip_shortcodes function.
	...
	public static function translate_score( $val, $css_value = true ) {
		$seo_rank = MCMSSEO_Rank::from_numeric_score( $val );
	...
	public static function sanitize_text_field( $value ) {
		$filtered = mcms_check_invalid_utf8( $value );

		if ( strpos( $filtered, '<' ) !== false ) {
			$filtered = mcms_pre_kses_less_than( $filtered );
			// This will strip extra whitespace for us.
			$filtered = mcms_strip_all_tags( $filtered, true );
		}
		else {
	...
	/**
	 * Clears the MCMS or W3TC cache depending on which is used
	 *
	 * @static
	 */
	public static function clear_cache() {
		...
		elseif ( function_exists( 'mcms_cache_clear_cache' ) ) {
	...
	public static function format_url( $url ) {
		...
		return apply_filters( 'mcmsseo_format_admin_url', $formatted_url );
	}
	 ...
	 *
	 * @param string $module Module path relative to modules directory.
	 *
	 * @return string|bool
	 */
	public static function get_module_name( $module ) {
		$module_details = get_module_data( MCMS_PLUGIN_DIR . '/' . $module );

		if ( $module_details['Name'] != '' ) {
			return $module_details['Name'];
		}
	...
	public static function get_title_separator() {
		$replacement = MCMSSEO_Options::get_default( 'mcmsseo_titles', 'separator' );

		// Get the titles option and the separator options.
		$titles_options    = MCMSSEO_Options::get_option( 'mcmsseo_titles' );
		$seperator_options = MCMSSEO_Option_Titles::get_instance()->get_separator_options();
		...
		/**
		 * Filter: 'mcmsseo_replacements_filter_sep' - Allow customization of the separator character(s)
		 *
		 * @api string $replacement The current separator
		 */
		return apply_filters( 'mcmsseo_replacements_filter_sep', $replacement );
	}

	/**
	 * Check if the current opened page is a Ultimatum SEO page.
	 *
	 * @return bool
	 */
	public static function is_ultimatum_seo_page() {
		static $is_ultimatum_seo;

		if ( $is_ultimatum_seo === null ) {
			$current_page = filter_input( INPUT_GET, 'page' );
			$is_ultimatum_seo = ( substr( $current_page, 0, 6 ) === 'mcmsseo_' );
		}

		return $is_ultimatum_seo;
	}

	/**
	 * Check if the current opened page belongs to Ultimatum SEO Free.
	 *
	 * @param string $current_page the current page the user is on.
	 *
	 * @return bool
	 */
	public static function is_ultimatum_seo_free_page( $current_page ) {
		$ultimatum_seo_free_pages = array(
			'mcmsseo_dashboard',
			'mcmsseo_titles',
			'mcmsseo_social',
			'mcmsseo_xml',
		...
		return in_array( $current_page, $ultimatum_seo_free_pages );
	}

	/**
	 * Determine if Ultimatum SEO is in development mode?
	 *
	...
	public static function is_development_mode() {
		...
		if ( defined( 'MCMSSEO_DEBUG' ) ) {
			$development_mode = MCMSSEO_DEBUG;
		}
		...
		/**
		 * Filter the Ultimatum SEO development mode.
		 *
		 * @since 3.0
		 *
		 * @param bool $development_mode Is Ultimatum SEOs development mode active.
		 */

		return apply_filters( 'ultimatum_seo_development_mode', $development_mode );
	}
	...
	/**
	 * Checks if the MCMS-REST-API is available.
	 *
	 * @since 3.6
	 *
	 ...
	 *
	 * @deprecated
	 * @see MCMSSEO_Sitemaps_Cache::register_clear_on_option_update()
	 *
	 * @param string $option Option name.
	 * @param string $type   Sitemap type.
	 */
	public static function register_cache_clear_option( $option, $type = '' ) {
		MCMSSEO_Sitemaps_Cache::register_clear_on_option_update( $option, $type );
	}
	 ...
	 *
	 * @deprecated
	 * @see MCMSSEO_Sitemaps_Cache::clear()
	 *
	 * @param array $types Set of sitemap types to invalidate cache for.
	 */
	public static function clear_sitemap_cache( $types = array() ) {
		MCMSSEO_Sitemaps_Cache::clear( $types );
	}

	/**
	 * Wrapper for encoding the array as a json string. Includes a fallback if mcms_json_encode doesn't exist.
	 *
	 * @deprecated 3.3 Core versions without mcms_json_encode() no longer supported, fallback unnecessary.
	 *
	...
	public static function json_encode( array $array_to_encode, $options = 0, $depth = 512 ) {
		return mcms_json_encode( $array_to_encode, $options, $depth );
	}
}
