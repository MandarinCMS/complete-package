...
/**
 * This class implements defaults and value validation for all MCMSSEO Post Meta values.
 *
 * Some guidelines:
 * - To update a meta value, you can just use update_post_meta() with the full (prefixed) meta key
 *        or the convenience method MCMSSEO_Meta::set_value() with the internal key.
 ...
 * - To retrieve a MCMSSEO meta value, you **must** use MCMSSEO_Meta::get_value() which will always return a
 *        string value, either the saved value or the default.
 *        This method can also retrieve a complete set of MCMSSEO meta values for one specific post, see
 ...
 *
 * @internal   all MCMS native get_meta() results get cached internally, so no need to cache locally.
 * @internal   use $key when the key is the MCMSSEO internal name (without prefix), $meta_key when it
 *             includes the prefix
 */
class MCMSSEO_Meta {

	/**
	 * @var    string    Prefix for all MCMSSEO meta values in the database
	 * @static
	 *
	 * @internal if at any point this would change, quite apart from an upgrade routine, this also will need to
	 * be changed in the mcmsml-config.xml file.
	 */
	public static $meta_prefix = '_ultimatum_mcmsseo_';


	/**
	 * @var    string   Prefix for all MCMSSEO meta value form field names and ids
	 * @static
	 */
	public static $form_prefix = 'ultimatum_mcmsseo_';
	...
	public static function init() {
		...
		/**
		 * Allow add-on modules to register their meta fields for management by this class
		 * add_filter() calls must be made before modules_loaded prio 14
		 */
		$extra_fields = apply_filters( 'add_extra_mcmsseo_meta_fields', array() );
		...
		foreach ( self::$meta_fields as $subset => $field_group ) {
			foreach ( $field_group as $key => $field_def ) {
				if ( $field_def['type'] !== 'snippetpreview' ) {
					/**
					 * Function register_meta() is undocumented and not used by MCMS internally, wrapped in
	...
	public static function get_meta_field_defs( $tab, $post_type = 'post' ) {
		...
		switch ( $tab ) {
			...
			case 'general':
				...
				if ( $options['usemetakeywords'] === true ) {
					...
					$field_defs['metakeywords']['description'] = sprintf( $field_defs['metakeywords']['description'], '<a target="_blank" href="' . esc_url( admin_url( 'admin.php?page=mcmsseo_titles#top#post_types' ) ) . '">', '</a>' );
				}
				else {
				...
				/**
				 * Filter the MCMSSEO metabox form field definitions for the general tab, backward compatibility
				 *
				 * @deprecated 1.5.0
				 * @deprecated use the 'mcmsseo_metabox_entries_general' filter instead
				 ...
				 * @return     array
				 */
				$field_defs = apply_filters( 'mcmsseo_metabox_entries', $field_defs );
				break;
			...
			case 'advanced':
				...
				$options = MCMSSEO_Options::get_options( array( 'mcmsseo', 'mcmsseo_titles', 'mcmsseo_internallinks' ) );
				...
				if ( $options['noodp'] !== false ) {
					...
					$robots_adv = __( 'None', 'mandarincms-seo' );
				}
				...
				if ( $options['breadcrumbs-enable'] !== true && ! current_myskin_supports( 'ultimatum-seo-breadcrumbs' ) ) {
		...
		/**
		 * Filter the MCMSSEO metabox form field definitions for a tab
		...
		return apply_filters( 'mcmsseo_metabox_entries_' . $tab, $field_defs, $post_type );
	}
	...
	public static function sanitize_post_meta( $meta_value, $meta_key ) {
		...
		switch ( true ) {
			case ( $meta_key === self::$meta_prefix . 'linkdex' ):
				$int = MCMSSEO_Utils::validate_int( $meta_value );
				if ( $int !== false && $int >= 0 ) {
			...
			case ( $field_def['type'] === 'text' && $meta_key === self::$meta_prefix . 'redirect' ):
				...
				$url = MCMSSEO_Utils::sanitize_url( $meta_value );
				if ( $url !== '' ) {
					$clean = $url;
				}
				break;
			...
			case ( $field_def['type'] === 'upload' && $meta_key === self::$meta_prefix . 'opengraph-image' ):
				// Validate as url.
				$url = MCMSSEO_Utils::sanitize_url( $meta_value, array( 'http', 'https', 'ftp', 'ftps' ) );
				if ( $url !== '' ) {
					$clean = $url;
				}
				break;
			...
			case ( $field_def['type'] === 'textarea' ):
				if ( is_string( $meta_value ) ) {
					...
					// @todo [JRF => Ultimatum] verify that line breaks and the likes aren't allowed/recommended in meta header fields.
					$meta_value = str_replace( array( "\n", "\r", "\t", '  ' ), ' ', $meta_value );
					$clean      = MCMSSEO_Utils::sanitize_text_field( trim( $meta_value ) );
				}
				break;
		...
		$clean = apply_filters( 'mcmsseo_sanitize_post_meta_' . $meta_key, $clean, $meta_value, $field_def, $meta_key );
	 ...
	 *
	 * @todo [JRF => Ultimatum] Verify that this logic for the prioritisation is correct
	 *
	 * @static
	 *
	 ...
	 *                        Objects and arrays are possible, but not used by this module
	 *                        and therefore discarted (except when the special 'serialized' field def
	 *                        value is set to true - only used by add-on modules for now).
	 ...
	 * where no MCMSSEO meta data has been set.
	 * Optionally deletes the $old_metakey values.
	 *
	 * @static
	 *
	 * @param  string $old_metakey The old key of the meta value.
	 * @param  string $new_metakey The new key, usually the MCMSSEO meta key (including prefix).
	...
	public static function replace_meta( $old_metakey, $new_metakey, $delete_old = false ) {
		...
		/*
		Get only those rows where no mcmsseo meta values exist for the same post
		...
		*/
		$query  = $mcmsdb->prepare(
			"
				SELECT `a`.*
				FROM {$mcmsdb->postmeta} AS a
				WHERE `a`.`meta_key` = %s
					AND NOT	EXISTS (
						SELECT DISTINCT `post_id` , count( `meta_id` ) AS count
						FROM {$mcmsdb->postmeta} AS b
				...
				;",
			$old_metakey,
			$mcmsdb->esc_like( self::$meta_prefix . '%' ),
			self::$meta_prefix . 'linkdex'
		);
		$oldies = $mcmsdb->get_results( $query );
	...
	public static function clean_up() {
		...
		/**
		 * Clean up '_ultimatum_mcmsseo_meta-robots'
		 *
		 * Retrieve all '_ultimatum_mcmsseo_meta-robots' meta values and convert if no new values found
		 *
		 * @internal Query is pretty well optimized this way
		 *
		 * @todo [JRF => Ultimatum] find out all possible values which the old '_ultimatum_mcmsseo_meta-robots' could contain
		...
		$query    = "SELECT meta_id FROM {$mcmsdb->postmeta} WHERE " . implode( ' OR ', $query ) . ';';
		$meta_ids = $mcmsdb->get_col( $query );

		if ( is_array( $meta_ids ) && $meta_ids !== array() ) {
			// MCMS native action.
			do_action( 'delete_post_meta', $meta_ids, null, null, null );

			$query = "DELETE FROM {$mcmsdb->postmeta} WHERE meta_id IN( " . implode( ',', $meta_ids ) . ' )';
			$count = $mcmsdb->query( $query );

			if ( $count ) {
				foreach ( $meta_ids as $object_id ) {
					mcms_cache_delete( $object_id, 'post_meta' );
				}
		 ...
		 */
		$query  = $mcmsdb->prepare(
			"SELECT meta_id, meta_value FROM {$mcmsdb->postmeta} WHERE meta_key = %s",
		...
		do_action( 'mcmsseo_meta_clean_up' );
	}
