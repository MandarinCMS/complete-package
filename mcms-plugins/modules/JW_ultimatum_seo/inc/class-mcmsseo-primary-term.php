...
class MCMSSEO_Primary_Term {
	...
	public function get_primary_term() {
		$primary_term = get_post_meta( $this->post_ID, MCMSSEO_Meta::$meta_prefix . 'primary_' . $this->taxonomy_name, true );

		$terms = $this->get_terms();

		if ( ! in_array( $primary_term, mcms_list_pluck( $terms, 'term_id' ) ) ) {
			$primary_term = false;
		}
	...
	public function set_primary_term( $new_primary_term ) {
		update_post_meta( $this->post_ID, MCMSSEO_Meta::$meta_prefix . 'primary_' . $this->taxonomy_name, $new_primary_term );
	}
