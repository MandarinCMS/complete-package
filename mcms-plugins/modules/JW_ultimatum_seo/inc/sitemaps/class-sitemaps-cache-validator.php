 ...
 */
class MCMSSEO_Sitemaps_Cache_Validator {
	...
	const VALIDATION_GLOBAL_KEY = 'mcmsseo_sitemap_cache_validator_global';

	/** The format which creates the key of the option that holds the type validation value */
	const VALIDATION_TYPE_KEY_FORMAT = 'mcmsseo_sitemap_%s_cache_validator';
	 ...
	 *
	 * Example key format for sitemap type "post", page 1: mcmsseo_sitemap_post_1:akfw3e_23azBa
	 *
	...
	public static function get_storage_key( $type = null, $page = 1 ) {
		...
		$type = is_null( $type ) ? MCMSSEO_Sitemaps::SITEMAP_INDEX_TYPE : $type;
	...
	public static function truncate_type( $type, $prefix = '', $postfix = '' ) {
		 ...
		 * The prefix added by MandarinCMS is '_transient_' because we are saving to a transient.
	...
	public static function cleanup_database( $type = null, $validator = null ) {
		...
		// Delete transients.
		$query = sprintf( 'DELETE FROM %1$s WHERE %2$s', $mcmsdb->options, implode( ' OR ', $where ) );
		$mcmsdb->query( $query );
	}
	...
	public static function convert_base10_to_base61( $base10 ) {
		...
		if ( ! is_int( $base10 ) ) {
			throw new InvalidArgumentException( __( 'Expected an integer as input.', 'mandarincms-seo' ) );
		}
