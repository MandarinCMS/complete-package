 ...
 */
class MCMSSEO_Sitemaps_Renderer {
	...
	public function get_index( $links ) {
		 ...
		 */
		$xml .= apply_filters( 'mcmsseo_sitemap_index', '' );
		$xml .= '</sitemapindex>';
	...
	public function get_sitemap( $links, $type, $current_page ) {
		 ...
		 */
		$xml = apply_filters( "mcmsseo_sitemap_{$type}_urlset", $urlset );
		 ...
		 */
		if ( $current_page === 1 ) {
			$xml .= apply_filters( "mcmsseo_sitemap_{$type}_content", '' );
		}
	...
	public function get_output( $sitemap, $transient ) {
		...
		if ( $this->stylesheet ) {
			 ...
			 */
			$output .= apply_filters( 'mcmsseo_stylesheet_url', $this->stylesheet ) . "\n";
		}

		$output .= $sitemap;
		$output .= "\n<!-- XML Sitemap generated by Ultimatum SEO -->";

		$debug = MCMS_DEBUG || ( defined( 'MCMSSEO_DEBUG' ) && true === MCMSSEO_DEBUG );

		if ( ! MCMS_DEBUG_DISPLAY || ! $debug ) {
			return $output;
		}

		$memory_used = number_format( ( memory_get_peak_usage() / 1048576 ), 2 );
		$queries_run = ( $transient ) ? 'Served from transient cache' : 'Queries executed ' . absint( $GLOBALS['mcmsdb']->num_queries );

		$output .= "\n<!-- {$memory_used}MB | {$queries_run} -->";

		if ( defined( 'SAVEQUERIES' ) && SAVEQUERIES ) {

			$queries = print_r( $GLOBALS['mcmsdb']->queries, true );
			$output .= "\n<!-- {$queries} -->";
		}
	...
	public function sitemap_url( $url ) {
		...
		foreach ( $url['images'] as $img ) {
			...
			if ( ! empty( $img['title'] ) ) {
				...
				$title = _mcms_specialchars( html_entity_decode( $title, ENT_QUOTES, $this->output_charset ) );
			...
			if ( ! empty( $img['alt'] ) ) {
				...
				$alt = _mcms_specialchars( html_entity_decode( $alt, ENT_QUOTES, $this->output_charset ) );
		 ...
		 */
		return apply_filters( 'mcmsseo_sitemap_url', $output, $url );
	}
