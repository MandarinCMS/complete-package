...
class MCMSSEO_Sitemaps {
	...
	/** @var MCMSSEO_Sitemap_Timezone $timezone */
	private $timezone;

	/** @var MCMSSEO_Sitemaps_Router $router */
	public $router;

	/** @var MCMSSEO_Sitemaps_Renderer $renderer */
	public $renderer;

	/** @var MCMSSEO_Sitemaps_Cache $cache */
	public $cache;

	/** @var MCMSSEO_Sitemap_Provider[] $providers */
	public $providers;
	...
	public function __construct() {
		...
		add_action( 'after_setup_myskin', array( $this, 'reduce_query_load' ), 99 );
		add_action( 'pre_get_posts', array( $this, 'redirect' ), 1 );
		add_action( 'mcmsseo_hit_sitemap_index', array( $this, 'hit_sitemap_index' ) );
		add_action( 'mcmsseo_ping_search_engines', array( __CLASS__, 'ping_search_engines' ) );

		$options           = MCMSSEO_Options::get_all();
		$this->max_entries = $options['entries-per-page'];
		$this->timezone    = new MCMSSEO_Sitemap_Timezone();
		$this->router      = new MCMSSEO_Sitemaps_Router();
		$this->renderer    = new MCMSSEO_Sitemaps_Renderer();
		$this->cache       = new MCMSSEO_Sitemaps_Cache();
		$this->providers   = array( // TODO API for add/remove. R.
			new MCMSSEO_Post_Type_Sitemap_Provider(),
			new MCMSSEO_Taxonomy_Sitemap_Provider(),
			new MCMSSEO_Author_Sitemap_Provider(),
		);
	...
	public function register_sitemap( $name, $function, $rewrite = '' ) {
		add_action( 'mcmsseo_do_sitemap_' . $name, $function );
		if ( ! empty( $rewrite ) ) {
	...
	public function register_xsl( $name, $function, $rewrite = '' ) {
		add_action( 'mcmsseo_xsl_' . $name, $function );
		if ( ! empty( $rewrite ) ) {
	...
	/**
	 * Set the sitemap current page to allow creating partial sitemaps with mcms-cli
	...
	/**
	 * Prevent stupid modules from running shutdown scripts when we're obviously not outputting HTML.
	 *
	 * @since 1.4.16
	 */
	public function sitemap_close() {
		remove_all_actions( 'mcms_footer' );
		die();
	}
	 ...
	 *
	 * @param \MCMS_Query $query Main query instance.
	 */
	...
	private function get_sitemap_from_cache( $type, $page_number ) {
		 ...
		 *
		 * @param MCMSSEO_Sitemaps $sitemaps Sitemaps object.
		 */
		do_action( 'mcmsseo_sitemap_stylesheet_cache_' . $type, $this );
	...
	public function build_sitemap( $type ) {
		 ...
		 */
		$type = apply_filters( 'mcmsseo_build_sitemap_post_type', $type );
		...
		if ( has_action( 'mcmsseo_do_sitemap_' . $type ) ) {
			/**
			 * Fires custom handler, if hooked to generate sitemap for the type.
			 */
			do_action( 'mcmsseo_do_sitemap_' . $type );
	...
	public function xsl_output( $type ) {
		...
		require_once( MCMSSEO_PATH . 'css/xml-sitemap-xsl.php' );
	}
	...
	public function hit_sitemap_index() {
		mcms_remote_get( MCMSSEO_Sitemaps_Router::get_base_url( 'sitemap_index.xml' ) );
	}
	...
	static public function get_last_modified_gmt( $post_types, $return_all = false ) {
		...
		if ( is_null( $post_type_dates ) ) {
			...
			foreach ( $mcmsdb->get_results( $sql ) as $obj ) {
	...
	public static function ping_search_engines( $url = null ) {
		...
		if ( empty( $url ) ) {
			$url = urlencode( MCMSSEO_Sitemaps_Router::get_base_url( 'sitemap_index.xml' ) );
		}

		// Ping Google and Bing.
		mcms_remote_get( 'http://www.google.com/webmasters/tools/ping?sitemap=' . $url, array( 'blocking' => false ) );
		mcms_remote_get( 'http://www.bing.com/ping?sitemap=' . $url, array( 'blocking' => false ) );
	}
	 ...
	 *
	 * @param string $filter  Expands to mcmsseo_sitemap_$filter_change_freq, allowing for a change of the frequency for
	...
	static public function filter_frequency( $filter, $default, $url ) {
		 ...
		 */
		$change_freq = apply_filters( 'mcmsseo_sitemap_' . $filter . '_change_freq', $default, $url );
