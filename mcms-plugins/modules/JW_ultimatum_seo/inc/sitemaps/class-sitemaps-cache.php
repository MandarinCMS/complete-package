...
class MCMSSEO_Sitemaps_Cache {
	...
	public function is_enabled() {
		 ...
		 */
		return apply_filters( 'mcmsseo_enable_xml_sitemap_transient_caching', true );
	}
	...
	public function get_sitemap( $type, $page ) {
		...
		$transient_key = MCMSSEO_Sitemaps_Cache_Validator::get_storage_key( $type, $page );
	 ...
	 * @param int    $page Page number to retrieve.
	 *
	 * @return null|MCMSSEO_Sitemap_Cache_Data Null on no cache found otherwise object containing sitemap and meta data.
	 */
	public function get_sitemap_data( $type, $page ) {
		...
		if ( is_string( $sitemap ) && 0 === strpos( $sitemap, 'C:24:"MCMSSEO_Sitemap_Cache_Data"' ) ) {

			$sitemap = unserialize( $sitemap );
		}

		// What we expect it to be if it is set.
		if ( $sitemap instanceof MCMSSEO_Sitemap_Cache_Data_Interface ) {
			return $sitemap;
		}
	...
	public function store_sitemap( $type, $page, $sitemap, $usable = true ) {
		...
		$status = ( $usable ) ? MCMSSEO_Sitemap_Cache_Data::OK : MCMSSEO_Sitemap_Cache_Data::ERROR;

		$sitemap_data = new MCMSSEO_Sitemap_Cache_Data();
	...
	public static function clear( $types = array() ) {
		...
		if ( ! in_array( MCMSSEO_Sitemaps::SITEMAP_INDEX_TYPE, $types ) ) {
			array_unshift( $types, MCMSSEO_Sitemaps::SITEMAP_INDEX_TYPE );
		}
	...
	public static function clear_queued() {
		...
		if ( self::$clear_all ) {
			...
			MCMSSEO_Sitemaps_Cache_Validator::invalidate_storage();
		...
		foreach ( self::$clear_types as $type ) {
			MCMSSEO_Sitemaps_Cache_Validator::invalidate_storage( $type );
		}
