 ...
 */
class MCMSSEO_Post_Type_Sitemap_Provider implements MCMSSEO_Sitemap_Provider {

	/** @var string $home_url Holds the home_url() value to speed up loops. */
	protected static $home_url;

	/** @var array $options All of module options. */
	protected static $options;

	/** @var MCMSSEO_Sitemap_Image_Parser $image_parser Holds image parser instance. */
	 ...
	 *
	 * @return MCMSSEO_Sitemap_Image_Parser
	 */
	protected function get_image_parser() {
		if ( ! isset( self::$image_parser ) ) {
			self::$image_parser = new MCMSSEO_Sitemap_Image_Parser();
		}
	 ...
	 * Get Home URL
	 *
	 * This has been moved from the constructor because mcms_rewrite is not available on modules_loaded in multisite.
	...
	protected function get_home_url() {
		if ( ! isset( self::$home_url ) ) {
			self::$home_url = MCMSSEO_Utils::home_url();
		}
	...
	protected function get_options() {
		if ( ! isset( self::$options ) ) {
			self::$options = MCMSSEO_Options::get_all();
		}
	...
	public function get_index_links( $max_entries ) {
		...
		$last_modified_times = MCMSSEO_Sitemaps::get_last_modified_gmt( $post_types, true );
		$index               = array();
		...
		foreach ( $post_types as $post_type ) {
			...
			if ( $max_pages > 1 ) {
				...
				SELECT post_modified_gmt
				    ...
				    JOIN {$mcmsdb->posts} USE INDEX( type_status_date )
				...
				$all_dates = $mcmsdb->get_col( $mcmsdb->prepare( $sql, $post_type, $max_entries ) );
			}
			...
			for ( $page_counter = 0; $page_counter < $max_pages; $page_counter++ ) {
				...
				$index[] = array(
					'loc'     => MCMSSEO_Sitemaps_Router::get_base_url( $post_type . '-sitemap' . $current_page . '.xml' ),
					'lastmod' => $date,
				);
			}
		}
	...
	public function get_sitemap_links( $type, $max_entries, $current_page ) {
		...
		while ( $total > $offset ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( MCMSSEO_Meta::get_value( 'meta-robots-noindex', $post->ID ) === '1' ) {
					continue;
				}
				 ...
				 */
				$url = apply_filters( 'mcmsseo_sitemap_entry', $url, 'post', $post );
	...
	public function is_valid_post_type( $post_type ) {
		 ...
		 */
		if ( apply_filters( 'mcmsseo_sitemap_exclude_post_type', false, $post_type ) ) {
			return false;
		}
	...
	protected function get_post_type_count( $post_type ) {
		 ...
		 * @param string $post_type Post type name.
		 */
		$join_filter = apply_filters( 'mcmsseo_typecount_join', '', $post_type );
		 ...
		 * @param string $post_type Post type name.
		 */
		$where_filter = apply_filters( 'mcmsseo_typecount_where', '', $post_type );

		$where = $this->get_sql_where_clause( $post_type );

		$sql   = "
			SELECT COUNT({$mcmsdb->posts}.ID)
			FROM {$mcmsdb->posts}
			{$join_filter}
			{$where}
				{$where_filter}
	...
	protected function get_first_links( $post_type ) {
		...
		/**
		 * Filter the URL Ultimatum SEO uses in the XML sitemap for this post type archive.
		 *
		 * @param string $archive_url The URL of this archive
		 * @param string $post_type   The post type this archive is for.
		 */
		$archive_url = apply_filters( 'mcmsseo_sitemap_post_type_archive_link', $archive_url, $post_type );

		if ( $archive_url ) {
			/**
			 * Filter the priority of the URL Ultimatum SEO uses in the XML sitemap.
			 *
			...
			$links[] = array(
				'loc' => $archive_url,
				'mod' => MCMSSEO_Sitemaps::get_last_modified_gmt( $post_type ),
	...
	protected function get_posts( $post_type, $count, $offset ) {
		...
		if ( ! isset( $filters[ $post_type ] ) ) {
			// Make sure you're mcmsdb->preparing everything you throw into this!!
			$filters[ $post_type ] = array(
				 ...
				 * @param string $post_type Post type name.
				 */
				'join'  => apply_filters( 'mcmsseo_posts_join', false, $post_type ),
				 ...
				 * @param string $post_type Post type name.
				 */
				'where' => apply_filters( 'mcmsseo_posts_where', false, $post_type ),
			);
		}
		...
		// Optimized query per this thread: http://mandarincms.com/support/topic/module-mandarincms-seo-by-ultimatum-performance-suggestion.
		...
		$sql = "
			...
			FROM (
				SELECT {$mcmsdb->posts}.ID
				FROM {$mcmsdb->posts}
				{$join_filter}
				{$where}
					{$where_filter}
				ORDER BY {$mcmsdb->posts}.post_modified ASC LIMIT %d OFFSET %d
			)
			o JOIN {$mcmsdb->posts} l ON l.ID = o.ID
		";

		$posts = $mcmsdb->get_results( $mcmsdb->prepare( $sql, $count, $offset ) );
	...
	protected function get_sql_where_clause( $post_type ) {
		...
		$join   = '';
		$status = "{$mcmsdb->posts}.post_status = 'publish'";

		// Based on MCMS_Query->get_posts(). R.
		...
		{$join}
		WHERE {$status}
			AND {$mcmsdb->posts}.post_type = '%s'
			AND {$mcmsdb->posts}.post_password = ''
			AND {$mcmsdb->posts}.post_date != '0000-00-00 00:00:00'
		";

		return $mcmsdb->prepare( $where_clause, $post_type );
	}
	...
	protected function get_url( $post ) {
		...
		/**
		 * Filter the URL Ultimatum SEO uses in the XML sitemap.
		 *
		 ...
		 */
		$url['loc'] = apply_filters( 'mcmsseo_xml_sitemap_post_url', get_permalink( $post ), $post );

		/**
		 * Do not include external URLs.
		 *
		 * @see https://mandarincms.com/modules/page-links-to/ can rewrite permalinks to external URLs.
		 */
		...
		$canonical = MCMSSEO_Meta::get_value( 'canonical', $post->ID );
	...
	private function calculate_priority( $post ) {
		 ...
		 */
		$return = apply_filters( 'mcmsseo_xml_sitemap_post_priority', $return, $post->post_type, $post );
