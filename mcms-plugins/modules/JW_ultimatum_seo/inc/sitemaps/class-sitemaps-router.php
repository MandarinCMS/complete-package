...
class MCMSSEO_Sitemaps_Router {
	...
	public function init() {
		...
		$mcms->add_query_var( 'sitemap' );
		$mcms->add_query_var( 'sitemap_n' );
		$mcms->add_query_var( 'xsl' );
	...
	public function template_redirect() {
		...
		if ( home_url( '/sitemap.xml' ) === $current_url && $mcms_query->is_404 ) {
	...
	public static function get_base_url( $page ) {
		...
		$base = $mcms_rewrite->using_index_permalinks() ? 'index.php/' : '/';
		 ...
		 */
		$base = apply_filters( 'mcmsseo_sitemaps_base_url', $base );

		// Get the scheme from the configured home url instead of letting MandarinCMS determine the scheme based on the requested URI.
