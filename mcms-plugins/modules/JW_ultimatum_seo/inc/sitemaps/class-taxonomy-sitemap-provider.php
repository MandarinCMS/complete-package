 ...
 */
class MCMSSEO_Taxonomy_Sitemap_Provider implements MCMSSEO_Sitemap_Provider {
	...
	public function get_index_links( $max_entries ) {
		 ...
		 */
		$hide_empty = apply_filters( 'mcmsseo_sitemap_exclude_empty_terms', true, $taxonomy_names );
		...
		foreach ( $taxonomies as $tax_name => $tax ) {
			...
			$last_modified_gmt = MCMSSEO_Sitemaps::get_last_modified_gmt( $tax->object_type );
			...
			for ( $page_counter = 0; $page_counter < $max_pages; $page_counter++ ) {
				...
				$index[] = array(
					'loc'     => MCMSSEO_Sitemaps_Router::get_base_url( $tax_name . '-sitemap' . $current_page . '.xml' ),
					'lastmod' => $date,
				);
			}
		}
	...
	public function get_sitemap_links( $type, $max_entries, $current_page ) {
		...
		$hide_empty = apply_filters( 'mcmsseo_sitemap_exclude_empty_terms', true, $taxonomy );
		...
		$sql = "
			...
			FROM	$mcmsdb->posts AS p
			INNER JOIN $mcmsdb->term_relationships AS term_rel
				ON		term_rel.object_id = p.ID
			INNER JOIN $mcmsdb->term_taxonomy AS term_tax
		...
		foreach ( $terms as $term ) {
			...
			$tax_noindex     = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'noindex' );
			$tax_sitemap_inc = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'sitemap_include' );
			...
			$url['loc'] = MCMSSEO_Taxonomy_Meta::get_term_meta( $term, $term->taxonomy, 'canonical' );
			...
			$url['mod']    = $mcmsdb->get_var( $mcmsdb->prepare( $sql, $term->taxonomy, $term->term_id ) );
			...
			$url = apply_filters( 'mcmsseo_sitemap_entry', $url, 'term', $term );
	...
	public function is_valid_taxonomy( $taxonomy_name ) {
		 ...
		 */
		if ( apply_filters( 'mcmsseo_sitemap_exclude_taxonomy', false, $taxonomy_name ) ) {
			return false;
		}
