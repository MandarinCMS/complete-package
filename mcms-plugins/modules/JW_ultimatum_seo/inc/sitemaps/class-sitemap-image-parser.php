 ...
 */
class MCMSSEO_Sitemap_Image_Parser {
	...
	public function get_images( $post ) {
		...
		if ( $thumbnail_id ) {
			...
			$alt      = get_post_meta( $thumbnail_id, '_mcms_attachment_image_alt', true );
		...
		if ( 'attachment' === $post->post_type && mcms_attachment_is_image( $post ) ) {
		 ...
		 */
		$images = apply_filters( 'mcmsseo_sitemap_urlimages', $images, $post->ID );
	...
	private function parse_html_images( $content ) {
		...
		foreach ( $post_dom->getElementsByTagName( 'img' ) as $img ) {
			...
			if ( // This detects MCMS-inserted images, which we need to upsize. R.
				! empty( $class )
				&& false === strpos( $class, 'size-full' )
				&& preg_match( '|mcms-image-(?P<id>\d+)|', $class, $matches )
	 ...
	 *
	 * @param MCMS_Post $post  Post object for the context.
	...
	protected function get_image_item( $post, $src, $title = '', $alt = '' ) {
		 ...
		 */
		$image['src'] = apply_filters( 'mcmsseo_xml_sitemap_img_src', $src, $post );
		 ...
		 * @param object $post  Post object.
		 */
		return apply_filters( 'mcmsseo_xml_sitemap_img', $image, $post );
	}
	...
	private function image_url( $post_id ) {
		...
		$file = get_post_meta( $post_id, '_mcms_attached_file', true );
		...
		if ( false !== strpos( $file, 'mcms-content/uploads' ) ) {
			return $uploads['baseurl'] . substr( $file, ( strpos( $file, 'mcms-content/uploads' ) + 18 ) );
		}
	...
	protected function get_absolute_url( $src ) {
		...
		if ( MCMSSEO_Utils::is_url_relative( $src ) === true ) {
