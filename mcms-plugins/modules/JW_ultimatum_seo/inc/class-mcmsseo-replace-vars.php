...
class MCMSSEO_Replace_Vars {
	...
	/**
	 * @var    array    Help texts for use in MCMSSEO -> Titles and Meta's help tabs
	 */
	protected static $help_texts = array();

	/**
	 * @var array    Register of additional variable replacements registered by other modules/myskins
	 */
	...
	public static function setup_statics_once() {
		...
		if ( self::$external_replacements === array() ) {
			/**
			 * Action: 'mcmsseo_register_extra_replacements' - Allows for registration of additional
			 * variables to replace
			 */
			do_action( 'mcmsseo_register_extra_replacements' );
		}
	}
	 ...
	 * For use by other modules/myskins to register extra variables
	 *
	 * @see mcmsseo_register_var_replacement() for a usage example
	 *
	...
	public static function register_replacement( $var, $replace_function, $type = 'advanced', $help_text = '' ) {
		...
		if ( is_string( $var ) && $var !== '' ) {
			...
			if ( preg_match( '`^[A-Z0-9_-]+$`i', $var ) === false ) {
				trigger_error( __( 'A replacement variable can only contain alphanumeric characters, an underscore or a dash. Try renaming your variable.', 'mandarincms-seo' ), E_USER_WARNING );
			}
			elseif ( strpos( $var, 'cf_' ) === 0 || strpos( $var, 'ct_' ) === 0 ) {
				trigger_error( __( 'A replacement variable can not start with "%%cf_" or "%%ct_" as these are reserved for the MCMSSEO standard variable variables for custom fields and custom taxonomies. Try making your variable name unique.', 'mandarincms-seo' ), E_USER_WARNING );
			}
			elseif ( ! method_exists( __CLASS__, 'retrieve_' . $var ) ) {
				if ( ! isset( self::$external_replacements[ $var ] ) ) {
					...
					trigger_error( __( 'A replacement variable with the same name has already been registered. Try making your variable name more unique.', 'mandarincms-seo' ), E_USER_WARNING );
				}
			}
			else {
				trigger_error( __( 'You cannot overrule a MCMSSEO standard variable replacement by registering a variable with the same name. Use the "mcmsseo_replacements" filter instead to adjust the replacement value.', 'mandarincms-seo' ), E_USER_WARNING );
			}
		}
	...
	public function replace( $string, $args, $omit = array() ) {
		...
		if ( isset( $args['post_content'] ) && ! empty( $args['post_content'] ) ) {
			$args['post_content'] = MCMSSEO_Utils::strip_shortcode( $args['post_content'] );
		}
		if ( isset( $args['post_excerpt'] ) && ! empty( $args['post_excerpt'] ) ) {
			$args['post_excerpt'] = MCMSSEO_Utils::strip_shortcode( $args['post_excerpt'] );
		}
		$this->args = (object) mcms_parse_args( $args, $this->defaults );
		...
		/**
		 * Filter: 'mcmsseo_replacements' - Allow customization of the replacements before they are applied
		 *
		 * @api array $replacements The replacements
		 */
		$replacements = apply_filters( 'mcmsseo_replacements', $replacements );
		...
		/**
		 * Filter: 'mcmsseo_replacements_final' - Allow overruling of whether or not to remove placeholders
		 * which didn't yield a replacement
		 *
		 * @example <code>add_filter( 'mcmsseo_replacements_final', '__return_false' );</code>
		 *
		 * @api     bool $final
		 */
		if ( apply_filters( 'mcmsseo_replacements_final', true ) === true && ( isset( $matches[1] ) && is_array( $matches[1] ) ) ) {
		...
		$string = MCMSSEO_Utils::standardize_whitespace( $string );
	...
	private function retrieve_excerpt() {
		...
		if ( ! empty( $this->args->ID ) ) {
			...
			elseif ( $this->args->post_content !== '' ) {
				$replacement = mcms_html_excerpt( strip_shortcodes( $this->args->post_content ), 155 );
			}
		}
	...
	private function retrieve_sep() {
		return MCMSSEO_Utils::get_title_separator();
	}
	...
	private function retrieve_sitename() {
		...
		if ( ! isset( $replacement ) ) {
			$sitename = MCMSSEO_Utils::get_site_name();
			if ( $sitename !== '' ) {
	...
	private function retrieve_primary_category() {
		...
		if ( ! empty( $this->args->ID ) ) {
			$mcmsseo_primary_category = new MCMSSEO_Primary_Term( 'category', $this->args->ID );

			$term_id = $mcmsseo_primary_category->get_primary_term();
	...
	private function determine_pagenumbering( $request = 'nr' ) {
		...
		if ( ! is_singular() ) {
			...
			if ( isset( $mcms_query->max_num_pages ) && ( $mcms_query->max_num_pages != '' && $mcms_query->max_num_pages != 0 ) ) {
	...
	private function determine_pt_names( $request = 'single' ) {
		...
		if ( isset( $mcms_query->query_vars['post_type'] ) && ( ( is_string( $mcms_query->query_vars['post_type'] ) && $mcms_query->query_vars['post_type'] !== '' ) || ( is_array( $mcms_query->query_vars['post_type'] ) && $mcms_query->query_vars['post_type'] !== array() ) ) ) {
			$post_type = $mcms_query->query_vars['post_type'];
		}
		else {
			// Make it work in preview mode.
			$post_type = $mcms_query->get_queried_object()->post_type;
		}
	...
	private function retrieve_focuskw() {
		...
		if ( ! empty( $this->args->ID ) ) {
			$focus_kw = MCMSSEO_Meta::get_value( 'focuskw', $this->args->ID );
			if ( $focus_kw !== '' ) {
	...
	private function retrieve_page() {
		...
		if ( $max > 1 && $nr > 1 ) {
			$replacement = sprintf( $sep . ' ' . __( 'Page %1$d of %2$d', 'mandarincms-seo' ), $nr, $max );
		}
	...
	/**
	 * Set the help text for a user/module/myskin defined extra variable.
	 *
	...
	/**
	 * Set/translate the help texts for the MCMSSEO standard basic variables.
	 */
	private static function set_basic_help_texts() {
		self::$help_texts['basic'] = array(
			'date'                 => __( 'Replaced with the date of the post/page', 'mandarincms-seo' ),
			'title'                => __( 'Replaced with the title of the post/page', 'mandarincms-seo' ),
			'parent_title'         => __( 'Replaced with the title of the parent page of the current page', 'mandarincms-seo' ),
			'sitename'             => __( 'The site\'s name', 'mandarincms-seo' ),
			'sitedesc'             => __( 'The site\'s tag line / description', 'mandarincms-seo' ),
			'excerpt'              => __( 'Replaced with the post/page excerpt (or auto-generated if it does not exist)', 'mandarincms-seo' ),
			'excerpt_only'         => __( 'Replaced with the post/page excerpt (without auto-generation)', 'mandarincms-seo' ),
			'tag'                  => __( 'Replaced with the current tag/tags', 'mandarincms-seo' ),
			'category'             => __( 'Replaced with the post categories (comma separated)', 'mandarincms-seo' ),
			'primary_category'     => __( 'Replaced with the primary category of the post/page', 'mandarincms-seo' ),
			'category_description' => __( 'Replaced with the category description', 'mandarincms-seo' ),
			'tag_description'      => __( 'Replaced with the tag description', 'mandarincms-seo' ),
			'term_description'     => __( 'Replaced with the term description', 'mandarincms-seo' ),
			'term_title'           => __( 'Replaced with the term name', 'mandarincms-seo' ),
			'searchphrase'         => __( 'Replaced with the current search phrase', 'mandarincms-seo' ),
			'sep'                  => sprintf(
				/* translators: %s: mcms_title() function */
				__( 'The separator defined in your myskin\'s %s tag.', 'mandarincms-seo' ),
				'<code>mcms_title()</code>'
			),
		);
	}

	/**
	 * Set/translate the help texts for the MCMSSEO standard advanced variables.
	 */
	private static function set_advanced_help_texts() {
		self::$help_texts['advanced'] = array(
			'pt_single'                 => __( 'Replaced with the post type single label', 'mandarincms-seo' ),
			'pt_plural'                 => __( 'Replaced with the post type plural label', 'mandarincms-seo' ),
			'modified'                  => __( 'Replaced with the post/page modified time', 'mandarincms-seo' ),
			'id'                        => __( 'Replaced with the post/page ID', 'mandarincms-seo' ),
			'name'                      => __( 'Replaced with the post/page author\'s \'nicename\'', 'mandarincms-seo' ),
			'user_description'          => __( 'Replaced with the post/page author\'s \'Biographical Info\'', 'mandarincms-seo' ),
			'userid'                    => __( 'Replaced with the post/page author\'s userid', 'mandarincms-seo' ),
			'currenttime'               => __( 'Replaced with the current time', 'mandarincms-seo' ),
			'currentdate'               => __( 'Replaced with the current date', 'mandarincms-seo' ),
			'currentday'                => __( 'Replaced with the current day', 'mandarincms-seo' ),
			'currentmonth'              => __( 'Replaced with the current month', 'mandarincms-seo' ),
			'currentyear'               => __( 'Replaced with the current year', 'mandarincms-seo' ),
			'page'                      => __( 'Replaced with the current page number with context (i.e. page 2 of 4)', 'mandarincms-seo' ),
			'pagetotal'                 => __( 'Replaced with the current page total', 'mandarincms-seo' ),
			'pagenumber'                => __( 'Replaced with the current page number', 'mandarincms-seo' ),
			'caption'                   => __( 'Attachment caption', 'mandarincms-seo' ),
			'focuskw'                   => __( 'Replaced with the posts focus keyword', 'mandarincms-seo' ),
			'term404'                   => __( 'Replaced with the slug which caused the 404', 'mandarincms-seo' ),
			'cf_<custom-field-name>'    => __( 'Replaced with a posts custom field value', 'mandarincms-seo' ),
			'ct_<custom-tax-name>'      => __( 'Replaced with a posts custom taxonomies, comma separated.', 'mandarincms-seo' ),
			'ct_desc_<custom-tax-name>' => __( 'Replaced with a custom taxonomies description', 'mandarincms-seo' ),
		);
	}
	...
	public function get_terms( $id, $taxonomy, $return_single = false ) {
		 ...
		 */
		return apply_filters( 'mcmsseo_terms', $output );
	}
} /* End of class MCMSSEO_Replace_Vars */


/**
 * Setup the class statics when the file is first loaded
 */
MCMSSEO_Replace_Vars::setup_statics_once();
