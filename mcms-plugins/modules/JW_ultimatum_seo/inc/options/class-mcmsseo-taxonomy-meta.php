...
/**
 * Option: mcmsseo_taxonomy_meta
 */
class MCMSSEO_Taxonomy_Meta extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_taxonomy_meta';
	...
	public static $defaults_per_term = array(
		...
		'mcmsseo_canonical'       => '',
		'mcmsseo_bctitle'         => '',
		'mcmsseo_noindex'         => 'default',
		'mcmsseo_sitemap_include' => '-',
		'mcmsseo_focuskw'         => '',
		'mcmsseo_linkdex'         => '',
		'mcmsseo_content_score'   => '',

		// Social fields.
		'mcmsseo_opengraph-title'         => '',
		'mcmsseo_opengraph-description'   => '',
		'mcmsseo_opengraph-image'         => '',
		'mcmsseo_twitter-title'           => '',
		'mcmsseo_twitter-description'     => '',
		'mcmsseo_twitter-image'           => '',
	);
	 ...
	 * @static
	 *
	 * @internal  Labels (translation) added on admin_init via MCMSSEO_Taxonomy::translate_meta_options()
	 */
	...
	protected function __construct() {
		...
		add_action( 'add_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'flush_w3tc_cache' ) );
		add_action( 'update_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'flush_w3tc_cache' ) );
	}
	...
	public function enrich_defaults() {
		$extra_defaults_per_term = apply_filters( 'mcmsseo_add_extra_taxmeta_term_defaults', array() );
	...
	*/
	...
	protected function validate_option( $dirty, $clean, $old ) {
		...
		*/
		if ( isset( $dirty['mcmsseo_already_validated'] ) && $dirty['mcmsseo_already_validated'] === true ) {
			unset( $dirty['mcmsseo_already_validated'] );
			...
			if ( is_array( $terms ) && $terms !== array() ) {
				foreach ( $terms as $term_id => $meta_data ) {
					...
					if ( taxonomy_exists( $taxonomy ) && get_term_by( 'id', $term_id, $taxonomy ) === false ) {
						...
						if ( has_filter( 'mcmsseo_tax_meta_special_term_id_validation_' . $term_id ) !== false ) {
							$clean[ $taxonomy ][ $term_id ] = apply_filters( 'mcmsseo_tax_meta_special_term_id_validation_' . $term_id, $meta_data, $taxonomy, $term_id );
						}
						continue;
					}
					...
					if ( ! isset( $clean[ $taxonomy ][ $term_id ] ) && has_filter( 'mcmsseo_tax_meta_special_term_id_validation_' . $term_id ) !== false ) {
						$clean[ $taxonomy ][ $term_id ] = apply_filters( 'mcmsseo_tax_meta_special_term_id_validation_' . $term_id, $meta_data, $taxonomy, $term_id );
					}
				}
			}
	...
	public static function validate_term_meta_data( $meta_data, $old_meta ) {
		...
		$meta_data = array_map( array( 'MCMSSEO_Utils', 'trim_recursive' ), $meta_data );
		...
		foreach ( $clean as $key => $value ) {
			switch ( $key ) {
				...
				case 'mcmsseo_metakey':
				case 'mcmsseo_bctitle':
					if ( isset( $meta_data[ $key ] ) ) {
						$clean[ $key ] = MCMSSEO_Utils::sanitize_text_field( stripslashes( $meta_data[ $key ] ) );
					}
					elseif ( isset( $old_meta[ $key ] ) ) {
						// Retain old value if field currently not in use.
						$clean[ $key ] = $old_meta[ $key ];
					...
					break;
				case 'mcmsseo_focuskw':
				case 'mcmsseo_title':
				case 'mcmsseo_desc':
				case 'mcmsseo_linkdex':
				default:
					if ( isset( $meta_data[ $key ] ) && is_string( $meta_data[ $key ] ) ) {
						$clean[ $key ] = MCMSSEO_Utils::sanitize_text_field( stripslashes( $meta_data[ $key ] ) );
					}

					if ( 'mcmsseo_focuskw' === $key ) {
			...
			$clean[ $key ] = apply_filters( 'mcmsseo_sanitize_tax_meta_' . $key, $clean[ $key ], ( isset( $meta_data[ $key ] ) ? $meta_data[ $key ] : null ), ( isset( $old_meta[ $key ] ) ? $old_meta[ $key ] : null ) );
		}
								...
								switch ( $key ) {
									...
									case 'mcmsseo_linkdex':
										...
										if ( $value !== '' ) {
											...
											$option_value[ $taxonomy ][ $term_id ][ $key ] = mcms_specialchars_decode( stripslashes( $value ), ENT_QUOTES );
										}
										break;
									...
									default:
										...
										if ( $value !== '' ) {
											...
											$option_value[ $taxonomy ][ $term_id ][ $key ] = mcms_specialchars_decode( $value, ENT_QUOTES );
										}
										break;
								}
							}
	...
	public static function get_term_meta( $term, $taxonomy, $meta = null ) {
		...
		*/
		...
		if ( isset( $tax_meta[ 'mcmsseo_' . $meta ] ) ) {
			return $tax_meta[ 'mcmsseo_' . $meta ];
		}
	...
	public static function set_value( $term_id, $taxonomy, $meta_key, $meta_value ) {
		...
		if ( substr( strtolower( $meta_key ), 0, 6 ) !== 'mcmsseo_' ) {
			$meta_key = 'mcmsseo_' . $meta_key;
		}
	...
	public static function get_keyword_usage( $keyword, $current_term_id, $current_taxonomy ) {
		...
		foreach ( $tax_meta as $taxonomy_name => $terms ) {
			foreach ( $terms as $term_id => $meta_values ) {
				...
				if ( ! $is_current  && ! empty( $meta_values['mcmsseo_focuskw'] ) && $meta_values['mcmsseo_focuskw'] === $keyword ) {
					$found[] = $term_id;
				}
			}
		}
	...
	private static function save_clean_values( $term_id, $taxonomy, array $clean ) {
		...
		$tax_meta['mcmsseo_already_validated'] = true;
