 ...
 * Option: mcmsseo_xml
 */
class MCMSSEO_Option_XML extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_xml';

	/**
	 * @var  string  option group name for use in settings forms
	 */
	public $group_name = 'ultimatum_mcmsseo_xml_sitemap_options';
	 ...
	 *
	 * @return \MCMSSEO_Option_XML
	 */
	protected function __construct() {
		parent::__construct();
		add_action( 'update_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'clear_rewrites' ) );
		add_action( 'update_option_' . $this->option_name, array( 'MCMSSEO_Sitemaps_Cache', 'clear' ) );
	}
	...
	public function enrich_defaults() {
		...
		$user_roles          = MCMSSEO_Utils::get_roles();
		$filtered_user_roles = apply_filters( 'mcmsseo_sitemaps_supported_user_roles', $user_roles );
		...
		$filtered_post_types = apply_filters( 'mcmsseo_sitemaps_supported_post_types', $post_type_names );
		...
		$filtered_taxonomies = apply_filters( 'mcmsseo_sitemaps_supported_taxonomies', $taxonomy_objects );
						...
						if ( $int !== false && $int > 0 ) {
							...
							if ( $int > 50000 ) {
								...
								$error_message = sprintf(
									__( '"Max entries per sitemap page" should be below %s to meet Google\'s requirements, which %s is not.', 'mandarincms-seo' ),
							...
							$error_message = sprintf(
								__( '"Max entries per sitemap page" should be a positive number, which %s is not. Please correct.', 'mandarincms-seo' ),
