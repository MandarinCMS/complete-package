...
/**
 * Option: mcmsseo_titles
 */
class MCMSSEO_Option_Titles extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_titles';
	...
	protected $defaults = array(
		...
		'title-home-mcmsseo'       => '%%sitename%% %%page%% %%sep%% %%sitedesc%%', // Text field.
		'title-author-mcmsseo'     => '', // Text field.
		'title-archive-mcmsseo'    => '%%date%% %%page%% %%sep%% %%sitename%%', // Text field.
		'title-search-mcmsseo'     => '', // Text field.
		'title-404-mcmsseo'        => '', // Text field.

		'metadesc-home-mcmsseo'    => '', // Text area.
		'metadesc-author-mcmsseo'  => '', // Text area.
		'metadesc-archive-mcmsseo' => '', // Text area.
		'metakey-home-mcmsseo'     => '', // Text field.
		'metakey-author-mcmsseo'   => '', // Text field.

		'noindex-subpages-mcmsseo' => false,
		'noindex-author-mcmsseo'   => false,
		'noindex-archive-mcmsseo'  => true,
	...
	/**
	 * @var array Array of the separator options. To get these options use MCMSSEO_Option_Titles::get_instance()->get_separator_options()
	 */
	 ...
	 *
	 * @return \MCMSSEO_Option_Titles
	 */
	protected function __construct() {
		parent::__construct();
		add_action( 'update_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'clear_cache' ) );
	...
	public function end_of_init() {
		do_action( 'mcmsseo_double_clean_titles' );
	}
	...
	public function get_separator_options() {
		 ...
		 */
		$filtered_separators = apply_filters( 'mcmsseo_separator_options', $separators );
	...
	public function translate_defaults() {
		$this->defaults['title-author-mcmsseo'] = sprintf( __( '%s, Author at %s', 'mandarincms-seo' ), '%%name%%', '%%sitename%%' ) . ' %%page%% ';
		$this->defaults['title-search-mcmsseo'] = sprintf( __( 'You searched for %s', 'mandarincms-seo' ), '%%searchphrase%%' ) . ' %%page%% %%sep%% %%sitename%%';
		$this->defaults['title-404-mcmsseo']    = __( 'Page not found', 'mandarincms-seo' ) . ' %%sep%% %%sitename%%';
	}
	...
	public function enrich_defaults() {
		...
		if ( $post_type_objects_custom !== array() ) {
			$archive = sprintf( __( '%s Archive', 'mandarincms-seo' ), '%%pt_plural%%' );
		...
		if ( $taxonomy_names !== array() ) {
			$archives = sprintf( __( '%s Archives', 'mandarincms-seo' ), '%%term_title%%' );
		...
		foreach ( $clean as $key => $value ) {
			...
			switch ( $switch_key ) {
				...
				*/
				...
				/*
				Covers:
					   'title-home-mcmsseo', 'title-author-mcmsseo', 'title-archive-mcmsseo',
					   'title-search-mcmsseo', 'title-404-mcmsseo'
					   'title-' . $pt->name
				...
				*/
				case 'title-':
					if ( isset( $dirty[ $key ] ) ) {
						$clean[ $key ] = MCMSSEO_Utils::sanitize_text_field( $dirty[ $key ] );
					}
					break;

				/*
				Covers:
					   'metadesc-home-mcmsseo', 'metadesc-author-mcmsseo', 'metadesc-archive-mcmsseo'
					   'metadesc-' . $pt->name
				...
				*/
				case 'metadesc-':
					/*
					Covers:
							 'metakey-home-mcmsseo', 'metakey-author-mcmsseo'
							 'metakey-' . $pt->name
				...
				case 'title_test':
					if ( isset( $dirty[ $key ] ) ) {
						$int = MCMSSEO_Utils::validate_int( $dirty[ $key ] );
						if ( $int !== false && $int >= 0 ) {
							$clean[ $key ] = $int;
						}
					}
					elseif ( isset( $old[ $key ] ) ) {
						$int = MCMSSEO_Utils::validate_int( $old[ $key ] );
						if ( $int !== false && $int >= 0 ) {
							$clean[ $key ] = $int;
						}
					}
				...
				*/
				...
				/*
				Covers:
				 *		'noindex-subpages-mcmsseo', 'noindex-author-mcmsseo', 'noindex-archive-mcmsseo'
				 *		'noindex-' . $pt->name
				 ...
				 */
				default:
					$clean[ $key ] = ( isset( $dirty[ $key ] ) ? MCMSSEO_Utils::validate_bool( $dirty[ $key ] ) : false );
					break;
			}
		}
	...
	protected function clean_option( $option_value, $current_version = null, $all_old_option_values = null ) {
		...
		if ( ! isset( $original ) && has_action( 'mcmsseo_double_clean_titles', array(
				$this,
				'clean',
			) ) === false
		) {
			add_action( 'mcmsseo_double_clean_titles', array( $this, 'clean' ) );
		...
		*/
		...
		if ( isset( $all_old_option_values ) ) {
			...
			if ( isset( $all_old_option_values['mcmsseo_indexation'] ) && is_array( $all_old_option_values['mcmsseo_indexation'] ) && $all_old_option_values['mcmsseo_indexation'] !== array() ) {
				$old_option = $all_old_option_values['mcmsseo_indexation'];
			}
		}
		else {
			$old_option = get_option( 'mcmsseo_indexation' );
		}
		...
		if ( isset( $option_value['title-home'] ) && $option_value['title-home'] === '%%sitename%% - %%sitedesc%% - 12345' ) {
			$option_value['title-home-mcmsseo'] = '%%sitename%% - %%sitedesc%%';
		}
		...
		*/
		$rename = array(
			'title-home'       => 'title-home-mcmsseo',
			'title-author'     => 'title-author-mcmsseo',
			'title-archive'    => 'title-archive-mcmsseo',
			'title-search'     => 'title-search-mcmsseo',
			'title-404'        => 'title-404-mcmsseo',
			'metadesc-home'    => 'metadesc-home-mcmsseo',
			'metadesc-author'  => 'metadesc-author-mcmsseo',
			'metadesc-archive' => 'metadesc-archive-mcmsseo',
			'metakey-home'     => 'metakey-home-mcmsseo',
			'metakey-author'   => 'metakey-author-mcmsseo',
			'noindex-subpages' => 'noindex-subpages-mcmsseo',
			'noindex-author'   => 'noindex-author-mcmsseo',
			'noindex-archive'  => 'noindex-archive-mcmsseo',
		);
		 ...
		 */
		if ( isset( $original ) && current_filter() === 'mcmsseo_double_clean_titles' || did_action( 'mcmsseo_double_clean_titles' ) > 0 ) {
			$rename          = array(
				'title-'           => 'title-tax-',
		...
		if ( is_array( $option_value ) && $option_value !== array() ) {
			foreach ( $option_value as $key => $value ) {
				...
				switch ( $switch_key ) {
					...
					case 'metakey-':
					case 'bctitle-ptarchive-':
						$option_value[ $key ] = MCMSSEO_Utils::sanitize_text_field( $value );
						break;
					...
					*/
					 ...
					 */
					default:
						$option_value[ $key ] = MCMSSEO_Utils::validate_bool( $value );
						break;
				}
			}
	...
	protected function retain_variable_keys( $dirty, $clean ) {
		if ( ( is_array( $this->variable_array_key_patterns ) && $this->variable_array_key_patterns !== array() ) && ( is_array( $dirty ) && $dirty !== array() ) ) {
			 ...
			 */
			$patterns = apply_filters( 'mcmsseo_option_titles_variable_array_key_patterns', $patterns );
