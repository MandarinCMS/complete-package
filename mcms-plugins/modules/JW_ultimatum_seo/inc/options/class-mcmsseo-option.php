 ...
 * - For multisite site_options, please use the MCMSSEO_Options::update_site_option() method.
 ...
 *    Some examples:
 *      - on change of mcmsseo[ultimatum_tracking], the cron schedule will be adjusted accordingly
 *      - on change of mcmsseo_permalinks and mcmsseo_xml, the rewrite rules will be flushed
 *      - on change of mcmsseo and mcmsseo_title, some caches will be cleared
 *
 *
 * [Important information about add/updating/changing these classes]
 * - Make sure that option array key names are unique across options. The MCMSSEO_Options::get_all()
 ...
 *    add the MCMSSEO_Option::clean_up( $option_name ) method to the upgrade run.
 ...
 *               from upgrade routine (some of the MCMS functions may not yet be available)
 */
abstract class MCMSSEO_Option {
	 ...
	 *               (i.e. if it confirm to the normal pattern 'ultimatum' . $option_name . 'options',
	 *               only set in conrete class if it doesn't)
	 */
	public $group_name;

	/**
	 * @var  bool  Whether to include the option in the return for MCMSSEO_Options::get_all().
	...
	protected function __construct() {
		...
		if ( $this->multisite_only !== true ) {
			...
			if ( version_compare( $GLOBALS['mcms_version'], '3.7', '!=' ) ) { // Adding back after non-MCMS 3.7 UPDATE.
				add_action( 'update_option', array( $this, 'add_default_filters' ) );
			}
			else { // Adding back after MCMS 3.7 UPDATE.
				add_filter( 'pre_update_option_' . $this->option_name, array( $this, 'mcms37_add_default_filters' ) );
			}
		}
			...
			The option validation routines remove the default filters to prevent failing
			   ...
			   on an insert/update failure. Please use the MCMSSEO_Options::update_site_option() method
		...
		*/
		...
		if ( ! isset( $this->group_name ) || $this->group_name === '' ) {
			$this->group_name = 'ultimatum_' . $this->option_name . '_options';
		}
...
// @codingStandardsIgnoreEnd
	 ...
	 * MCMS 3.7 specific as update_option action hook was in the wrong place temporarily
	 *
	 * @see http://core.trac.mandarincms.com/ticket/25705
	 *
	 ...
	 */
	public function mcms37_add_default_filters( $new_value ) {
				...
				if ( preg_match( $regex, $meta ) ) {
					...
					if ( function_exists( 'add_settings_error' ) ) {
						add_settings_error(
							...
							sprintf( __( '%s does not seem to be a valid %s verification string. Please correct.', 'mandarincms-seo' ), '<strong>' . esc_html( $meta ) . '</strong>', $service ), // The error message.
	...
	public function validate_url( $key, $dirty, $old, &$clean ) {
		if ( isset( $dirty[ $key ] ) && $dirty[ $key ] !== '' ) {
			$url = MCMSSEO_Utils::sanitize_url( $dirty[ $key ] );
			if ( $url !== '' ) {
				$clean[ $key ] = $url;
			}
			else {
				if ( isset( $old[ $key ] ) && $old[ $key ] !== '' ) {
					$url = MCMSSEO_Utils::sanitize_url( $old[ $key ] );
					if ( $url !== '' ) {
						$clean[ $key ] = $url;
					}
				}
				if ( function_exists( 'add_settings_error' ) ) {
					...
					add_settings_error(
						$this->group_name, // Slug title of the setting.
						'_' . $key, // Suffix-id for the error message box.
						sprintf( __( '%s does not seem to be a valid url. Please correct.', 'mandarincms-seo' ), '<strong>' . esc_html( $url ) . '</strong>' ), // The error message.
						'error' // Error type, either 'error' or 'updated'.
					);
				}
			}
	...
	public function get_defaults() {
		...
		return apply_filters( 'mcmsseo_defaults', $this->defaults, $this->option_name );
	}
	...
	public function get_option( $options = null ) {
		...
		If the option contains variable option keys, make sure we don't remove those settings
			   ...
			   which are no longer in the MCMS install, but rather that than the other way around
		*/
	...
	public function validate( $option_value ) {
		...
		$option_value = array_map( array( 'MCMSSEO_Utils', 'trim_recursive' ), $option_value );
		if ( $this->multisite_only !== true ) {
	 ...
	 *
	 * @uses MCMSSEO_Option::get_original_option()
	 *
	 * @return void
	 */
	 ...
	 * Aka: use the MCMSSEO_Options::update_site_option() method (which calls this method) for
	 ...
	 * -> next time an admin is in the MCMS back-end, show the errors and delete the transient or only delete it
	 ...
	 *
	 * @internal The mcmsseo_titles concrete class overrules this method. Make sure that any changes
	...
	/**
	 * Emulate the MCMS native sanitize_text_field function in a %%variable%% safe way
	 *
	 * @see        https://core.trac.mandarincms.com/browser/trunk/src/mcms-includes/formatting.php for the original
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::sanitize_text_field()
	 * @see        MCMSSEO_Utils::sanitize_text_field()
	 *
	...
	public static function sanitize_text_field( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::sanitize_text_field()' );

		return MCMSSEO_Utils::sanitize_text_field( $value );
	}


	/**
	 * Sanitize a url for saving to the database
	 * Not to be confused with the old native MCMS function
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::sanitize_url()
	 * @see        MCMSSEO_Utils::sanitize_url()
	 *
	...
	public static function sanitize_url( $value, $allowed_protocols = array( 'http', 'https' ) ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::sanitize_url()' );

		return MCMSSEO_Utils::sanitize_url( $value, $allowed_protocols );
	}
	 ...
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::validate_bool()
	 * @see        MCMSSEO_Utils::validate_bool()
	 *
	 * @static
	 *
	...
	public static function validate_bool( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::validate_bool()' );

		return MCMSSEO_Utils::validate_bool( $value );
	}
	 ...
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::emulate_filter_bool()
	 * @see        MCMSSEO_Utils::emulate_filter_bool()
	 *
	 * @static
	 *
	...
	public static function emulate_filter_bool( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::emulate_filter_bool()' );

		return MCMSSEO_Utils::emulate_filter_bool( $value );
	}
	 ...
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::validate_int()
	 * @see        MCMSSEO_Utils::validate_int()
	 *
	 * @param mixed $value Value to validate.
	 *
	...
	public static function validate_int( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::validate_int()' );

		return MCMSSEO_Utils::validate_int( $value );
	}
	 ...
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::emulate_filter_int()
	 * @see        MCMSSEO_Utils::emulate_filter_int()
	 *
	 * @static
	 *
	 * @param    mixed $value Value to cast.
	...
	public static function emulate_filter_int( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::emulate_filter_int()' );

		return MCMSSEO_Utils::emulate_filter_int( $value );
	}
	 ...
	 *
	 * @deprecated 1.5.6.1
	 * @deprecated use MCMSSEO_Utils::trim_recursive()
	 * @see        MCMSSEO_Utils::trim_recursive()
	 *
	 * @static
	 *
	...
	public static function trim_recursive( $value ) {
		_deprecated_function( __FUNCTION__, 'MCMSSEO 1.5.6.1', 'MCMSSEO_Utils::trim_recursive()' );

		return MCMSSEO_Utils::trim_recursive( $value );
	}
}
