 ...
 * MCMS functions.
 */
class MCMSSEO_Option_MS extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_ms';

	/**
	 * @var  string  option group name for use in settings forms
	 */
	public $group_name = 'ultimatum_mcmsseo_multisite_options';
				...
				case 'access':
					...
					elseif ( function_exists( 'add_settings_error' ) ) {
						add_settings_error(
							...
							/* translators: %1$s expands to the option name and %2$sexpands to Ultimatum SEO */
							sprintf( __( '%1$s is not a valid choice for who should be allowed access to the %2$s settings. Value reset to the default.', 'mandarincms-seo' ), esc_html( sanitize_text_field( $dirty[ $key ] ) ), 'Ultimatum SEO' ), // The error message.
					...
					if ( isset( $dirty[ $key ] ) && ( $dirty[ $key ] !== '' && $dirty[ $key ] !== '-' ) ) {
						...
						if ( $int !== false && $int > 0 ) {
							...
							elseif ( function_exists( 'add_settings_error' ) ) {
								add_settings_error(
									$this->group_name, // Slug title of the setting.
									'_' . $key, // Suffix-id for the error message box.
									esc_html__( 'The default blog setting must be the numeric blog id of the blog you want to use as default.', 'mandarincms-seo' ) . '<br>' . sprintf( esc_html__( 'This must be an existing blog. Blog %s does not exist or has been marked as deleted.', 'mandarincms-seo' ), '<strong>' . esc_html( sanitize_text_field( $dirty[ $key ] ) ) . '</strong>' ), // The error message.
									'error' // Error type, either 'error' or 'updated'.
								);
							}
							unset( $exists );
						...
						elseif ( function_exists( 'add_settings_error' ) ) {
							add_settings_error(
								$this->group_name, // Slug title of the setting.
								'_' . $key, // Suffix-id for the error message box.
								esc_html__( 'The default blog setting must be the numeric blog id of the blog you want to use as default.', 'mandarincms-seo' ) . '<br>' . esc_html__( 'No numeric value was received.', 'mandarincms-seo' ), // The error message.
								'error' // Error type, either 'error' or 'updated'.
							);
						}
						unset( $int );
