 ...
 * Option: mcmsseo
 */
class MCMSSEO_Option_Wpseo extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo';
	 ...
	 *
	 * @return \MCMSSEO_Option_Wpseo
	 */
	protected function __construct() {
		parent::__construct();

		/* Clear the cache on update/add */
		add_action( 'add_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'clear_cache' ) );
		add_action( 'update_option_' . $this->option_name, array( 'MCMSSEO_Utils', 'clear_cache' ) );

		/**
		 * Filter the `mcmsseo` option defaults.
		 *
		 * @param array $defaults Array the defaults for the `mcmsseo` option attributes.
		 */
		$this->defaults = apply_filters( 'mcmsseo_option_mcmsseo_defaults', $this->defaults );
	}
		...
		foreach ( $clean as $key => $value ) {
			switch ( $key ) {
				...
				case 'ms_defaults_set':
					if ( isset( $dirty[ $key ] ) ) {
						$clean[ $key ] = MCMSSEO_Utils::validate_bool( $dirty[ $key ] );
					}
					elseif ( isset( $old[ $key ] ) ) {
						$clean[ $key ] = MCMSSEO_Utils::validate_bool( $old[ $key ] );
					}
					break;
				...
				case 'first_activated_on' :
					$clean[ $key ] = false;
					if ( isset( $dirty[ $key ] ) ) {
						if ( $dirty[ $key ] === false || MCMSSEO_Utils::validate_int( $dirty[ $key ] ) ) {
							$clean[ $key ] = $dirty[ $key ];
						}
					}
					break;
