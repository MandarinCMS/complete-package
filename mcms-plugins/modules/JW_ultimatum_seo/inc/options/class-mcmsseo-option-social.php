...
/**
 * Option: mcmsseo_social
 */
class MCMSSEO_Option_Social extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_social';
	...
	public function translate_defaults() {
		...
		self::$twitter_card_types['summary']             = __( 'Summary', 'mandarincms-seo' );
		self::$twitter_card_types['summary_large_image'] = __( 'Summary with large image', 'mandarincms-seo' );
	}
					...
					if ( isset( $dirty[ $key ] ) && is_array( $dirty[ $key ] ) ) {
						if ( $dirty[ $key ] === array() ) {
							...
							foreach ( $dirty[ $key ] as $user_id => $fb_array ) {
								/*
								 * @todo [JRF/JRF => Ultimatum/whomever] add user_id validation -
								 * are these MCMS user-ids or FB user-ids ? Probably FB user-ids,
								...
								if ( is_array( $fb_array ) && $fb_array !== array() ) {
									foreach ( $fb_array as $fb_key => $fb_value ) {
										switch ( $fb_key ) {
											...
											case 'link':
												$clean[ $key ][ $user_id ][ $fb_key ] = MCMSSEO_Utils::sanitize_url( $fb_value );
												break;
										}
									}
								}
						...
						elseif ( preg_match( '`^http(?:s)?://(?:www\.)?twitter\.com/(?P<handle>[A-Za-z0-9_]{1,25})/?$`', $twitter_id, $matches ) ) {
							...
							if ( function_exists( 'add_settings_error' ) ) {
								add_settings_error(
									...
									sprintf( __( '%s does not seem to be a valid Twitter user-id. Please correct.', 'mandarincms-seo' ), '<strong>' . esc_html( sanitize_text_field( $dirty[ $key ] ) ) . '</strong>' ), // The error message.
