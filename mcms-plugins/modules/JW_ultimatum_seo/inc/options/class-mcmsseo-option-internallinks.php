...
/**
 * Option: mcmsseo_internallinks
 */
class MCMSSEO_Option_InternalLinks extends MCMSSEO_Option {

	/**
	 * @var  string  option name
	 */
	public $option_name = 'mcmsseo_internallinks';
	...
	public function translate_defaults() {
		$this->defaults['breadcrumbs-404crumb']      = __( 'Error 404: Page not found', 'mandarincms-seo' );
		$this->defaults['breadcrumbs-archiveprefix'] = __( 'Archives for', 'mandarincms-seo' );
		$this->defaults['breadcrumbs-home']          = __( 'Home', 'mandarincms-seo' );
		$this->defaults['breadcrumbs-searchprefix']  = __( 'You searched for', 'mandarincms-seo' );
	}
		...
		foreach ( $clean as $key => $value ) {
			...
			switch ( $switch_key ) {
				...
				case 'breadcrumbs-sep':
					if ( isset( $dirty[ $key ] ) ) {
						$clean[ $key ] = mcms_kses_post( $dirty[ $key ] );
					}
					break;
						...
						elseif ( sanitize_title_with_dashes( $dirty[ $key ] ) === $dirty[ $key ] ) {
							...
							if ( function_exists( 'add_settings_error' ) ) {
								...
								add_settings_error(
									...
									sprintf( __( 'Please select a valid taxonomy for post type "%s"', 'mandarincms-seo' ), $post_type ), // The error message.
						...
						elseif ( sanitize_key( $dirty[ $key ] ) === $dirty[ $key ] ) {
							...
							if ( function_exists( 'add_settings_error' ) ) {
								...
								add_settings_error(
									...
									sprintf( __( 'Please select a valid post type for taxonomy "%s"', 'mandarincms-seo' ), $tax ), // The error message.
									'error' // Error type, either 'error' or 'updated'.
								);
								unset( $tax );
							}
	...
	protected function clean_option( $option_value, $current_version = null, $all_old_option_values = null ) {
		...
		if ( isset( $current_version ) && version_compare( $current_version, '1.5.2.3', '<' ) ) {
			if ( has_action( 'init', array( 'MCMSSEO_Options', 'bring_back_breadcrumb_defaults' ) ) === false ) {
				add_action( 'init', array( 'MCMSSEO_Options', 'bring_back_breadcrumb_defaults' ), 3 );
			}
		}
