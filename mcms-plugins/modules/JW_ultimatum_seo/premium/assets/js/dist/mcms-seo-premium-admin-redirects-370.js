...
/* jshint -W098 */
/* jshint -W107 */
/* global ajaxurl, alert, mcmsseo_premium_strings, mcms, _, mcmsseoSelect2Locale */
...
(function ($) {
  ...
  */
	RedirectForm.prototype.getOriginField = function () {
		return this.form.find("input[name=mcmsseo_redirects_origin]");
	};
  ...
  */
	RedirectForm.prototype.getTargetField = function () {
		return this.form.find("input[name=mcmsseo_redirects_target]");
	};
  ...
  */
	RedirectForm.prototype.getTypeField = function () {
		return this.form.find("select[name=mcmsseo_redirects_type]");
	};
  ...
  */
	RedirectForm.prototype.clearErrorMessage = function () {
		this.form.find(".mcmsseo_redirect_form .form_error").remove();
	};
  ...
  */
	RedirectForm.prototype.setErrorMessage = function (errorMessage) {
		this.form.find(".mcmsseo_redirect_form").prepend("<div class=\"form_error error\"><p>" + errorMessage + "</p></div>");
	};
  ...
  */
	ValidateRedirect.prototype.runValidation = function (originField, targetField, typeField) {
		...
		if ("" === originField.val()) {
			...
			if ("plain" === this.type) {
				return this.setError(mcmsseo_premium_strings.error_old_url);
			}

			return this.setError(mcmsseo_premium_strings.error_regex);
		}
		...
		if (jQuery.inArray(parseInt(typeField.val(), 10), ALLOW_EMPTY_TARGET) === -1) {
			...
			if ("" === targetField.val()) {
				...
				return this.setError(mcmsseo_premium_strings.error_new_url);
			}

			// Check if both fields aren't the same.
			if (targetField.val() === originField.val()) {
				this.form.highlightRow(targetField);
				return this.setError(mcmsseo_premium_strings.error_circular);
			}
		}
		...
		if ("" === typeField.val()) {
			...
			return this.setError(mcmsseo_premium_strings.error_new_type);
		}
  ...
  */
	...
	/**
  * Extending the elements with a mcmsseo_redirects object
  *
  * @param {string} argType The redirect table.
  *
  * @returns {void}
  */
	$.fn.mcmsseo_redirects = function (argType) {
		var that = this;
   ...
   */
		...
		this.getButtons = function (type) {
			if (type === "default") {
				return [{
					text: mcmsseo_premium_strings.button_ok,
			...
			return [{
				text: mcmsseo_premium_strings.button_cancel,
				click: function click() {
					resetIgnore();
					$(this).dialog("close");
				}
			}, {
				text: mcmsseo_premium_strings.button_save_anyway,
   ...
   */
		this.dialog = function (title, text, type) {
			...
			$("#UltimatumRedirectDialogText").html(text);
			$("#UltimatumRedirectDialog").dialog({
				title: title,
				width: 500,
				draggable: false,
				resizable: false,
   ...
   */
		this.editRow = function (row) {
			...
			if ($("#the-list").find("#inline-edit").length > 0) {
				this.dialog(mcmsseo_premium_strings.edit_redirect, mcmsseo_premium_strings.editing_redirect);
   ...
   */
		this.createRedirectRow = function (oldUrl, newUrl, redirectType, redirectInfo) {
			...
			var tr = $("<tr>").append($("<th>").addClass("check-column").attr("scope", "row").append($("<input>").attr("name", "mcmsseo_redirects_bulk_delete[]").attr("type", "checkbox").val(_.escape(oldUrl)))).append($("<td>").addClass("type column-type has-row-actions column-primary").append($("<div>").addClass("val type").html(_.escape(redirectType))).append($("<div>").addClass("row-actions").append($("<span>").addClass("edit").append($("<a>").attr({ href: "#", role: "button", "class": "redirect-edit" }).html(mcmsseo_premium_strings.editAction)).append(" | ")).append($("<span>").addClass("trash").append($("<a>").attr({ href: "#", role: "button", "class": "redirect-delete" }).html(mcmsseo_premium_strings.deleteAction))))).append($("<td>").addClass("column-old").append($("<div>").addClass("val").html(_.escape(oldUrl)))).append($("<td>").addClass("column-new").append($("<div>").addClass(targetClasses.join(" ")).html(_.escape(newUrl))));
   ...
   */
		this.handleError = function (validateRedirect, error) {
			...
			if (error.type === "warning") {
				that.dialog(mcmsseo_premium_strings.error_saving_redirect, error.message, error.type);
			}
		};
   ...
   */
		this.addRedirect = function () {
			// Do the validation.
			var redirectForm = new RedirectForm($(".mcmsseo-new-redirect-form"));
			...
			// Do post.
			that.post({
				action: "mcmsseo_add_redirect_" + type,
				ajax_nonce: $(".mcmsseo_redirects_ajax_nonce").val(),
				redirect: {
				...
				that.openDialog(mcmsseo_premium_strings.redirect_added);
			});
   ...
   */
		this.updateRedirect = function () {
			...
			// Post the request.
			that.post({
				action: "mcmsseo_update_redirect_" + type,
				ajax_nonce: $(".mcmsseo_redirects_ajax_nonce").val(),
				old_redirect: {
				...
				that.openDialog(mcmsseo_premium_strings.redirect_updated);
			});
   ...
   */
		this.deleteRedirect = function (row) {
			...
			that.post({
				action: "mcmsseo_delete_redirect_" + type,
				ajax_nonce: $(".mcmsseo_redirects_ajax_nonce").val(),
				redirect: {
					origin: encodeURIComponent(rowCells.origin.html()),
					target: encodeURIComponent(rowCells.target.html()),
				...
				that.openDialog(mcmsseo_premium_strings.redirect_deleted);
			});
		};
   ...
   */
		this.setup = function () {
			var $row;
			// Adding dialog.
			$("body").append("<div id=\"UltimatumRedirectDialog\"><div id=\"UltimatumRedirectDialogText\"></div></div>");

			// When the window will be closed/reloaded and there is a inline edit opened show a message.
			$(window).on("beforeunload", function () {
				if ($("#the-list").find("#inline-edit").length > 0) {
					return mcmsseo_premium_strings.unsaved_redirects;
				}
			});

			// Adding the onchange event.
			$(".redirect-table-tab").on("change", "select[name=mcmsseo_redirects_type]", function (evt) {
				var type = parseInt($(evt.target).val(), 10);
				var fieldToToggle = $(evt.target).closest(".mcmsseo_redirect_form").find(".mcmsseo_redirect_target_holder");
			...
			$(".mcmsseo-new-redirect-form").on("click", ".button-primary", function () {
			...
			$(".mcms-list-table").on("click", ".redirect-edit", function (evt) {
  ...
  */
	function initSelect2() {
		$("#mcmsseo_redirects_type").select2({
			width: "400px",
			language: mcmsseoSelect2Locale
		});
	}
  ...
  */
	function init() {
		templateQuickEdit = mcms.template("redirects-inline-edit");

		$.each($(".redirect-table-tab"), function (key, element) {
			$(element).mcmsseo_redirects($(element).attr("id"));
		});
