	...
	$.fn.mcmsseo_redirects = function( argType ) {
		...
		this.editRow = function( row ) {
			...
			if( $( "#the-list" ).find( "#inline-edit" ).length > 0 ) {
				this.dialog(
					mcmsseo_premium_strings.edit_redirect,
					mcmsseo_premium_strings.editing_redirect
				);
		...
		this.createRedirectRow = function( oldUrl, newUrl, redirectType, redirectInfo ) {
			...
			var tr = $( "<tr>" ).append(
				$( "<th>" ).addClass( "check-column" ).attr( "scope", "row" ).append(
					$( "<input>" )
						.attr( "name", "mcmsseo_redirects_bulk_delete[]" )
						.attr( "type", "checkbox" )
						.val( _.escape( oldUrl ) )
				)
			).append(
				$( "<td>" ).addClass( "type column-type has-row-actions column-primary" ).append(
					...
					$( "<div>" ).addClass( "row-actions" ).append(
						$( "<span>" ).addClass( "edit" ).append(
							$( "<a>" ).attr( { href: "#", role: "button", "class": "redirect-edit" } ).html( mcmsseo_premium_strings.editAction )
						).append( " | " )
					).append(
						$( "<span>" ).addClass( "trash" ).append(
							$( "<a>" ).attr( { href: "#", role: "button", "class": "redirect-delete" } ).html( mcmsseo_premium_strings.deleteAction )
						)
					)
				)
			).append(
		...
		this.setup = function() {
			...
			$( ".redirect-table-tab" )
				.on( "change", "select[name=mcmsseo_redirects_type]", function( evt ) {
