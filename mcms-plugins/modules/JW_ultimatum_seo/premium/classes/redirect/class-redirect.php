 ...
 */
class MCMSSEO_Redirect implements ArrayAccess {
	...
	public function __construct( $origin, $target = '', $type = self::PERMANENT, $format = self::FORMAT_PLAIN ) {
		$this->origin = ($format === MCMSSEO_Redirect::FORMAT_PLAIN) ? $this->sanitize_url( $origin ) : $origin;
	...
	public function origin_is( $url ) {
		...
		if ( $this->format === MCMSSEO_Redirect::FORMAT_PLAIN ) {
	...
	private function sanitize_slash( $url_to_sanitize ) {
		$url = $url_to_sanitize;
		if ( MCMSSEO_Redirect_Util::is_relative_url( $url_to_sanitize ) && $url !== '/' ) {
