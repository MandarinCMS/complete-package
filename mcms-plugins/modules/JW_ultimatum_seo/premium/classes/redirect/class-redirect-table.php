...
/**
 * Class MCMSSEO_Redirect_Table
 */
class MCMSSEO_Redirect_Table extends MCMS_List_Table {
	...
	/**
	 * MCMSSEO_Redirect_Table constructor
	 *
	...
	public function get_columns() {
		$columns = array(
			...
			'type' => _x( 'Type', 'noun', 'mandarincms-seo-premium' ),
			'old'  => $this->current_column,
			'new'  => __( 'New URL', 'mandarincms-seo-premium' ),
		);
	...
	public function redirect_list_table_primary_column( $column, $screen ) {
		if ( 'seo_page_mcmsseo_redirects' === $screen ) {
	 ...
	 * Set the items as the MCMSSEO_Redirect_Table items variable.
	 */
	...
	public function column_cb( $item ) {
		return sprintf(
			'<label class="screen-reader-text" for="mcmsseo-redirects-bulk-cb-%2$s">%3$s</label> <input type="checkbox" name="mcmsseo_redirects_bulk_delete[]" id="mcmsseo-redirects-bulk-cb-%2$s" value="%1$s" />',
			esc_attr( $item['old'] ),
			$item['row_number'],
			esc_html( __( 'Select this redirect', 'mandarincms-seo-premium' ) )
		);
	}
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			case 'new':
				...
				if ( ! $is_regex && MCMSSEO_Redirect_Util::requires_trailing_slash( $new_url ) ) {
				...
				if (
					'' === $new_url ||
					'/' === $new_url ||
					! MCMSSEO_Redirect_Util::is_relative_url( $new_url )
				) {
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete' => __( 'Delete', 'mandarincms-seo-premium' ),
		);
	...
	private function get_row_actions( $column ) {
		if ( $column === $this->primary_column ) {
			$actions = array(
				'edit'  => '<a href="#" role="button" class="redirect-edit">' . __( 'Edit', 'mandarincms-seo-premium' ) . '</a>',
				'trash' => '<a href="#" role="button" class="redirect-delete">' . __( 'Delete', 'mandarincms-seo-premium' ) . '</a>',
			);
