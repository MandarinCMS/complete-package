 ...
 */
class MCMSSEO_Redirect_Option {

	/**
	 * The plain redirect option before 3.1
	 */
	const OLD_OPTION_PLAIN = 'mcmsseo-premium-redirects';

	/**
	 * The regex redirect option before 3.1
	 */
	const OLD_OPTION_REGEX = 'mcmsseo-premium-redirects-regex';

	/**
	 * @since 3.1
	 */
	const OPTION = 'mcmsseo-premium-redirects-base';

	/**
	 * @since 3.1
	 */
	const OPTION_PLAIN = 'mcmsseo-premium-redirects-export-plain';

	/**
	 * @since 3.1
	 */
	const OPTION_REGEX = 'mcmsseo-premium-redirects-export-regex';

	/**
	 * @var MCMSSEO_Redirect[]
	 */
	 ...
	 *
	 * @param MCMSSEO_Redirect $redirect The redirect object to save.
	 *
	 * @return bool
	 */
	public function add( MCMSSEO_Redirect $redirect ) {
	 ...
	 *
	 * @param MCMSSEO_Redirect $current_redirect The current redirect value.
	 * @param MCMSSEO_Redirect $redirect         The redirect object to save.
	 *
	 * @return bool
	 */
	public function update( MCMSSEO_Redirect $current_redirect, MCMSSEO_Redirect $redirect ) {
	 ...
	 *
	 * @param MCMSSEO_Redirect $current_redirect The redirect that will be removed.
	 *
	 * @return bool
	 */
	public function delete( MCMSSEO_Redirect $current_redirect ) {
		if ( ( $found = $this->search( $current_redirect->get_origin() ) ) !== false ) {
	...
	public function save( $retry_upgrade = true ) {
		...
		update_option( self::OPTION, apply_filters( 'mcmsseo_premium_save_redirects', $redirects ), false );
	}
	...
	public function get_from_option( $option_name = self::OPTION ) {
		$redirects = apply_filters( 'mcmsseo_premium_get_redirects', get_option( $option_name ) );
	...
	private function map_option_to_object( array &$redirect_values ) {
		$redirect_values = new MCMSSEO_Redirect( $redirect_values['origin'], $redirect_values['url'], $redirect_values['type'], $redirect_values['format'] );
	}

	/**
	 * Maps a redirect object to an array option.
	 *
	 * @param MCMSSEO_Redirect $redirect The redirect to map.
	 */
	private function map_object_to_option( MCMSSEO_Redirect &$redirect ) {
		$redirect = array(
