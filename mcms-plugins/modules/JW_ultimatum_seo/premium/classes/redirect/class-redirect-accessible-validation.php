 ...
 */
class MCMSSEO_Redirect_Accessible_Validation implements MCMSSEO_Redirect_Validation {
	...
	public function run( MCMSSEO_Redirect $redirect, MCMSSEO_Redirect $old_redirect = null, array $redirects = null ) {
		...
		$response      = mcms_remote_head( $decoded_url, array( 'sslverify' => false ) );

		if ( is_mcms_error( $response ) ) {
			$this->error = new MCMSSEO_Validation_Warning(
				__( 'The URL you entered could not be resolved.', 'mandarincms-seo-premium' ),
				'target'
			);
		...
		if ( $this->is_temporary( $response_code ) ) {
			...
			$this->error = new MCMSSEO_Validation_Warning( sprintf(
				__( 'The URL you are redirecting to seems to return a %1$s status. You might want to check if the target can be reached manually before saving.', 'mandarincms-seo-premium' ),
				$response_code
			), 'target' );
		...
		if ( $response_code === 301 ) {
			$this->error = new MCMSSEO_Validation_Warning(
				__( 'You\'re redirecting to a target that returns a 301 HTTP code (permanently moved). Make sure the target you specify is directly reachable.', 'mandarincms-seo-premium' ),
				'target'
			);
		...
		if ( $response_code !== 200 ) {
			/* translators: %1$s expands to the returned http code  */
			$this->error = new MCMSSEO_Validation_Warning( sprintf(
				__( 'The URL you entered returned a HTTP code different than 200(OK). The received HTTP code is %1$s.', 'mandarincms-seo-premium' ),
				$response_code
			), 'target' );
