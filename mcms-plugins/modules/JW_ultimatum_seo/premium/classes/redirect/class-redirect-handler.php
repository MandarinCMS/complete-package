...
/**
 * Class MCMSSEO_Redirect_Handler
 */
class MCMSSEO_Redirect_Handler {
	 ...
	 */
	private $normal_option_name = 'mcmsseo-premium-redirects-export-plain';

	/**
	 * @var string The option name where the regex redirects are stored.
	 */
	private $regex_option_name  = 'mcmsseo-premium-redirects-export-regex';
	...
	/**
	 * Sets the mcms_query to 404 when this is an object
	 */
	public function set_404() {
		global $mcms_query;

		if ( is_object( $mcms_query ) ) {
			$mcms_query->is_404 = true;
		}
	}
	...
	private function match_regex_redirect( $regex, array $redirect ) {
		...
		// See issue: https://github.com/Ultimatum/mandarincms-seo-premium/issues/662.
	...
	private function do_redirect( $redirect_url, $redirect_type ) {
		...
		if ( 410 === $redirect_type ) {
			add_action( 'mcms', array( $this, 'do_410' ) );
			return;
		}

		if ( 451 === $redirect_type ) {
			add_action( 'mcms', array( $this, 'do_451' ) );
			return;
		}
		...
		if ( apply_filters( 'mcmsseo_add_x_redirect', true ) === true ) {
			header( 'X-Redirect-By: Ultimatum SEO' );
		}

		mcms_redirect( $this->parse_target_url( $redirect_url ), $redirect_type );
		exit;
	}
	...
	private function parse_target_url( $target_url ) {
		...
		if ( empty( $scheme ) ) {
			...
			if ( MCMSSEO_Redirect_Util::requires_trailing_slash( $target_url ) ) {
	...
	private function get_redirects_from_options() {
		...
		$redirects = array();
		$results   = $mcmsdb->get_results( "SELECT option_name, option_value FROM  {$mcmsdb->options} WHERE option_name = '{$this->normal_option_name}' || option_name = '{$this->regex_option_name}'" );
	...
	private function load_php_redirects() {
		...
		if ( defined( 'MCMSSEO_DISABLE_PHP_REDIRECTS' ) && true === MCMSSEO_DISABLE_PHP_REDIRECTS ) {
			return false;
		}

		global $mcmsdb;
		if ( $options = $mcmsdb->get_row( "SELECT option_value FROM {$mcmsdb->options} WHERE option_name = 'mcmsseo_redirect'" ) ) {
