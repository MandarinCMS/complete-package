...
/**
 * Class MCMSSEO_Redirect_Table_Presenter
 */
class MCMSSEO_Redirect_Table_Presenter extends MCMSSEO_Redirect_Tab_Presenter {

	/**
	 * Adding the redirect table to the view vars
	 *
	 * @param MCMSSEO_Redirect[] $redirects The redirect for the table.
	 */
	public function set_table( $redirects ) {
		$this->view_vars['redirect_table'] = new MCMSSEO_Redirect_Table(
			$this->view,
	...
	protected function get_view_vars() {
		return array_merge(
			...
			array(
				...
				'quick_edit_table' => new MCMSSEO_Redirect_Quick_Edit_Presenter(),
				'form_presenter'   => new MCMSSEO_Redirect_Form_Presenter(
					array(
	...
	private function get_first_column_value() {
		if ( $this->view === 'regex' ) {
			return  __( 'Regular Expression', 'mandarincms-seo-premium' );
		}

		return __( 'Old URL', 'mandarincms-seo-premium' );
	}
}
