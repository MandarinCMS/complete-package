...
/**
 * Class MCMSSEO_Redirect_Ajax
 */
class MCMSSEO_Redirect_Ajax {

	/**
	 * @var MCMSSEO_Redirect_Manager
	 */
	...
	public function __construct( $redirect_format ) {
		$this->redirect_manager = new MCMSSEO_Redirect_Manager( $redirect_format );
	...
	/**
	 * Function that handles the AJAX 'mcmsseo_add_redirect' action
	 */
	public function ajax_add_redirect() {
		...
		if ( $this->redirect_manager->create_redirect( $redirect ) ) {
			$response = array(
				...
				'info'   => array(
					'hasTrailingSlash' => MCMSSEO_Redirect_Util::requires_trailing_slash( $redirect->get_target() ),
					'isTargetRelative' => MCMSSEO_Redirect_Util::is_relative_url( $redirect->get_target() ),
				),
			);
		}
		else {
			...
			$error = array(
				'type'    => 'error',
				'message' => __( 'Unknown error. Failed to create redirect.', 'mandarincms-seo-premium' ),
			);
		...
		// Response.
		mcms_die( MCMSSEO_Utils::json_encode( $response ) );
	}

	/**
	 * Function that handles the AJAX 'mcmsseo_update_redirect' action
	 */
	public function ajax_update_redirect() {
		...
		if (  $this->redirect_manager->update_redirect( $current_redirect, $new_redirect ) ) {
			...
			$error = array(
				'type'    => 'error',
				'message' => __( 'Unknown error. Failed to update redirect.', 'mandarincms-seo-premium' ),
			);
	...
	/**
	 * Function that handles the AJAX 'mcmsseo_delete_redirect' action
	 */
	 ...
	 * Run the validation
	 *
	 * @param MCMSSEO_Redirect      $redirect			The redirect to save.
	 * @param MCMSSEO_Redirect|null $current_redirect The current redirect.
	 */
	private function validate( MCMSSEO_Redirect $redirect, MCMSSEO_Redirect $current_redirect = null ) {
		$validator = new MCMSSEO_Redirect_Validator();
		...
		if (  $error->get_type() === 'error' || ( $error->get_type() === 'warning'  && $ignore_warning === 'false' ) ) {
			mcms_die(
				MCMSSEO_Utils::json_encode( array( 'error' => $error->to_array() ) )
			);
		}
	}
	...
	private function set_hooks( $hook_suffix ) {
		...
		add_action( 'mcms_ajax_mcmsseo_add_redirect_' . $hook_suffix, array( $this, 'ajax_add_redirect' ) );

		// Update an existing redirect.
		add_action( 'mcms_ajax_mcmsseo_update_redirect_' . $hook_suffix, array( $this, 'ajax_update_redirect' ) );

		// Delete an existing redirect.
		add_action( 'mcms_ajax_mcmsseo_delete_redirect_' . $hook_suffix, array( $this, 'ajax_delete_redirect' ) );

		// Add URL response code check AJAX.
		if ( ! has_action( 'mcms_ajax_mcmsseo_check_url' ) ) {
			add_action( 'mcms_ajax_mcmsseo_check_url', array( $this, 'ajax_check_url' ) );
		}
	}
	...
	private function valid_ajax_check() {
		// Check nonce.
		check_ajax_referer( 'mcmsseo-redirects-ajax-security', 'ajax_nonce' );
