 ...
 * will try to fetch the full name via the Facebook following module (widget). If the user has chosen to disallow
 ...
 */
class MCMSSEO_Facebook_Profile {

	const TRANSIENT_NAME = 'ultimatum_facebook_profiles';

	/**
	 * @var string URL providing us the full name belonging to the user.
	 */
	private $facebook_endpoint = 'https://www.facebook.com/modules/follow.php?href=';

	/**
	 * Sets the AJAX action hook, to catch the AJAX request for getting the name on Facebook.
	 */
	public function set_hooks() {
		add_action( 'mcms_ajax_mcmsseo_get_facebook_name', array( $this, 'ajax_get_facebook_name' ) );
	}
	...
	public function ajax_get_facebook_name() {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX  ) {
			...
			if ( $facebook_profile !== '' ) {
				mcms_die( $this->get_name( $facebook_profile ) );
			}
	...
	private function get_name_from_facebook( $facebook_profile ) {
		...
		if ( mcms_remote_retrieve_response_code( $response ) === 200 ) {
			$full_name = $this->extract_name_from_response(
				mcms_remote_retrieve_body( $response )
			);
	...
	private function extract_name_from_response( $response_body ) {
		$full_name_regex = '/<div class="moduleButton moduleButtonInline moduleConnectButtonDisconnected" title="Follow(.*)&#039;s public updates">/i';
