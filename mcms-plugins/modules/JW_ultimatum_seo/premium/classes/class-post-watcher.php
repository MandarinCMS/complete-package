...
/**
 * Class MCMSSEO_Post_Watcher
 */
class MCMSSEO_Post_Watcher extends MCMSSEO_Watcher {
	 ...
	 *
	 * @param MCMS_Post $post The post object to get the ID from.
	 */
	 ...
	 * @param MCMS_Post $post        The post with the new values.
	 * @param MCMS_Post $post_before The post with the previous values.
	 *
	 * @return bool
	 */
	public function detect_slug_change( $post_id, $post, $post_before ) {
		...
		if ( mcms_is_post_revision( $post_before ) && mcms_is_post_revision( $post ) ) {
			return false;
		}

		/**
		 * Filter: 'mcmsseo_premium_post_redirect_slug_change' - Check if a redirect should be created on post slug change
		 *
		 * @api bool unsigned
		 */
		if ( apply_filters( 'mcmsseo_premium_post_redirect_slug_change', false ) === true ) {
			return true;
		}
	...
	private function check_public_post_status( $post_id ) {
		...
		/**
		 * Filter: 'mcmsseo_public_post_statuses' - Allow changing the statuses that are expected to have caused a URL to be public
		 *
		 ...
		 */
		$public_post_statuses = apply_filters( 'mcmsseo_public_post_statuses', $public_post_statuses, $post_id );
	...
	public function detect_post_trash( $post_id ) {
		...
		if ( $url = $this->check_if_redirect_needed( $post_id ) ) {
			...
			$id = 'mcmsseo_redirect_' . md5( $url );

			// Format the message.
			/* translators: %1$s: Ultimatum SEO, %2$s: List with actions, %3$s: <a href=''>, %4$s: </a> */
			$message = sprintf(
				__( '%1$s detected that you moved a post to the trash. You can either: %2$s Don\'t know what to do? %3$sRead this post%4$s.', 'mandarincms-seo-premium' ),
				'Ultimatum SEO',
	...
	public function detect_post_untrash( $post_id ) {
		...
		if ( $redirect = $this->check_if_redirect_needed( $post_id, true ) ) {
			...
			// Format the message.
			/* translators: %1$s: Ultimatum SEO, %2$s: <a href='{undo_redirect_url}'>, %3$s: </a> */
			$message = sprintf(
				__( '%1$s detected that you restored a post from the trash. %2$sClick here to remove the redirect%3$s', 'mandarincms-seo-premium' ),
				'Ultimatum SEO',
	...
	public function detect_post_delete( $post_id ) {
		...
		if ( get_post_status( $post_id ) === 'trash' || mcms_is_post_revision( $post_id ) ) {
			return;
		}
	 ...
	 *
	 * @return MCMSSEO_Redirect|string|void
	 */
	protected function check_if_redirect_needed( $post_id, $should_exist = false ) {
		...
		if ( $this->check_public_post_status( $post_id ) ) {
			...
			if ( $url !== '/' ) {
				...
				if ( is_a( $redirect, 'MCMSSEO_Redirect' ) === $should_exist ) {
	...
	protected function get_old_url( $post, $post_before ) {
		$mcmsseo_old_post_url = filter_input( INPUT_POST, 'mcmsseo_old_post_url' );

		if ( empty( $mcmsseo_old_post_url ) ) {
			// Check if request is inline action and new slug is not old slug, if so set mcmsseo_post_old_url.
	...
	protected function set_hooks() {
		...
		// Detect a post trash.
		add_action( 'mcms_trash_post', array( $this, 'detect_post_trash' ) );
	...
	protected function get_undo_slug_notification() {
		/* translators: %1$s: Ultimatum SEO, %2$s and %3$s expand to a link to the admin page, %4$s: Old slug of the post, %5$s: New slug of the post, the text surrounded by %6$s and %7$s is placed in a button that can undo the created redirect */
		return __(
