...
( function ( exports, $ ) {
	...
	var api = mcms.customize, OldPreviewer;
	...
	api.myCustomizerPreviewer = {
		...
		init: function () {
			...
			if (! $("#customize-preview iframe").hasClass('gardenlogin_multisite_active') ) {

					$('#customize-preview iframe').contents().find('.gardenlogin-partial.customize-partial-edit-shortcut').each( function(){
						$(this).removeClass('active');
					} );
				}
			} );

			// activated gardenlogin partial icons
			...
			$('#customize-controls h3.gardenlogin-group-heading').each(function(){
				if($(this).next('.gardenlogin-group-info').length>0){
					$(this).next('.gardenlogin-group-info').hide();
			...
			$(document).on('click', '#customize-controls h3.gardenlogin-group-heading .customize-help-toggle', function(){
				$(this).parent().next('.gardenlogin-group-info').slideToggle();
			});
