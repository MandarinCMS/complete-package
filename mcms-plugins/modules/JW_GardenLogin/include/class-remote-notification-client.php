 ...
 *
 * This class is part of the Remote Dashboard Notifications module.
 * This module allows you to send notifications to your client's
 * MandarinCMS dashboard easily.
 *
 * Notification you send will be displayed as admin notifications
 * using the standard MandarinCMS hooks. A "dismiss" option is added
 * in order to let the user hide the notification.
 *
 * @package   Remote Dashboard Notifications
 * @author    MySkinAvenue <web@myskinavenue.net>
 * @license   GPL-2.0+
 * @link      http://myskinavenue.net
 * @link      http://mandarincms.org/modules/remote-dashboard-notifications/
 * @link 	  https://github.com/MySkinAvenue/Remote-Dashboard-Notifications
 * @copyright 2016 MySkinAvenue
 */
...
if ( ! class_exists( 'Remote_Dashboard_Notifications_Client' ) ) {
	...
	final class Remote_Dashboard_Notifications_Client {
		...
		/**
		 * Minimum version of MandarinCMS required ot run the module
		 *
		 * @since 1.3.0
		 * @var string
		 */
		public $mandarincms_version_required = '3.8';

		/**
		 * Required version of PHP.
		 *
		 * Follow MandarinCMS latest requirements and require
		...
		private function init() {
			...
			// Make sure the MandarinCMS version is recent enough
			...
			if ( isset( $_GET['rn'] ) && isset( $_GET['notification'] ) ) {
				add_action( 'modules_loaded', array( self::$instance, 'dismiss' ) );
			}
			...
			add_action( 'mcms_ajax_rdn_fetch_notifications', array( $this, 'remote_get_notice_ajax' ) );
		...
		private function is_version_compatible() {
			...
			if ( empty( self::$instance->mandarincms_version_required ) ) {
				return true;
			}

			if ( version_compare( get_bloginfo( 'version' ), self::$instance->mandarincms_version_required, '<' ) ) {
				return false;
			}
		...
		protected function get_notice_dismissal_url( $slug ) {
			...
			$args                 = $_GET;
			$args['rn']           = mcms_create_nonce( 'rn-dismiss' );
		...
		public function dismiss() {
			...
			/* Validate nonce */
			if ( ! mcms_verify_nonce( sanitize_key( $_GET['rn'] ), 'rn-dismiss' ) ) {
				return;
			}
		...
		protected function remote_get_notification( $notification ) {
			...
			/* Query the server */
			$response = mcms_remote_get( $this->build_query_url( $notification['server_url'], $this->get_payload( $notification ) ), array( 'timeout' => apply_filters( 'rn_http_request_timeout', 5 ) ) );

			/* If we have a MCMS_Error object we abort */
			if ( is_mcms_error( $response ) ) {
				return $response;
			}

			if ( 200 !== (int) mcms_remote_retrieve_response_code( $response ) ) {
				return new MCMS_Error( 'invalid_response', sprintf( __( 'The server response was invalid (code %s)', 'remote-notifications' ), mcms_remote_retrieve_response_code( $response ) ) );
			}

			$body = mcms_remote_retrieve_body( $response );

			if ( empty( $body ) ) {
				return new MCMS_Error( 'empty_response', __( 'The server response is empty', 'remote-notifications' ) );
			}

			$body = json_decode( $body );

			if ( is_null( $body ) ) {
				return new MCMS_Error( 'json_decode_error', __( 'Cannot decode the response content', 'remote-notifications' ) );
			}
			...
			if ( $this->is_notification_error( $body ) ) {
				return new MCMS_Error( 'notification_error', $this->get_notification_error_message( $body ) );
			}
