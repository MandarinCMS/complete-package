...
class MCMSCF7_Submission {
	...
	public static function get_instance( MCMSCF7_ContactForm $contact_form = null, $args = '' ) {
	...
	private function setup_posted_data() {
		...
		$posted_data = array_diff_key( $posted_data, array( '_mcmsnonce' => '' ) );
		...
		foreach ( (array) $tags as $tag ) {
			...
			if ( MCMSCF7_USE_PIPE
			&& $pipes instanceof MCMSCF7_Pipes
			&& ! $pipes->zero() ) {
				if ( is_array( $value_orig ) ) {
					$value = array();

					foreach ( $value_orig as $v ) {
						$value[] = $pipes->do_pipe( mcms_unslash( $v ) );
					}
				} else {
					$value = $pipes->do_pipe( mcms_unslash( $value_orig ) );
				}
			}

			$value = apply_filters( "mcmscf7_posted_data_{$type}", $value,
				$value_orig, $tag );

			$posted_data[$name] = $value;
		}

		$this->posted_data = apply_filters( 'mcmscf7_posted_data', $posted_data );
	...
	private function sanitize_posted_data( $value ) {
		if ( is_array( $value ) ) {
			...
			$value = mcms_check_invalid_utf8( $value );
			$value = mcms_kses_no_null( $value );
		}
	...
	private function submit() {
		...
		$this->meta = array(
			...
			'unit_tag' =>
				isset( $_POST['_mcmscf7_unit_tag'] ) ? $_POST['_mcmscf7_unit_tag'] : '',
			'container_post_id' => isset( $_POST['_mcmscf7_container_post'] )
				? (int) $_POST['_mcmscf7_container_post'] : 0,
		...
		if ( ! $this->validate() ) { // Validation error occured
			...
			if ( '' === $this->get_response() ) {
				$this->set_response( $contact_form->filter_message(
					__( "Sending mail has been aborted.", 'jw-contact-support' ) )
				);
			}
			...
			do_action( 'mcmscf7_mail_sent', $contact_form );

		} else {
			$this->set_status( 'mail_failed' );
			$this->set_response( $contact_form->message( 'mail_sent_ng' ) );

			do_action( 'mcmscf7_mail_failed', $contact_form );
		}
	...
	private function get_remote_ip_addr() {
		...
		&& MCMS_Http::is_ip_address( $_SERVER['REMOTE_ADDR'] ) ) {
			$ip_addr = $_SERVER['REMOTE_ADDR'];
		}

		return apply_filters( 'mcmscf7_remote_ip_addr', $ip_addr );
	}
	...
	private function validate() {
		...
		require_once MCMSCF7_MODULE_DIR . '/includes/validation.php';
		$result = new MCMSCF7_Validation();

		$tags = $this->contact_form->scan_form_tags();

		foreach ( $tags as $tag ) {
			$type = $tag->type;
			$result = apply_filters( "mcmscf7_validate_{$type}", $result, $tag );
		}

		$result = apply_filters( 'mcmscf7_validate', $result, $tags );
	...
	private function accepted() {
		return apply_filters( 'mcmscf7_acceptance', true, $this );
	}
	...
	private function spam() {
		...
		&& current_user_can( 'mcmscf7_submit', $this->contact_form->id() ) ) {
			return $spam;
		}
		...
		return apply_filters( 'mcmscf7_spam', $spam );
	}
	...
	private function verify_nonce() {
		...
		return mcmscf7_verify_nonce( $_POST['_mcmsnonce'] );
	}

	private function is_blacklisted() {
		$target = mcmscf7_array_flatten( $this->posted_data );
		$target[] = $this->get_meta( 'remote_ip' );
		$target[] = $this->get_meta( 'user_agent' );
		$target = implode( "\n", $target );

		return (bool) apply_filters( 'mcmscf7_submission_is_blacklisted',
			mcmscf7_blacklist_check( $target ), $this );
	}
	...
	private function before_send_mail() {
		...
		do_action_ref_array( 'mcmscf7_before_send_mail', array(
			$this->contact_form,
			&$abort,
			$this,
		) );
	...
	private function mail() {
		...
		$skip_mail = apply_filters( 'mcmscf7_skip_mail',
		...
		$result = MCMSCF7_Mail::send( $contact_form->prop( 'mail' ), 'mail' );
		...
		if ( $result ) {
			...
			$additional_mail = apply_filters( 'mcmscf7_additional_mail',
				$additional_mail, $contact_form );

			foreach ( $additional_mail as $name => $template ) {
				MCMSCF7_Mail::send( $template, $name );
			}
