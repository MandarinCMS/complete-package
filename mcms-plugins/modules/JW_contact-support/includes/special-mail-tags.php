...
/**
** Special Mail Tags
** https://jiiworks.net/special-mail-tags/
**/

add_filter( 'mcmscf7_special_mail_tags', 'mcmscf7_special_mail_tag', 10, 3 );

function mcmscf7_special_mail_tag( $output, $name, $html ) {
	$name = preg_replace( '/^mcmscf7\./', '_', $name ); // for back-compat
...
add_filter( 'mcmscf7_special_mail_tags', 'mcmscf7_post_related_smt', 10, 3 );

function mcmscf7_post_related_smt( $output, $name, $html ) {
	...
	$user = new MCMS_User( $post->post_author );
...
add_filter( 'mcmscf7_special_mail_tags', 'mcmscf7_site_related_smt', 10, 3 );

function mcmscf7_site_related_smt( $output, $name, $html ) {
...
add_filter( 'mcmscf7_special_mail_tags', 'mcmscf7_user_related_smt', 10, 3 );

function mcmscf7_user_related_smt( $output, $name, $html ) {
