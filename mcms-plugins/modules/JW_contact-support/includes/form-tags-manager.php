...
function mcmscf7_add_form_tag( $tag, $func, $features = '' ) {
	$manager = MCMSCF7_FormTagsManager::get_instance();

	return $manager->add( $tag, $func, $features );
}

function mcmscf7_remove_form_tag( $tag ) {
	$manager = MCMSCF7_FormTagsManager::get_instance();

	return $manager->remove( $tag );
}

function mcmscf7_replace_all_form_tags( $content ) {
	$manager = MCMSCF7_FormTagsManager::get_instance();

	return $manager->replace_all( $content );
}

function mcmscf7_scan_form_tags( $cond = null ) {
	$contact_form = MCMSCF7_ContactForm::get_current();
...
function mcmscf7_form_tag_supports( $tag, $feature ) {
	$manager = MCMSCF7_FormTagsManager::get_instance();

	return $manager->tag_type_supports( $tag, $feature );
}

class MCMSCF7_FormTagsManager {
	...
	public function add( $tag, $func, $features = '' ) {
		...
		$features = mcms_parse_args( $features, array() );
	...
	public function filter( $input, $cond ) {
		...
		$cond = mcms_parse_args( $cond, array(
			'type' => array(),
			'name' => array(),
			'feature' => '',
		) );
	...
	private function scan_callback( $m, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				...
				&& ! empty( $attr['options'] ) ) {
					...
					if ( ! mcmscf7_is_name( $scanned_tag['name'] ) ) {
			...
			if ( MCMSCF7_USE_PIPE ) {
				$pipes = new MCMSCF7_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'mcmscf7_form_tag', $scanned_tag, $replace );

		$scanned_tag = new MCMSCF7_FormTag( $scanned_tag );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = mcmscf7_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
