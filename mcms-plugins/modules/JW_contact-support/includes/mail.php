...
class MCMSCF7_Mail {
	...
	private function __construct( $name, $template ) {
		...
		$this->template = mcms_parse_args( $template, array(
			'subject' => '',
			'sender' => '',
			'body' => '',
			'recipient' => '',
	...
	private function htmlize( $body ) {
		...
		$header = apply_filters( 'mcmscf7_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'mcmscf7_mail_html_footer',
			'</body>
</html>', $this );

		$html = $header . mcmsautop( $body ) . $footer;
		return $html;
	}
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'mcmscf7_mail_components',
			$components, mcmscf7_get_current_contact_form(), $this );

		if ( ! $send ) {
			return $components;
		}

		$subject = mcmscf7_strip_newline( $components['subject'] );
		$sender = mcmscf7_strip_newline( $components['sender'] );
		$recipient = mcmscf7_strip_newline( $components['recipient'] );
		...
		if ( $this->use_html ) {
			...
			$headers .= "X-MCMSCF7-Content-Type: text/html\n";
		} else {
			$headers .= "X-MCMSCF7-Content-Type: text/plain\n";
		}
		...
		return mcms_mail( $recipient, $subject, $body, $headers, $attachments );
	}
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return mcmscf7_mail_replace_tags( $content, $args );
	}
	...
	private function attachments( $template = null ) {
		...
		foreach ( explode( "\n", $template ) as $line ) {
			...
			$path = path_join( MCMS_CONTENT_DIR, $line );
...
function mcmscf7_mail_replace_tags( $content, $args = '' ) {
	$args = mcms_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false,
	) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = mcmscf7_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new MCMSCF7_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'mcmscf7_phpmailer_init' );

function mcmscf7_phpmailer_init( $phpmailer ) {
	$custom_headers = $phpmailer->getCustomHeaders();
	$phpmailer->clearCustomHeaders();
	$mcmscf7_content_type = false;

	foreach ( (array) $custom_headers as $custom_header ) {
		$name = $custom_header[0];
		$value = $custom_header[1];

		if ( 'X-MCMSCF7-Content-Type' === $name ) {
			$mcmscf7_content_type = trim( $value );
		} else {
			$phpmailer->addCustomHeader( $name, $value );
		}
	}

	if ( 'text/html' === $mcmscf7_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	} elseif ( 'text/plain' === $mcmscf7_content_type ) {
		$phpmailer->AltBody = '';
	}
}

class MCMSCF7_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( null !== $submitted ) {
			...
			$replaced = mcmscf7_flat_join( $replaced );

			if ( $html ) {
				$replaced = esc_html( $replaced );
				$replaced = mcmstexturize( $replaced );
			}
			...
			if ( $form_tag = $mail_tag->corresponding_form_tag() ) {
				...
				$replaced = apply_filters(
					"mcmscf7_mail_tag_replaced_{$type}", $replaced,
					$submitted, $html, $mail_tag );
			}

			$replaced = apply_filters( 'mcmscf7_mail_tag_replaced', $replaced,
				$submitted, $html, $mail_tag );

			$replaced = mcms_unslash( trim( $replaced ) );

			$this->replaced_tags[$tag] = $replaced;
			return $replaced;
		}

		$special = apply_filters( 'mcmscf7_special_mail_tags', null,
...
class MCMSCF7_MailTag {
	...
	public function __construct( $tag, $tagname, $values ) {
		...
		if ( ! empty( $values ) ) {
			...
			$this->values = mcmscf7_strip_quote_deep( $matches[0] );
		}
	...
	public function corresponding_form_tag() {
		if ( $this->form_tag instanceof MCMSCF7_FormTag ) {
