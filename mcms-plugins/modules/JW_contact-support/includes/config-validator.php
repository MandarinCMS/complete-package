...
class MCMSCF7_ConfigValidator {
	...
	public static function get_doc_link( $error_code = '' ) {
		$url = __( 'https://jiiworks.net/configuration-errors/',
			'jw-contact-support' );
	...
	public function build_message( $message, $params = '' ) {
		$params = mcms_parse_args( $params, array() );
	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'jw-contact-support' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'jw-contact-support' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'jw-contact-support' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'jw-contact-support' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'jw-contact-support' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'jw-contact-support' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'jw-contact-support' );
			default:
				return '';
		}
	}
	...
	public function validate() {
		...
		do_action( 'mcmscf7_config_validator_validate', $this );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		$mail_tag = new MCMSCF7_MailTag( $tag, $tagname, $values );
		...
		if ( $form_tags ) {
			$form_tag = new MCMSCF7_FormTag( $form_tags[0] );
			...
			if ( mcmscf7_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof MCMSCF7_Pipes ) {
				...
				if ( $last_item && mcmscf7_is_mailbox_list( $last_item ) ) {
			...
			// for back-compat
			$field_name = preg_replace( '/^mcmscf7\./', '_', $field_name );
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = mcmscf7_form_tag_supports(
						$tag->type, 'multiple-controls-container' );
					$is_zero_controls_container = mcmscf7_form_tag_supports(
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'jw-contact-support' ),
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$components = mcms_parse_args( $components, array(
			'subject' => '',
			'sender' => '',
			'recipient' => '',
			'additional_headers' => '',
		...
		$subject = new MCMSCF7_MailTaggedText( $subject,
			array( 'callback' => $callback ) );
		$subject = $subject->replace_tags();
		$subject = mcmscf7_strip_newline( $subject );
		$this->detect_maybe_empty( sprintf( '%s.subject', $template ), $subject );

		$sender = $components['sender'];
		$sender = new MCMSCF7_MailTaggedText( $sender,
			array( 'callback' => $callback ) );
		$sender = $sender->replace_tags();
		$sender = mcmscf7_strip_newline( $sender );

		if ( ! $this->detect_invalid_mailbox_syntax( sprintf( '%s.sender', $template ), $sender )
		&& ! mcmscf7_is_email_in_site_domain( $sender ) ) {
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				if ( in_array( strtolower( $header_name ), $mailbox_header_types ) ) {
					$this->detect_invalid_mailbox_syntax(
						...
						$header_value, array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'jw-contact-support' ),
		...
		$body = new MCMSCF7_MailTaggedText( $body,
			array( 'callback' => $callback ) );
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! mcmscf7_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
	...
	public function detect_file_not_found( $section, $content ) {
		$path = path_join( MCMS_CONTENT_DIR, $content );
		...
		if ( ! is_readable( $path ) || ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'jw-contact-support' ),
	...
	public function detect_html_in_message( $section, $content ) {
		$stripped = mcms_strip_all_tags( $content );
