...
class MCMSCF7_ContactForm {

	const post_type = 'mcmscf7_contact_form';
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Contact Supports', 'jw-contact-support' ),
				'singular_name' => __( 'Contact Form', 'jw-contact-support' ),
			),
			'rewrite' => false,
			'query_var' => false,
		) );
	...
	public static function get_template( $args = '' ) {
		...
		if ( $locale ) {
			$mo_orig = $l10n['jw-contact-support'];
			mcmscf7_load_textdomain( $locale );
		}

		self::$current = $contact_form = new self;
		$contact_form->title =
			( $title ? $title : __( 'Untitled', 'jw-contact-support' ) );
		$contact_form->locale = ( $locale ? $locale : get_user_locale() );

		$properties = $contact_form->get_properties();

		foreach ( $properties as $key => $value ) {
			$properties[$key] = MCMSCF7_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'mcmscf7_contact_form_default_pack',
			$contact_form, $args );

		if ( isset( $mo_orig ) ) {
			$l10n['jw-contact-support'] = $mo_orig;
		}
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'mcmscf7-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'mcmscf7-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'mcmscf7_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>MCMSCF7_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'jw-contact-support' );
	...
	public function get_properties() {
		...
		$properties = mcms_parse_args( $properties, array(
			'form' => '',
			'mail' => array(),
			'mail_2' => array(),
			'messages' => array(),
		...
		$properties = (array) apply_filters( 'mcmscf7_contact_form_properties',
			$properties, $this );
	...
	public function set_properties( $properties ) {
		...
		$properties = mcms_parse_args( $properties, $defaults );
	...
	public function set_title( $title ) {
		...
		if ( '' === $title ) {
			$title = __( 'Untitled', 'jw-contact-support' );
		}
	...
	public function locale() {
		if ( mcmscf7_is_valid_locale( $this->locale ) ) {
			return $this->locale;
		} else {
			return '';
		}
	...
	public function set_locale( $locale ) {
		...
		if ( mcmscf7_is_valid_locale( $locale ) ) {
			$this->locale = $locale;
		} else {
			$this->locale = 'en_US';
		}
	...
	public function is_posted() {
		if ( ! MCMSCF7_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_mcmscf7_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_mcmscf7_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="mcmscf7-raw-form"><code>'
				. esc_html( $this->prop( 'form' ) ) . '</code></pre>';
		}

		if ( $this->is_true( 'subscribers_only' )
		&& ! current_user_can( 'mcmscf7_submit', $this->id() ) ) {
			$notice = __(
				"This contact form is available only for logged in users.",
				'jw-contact-support' );
			$notice = sprintf(
				'<p class="mcmscf7-subscribers-only">%s</p>',
				esc_html( $notice ) );

			return apply_filters( 'mcmscf7_subscribers_only_notice', $notice, $this );
		}
		...
		$html = sprintf( '<div %s>',
			mcmscf7_format_atts( array(
				'role' => 'form',
				'class' => 'mcmscf7',
				'id' => $this->unit_tag,
				( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
					=> $lang_tag,
				'dir' => mcmscf7_is_rtl( $this->locale ) ? 'rtl' : 'ltr',
			) )
		);

		$html .= "\n" . $this->screen_reader_response() . "\n";

		$url = mcmscf7_get_request_uri();
		...
		$url = apply_filters( 'mcmscf7_form_action_url', $url );

		$id_attr = apply_filters( 'mcmscf7_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'mcmscf7_form_name_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_name'] ) );

		$class = 'mcmscf7-form';

		if ( $this->is_posted() ) {
			$submission = MCMSCF7_Submission::get_instance();
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'mcmscf7_form_class_attr', $class );

		$enctype = apply_filters( 'mcmscf7_form_enctype', '' );
		$autocomplete = apply_filters( 'mcmscf7_form_autocomplete', '' );

		$novalidate = apply_filters( 'mcmscf7_form_novalidate',
			mcmscf7_support_html5() );

		$atts = array(
			'action' => esc_url( $url ),
			'method' => 'post',
			'class' => $class,
			'enctype' => mcmscf7_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_mcmscf7' => $this->id(),
			'_mcmscf7_version' => MCMSCF7_VERSION,
			'_mcmscf7_locale' => $this->locale(),
			'_mcmscf7_unit_tag' => $this->unit_tag,
			'_mcmscf7_container_post' => 0,
		);

		if ( in_the_loop() ) {
			$hidden_fields['_mcmscf7_container_post'] = (int) get_the_ID();
		}

		if ( $this->nonce_is_active() ) {
			$hidden_fields['_mcmsnonce'] = mcmscf7_create_nonce();
		}

		$hidden_fields += (array) apply_filters(
			'mcmscf7_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			switch ( $submission->get_status() ) {
				case 'validation_failed':
					$class .= ' mcmscf7-validation-errors';
					break;
				case 'acceptance_missing':
					$class .= ' mcmscf7-acceptance-missing';
					break;
				case 'spam':
					$class .= ' mcmscf7-spam-blocked';
					break;
				case 'aborted':
					$class .= ' mcmscf7-aborted';
					break;
				case 'mail_sent':
					$class .= ' mcmscf7-mail-sent-ok';
					break;
				case 'mail_failed':
					$class .= ' mcmscf7-mail-sent-ng';
					break;
				default:
					$class .= sprintf( ' mcmscf7-custom-%s',
						preg_replace( '/[^0-9a-z]+/i', '-', $submission->get_status() )
					);
			}
		} else {
			$class .= ' mcmscf7-display-none';
		}
		...
		$output = apply_filters( 'mcmscf7_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="mcmscf7-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'mcmscf7_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function replace_all_form_tags() {
		$manager = MCMSCF7_FormTagsManager::get_instance();
		$form = $this->prop( 'form' );

		if ( mcmscf7_autop_or_not() ) {
			$form = $manager->normalize( $form );
			$form = mcmscf7_autop( $form );
		}
	...
	public function form_do_shortcode() {
		mcmscf7_deprecated_function( __METHOD__, '4.6',
			'MCMSCF7_ContactForm::replace_all_form_tags' );
	...
	public function form_scan_shortcode( $cond = null ) {
		mcmscf7_deprecated_function( __METHOD__, '4.6',
			'MCMSCF7_ContactForm::scan_form_tags' );

		return $this->scan_form_tags( $cond );
	}

	public function form_elements() {
		return apply_filters( 'mcmscf7_form_elements',
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'mcmscf7_collect_mail_tags', $mailtags, $args, $this );
	}

	public function suggest_mail_tags( $for = 'mail' ) {
		$mail = mcms_parse_args( $this->prop( $for ),
			array(
				'active' => false,
				'recipient' => '',
				'sender' => '',
	...
	public function submit( $args = '' ) {
		...
		$submission = MCMSCF7_Submission::get_instance( $this, array(
		...
		do_action( 'mcmscf7_submit', $this, $result );
	...
	public function filter_message( $message, $status = '' ) {
		$message = mcms_strip_all_tags( $message );
		$message = mcmscf7_mail_replace_tags( $message, array( 'html' => true ) );
		$message = apply_filters( 'mcmscf7_display_message', $message, $status );
	...
	public function nonce_is_active() {
		$is_active = MCMSCF7_VERIFY_NONCE;

		if ( $this->is_true( 'subscribers_only' ) ) {
			$is_active = true;
		}

		return (bool) apply_filters( 'mcmscf7_verify_nonce', $is_active, $this );
	}
	...
	private function upgrade() {
		...
		if ( is_array( $messages ) ) {
			foreach ( mcmscf7_messages() as $key => $arr ) {
	...
	public function save() {
		...
		$post_content = implode( "\n", mcmscf7_array_flatten( $props ) );

		if ( $this->initial() ) {
			$post_id = mcms_insert_post( array(
			...
			$post_id = mcms_update_post( array(
				'ID' => (int) $this->id,
				'post_status' => 'publish',
				'post_title' => $this->title,
				'post_content' => trim( $post_content ),
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					mcmscf7_normalize_newline_deep( $value ) );
			}
			...
			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'mcmscf7_after_create', $this );
			} else {
				do_action( 'mcmscf7_after_update', $this );
			}

			do_action( 'mcmscf7_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'mcmscf7_copy', $new, $this );
	}
	...
	public function delete() {
		...
		if ( mcms_delete_post( $this->id, true ) ) {
			$this->id = 0;
			return true;
		}
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			...
			if ( $old_unit_id ) {
				$shortcode = sprintf( '[contact-support %1$d "%2$s"]', $old_unit_id, $title );
			} else {
				$shortcode = '';
			}
		} else {
			$shortcode = sprintf( '[jw-contact-support id="%1$d" title="%2$s"]',
				$this->id, $title );
		}

		return apply_filters( 'mcmscf7_contact_form_shortcode', $shortcode, $args, $this );
	}
}
