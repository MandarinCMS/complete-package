( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		mcmscf7.taggen.normalize( $( this ) );
		mcmscf7.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		mcmscf7.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	mcmscf7.taggen.update = function( $form ) {
		...
		if ( $.isFunction( mcmscf7.taggen.update[ id ] ) ) {
			return mcmscf7.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = mcmscf7.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	mcmscf7.taggen.update.captcha = function( $form ) {
		var captchac = mcmscf7.taggen.compose( 'captchac', $form );
		var captchar = mcmscf7.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	mcmscf7.taggen.compose = function( tagType, $form ) {
	...
	mcmscf7.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			mcmscf7.taggen.exclusiveCheckbox( $input );
		}
	};

	mcmscf7.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	mcmscf7.taggen.insert = function( content ) {
		$( 'textarea#mcmscf7-form' ).each( function() {
			this.focus();
