...
class MCMSCF7_TagGenerator {
	...
	public function print_buttons() {
		...
		foreach ( (array) $this->panels as $panel ) {
			echo sprintf(
				...
				esc_attr( sprintf(
					__( 'Form-tag Generator: %s', 'jw-contact-support' ),
					$panel['title'] ) ),
	...
	public function print_panels( MCMSCF7_ContactForm $contact_form ) {
		foreach ( (array) $this->panels as $id => $panel ) {
			$callback = $panel['callback'];

			$options = mcms_parse_args( $panel['options'], array() );
