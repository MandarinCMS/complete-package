...
add_action( 'mcmscf7_init', 'mcmscf7_add_form_tag_number' );

function mcmscf7_add_form_tag_number() {
	mcmscf7_add_form_tag( array( 'number', 'number*', 'range', 'range*' ),
		'mcmscf7_number_form_tag_handler', array( 'name-attr' => true ) );
}

function mcmscf7_number_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
	...
	$class .= ' mcmscf7-validates-as-number';
...
add_filter( 'mcmscf7_validate_number', 'mcmscf7_number_validation_filter', 10, 2 );
add_filter( 'mcmscf7_validate_number*', 'mcmscf7_number_validation_filter', 10, 2 );
add_filter( 'mcmscf7_validate_range', 'mcmscf7_number_validation_filter', 10, 2 );
add_filter( 'mcmscf7_validate_range*', 'mcmscf7_number_validation_filter', 10, 2 );

function mcmscf7_number_validation_filter( $result, $tag ) {
	$name = $tag->name;
	...
	} elseif ( '' != $value && ! mcmscf7_is_number( $value ) ) {
		$result->invalidate( $tag, mcmscf7_get_message( 'invalid_number' ) );
	} elseif ( '' != $value && '' != $min && (float) $value < (float) $min ) {
		$result->invalidate( $tag, mcmscf7_get_message( 'number_too_small' ) );
	} elseif ( '' != $value && '' != $max && (float) $max < (float) $value ) {
		$result->invalidate( $tag, mcmscf7_get_message( 'number_too_large' ) );
	}
...
add_filter( 'mcmscf7_messages', 'mcmscf7_number_messages' );

function mcmscf7_number_messages( $messages ) {
	return array_merge( $messages, array(
		'invalid_number' => array(
			'description' => __( "Number format that the sender entered is invalid", 'jw-contact-support' ),
			'default' => __( "The number format is invalid.", 'jw-contact-support' )
		),

		'number_too_small' => array(
			'description' => __( "Number is smaller than minimum limit", 'jw-contact-support' ),
			'default' => __( "The number is smaller than the minimum allowed.", 'jw-contact-support' )
		),

		'number_too_large' => array(
			'description' => __( "Number is larger than maximum limit", 'jw-contact-support' ),
			'default' => __( "The number is larger than the maximum allowed.", 'jw-contact-support' )
		),
	) );
}
...
add_action( 'mcmscf7_admin_init', 'mcmscf7_add_tag_generator_number', 18 );

function mcmscf7_add_tag_generator_number() {
	$tag_generator = MCMSCF7_TagGenerator::get_instance();
	$tag_generator->add( 'number', __( 'number', 'jw-contact-support' ),
		'mcmscf7_tag_generator_number' );
}

function mcmscf7_tag_generator_number( $contact_form, $args = '' ) {
	$args = mcms_parse_args( $args, array() );
	$type = 'number';

	$description = __( "Generate a form-tag for a field for numeric value input. For more details, see %s.", 'jw-contact-support' );

	$desc_link = mcmscf7_link( __( 'https://jiiworks.net/number-fields/', 'jw-contact-support' ), __( 'Number Fields', 'jw-contact-support' ) );
...
<tbody>
	...
	<td>
		...
		<select name="tagtype">
			<option value="number" selected="selected"><?php echo esc_html( __( 'Spinbox', 'jw-contact-support' ) ); ?></option>
			<option value="range"><?php echo esc_html( __( 'Slider', 'jw-contact-support' ) ); ?></option>
		</select>
		<br />
