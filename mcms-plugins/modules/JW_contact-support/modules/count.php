...
add_action( 'mcmscf7_init', 'mcmscf7_add_form_tag_count' );

function mcmscf7_add_form_tag_count() {
	mcmscf7_add_form_tag( 'count',
		'mcmscf7_count_form_tag_handler',
		array(
			'name-attr' => true,
...
function mcmscf7_count_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}

	$targets = mcmscf7_scan_form_tags( array( 'name' => $tag->name ) );
	...
	if ( $tag->has_option( 'down' ) ) {
		$value = (int) $maxlength;
		$class = 'mcmscf7-character-count down';
	} else {
		$value = '0';
		$class = 'mcmscf7-character-count up';
	}
