<?php
/**
** Module for Flamingo module.
** http://mandarincms.com/extend/modules/flamingo/
**/

add_action( 'mcmscf7_submit', 'mcmscf7_flamingo_submit', 10, 2 );

function mcmscf7_flamingo_submit( $contact_form, $result ) {
	...
	$cases = (array) apply_filters( 'mcmscf7_flamingo_submit_if',
	...
	$email = mcmscf7_flamingo_get_value( 'email', $contact_form );
	$name = mcmscf7_flamingo_get_value( 'name', $contact_form );
	$subject = mcmscf7_flamingo_get_value( 'subject', $contact_form );
	...
	foreach ( $special_mail_tags as $smt ) {
		$meta[$smt] = apply_filters( 'mcmscf7_special_mail_tags', '',
	...
	if ( $channel_id ) {
		...
		if ( ! $channel || is_mcms_error( $channel ) ) {
			$channel = 'jw-contact-support';
		} else {
	...
	do_action( 'mcmscf7_after_flamingo', $result );
}

function mcmscf7_flamingo_get_value( $field, $contact_form ) {
	...
	if ( in_array( $field, array( 'email', 'name', 'subject' ) ) ) {
		...
		if ( empty( $templates[0] ) ) {
			...
			$template = trim( mcmscf7_strip_quote( $templates[0] ) );
		}

		$value = mcmscf7_mail_replace_tags( $template );
	}

	$value = apply_filters( 'mcmscf7_flamingo_get_value', $value,
		$field, $contact_form );

	return $value;
}

function mcmscf7_flamingo_add_channel( $slug, $name = '' ) {
	if ( ! class_exists( 'Flamingo_Inbound_Message' ) ) {
		return false;
	}

	$parent = term_exists( 'jw-contact-support',
		Flamingo_Inbound_Message::channel_taxonomy );

	if ( ! $parent ) {
		$parent = mcms_insert_term( __( 'Contact Form 7', 'jw-contact-support' ),
			Flamingo_Inbound_Message::channel_taxonomy,
			array( 'slug' => 'jw-contact-support' ) );
	...
	if ( ! $channel ) {
		$channel = mcms_insert_term( $name,
			Flamingo_Inbound_Message::channel_taxonomy,
			array( 'slug' => $slug, 'parent' => $parent ) );

		if ( is_mcms_error( $channel ) ) {
			return false;
		}
	}
...
add_action( 'mcmscf7_after_update', 'mcmscf7_flamingo_update_channel' );

function mcmscf7_flamingo_update_channel( $contact_form ) {
	if ( ! class_exists( 'Flamingo_Inbound_Message' ) ) {
		return false;
	}
	...
	if ( $channel->name !== mcms_unslash( $contact_form->title() ) ) {
		mcms_update_term( $channel->term_id,
			Flamingo_Inbound_Message::channel_taxonomy,
			array(
...
add_filter( 'mcmscf7_special_mail_tags', 'mcmscf7_flamingo_serial_number', 10, 3 );

function mcmscf7_flamingo_serial_number( $output, $name, $html ) {
	...
	if ( ! $contact_form = MCMSCF7_ContactForm::get_current() ) {
		return $output;
	}
