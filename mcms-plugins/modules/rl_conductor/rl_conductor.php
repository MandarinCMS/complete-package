<?php
/*
Module Name: RazorLeaf Conductor
Description: Drag and drop page builder for MandarinCMS. Take full control over structure , build any layout you can imagine.
Version: 5.1.1
*/
...
if ( ! defined( 'MCMSB_VC_VERSION' ) ) {
	 ...
	 *
	 */
	define( 'MCMSB_VC_VERSION', '5.1.1' );
}

/**
 * Vc starts here. Manager sets mode, adds required mcms hooks and loads required object of structure
 *
...
class Vc_Manager {
	 ...
	 *
	 * It depends on what functionality is required from vc to work with current page/part of MCMS.
	 *
	 * Possible values:
	 *  none - current status is unknown, default mode;
	 *  page - simple mcms page;
	 *  admin_page - mcms dashboard;
	...
	private $mode = 'none';
	/**
	 * Enables RazorLeaf Conductor to act as the myskin module.
	 *
	 * @since 4.2
	 * @var bool
	 */
	private $is_as_myskin = false;
	/**
	 * Vc is network module or not.
	 * @since 4.2
	 * @var bool
	 */
	private $is_network_module = null;
	/**
	 * List of paths.
	 *
	 * @since 4.2
	...
	/**
	 * Directory name in myskin folder where composer should search for alternative templates of the shortcode.
	 * @since 4.2
	 * @var string
	 */
	 ...
	 * @var string
	 */
	private $module_name = 'rl_conductor/rl_conductor.php';
	...
	/**
	 * Constructor loads API functions, defines paths and adds required mcms actions
	 *
	 * @since  4.2
	 */
	private function __construct() {
		 ...
		 *
		 * APP_ROOT        - module directory.
		 * MCMS_ROOT         - MCMS application root directory.
		 * APP_DIR         - module directory name.
		...
		$this->setPaths( array(
			'APP_ROOT' => $dir,
			'MCMS_ROOT' => preg_replace( '/$\//', '', BASED_TREE_URI ),
			'APP_DIR' => basename( module_basename( $dir ) ),
		...
		require_once $this->path( 'CORE_DIR', 'class-vc-sort.php' ); // used by mcmsb-map
		require_once $this->path( 'CORE_DIR', 'class-mcmsb-map.php' );
		...
		// Add hooks
		add_action( 'modules_loaded', array(
			$this,
			'modulesLoaded',
		), 9 );
		...
		$this->setModuleName( $this->path( 'APP_DIR', 'rl_conductor.php' ) );
	...
	/**
	 * Callback function MCMS module_loaded action hook. Loads locale
	 *
	 * @since  4.2
	 * @access public
	 */
	public function modulesLoaded() {
		// Setup locale
		do_action( 'vc_modules_loaded' );
		load_module_textdomain( 'rl_conductor', false, $this->path( 'APP_DIR', 'locale' ) );
	}

	/**
	 * Callback function for MCMS init action hook. Sets Vc mode and loads required objects.
	 *
	 * @since  4.2
	 * @access public
	 *
	...
	public function init() {
		...
		vc_automapper()->map();
		if ( vc_user_access()->mcmsAny( 'manage_options' )->part( 'settings' )->can( 'vc-updater-tab' )->get() ) {
	...
	protected function setMode() {
		...
		if ( is_admin() ) {
			...
			} elseif ( ( vc_user_access()->mcmsAny( 'edit_posts', 'edit_pages' )
					->get() ) && ( 'vc_upgrade' === vc_action() || ( 'update-selected' === vc_get_param( 'action' ) && $this->moduleName() === vc_get_param( 'modules' ) ) )
			) {
				$this->mode = 'admin_updater';
			} elseif ( vc_user_access()->mcmsAny( 'manage_options' )->get() && isset( $_GET['page'] ) && array_key_exists( $_GET['page'], vc_settings()->getTabs() ) ) {
			...
			if ( isset( $_GET['vc_editable'] ) && 'true' === $_GET['vc_editable'] ) {
				vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( array(
					'edit_post',
	...
	protected function setVersion() {
		...
		if ( ! is_string( $version ) || version_compare( $version, MCMSB_VC_VERSION ) !== 0 ) {
			add_action( 'vc_after_init', array(
				vc_settings(),
				'rebuild',
			) );
			update_option( 'vc_version', MCMSB_VC_VERSION );
		}
	}
	...
	/**
	 * Setter for as-myskin-module status for VC.
	 *
	 * @since  4.2
	 * @access public
	 *
	 * @param bool $value
	 */
	public function setIsAsMySkin( $value = true ) {
		$this->is_as_myskin = (boolean) $value;
	}

	/**
	 * Get as-myskin-module status
	 *
	 * As myskin module status used by myskin developers. It disables settings
	 *
	 * @since  4.2
	 * @access public
	 *
	 * @return bool
	 */
	public function isAsMySkin() {
		return (boolean) $this->is_as_myskin;
	}

	/**
	 * Setter for as network module for MultiMCMS.
	 *
	 * @since  4.2
	 * @access public
	 *
	 * @param bool $value
	 */
	public function setAsNetworkModule( $value = true ) {
		$this->is_network_module = $value;
	}

	/**
	 * Gets VC is activated as network module.
	 *
	 * @since  4.2
	 * @access public
	 *
	 * @return bool
	 */
	public function isNetworkModule() {
		if ( is_null( $this->is_network_module ) ) {
			// Check is VC as network module
			if ( is_multisite() && ( is_module_active_for_network( $this->moduleName() ) || is_network_only_module( $this->moduleName() ) ) ) {
				$this->setAsNetworkModule( true );
			}
		}

		return $this->is_network_module ? true : false;
	}
	 ...
	 * @access   public
	 *
	 * @param $dir - path to shortcodes templates inside developers myskin
	 */
	...
	public function updater() {
		...
		if ( ! isset( $this->factory['updater'] ) ) {
			...
			$updater->setUpdateManager( new Vc_Updating_Manager( MCMSB_VC_VERSION, $updater->versionUrl(), $this->moduleName() ) );
	...
	/**
	 * Getter for module name variable.
	 * @since 4.2
	 *
	 * @return string
	 */
	...
	public function assetUrl( $file ) {
		return preg_replace( '/\s/', '%20', modules_url( $this->path( 'ASSETS_DIR_NAME', $file ), __FILE__ ) );
	}
}
