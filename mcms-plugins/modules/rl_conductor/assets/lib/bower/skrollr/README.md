...
* You can't use `#00f` or `#0000ff`. You need to use `rgb` or `hsl` and explicitly decide which color space you want because they result in different animations (HSL is much cooler most of the time). Don't worry, the IE module teaches IE < 9 to display `rgb` and `hsl` correctly.
...
Starting with `0.6.22` there's experimental AMD support. Please note that only skrollr core has AMD support so far. We will update the modules in the future.
...
**Note**: If you need to support IE 7, then you may only use IDs as `anchor-target`s, i.e. `#foo`. The IE module maps `querySelector` to `getElementById`.
...
Check out the [skrollr-menu](https://github.com/Prinzhorn/skrollr-menu) module.
...
* Color animations don't support named values like "red" or hex values like "#ff0000". Instead, you have to use `rgb()`, `rgba()`, `hsl()` and `hsla()`. Don't worry, there's a skrollr module for IE < 9 to support `hsl()` (without "a"!) and to fall rgba back to rgb.
