(function ( $ ) {
	...
	window.InlineShortcodeView_vc_tabs = window.InlineShortcodeView_vc_row.extend( {
		events: {
			...
			'click > :first > .mcmsb_wrapper > .ui-tabs-nav > li': 'setActiveTab'
		},
		already_build: false,
		...
		render: function () {
			...
			this.$tabs = this.$el.find( '> .mcmsb_tabs' );
		...
		buildNav: function () {
			...
			this.$tabs.find( '> .mcmsb_wrapper > .vc_element[data-tag="vc_tab"]' ).each( function ( key ) {
		...
		tabsControls: function () {
			return this.$tabsNav ? this.$tabsNav : this.$tabsNav = this.$el.find( '.mcmsb_tabs_nav' );
		},
		...
		checkCount: function () {
			return this.$tabs.find( '> .mcmsb_wrapper > .vc_element[data-tag="vc_tab"]' ).length != this.$tabs.find(
					'> .mcmsb_wrapper > .vc_element.vc_vc_tab' ).length;
		},
		beforeUpdate: function () {
			this.$tabs.find( '.mcmsb_tabs_heading' ).remove();
			vc.frame_window.vc_iframe.destroyTabs( this.$tabs );
		},
		updated: function () {
			window.InlineShortcodeView_vc_tabs.__super__.updated.call( this );
			this.$tabs.find( '.mcmsb_tabs_nav:first' ).remove();
			this.buildNav();
		...
		cloneTabAfter: function ( model ) {
			this.$tabs.find( '> .mcmsb_wrapper > .mcmsb_tabs_nav > div' ).remove();
