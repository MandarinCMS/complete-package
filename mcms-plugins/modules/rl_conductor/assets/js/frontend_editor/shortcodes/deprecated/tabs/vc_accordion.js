(function ( $ ) {
	window.InlineShortcodeView_vc_accordion = window.InlineShortcodeView_vc_row.extend( {
		events: {
			'click > .mcmsb_accordion > .vc_empty-element': 'addElement'
		},
		render: function () {
			_.bindAll( this, 'stopSorting' );
			this.$accordion = this.$el.find( '> .mcmsb_accordion' );
		...
		beforeUpdate: function () {
			this.$el.find( '.mcmsb_accordion_heading' ).remove();
			window.InlineShortcodeView_vc_accordion.__super__.beforeUpdate.call( this );
		},
		stopSorting: function () {
			this.$accordion.find( '> .mcmsb_accordion_wrapper > .vc_element[data-tag]' ).each( function () {
