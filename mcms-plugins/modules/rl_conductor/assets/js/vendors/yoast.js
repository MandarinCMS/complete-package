...
jQuery( window ).on( 'YoastSEO:ready', function () {
	'use strict';
	var imageEventString, vcYoast, relevantData = {}, moduleName = 'vcVendorYoast', eventsList = [
		'sync',
		'add',
		'update'
	];
		...
		data = _.reduce( relevantData, function ( memo, value, key ) {
			...
			if ( value.image && value.param ) {
				...
				for ( i = 0;
					  ...
					  i ++ ) {
					attachment = window.mcms.media.model.Attachment.get( value.image[ i ] );
	...
	vc.events.on( imageEventString, function ( model, param, settings ) {
		if ( param && param.length > 0 ) {
			...
			_.each( ids, function ( id ) {
				var attachment = window.mcms.media.model.Attachment.get( id );
				if ( ! attachment.get( 'url' ) ) {
					attachment.once( 'sync', function () {
						YoastSEO.app.moduleReloaded( moduleName );
					} );
					attachment.fetch();
				}
			} );
	...
	var VcVendorYoast = function () {
		// init
		YoastSEO.app.registerModule( moduleName, { status: 'ready' } );
		YoastSEO.app.moduleReady( moduleName );
		YoastSEO.app.registerModification( 'content', contentModification, moduleName, 5 );
	};
