...
return array(
	...
	'params' => array(
		array(
			'type' => 'dropdown',
			'heading' => __( 'Row stretch', 'rl_conductor' ),
			'param_name' => 'full_width',
			'value' => array(
				__( 'Default', 'rl_conductor' ) => '',
				__( 'Stretch row', 'rl_conductor' ) => 'stretch_row',
				__( 'Stretch row and content', 'rl_conductor' ) => 'stretch_row_content',
				__( 'Stretch row and content (no paddings)', 'rl_conductor' ) => 'stretch_row_content_no_spaces',
			),
			'description' => __( 'Select stretching options for row and content (Note: stretched may not work properly if parent container has "overflow: hidden" CSS property).', 'rl_conductor' ),
		),
		array(
			'type' => 'dropdown',
		...
		array(
			'type' => 'checkbox',
			'heading' => __( 'Full height row?', 'rl_conductor' ),
			'param_name' => 'full_height',
			'description' => __( 'If checked row will be set to full height.', 'rl_conductor' ),
			'value' => array( __( 'Yes', 'rl_conductor' ) => 'yes' ),
		),
		array(
			'type' => 'dropdown',
			'heading' => __( 'Columns position', 'rl_conductor' ),
			'param_name' => 'columns_placement',
			'value' => array(
				__( 'Middle', 'rl_conductor' ) => 'middle',
				__( 'Top', 'rl_conductor' ) => 'top',
				__( 'Bottom', 'rl_conductor' ) => 'bottom',
				__( 'Stretch', 'rl_conductor' ) => 'stretch',
			),
			'description' => __( 'Select columns position within row.', 'rl_conductor' ),
			'dependency' => array(
				'element' => 'full_height',
				'not_empty' => true,
			),
		...
		array(
			...
			'param_name' => 'video_bg',
			'description' => __( 'If checked, video will be used as row background.', 'rl_conductor' ),
			'value' => array( __( 'Yes', 'rl_conductor' ) => 'yes' ),
		),
		array(
			'type' => 'textfield',
		...
		array(
			...
			'description' => __( 'Add parallax type background for row.', 'rl_conductor' ),
			'dependency' => array(
				'element' => 'video_bg',
				'not_empty' => true,
			),
		...
		array(
			...
			'description' => __( 'Add parallax type background for row (Note: If no image is specified, parallax will use background image from Design Options).', 'rl_conductor' ),
			'dependency' => array(
				'element' => 'video_bg',
				'is_empty' => true,
			),
		...
		array(
			...
			'param_name' => 'el_id',
			'description' => sprintf( __( 'Enter row ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>).', 'rl_conductor' ), 'http://www.w3schools.com/tags/att_global_id.asp' ),
		),
		array(
			'type' => 'checkbox',
