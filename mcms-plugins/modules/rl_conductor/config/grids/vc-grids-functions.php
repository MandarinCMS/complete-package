 ...
 * @return string
 */
function vc_search_by_title_only( $search, &$mcms_query ) {
	global $mcmsdb;
	if ( empty( $search ) ) {
		return $search;
	} // skip processing - no search term in query
	$q = $mcms_query->query_vars;
	if ( isset( $q['vc_search_by_title_only'] ) && true == $q['vc_search_by_title_only'] ) {
		$n = ! empty( $q['exact'] ) ? '' : '%';
		$search = $searchand = '';
		foreach ( (array) $q['search_terms'] as $term ) {
			$term = $mcmsdb->esc_like( $term );
			$like = $n . $term . $n;
			$search .= $mcmsdb->prepare( "{$searchand}($mcmsdb->posts.post_title LIKE %s)", $like );
			$searchand = ' AND ';
		}
		if ( ! empty( $search ) ) {
			$search = " AND ({$search}) ";
