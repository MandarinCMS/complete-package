...
<form action="<?php echo admin_url( 'admin-ajax.php' ); ?>" method="post"
	...
	data-vc-roles="form">
	<div class="tab_intro">
		<p><?php _e( 'Control user group role access to the features and options of RazorLeaf Conductor - manage MandarinCMS default and custom roles.', 'rl_conductor' ) ?></p>
	</div>
	<!-- Settings template start -->
	<div class="vc_mcms-settings">
		<div class="vc_mcms-accordion" data-vc-action="collapseAll">
			<?php foreach ( $editable_roles as $role => $details ) :
				...
				if ( count( $valid_roles ) > 0 ) :
					?>
					<div
						class="vc_mcms-accordion-panel vc_ui-settings-roles-role<?php echo ! isset( $next ) ? ' vc_active' : '' ?>"
						data-vc-unique-id="<?php echo esc_attr( $unique_id ) ?>"
						data-vc-content=".vc_mcms-accordion-panel-body"
						data-vc-role="<?php echo esc_attr( $role ) ?>">
						<div class="widget" data-vc-accordion=""
							data-vc-container=".vc_mcms-accordion"
						...
						<div class="vc_mcms-accordion-panel-body">
	...
	<span class="vc_settings-spinner vc_ui-mcms-spinner" style="display: none;" id="vc_mcms-spinner"></span>
	<!-- Settings template end -->
	<?php
	mcms_nonce_field( 'vc_settings-' . $tab . '-action', 'vc_nonce_field' );
	...
	submit_button( __( 'Save Changes', 'rl_conductor' ), 'primary', 'submit_btn', true, $submit_button_attributes );
	?>
