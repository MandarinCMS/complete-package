...
/**
 * @var $vc_btn MCMSBakeryShortCode_VC_Gitem_Post_Categories
 * @var $post MCMS_Post
 * @var $atts
 *
 */
VcShortcodeAutoloader::getInstance()->includeClass( 'MCMSBakeryShortCode_VC_Gitem_Post_Categories' );
...
if ( ! empty( $categories ) ) {
	foreach ( $categories as $category ) {
		$category_link = '';
		if ( ! empty( $atts['link'] ) ) {
			$category_link = 'href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'rl_conductor' ), $category->name ) ) . '"';
		}
