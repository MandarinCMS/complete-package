...
VcShortcodeAutoloader::getInstance()->includeClass( 'MCMSBakeryShortCode_VC_Single_image' );

$atts = array();
parse_str( $data, $atts );
$el_class = $image = $img_size = $img_link = $img_link_target = $img_link_large = $title = $alignment = $css_animation = $css = '';
$image_string = '';
$img_class = new MCMSBakeryShortCode_VC_Single_image( array( 'base' => 'vc_single_image' ) );
/** @var $img_class MCMSBakeryShortCode_VC_Single_image */
...
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'mcmsb_single_image mcmsb_content_element' . $el_class . vc_shortcode_custom_css_class( $css, ' ' ), $img_class->settings( 'base' ), $atts );
...
$output = '
	<div class="' . $css_class . '">
		<figure class="mcmsb_wrapper vc_figure">
			' . $image_string . '
		</figure>
	</div>
';
