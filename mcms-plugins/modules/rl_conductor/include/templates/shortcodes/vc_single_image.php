 ...
 * @var $css
 * Shortcode class
 * @var $this MCMSBakeryShortCode_VC_Single_image
 */
...
switch ( $onclick ) {
	case 'img_link_large':
		...
		if ( 'external_link' === $source ) {
			...
			$link = mcms_get_attachment_image_src( $img_id, 'large' );
			$link = $link[0];
		}
	...
	case 'zoom':
		mcms_enqueue_script( 'vc_image_zoom' );

		if ( 'external_link' === $source ) {
			$large_img_src = $custom_src;
		} else {
			$large_img_src = mcms_get_attachment_image_src( $img_id, 'large' );
			if ( $large_img_src ) {
...
$class_to_filter = 'mcmsb_single_image mcmsb_content_element vc_align_' . $alignment . ' ' . $this->getCSSAnimation( $css_animation );
...
$output = '
	<div ' . implode( ' ', $wrapper_attributes ) . ' class="' . esc_attr( trim( $css_class ) ) . '">
		' . mcmsb_widget_title( array( 'title' => $title, 'extraclass' => 'mcmsb_singleimage_heading' ) ) . '
