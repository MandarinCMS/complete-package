 ...
 * Shortcode class
 * @var $this MCMSBakeryShortCode_VC_Posts_Grid
 */
			...
			foreach ( $data as $block ) {
				...
				if ( 'title' === $block->name ) {
					...
					if ( 'featured' === $block->image ) {
						...
						$post->thumbnail_data = mcmsb_getImageBySize( array(
...
if ( ! empty( $args['post_type'] ) && is_array( $args['post_type'] ) ) {
	foreach ( $args['post_type'] as $post_type ) {
		$post_types_teasers .= 'mcmsb_teaser_grid_' . $post_type . ' ';
	}
}
$li_span_class = $this->spanClass( $grid_columns_count );

$css_class = 'mcmsb_row mcmsb_teaser_grid mcmsb_content_element ' .
			 $this->getMainCssClass( $filter ) . // Css class as selector for isotope module
...
?>
	<div class="<?php echo esc_attr( trim( $css_class ) ); ?>">
		<div class="mcmsb_wrapper">
			<?php echo mcmsb_widget_title( array( 'title' => $title, 'extraclass' => 'mcmsb_teaser_grid_heading' ) ) ?>
			<div class="teaser_grid_container">
				...
				<?php endif ?>
				<ul class="mcmsb_thumbnails mcmsb_thumbnails-fluid vc_clearfix" data-layout-mode="<?php echo $grid_layout_mode ?>">
					<?php
					/**
					 * Enqueue js/css
					 * {{
					...
					<?php if ( count( $posts ) > 0 ) :  ?>
						...
						<?php endforeach ?>
					<?php else : ?>
						<li class="<?php echo $this->spanClass( 1 ); ?>"><?php _e( 'Nothing found.', 'rl_conductor' ) ?></li>
					<?php endif ?>
				</ul>
			</div>
		</div>
