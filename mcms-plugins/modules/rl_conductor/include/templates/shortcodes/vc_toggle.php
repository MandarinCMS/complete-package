 ...
 * @var $css
 * Shortcode class
 * @var $this MCMSBakeryShortCode_VC_Toggle
 */
...
<div <?php echo isset( $el_id ) && ! empty( $el_id ) ? 'id="' . esc_attr( $el_id ) . '"' : ''; ?> class="<?php echo esc_attr( $css_class ); ?>">
	<div class="vc_toggle_title"><?php echo apply_filters( 'mcmsb_toggle_heading', $this->getHeading( $atts ), array(
			'title' => $title,
			'open' => $open,
		) ); ?><i class="vc_toggle_icon"></i></div>
	<div class="vc_toggle_content"><?php echo mcmsb_js_remove_mcmsautop( apply_filters( 'the_content', $content ), true ); ?></div>
</div>
