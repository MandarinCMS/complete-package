 ...
 * @var $css_animation
 * Shortcode class
 * @var $this MCMSBakeryShortCode_VC_gallery
 */
$thumbnail = '';
...
if ( 'nivo' === $type ) {
	...
	mcms_enqueue_script( 'isotope' );
	mcms_enqueue_style( 'isotope-css' );

	$el_start = '<li class="isotope-item">';
	$el_end = '</li>';
	$slides_wrap_start = '<ul class="mcmsb_image_grid_ul">';
	$slides_wrap_end = '</ul>';
}
...
$class_to_filter = 'mcmsb_gallery mcmsb_content_element vc_clearfix';
...
$output .= '<div class="mcmsb_wrapper">';
$output .= mcmsb_widget_title( array(
	'title' => $title,
	'extraclass' => 'mcmsb_gallery_heading',
) );
$output .= '<div class="mcmsb_gallery_slides' . $type . '" data-interval="' . $interval . '"' . $flex_fx . '>' . $slides_wrap_start . $gal_images . $slides_wrap_end . '</div>';
$output .= '</div>';
$output .= '</div>';
