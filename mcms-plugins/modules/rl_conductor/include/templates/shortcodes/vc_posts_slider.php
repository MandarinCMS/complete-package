 ...
 * @var $css
 * Shortcode class
 * @var $this MCMSBakeryShortCode_VC_Posts_slider
 */
...
if ( 'nivo' === $type ) {
	$type = ' mcmsb_slider_nivo myskin-default';
	mcms_enqueue_script( 'nivo-slider' );
	mcms_enqueue_style( 'nivo-slider-css' );
	mcms_enqueue_style( 'nivo-slider-myskin' );
	...
	$slides_wrap_end = '</ul>';
	mcms_enqueue_style( 'flexslider' );
	mcms_enqueue_script( 'flexslider' );
}
$flex_fx = '';
if ( 'flexslider' === $type || 'flexslider_fade' === $type || 'fading' === $type ) {
	$type = ' mcmsb_flexslider flexslider_fade flexslider';
	$flex_fx = ' data-flex_fx="fade"';
} elseif ( 'flexslider_slide' === $type ) {
	$type = ' mcmsb_flexslider flexslider_slide flexslider';
...
if ( '' !== $categories ) {
	...
	$gc = implode( ',', $gc );
	// http://snipplr.com/view/17434/mandarincms-get-category-slug/
...
// Run query
$my_query = new MCMS_Query( $query_args );
...
while ( $my_query->have_posts() ) {
	...
	$post_thumbnail = mcmsb_getImageBySize( array( 'post_id' => $post_id, 'thumb_size' => $thumb_size ) );
	...
	if ( 'link_no' !== $link ) {
		if ( 'link_post' === $link ) {
			$link_image_start = '<a class="link_image" href="' . get_permalink( $post_id ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'rl_conductor' ), the_title_attribute( 'echo=0' ) ) . '">';
	...
	$description = '';
	if ( '' !== $slides_content && '' !== $content && ( ' mcmsb_flexslider flexslider_fade flexslider' === $type || ' mcmsb_flexslider flexslider_slide flexslider' === $type ) ) {
...
if ( $teasers ) {
	...
	$teasers = __( 'Nothing found.', 'rl_conductor' );
}

$class_to_filter = 'mcmsb_gallery mcmsb_posts_slider mcmsb_content_element';
...
$output = '
	<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $wrapper_attributes ) . '>
		<div class="mcmsb_wrapper">
			' . mcmsb_widget_title( array( 'title' => $title, 'extraclass' => 'mcmsb_posts_slider_heading' ) ) . '
			<div class="mcmsb_gallery_slides' . $type . '" data-interval="' . $interval . '"' . $flex_fx . '>' . $teasers . '</div>
		</div>
	</div>
';
