...
function vc_lean_map( $tag, $settings_function = null, $settings_file = null ) {
	MCMSBMap::leanMap( $tag, $settings_function, $settings_file );
}
...
function vc_map( $attributes ) {
	if ( ! isset( $attributes['base'] ) ) {
		trigger_error( __( 'Wrong vc_map object. Base attribute is required', 'rl_conductor' ), E_USER_ERROR );
		die();
	}
	MCMSBMap::map( $attributes['base'], $attributes );
}
 ...
 */
function mcmsb_map( $attributes ) {
	_deprecated_function( 'mcmsb_map', '4.2 (will be removed in 5.1)', 'vc_map' );
...
function vc_remove_element( $shortcode ) {
	MCMSBMap::dropShortcode( $shortcode );
}
 ...
 */
function mcmsb_remove( $shortcode ) {
	_deprecated_function( 'mcmsb_remove', '4.2 (will be removed in 5.1)', 'vc_remove_element' );
...
function vc_add_param( $shortcode, $attributes ) {
	MCMSBMap::addParam( $shortcode, $attributes );
}
 ...
 * @deprecated
 */
function mcmsb_add_param( $shortcode, $attributes ) {
	_deprecated_function( 'mcmsb_add_param', '4.2 (will be removed in 5.1)', 'vc_add_param' );

	vc_add_param( $shortcode, $attributes );
}

/**
 * Shorthand function for MCMSBMap::modify
 *
 * @param $name
 * @param $setting
 * @param string $value
...
function vc_map_update( $name = '', $setting = '', $value = '' ) {
	return MCMSBMap::modify( $name, $setting, $value );
}

/**
 * Shorthand function for MCMSBMap::mutateParam
 *
 * @param $name
...
function vc_update_shortcode_param( $name, $attribute = array() ) {
	return MCMSBMap::mutateParam( $name, $attribute );
}

/**
 * Shorthand function for MCMSBMap::dropParam
 *
 * @param $name
...
function vc_remove_param( $name = '', $attribute_name = '' ) {
	return MCMSBMap::dropParam( $name, $attribute_name );
}

if ( ! function_exists( 'vc_set_as_myskin' ) ) {
	/**
	 * Sets module as myskin module.
	 *
	 * @internal param bool $disable_updater - If value is true disables auto updater options.
	 *
	 * @since 4.2
	 */
	function vc_set_as_myskin() {
		vc_manager()->setIsAsMySkin( true );
	}
}
if ( ! function_exists( 'vc_is_as_myskin' ) ) {
	/**
	 * Is VC as-myskin-module.
	 * @since 4.2
	 * @return bool
	 */
	function vc_is_as_myskin() {
		return vc_manager()->isAsMySkin();
	}
}
...
if ( ! function_exists( 'vc_set_shortcodes_templates_dir' ) ) {
	/**
	 * Sets directory where RazorLeaf Conductor should look for template files for content elements.
	 * @since 4.2
	 *
...
if ( ! function_exists( 'vc_shortcodes_myskin_templates_dir' ) ) {
	/**
	 * Get custom myskin template path
	 * @since 4.2
	 *
	 ...
	 * @return string
	 */
	function vc_shortcodes_myskin_templates_dir( $template ) {
...
function vc_map_integrate_shortcode( $shortcode, $field_prefix = '', $group_prefix = '', $change_fields = null, $dependency = null ) {
	if ( is_string( $shortcode ) ) {
		$shortcode_data = MCMSBMap::getShortCode( $shortcode );
	} else {
		$shortcode_data = $shortcode;
	}
	if ( is_array( $shortcode_data ) && ! empty( $shortcode_data ) ) {
		/**
		 * @var $shortcode MCMSBakeryShortCodeFishBones
		 */
...
function vc_map_integrate_get_params( $base_shortcode, $integrated_shortcode, $field_prefix = '' ) {
	$shortcode_data = MCMSBMap::getShortCode( $base_shortcode );
	$params = array();
...
function vc_map_add_css_animation( $label = true ) {
	$data = array(
		...
		'heading' => __( 'CSS Animation', 'rl_conductor' ),
		...
		'settings' => array(
			...
			'custom' => array(
				array(
					'label' => __( 'Default', 'rl_conductor' ),
					'values' => array(
						__( 'Top to bottom', 'rl_conductor' ) => 'top-to-bottom',
						__( 'Bottom to top', 'rl_conductor' ) => 'bottom-to-top',
						__( 'Left to right', 'rl_conductor' ) => 'left-to-right',
						__( 'Right to left', 'rl_conductor' ) => 'right-to-left',
						__( 'Appear from center', 'rl_conductor' ) => 'appear',
					),
				),
			),
		),
		'description' => __( 'Select type of animation for element to be animated when it "enters" the browsers viewport (Note: works only in modern browsers).', 'rl_conductor' ),
	);
...
function vc_get_shortcode( $tag ) {
	return MCMSBMap::getShortCode( $tag );
}
