...
/**
 * MCMSBakery RazorLeaf Conductor helpers functions
 *
 * @package MCMSBakeryVisualComposer
 *
 */

// Check if this file is loaded in rl_conductor
 ...
 *
 * @since 4.2
 * vc_filter: vc_mcmsb_getimagesize - to override output of this function
 * @return array|bool
 */
function mcmsb_getImageBySize( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_mcms_additional_image_sizes[ $thumb_size ] ) && is_array( $_mcms_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
				'thumbnail',
				'thumb',
				'medium',
				'large',
		...
		$thumbnail = mcms_get_attachment_image( $attach_id, $thumb_size, false, $attributes );
	} elseif ( $attach_id ) {
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = mcmsb_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], true );
			$alt = trim( strip_tags( get_post_meta( $attach_id, '_mcms_attachment_image_alt', true ) ) );
	...
	$p_img_large = mcms_get_attachment_image_src( $attach_id, 'large' );

	return apply_filters( 'vc_mcmsb_getimagesize', array(
		'thumbnail' => $thumbnail,
...
function vc_get_image_by_size( $id, $size ) {
	...
	if ( is_string( $size ) && ( ( ! empty( $_mcms_additional_image_sizes[ $size ] ) && is_array( $_mcms_additional_image_sizes[ $size ] ) ) || in_array( $size, array(
				'thumbnail',
				'thumb',
				'medium',
				'large',
		...
		return mcms_get_attachment_image_src( $id, $size );
	} else {
		if ( is_string( $size ) ) {
		...
		if ( is_array( $size ) ) {
			// Resize image to custom size
			$p_img = mcmsb_resize( $id, null, $size[0], $size[1], true );
 ...
 * @return string
 */
function mcmsb_getColumnControls( $width ) {
	_deprecated_function( 'mcmsb_getColumnControls', '4.5 (will be removed in 5.1)' );
 ...
 * @return string
 */
function mcmsb_translateColumnWidthToFractional( $width ) {
	switch ( $width ) {
		case 'vc_col-sm-2' :
			$w = '1/6';
			break;
 ...
 * @return bool|string
 */
function mcmsb_translateColumnWidthToSpan( $width ) {
 ...
 * @return string
 */
function mcmsb_js_remove_mcmsautop( $content, $autop = false ) {

	if ( $autop ) { // Possible to use !preg_match('('.MCMSBMap::getTagsRegexp().')', $content)
		$content = mcmsautop( preg_replace( '/<\/?p\>/', "\n", $content ) . "\n" );
	}
...
if ( ! function_exists( 'shortcode_exists' ) ) {
	/**
	 * Check if a shortcode is registered in MandarinCMS.
	 *
...
if ( ! function_exists( 'vc_siteAttachedImages' ) ) {
	...
	function vc_siteAttachedImages( $att_ids = array() ) {
		...
		foreach ( $media_images as $image_post ) {
			$thumb_src = mcms_get_attachment_image_src( $image_post->ID, 'thumbnail' );
			...
			$output .= '<li' . $class . '>
						...
						<span class="img-added">' . __( 'Added', 'rl_conductor' ) . '</span>
					</li>';
		}
...
function fieldAttachedImages( $images = array() ) {
	...
	foreach ( $images as $image ) {
		if ( is_numeric( $image ) ) {
			$thumb_src = mcms_get_attachment_image_src( $image, 'thumbnail' );
 ...
 * @return array
 */
function mcmsb_removeNotExistingImgIDs( $param_value ) {
	$tmp = explode( ',', $param_value );
	$return_ar = array();
	foreach ( $tmp as $id ) {
		if ( mcms_get_attachment_image( $id ) ) {
			$return_ar[] = $id;
		}
	}
...
/*
* Resize images dynamically using mcms built in functions
* Victor Teixeira
*
* php 5.2+
*
...
*
*/
if ( ! function_exists( 'mcmsb_resize' ) ) {
	/**
	 * @param int $attach_id
	 ...
	 * @return array
	 */
	function mcmsb_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
		// this is an attachment, so we have the ID
		$image_src = array();
		if ( $attach_id ) {
			$image_src = mcms_get_attachment_image_src( $attach_id, 'full' );
			$actual_file_path = get_attached_file( $attach_id );
			// this is not an attachment, let's use the image url
		} elseif ( $img_url ) {
			$file_path = parse_url( $img_url );
			$actual_file_path = rtrim( BASED_TREE_URI, '/' ) . $file_path['path'];
		...
		if ( ! empty( $actual_file_path ) ) {
			...
			if ( $image_src[1] > $width || $image_src[2] > $height ) {
				...
				if ( false == $crop ) {
					...
					$proportional_size = mcms_constrain_dimensions( $image_src[1], $image_src[2], $width, $height );
				...
				$img_editor = mcms_get_image_editor( $actual_file_path );

				if ( is_mcms_error( $img_editor ) || is_mcms_error( $img_editor->resize( $width, $height, $crop ) ) ) {
					return array(
						'url' => '',
						'width' => '',
						'height' => '',
				...
				if ( is_mcms_error( $img_editor->save( $new_img_path ) ) ) {
					return array(
						'url' => '',
						'width' => '',
						'height' => '',
...
if ( ! function_exists( 'mcmsb_debug' ) ) {
	/**
	 * Returns bool if mcmsb_debug is provided in url - set visual composer debug mode.
	 * Used for example in shortcodes (end block comment for example)
	 * @since 4.2
	 * @return bool
	 */
	function mcmsb_debug() {
		if ( ( isset( $_GET['mcmsb_debug'] ) && 'true' === $_GET['mcmsb_debug'] ) || ( isset( $_GET['vc_debug'] ) && 'true' === $_GET['vc_debug'] ) ) {
			return true;
		} else {
			return false;
		}
 ...
 * Used in mcms-content/modules/rl_conductor/include/classes/core/class-vc-base.php\Vc_Base\bodyClass
 *
 * @param $classes
 *
 * @since 4.2
 * @return array
 */
function rl_conductor_body_class( $classes ) {
	$classes[] = 'mcmsb-js-composer js-comp-ver-' . MCMSB_VC_VERSION;
 ...
 * @return string
 */
function mcmsb_vc_get_column_width_indent( $width ) {
	$identy = '11';
 ...
 * @return string
 */
function mcmsb_stripslashes_if_gpc_magic_quotes( $string ) {
	_deprecated_function( 'mcmsb_stripslashes_if_gpc_magic_quotes', '4.5 (will be removed in 5.1)', 'stripslashes' );
...
function vc_parse_options_string( $string, $tag, $param ) {
	...
	$settings = MCMSBMap::getParam( $tag, $param );
 ...
 * @deprecated 4.2
 */
function mcmsb_rl_conductor_check_version_schedule_deactivation() {
	_deprecated_function( 'mcmsb_rl_conductor_check_version_schedule_deactivation', '4.2 (will be removed in 5.1)' );
	mcms_clear_scheduled_hook( 'mcmsb_check_for_update' );
	delete_option( 'mcmsb_rl_conductor_show_new_version_message' );
}
...
function vc_icon_element_fonts_enqueue( $font ) {
	switch ( $font ) {
		case 'fontawesome':
			...
			break;
		case 'openiconic':
			mcms_enqueue_style( 'vc_openiconic' );
			break;
		case 'typicons':
			mcms_enqueue_style( 'vc_typicons' );
			break;
		case 'entypo':
			mcms_enqueue_style( 'vc_entypo' );
			break;
		case 'linecons':
			mcms_enqueue_style( 'vc_linecons' );
			break;
		case 'monosocial':
			mcms_enqueue_style( 'vc_monosocialiconsfont' );
			break;
		case 'material':
			mcms_enqueue_style( 'vc_material' );
			break;
		default:
...
function vc_message_warning( $message ) {
	return '<div class="mcmsb_element_wrapper"><div class="vc_message_box vc_message_box-standard vc_message_box-rounded vc_color-warning">
...
function vc_get_term_object( $term ) {
	...
	return array(
		...
		'group' => isset( $vc_taxonomies_types[ $term->taxonomy ], $vc_taxonomies_types[ $term->taxonomy ]->labels, $vc_taxonomies_types[ $term->taxonomy ]->labels->name ) ? $vc_taxonomies_types[ $term->taxonomy ]->labels->name : __( 'Taxonomies', 'rl_conductor' ),
	);
}
