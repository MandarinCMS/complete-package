...
if ( ! function_exists( 'vc_is_network_module' ) ) {
	/**
	 * Vc is network module or not.
	 * @since 4.2
	 * @return bool
	 */
	function vc_is_network_module() {
		return vc_manager()->isNetworkModule();
	}
}
 ...
 * @return string
 */
function vc_module_name() {
	return vc_manager()->moduleName();
}
...
function vc_file_get_contents( $filename, $partial = false ) {
	...
	if ( empty( $mcms_filesystem ) ) {
		require_once( BASED_TREE_URI . '/mcms-admin/includes/file.php' );
		MCMS_Filesystem();
	}
	/** @var $mcms_filesystem MCMS_Filesystem_Base */
	if ( ! is_object( $mcms_filesystem ) || ! $output = $mcms_filesystem->get_contents( $filename ) ) {
...
function vc_generate_nonce( $data ) {
	return mcms_create_nonce( is_array( $data ) ? ( 'vc-nonce-' . implode( '|', $data ) ) : ( 'vc-nonce-' . $data ) );
}
...
function vc_verify_nonce( $nonce, $data ) {
	return (bool) mcms_verify_nonce( $nonce, ( is_array( $data ) ? ( 'vc-nonce-' . implode( '|', $data ) ) : ( 'vc-nonce-' . $data ) ) );
}
