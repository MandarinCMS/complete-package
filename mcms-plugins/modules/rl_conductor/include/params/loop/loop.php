...
function vc_loop_form_field( $settings, $value ) {
	...
	return '<div class="vc_loop">'
	       . '<input name="' . $settings['param_name'] . '" class="mcmsb_vc_param_value  ' . $settings['param_name'] . ' ' . $settings['type'] . '_field" type="hidden" value="' . $value . '"/>'
	       . '<a href="#" class="button vc_loop-build ' . $settings['param_name'] . '_button" data-settings="' . rawurlencode( json_encode( $settings['settings'] ) ) . '">' . __( 'Build query', 'rl_conductor' ) . '</a>'
...
/**
 * Parses loop settings and creates MCMS_Query according to manual
 * @since 4.2
 * @link http://codex.mandarincms.com/Class_Reference/MCMS_Query
 */
class VcLoopQueryBuilder {
	...
	public function build() {
		return array( $this->args, new MCMS_Query( $this->args ) );
	}
}
 ...
 */
class VcLoopSettings {
	// Available parts of loop for MCMS_Query object.
	/**
	 * @since 4.2
	 * @var array
	 */
	...
	function __construct( $value, $settings = array() ) {
		$this->parts = array(
			'size' => __( 'Post count', 'rl_conductor' ),
			'order_by' => __( 'Order by', 'rl_conductor' ),
			'order' => __( 'Sort order', 'rl_conductor' ),
			'post_type' => __( 'Post types', 'rl_conductor' ),
			'authors' => __( 'Author', 'rl_conductor' ),
			'categories' => __( 'Categories', 'rl_conductor' ),
			'tags' => __( 'Tags', 'rl_conductor' ),
			'tax_query' => __( 'Taxonomies', 'rl_conductor' ),
			'by_id' => __( 'Individual posts/pages', 'rl_conductor' ),
		);
	...
	public function parse_order_by( $value ) {
		return $this->parseDropDown( $value, array(
			array( 'date', __( 'Date', 'rl_conductor' ) ),
			'ID',
			array( 'author', __( 'Author', 'rl_conductor' ) ),
			array( 'title', __( 'Title', 'rl_conductor' ) ),
			array( 'modified', __( 'Modified', 'rl_conductor' ) ),
			array( 'rand', __( 'Random', 'rl_conductor' ) ),
			array( 'comment_count', __( 'Comment count', 'rl_conductor' ) ),
			array( 'menu_order', __( 'Menu order', 'rl_conductor' ) ),
		) );
	}
	...
	public function parse_order( $value ) {
		return $this->parseDropDown( $value, array(
			array( 'ASC', __( 'Ascending', 'rl_conductor' ) ),
			array( 'DESC', __( 'Descending', 'rl_conductor' ) ),
		) );
	}
...
/**
 * Suggestion list for mcms_query field
...
/**
 * Build MCMS_Query object from query string.
...
add_action( 'mcms_ajax_mcmsb_get_loop_suggestion', 'vc_get_loop_suggestion' );
add_action( 'mcms_ajax_mcmsb_get_loop_settings', 'vc_get_loop_settings_json' );
