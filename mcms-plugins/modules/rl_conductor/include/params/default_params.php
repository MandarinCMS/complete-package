...
/**
 * MCMSBakery RazorLeaf Conductor shortcode default attributes functions for rendering.
 *
...
function vc_dropdown_form_field( $settings, $value ) {
	...
	$output .= '<select name="'
	           . $settings['param_name']
	           . '" class="mcmsb_vc_param_value mcmsb-input mcmsb-select '
	           . $settings['param_name']
	           . ' ' . $settings['type']
	           . ' ' . $css_option
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( 'attachment' !== $post_type ) {
			...
			$output .= ' <label class="vc_checkbox-label"><input id="'
			           ...
			           . $post_type . '" class="mcmsb_vc_param_value '
			           . $settings['param_name']
			           . ' ' . $settings['type']
			           . '" type="checkbox" name="'
...
function vc_exploded_textarea_form_field( $settings, $value ) {
	...
	return '<textarea name="'
	       . $settings['param_name'] . '" class="mcmsb_vc_param_value mcmsb-textarea '
...
function vc_textarea_raw_html_form_field( $settings, $value ) {
	return '<textarea name="'
	       . $settings['param_name'] . '" class="mcmsb_vc_param_value mcmsb-textarea_raw_html '
...
function vc_attach_images_form_field( $settings, $value, $tag, $single = false ) {
	$output = '';
	$param_value = mcmsb_removeNotExistingImgIDs( $value );
	$output .= '<input type="hidden" class="mcmsb_vc_param_value gallery_widget_attached_images_ids '
	           . $settings['param_name'] . ' '
	...
	if ( true === $single ) {
		$output .= '<a class="gallery_widget_add_images" href="#" use-single="true" title="'
		           . __( 'Add image', 'rl_conductor' ) . '"><i class="vc-composer-icon vc-c-icon-add"></i>' . __( 'Add image', 'rl_conductor' ) . '</a>'; //class: button
	} else {
		$output .= '<a class="gallery_widget_add_images" href="#" title="'
		           . __( 'Add images', 'rl_conductor' ) . '"><i class="vc-composer-icon vc-c-icon-add"></i>' . __( 'Add images', 'rl_conductor' ) . '</a>'; //class: button
	}
...
function vc_widgetised_sidebars_form_field( $settings, $value ) {
	$output = '';
	$sidebars = $GLOBALS['mcms_registered_sidebars'];

	$output .= '<select name="' . $settings['param_name']
	           . '" class="mcmsb_vc_param_value dropdown mcmsb-input mcmsb-select '
	           . $settings['param_name'] . ' '
	           . $settings['type'] . '">';
