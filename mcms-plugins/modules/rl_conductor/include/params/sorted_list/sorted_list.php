...
class Vc_Teaser_Box {
	...
	public function jsComposerEditPage() {
		...
		foreach ( $pt_array as $pt ) {
			add_meta_box( 'vc_teaser', __( 'VC: Custom Teaser', 'rl_conductor' ), array(
				&$this,
				'outputTeaser',
			), $pt, 'side' );
		}
	...
	public function outputTeaser() {
		mcms_enqueue_script( 'mcmsb_jscomposer_teaser_js' );
		mcms_localize_script( 'mcmsb_jscomposer_teaser_js', 'i18nVcTeaser', array(
			'empty_title' => __( 'Empty title', 'rl_conductor' ),
			'text_label' => __( 'Text', 'rl_conductor' ),
			'image_label' => __( 'Image', 'rl_conductor' ),
			'title_label' => __( 'Title', 'rl_conductor' ),
			'link_label' => __( 'Link', 'rl_conductor' ),
			'text_text' => __( 'Text', 'rl_conductor' ),
			'text_excerpt' => __( 'Excerpt', 'rl_conductor' ),
			'text_custom' => __( 'Custom', 'rl_conductor' ),
			'image_featured' => __( 'Featered', 'rl_conductor' ),
			'image_custom' => __( 'Custom', 'rl_conductor' ),
			'link_label_text' => __( 'Link text', 'rl_conductor' ),
			'no_link' => __( 'No link', 'rl_conductor' ),
			'link_post' => __( 'Link to post', 'rl_conductor' ),
			'link_big_image' => __( 'Link to big image', 'rl_conductor' ),
			'add_custom_image' => __( 'Add custom image', 'rl_conductor' ),
		) );
		$output = '<div class="vc_teaser-switch"><label><input type="checkbox" name="' . self::$meta_data_name . '[enable]" value="1" id="vc_teaser-checkbox"' . ( '1' === $this->getTeaserData( 'enable' ) ? ' checked="true"' : '' ) . '> ' . __( 'Enable custom teaser', 'rl_conductor' ) . '</label></div>';
		...
		$output .= '<p>' . __( 'Customize teaser block design to overwrite default settings used in "Carousel" content element.', 'rl_conductor' ) . '</p>';
		$output .= '</div>';
		...
		$output .= '<img src="' . get_site_url() . '/mcms-admin/images/mcmsspin_light.gif" /></div>';
