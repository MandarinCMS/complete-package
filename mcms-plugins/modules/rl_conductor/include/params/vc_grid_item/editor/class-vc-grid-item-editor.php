...
class Vc_Grid_Item_Editor extends Vc_Backend_Editor {
	...
	public function replaceTemplatesPanelEditorJsAction() {
		mcms_dequeue_script( 'vc-template-preview-script' );
	...
	public static function getPostTypesLabels() {
		return array(
			'add_new_item' => __( 'Add Grid template', 'rl_conductor' ),
			'name' => __( 'Grid Builder', 'rl_conductor' ),
			'singular_name' => __( 'Grid template', 'rl_conductor' ),
			'edit_item' => __( 'Edit Grid template', 'rl_conductor' ),
			'view_item' => __( 'View Grid template', 'rl_conductor' ),
			'search_items' => __( 'Search Grid templates', 'rl_conductor' ),
			'not_found' => __( 'No Grid templates found', 'rl_conductor' ),
			'not_found_in_trash' => __( 'No Grid templates found in Trash', 'rl_conductor' ),
		);
	}
	...
	public function addMetaBox() {
		add_meta_box( 'mcmsb_visual_composer', __( 'Grid Builder', 'rl_conductor' ), array(
			&$this,
			'renderEditor',
	...
	public function renderEditor( $post = null ) {
		...
		//		add_filter( 'vc_mcmsbakery_shortcode_get_controls_list', array(
		//			$this,
		//			'shortcodesControls',
		//		) );
	...
	public function registerBackendJavascript() {
		...
		mcms_register_script( 'vc_grid_item_editor', vc_asset_url( 'js/dist/grid-builder.min.js' ), array( 'vc-backend-min-js' ), MCMSB_VC_VERSION, true );
		mcms_localize_script( 'vc_grid_item_editor', 'i18nLocaleGItem', array(
			'preview' => __( 'Preview', 'rl_conductor' ),
			'builder' => __( 'Builder', 'rl_conductor' ),
			'add_template_message' => __( 'If you add this template, all your current changes will be removed. Are you sure you want to add template?', 'rl_conductor' ),
		) );
	}

	public function enqueueJs() {
		parent::enqueueJs();
		mcms_enqueue_script( 'vc_grid_item_editor' );
	}
	...
	public function renderTemplatePreview() {
		vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( 'edit_posts', 'edit_pages' )->validateDie()->part( 'grid_builder' )->can()->validateDie();
