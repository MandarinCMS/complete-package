...
/**
 * MCMSBakery RazorLeaf Conductor shortcodes attributes class.
 *
 ...
 * New attributes can be added to shortcode settings by using param array in mcms_map function
 *
	...
	public static function enqueueScripts() {
		...
		if ( self::isEnqueue() ) {
			foreach ( self::$enqueue_script as $item ) {
				mcms_register_script( $item['name'], $item['script'], array(
					'jquery',
					'mcms-color-picker',
				), MCMSB_VC_VERSION, true );
				mcms_enqueue_script( $item['name'] );
			}
		}
	}
