...
if ( ! class_exists( 'MCMSBakeryVisualComposerCssEditor' ) ) {
	/**
	 * Class MCMSBakeryVisualComposerCssEditor
	 */
	class MCMSBakeryVisualComposerCssEditor {
		/**
		 * @var array
		...
		function render() {
			...
			$output .= '<div class="vc_col-xs-5 vc_settings">'
			           ...
			           . '    <label>' . __( 'Box controls', 'rl_conductor' ) . '</label>'
			           . '    <label class="vc_checkbox"><input type="checkbox" name="simply" class="vc_simplify" value=""> ' . __( 'Simplify controls', 'rl_conductor' ) . '</label>'
			           . '</div>';
			$output .= '<input name="' . $this->setting( 'param_name' ) . '" class="mcmsb_vc_param_value  ' . $this->setting( 'param_name' ) . ' ' . $this->setting( 'type' ) . '_field" type="hidden" value="' . esc_attr( $this->value() ) . '"/>';
		...
		function getBackgroundImageControl() {
			return apply_filters( 'vc_css_editor_background_image_control', '<ul class="vc_image">'
				. '</ul>'
			. '<a href="#" class="vc_add-image"><i class="vc-composer-icon vc-c-icon-add"></i>' . __( 'Add image', 'rl_conductor' ) . '</a>' );
		}
		...
		function getBorderStyleOptions() {
			$output = '<option value="">' . __( 'MySkin defaults', 'rl_conductor' ) . '</option>';
			...
			foreach ( $styles as $style ) {
				$output .= '<option value="' . $style . '">' . __( ucfirst( $style ), 'rl_conductor' ) . '</option>';
			}
		...
		function getBackgroundStyleOptions() {
			...
			$styles = apply_filters( 'vc_css_editor_background_style_options_data', array(
				__( 'Cover', 'rl_conductor' ) => 'cover',
				__( 'Contain', 'rl_conductor' ) => 'contain',
				__( 'No Repeat', 'rl_conductor' ) => 'no-repeat',
				__( 'Repeat', 'rl_conductor' ) => 'repeat',
			) );
...
function vc_css_editor_form_field( $settings, $value ) {
	$css_editor = new MCMSBakeryVisualComposerCssEditor();
