...
class Vc_Column_Offset {
	...
	public function __construct( $settings, $value ) {
		...
		$this->column_width_list = array(
			__( '1 column - 1/12', 'rl_conductor' ) => '1',
			__( '2 columns - 1/6', 'rl_conductor' ) => '2',
			__( '3 columns - 1/4', 'rl_conductor' ) => '3',
			__( '4 columns - 1/3', 'rl_conductor' ) => '4',
			__( '5 columns - 5/12', 'rl_conductor' ) => '5',
			__( '6 columns - 1/2', 'rl_conductor' ) => '6',
			__( '7 columns - 7/12', 'rl_conductor' ) => '7',
			__( '8 columns - 2/3', 'rl_conductor' ) => '8',
			__( '9 columns - 3/4', 'rl_conductor' ) => '9',
			__( '10 columns - 5/6', 'rl_conductor' ) => '10',
			__( '11 columns - 11/12', 'rl_conductor' ) => '11',
			__( '12 columns - 1/1', 'rl_conductor' ) => '12',
		);
	}
	...
	public function sizeControl( $size ) {
		if ( 'sm' === $size ) {
			return '<span class="vc_description">' . __( 'Default value from width attribute', 'rl_conductor' ) . '</span>';
		}
		$empty_label = 'xs' === $size ? '' : __( 'Inherit from smaller', 'rl_conductor' );
	...
	public function offsetControl( $size ) {
		...
		$empty_label = 'xs' === $size ? __( 'No offset', 'rl_conductor' ) : __( 'Inherit from smaller', 'rl_conductor' );
		$output = '<select name="vc_' . $size . '_offset_size" class="vc_column_offset_field" data-type="offset-' . $size . '">' . '<option value="" style="color: #ccc;">' . $empty_label . '</option>' . ( 'xs' === $size ? '' : '<option value="' . $prefix . '0" style="color: #ccc;"' . ( in_array( $prefix . '0', $this->data ) ? ' selected="true"' : '' ) . '>' . __( 'No offset', 'rl_conductor' ) . '</option>' );
		foreach ( $this->column_width_list as $label => $index ) {
			$value = $prefix . $index;
			$output .= '<option value="' . $value . '"' . ( in_array( $value, $this->data ) ? ' selected="true"' : '' ) . '>' . $label . '</option>';
		}
