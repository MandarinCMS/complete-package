...
/**
 * Return true value for filter 'mcmsb_vc_js_status_filter'.
 * It allows to start backend editor on load.
 * @since 4.12
 *
 * @return string
 */
function vc_set_default_content_for_post_type_mcmsb_vc_js_status_filter() {
	return 'true';
}
...
function vc_set_default_content_for_post_type( $post_content, $post ) {
	...
	if ( ( $new_post_content = $template_settings->getTemplateByPostType( $post->post_type ) ) !== null ) {
		add_filter( 'mcmsb_vc_js_status_filter', 'vc_set_default_content_for_post_type_mcmsb_vc_js_status_filter' );
...
class Vc_Setting_Post_Type_Default_Template_Field {
	...
	protected function getFieldName() {
		return __( 'Default template for post types', 'rl_conductor' );
	}
	...
	public function sanitize( $settings ) {
		foreach ( $settings as $type => $template ) {
			if ( empty( $template ) ) {
				...
				add_settings_error( $this->getFieldKey(), 1, __( 'Invalid template or post type.', 'rl_conductor' ), 'error' );
