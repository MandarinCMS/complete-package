...
function vc_grid_item_editor_init() {
	global $vc_grid_item_editor;
	require_once vc_path_dir( 'PARAMS_DIR', 'vc_grid_item/editor/class-vc-grid-item-editor.php' );
	require_once vc_path_dir( 'PARAMS_DIR', 'vc_grid_item/class-mcmsb-map-grid-item.php' );
	$vc_grid_item_editor = new Vc_Grid_Item_Editor();
	$vc_grid_item_editor->addMetaBox();
	add_action( 'mcms_ajax_vc_grid_item_editor_load_template_preview', array(
		&$vc_grid_item_editor,
...
function vc_grid_item_get_post_type() {
	...
	if ( vc_request_param( 'post_type' ) ) {
		...
		$post_type = $post instanceof MCMS_Post && $post->post_type ? $post->post_type : null;
	}
...
function vc_grid_item_editor_shortcodes() {
	...
	if ( ( 'true' === vc_request_param( 'vc_grid_item_editor' ) || ( is_admin() && vc_grid_item_get_post_type() === Vc_Grid_Item_Editor::postType() ) && vc_user_access()
			->mcmsAny( 'edit_posts', 'edit_pages' )->part( 'grid_builder' )->can()->get() )
	) {
 ...
 */
add_action( 'mcms_ajax_vc_gitem_preview', 'vc_grid_item_render_preview', 5 );

/**
 * Add MCMS ui pointers in grid element editor.
 */
if ( is_admin() ) {
		...
		$pointers['vc_grid_item'] = array(
			...
			'messages' => array(
				array(
					...
					'options' => array(
						'content' => sprintf( '<h3> %s </h3> <p> %s </p>', __( 'Start Here!', 'rl_conductor' ), __( 'Start easy - use predefined template as a starting point and modify it.', 'rl_conductor' ) ),
						'position' => array(
							'edge' => 'left',
							'align' => 'center',
						),
				...
				array(
					...
					'options' => array(
						'content' => sprintf( '<h3> %s </h3> <p> %s </p>', __( 'Use Animations', 'rl_conductor' ), __( 'Select animation preset for grid element. "Hover" state will be added next to the "Normal" state tab.', 'rl_conductor' ) ),
						'position' => array(
							'edge' => 'right',
							'align' => 'center',
						),
				...
				array(
					...
					'options' => array(
						'content' => sprintf( '<h3> %s </h3> <p> %s </p>', __( 'Style Design Options', 'rl_conductor' ), __( 'Edit "Normal" state to set "Featured image" as a background, control zone sizing proportions and other design options (Height mode: Select "Original" to scale image without cropping).', 'rl_conductor' ) ),
						'position' => array(
							'edge' => 'bottom',
							'align' => 'center',
						),
				...
				array(
					...
					'options' => array(
						'content' => sprintf( '<h3> %s </h3> <p> %s </p>', __( 'Extend Element', 'rl_conductor' ), __( 'Additional content zone can be added to grid element edges (Note: This zone can not be animated).', 'rl_conductor' ) ) . '<p><img src="' . vc_asset_url( 'vc/gb_additional_content.png' ) . '" alt="" /></p>',
						'position' => array(
							'edge' => 'right',
							'align' => 'center',
						),
				...
				array(
					'target' => '#mcmsadminbar',
					'options' => array(
						'content' => sprintf( '<h3> %s </h3> %s', __( 'Watch Video Tutorial', 'rl_conductor' ), '<p>' . __( 'Have a look how easy it is to work with grid element builder.', 'rl_conductor' ) . '</p>' . '<iframe width="500" height="281" src="//www.youtube.com/embed/sBvEiIL6Blo" frameborder="0" allowfullscreen></iframe>' ),
						'position' => array(
							'edge' => 'top',
							'align' => 'center',
						),
...
/**
 * Add sub page to RazorLeaf Conductor pages
 *
 * @since 4.5
 */
...
function vc_gitem_set_mapper_check_access() {
	if ( vc_user_access()->checkAdminNonce()->mcmsAny( 'edit_posts', 'edit_pages' )->part( 'grid_builder' )->can()->get() && 'true' === vc_post_param( 'vc_grid_item_editor' ) ) {
		vc_mapper()->setCheckForAccess( false );
	}
}

add_action( 'mcms_ajax_vc_edit_form', 'vc_gitem_set_mapper_check_access' );
