...
function vc_check_for_custom_css_build() {
	...
	if ( vc_user_access()
			...
			->part( 'settings' )
			->can( 'vc-color-tab' )
			->get() && vc_settings()->useCustomCss() && ( ! $version || version_compare( MCMSB_VC_VERSION, $version, '<>' ) )
	) {
...
function vc_custom_css_admin_notice() {
	...
	vc_settings()->set( 'compiled_rl_conductor_less', '' );
	$class = 'notice notice-warning vc_settings-custom-design-notice';
	$message_important = __( 'Important notice', 'rl_conductor' );
	if ( is_object( $current_screen ) && isset( $current_screen->id ) && 'visual-composer_page_vc-color' === $current_screen->id ) {
		$message = __( 'You have an outdated version of RazorLeaf Conductor Design Options. It is required to review and save it.', 'rl_conductor' );
		...
		$html = '<p><strong>' . esc_html( $message_important ) . '</strong>: ' . esc_html( $message ) . '</p>'
		        . '<p><a ' . implode( ' ', $btnAtts ) . '>'
		        . __( 'Open Design Options', 'rl_conductor' ) . '</a></p>';
	}
...
function vc_page_settings_tab_color_submit_attributes( $submitButtonAttributes ) {
	$submitButtonAttributes['data-vc-less-path'] = vc_str_remove_protocol( vc_asset_url( 'less/rl_conductor.less' ) );
	...
	$submitButtonAttributes['data-vc-less-variables'] = json_encode( apply_filters( 'vc_settings-less-variables', array(
		// Main accent color:
		'vc_grey' => array(
			'key' => 'mcmsb_js_vc_color',
			'default' => vc_settings()->getDefault( 'vc_color' ),
		),
		// Hover color
		'vc_grey_hover' => array(
			'key' => 'mcmsb_js_vc_color_hover',
			'default' => vc_settings()->getDefault( 'vc_color_hover' ),
		),
		'vc_image_slider_link_active' => 'mcmsb_js_vc_color_hover',
		// Call to action background color
		'vc_call_to_action_bg' => 'mcmsb_js_vc_color_call_to_action_bg',
		'vc_call_to_action_2_bg' => 'mcmsb_js_vc_color_call_to_action_bg',
		'vc_call_to_action_border' => array(
			'key' => 'mcmsb_js_vc_color_call_to_action_border',
			// darken 5%
			'default_key' => 'mcmsb_js_vc_color',
			'modify_output' => array(
				array(
					'plain' => array(
						'darken({{ value }}, 5%)',
		...
		'vc_google_maps_bg' => 'mcmsb_js_vc_color_google_maps_bg',
		// Post slider caption background color
		'vc_post_slider_caption_bg' => 'mcmsb_js_vc_color_post_slider_caption_bg',
		// Progress bar background color
		'vc_progress_bar_bg' => 'mcmsb_js_vc_color_progress_bar_bg',
		// Separator border color
		'vc_separator_border' => 'mcmsb_js_vc_color_separator_border',
		// Tabs navigation background color
		'vc_tab_bg' => 'mcmsb_js_vc_color_tab_bg',
		// Active tab background color
		'vc_tab_bg_active' => 'mcmsb_js_vc_color_tab_bg_active',
...
function vc_page_settings_desing_options_load() {
	...
	mcms_enqueue_script( 'vc_less_js', vc_asset_url( 'lib/bower/lessjs/dist/less.min.js' ), array(), MCMSB_VC_VERSION );
}
