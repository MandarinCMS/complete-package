...
function vc_settings_tabs_vc_roles( $tabs ) {
	...
	if ( array_key_exists( 'vc-general', $tabs ) ) {
		...
		foreach ( $tabs as $key => $value ) {
			$new[ $key ] = $value;
			if ( 'vc-general' === $key ) {
				$new['vc-roles'] = __( 'Role Manager', 'rl_conductor' );
			}
		}
		$tabs = $new;
	} else {
		$tabs['vc-roles'] = __( 'Roles Manager', 'rl_conductor' );
	}
...
add_action( 'mcms_ajax_vc_roles_settings_save', 'vc_roles_settings_save' );
