...
function vc_page_welcome_add_sub_page() {
	// Add submenu page
	$page = add_submenu_page( VC_PAGE_MAIN_SLUG, __( 'About', 'rl_conductor' ), __( 'About', 'rl_conductor' ), 'exist', vc_page_welcome_slug(), 'vc_page_welcome_render' );
...
function vc_welcome_menu_hooks() {
	$settings_tab_enabled = vc_user_access()->mcmsAny( 'manage_options' )->part( 'settings' )->can( 'vc-general-tab' )->get();
...
/**
 * ====================
 * Redirect to welcome page on module activation.
 * ====================
 */
...
function vc_page_welcome_redirect() {
	...
	$redirect && mcms_redirect( admin_url( 'admin.php?page=' . rawurlencode( vc_page_welcome_slug() ) ) );
}
...
function vc_get_page_welcome_tabs() {
	...
	$vc_page_welcome_tabs = apply_filters( 'vc_page-welcome-slugs-list', array(
		'vc-welcome' => __( 'What\'s New', 'rl_conductor' ),
		'vc-faq' => __( 'FAQ', 'rl_conductor' ),
		'vc-resources' => __( 'Resources', 'rl_conductor' ),
	) );
