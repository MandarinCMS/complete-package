...
add_action( 'mcms_ajax_vc_media_editor_add_image', 'vc_media_editor_add_image' );
add_action( 'mcms_ajax_vc_media_editor_preview_image', 'vc_media_editor_preview_image' );
...
function vc_get_filters() {
	return array(
		'antique' => __( 'Antique', 'rl_conductor' ),
		'blackwhite' => __( 'Black & White', 'rl_conductor' ),
		'boost' => __( 'Boost', 'rl_conductor' ),
		'concentrate' => __( 'Concentrate', 'rl_conductor' ),
		'country' => __( 'Country', 'rl_conductor' ),
		'darken' => __( 'Darken', 'rl_conductor' ),
		'dream' => __( 'Dream', 'rl_conductor' ),
		'everglow' => __( 'Everglow', 'rl_conductor' ),
		'forest' => __( 'Forest', 'rl_conductor' ),
		'freshblue' => __( 'Fresh Blue', 'rl_conductor' ),
		'frozen' => __( 'Frozen', 'rl_conductor' ),
		'hermajesty' => __( 'Her Majesty', 'rl_conductor' ),
		'light' => __( 'Light', 'rl_conductor' ),
		'orangepeel' => __( 'Orange Peel', 'rl_conductor' ),
		'rain' => __( 'Rain', 'rl_conductor' ),
		'retro' => __( 'Retro', 'rl_conductor' ),
		'sepia' => __( 'Sepia', 'rl_conductor' ),
		'summer' => __( 'Summer', 'rl_conductor' ),
		'tender' => __( 'Tender', 'rl_conductor' ),
		'vintage' => __( 'Vintage', 'rl_conductor' ),
		'washed' => __( 'Washed', 'rl_conductor' ),
	);
}
...
function vc_attachment_filter_field( $form_fields, $post ) {
	...
	$html_options = '<option value="">' . __( 'None', 'rl_conductor' ) . '</option>';
	...
	$form_fields['vc-image-filter'] = array(
		...
		'html' => '
			<div style="display:none">
				<span class="vc-filter-label">' . __( 'Image filter', 'rl_conductor' ) . '</span>
...
function vc_media_editor_add_image() {
	...
	// default action is mcms_handle_upload, which forces mcms to check upload with is_uploaded_file()
	...
	foreach ( $ids as $key => $attachment_id ) {
		...
		if ( ! $new_attachment_id || is_mcms_error( $new_attachment_id ) ) {
			continue;
		}
...
function vc_media_editor_preview_image() {
	...
	$attachment_details = mcms_prepare_attachment_for_js( $attachment_id );

	if ( ! isset( $attachment_details['sizes'][ $preferred_size ] ) ) {
		$preferred_size = 'thumbnail';
	}

	$attachment_url = mcms_get_attachment_image_src( $attachment_id, $preferred_size );
...
function vc_attachment_filter_media_meta( $media_meta, $post ) {
	...
	$media_meta .= __( 'Filter:', 'rl_conductor' ) . ' ' . $filters[ $filter_name ];
