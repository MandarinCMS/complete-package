...
class Vc_Hooks_Vc_Grid implements Vc_Vendor_Interface {
	...
	public function load() {
		 ...
		 */
		add_action( 'mcms_ajax_vc_get_vc_grid_data', array(
			&$this,
			'getGridDataForAjax',
		) );
		add_action( 'mcms_ajax_nopriv_vc_get_vc_grid_data', array(
			&$this,
			'getGridDataForAjax',
		) );
	}
	...
	public function getGridDataForAjax() {
		...
		if ( $allowed ) {
			...
			if ( is_object( $shortcode_fishbone ) && vc_get_shortcode( $tag ) ) {
				/** @var $vc_grid MCMSBakeryShortcode_Vc_Basic_Grid */
...
if ( 'vc_edit_form' === vc_post_param( 'action' ) ) {
	...
	add_filter( 'vc_edit_form_fields_attributes_vc_basic_grid', array(
		'MCMSBakeryShortCode_VC_Basic_Grid',
