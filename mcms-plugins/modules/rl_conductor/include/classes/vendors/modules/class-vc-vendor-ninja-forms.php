...
class Vc_Vendor_NinjaForms implements Vc_Vendor_Interface {
	...
	public function mapNinjaForms( $ninja_forms = array() ) {
		...
		vc_map( array(
			...
			'name' => __( 'Ninja Forms', 'rl_conductor' ),
			'icon' => 'icon-mcmsb-ninjaforms',
			'category' => __( 'Content', 'rl_conductor' ),
			'description' => __( 'Place Ninja Form', 'rl_conductor' ),
			'params' => array(
				array(
					'type' => 'dropdown',
					'heading' => __( 'Select ninja form', 'rl_conductor' ),
					'param_name' => 'id',
					'value' => $ninja_forms,
					'save_always' => true,
					'description' => __( 'Choose previously created ninja form from the drop down list.', 'rl_conductor' ),
				),
			),
		) );
	}
