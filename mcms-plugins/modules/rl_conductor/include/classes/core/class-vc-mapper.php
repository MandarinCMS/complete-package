	...
	protected function callActivities() {
		...
		while ( $activity = each( $this->init_activity ) ) {
			...
			if ( 'mapper' === $object ) {
				switch ( $method ) {
					case 'map':
						MCMSBMap::map( $params['tag'], $params['attributes'] );
						break;
					case 'drop_param':
						MCMSBMap::dropParam( $params['name'], $params['attribute_name'] );
						break;
					case 'add_param':
						MCMSBMap::addParam( $params['name'], $params['attribute'] );
						break;
					case 'mutate_param':
						MCMSBMap::mutateParam( $params['name'], $params['attribute'] );
						break;
					case 'drop_all_shortcodes':
						MCMSBMap::dropAllShortcodes();
						break;
					case 'drop_shortcode':
						MCMSBMap::dropShortcode( $params['name'] );
						break;
					case 'modify':
						MCMSBMap::modify( $params['name'], $params['setting_name'], $params['value'] );
						break;
				}
			}
		}
