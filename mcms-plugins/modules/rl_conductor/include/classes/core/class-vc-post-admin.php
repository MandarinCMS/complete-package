...
class Vc_Post_Admin {
	...
	public function save( $post_id ) {
		...
		if ( ! vc_user_access()
			->mcmsAny( array( 'edit_post', $post_id ) )
			->get()
		) {
			return;
		}
		$this->setJsStatus( $post_id );
		if ( ! ( isset( $_POST['mcms-preview'] ) && 'dopreview' === $_POST['mcms-preview'] ) ) {
		...
		if ( null !== $post_custom_css && empty( $post_custom_css ) ) {
			delete_post_meta( $post_id, '_mcmsb_post_custom_css' );
		} elseif ( null !== $post_custom_css ) {
			$post_custom_css = strip_tags( $post_custom_css );
			update_post_meta( $post_id, '_mcmsb_post_custom_css', $post_custom_css );
		}
	 ...
	 *
	 * If post param 'mcmsb_vc_js_status' set to true, then methods adds/updated post
	 * meta option with tag '_mcmsb_vc_js_status'.
	 * @since 4.4
	 *
	 * @param $post_id
	 */
	public function setJsStatus( $post_id ) {
		$value = vc_post_param( 'mcmsb_vc_js_status' );
		if ( null !== $value ) {
			// Add value
			if ( '' === get_post_meta( $post_id, '_mcmsb_vc_js_status' ) ) {
				add_post_meta( $post_id, '_mcmsb_vc_js_status', $value, true );
			} // Update value
			elseif ( get_post_meta( $post_id, '_mcmsb_vc_js_status', true ) != $value ) {
				update_post_meta( $post_id, '_mcmsb_vc_js_status', $value );
			} // Delete value
			elseif ( '' === $value ) {
				delete_post_meta( $post_id, '_mcmsb_vc_js_status', get_post_meta( $post_id, '_mcmsb_vc_js_status', true ) );
			}
		}
	}
	...
	public function setInterfaceVersion( $post_id ) {
		if ( null !== ( $value = vc_post_param( 'mcmsb_vc_js_interface_version' ) ) ) {
			update_post_meta( $post_id, '_mcmsb_vc_js_interface_version', $value );
		}
	}
