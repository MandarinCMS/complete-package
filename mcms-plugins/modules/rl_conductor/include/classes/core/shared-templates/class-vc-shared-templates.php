...
require_once dirname( __FILE__ ) . '/importer/mandarincms-importer.php';
require_once dirname( __FILE__ ) . '/importer/module.php';
...
class Vc_Shared_Templates {
	 ...
	 * @var string
	 */
	protected $download_link_url = 'http://support.jiiworks.net/templates/download-link';
	...
	public function init() {
		...
		*/
		...
		add_filter( 'mcms_ajax_vc_shared_templates_download', array(
			$this,
	...
	public function delete( $templateId, $templateType ) {
		if ( 'shared_templates' === $templateType ) {
			...
			if ( ! empty( $templates ) ) {
				$template = $templates[0];
				if ( mcms_delete_post( $template->ID ) ) {
	...
	/**
	 * Post type from templates registration in mandarincms
	 */
	...
	public function ajaxDownloadTemplate() {
		...
		$data = array();
		if ( $file ) {
			new Vc_WXR_Parser_Module();
			$importer = new Vc_MCMS_Import();
			ob_start();
	...
	private function downloadTemplate( $requestUrl ) {
		$downloadUrlRequest = mcms_remote_get( $requestUrl );
	...
	private function parseRequest( $request ) {
		...
		if ( isset( $body['status'], $body['url'] ) && 1 === $body['status'] ) {
			...
			if ( is_mcms_error( $downloadedTemplateFile ) || ! $downloadedTemplateFile ) {
				return false;
			}
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			...
			'category_name' => __( 'Template library', 'rl_conductor' ),
			'category_weight' => 10,
	...
	private function getTemplateDownloadLink( $id ) {
		...
		$url = $this->download_link_url . '?product=vc&url=' . $url . '&key=' . $key . '&version=' . MCMSB_VC_VERSION . '&id=' . esc_attr( $id );
