...
/**
 * MCMSBakery RazorLeaf Conductor Main manager.
 *
...
class MCMSBMap {
	...
	protected static function getSettings() {
		...
		// @todo fix_roles? what is this and why it is inside class-mcmsb-map?
	 ...
	 * The best way is to call this method with "init" action callback function of MCMS.
	 *
	...
	public static function map( $tag, $attributes ) {
		if ( ! self::$is_init ) {
			if ( empty( $attributes['name'] ) ) {
				trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'rl_conductor' ), $tag ) );
			} elseif ( empty( $attributes['base'] ) ) {
				trigger_error( sprintf( __( 'Wrong base for shortcode:%s. Base required', 'rl_conductor' ), $tag ) );
			} else {
	...
	public static function getParam( $tag, $param_name ) {
		if ( ! isset( self::$sc[ $tag ] ) ) {
			return trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'rl_conductor' ), $tag ) );
		}
	...
	public static function addParam( $name, $attribute = array() ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'rl_conductor' ), $name ) );
		} elseif ( ! isset( $attribute['param_name'] ) ) {
			trigger_error( sprintf( __( "Wrong attribute for '%s' shortcode. Attribute 'param_name' required", 'rl_conductor' ), $name ) );
		} else {
	...
	public static function mutateParam( $name, $attribute = array() ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			return trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'rl_conductor' ), $name ) );
		} elseif ( ! isset( $attribute['param_name'] ) ) {
			trigger_error( sprintf( __( "Wrong attribute for '%s' shortcode. Attribute 'param_name' required", 'rl_conductor' ), $name ) );
		} else {
	...
	public static function modify( $name, $setting_name, $value = '' ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			...
			return trigger_error( sprintf( __( "Wrong setting_name for shortcode:%s. Base can't be modified.", 'rl_conductor' ), $name ) );
		}
	...
	/**
	 * Sorting method for MCMSBMap::generateUserData method. Called by uasort php function.
	...
	public static function sort( $a, $b ) {
		// _deprecated_function( 'MCMSBMap::sort', '4.4 (will be removed in 4.10)', 'Vc_Sort class, :sortByKey' );
