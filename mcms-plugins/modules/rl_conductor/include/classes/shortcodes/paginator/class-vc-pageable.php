 ...
 * Class Vc_Pageable
 */
abstract class MCMSBakeryShortCode_Vc_Pageable extends MCMSBakeryShortCode {
	/**
	 * @param $settings
	 */
	...
	public function shortcodeScripts() {
		mcms_register_script( 'vc_pageable_owl-carousel', vc_asset_url( 'lib/owl-carousel2-dist/owl.carousel.min.js' ), array(
			'jquery',
		...
		mcms_register_style( 'vc_pageable_owl-carousel-css', vc_asset_url( 'lib/owl-carousel2-dist/assets/owl.min.css' ), array(), MCMSB_VC_VERSION );
	}
	...
	protected function contentLoadMore( $grid_style, $settings, $content ) {
		if ( ! isset( $settings['btn_data'] ) && isset( $settings['button_style'] ) && isset( $settings['button_size'] ) && isset( $settings['button_color'] ) ) {
			...
			. do_shortcode( '[vc_button2 size="' . $settings['button_size'] . '" title="' . __( 'Load more', 'rl_conductor' ) . '" style="' . $settings['button_style'] . '" color="' . $settings['button_color'] . '" el_class="vc_grid-btn-load_more"]' )
			. '</div>';
		} elseif ( isset( $settings['btn_data'] ) ) {
			$data = $settings['btn_data'];
			$data['el_class'] = 'vc_grid-btn-load_more';
			$data['link'] = 'load-more-grid';
			$button3 = new MCMSBakeryShortCode_VC_Btn( array( 'base' => 'vc_btn' ) );
	...
	public function enqueueScripts() {
		mcms_enqueue_script( 'vc_pageable_owl-carousel' );
		mcms_enqueue_style( 'vc_pageable_owl-carousel-css' );
	...
	public function currentUserCanManage( $page_id ) {
		return vc_user_access()
			->mcmsAny( array( 'edit_post', (int) $page_id ) )
			->get();
	}
}
