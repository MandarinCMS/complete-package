...
class MCMSBakeryShortCode_VC_Tta_Accordion extends MCMSBakeryShortCodesContainer {
	...
	/**
	 * @var MCMSBakeryShortCode_VC_Tta_Section
	 */
	...
	public function setGlobalTtaInfo() {
		...
		/** @var $sectionClass MCMSBakeryShortCode_VC_Tta_Section */
		if ( is_object( $sectionClass ) ) {
			VcShortcodeAutoloader::getInstance()->includeClass( 'MCMSBakeryShortCode_VC_Tta_Section' );
			MCMSBakeryShortCode_VC_Tta_Section::$tta_base_shortcode = $this;
			MCMSBakeryShortCode_VC_Tta_Section::$self_count = 0;
			MCMSBakeryShortCode_VC_Tta_Section::$section_info = array();
	...
	public function getParamContent( $atts, $content ) {
		$panelsContent = mcmsb_js_remove_mcmsautop( $content );
	...
	function getActiveSection( $atts, $strict_bounds = false ) {
		...
		if ( $strict_bounds ) {
			...
			if ( $active_section < 1 ) {
				$active_section = 1;
			} elseif ( $active_section > MCMSBakeryShortCode_VC_Tta_Section::$self_count ) {
				$active_section = MCMSBakeryShortCode_VC_Tta_Section::$self_count;
			}
		}
	...
	public function getParamPaginationList( $atts, $content ) {
		...
		if ( ! $isPageEditabe ) {
			VcShortcodeAutoloader::getInstance()->includeClass( 'MCMSBakeryShortCode_VC_Tta_Section' );
			foreach ( MCMSBakeryShortCode_VC_Tta_Section::$section_info as $nth => $section ) {
	...
	public function enqueueTtaStyles() {
		mcms_register_style( 'vc_tta_style', vc_asset_url( 'css/rl_conductor_tta.min.css' ), false, MCMSB_VC_VERSION );
		mcms_enqueue_style( 'vc_tta_style' );
	}
	...
	public function enqueueTtaScript() {
		...
		mcms_register_script( 'vc_tta_autoplay_script', vc_asset_url( 'lib/vc-tta-autoplay/vc-tta-autoplay.min.js' ), array( 'vc_accordion_script' ), MCMSB_VC_VERSION, true );

		mcms_enqueue_script( 'vc_accordion_script' );
		if ( ! vc_is_page_editable() ) {
			mcms_enqueue_script( 'vc_tta_autoplay_script' );
		}
	}
