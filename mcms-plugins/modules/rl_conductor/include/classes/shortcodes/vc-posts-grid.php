...
class MCMSBakeryShortCode_VC_Posts_Grid extends MCMSBakeryShortCode {
	...
	protected function getCategoriesCss( $post_id ) {
		$categories_css = '';
		$post_categories = mcms_get_object_terms( $post_id, $this->getTaxonomies() );
	...
	protected function getMainCssClass( $filter ) {
		return 'mcmsb_' . ( 'yes' === $filter ? 'filtered_' : '' ) . 'grid';
	}
	...
	protected function getPostThumbnail( $post_id, $grid_thumb_size ) {
		return mcmsb_getImageBySize( array( 'post_id' => $post_id, 'thumb_size' => $grid_thumb_size ) );
	}
	...
	protected function getPostExcerpt() {
		remove_filter( 'the_excerpt', 'mcmsautop' );
	...
	protected function getLinked( $post, $content, $type, $css_class ) {
		...
		if ( 'link_post' === $type || empty( $type ) ) {
			...
			$title = sprintf( esc_attr__( 'Permalink to %s', 'rl_conductor' ), $post->title_attribute );
	...
	protected function findBlockTemplate() {
		...
		$user_template = vc_shortcodes_myskin_templates_dir( $template_path );
