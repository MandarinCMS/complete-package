...
class MCMSBakeryShortCode_VC_Basic_Grid extends MCMSBakeryShortCode_Vc_Pageable {
	...
	function __construct( $settings ) {
		...
		$this->attributes_defaults['btn_title'] = __( 'Load more', 'rl_conductor' );
	...
	public function shortcodeScripts() {
		...
		mcms_register_script( 'vc_grid-js-imagesloaded', vc_asset_url( 'lib/bower/imagesloaded/imagesloaded.pkgd.min.js' ) );
		mcms_register_script( 'vc_grid', vc_asset_url( 'js/dist/vc_grid.min.js' ), array(
				'jquery',
				'underscore',
	...
	public function enqueueScripts() {
		...
		mcms_enqueue_script( 'vc_grid-js-imagesloaded' );
		mcms_enqueue_script( 'vc_grid' );
	}
	...
	public function getHash( $atts, $content ) {
		// _deprecated_function( 'MCMSBakeryShortCode_VC_Basic_Grid::getHash', '4.4.3 (will be removed in 4.10)', 'getId resave your grid' );
	...
	public function findPostShortcodeByHash( $page_id, $hash ) {
		// _deprecated_function( 'MCMSBakeryShortCode_VC_Basic_Grid::findPostShortcodeByHash', '4.4.3 (will be removed in 4.10)', 'findPostShortcodeById resave your grid to renew' );
	...
	public function buildItems() {
		...
		foreach ( $post_data as $post ) {
			$post->filter_terms = mcms_get_object_terms( $post->ID, $this->atts['filter_source'], array( 'fields' => 'ids' ) );
			$this->filter_terms = mcms_parse_args( $this->filter_terms, $post->filter_terms );
			$this->items[] = $post;
		}
	}
	...
	public static function convertButton2ToButton3( $atts ) {
		if ( isset( $atts['button_style'] ) || isset( $atts['button_size'] ) || isset( $atts['button_color'] ) ) {
			...
			$atts['button_size'] = '';
			$atts['button_color'] = '';
			$newData = MCMSBakeryShortCode_VC_Btn::convertAttributesToButton3( $oldData );
