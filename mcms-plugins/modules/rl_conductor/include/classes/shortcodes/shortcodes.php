...
/**
 * MCMSBakery RazorLeaf Conductor Shortcodes main
 *
...
if ( ! class_exists( 'MCMSBakeryVisualComposerAbstract' ) ) {
	/**
	 * Class MCMSBakeryVisualComposerAbstract
	 */
	abstract class MCMSBakeryVisualComposerAbstract {
		/**
		 * @var
		 */
		 ...
		 * @var bool
		 */
		protected $settings_as_myskin = false;
		/**
		 * @var bool
		 */
		protected $as_network_module = false;
		/**
		 * @var bool
		 */
		...
		public function assetURL( $asset ) {
			// _deprecated_function( 'MCMSBakeryShortCode::assetURL', '4.5 (will be removed in 4.10)', 'vc_asset_url' );
 ...
 *
 */
define( 'VC_SHORTCODE_CUSTOMIZE_PREFIX', 'vc_myskin_' );
/**
 *
 */
define( 'VC_SHORTCODE_BEFORE_CUSTOMIZE_PREFIX', 'vc_myskin_before_' );
/**
 *
 */
define( 'VC_SHORTCODE_AFTER_CUSTOMIZE_PREFIX', 'vc_myskin_after_' );
/**
 *
 */
define( 'VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG', 'vc_shortcodes_css_class' );
if ( ! class_exists( 'MCMSBakeryShortCode' ) ) {
	/**
	 * Class MCMSBakeryShortCode
	 */
	abstract class MCMSBakeryShortCode extends MCMSBakeryVisualComposerAbstract {
		...
		public function printIconStyles() {
            ...
            <style>
                ...
                .mcmsb_' . esc_attr( $this->settings['base'] ) . ' > .mcmsb_element_wrapper > .mcmsb_element_title > .vc_element-icon,
                .vc_el-container > #' . esc_attr( $this->settings['base'] ) . ' > .vc_element-icon,
                ...
                .mcmsb_' . esc_attr( $this->settings['base'] ) . ' > .mcmsb_element_wrapper > .mcmsb_element_title > .vc_element-icon,
                .mcmsb_' . esc_attr( $this->settings['base'] ) . ' > .mcmsb_element_wrapper > .mcmsb_element_title > .vc_element-icon[data-is-container="true"] {
		...
		protected function registerJs( $param ) {
			if ( is_array( $param ) && ! empty( $param ) ) {
				...
				mcms_register_script( $name, $param, array( 'jquery' ), MCMSB_VC_VERSION, true );
			}
		}
		...
		protected function registerCss( $param ) {
			if ( is_array( $param ) && ! empty( $param ) ) {
				...
				mcms_register_style( $name, $param, array( 'rl_conductor' ), MCMSB_VC_VERSION );
			}
		}
		...
		protected function findShortcodeTemplate() {
			...
			// Check template in myskin directory
			$user_template = vc_shortcodes_myskin_templates_dir( $this->getFileName() . '.php' );
		...
		protected function loadTemplate( $atts, $content = null ) {
			...
			if ( $this->html_template ) {
				...
				trigger_error( sprintf( __( 'Template file is missing for `%s` shortcode. Make sure you have `%s` file in your myskin folder.', 'rl_conductor' ), $this->shortcode, 'mcms-content/myskins/your_myskin/vc_templates/' . $this->shortcode . '.php' ) );
			}
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			if ( null !== $content ) {
				$content = mcmsautop( stripslashes( $content ) );
			}
			...
			if ( isset( $this->settings['custom_markup'] ) && '' !== $this->settings['custom_markup'] ) {
				...
				$elem = str_ireplace( '%mcmsb_element_content%', $this->customMarkup( $markup, $content ), $elem );
				$output .= $elem;
			} else {
				$inner = $this->outputTitle( $this->settings['name'] );
				$inner .= $this->paramsHtmlHolders( $atts );
				$elem = str_ireplace( '%mcmsb_element_content%', $inner, $elem );
				$output .= $elem;
			}
		...
		public function enqueueDefaultScripts() {
			if ( false === self::$default_scripts_enqueued ) {
				mcms_enqueue_script( 'mcmsb_composer_front_js' );
		...
		/**
		 * Return shortcode attributes, see \MCMSBakeryShortCode::output
		 * @since 4.4
		 * @return array
		 */
		...
		public function getCSSAnimation( $css_animation ) {
			...
			if ( '' !== $css_animation && 'none' !== $css_animation ) {
				mcms_enqueue_script( 'waypoints' );
				mcms_enqueue_style( 'animate-css' );
				$output = ' mcmsb_animate_when_almost_visible mcmsb_' . $css_animation . ' ' . $css_animation;
			}
		...
		/**
		 * Create HTML comment for blocks only if mcmsb_debug=true
		 *
		...
		public function endBlockComment( $string ) {
			// _deprecated_function( 'MCMSBakeryShortCode::endBlockComment', '4.7 (will be removed in 4.10)', 'vc_asset_url' );

			return mcmsb_debug() ? '<!-- END ' . $string . ' -->' : '';
		}

		/**
		 * if mcmsb_debug=true return HTML comment
		 *
		 * @since 4.7
		 *
		...
		public function getElementHolder( $width ) {
			...
			$css_class = 'mcmsb_' . $this->settings['base'] . ' mcmsb_content_element ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' );
			$output .= '<div data-element_type="' . $this->settings['base'] . '" class="' . $css_class . '">';
			$output .= str_replace( '%column_size%', mcmsb_translateColumnWidthToFractional( $width ), $column_controls );
			$output .= $this->getCallbacks( $this->shortcode );
			$output .= '<div class="mcmsb_element_wrapper ' . $this->settings( 'wrapper_class' ) . '">';
			$output .= '%mcmsb_element_content%';
			$output .= '</div>';
			$output .= '</div>';
		...
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			$controls_add = '';
			$controls_edit = ' <a class="vc_control column_edit" href="#" title="' . sprintf( __( 'Edit %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_delete = ' <a class="vc_control column_clone" href="#" title="' . sprintf( __( 'Clone %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a> <a class="column_delete" href="#" title="' . sprintf( __( 'Delete %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
		...
		public function getControlsList() {
			...
			$allAccess = vc_user_access_check_shortcode_all( $this->shortcode );
			if ( $allAccess ) {
				return apply_filters( 'vc_mcmsbakery_shortcode_get_controls_list', $this->controls_list, $this->shortcode );
			} else {
				$controls = apply_filters( 'vc_mcmsbakery_shortcode_get_controls_list', $this->controls_list, $this->shortcode );
				if ( $editAccess ) {
		...
		public function getColumnControlsModular( $extended_css = '' ) {
			ob_start();
			vc_include_template( apply_filters( 'vc_mcmsbakery_shortcode_get_column_controls_modular_template', $this->controls_template_file ), array(
		...
		public function getCallbacks( $id ) {
			...
			if ( isset( $this->settings['js_callback'] ) ) {
				foreach ( $this->settings['js_callback'] as $text_val => $val ) {
					// TODO: name explain
					$output .= '<input type="hidden" class="mcmsb_vc_callback mcmsb_vc_' . $text_val . '_callback " name="' . $text_val . '" value="' . $val . '" />';
				}
			}
		 ...
		 * @param $value
		 *
		 * vc_filter: vc_mcmsbakeryshortcode_single_param_html_holder_value - hook to override param value (param type and etc is available in args)
		 *
		 * @return string
		 */
		public function singleParamHtmlHolder( $param, $value ) {
			$value = apply_filters( 'vc_mcmsbakeryshortcode_single_param_html_holder_value', $value, $param, $this->settings, $this->atts );
			$output = '';
			// Compatibility fixes
			$old_names = array(
				'yellow_message',
			...
			if ( ! empty( $param['holder'] ) ) {
				if ( 'input' === $param['holder'] ) {
					$output .= '<' . $param['holder'] . ' readonly="true" class="mcmsb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" value="' . $value . '">';
				} elseif ( in_array( $param['holder'], array(
					'img',
					'iframe',
				) ) ) {
					$output .= '<' . $param['holder'] . ' class="mcmsb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" src="' . $value . '">';
				} elseif ( 'hidden' !== $param['holder'] ) {
					$output .= '<' . $param['holder'] . ' class="mcmsb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '">' . $value . '</' . $param['holder'] . '>';
				}
			}
		...
		protected function outputTitle( $title ) {
			...
			return '<h4 class="mcmsb_element_title"> ' . $this->getIcon( $params ) . esc_attr( $title ) . '</h4>';
		}
...
if ( ! class_exists( 'MCMSBakeryShortCodesContainer' ) ) {
	/**
	 * Class MCMSBakeryShortCodesContainer
	 */
	abstract class MCMSBakeryShortCodesContainer extends MCMSBakeryShortCode {
		/**
		 * @var array
		 */
		...
		public function mainHtmlBlockParams( $width, $i ) {
			...
			return 'data-element_type="' . $this->settings['base'] . '" class="mcmsb_' . $this->settings['base'] . ' ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' ) . ' mcmsb_content_holder vc_shortcodes_container"' . $this->customAdminBlockParams();
		}
		...
		public function containerContentClass() {
			return 'mcmsb_column_container vc_container_for_children vc_clearfix';
		}
		...
		public function getColumnControls( $controls = 'full', $extended_css = '' ) {
			...
			if ( 'bottom-controls' === $extended_css ) {
				$control_title = sprintf( __( 'Append to this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) );
			} else {
				$control_title = sprintf( __( 'Prepend to this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) );
			}

			$controls_move = '<a class="vc_control column_move vc_column-move" data-vc-control="move" href="#" title="' . sprintf( __( 'Move this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><i class="vc-composer-icon vc-c-icon-dragndrop"></i></a>';
			...
			$controls_edit = '<a class="vc_control column_edit" data-vc-control="edit" href="#" title="' . sprintf( __( 'Edit this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><i class="vc-composer-icon vc-c-icon-mode_edit"></i></a>';
			$controls_clone = '<a class="vc_control column_clone" data-vc-control="clone" href="#" title="' . sprintf( __( 'Clone this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><i class="vc-composer-icon vc-c-icon-content_copy"></i></a>';
			$controls_delete = '<a class="vc_control column_delete" data-vc-control="delete" href="#" title="' . sprintf( __( 'Delete this %s', 'rl_conductor' ), strtolower( $this->settings( 'name' ) ) ) . '"><i class="vc-composer-icon vc-c-icon-delete_empty"></i></a>';
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			for ( $i = 0; $i < count( $width ); $i ++ ) {
				...
				$output .= '<div class="mcmsb_element_wrapper">';
		...
		protected function outputTitle( $title ) {
			...
			return '<h4 class="mcmsb_element_title"> ' . $this->getIcon( $params ) . '</h4>';
		}
...
if ( ! class_exists( 'MCMSBakeryShortCodeFishBones' ) ) {
	/**
	 * Class MCMSBakeryShortCodeFishBones
	 */
	class MCMSBakeryShortCodeFishBones extends MCMSBakeryShortCode {
		/**
		 * @var bool
		 */
		...
		public function hookAdmin() {
			...
			if ( vc_is_page_editable() ) {
				...
				$this->addAction( 'mcms_head', 'printIconStyles' );
			}
		...
		/**
		 * @return MCMSBakeryShortCodeFishBones
		 */
		public function shortcodeClass() {
			// _deprecated_function( '\MCMSBakeryShortCodeFishBones::shortcodeClass', '4.9 (will be removed in 4.11)' );
			if ( false !== $this->shortcode_class ) {
				return $this->shortcode_class;
			}

			require_once vc_path_dir( 'SHORTCODES_DIR', 'mandarincms-widgets.php' );

			$class_name = $this->settings( 'php_class_name' ) ? $this->settings( 'php_class_name' ) : 'MCMSBakeryShortCode_' . $this->settings( 'base' );
			...
			if ( class_exists( $class_name ) && is_subclass_of( $class_name, 'MCMSBakeryShortCode' ) ) {
				$this->shortcode_class = new $class_name( $this->settings );
			} else {
				$this->shortcode_class = new MCMSBakeryShortCodeFishBones( $this->settings );
			}
		...
		public static function getElementClass( $tag ) {
			$settings = MCMSBMap::getShortCode( $tag );
			if ( ! $settings ) {
				throw new Exception( 'Element must be mapped in system' );
			}
			require_once vc_path_dir( 'SHORTCODES_DIR', 'mandarincms-widgets.php' );

			$class_name = ! empty( $settings['php_class_name'] ) ? $settings['php_class_name'] : 'MCMSBakeryShortCode_' . $tag;
			...
			if ( class_exists( $class_name ) && is_subclass_of( $class_name, 'MCMSBakeryShortCode' ) ) {
				...
				$shortcode_class = new MCMSBakeryShortCodeFishBones( $settings );
			}
...
final class Vc_Shortcodes_Manager {
	 ...
	 */
	public function settings( $name ) {
		$settings = MCMSBMap::getShortCode( $this->tag );
	...
	public function buildShortcodesAssets() {
		$elements = MCMSBMap::getAllShortCodes();
	...
	public function buildShortcodesAssetsForEditable() {
		$elements = MCMSBMap::getAllShortCodes(); // @todo create pull to use only where it is set inside function. BC problem
		foreach ( $elements as $tag => $settings ) {
			$element_class = $this->getElementClass( $tag );
			$element_class->printIconStyles();
		}
