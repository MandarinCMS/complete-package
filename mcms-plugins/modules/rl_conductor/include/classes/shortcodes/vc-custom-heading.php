...
/**
 * Class MCMSBakeryShortCode_VC_Custom_heading
 * @since 4.3
 */
class MCMSBakeryShortCode_VC_Custom_heading extends MCMSBakeryShortCode {
	/**
	...
	protected function getParamData( $key ) {
		return MCMSBMap::getParam( $this->shortcode, $this->getField( $key ) );
	}
	...
	public function getStyles( $el_class, $css, $google_fonts_data, $font_container_data, $atts ) {
		...
		if ( ( ! isset( $atts['use_myskin_fonts'] ) || 'yes' !== $atts['use_myskin_fonts'] ) && ! empty( $google_fonts_data ) && isset( $google_fonts_data['values'], $google_fonts_data['values']['font_family'], $google_fonts_data['values']['font_style'] ) ) {
