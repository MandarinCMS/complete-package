...
class MCMSBakeryShortCode_VC_Accordion_tab extends MCMSBakeryShortCode_VC_Tab {
	...
	public function mainHtmlBlockParams( $width, $i ) {
		return 'data-element_type="' . $this->settings['base'] . '" class=" mcmsb_' . $this->settings['base'] . '"' . $this->customAdminBlockParams();
	}
	...
	public function contentAdmin_old( $atts, $content = null ) {
		...
		$column_controls = $this->getColumnControls( $this->settings( 'controls' ) );
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			$output .= '<div class="group mcmsb_sortable">';
			$output .= '<div class="mcmsb_element_wrapper">';
			$output .= '<div class="vc_row-fluid mcmsb_row_container">';
			$output .= '<h3><a href="#">' . $title . '</a></h3>';
			$output .= '<div data-element_type="' . $this->settings['base'] . '" class=" mcmsb_' . $this->settings['base'] . ' mcmsb_sortable">';
			$output .= '<div class="mcmsb_element_wrapper">';
			$output .= '<div class="vc_row-fluid mcmsb_row_container">';
			$output .= do_shortcode( shortcode_unautop( $content ) );
			$output .= '</div>';
