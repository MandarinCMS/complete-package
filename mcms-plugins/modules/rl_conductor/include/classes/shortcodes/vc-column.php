 ...
 *
 */
class MCMSBakeryShortCode_VC_Column extends MCMSBakeryShortCode {
	/**
	 * @var array
	 */
	...
	public function getColumnControls( $controls, $extended_css = '' ) {
		...
		if ( ' bottom-controls' === $extended_css ) {
			$control_title = __( 'Append to this column', 'rl_conductor' );
		} else {
			$control_title = __( 'Prepend to this column', 'rl_conductor' );
		}
		...
		$controls_edit = '<a class="vc_control column_edit vc_column-edit"  data-vc-control="edit" href="#" title="' . __( 'Edit this column', 'rl_conductor' ) . '"><i class="vc-composer-icon vc-c-icon-mode_edit"></i></a>';
		$controls_delete = '<a class="vc_control column_delete vc_column-delete" data-vc-control="delete"  href="#" title="' . __( 'Delete this column', 'rl_conductor' ) . '"><i class="vc-composer-icon vc-c-icon-delete_empty"></i></a>';
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			...
			$output .= str_replace( '%column_size%', mcmsb_translateColumnWidthToFractional( $width[ $i ] ), $column_controls );
			...
			$output .= '</div>';
			$output .= str_replace( '%column_size%', mcmsb_translateColumnWidthToFractional( $width[ $i ] ), $column_controls_bottom );
			$output .= '</div>';
		}
	...
	public function mainHtmlBlockParams( $width, $i ) {
		...
		return 'data-element_type="' . $this->settings['base'] . '" data-vc-column-width="' . mcmsb_vc_get_column_width_indent( $width[ $i ] ) . '" class="mcmsb_' . $this->settings['base'] . ' ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' ) . ' ' . $this->templateWidth() . ' mcmsb_content_holder"' . $this->customAdminBlockParams();
	}
