...
/**
 * MCMSBakery RazorLeaf Conductor shortcode attributes fields
 *
...
class Vc_Edit_Form_Fields implements Vc_Render {
	...
	public function __construct( $tag, $atts ) {
		...
		$this->setSettings( MCMSBMap::getShortCode( $this->tag ) );
	}
	...
	protected function renderGroupedFields( $groups, $groups_content ) {
		...
		if ( sizeof( $groups ) > 1 || ( sizeof( $groups ) >= 1 && empty( $groups_content['_general'] ) ) ) {
			...
			$key = 0;
			foreach ( $groups as $g ) {
				$output .= '<li class="vc_edit-form-tab-control" data-tab-index="' . $key . '"><button data-vc-ui-element-target="#vc_edit-form-tab-' . $key ++ . '" class="vc_ui-tabs-line-trigger" data-vc-ui-element="panel-tab-control">' . ( '_general' === $g ? __( 'General', 'rl_conductor' ) : $g ) . '</button></li>';
			}
	...
	public function render() {
		...
		if ( ! empty( $deprecated ) ) {
			$output .= '<div class="vc_row vc_ui-flex-row vc_shortcode-edit-form-deprecated-message"><div class="vc_col-sm-12 mcmsb_element_wrapper">' . vc_message_warning( sprintf( __( 'You are using outdated element, it is deprecated since version %s.', 'rl_conductor' ), $this->setting( 'deprecated' ) ) ) . '</div></div>';
		}
		$output .= '<div class="' . implode( ' ', $editor_css_classes ) . '" data-title="' . htmlspecialchars( __( 'Edit', 'rl_conductor' ) . ' ' . __( $this->setting( 'name' ), 'rl_conductor' ) ) . '">';
		if ( is_array( $params ) ) {
	...
	protected function renderField( $param, $value ) {
		...
		$output .= ( isset( $param['heading'] ) ) ? '<div class="mcmsb_element_label">' . $param['heading'] . '</div>' : '';
