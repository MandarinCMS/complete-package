...
class Vc_Templates_Panel_Editor {
	 ...
	 * @var string
	 */
	protected $option_name = 'mcmsb_js_templates';
	/**
	 * @since 4.4
	 * @var bool
	 */
	...
	public function init() {
		 ...
		 */
		add_action( 'mcms_ajax_vc_save_template', array(
			&$this,
			'save',
		) );
		add_action( 'mcms_ajax_vc_backend_load_template', array(
			&$this,
			'renderBackendTemplate',
		) );
		add_action( 'mcms_ajax_vc_frontend_load_template', array(
			&$this,
			'renderFrontendTemplate',
		) );
		add_action( 'mcms_ajax_vc_load_template_preview', array(
			&$this,
			'renderTemplatePreview',
		) );
		add_action( 'mcms_ajax_vc_delete_template', array(
			&$this,
			'delete',
		) );
		...
		if ( 'my_templates' === $category['category'] ) {
			...
			if ( vc_user_access()->part( 'templates' )->checkStateAny( true, null )->get() ) {
				...
				<div class="vc_column vc_col-sm-12" data-vc-hide-on-search="true">
					<div class="vc_element_label">' . esc_html( __( 'Save current layout as a template', 'rl_conductor' ) ) . '</div>
					<div class="vc_input-group">
						<input name="padding" data-js-element="vc-templates-input" class="vc_form-control mcmsb-textinput vc_panel-templates-name" type="text" value="" placeholder="' . esc_attr( 'Template name', 'rl_conductor' ) . '" data-vc-disable-empty="#vc_ui-save-template-btn">
						<span class="vc_input-group-btn">
							<button class="vc_general vc_ui-button vc_ui-button-size-sm vc_ui-button-action vc_ui-button-shape-rounded vc_template-save-btn" id="vc_ui-save-template-btn" disabled data-vc-ui-element="button-save">' . esc_html( __( 'Save Template', 'rl_conductor' ) ) . '</button>
						</span>
					</div>
					<span class="vc_description">' . esc_html( __( 'Save layout and reuse it on different sections of this site.', 'rl_conductor' ) ) . '</span>
				</div>';
			}
	...
	public function renderTemplateWindowMyTemplates( $template_name, $template_data ) {
		...
		$preview_template_title = esc_attr( 'Preview template', 'rl_conductor' );
		$add_template_title = esc_attr( 'Add template', 'rl_conductor' );
		$deleteControlRender = '';
		if ( vc_user_access()->part( 'templates' )->checkStateAny( true, null )->get() ) {
			$delete_template_title = esc_attr( 'Delete template', 'rl_conductor' );
...
HTML;
	...
	function renderFrontendTemplate() {
		vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( 'edit_posts', 'edit_pages' )->validateDie()->part( 'templates' )->can()->validateDie();
		...
		add_action( 'mcms_print_scripts', array(
			&$this,
	...
	public function save() {
		vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( 'edit_posts', 'edit_pages' )->validateDie()->part( 'templates' )->checkStateAny( true, null )->validateDie();
	...
	public function renderTemplatePreview() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( __( 'Error: wrong template id.', 'rl_conductor' ) );
		}
		MCMSBMap::addAllMappedShortcodes();
		if ( 'my_templates' === $template_type ) {
			$saved_templates = get_option( $this->option_name );
	...
	public function registerPreviewScripts() {
		...
		mcms_register_script( 'vc_editors-templates-preview-js', vc_asset_url( 'js/editors/templates-preview.js' ), array(
			'vc-backend-min-js',
	...
	public function enqueuePreviewScripts() {
		...
		mcms_enqueue_script( 'vc_editors-templates-preview-js' );
	}
	...
	public function getAllTemplates() {
		...
		if ( apply_filters( 'vc_show_user_templates', true ) ) {
			...
			$arr_category = array(
				...
				'category_name' => __( 'My Templates', 'rl_conductor' ),
				'category_description' => __( 'Append previously saved template to the current layout.', 'rl_conductor' ),
				'category_weight' => 10,
			);
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Default Templates', 'rl_conductor' ),
				'category_description' => __( 'Append default template to the current layout.', 'rl_conductor' ),
				'category_weight' => 11,
			);
			$category_templates = array();
	 ...
	 *      array(
	 *          array(
	 *              'name'=>__('My custom template','my_module'),
	 *              'image_path'=> preg_replace( '/\s/', '%20', modules_url( 'images/my_image.png', __FILE__ ) ), //
	 ...
	 *
	 * Used by action 'mcms_print_scripts'.
	 *
	...
	public function addScriptsToTemplatePreview() {
		// mcms_enqueue_script( 'vc-template-preview-script', vc_asset_url( 'js/editors/vc_ui-panel-templates-preview-be.js' ), array( 'vc-backend-min-js' ), MCMSB_VC_VERSION, true );
	}

	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'rl_conductor' ) );
