...
class Vc_Templates_Editor implements Vc_Render {
	...
	public function init() {
		...
		add_action( 'mcms_ajax_mcmsb_save_template', array( &$this, 'save' ) );
		add_action( 'mcms_ajax_vc_backend_template', array( &$this, 'load' ) );
		add_action( 'mcms_ajax_mcmsb_load_template_shortcodes', array(
			&$this,
			'loadTemplateShortcodes',
		) );
		add_action( 'mcms_ajax_vc_backend_default_template', array(
			&$this,
			'getBackendDefaultTemplate',
		) );
		add_action( 'mcms_ajax_mcmsb_delete_template', array( &$this, 'delete' ) );
	}
	...
	public function outputMenuButton( $id, $params ) {
		...
		$output = '<li data-vc-ui-element="template" class="mcmsb_template_li"><a data-template_id="' . $id . '" href="#">' . htmlspecialchars( $params['name'] ) . '</a> <span class="mcmsb_remove_template" title="' . __( 'Delete template', 'rl_conductor' ) . '" rel="' . $id . '"><i class="icon mcmsb_template_delete_icon"> </i></span></li>';
	...
	public function renderMenu( $only_list = false ) {
		...
		if ( false === $only_list ) {
			$output .= '<li><ul>
                        <li id="mcmsb_save_template"><a href="#" id="mcmsb_save_template_button" class="button">' . __( 'Save current page as a Template', 'rl_conductor' ) . '</a></li>
                        <li class="divider"></li>
                        <li class="nav-header">' . __( 'Load Template', 'rl_conductor' ) . '</li>
                        </ul></li>
                        <li>
                        <ul class="mcmsb_templates_list">';
		}
		if ( empty( $templates ) ) {
			$output .= '<li class="mcmsb_no_templates"><span>' . __( 'No custom templates yet.', 'rl_conductor' ) . '</span></li></ul></li>';
			echo $output;
