...
/**
 * MCMSBakery RazorLeaf Conductor front end editor
 *
...
class Vc_Frontend_Editor implements Vc_Editor_Interface {
	...
	protected $settings = array(
		...
		'module_path' => 'rl_conductor/inline',
	);
	/**
	 * @var string
	 */
	 ...
	 * @var string
	 */
	protected static $brand_url = 'http://vc.jiiworks.net/?utm_campaign=VCmodule&utm_source=vc_user&utm_medium=frontend_editor';
	...
	public function buildEditablePage() {
		...
		add_filter( 'mcms_enqueue_scripts', array(
			&$this,
			'loadIFrameJsCss',
		) );

		add_action( 'mcms_footer', array(
			&$this,
			'printPostShortcodes',
		) );
	}
	...
	public function parseEditableContent( $post ) {
		...
		if ( $post_id > 0 && $post->ID === $post_id && ! defined( 'VC_LOADING_EDITABLE_CONTENT' ) ) {
			...
			remove_filter( 'the_content', 'mcmsautop' );
	...
	public static function shortcodesRegexp() {
		$tagnames = array_keys( MCMSBMap::getShortCodes() );
	...
	/**
	 * Used for mcms filter 'mcms_insert_post_empty_content' to allow empty post insertion.
	 *
	 * @param $allow_empty
	 *
	 * @return bool
	...
	function renderEditor() {
		...
		if ( ! self::inlineEnabled() || ! vc_user_access()->mcmsAny( array(
				'edit_post',
				$this->post_id,
			) )->get()
		) {
		...
		if ( $this->post && 'auto-draft' === $this->post->post_status ) {
			...
			add_filter( 'mcms_insert_post_empty_content', array(
				$this,
				'allowInsertEmptyPost',
			) );
			mcms_update_post( $post_data, true );
		...
		mcms_enqueue_media( array( 'post' => $this->post_id ) );
		remove_all_actions( 'admin_notices', 3 );
		remove_all_actions( 'network_admin_notices', 3 );

		$post_custom_css = strip_tags( get_post_meta( $this->post_id, '_mcmsb_post_custom_css', true ) );
	...
	function setEditorTitle() {
		return sprintf( __( 'Edit %s with RazorLeaf Conductor', 'rl_conductor' ), $this->post_type->labels->singular_name );
	}
	...
	function setEmptyTitlePlaceholder( $title ) {
		return ! is_string( $title ) || strlen( $title ) === 0 ? __( '(no title)', 'rl_conductor' ) : $title;
	}
	...
	function renderEditButton( $link ) {
		if ( $this->showButton( get_the_ID() ) ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc_load-inline-editor" class="vc_inline-link">' . __( 'Edit with RazorLeaf Conductor', 'rl_conductor' ) . '</a>';
		}
	...
	function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			$actions['edit_vc'] = '<a
		href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with RazorLeaf Conductor', 'rl_conductor' ) . '</a>';
		}
	 ...
	 */
	function adminBarEditLink( $mcms_admin_bar ) {
		if ( ! is_object( $mcms_admin_bar ) ) {
		...
		if ( is_singular() ) {
			if ( $this->showButton( get_the_ID() ) ) {
				$mcms_admin_bar->add_menu( array(
					...
					'title' => __( 'Edit with RazorLeaf Conductor', 'rl_conductor' ),
	...
	function loadTinyMceSettings() {
		if ( ! class_exists( '_MCMS_Editors' ) ) {
			require( BASED_TREE_URI . MCMSINC . '/class-mcms-editor.php' );
		}
		$set = _MCMS_Editors::parse_settings( self::$content_editor_id, self::$content_editor_settings );
		_MCMS_Editors::editor_settings( self::$content_editor_id, $set );
	}
	...
	function loadIFrameJsCss() {
		...
		mcms_enqueue_script( 'jquery-ui-droppable' );
		mcms_enqueue_script( 'jquery-ui-draggable' );
		...
		mcms_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/rl_conductor_frontend_editor_iframe.min.css' ), array(), MCMSB_VC_VERSION );
		mcms_enqueue_script( 'waypoints' );
		mcms_enqueue_script( 'mcmsb_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), MCMSB_VC_VERSION, true );
		mcms_enqueue_style( 'rl_conductor_custom_css' );

		mcms_enqueue_script( 'mcmsb_php_js', vc_asset_url( 'lib/php.default/php.default.min.js' ), array( 'jquery' ), MCMSB_VC_VERSION, true );
		mcms_enqueue_script( 'vc_inline_iframe_js', vc_asset_url( 'js/dist/page_editable.min.js' ), array(
			'jquery',
			'underscore',
	...
	function adminFile( $path ) {
		return BASED_TREE_URI . 'mcms-admin/' . $path;
	}

	public function registerJs() {

		mcms_register_script( 'vc_bootstrap_js', vc_asset_url( 'lib/bower/bootstrap3/dist/js/bootstrap.min.js' ), array( 'jquery' ), MCMSB_VC_VERSION, true );
		...
		mcms_register_script( 'mcmsb_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), MCMSB_VC_VERSION, true );
		mcms_register_script( 'vc_accordion_script', vc_asset_url( 'lib/vc_accordion/vc-accordion.min.js' ), array( 'jquery' ), MCMSB_VC_VERSION, true );
		mcms_register_script( 'vc-frontend-editor-min-js', vc_asset_url( 'js/dist/frontend-editor.min.js' ), array(), MCMSB_VC_VERSION, true );
		mcms_localize_script( 'vc-frontend-editor-min-js', 'i18nLocale', visual_composer()->getEditorsLocale() );
	}
	...
	public function registerCss() {
		mcms_register_style( 'ui-custom-myskin', vc_asset_url( 'css/ui-custom-myskin/jquery-ui-less.custom.min.css' ), false, MCMSB_VC_VERSION );
		mcms_register_style( 'animate-css', vc_asset_url( 'lib/bower/animate-css/animate.min.css' ), false, MCMSB_VC_VERSION, 'screen' );
		mcms_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), false, MCMSB_VC_VERSION, 'screen' );

		mcms_register_style( 'vc_inline_css', vc_asset_url( 'css/rl_conductor_frontend_editor.min.css' ), array(), MCMSB_VC_VERSION );
	...
	function enqueueMappedShortcode() {
		foreach ( MCMSBMap::getUserShortCodes() as $shortcode ) {
	...
	public function enqueueMappedShortcodeJs( $value ) {
		mcms_enqueue_script( 'front_enqueue_js_' . md5( $value ), $value, array( 'vc-frontend-editor-min-js' ), MCMSB_VC_VERSION, true );
	}

	public function enqueueMappedShortcodeCss( $value ) {
		mcms_enqueue_style( 'front_enqueue_css_' . md5( $value ), $value, array( 'vc_inline_css' ), MCMSB_VC_VERSION );
	}
	...
	function getPageShortcodesByContent( $content ) {
		...
		foreach ( $not_shortcodes as $string ) {
			$temp = str_replace( array(
				'<p>',
				'</p>',
			), '', $string ); // just to avoid autop @todo maybe do it better like vc_mcmsnop in js.
	...
	function parseShortcodesString( $content, $is_container = false, $parent_id = false ) {
		...
		foreach ( $found[2] as $index => $s ) {
			...
			if ( false !== MCMSBMap::getParam( $s, 'content' ) ) {
