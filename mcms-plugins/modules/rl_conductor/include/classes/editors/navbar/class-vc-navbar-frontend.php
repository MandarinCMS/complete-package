...
class Vc_Navbar_Frontend extends Vc_Navbar {
	 ...
	 * @var string
	 */
	protected $brand_url = 'http://vc.jiiworks.net/?utm_campaign=VCmodule&utm_source=vc_user&utm_medium=frontend_editor';
	...
	public function getControlGuidesSwitch() {
		return '<li class="vc_pull-right">'
		       ...
		       . ' title="' . __( "Toggle editor's guides", 'rl_conductor' ) . '">'
		       . __( 'Guides ON', 'rl_conductor' )
		       . '</button>'
		       . '</li>';
	}
	...
	public function getControlScreenSize() {
		...
		if ( '1' !== $disable_responsive ) {
			$screen_sizes = array(
				array(
					'title' => __( 'Desktop', 'rl_conductor' ),
					'size' => '100%',
					'key' => 'default',
					'active' => true,
				),
				array(
					'title' => __( 'Tablet landscape mode', 'rl_conductor' ),
					'size' => '1024px',
					'key' => 'landscape-tablets',
				),
				array(
					'title' => __( 'Tablet portrait mode', 'rl_conductor' ),
					'size' => '768px',
					'key' => 'portrait-tablets',
				),
				array(
					'title' => __( 'Smartphone landscape mode', 'rl_conductor' ),
					'size' => '480px',
					'key' => 'landscape-smartphones',
				),
				array(
					'title' => __( 'Smartphone portrait mode', 'rl_conductor' ),
					'size' => '320px',
			...
			$output = '<li class="vc_pull-right">'
			          ...
			          . ' title="' . __( 'Responsive preview', 'rl_conductor' ) . '"><i class="vc-composer-icon vc_current-layout-icon vc-c-icon-layout_default"'
	...
	public function getControlSaveUpdate() {
		...
		<li class="vc_show-mobile vc_pull-right">
			<button data-url="<?php esc_attr_e( get_edit_post_link( $post->ID ) . '&mcmsb_vc_js_status=true' ) ?>"
			        class="vc_btn vc_btn-default vc_btn-sm vc_navbar-btn vc_btn-backend-editor" id="vc_button-cancel"
			        title="<?php _e( 'Cancel all changes and return to MCMS dashboard', 'rl_conductor' ) ?>"><?php echo vc_user_access()
					->part( 'backend_editor' )
					->can()
					->get() ? __( 'Backend Editor', 'rl_conductor' ) : __( 'Edit', 'rl_conductor' ); ?></button>
			<?php if ( ! in_array( $post->post_status, array(
				...
				<?php if ( 'draft' === $post->post_status ) : ?>
					<button type="button" class="vc_btn vc_btn-default vc_btn-sm vc_navbar-btn vc_btn-save-draft"
					        ...
					        title="<?php esc_attr_e( 'Save Draft', 'rl_conductor' ) ?>"><?php _e( 'Save Draft', 'rl_conductor' ) ?></button>
				<?php elseif ( 'pending' === $post->post_status && $can_publish ) : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-save-as-pending"
					        title="<?php esc_attr_e( 'Save as Pending', 'rl_conductor' ) ?>"><?php _e( 'Save as Pending', 'rl_conductor' ) ?></button>
				<?php endif ?>
				<?php if ( $can_publish ) : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-update" title="<?php esc_attr_e( 'Publish', 'rl_conductor' ) ?>"
					        data-change-status="publish"><?php _e( 'Publish', 'rl_conductor' ) ?></button>
				<?php else : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-update" title="<?php esc_attr_e( 'Submit for Review', 'rl_conductor' ) ?>"
					        data-change-status="pending"><?php _e( 'Submit for Review', 'rl_conductor' ) ?></button>
				<?php endif ?>
			<?php else : ?>
				<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
				        id="vc_button-update"
				        title="<?php esc_attr_e( 'Update', 'rl_conductor' ) ?>"><?php _e( 'Update', 'rl_conductor' ) ?></button>
			<?php endif ?>
		</li>
		<?php
	...
	public function getControlViewPost() {
		return '<li class="vc_pull-right">'
		       ...
		       . ' title="' . esc_attr__( 'Exit RazorLeaf Conductor edit mode', 'rl_conductor' ) . '"><i class="vc-composer-icon vc-c-icon-close"></i></a>'
		       . '</li>';
	}
}
