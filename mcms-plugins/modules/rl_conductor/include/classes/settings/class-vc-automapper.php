...
if ( ! class_exists( 'Vc_Automapper' ) ) {
	...
	class Vc_Automapper {
		...
		public function __construct() {
			$this->title = __( 'Shortcode Mapper', 'rl_conductor' );
		}
		...
		public function addAjaxActions() {
			add_action( 'mcms_ajax_vc_automapper', array(
				&$this,
				'goAction',
			) );
		...
		public function renderHtml() {
			...
			?>
			<div class="tab_intro">
				<p><?php _e( 'RazorLeaf Conductor Shortcode Mapper adds custom 3rd party vendors shortcodes to the list of RazorLeaf Conductor content elements menu (Note: to map shortcode it needs to be installed on site).', 'rl_conductor' ) ?></p>
			</div>
			<div class="vc_automapper-toolbar">
				<a href="#" class="button button-primary"
					id="vc_automapper-add-btn"><?php _e( 'Map Shortcode', 'rl_conductor' ) ?></a>
			</div>
		...
		public function renderMapFormTpl() {
			...
			<script type="text/html" id="vc_automapper-add-form-tpl">
				<label for="vc_atm-shortcode-string"
					class="vc_info"><?php _e( 'Shortcode string', 'rl_conductor' ) ?></label>

				<div class="vc_wrapper">
					<div class="vc_string">
						<input id="vc_atm-shortcode-string"
							placeholder="<?php _e( 'Please enter valid shortcode', 'rl_conductor' ) ?>"
							type="text" class="vc_atm-string">
					</div>
					<div class="vc_buttons">
						<a href="#" id="vc_atm-parse-string"
							class="button button-primary vc_parse-btn"><?php _e( 'Parse Shortcode', 'rl_conductor' ) ?></a>
						<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'rl_conductor' ) ?></a>
					</div>
				</div>
				<span
					class="description"><?php _e( 'Enter valid shortcode (Example: [my_shortcode first_param="first_param_value"]My shortcode content[/my_shortcode]).', 'rl_conductor' ) ?></span>
			</script>
				...
				<div class="widget-top">
					<div class="widget-title-action">
						...
						<a class="widget-control-edit hide-if-js">
							<span class="edit vc_automapper-edit-btn"><?php _e( 'Edit', 'rl_conductor' ) ?></span>
							<span class="add vc_automapper-delete-btn"><?php _e( 'Add', 'rl_conductor' ) ?></span>
							<span class="screen-reader-text"><?php _e( 'Search', 'rl_conductor' ) ?></span>
						</a>
					</div>
			...
			<script type="text/html" id="vc_automapper-form-tpl">
				...
				<div class="vc_wrapper">
					<h4 class="vc_h"><?php _e( 'General Information', 'rl_conductor' ) ?></h4>

					<div class="vc_field vc_tag">
						<label for="vc_atm-tag"><?php _e( 'Tag:', 'rl_conductor' ) ?></label>
						<input type="text" name="tag" id="vc_atm-tag" value="{{ tag }}">
					</div>
					<div class="vc_field vc_description">
						<label for="vc_atm-description"><?php _e( 'Description:', 'rl_conductor' ) ?></label>
						<textarea name="description" id="vc_atm-description">{{ description }}</textarea>
					</div>
					<div class="vc_field vc_category">
						<label for="vc_atm-category"><?php _e( 'Category:', 'rl_conductor' ) ?></label>
						<input type="text" name="category" id="vc_atm-category" value="{{ category }}">
						<span
							class="description"><?php _e( 'Comma separated categories names', 'rl_conductor' ) ?></span>
					</div>
					<div class="vc_field vc_is-container">
						<label for="vc_atm-is-container"><input type="checkbox" name="is_container"
								...
								value=""> <?php _e( 'Include content param into shortcode', 'rl_conductor' ) ?>
						</label>
					</div>
				</div>
				<div class="vc_line"></div>
				<div class="vc_wrapper">
					<h4 class="vc_h"><?php _e( 'Shortcode Parameters', 'rl_conductor' ) ?></h4>
					<a href="#" id="vc_atm-add-param"
						class="button vc_add-param">+ <?php _e( 'Add Param', 'rl_conductor' ) ?></a>

					<div class="vc_params" id="vc_atm-params-list"></div>
				</div>
				<div class="vc_buttons">
					<a href="#" id="vc_atm-save"
						class="button button-primary"><?php _e( 'Save Changes', 'rl_conductor' ) ?></a>
					<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'rl_conductor' ) ?></a>
					<a href="#" class="button vc_atm-delete"><?php _e( 'Delete', 'rl_conductor' ) ?></a>
				</div>
			</script>
			<script type="text/html" id="vc_atm-form-param-tpl">
				<div class="vc_controls vc_controls-row vc_clearfix"><a
						...
						title="<?php _e( 'Drag row to reorder', 'rl_conductor' ) ?>" data-vc-control="move"><i
							...
							title="<?php _e( 'Delete this param', 'rl_conductor' ) ?>"><i class="vc-composer-icon vc-c-icon-delete_empty"></i></a></span>
				</div>
							...
							<div class="mcmsb_element_wrapper">
								<div class="vc_fields vc_clearfix">
									<div class="vc_param_name vc_param-field">
										<label><?php _e( 'Param name', 'rl_conductor' ) ?></label>
										<# if ( 'content' === param_name) { #>
											<span class="vc_content"><?php _e( 'Content', 'rl_conductor' ) ?></span>
											<input type="text" style="display: none;" name="param_name"
												value="{{ param_name }}"
												placeholder="<?php _e( 'Required value', 'rl_conductor' ) ?>"
												class="vc_param-name"
												data-system="true">
											<span class="description"
												style="display: none;"><?php _e( 'Use only letters, numbers and underscore.', 'rl_conductor' ) ?></span>
											<# } else { #>
												<input type="text" name="param_name" value="{{ param_name }}"
													placeholder="<?php _e( 'Required value', 'rl_conductor' ) ?>"
													class="vc_param-name">
												<span
													class="description"><?php _e( 'Please use only letters, numbers and underscore.', 'rl_conductor' ) ?></span>
												<# } #>
									</div>
									<div class="vc_heading vc_param-field">
										<label><?php _e( 'Heading', 'rl_conductor' ) ?></label>
										<input type="text" name="heading" value="{{ heading }}"
											placeholder="<?php _e( 'Input heading', 'rl_conductor' ) ?>"
										<# if ( 'hidden' === type) { #>
											disabled="disabled"
											<# } #>>
												<span
													class="description"><?php _e( 'Heading for field in shortcode edit form.', 'rl_conductor' ) ?></span>
									</div>
									<div class="vc_type vc_param-field">
										<label><?php _e( 'Field type', 'rl_conductor' ) ?></label>
										<select name="type">
											<option value=""><?php _e( 'Select field type', 'rl_conductor' ) ?></option>
											<option
												value="textfield"<?php echo '<# if (type === "textfield") { #> selected<# } #>' ?>><?php _e( 'Textfield', 'rl_conductor' ) ?></option>
											<option
												value="dropdown"<?php echo '<# if (type === "dropdown") { #> selected<# } #>' ?>><?php _e( 'Dropdown', 'rl_conductor' ) ?></option>
											<option
												value="textarea"<?php echo '<# if(type==="textarea") { #> selected="selected"<# } #>' ?>><?php _e( 'Textarea', 'rl_conductor' ) ?></option>
											<# if ( 'content' === param_name ) { #>
												<option
													value="textarea_html"<?php echo '<# if (type === "textarea_html") { #> selected<# } #>' ?>><?php _e( 'Textarea HTML', 'rl_conductor' ) ?></option>
												<# } #>
													<option
														value="hidden"<?php echo '<# if (type === "hidden") { #> selected<# } #>' ?>><?php _e( 'Hidden', 'rl_conductor' ) ?></option>

										</select>
										<span
											class="description"><?php _e( 'Field type for shortcode edit form.', 'rl_conductor' ) ?></span>
									</div>
									<div class="vc_value vc_param-field">
										<label><?php _e( 'Default value', 'rl_conductor' ) ?></label>
										<input type="text" name="value" value="{{ value }}" class="vc_param-value">
										<span
											class="description"><?php _e( 'Default value or list of values for dropdown type (Note: separate by comma).', 'rl_conductor' ) ?></span>
									</div>
									<div class="description vc_param-field">
										<label><?php _e( 'Description', 'rl_conductor' ) ?></label>
										...
										<# if ( 'hidden' === type ) { #>
											...
											<# } #> >{{ description
												...
												<span
													class="description"><?php _e( 'Enter description for parameter.', 'rl_conductor' ) ?></span>
									</div>
								</div>
							</div>
						</div>
		...
		public function renderTemplates() {
			...
			<script type="text/html" id="vc_automapper-item-tpl">
				...
				<span class="vc_automapper-item-controls">
					<a href="#" class="vc_automapper-delete-btn" title="<?php _e( 'Delete', 'rl_conductor' ) ?>"></a>
					<a href="#" class="vc_automapper-edit-btn" title="<?php _e( 'Edit', 'rl_conductor' ) ?>"></a>
				</span>
			</script>
			<?php
		...
		public function goAction() {
			vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( 'manage_options' )->validateDie()->part( 'settings' )->can( 'vc-automapper-tab' )->validateDie();
