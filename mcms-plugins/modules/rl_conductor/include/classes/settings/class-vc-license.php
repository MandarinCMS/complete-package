...
/**
 * MCMSBakery RazorLeaf Conductor Module
 *
...
class Vc_License {
	 ...
	 * @var string
	 */
	static protected $license_key_option = 'rl_conductor_purchase_code';
	 ...
	 * @var string
	 */
	static protected $support_host = 'http://support.jiiworks.net';
	...
	public function init() {
		...
		add_action( 'mcms_ajax_vc_get_activation_url', array(
			$this,
			'startActivationResponse',
		) );
		add_action( 'mcms_ajax_vc_get_deactivation_url', array(
			$this,
			'startDeactivationResponse',
		) );

		// @deprecated 4.8 Remove after 2015-12-01
		add_action( 'mcms_ajax_mcmsb_activate_license', array(
			$this,
			'activate',
		) );
		add_action( 'mcms_ajax_mcmsb_deactivate_license', array(
			$this,
			'deactivate',
		) );

		add_action( 'mcms_ajax_nopriv_vc_check_license_key', array(
			vc_license(),
	...
	function outputActivatedSuccess() {
		$this->outputNotice( __( 'RazorLeaf Conductor successfully activated.', 'rl_conductor' ), true );
	}
	...
	function outputDeactivatedSuccess() {
		$this->outputNotice( __( 'RazorLeaf Conductor successfully deactivated.', 'rl_conductor' ), true );
	}
	...
	function finishActivationDeactivation( $activation, $user_token ) {
		if ( ! $this->isValidToken( $user_token ) ) {
			$this->showError( __( 'Token is not valid or has expired', 'rl_conductor' ) );
		...
		$response = mcms_remote_post( $url, $params );

		if ( is_mcms_error( $response ) ) {
			$this->showError( __( sprintf( '%s. Please try again.', $response->get_error_message() ), 'rl_conductor' ) );

			return false;
		}

		if ( 200 !== $response['response']['code'] ) {
			$this->showError( __( sprintf( 'Server did not respond with OK: %s', $response['response']['code'] ), 'rl_conductor' ) );
		...
		if ( ! $json || ! isset( $json['status'] ) ) {
			$this->showError( __( 'Invalid response structure. Please contact us for support.', 'rl_conductor' ) );

			return false;
		}

		if ( ! $json['status'] ) {
			$this->showError( __( 'Something went wrong. Please contact us for support.', 'rl_conductor' ) );
	...
	public function checkLicenseKeyFromRemote() {
		...
		if ( ! $this->isValid( $license_key ) ) {
			$response = array(
				'status' => false,
				'error' => __( 'Invalid license key', 'rl_conductor' ),
			);
		} else {
	...
	public function startActivationResponse() {
		vc_user_access()->checkAdminNonce()->validateDie()->mcmsAny( 'manage_options' )->validateDie()->part( 'settings' )->can( 'vc-updater-tab' )->validateDie();
	...
	public function startDeactivationResponse() {
		vc_user_access()->checkAdminNonce()->validateDie( 'Failed nonce check' )->mcmsAny( 'manage_options' )->validateDie( 'Failed access check' )->part( 'settings' )->can( 'vc-updater-tab' )
	...
	public function activate() {
		...
		$params['version'] = MCMSB_VC_VERSION;
		...
		$response = mcms_remote_get( $request_url, array( 'timeout' => 300 ) );
		if ( is_mcms_error( $response ) ) {
	...
	public function setLicenseKey( $license_key ) {
		if ( vc_is_network_module() ) {
			update_site_option( 'mcmsb_js_' . self::$license_key_option, $license_key );
		} else {
	...
	public function getLicenseKey() {
		if ( vc_is_network_module() ) {
			$value = get_site_option( 'mcmsb_js_' . self::$license_key_option );
		} else {
	...
	public function setupReminder() {
		...
		if ( ! $this->isActivated() && ( empty( $_COOKIE['vchideactivationmsg_vc11'] ) || version_compare( $_COOKIE['vchideactivationmsg_vc11'], MCMSB_VC_VERSION, '<' ) ) && ! ( vc_is_network_module() && is_network_admin() ) ) {
			add_action( 'admin_notices', array(
				$this,
	 ...
	 * - ip address
	 * - tld is local, dev, mcms, test, example, localhost or invalid
	...
	public function adminNoticeLicenseActivation() {
		if ( vc_is_network_module() ) {
			update_site_option( 'mcmsb_rl_conductor_license_activation_notified', 'yes' );
		} else {
				...
				$( document ).on( 'click.vc-notice-dismiss',
					...
					function ( e ) {
						...
						setCookie( 'vchideactivationmsg_vc11',
							'<?php echo MCMSB_VC_VERSION; ?>',
							30 );
					} );
			})( window.jQuery );
		</script>
		<?php
		#echo '<div class="updated vc_license-activation-notice" id="vc_license-activation-notice"><p>' . sprintf( __( 'Hola! Would you like to receive automatic updates and unlock premium support? Please <a href="%s">activate your copy</a> of RazorLeaf Conductor.', 'rl_conductor' ), mcms_nonce_url( $redirect ) ) . '</p>' . '<button type="button" class="notice-dismiss vc-notice-dismiss"><span class="screen-reader-text">' . __( 'Dismiss this notice.' ) . '</span></button></div>';
	}
	...
	public function getLicenseKeyToken() {
		$value = vc_is_network_module() ? get_site_option( self::$license_key_token_option ) : get_option( self::$license_key_token_option );
