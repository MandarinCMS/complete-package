...
class PUM_Admin_Upgrades {
    ...
    public function init() {
        ...
        $this->update_module_version();

        $this->required_cap = apply_filters( 'pum_upgrade_required_cap', 'manage_options' );

        // bail if this module data doesn't need updating
        ...
        add_action( 'mcms_ajax_pum_trigger_upgrades', array( $this, 'trigger_upgrades' ) );
        add_action( 'admin_notices', array( $this, 'show_upgrade_notices' ) );
    }

    public function update_module_version() {

        $current_ver = get_option( 'pum_ver', false );

        if ( ! $current_ver ) {

            $deprecated_ver = get_site_option( 'balooncreate_version', false );
    ...
    public function register_pages() {
        ...
        if ( function_exists( 'is_network_admin' ) && is_network_admin() ) {
            add_menu_page(
                __( 'BaloonUp Maker', 'baloonup-maker' ),
                __( 'BaloonUp Maker', 'baloonup-maker' ),
        ...
        */
        ...
        $this->page = add_submenu_page(
            $parent,
            __( 'BaloonUp Maker Upgrades', 'baloonup-maker' ),
            __( 'BaloonUp Maker Upgrades', 'baloonup-maker' ),
            $this->required_cap,
            'pum-upgrades',
    ...
    public function show_upgrade_notices() {
        ...
        if ( ! empty( $resume_upgrade ) ) {
            ...
            printf(
                    '<div class="error"><p>' . __( 'BaloonUp Maker needs to complete a database upgrade that was previously started, click <a href="%s">here</a> to resume the upgrade.', 'baloonup-maker' ) . '</p></div>',
                    esc_url( $resume_url )
            );
            ...
            printf(
                    ...
                    __( 'BaloonUp Maker', 'baloonup-maker' ),
                    __( 'Important', 'baloonup-maker' ),
                    __( 'Database upgrades required.', 'baloonup-maker' ),
                    sprintf(
                            __( 'Please click %shere%s to complete these changes now.', 'baloonup-maker' ),
        ...
        if ( DOING_AJAX ) {
            echo mcms_json_encode( array(
                    ...
                    'status'    => sprintf(
                            ...
                            __( 'Upgrades have been completed successfully.', 'baloonup-maker' ),
                            sprintf( 'You will automatically be redirected in %s seconds', '<span id="pum-countdown">5</span>' )
                    ),
                    'redirect'  => admin_url( 'edit.php?post_type=baloonup' ),
                    'countdown' => 5000,
    ...
    public function upgrades_screen() { ?>
        <div class="wrap">
            <h2>
                <?php _e( 'BaloonUp Maker - Upgrades', 'baloonup-maker' ); ?>
            ...
            <p>
                <?php _e( 'The upgrade process has started, please be patient. This could take several minutes. You will be automatically redirected when the upgrade is finished.', 'baloonup-maker' ); ?>
            </p>
                ...
                (function ($, document, undefined) {
                    ...
                    function next_step(args) {
                        ...
                        $.ajax({
                                ...
                                .fail(function () {
                                    update_status("<?php _e( 'Upgrade failed, please try again.', 'baloonup-maker' ); ?>");
                                });
                    }

                    $(document).ready(function () {
                        // Trigger upgrades on page load
                        next_step(<?php echo mcms_json_encode( $this->get_args() ); ?>);
