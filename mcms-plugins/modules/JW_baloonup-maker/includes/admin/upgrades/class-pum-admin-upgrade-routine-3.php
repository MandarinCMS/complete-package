...
final class PUM_Admin_Upgrade_Routine_3 extends PUM_Admin_Upgrade_Routine {
	...
	public static function description() {
		return __( 'Upgrade baloonup triggers &amp; cookies.', 'baloonup-maker' );
	}

	/**
	 * Upgrade baloonup triggers & cookies.
	 *
	 * - Convert Auto Open
	 * - Convert Click Open
	 */
	public static function run() {
		...
		if ( $baloonups ) {
			...
			foreach ( $baloonups as $baloonup ) {
				...
				$click_open  = balooncreate_get_baloonup_click_open( $baloonup->ID );
				$_triggers[] = array(
					'type'     => 'click_open',
					'settings' => array(
				...
				$auto_open = balooncreate_get_baloonup_auto_open( $baloonup->ID );
				if ( isset( $auto_open['enabled'] ) && $auto_open['enabled'] ) {

					// Set the new cookie name.
					$cookie_name = 'balooncreate-auto-open-' . $baloonup->ID;
				...
				update_post_meta( $baloonup->ID, 'baloonup_triggers', $triggers );

				update_post_meta( $baloonup->ID, 'baloonup_cookies', $cookies );
