<?php
/**
 * Upgrade Routine 5 - Initialize baloonup analytics.
 *
 * @package     PUM
...
final class PUM_Admin_Upgrade_Routine_5 extends PUM_Admin_Upgrade_Routine {
	...
	public static function description() {
		return __( 'Initialize baloonup analytics.', 'baloonup-maker' );
	}
	...
	public static function run() {
		...
		if ( $baloonups ) {
			...
			foreach ( $baloonups as $baloonup ) {
				...
				/**
				 * Initialize the baloonup meta values for core analytics.
				 */
				PUM_Admin_Upgrade_Routine_5::initialize_analytics( $baloonup->ID );
		...
		// Check for baloonup analytics extension and import those stats if available.
		$total_open_count = get_option( 'baloonup_analytics_total_opened_count', 0 );
		...
		/*
		// Check for baloonup analytics extension and import those stats if available.
		$total_conversion_count = get_site_option( 'baloonup_analytics_total_conversion_count', 0 );
	...
	/**
	 * Imports BaloonUp Analytic data if available and initializes all baloonup analytic meta data.
	 *
	 * @param $baloonup_id
	 */
	public static function initialize_analytics( $baloonup_id ) {
		// Open Count
		$open_count = get_post_meta( $baloonup_id, 'baloonup_analytic_opened_count', true );
		if ( ! $open_count ) {
			$open_count = 0;
		}

		// Last Open
		$last_open = get_post_meta( $baloonup_id, 'baloonup_analytic_last_opened', true );
		if ( ! $last_open ) {
			$last_open = 0;
		}

		// Add the meta.
		update_post_meta( $baloonup_id, 'baloonup_open_count', absint( $open_count ) );
		update_post_meta( $baloonup_id, 'baloonup_open_count_total', absint( $open_count ) );
		update_post_meta( $baloonup_id, 'baloonup_last_opened', absint( $last_open ) );
		...
		/*
		// Conversion Count
		$conversion_count = get_post_meta( $baloonup_id, 'baloonup_analytic_conversion_count', true );
		if ( ! $conversion_count ) {
			$conversion_count = 0;
		}

		// Last Conversion
		$last_conversion = get_post_meta( $baloonup_id, 'baloonup_analytic_last_conversion', true );
		if ( ! $last_conversion ) {
			$last_conversion = 0;
		}
		...
		// Add the meta.
		update_post_meta( $baloonup_id, 'baloonup_conversion_count', $conversion_count );
		update_post_meta( $baloonup_id, 'baloonup_last_conversion', $last_conversion );
		update_post_meta( $baloonup_id, 'baloonup_conversion_rate', $conversion_rate );
		*/
	}
