...
final class PUM_Admin_Upgrade_Routine_4 extends PUM_Admin_Upgrade_Routine {
	...
	public static function description() {
		return __( 'Upgrade baloonup targeting conditions.', 'baloonup-maker' );
	}

	/**
	 * Upgrade baloonup targeting conditions.
	 *
	 * - Convert Conditions
	 * - Default baloonups with no conditions to draft
	 */
	public static function run() {
		...
		if ( $baloonups ) {
			...
			foreach ( $baloonups as $baloonup ) {
				...
				// Convert Conditions
				$targeting_conditions = balooncreate_get_baloonup_targeting_condition( $baloonup->ID );

				if ( empty( $targeting_conditions ) ) {
					if ( $baloonup->post_status == 'publish' ) {
						// Default baloonups with no conditions to draft
						PUM_Admin_Upgrade_Routine_4::change_post_status( $baloonup->ID, 'draft' );
					}
					update_post_meta( $baloonup->ID, 'baloonup_conditions', $conditions );
					$completed ++;
					continue;
				}
