<?php
    ...
    class Freemius extends Freemius_Abstract {
         ...
         * @var string
         */
        public $version = MCMS_FS__SDK_VERSION;
         ...
         * @var string
         */
        private $_module_basename;
        /**
         * @since 1.0.0
         *
         * @var string
         */
        private $_free_module_basename;
        /**
         * @since 1.0.0
         *
         * @var string
         */
        private $_module_dir_path;
        /**
         * @since 1.0.0
         *
         * @var string
         */
        private $_module_dir_name;
        /**
         * @since 1.0.0
         *
         * @var string
         */
        private $_module_main_file_path;
        /**
         * @var string[]
         */
         ...
         * @var bool Hints the SDK if module can support anonymous mode (if skip connect is visible).
         */
        private $_enable_anonymous = true;

        /**
         * @since 1.1.7.5
         * @var bool Hints the SDK if module should run in anonymous mode (only adds feedback form).
         */
        private $_anonymous_mode;

        /**
         * @since 1.1.9
         * @var bool Hints the SDK if module have any free plans.
         */
        private $_is_premium_only;

        /**
         * @since 1.2.1.6
         * @var bool Hints the SDK if module have premium code version at all.
         */
        private $_has_premium_version;

        /**
         * @since 1.2.1.6
         * @var bool Hints the SDK if module should ignore pending mode by simulating a skip.
         */
        private $_ignore_pending_mode;

        /**
         * @since 1.0.8
         * @var bool Hints the SDK if the module has any paid plans.
         */
        private $_has_paid_plans;

        /**
         * @since 1.2.1.5
         * @var int Hints the SDK if the module offers a trial period. If negative, no trial, if zero - has a trial but
        ...
        /**
         * @since 1.0.7
         * @var bool Hints the SDK if the module is MandarinCMS.org compliant.
         */
        private $_is_org_compliant;

        /**
         * @since 1.0.7
         * @var bool Hints the SDK if the module is has add-ons.
         */
        private $_has_addons;
         ...
         */
        private $module_affiliate_terms = null;
         ...
         * @since  2.0.0
         *
         * @var bool True if the current request is for a network admin screen and the module is network active.
         */
         ...
         * @since  2.0.0
         *
         * @var int|null The original blog ID the module was loaded with.
         */
...
------------------------------------------------------------------------------------------------------------------*/
        ...
        private function __construct( $module_id, $slug = false, $is_init = false ) {
            ...
            $this->_cache = FS_Cache_Manager::get_manager( MCMS_FS___OPTION_PREFIX . "cache_{$module_id}" );

            $this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_' . $this->get_unique_affix(), MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );

            $this->_module_main_file_path = $this->_find_caller_module_file( $is_init );
            $this->_module_dir_path       = module_dir_path( $this->_module_main_file_path );
            $this->_module_basename       = $this->get_module_basename();
            $this->_free_module_basename  = str_replace( '-premium/', '/', $this->_module_basename );

            $this->_is_multisite_integrated = (
                defined( "MCMS_FS__PRODUCT_{$module_id}_MULTISITE" ) &&
                ( true === constant( "MCMS_FS__PRODUCT_{$module_id}_MULTISITE" ) )
            );

            $this->_is_network_active = (
                is_multisite() &&
                $this->_is_multisite_integrated &&
                // mySkins are always network activated, but the ACTUAL activation is per site.
                $this->is_module() &&
                ( is_module_active_for_network( $this->_module_basename ) ||
                  // Module network level activation or uninstall.
                  is_module_inactive( $this->_module_basename ) )
            );
            ...
            $base_name_split        = explode( '/', $this->_module_basename );
            $this->_module_dir_name = $base_name_split[0];

            if ( $this->_logger->is_on() ) {
                $this->_logger->info( 'module_main_file_path = ' . $this->_module_main_file_path );
                $this->_logger->info( 'module_dir_path = ' . $this->_module_dir_path );
                $this->_logger->info( 'module_basename = ' . $this->_module_basename );
                $this->_logger->info( 'free_module_basename = ' . $this->_free_module_basename );
                $this->_logger->info( 'module_dir_name = ' . $this->_module_dir_name );
            }

            // Remember link between file to slug.
            $this->store_file_slug_map();

            // Store module's initial install timestamp.
            if ( ! isset( $this->_storage->install_timestamp ) ) {
                $this->_storage->install_timestamp = MCMS_FS__SCRIPT_START_TIME;
            }

            if ( ! is_object( $this->_module ) ) {
                $this->_module = FS_Module_Manager::instance( $this->_module_id )->get();
            }

            $this->_admin_notices = FS_Admin_Notices::instance(
                $this->_slug . ( $this->is_myskin() ? ':myskin' : '' ),
                /**
                 * Ensure that the admin notice will always have a title by using the stored module title if available and
                 * retrieving the title via the "get_module_name" method if there is no stored module title available.
                 *
                 * @author Leo Fajardo (@leorw)
                 * @since  1.2.2
                 */
                ( is_object( $this->_module ) ? $this->_module->title : $this->get_module_name() ),
        ...
        /**
         * Check if the context module is free mcms.org myskin.
         *
         * This method is helpful because:
         *      1. mcms.org myskins are limited to a single submenu item,
         *         and sub-submenu items are most likely not allowed (never verified).
         *      2. mcms.org myskins are not allowed to redirect the user
         *         after the myskin activation, therefore, the agreed UX
         ...
         * @return bool
         */
        function is_free_mcms_org_myskin() {
            return (
                $this->is_myskin() &&
         ...
         *
         * @param string $slug
         * @param bool   $ignore_free_mcms_org_myskin_context This is used to decide if the associated tab should be shown
         *                                                 or hidden.
         *
         * @return bool
         */
        function is_submenu_item_visible( $slug, $ignore_free_mcms_org_myskin_context = false ) {
            ...
            if ( ! $ignore_free_mcms_org_myskin_context && $this->is_free_mcms_org_myskin() ) {
                /**
                 * mcms.org myskins are limited to a single submenu item, and
        ...
        private function _version_updates_handler() {
            ...
            if ( ! isset( $this->_storage->module_version ) || $this->_storage->module_version != $module_version ) {
                // Module version upgrade mode.
                $this->_storage->module_last_version = $this->_storage->module_version;
                $this->_storage->module_version      = $module_version;

                if ( empty( $this->_storage->module_last_version ) ||
                     version_compare( $this->_storage->module_last_version, $module_version, '<' )
                ) {
                    $this->_storage->module_upgrade_mode   = true;
                    $this->_storage->module_downgrade_mode = false;
                } else {
                    $this->_storage->module_downgrade_mode = true;
                    $this->_storage->module_upgrade_mode   = false;
                }

                if ( ! empty( $this->_storage->module_last_version ) ) {
                    // Different version of the module was installed before, therefore it's an update.
                    $this->_storage->is_module_new_install = false;
                }

                $this->do_action( 'module_version_update', $this->_storage->module_last_version, $module_version );
            }
        }
        ...
        private static function migrate_install_plan_to_plan_id( FS_Storage $storage, $blog_id = null ) {
            if ( empty( $storage->sdk_version ) ) {
                // New installation of the module, no need to upgrade.
                return;
            }
        ...
        private function consolidate_licenses() {
            $module_licenses = self::get_account_option( 'licenses', MCMS_FS__MODULE_TYPE_PLUGIN );
            if ( isset( $module_licenses[ $this->_slug ] ) ) {
                $module_licenses = $module_licenses[ $this->_slug ];
            } else {
                $module_licenses = array();
            }

            $myskin_licenses = self::get_account_option( 'licenses', MCMS_FS__MODULE_TYPE_THEME );
            if ( isset( $myskin_licenses[ $this->_slug ] ) ) {
                $myskin_licenses = $myskin_licenses[ $this->_slug ];
            } else {
                $myskin_licenses = array();
            }

            if ( empty( $module_licenses ) && empty( $myskin_licenses ) ) {
                return;
            }
            ...
            foreach ( $module_licenses as $user_id => $user_licenses ) {
            ...
            foreach ( $myskin_licenses as $user_id => $user_licenses ) {
                if ( is_array( $user_licenses ) ) {
                    if ( ! isset( $user_license_ids[ $user_id ] ) ) {
                        $user_id_license_ids_map[ $user_id ] = array();
                    }
        ...
        private function make_paths_relative() {
            ...
            if ( isset( $this->_storage->module_main_file ) ) {
                $module_main_file = $this->_storage->module_main_file;

                if ( isset( $module_main_file->path ) ) {
                    $this->_storage->module_main_file->path = $this->get_relative_path( $this->_storage->module_main_file->path );
                } else if ( isset( $module_main_file->prev_path ) ) {
                    $this->_storage->module_main_file->prev_path = $this->get_relative_path( $this->_storage->module_main_file->prev_path );
                }
            }

            // Remove invalid path that is still associated with the current slug if there's any.
            $file_slug_map = self::$_accounts->get_option( 'file_slug_map', array() );
            foreach ( $file_slug_map as $module_basename => $slug ) {
                if ( $slug === $this->_slug &&
                     $module_basename !== $this->_module_basename &&
                     ! file_exists( $this->get_absolute_path( $module_basename ) )
                ) {
                    unset( $file_slug_map[ $module_basename ] );
         ...
         * @since  1.2.2.7
         *
         * @param string $module_prev_version
         * @param string $module_version
         */
        function _after_version_update( $module_prev_version, $module_version ) {
            if ( $this->is_myskin() ) {
                // Expire the cache of the previous tabs since the myskin may
        ...
        /**
         * A special migration logic for the $_accounts, executed for all the modules in the system:
         *  - Moves some data to the network level storage.
         *  - If the module's connection was skipped for all sites, set the module as if it was network skipped.
         *  - If the module's connection was ignored for all sites, don't do anything in terms of the network connection.
         *  - If the module was connected to all sites by the same super-admin, set the module as if was network opted-in for all sites.
         *  - If there's at least one site that was connected by a super-admin, find the "main super-admin" (the one that installed the majority of the module installs) and set the module as if was network activated with the main super-admin, set all the sites that were skipped or opted-in with a different user to delegated mode. Then, prompt the currently logged super-admin to choose what to do with the ignored sites.
         *  - If there are any sites in the network which the connection decision was not yet taken for, set this module into network activation mode so a super-admin can choose what to do with the rest of the sites.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         */
        private static function migrate_accounts_to_network() {
            ...
            foreach ( $sites as $site ) {
                ...
                /**
                 * Build a list of all Freemius powered modules slugs.
                 */
                ...
                foreach ( $id_slug_type_path_map as $module_id => $data ) {
                    if ( MCMS_FS__MODULE_TYPE_PLUGIN === $data['type'] ) {
                        $module_slugs[ $data['slug'] ] = true;
                    }
                }

                $installs = self::get_account_option( 'sites', MCMS_FS__MODULE_TYPE_PLUGIN, $blog_id );
            ...
            foreach ( $module_slugs as $slug => $true ) {
                if ( ! isset( $connection_status[ $slug ] ) ) {
                    $connection_status[ $slug ] = array();
                }
                ...
                foreach ( $sites as $site ) {
                    ...
                    $storage = FS_Storage::instance( MCMS_FS__MODULE_TYPE_PLUGIN, $slug );
            ...
            foreach ( $connection_status as $slug => $blogs_status ) {
                ...
                foreach ( $blogs_status as $blog_id => $status_or_user_id ) {
                    if ( 'skipped' === $status_or_user_id ) {
                        ...
                        if ( isset( $super_admins[ $status_or_user_id ] ) ||
                            ...
                            // Remember opted-in super-admins for the module.
                ...
                if ( $sites_count == $skips ) {
                     ...
                     * the 1st page that will be loaded is the network level MCMS Admin and $storage->network_install_blog_id
                ...
                if ( ( $connections + $skips > 0 ) ) {
                    if ( $ignores > 0 ) {
                         ...
                         * have sites which the connection decision was not yet taken, set this module
        ...
        /**
         * This action is connected to the 'modules_loaded' hook and helps to determine
         * if this is a new module installation or a module update.
         *
         * There are 3 different use-cases:
         *    1) New module installation right with Freemius:
         *       1.1 _activate_module_event_hook() will be executed first
         *       1.2 Since $this->_storage->is_module_new_install is not set,
         *           and $this->_storage->module_last_version is not set,
         *           $this->_storage->is_module_new_install will be set to TRUE.
         *       1.3 When _modules_loaded() will be executed, $this->_storage->is_module_new_install will
         *           be already set to TRUE.
         *
         *    2) Module update, didn't have Freemius before, and now have the SDK:
         *       2.1 _activate_module_event_hook() will not be executed, because
         *           the activation hook do NOT fires on updates since MCMS 3.1.
         *       2.2 When _modules_loaded() will be executed, $this->_storage->is_module_new_install will
         *           be empty, therefore, it will be set to FALSE.
         *
         *    3) Module update, had Freemius in prev version as well:
         *       3.1 _version_updates_handler() will be executed 1st, since FS was installed
         *           before, $this->_storage->module_last_version will NOT be empty,
         *           therefore, $this->_storage->is_module_new_install will be set to FALSE.
         *       3.2 When _modules_loaded() will be executed, $this->_storage->is_module_new_install is
         *           already set, therefore, it will not be modified.
         *
         *    Use-case #3 is backward compatible, #3.1 will be executed since 1.0.9.
         *
         * NOTE:
         *    The only fallback of this mechanism is if an admin updates a module based on use-case #2,
         *    and then, the next immediate PageView is the module's main settings page, it will not
         *    show the opt-in right away. The reason it will happen is because Freemius execution
         *    will be turned off till the module is fully loaded at least once
         *    (till $this->_storage->was_module_loaded is TRUE).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.9
         *
         */
        function _modules_loaded() {
            // Update flag that module was loaded with Freemius at least once.
            $this->_storage->was_module_loaded = true;

            /**
             * Bug fix - only set to false when it's a module, due to the
             * execution sequence of the myskin hooks and our methods, if
             * this will be set for myskins, Freemius will always assume
             * it's a myskin update.
             *
             * @author Vova Feldman (@svovaf)
             * @since  1.2.2.2
             */
            if ( $this->is_module() &&
                 ! isset( $this->_storage->is_module_new_install )
            ) {
                $this->_storage->is_module_new_install = false;
            }
        }
        ...
        /**
         * Add special parameter to MCMS admin AJAX calls so when we
        ...
        private function _register_hooks() {
            ...
            if ( is_admin() ) {
                add_action( 'modules_loaded', array( &$this, '_hook_action_links_and_register_account_hooks' ) );

                if ( $this->is_module() ) {
                    $module_dir = dirname( $this->_module_dir_path ) . '/';
                    ...
                    register_activation_hook(
                        $module_dir . $this->_free_module_basename,
                        array( &$this, '_activate_module_event_hook' )
                    );

                    register_activation_hook(
                        $module_dir . $this->premium_module_basename(),
                        array( &$this, '_activate_module_event_hook' )
                    );
                } else {
                    add_action( 'after_switch_myskin', array( &$this, '_activate_myskin_event_hook' ), 10, 2 );

                    /**
                     * Include the required hooks to capture the myskin settings' page tabs
                     * and cache them.
                     *
                     * @author Vova Feldman (@svovaf)
                     * @since  1.2.2.7
                    ...
                    add_action( 'admin_footer', array( &$this, '_style_premium_myskin' ) );
                }

                /**
                 * Part of the mechanism to identify new module install vs. module update.
                 *
                 * @author Vova Feldman (@svovaf)
                 * @since  1.1.9
                 */
                if ( empty( $this->_storage->was_module_loaded ) ) {
                    /**
                     * During the module activation (not myskin), 'modules_loaded' will be already executed
                     * when the logic gets here since the activation logic first add the activate modules,
                     * then triggers 'modules_loaded', and only then include the code of the module that
                     * is activated. Which means that _modules_loaded() will NOT be executed during the
                     * module activation, and that IS intentional.
                     *
                     * @author Vova Feldman (@svovaf)
                     */
                    if ( $this->is_module() && $this->is_activation_mode( false ) ) {
                        add_action( 'modules_loaded', array( &$this, '_modules_loaded' ) );
                    } else {
                        // If was activated before, then it was already loaded before.
                        $this->_modules_loaded();
                    }
                }

                if ( ! self::is_ajax() ) {
                    if ( ! $this->is_addon() ) {
                        add_action( 'init', array( &$this, '_add_default_submenu_items' ), MCMS_FS__LOWEST_PRIORITY );
                    }
                }
            ...
            if ( $this->is_module() ) {
                if ( $this->_is_network_active ) {
                    add_action( 'mcmsmu_new_blog', array( $this, '_after_new_blog_callback' ), 10, 6 );
                }

                register_deactivation_hook( $this->_module_main_file_path, array( &$this, '_deactivate_module_hook' ) );
            }
            ...
            if ( $this->is_myskin() && self::is_customizer() ) {
                // Register customizer upsell.
                add_action( 'customize_register', array( &$this, '_customizer_register' ) );
            }

            add_action( 'admin_init', array( &$this, '_redirect_on_clicked_menu_link' ), MCMS_FS__LOWEST_PRIORITY );
            ...
            $this->add_action( 'sdk_version_update', array( &$this, '_sdk_version_update' ), MCMS_FS__DEFAULT_PRIORITY, 2 );

            $this->add_action(
                'module_version_update',
                array( &$this, '_after_version_update' ),
                MCMS_FS__DEFAULT_PRIORITY,
                2
            );
        ...
        /**
         * Keeping the uninstall hook registered for free or premium module version may result to a fatal error that
         * could happen when a user tries to uninstall either version while one of them is still active. Uninstalling a
         * module will trigger inclusion of the free or premium version and if one of them is active during the
         * uninstallation, a fatal error may occur in case the module's class or functions are already defined.
         *
         * @author Leo Fajardo (@leorw)
         *
         * @since  1.2.0
        ...
        private function unregister_uninstall_hook() {
            ...
            unset( $uninstallable_modules[ $this->_free_module_basename ] );
            unset( $uninstallable_modules[ $this->premium_module_basename() ] );

            update_option( 'uninstall_modules', $uninstallable_modules );
        }

        /**
         * @since 1.2.0 Invalidate module's main file cache, otherwise, FS_Module_Updater will not fetch updates.
         */
        private function clear_module_main_file_cache() {
            if ( ! isset( $this->_storage->module_main_file ) ||
                 empty( $this->_storage->module_main_file->path )
            ) {
                return;
            }

            $module_main_file = clone $this->_storage->module_main_file;

            // Store cached path (2nd layer cache).
            $module_main_file->prev_path = $module_main_file->path;

            // Clear cached path.
            unset( $module_main_file->path );

            $this->_storage->module_main_file = $module_main_file;
        ...
        function _hook_action_links_and_register_account_hooks() {
            ...
            if ( self::is_modules_page() && $this->is_module() ) {
                $this->hook_module_action_links();
            }
        ...
        private function _register_account_hooks() {
            ...
            if ( $this->is_myskin() && $this->is_premium() && ! $this->has_active_valid_license() ) {
                $this->add_ajax_action(
                    'delete_myskin_update_data',
                    array( &$this, '_delete_myskin_update_data_action' )
                );
            }

            if ( ! $this->is_addon() || $this->is_parent_module_installed() ) {
                if ( ( $this->is_module() && self::is_modules_page() ) ||
                     ( $this->is_myskin() && self::is_myskins_page() )
                ) {
        ...
        /**
         * Leverage backtrace to find caller module file path.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.6
         *
         ...
         * @return string
         */
        private function _find_caller_module_file( $is_init = false ) {
            // Try to load the cached value of the file path.
            if ( isset( $this->_storage->module_main_file ) ) {
                $module_main_file = $this->_storage->module_main_file;
                if ( isset( $module_main_file->path ) ) {
                    $absolute_path = $this->get_absolute_path( $module_main_file->path );
             ...
             * @since 1.2.1
             *
             * `clear_module_main_file_cache()` is clearing the module's cached path on
             * deactivation. Therefore, if any module/myskin was initiating `Freemius`
             * with that module's slug, it was overriding the empty module path with a wrong path.
             *
            ...
            if ( ! $is_init ) {
                ...
                if ( isset( $this->_storage->module_main_file ) &&
                     isset( $this->_storage->module_main_file->prev_path )
                ) {
                    $absolute_path = $this->get_absolute_path( $this->_storage->module_main_file->prev_path );
                    if ( file_exists( $absolute_path ) ) {
                        return $absolute_path;
                    }
                }
                ...
                mcms_die(
                    $this->get_text_inline( 'Freemius SDK couldn\'t find the module\'s main file. Please contact sdk@freemius.com with the current error.', 'failed-finding-main-path' ) .
                    " Module: {$this->_slug}; SDK: " . MCMS_FS__SDK_VERSION . ";",
            ...
            // Find caller module.
            $id_slug_type_path_map            = self::$_accounts->get_option( 'id_slug_type_path_map', array() );
            $this->_storage->module_main_file = (object) array(
        ...
        private function get_module_root_dir_path( $module_type = false ) {
            $is_module = empty( $module_type ) ?
                $this->is_module() :
                ( MCMS_FS__MODULE_TYPE_PLUGIN === $module_type );

            return fs_normalize_path( trailingslashit( $is_module ?
                MCMS_PLUGIN_DIR :
                get_myskin_root() ) );
        }
        ...
        /**
         * Identifies the caller type: module or myskin.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.2
         *
         ...
         *         add-ons are relying on loading the SDK from the parent module, and also allows myskins including the
         *         SDK an internal file instead of directly from functions.php.
         * @since  1.2.1.7 Knows how to handle cases when an add-on includes the parent module logic.
         */
        private function get_caller_main_file_and_type() {
            self::require_module_essentials();
            ...
            foreach ( $all_modules as $relative_path => &$data ) {
                if ( false === strpos( fs_normalize_path( $relative_path ), '/' ) ) {
                    /**
                     * Ignore modules that don't have a folder (e.g. Hello Dolly) since they
            ...
            $caller_map            = array();
            $module_type           = MCMS_FS__MODULE_TYPE_PLUGIN;
            $myskins_dir            = fs_normalize_path( get_myskin_root() );
            ...
            for ( $i = 1, $bt = debug_backtrace(), $len = count( $bt ); $i < $len; $i ++ ) {
                ...
                if ( ! empty( $bt[ $i ]['function'] ) && in_array( $bt[ $i ]['function'], array(
                        'do_action',
                        'apply_filter',
                        // The string split is stupid, but otherwise, myskin check
                        // throws info notices.
                        'requir' . 'e_once',
                        'requir' . 'e',
                        'includ' . 'e_once',
                ...
                if ( 'functions.php' === basename( $caller_file_path ) ) {
                    /**
                     * 1. Assumes that myskin's starting execution file is functions.php.
                    ...
                    if ( $caller_file_path == fs_normalize_path( realpath( trailingslashit( $myskins_dir ) . basename( dirname( $caller_file_path ) ) . '/' . basename( $caller_file_path ) ) ) ) {
                        $module_type = MCMS_FS__MODULE_TYPE_THEME;

                        /**
                         * Relative path of the myskin, e.g.:
                         * `my-myskin/functions.php`
                         *
                         * @author Leo Fajardo (@leorw)
                         */
                ...
                if ( ! isset( $caller_map[ $caller_file_hash ] ) ) {
                    foreach ( $all_modules_paths as $module_path ) {
                        if ( false !== strpos( $caller_file_path, fs_normalize_path( dirname( $module_path ) . '/' ) ) ) {
                            $caller_map[ $caller_file_hash ] = fs_normalize_path( $module_path );
                            break;
                        }
                    }
                }
                ...
                if ( isset( $caller_map[ $caller_file_hash ] ) ) {
                    $module_type           = MCMS_FS__MODULE_TYPE_PLUGIN;
                    $caller_file_candidate = module_basename( $caller_map[ $caller_file_hash ] );
                }
            }
        ...
        /**
         * Displays a confirmation and feedback dialog box when the user clicks on the "Deactivate" link on the modules
         * page.
         *
         * @author Vova Feldman (@svovaf)
         * @author Leo Fajardo (@leorw)
        ...
        function _add_deactivation_feedback_dialog_box() {
            ...
            /**
             * @todo Deactivation form core functions should be loaded only once! Otherwise, when there are multiple Freemius powered modules the same code is loaded multiple times. The only thing that should be loaded differently is the various deactivation reasons object based on the state of the module.
             */
        ...
        function _get_uninstall_reasons( $user_type = 'long-term' ) {
            ...
            $reason_found_better_module = array(
                'id'                => self::REASON_FOUND_A_BETTER_PLUGIN,
                'text'              => sprintf( $this->get_text_inline( 'I found a better %s', 'reason-found-a-better-module' ), $module_type ),
                'input_type'        => 'textfield',
                'input_placeholder' => sprintf( $this->get_text_inline( "What's the %s's name?", 'placeholder-module-name' ), $module_type ),
            );
            ...
            $reason_temporary_deactivation = array(
                ...
                'text'              => sprintf(
                    ...
                    strtolower( $this->is_module() ?
                        $this->get_text_inline( 'Deactivation', 'deactivation' ) :
                        $this->get_text_inline( 'mySkin Switch', 'myskin-switch' )
                    )
                ),
                'input_type'        => '',
                'input_placeholder' => ''
        ...
        /**
         * Called after the user has submitted his reason for deactivating the module.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.1.2
         */
        function _submit_uninstall_reason_action() {
            ...
            /**
             * If the module type is "myskin", trigger the uninstall event here (on myskin deactivation) since myskins do
            ...
            if ( $this->is_myskin() ) {
                if ( $this->is_premium() && ! $this->has_active_valid_license() ) {
                    FS_Module_Updater::instance( $this )->delete_update_data();
                }

                $this->_uninstall_module_event( false );
         ...
         * @since 2.0.2
         */
        function _delete_myskin_update_data_action() {
            FS_Module_Updater::instance( $this )->delete_update_data();
        }
        ...
        static function instance( $module_id, $slug = false, $is_init = false ) {
            ...
            /**
             * Load the essential static data prior to initiating FS_Module_Manager since there's an essential MS network migration logic that needs to be executed prior to the initiation.
             */
            ...
            if ( ! is_numeric( $module_id ) ) {
                ...
                $module = FS_Module_Manager::instance( $slug )->get();
        ...
        private static function get_module_id( $id_or_slug ) {
            ...
            foreach ( self::$_instances as $instance ) {
                if ( $instance->is_module() && ( $id_or_slug === $instance->get_slug() ) ) {
         ...
         * @return false|Freemius
         */
        static function get_instance_by_file( $module_file ) {
            $slug = self::find_slug_by_basename( $module_file );
        ...
        function get_parent_instance() {
            return self::get_instance_by_id( $this->_module->parent_module_id );
        }
         ...
         * @return bool
         */
        function is_parent_module_installed() {
            $is_active = self::has_instance( $this->_module->parent_module_id );
            ...
            /**
             * Parent module might be a myskin. If that's the case, the add-on's FS
             * instance will be loaded prior to the myskin's FS instance, therefore,
            ...
            if ( is_object( $fs_active_modules ) && is_array( $fs_active_modules->modules ) ) {
                $active_myskin = mcms_get_myskin();

                foreach ( $fs_active_modules->modules as $sdk => $module ) {
                    if ( MCMS_FS__MODULE_TYPE_THEME === $module->type ) {
                        if ( $module->module_path == $active_myskin->get_stylesheet() ) {
                            // Parent module is a myskin and it's currently active.
                            return true;
                        }
                    }
                }
        ...
        /**
         * Check if add-on parent module in activation mode.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         *
        ...
        private function is_network_activation_mode( $and_on = true ) {
            ...
            if ( ! $this->is_site_activation_mode( $and_on ) ) {
                // Whether the context is single site or the network, if the module is no longer in activation mode then it is not in network activation mode as well.
                return false;
            }
        ...
        /**
         * Get the basenames of all active modules for specific blog. Including network activated modules.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         *
         ...
         * @return string[]
         */
        private static function get_active_modules_basenames( $blog_id = 0 ) {
            if ( is_multisite() && $blog_id > 0 ) {
                $active_basenames = get_blog_option( $blog_id, 'active_modules' );
            } else {
                $active_basenames = get_option( 'active_modules' );
            }

            if ( is_multisite() ) {
                $network_active_basenames = get_site_option( 'active_sitewide_modules' );
        ...
        /**
         * Get collection of all active modules. Including network activated modules.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         */
        private static function get_active_modules( $blog_id = 0 ) {
            self::require_module_essentials();

            $active_module            = array();
            $all_modules              = get_modules();
            $active_modules_basenames = self::get_active_modules_basenames( $blog_id );

            foreach ( $active_modules_basenames as $module_basename ) {
                $active_module[ $module_basename ] = $all_modules[ $module_basename ];
            }
        ...
        /**
         * Get collection of all site active modules for a specified blog.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         *
         ...
         * @return array[string]array
         */
        private static function get_site_active_modules( $blog_id = 0 ) {
            $active_basenames = ( is_multisite() && $blog_id > 0 ) ?
                get_blog_option( $blog_id, 'active_modules' ) :
                get_option( 'active_modules' );
            ...
            foreach ( $active_basenames as $basename ) {
                $active[ $basename ] = array(
                    ...
                    'slug'      => self::get_module_slug( $basename ),
                );
            }
        ...
        /**
         * Get collection of all modules with their activation status for a specified blog.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.8
         *
         ...
         */
        private static function get_all_modules( $blog_id = 0 ) {
            self::require_module_essentials();
            ...
            foreach ( $all_modules as $basename => &$data ) {
                // By default set to inactive (next foreach update the active modules).
                $data['is_active'] = false;
                // Enrich with module slug.
                $data['slug'] = self::get_module_slug( $basename );
            }

            // Flag active modules.
            foreach ( $active_modules_basenames as $basename ) {
                if ( isset( $all_modules[ $basename ] ) ) {
                    $all_modules[ $basename ]['is_active'] = true;
                }
            }
        ...
        /**
         * Get collection of all modules and if they are network level activated.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         *
         * @return array Key is the module basename and the value is an array of the module data.
         */
        private static function get_network_modules() {
            self::require_module_essentials();

            $all_modules = get_modules();

            $network_active_basenames = is_multisite() ?
                get_site_option( 'active_sitewide_modules' ) :
                array();
        ...
        /**
         * Cached result of get_site_transient( 'update_modules' )
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.8
         *
         * @var object
         */
        private static $_modules_info;

        /**
         * Helper function to get specified module's slug.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.8
         *
         ...
         * @return string
         */
        private static function get_module_slug( $basename ) {
            if ( ! isset( self::$_modules_info ) ) {
                self::$_modules_info = get_site_transient( 'update_modules' );
            }

            $slug = '';

            if ( is_object( self::$_modules_info ) ) {
                if ( isset( self::$_modules_info->no_update ) &&
                     isset( self::$_modules_info->no_update[ $basename ] ) &&
                     ! empty( self::$_modules_info->no_update[ $basename ]->slug )
                ) {
                    $slug = self::$_modules_info->no_update[ $basename ]->slug;
                } else if ( isset( self::$_modules_info->response ) &&
                            isset( self::$_modules_info->response[ $basename ] ) &&
                            ! empty( self::$_modules_info->response[ $basename ]->slug )
                ) {
                    $slug = self::$_modules_info->response[ $basename ]->slug;
                }
            }
            ...
            if ( empty( $slug ) ) {
                // Fallback to module's folder name.
        ...
        private static function _load_required_static() {
            ...
            self::$_static_logger = FS_Logger::get_logger( MCMS_FS__SLUG, MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );

            self::$_static_logger->entrance();

            self::$_accounts = FS_Options::instance( MCMS_FS__ACCOUNTS_OPTION_NAME, true );
            ...
            if ( is_multisite() ) {
                ...
                if ( null === self::$_accounts->get_option( 'id_slug_type_path_map', null, true ) &&
                    ...
                    $api_network_options = FS_Option_Manager::get_manager( MCMS_FS__OPTIONS_OPTION_NAME, true, true );
                    $api_network_options->migrate_to_network();

                    // Migrate API cache to network level storage.
                    FS_Cache_Manager::get_manager( MCMS_FS__API_CACHE_OPTION_NAME )->migrate_to_network();
                }
            }

            self::$_global_admin_notices = FS_Admin_Notices::instance( 'global' );

            if ( ! MCMS_FS__DEMO_MODE ) {
            ...
            add_action( "mcms_ajax_fs_toggle_debug_mode", array( 'Freemius', '_toggle_debug_mode' ) );
            ...
            if ( 0 == did_action( 'modules_loaded' ) ) {
                add_action( 'modules_loaded', array( 'Freemius', '_load_textdomain' ), 1 );
            }
        ...
        static function _load_textdomain() {
            ...
            // Works both for modules and myskins.
            load_module_textdomain(
                'freemius',
                false,
                $fs_active_modules->newest->sdk_path . '/languages/'
            );
        }
        ...
        static function _add_debug_section() {
            ...
            $title = sprintf( '%s [v.%s]', fs_text_inline( 'Freemius Debug' ), MCMS_FS__SDK_VERSION );

            if ( MCMS_FS__DEV_MODE ) {
        ...
        static function _debug_page_actions() {
            ...
            if ( fs_request_is_action( 'restart_freemius' ) ) {
                ...
                delete_option( 'fs_active_modules' );
                ...
                $fs->_storage->install_timestamp = ( time() - MCMS_FS__TIME_24_HOURS_IN_SEC );
                ...
                if ( false === $download_url ) {
                    mcms_die( 'Oops... there was an error while generating the logs download file. Please try again and if it doesn\'t work contact support@freemius.com.' );
                }
        ...
        static function _debug_page_render() {
            ...
            if ( ! is_multisite() ) {
                $all_modules_installs = self::get_all_sites( MCMS_FS__MODULE_TYPE_PLUGIN );
                $all_myskins_installs  = self::get_all_sites( MCMS_FS__MODULE_TYPE_THEME );
            } else {
                $sites = self::get_sites();

                $all_modules_installs = array();
                $all_myskins_installs  = array();

                foreach ( $sites as $site ) {
                    $blog_id = self::get_site_blog_id( $site );

                    $modules_installs = self::get_all_sites( MCMS_FS__MODULE_TYPE_PLUGIN, $blog_id );

                    foreach ( $modules_installs as $slug => $install ) {
                        if ( ! isset( $all_modules_installs[ $slug ] ) ) {
                            $all_modules_installs[ $slug ] = array();
                        }

                        $install->blog_id = $blog_id;

                        $all_modules_installs[ $slug ][] = $install;
                    }

                    $myskins_installs = self::get_all_sites( MCMS_FS__MODULE_TYPE_THEME, $blog_id );

                    foreach ( $myskins_installs as $slug => $install ) {
                        if ( ! isset( $all_myskins_installs[ $slug ] ) ) {
                            $all_myskins_installs[ $slug ] = array();
                        }

                        $install->blog_id = $blog_id;

                        $all_myskins_installs[ $slug ][] = $install;
                    }
                }
            }
            ...
            $vars = array(
                'module_sites'    => $all_modules_installs,
                'myskin_sites'     => $all_myskins_installs,
                'users'           => self::get_all_users(),
                'addons'          => self::get_all_addons(),
                'account_addons'  => self::get_all_account_addons(),
                'module_licenses' => $licenses_by_module_type[ MCMS_FS__MODULE_TYPE_PLUGIN ],
                'myskin_licenses'  => $licenses_by_module_type[ MCMS_FS__MODULE_TYPE_THEME ]
            );
        ...
        /**
         * Check if Freemius should be turned on for the current module install.
         *
         * Note:
        ...
        private function should_run_connectivity_test( $flush_if_no_connectivity = false ) {
            ...
            if ( MCMS_FS__PING_API_ON_IP_OR_HOST_CHANGES ) {
                if ( MCMS_FS__IS_HTTP_REQUEST ) {
                    if ( $_SERVER['HTTP_HOST'] != $this->_storage->connectivity_test['host'] ) {
                        // Domain changed.
                        return true;
                    }

                    if ( MCMS_FS__REMOTE_ADDR != $this->_storage->connectivity_test['server_ip'] ) {
                        // Server IP changed.
                        return true;
                    }
                }
            ...
            if ( $flush_if_no_connectivity ) {
                ...
                return ( isset( $this->_storage->connectivity_test['timestamp'] ) &&
                         ( MCMS_FS__SCRIPT_START_TIME - $this->_storage->connectivity_test['timestamp'] ) > 10 );
            }

            /**
             * @since 1.1.7 Don't check for connectivity on module downgrade.
             */
            $version = $this->get_module_version();
            if ( version_compare( $version, $this->_storage->connectivity_test['version'], '>' ) ) {
                // If it's a module version upgrade and Freemius is off or no connectivity, run connectivity test.
                return true;
            }
        ...
        private function ping( $blog_id = null, $is_gdpr_test = false ) {
            if ( MCMS_FS__SIMULATE_NO_API_CONNECTIVITY ) {
                return false;
            }

            $version = $this->get_module_version();

            $is_update = $this->apply_filters( 'is_module_update', $this->is_module_update() );

            return $this->get_api_module_scope()->ping(
                ...
                array(
                    ...
                    'is_http'      => json_encode( MCMS_FS__IS_HTTP_REQUEST ),
                )
            );
        }
        ...
        function has_api_connectivity( $flush_if_no_connectivity = false ) {
            ...
            if ( ! $this->should_run_connectivity_test( $flush_if_no_connectivity ) ) {
                ...
                $this->_is_on = $this->_storage->connectivity_test['is_active'] ||
                                $this->is_premium() ||
                                ( MCMS_FS__DEV_MODE && $this->_has_api_connection && ! MCMS_FS__SIMULATE_FREEMIUS_OFF );

                return $this->_has_api_connection;
            }

            $pong         = $this->ping();
            $is_connected = $this->get_api_module_scope()->is_valid_ping( $pong );
        ...
        private function store_connectivity_info( $pong, $is_connected ) {
            ...
            if ( ! $is_connected || MCMS_FS__SIMULATE_FREEMIUS_OFF ) {
                $is_active = false;
            } else {
            ...
            $this->_storage->connectivity_test = array(
                ...
                'server_ip'    => MCMS_FS__REMOTE_ADDR,
                'is_active'    => $is_active,
                'timestamp'    => MCMS_FS__SCRIPT_START_TIME,
                // Last version with connectivity attempt.
                'version'      => $version,
            );

            $this->_has_api_connection = $is_connected;
            $this->_is_on              = $is_active || ( MCMS_FS__DEV_MODE && $is_connected && ! MCMS_FS__SIMULATE_FREEMIUS_OFF );
        }
        ...
        private function turn_on() {
            ...
            $updated_connectivity['timestamp'] = MCMS_FS__SCRIPT_START_TIME;
        ...
        function get_anonymous_id( $blog_id = null ) {
            ...
            if ( empty( $unique_id ) || ! is_string( $unique_id ) ) {
                ...
                /**
                 * Base the unique identifier on the MCMS secure authentication key. Which
         ...
         * @return \MCMS_User
         */
        static function _get_current_mcms_user() {
            self::require_pluggable_essentials();

            return mcms_get_current_user();
        }
         ...
         * @return int
         */
        static function get_current_mcms_user_id() {
            $mcms_user = self::_get_current_mcms_user();
        ...
        function _add_connectivity_issue_message( $api_result, $is_first_failure = true ) {
            ...
            if ( ! function_exists( 'mcms_nonce_url' ) ) {
                require_once BASED_TREE_URI . 'mcms-includes/functions.php';
            }

            $current_user = self::_get_current_mcms_user();
//			$admin_email = get_option( 'admin_email' );
            $admin_email = $current_user->user_email;

            // Aliases.
            $deactivate_module_title = $this->esc_html_inline( 'That\'s exhausting, please deactivate', 'deactivate-module-title' );
            $deactivate_module_desc  = $this->esc_html_inline( 'We feel your frustration and sincerely apologize for the inconvenience. Hope to see you again in the future.', 'deactivate-module-desc' );
            ...
            /* translators: %s: product title (e.g. "Awesome Module" requires an access to...) */
                    ...
                    case 'curl_missing':
                        ...
                        $message = sprintf(
                            ...
                            sprintf(
                                ...
                                sprintf(
                                    '<a href="%s"><b>%s</b></a> - %s',
                                    mcms_nonce_url( 'modules.php?action=deactivate&amp;module=' . $this->_module_basename . '&amp;module_status=all&amp;paged=1&amp;s=', 'deactivate-module_' . $this->_module_basename ),
                                    $deactivate_module_title,
                                    $deactivate_module_desc
                                )
                            )
                        );
                        break;
                        ...
                        $message = sprintf(
                            ...
                            sprintf(
                                ...
                                sprintf(
                                    ...
                                    sprintf( 'https://mandarincms.org/modules/%s/download/', $this->_slug ),
                            ...
                            sprintf(
                                ...
                                sprintf(
                                    ...
                                    sprintf(
                                        ...
                                        // We use a filter since the module might require additional API connectivity.
...
//						break;
         ...
         * The email will contain server's info and installed modules (might be caching issue).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         */
        function _email_about_firewall_issue() {
            ...
            if ( $is_connected ) {
                ...
                echo $this->get_after_module_activation_redirect_url();
                exit;
            }
            ...
            $this->send_email(
                ...
                $title . ' [' . $this->get_module_name() . ']',  // subject
        ...
        private function get_email_sections() {
            ...
            $active_module = self::get_active_modules();

            // Generate the list of active modules separated by new line.
            $active_module_string = '';
            foreach ( $active_module as $module ) {
                $active_module_string .= sprintf(
                    '<a href="%s">%s</a> [v%s]<br>',
                    $module['ModuleURI'],
                    $module['Name'],
                    $module['Version']
            ...
            $sections = array(
                ...
                'module'   => array(
                    ...
                    'rows'  => array(
                        'name'    => array( 'Name', $this->get_module_name() ),
                        'version' => array( 'Version', $this->get_module_version() )
                    )
                ),
                'api'      => array(
                    'title' => 'API Subdomain',
                ...
                'modules'  => array(
                    'title' => 'Modules',
                    'rows'  => array(
                        'active_modules' => array( 'Active Modules', $active_module_string )
                    )
                ),
                'php_info' => array(
                    'title' => 'PHP Info',
        ...
        /**
         * Init module's Freemius instance.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.1
         *
         ...
         */
        function dynamic_init( array $module_info ) {
            $this->_logger->entrance();

            $this->parse_settings( $module_info );
                    ...
                    if ( ! $this->has_api_connectivity() ) {
                        if ( $this->_admin_notices->has_sticky( 'failed_connect_api_first' ) ||
                            ...
                            if ( ! $this->_enable_anonymous || $this->is_premium() ) {
                                ...
                                $ajax_action_suffix = $this->_slug . ( $this->is_myskin() ? ':myskin' : '' );
                                add_action( "mcms_ajax_fs_resolve_firewall_issues_{$ajax_action_suffix}", array(
                                    &$this,
                                    '_email_about_firewall_issue'
                                ) );

                                add_action( "mcms_ajax_fs_retry_connectivity_test_{$ajax_action_suffix}", array(
                                    &$this,
            ...
            if ( $this->is_user_in_admin() &&
                 ! $this->is_addon() &&
                 $this->has_addons() &&
                 'module-information' === fs_request_get( 'tab', false ) &&
                 $this->get_id() == fs_request_get( 'parent_module_id', false )
            ) {
                require_once MCMS_FS__DIR_INCLUDES . '/fs-module-info-dialog.php';

                new FS_Module_Info_Dialog( $this );
            }

            // Check if Freemius is on for the current module.
            // This MUST be executed after all the module variables has been loaded.
            ...
            if ( $this->is_addon() ) {
                if ( $this->is_parent_module_installed() ) {
                    // Link to parent FS.
                    $this->_parent = self::get_instance_by_id( $this->_module->parent_module_id );

                    // Get parent module reference.
                    $this->_parent_module = $this->_parent->get_module();
                }
            }
            ...
            if ( $this->is_user_in_admin() ) {
                if ( $this->is_addon() ) {
                    if ( ! $this->is_parent_module_installed() ) {
                        $parent_name = $this->get_option( $module_info, 'parent_name', null );

                        if ( isset( $module_info['parent'] ) ) {
                            $parent_name = $this->get_option( $module_info['parent'], 'name', null );
                        }

                        $this->_admin_notices->add(
                            ( ! empty( $parent_name ) ?
                                sprintf( $this->get_text_x_inline( '%s cannot run without %s.', 'addonX cannot run without moduleY', 'addon-x-cannot-run-without-y' ), $this->get_module_name(), $parent_name ) :
                                sprintf( $this->get_text_x_inline( '%s cannot run without the module.', 'addonX cannot run...', 'addon-x-cannot-run-without-parent' ), $this->get_module_name() )
                            ),
                            $this->get_text_x_inline( 'Oops', 'exclamation', 'oops' ) . '...',
                            'error'
                        );
                        ...
                        if ( $this->_parent->is_registered() && ! $this->is_registered() ) {
                            // If parent module activated, automatically install add-on for the user.
...
//				}
            ...
            if ( $this->is_premium() && $this->has_release_on_freemius() ) {
                FS_Module_Updater::instance( $this );
            }

            $this->do_action( 'initiated' );

            if ( $this->_storage->prev_is_premium !== $this->_module->is_premium ) {
                if ( isset( $this->_storage->prev_is_premium ) ) {
                    $this->apply_filters(
                        'after_code_type_change',
                        // New code type.
                        $this->_module->is_premium
                    );
                } else {
                    // Set for code type for the first time.
                    $this->_storage->prev_is_premium = $this->_module->is_premium;
                }
            }
            ...
            if ( ! $this->is_addon() ) {
                if ( $this->is_registered() ) {
                    ...
                    if ( ! isset( $this->_storage->activation_timestamp ) ) {
                        $this->_storage->activation_timestamp = MCMS_FS__SCRIPT_START_TIME;
                    }

                    $this->do_action( 'after_init_module_registered' );
                } else if ( $this->is_anonymous() ) {
                    $this->do_action( 'after_init_module_anonymous' );
                } else if ( $this->is_pending_activation() ) {
                    $this->do_action( 'after_init_module_pending_activations' );
                }
            } else {
                if ( $this->is_registered() ) {
        ...
        function stop_network_tracking() {
            ...
            // Send update to FS.
            $result = $this->get_current_or_network_user_api_scope()->call( "/modules/{$this->_module_id}/installs.json", 'put', $params );
        ...
        /**
         * Parse module's settings (as defined by the module dev).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.7.3
         *
         ...
         * @throws \Freemius_Exception
         */
        private function parse_settings( &$module_info ) {
            $this->_logger->entrance();

            $id          = $this->get_numeric_option( $module_info, 'id', false );
            $public_key  = $this->get_option( $module_info, 'public_key', false );
            $secret_key  = $this->get_option( $module_info, 'secret_key', null );
            $parent_id   = $this->get_numeric_option( $module_info, 'parent_id', null );
            $parent_name = $this->get_option( $module_info, 'parent_name', null );
             ...
             */
            if ( is_null( $secret_key ) && defined( "MCMS_FS__{$this->_slug}_SECRET_KEY" ) ) {
                $secret_key = constant( "MCMS_FS__{$this->_slug}_SECRET_KEY" );
            }

            if ( isset( $module_info['parent'] ) ) {
                $parent_id = $this->get_numeric_option( $module_info['parent'], 'id', null );
//				$parent_slug       = $this->get_option( $module_info['parent'], 'slug', null );
//				$parent_public_key = $this->get_option( $module_info['parent'], 'public_key', null );
//				$parent_name = $this->get_option( $module_info['parent'], 'name', null );
            }
            ...
            if ( false === $id ) {
                throw new Freemius_Exception( array(
                    'error' => array(
                        ...
                        'message' => 'Module id parameter is not set.',
                        'code'    => 'module_id_not_set',
                        'http'    => 500,
                    )
                ) );
            }
            if ( false === $public_key ) {
                throw new Freemius_Exception( array(
                    'error' => array(
                        'type'    => 'ParameterNotSet',
                        'message' => 'Module public_key parameter is not set.',
                        'code'    => 'module_public_key_not_set',
                        'http'    => 500,
                    )
                ) );
            }

            $module = ( $this->_module instanceof FS_Module ) ?
                $this->_module :
                new FS_Module();

            $module->update( array(
                'id'                   => $id,
                'type'                 => $this->get_option( $module_info, 'type', $this->_module_type ),
                'public_key'           => $public_key,
                'slug'                 => $this->_slug,
                'parent_module_id'     => $parent_id,
                'version'              => $this->get_module_version(),
                'title'                => $this->get_module_name(),
                'file'                 => $this->_module_basename,
                'is_premium'           => $this->get_bool_option( $module_info, 'is_premium', true ),
                'is_live'              => $this->get_bool_option( $module_info, 'is_live', true ),
                'affiliate_moderation' => $this->get_option( $module_info, 'has_affiliation' ),
            ) );

            if ( $module->is_updated() ) {
                // Update module details.
                $this->_module = FS_Module_Manager::instance( $this->_module_id )->store( $module );
            }
            // Set the secret key after storing the module, we don't want to store the key in the storage.
            $this->_module->secret_key = $secret_key;

            if ( ! isset( $module_info['menu'] ) ) {
                $module_info['menu'] = array();

                if ( ! empty( $this->_storage->sdk_last_version ) &&
                     version_compare( $this->_storage->sdk_last_version, '1.1.2', '<=' )
                ) {
                    // Backward compatibility to 1.1.2
                    $module_info['menu']['slug'] = isset( $module_info['menu_slug'] ) ?
                        $module_info['menu_slug'] :
                        $this->_slug;
                }
            }
            ...
            $this->_menu->init( $module_info['menu'], $this->is_addon() );

            $this->_has_addons          = $this->get_bool_option( $module_info, 'has_addons', false );
            $this->_has_paid_plans      = $this->get_bool_option( $module_info, 'has_paid_plans', true );
            $this->_has_premium_version = $this->get_bool_option( $module_info, 'has_premium_version', $this->_has_paid_plans );
            $this->_ignore_pending_mode = $this->get_bool_option( $module_info, 'ignore_pending_mode', false );
            $this->_is_org_compliant    = $this->get_bool_option( $module_info, 'is_org_compliant', true );
            $this->_is_premium_only     = $this->get_bool_option( $module_info, 'is_premium_only', false );
            if ( $this->_is_premium_only ) {
                // If premium only module, disable anonymous mode.
                $this->_enable_anonymous = false;
                $this->_anonymous_mode   = false;
            } else {
                $this->_enable_anonymous = $this->get_bool_option( $module_info, 'enable_anonymous', true );
                $this->_anonymous_mode   = $this->get_bool_option( $module_info, 'anonymous_mode', false );
            }
            $this->_permissions = $this->get_option( $module_info, 'permissions', array() );

            if ( ! empty( $module_info['trial'] ) ) {
                ...
                $this->_is_trial_require_payment = $this->get_bool_option( $module_info['trial'], 'is_require_payment', false );
            }
        }
        ...
        private function should_stop_execution() {
            if ( empty( $this->_storage->was_module_loaded ) ) {
                /**
                 * Don't execute Freemius until module was fully loaded at least once,
                 ...
                 * identification of new module install vs. module update.
                 *
                 * @author Vova Feldman (@svovaf)
                 * @since  1.1.9
                 */
            ...
            if ( $this->is_activation_mode() ) {
                ...
                if ( ! MCMS_FS__IS_HTTP_REQUEST ) {
                     ...
                     * @since  1.1.6.3
                     *
                     * @link   https://mandarincms.org/support/topic/errors-in-the-freemius-class-when-running-in-mandarincms-in-cli
                     */
                    return true;
                }
                ...
                if ( self::is_cron() ) {
                    /**
                     * If in activation mode, don't execute Freemius during mcms crons
                     * (mcms crons have HTTP context - called as HTTP request).
                     *
                     * @author Vova Feldman (@svovaf)
                     * @since  1.1.7.3
                     */
            ...
            if ( $this->is_registered() ) {
                if ( ! $this->is_addon() ) {
                    add_action(
                        ...
                        array( &$this, '_module_code_type_changed' )
                    );
                }
                ...
                if ( $this->is_premium() ) {
                    ...
                    $this->get_api_user_scope()->purge_cache( "/modules/{$this->_module_id}/payments.json?include_addons=true" );
                }
            }
        }

        /**
         * Handles module's code type change (free <--> premium).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         */
        function _module_code_type_changed() {
            $this->_logger->entrance();
            ...
            /**
             * Unregister the uninstall hook for the other version of the module (with different code type) to avoid
             * triggering a fatal error when uninstalling that module. For example, after deactivating the "free" version
             * of a specific module, its uninstall hook should be unregistered after the "premium" version has been
        ...
        function is_addon_connected( $id_or_slug ) {
            ...
            $sites = self::get_all_sites( MCMS_FS__MODULE_TYPE_PLUGIN );
            ...
            $module = FS_Module_Manager::instance( $addon_id )->get();

            if ( $module->parent_module_id != $this->_module->id ) {
                // The given slug do NOT belong to any of the module's add-ons.
                return false;
            }
            ...
            return ( is_object( $site ) &&
                     ...
                     FS_Module_Plan::is_valid_id( $site->plan_id )
            );
        }
        ...
        function is_addon_installed( $id_or_slug ) {
            ...
            return file_exists( fs_normalize_path( MCMS_PLUGIN_DIR . '/' . $this->get_addon_basename( $addon_id ) ) );
        }
        ...
        function get_addon_basename( $id_or_slug ) {
            ...
            if ( $this->is_addon_activated( $addon_id ) ) {
                return self::instance( $addon_id )->get_module_basename();
            }

            $addon            = $this->get_addon( $addon_id );
            $premium_basename = "{$addon->slug}-premium/{$addon->slug}.php";

            if ( file_exists( fs_normalize_path( MCMS_PLUGIN_DIR . '/' . $premium_basename ) ) ) {
        ...
        function get_installed_addons() {
            ...
            foreach ( self::$_instances as $instance ) {
                if ( $instance->is_addon() && is_object( $instance->_parent_module ) ) {
                    if ( $this->_module->id == $instance->_parent_module->id ) {
        ...
        /**
         * Check if any add-ons of the module are installed.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.1.1
         *
        ...
        /**
         * Tell Freemius that the current module is an add-on.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.6
         *
         * @param number $parent_module_id The parent module ID
         */
        function init_addon( $parent_module_id ) {
            $this->_module->parent_module_id = $parent_module_id;
        }
         ...
         */
        function is_addon() {
            return isset( $this->_module->parent_module_id ) && is_numeric( $this->_module->parent_module_id );
        }
        ...
        private function deactivate_premium_only_addon_without_license( $is_after_trial_cancel = false ) {
            if ( ! $this->has_free_plan() &&
                ...
                if ( $this->is_registered() ) {
                    // IF wrapper is turned off because activation_timestamp is currently only stored for modules (not addons).
                    //                if (empty($this->_storage->activation_timestamp) ||
                    //                    (MCMS_FS__SCRIPT_START_TIME - $this->_storage->activation_timestamp) > 30
                    //                ) {
                    /**
                ...
                if ( ! $this->has_free_plan() &&
                    ...
                    // @todo Check if deactivate modules also call the deactivation hook.
                    ...
                    $this->_parent->_admin_notices->add_sticky(
                        sprintf(
                            ( $is_after_trial_cancel ?
                                ...
                                $this->_parent->get_text_inline(
                                    '%s is a premium only add-on. You have to purchase a license first before activating the module.',
                                    'addon-no-license-message'
                                )
                            ),
                            '<b>' . $this->_module->title . '</b>'
                        ) . ' ' . sprintf(
                            '<a href="%s" aria-label="%s" class="button button-primary" style="margin-left: 10px; vertical-align: middle;">%s &nbsp;&#10140;</a>',
                            $this->_parent->addon_url( $this->_slug ),
                            esc_attr( sprintf( $this->_parent->get_text_inline( 'More information about %s', 'more-information-about-x' ), $this->_module->title ) ),
                    ...
                    deactivate_modules( array( $this->_module_basename ), true );
        ...
        function init_sandbox( $secret_key ) {
            ...
            // Update module details.
            FS_Module_Manager::instance( $this->_module_id )->update( $this->_module, true );
        }
        ...
        function is_payments_sandbox() {
            return ( ! $this->is_live() ) || isset( $this->_module->secret_key );
        }

        #endregion

        /**
         * Check if running test vs. live module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.5
         *
        ...
        /**
         * Check if module must be MandarinCMS.org compliant.
         *
         * @since 1.0.7
         *
         * @return bool
         ...
         * @return bool
         */
        private function is_cron_executed( $name, $period = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
            $this->_logger->entrance( $name );
            ...
            return ( $last_execution > ( MCMS_FS__SCRIPT_START_TIME - $period ) );
        }

        /**
         * MCMS Cron is executed on a site level. When running in a multisite network environment
        ...
        private function clear_cron( $name, $action_tag = '', $is_network_clear = false ) {
            ...
            mcms_clear_scheduled_hook( $this->get_action_tag( $action_tag ) );
        ...
        private function get_next_scheduled_cron( $name, $action_tag = '' ) {
            ...
            $next_scheduled = mcms_next_scheduled( $this->get_action_tag( $action_tag ) );
        ...
        private function schedule_cron(
            ...
            $action_tag = '',
            $recurrence = 'single',
            $start_at = MCMS_FS__SCRIPT_START_TIME,
            ...
            if ( 'daily' === $recurrence ) {
                if ( $randomize_start ) {
                    ...
                    $start_at += rand( 0, ( MCMS_FS__TIME_24_HOURS_IN_SEC / 2 ) );
                }

                // Schedule daily MCMS cron.
                mcms_schedule_event(
                    $start_at,
                    'daily',
        ...
        private function run_manual_sync() {
            ...
            $this->schedule_sync_cron( time() + MCMS_FS__TIME_24_HOURS_IN_SEC, false );
        }
        ...
        function _sync_cron_method( array $blog_ids ) {
            if ( $this->is_registered() ) {
                if ( $this->has_paid_plan() ) {
                    ...
                    if ( $this->is_paying() ) {
                        // Check for premium module updates.
         ...
         * @return bool
         */
        private function is_sync_executed( $period = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
        ...
        /**
         * Instead of running blocking install sync event, execute non blocking scheduled mcms-cron.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.7.3
         *
        ...
        /**
         * Check if currently in module activation.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.4
         *
        ...
        function _admin_init_action() {
            /**
             * Automatically redirect to connect/activation page after module activation.
             *
             * @since 1.1.7 Do NOT redirect to opt-in when running in network admin mode.
             */
            if ( $this->is_module_activation() ) {
                delete_transient( "fs_{$this->_module_type}_{$this->_slug}_activated" );

                if ( isset( $_GET['activate-multi'] ) ) {
                    /**
                     * Don't redirect if activating multiple modules at once (bulk activation).
                     */
                } else {
            ...
            if ( ! $this->is_addon() &&
                ...
                if ( ! $this->is_pending_activation() ) {
                    if ( ! $this->_menu->is_main_settings_page() ) {
                        /**
                         * If a user visits any other admin page before activating the premium-only myskin with a valid
                         * license, reactivate the previous myskin.
                         *
                         * @author Leo Fajardo (@leorw)
                         * @since  1.2.2
                         */
                        if ( $this->is_myskin()
                             ...
                             && $this->can_activate_previous_myskin()
                        ) {
                            $this->activate_previous_myskin();
                        ...
                        if ( $this->is_module_new_install() || $this->is_only_premium() ) {
                            // Show notice for new module installations.
                            $this->_admin_notices->add(
                                sprintf(
                                    ...
                                    sprintf( '<b><a href="%s">%s</a></b>',
                                        ...
                                        sprintf( $this->get_text_x_inline( 'Complete "%s" Activation Now',
                                            '%s - module name. As complete "ModuleX" activation now', 'activate-x-now' ), $this->get_module_name() )
                                    )
                                ),
                                '',
                                'update-nag'
                            ...
                            if ( $this->has_filter( 'optin_pointer_element' ) ) {
                                // Don't show admin nag if module update.
                ...
                if ( $this->is_myskin() &&
                    ...
                    $this->_show_myskin_activation_optin_dialog();
                }
            }
        }
        ...
        private function add_sticky_optin_admin_notice() {
            ...
            $this->_admin_notices->add_sticky(
                sprintf(
                    $this->get_text_inline( 'We made a few tweaks to the %s, %s', 'few-module-tweaks' ),
                    $this->_module_type,
                    sprintf( '<b><a href="%s">%s</a></b>',
                        $this->get_activation_url(),
                        sprintf( $this->get_text_inline( 'Opt in to make "%s" Better!', 'optin-x-now' ), $this->get_module_name() )
                    )
                ),
                'connect_account',
                '',
                ...
                jQuery(document).ready(function ($) {
                    if ('undefined' !== typeof(jQuery().pointer)) {
                        ...
                        if (element.length > 0) {
							...
							$('.mcms-pointer #fs_connect')
								.parents('.mcms-pointer.mcms-pointer-top')
        ...
        /**
         * Check if the current page is the module's main admin settings page.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         *
         * @return bool
         */
        function _is_module_page() {
            return fs_is_module_page( $this->_menu->get_raw_slug() ) ||
                   fs_is_module_page( $this->_slug );
        }
        ...
        /**
         * Delete module's plans information.
         *
        ...
        /**
         * Check if Freemius was added on new module installation.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.5
         *
         * @return bool
         */
        function is_module_new_install() {
            return isset( $this->_storage->is_module_new_install ) &&
                   $this->_storage->is_module_new_install;
        }

        /**
         * Check if it's the first module release that is running Freemius.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.1.5
         *
        ...
        function is_first_freemius_powered_version() {
            return empty( $this->_storage->module_last_version );
        }
         ...
         * @return bool|string
         */
        private function get_previous_myskin_slug() {
            return isset( $this->_storage->previous_myskin ) ?
                $this->_storage->previous_myskin :
                false;
        }
         ...
         * @return string
         */
        private function can_activate_previous_myskin() {
            $slug = $this->get_previous_myskin_slug();
            if ( false !== $slug && current_user_can( 'switch_myskins' ) ) {
                $myskin_instance = mcms_get_myskin( $slug );

                return $myskin_instance->exists();
            }
         ...
         * @since  1.2.2
         */
        private function activate_previous_myskin() {
            switch_myskin( $this->get_previous_myskin_slug() );
            unset( $this->_storage->previous_myskin );

            global $pagenow;
            if ( 'myskins.php' === $pagenow ) {
                /**
                 * Refresh the active myskin information.
                 *
                 * @author Leo Fajardo (@leorw)
                 * @since  1.2.2
                 */
         ...
         * @return string
         */
        function get_previous_myskin_activation_url() {
            if ( ! $this->can_activate_previous_myskin() ) {
                return '';
            }
             ...
             * @since  1.2.2
             */
            return mcms_nonce_url(
                $this->admin_url( 'myskins.php?action=activate&stylesheet=' . urlencode( $this->get_previous_myskin_slug() ) ),
                'switch-myskin_' . $this->get_previous_myskin_slug()
            );
        }

        /**
         * Saves the slug of the previous myskin if it still exists so that it can be used by the logic in the opt-in
         * form that decides whether to add a close button to the opt-in dialog or not. So after a premium-only myskin is
         * activated, the close button will appear and will reactivate the previous myskin if clicked. If the previous
         * myskin doesn't exist, then there will be no close button.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.2
         *
         * @param  string        $slug_or_name Old myskin's slug or name.
         * @param  bool|MCMS_mySkin $old_myskin    MCMS_mySkin instance of the old myskin if it still exists.
         */
        function _activate_myskin_event_hook( $slug_or_name, $old_myskin = false ) {
            $this->_storage->previous_myskin = ( false !== $old_myskin ) ?
                $old_myskin->get_stylesheet() :
                $slug_or_name;

            $this->_activate_module_event_hook();
        }
         ...
         * @uses   FS_Api
         */
        function _activate_module_event_hook() {
            ...
            $is_premium_version_activation = ( current_filter() !== ( 'activate_' . $this->_free_module_basename ) );

            $this->_logger->info( 'Activating ' . ( $is_premium_version_activation ? 'premium' : 'free' ) . ' module version.' );

            // 1. If running in the activation of the FREE module, get the basename of the PREMIUM.
            // 2. If running in the activation of the PREMIUM module, get the basename of the FREE.
            $other_version_basename = $is_premium_version_activation ?
                $this->_free_module_basename :
                $this->premium_module_basename();

            if ( ! $this->_is_network_active ) {
                /**
                 * During the activation, the module isn't yet active, therefore,
             ...
             *
             * is_module_active() checks if the module active on the site or the network level
             * and deactivate_modules() deactivates the module whether its activated on the site
             * or network level.
             *
             * @author Leo Fajardo (@leorw)
             * @since  1.2.2
             */
            if ( is_module_active( $other_version_basename ) ) {
                deactivate_modules( $other_version_basename );
            }
...
//				$this->sync_install( array(), true );
                ...
                if ( $is_premium_version_activation ) {
                    $this->_admin_notices->add(
                        sprintf( $this->get_text_inline( 'The upgrade of %s was successfully completed.', 'successful-version-upgrade-message' ), sprintf( '<b>%s</b>', $this->_module->title ) ),
                        $this->get_text_x_inline( 'W00t',
                            'Used to express elation, enthusiasm, or triumph (especially in electronic communication).', 'woot' ) . '!'
                    );
                }
                ...
                if ( isset( $this->_storage->is_anonymous_ms ) && $this->_storage->is_anonymous_ms['is'] ) {
                    $module_version = $this->_storage->is_anonymous_ms['version'];
                    $network        = true;
                } else {
                    $module_version = $this->_storage->is_anonymous['version'];
                    $network        = false;
                }
                 ...
                 *  1. Freemius DEV mode.
                 *  2. MandarinCMS DEBUG mode.
                 *  3. If a module and the user skipped the exact same version before.
                 *
                 * @since 1.2.2.7 Ulrich Pogson (@grapplerulrich) asked to not reset the SKIPPED flag if the exact same THEME version was activated before unless the developer is running with MCMS_DEBUG on, or Freemius debug mode on (MCMS_FS__DEV_MODE).
                 *
                 * @todo  4. If explicitly asked to retry after every activation.
                 */
                if ( MCMS_FS__DEV_MODE ||
                     (
                         ( $this->is_module() || ( defined( 'MCMS_DEBUG' ) && MCMS_DEBUG ) ) &&
                         $this->get_module_version() == $module_version
                     )
                ) {
            ...
            if ( ! isset( $this->_storage->is_module_new_install ) ) {
                /**
                 * If no previous version of module's version exist, it means that it's either
                 * the first time that the module installed on the site, or the module was installed
                 ...
                 * on manual activation via the dashboard, is_module_activation() is TRUE
                 * only after immediate activation.
                 *
                 * @since 1.1.4
                 * @link  https://make.mandarincms.org/core/2010/10/27/module-activation-hooks-no-longer-fire-for-updates/
                 */
                $this->_storage->is_module_new_install = empty( $this->_storage->module_last_version );
            }

            if ( ! $this->_anonymous_mode &&
                 $this->has_api_connectivity( MCMS_FS__DEV_MODE ) &&
                 ! $this->_isAutoInstall
            ) {
                // Store hint that the module was just activated to enable auto-redirection to settings.
                set_transient( "fs_{$this->_module_type}_{$this->_slug}_activated", true, 60 );
            }

            /**
             * Activation hook is executed after the module's main file is loaded, therefore,
             * after the module was loaded. The logic is located at activate_module()
             * ./mcms-admin/includes/module.php.
             *
             * @author Vova Feldman (@svovaf)
             * @since  1.1.9
             */
         ...
         * @since  1.0.3
         *
         * @param bool $check_user Enforce checking if user have modules activation privileges.
         */
        function delete_account_event( $check_user = true ) {
            ...
            if ( $delete_network_common_data ) {
                ...
                // Delete add-ons related to module's account.
        ...
        function delete_network_account_event( $check_user = true ) {
            ...
            if ( ! empty( $install_ids ) ) {
                $result = $this->get_current_or_network_user_api_scope()->call( "/modules/{$this->_module_id}/installs.json?ids=" . implode( ',', $install_ids ), 'delete' );
            }
         ...
         * @since  1.0.1
         */
        function _deactivate_module_hook() {
            $this->_logger->entrance( 'slug = ' . $this->_slug );
            ...
            if ( ! isset( $this->_storage->is_module_new_install ) ) {
                // Remember that module was already installed.
                $this->_storage->is_module_new_install = false;
            }

            // Hook to module uninstall.
            register_uninstall_hook( $this->_module_main_file_path, array( 'Freemius', '_uninstall_module_hook' ) );
        ...
        private function remove_sdk_reference() {
            ...
            foreach ( $fs_active_modules->modules as $sdk_path => &$data ) {
                if ( $this->_module_basename == $data->module_path ) {
        ...
        private function reset_anonymous_mode( $network_or_blog_id = 0 ) {
            ...
            /**
             * Ensure that this field is also "false", otherwise, if the current module's type is "myskin" and the module
             * has no menus, the opt-in baloonup will not be shown immediately (in this case, the user will have to click
             * on the admin notice that contains the opt-in link in order to trigger the opt-in baloonup).
             *
             * @author Leo Fajardo (@leorw)
             * @since  1.2.2
             */
        ...
        function skip_connection( $sites = null, $skip_all_network = false ) {
            ...
            if ( ! $skip_all_network && empty( $sites ) ) {
                ...
                $this->get_api_module_scope()->call( 'skip.json', 'put', array(
                    'uids' => $uids,
                ) );
            }
        ...
        /**
         * Module version update hook.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.4
         */
        private function update_module_version_event() {
            $this->_logger->entrance();
...
//			$this->sync_install( array(), true );
        ...
        /**
         * Generate an MD5 signature of a modules collection.
         * This helper methods used to identify changes in a modules collection.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         *
         ...
         * @return string
         */
        private function get_modules_thumbprint( $modules ) {
            ksort( $modules );

            $thumbprint = '';
            foreach ( $modules as $basename => $data ) {
        ...
        /**
         * Return a list of modified modules since the last sync.
         *
         * Note:
         *  There's no point to store a modules counter since even if the number of
         *  modules didn't change, we still need to check if the versions are all the
         ...
         * @return array|false
         */
        private function get_modules_data_for_api() {
            // Alias.
            $site_active_modules_option_name = 'active_modules';
            $network_modules_option_name     = 'all_modules';

            /**
             * Collection of all site level active modules.
             */
            $site_active_modules_cache = self::$_accounts->get_option( $site_active_modules_option_name );

            if ( ! is_object( $site_active_modules_cache ) ) {
                $site_active_modules_cache = (object) array(
                    'timestamp' => '',
                    'md5'       => '',
                    'modules'   => array(),
                );
            ...
            if ( ! empty( $site_active_modules_cache->timestamp ) &&
                 ( $time - $site_active_modules_cache->timestamp ) < MCMS_FS__TIME_5_MIN_IN_SEC
            ) {
                // Don't send module updates if last update was in the past 5 min.
                return false;
            }

            // Write timestamp to lock the logic.
            $site_active_modules_cache->timestamp = $time;
            self::$_accounts->set_option( $site_active_modules_option_name, $site_active_modules_cache, true );

            // Reload options from DB.
            self::$_accounts->load( true );
            $site_active_modules_cache = self::$_accounts->get_option( $site_active_modules_option_name );

            if ( $time != $site_active_modules_cache->timestamp ) {
                // If timestamp is different, then another thread captured the lock.
                return false;
            }

            /**
             * Collection of all modules (network level).
             */
            $network_modules_cache = self::$_accounts->get_option( $network_modules_option_name );

            if ( ! is_object( $network_modules_cache ) ) {
                $network_modules_cache = (object) array(
                    'timestamp' => '',
                    'md5'       => '',
                    'modules'   => array(),
                );
            ...
            // Check if there's a change in modules.
            $network_modules     = self::get_network_modules();
            $site_active_modules = self::get_site_active_modules();

            $network_modules_thumbprint     = $this->get_modules_thumbprint( $network_modules );
            $site_active_modules_thumbprint = $this->get_modules_thumbprint( $site_active_modules );

            // Check if modules status changed (version or active/inactive).
            $network_modules_changed     = ( $network_modules_cache->md5 !== $network_modules_thumbprint );
            $site_active_modules_changed = ( $site_active_modules_cache->md5 !== $site_active_modules_thumbprint );

            if ( ! $network_modules_changed &&
                 ! $site_active_modules_changed
            ) {
                // No changes.
                return array();
            }

            $modules_update_data = array();

            foreach ( $network_modules_cache->modules as $basename => $data ) {
                if ( ! isset( $network_modules[ $basename ] ) ) {
                    // Module uninstalled.
                    $uninstalled_module_data                   = $data;
                    $uninstalled_module_data['is_active']      = false;
                    $uninstalled_module_data['is_uninstalled'] = true;
                    $modules_update_data[]                     = $uninstalled_module_data;

                    unset( $network_modules[ $basename ] );

                    unset( $network_modules_cache->modules[ $basename ] );
                    unset( $site_active_modules_cache->modules[ $basename ] );

                    continue;
                }

                $was_active = $data['is_active'] ||
                              ( isset( $site_active_modules_cache->modules[ $basename ] ) &&
                                true === $site_active_modules_cache->modules[ $basename ]['is_active'] );
                $is_active  = $network_modules[ $basename ]['is_active'] ||
                              ( isset( $site_active_modules[ $basename ] ) &&
                                $site_active_modules[ $basename ]['is_active'] );

                if ( ! isset( $site_active_modules_cache->modules[ $basename ] ) &&
                     isset( $site_active_modules[ $basename ] )
                ) {
                    // Module was site level activated.
                    $site_active_modules_cache->modules[ $basename ]              = $network_modules[ $basename ];
                    $site_active_modules_cache->modules[ $basename ]['is_active'] = true;
                } else if ( isset( $site_active_modules_cache->modules[ $basename ] ) &&
                            ! isset( $site_active_modules[ $basename ] )
                ) {
                    // Module was site level deactivated.
                    unset( $site_active_modules_cache->modules[ $basename ] );
                }

                $prev_version    = $data['version'];
                $current_version = $network_modules[ $basename ]['Version'];

                if ( $was_active !== $is_active || $prev_version !== $current_version ) {
                    // Module activated or deactivated, or version changed.

                    if ( $was_active !== $is_active ) {
                        if ( $data['is_active'] != $network_modules[ $basename ]['is_active'] ) {
                            $network_modules_cache->modules[ $basename ]['is_active'] = $data['is_active'];
                        }
                    }

                    if ( $prev_version !== $current_version ) {
                        $network_modules_cache->modules[ $basename ]['Version'] = $current_version;
                    }

                    $updated_module_data              = $data;
                    $updated_module_data['is_active'] = $is_active;
                    $updated_module_data['version']   = $current_version;
                    $updated_module_data['title']     = $network_modules[ $basename ]['Name'];
                    $modules_update_data[]            = $updated_module_data;
                }
            }

            // Find new modules that weren't yet seen before.
            foreach ( $network_modules as $basename => $data ) {
                if ( ! isset( $network_modules_cache->modules[ $basename ] ) ) {
                    // New module.
                    $new_module = array(
                        'slug'           => $data['slug'],
                    ...
                    $modules_update_data[]                       = $new_module;
                    $network_modules_cache->modules[ $basename ] = $new_module;

                    if ( isset( $site_active_modules[ $basename ] ) ) {
                        $site_active_modules_cache->modules[ $basename ]              = $new_module;
                        $site_active_modules_cache->modules[ $basename ]['is_active'] = true;
                    }
                }
            }

            $site_active_modules_cache->md5       = $site_active_modules_thumbprint;
            $site_active_modules_cache->timestamp = $time;
            self::$_accounts->set_option( $site_active_modules_option_name, $site_active_modules_cache, true );

            $network_modules_cache->md5       = $network_modules_thumbprint;
            $network_modules_cache->timestamp = $time;
            self::$_accounts->set_option( $network_modules_option_name, $network_modules_cache, true );

            return $modules_update_data;
        }

        /**
         * Return a list of modified myskins since the last sync.
         *
         * Note:
         *  There's no point to store a myskins counter since even if the number of
         *  myskins didn't change, we still need to check if the versions are all the
         *  same and the activity state is similar.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.8
         ...
         * @return array|false
         */
        private function get_myskins_data_for_api() {
            // Alias.
            $option_name = 'all_myskins';

            $all_cached_myskins = self::$_accounts->get_option( $option_name );

            if ( ! is_object( $all_cached_myskins ) ) {
                $all_cached_myskins = (object) array(
                    'timestamp' => '',
                    'md5'       => '',
                    'myskins'    => array(),
                );
            ...
            if ( ! empty( $all_cached_myskins->timestamp ) &&
                 ( $time - $all_cached_myskins->timestamp ) < MCMS_FS__TIME_5_MIN_IN_SEC
            ) {
                // Don't send myskin updates if last update was in the past 5 min.
                return false;
            }

            // Write timestamp to lock the logic.
            $all_cached_myskins->timestamp = $time;
            self::$_accounts->set_option( $option_name, $all_cached_myskins, true );

            // Reload options from DB.
            self::$_accounts->load( true );
            $all_cached_myskins = self::$_accounts->get_option( $option_name );

            if ( $time != $all_cached_myskins->timestamp ) {
                // If timestamp is different, then another thread captured the lock.
                return false;
            }
            ...
            // Get active myskin.
            $active_myskin            = mcms_get_myskin();
            $active_myskin_stylesheet = $active_myskin->get_stylesheet();

            // Check if there's a change in myskins.
            $all_myskins = mcms_get_myskins();

            // Check if myskins changed.
            ksort( $all_myskins );

            $myskins_signature = '';
            foreach ( $all_myskins as $slug => $data ) {
                $is_active = ( $slug === $active_myskin_stylesheet );
                $myskins_signature .= $slug . ',' .
                                     $data->version . ',' .
                                     ( $is_active ? '1' : '0' ) . ';';
            }

            // Check if myskins status changed (version or active/inactive).
            $myskins_changed = ( $all_cached_myskins->md5 !== md5( $myskins_signature ) );

            $myskins_update_data = array();

            if ( $myskins_changed ) {
                // Change in myskins, report changes.

                // Update existing myskins info.
                foreach ( $all_cached_myskins->myskins as $slug => $data ) {
                    $is_active = ( $slug === $active_myskin_stylesheet );

                    if ( ! isset( $all_myskins[ $slug ] ) ) {
                        // Module uninstalled.
                        $uninstalled_myskin_data                   = $data;
                        $uninstalled_myskin_data['is_active']      = false;
                        $uninstalled_myskin_data['is_uninstalled'] = true;
                        $myskins_update_data[]                     = $uninstalled_myskin_data;

                        unset( $all_myskins[ $slug ] );
                        unset( $all_cached_myskins->myskins[ $slug ] );
                    } else if ( $data['is_active'] !== $is_active ||
                                $data['version'] !== $all_myskins[ $slug ]->version
                    ) {
                        // Module activated or deactivated, or version changed.

                        $all_cached_myskins->myskins[ $slug ]['is_active'] = $is_active;
                        $all_cached_myskins->myskins[ $slug ]['version']   = $all_myskins[ $slug ]->version;

                        $myskins_update_data[] = $all_cached_myskins->myskins[ $slug ];
                    }
                }

                // Find new myskins that weren't yet seen before.
                foreach ( $all_myskins as $slug => $data ) {
                    if ( ! isset( $all_cached_myskins->myskins[ $slug ] ) ) {
                        $is_active = ( $slug === $active_myskin_stylesheet );
                        ...
                        $myskins_update_data[]               = $new_module;
                        $all_cached_myskins->myskins[ $slug ] = $new_module;
                    }
                }

                $all_cached_myskins->md5       = md5( $myskins_signature );
                $all_cached_myskins->timestamp = time();
                self::$_accounts->set_option( $option_name, $all_cached_myskins, true );
            }

            return $myskins_update_data;
        }
         ...
         * @param bool     $include_modules Since 1.1.8 by default include module changes.
         * @param bool     $include_myskins  Since 1.1.8 by default include module changes.
         *
         * @return array
         */
        private function get_install_data_for_api(
            ...
            $include_myskins = true
        ) {
            /**
             * @since 1.1.8 Also send module updates.
             */
            if ( $include_modules && ! isset( $override['modules'] ) ) {
                $modules = $this->get_modules_data_for_api();
                if ( ! empty( $modules ) ) {
                    $override['modules'] = $modules;
                }
            }
            /**
             * @since 1.1.8 Also send myskins updates.
             */
            if ( $include_myskins && ! isset( $override['myskins'] ) ) {
                $myskins = $this->get_myskins_data_for_api();
                if ( ! empty( $myskins ) ) {
                    $override['myskins'] = $myskins;
                }
            }
         ...
         *
         * @todo   V1 of multiste network support doesn't support module and myskin data sending.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         *
...
//            }
        ...
        private function send_installs_update( $override = array(), $flush = false ) {
            ...
            // Send updated values to FS.
            $result = $this->get_api_user_scope()->call( "/modules/{$this->_module->id}/installs.json", 'put', $installs_data );
         ...
         *      the API will simply ignore your request based on the module ID.
         *
         * Need custom tracking for your module or myskin?
         ...
         * @param bool $check_user Enforce checking if user have modules activation privileges.
         */
        function _uninstall_module_event( $check_user = true ) {
            $this->_logger->entrance( 'slug = ' . $this->_slug );

            if ( $check_user && ! current_user_can( 'activate_modules' ) ) {
                return;
            }
            ...
            if ( ! $this->is_registered() ) {
                ...
                if ( isset( $uninstall_reason ) ) {
                    if ( isset( $uninstall_reason->is_anonymous ) && ! $uninstall_reason->is_anonymous ) {
                        ...
                        $params['uid'] = $this->get_anonymous_id();
                        $this->get_api_module_scope()->call( 'uninstall.json', 'put', $params );
                    }
                }
            } else {
            ...
            // @todo Decide if we want to delete module information from db.
        }
         ...
         * @return string
         */
        function premium_module_basename() {
            return "{$this->_slug}-premium/" . basename( $this->_free_module_basename );
        }

        /**
         * Uninstall module hook. Called only when connected his account with Freemius for active sites tracking.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.2
         */
        public static function _uninstall_module_hook() {
            self::_load_required_static();
            ...
            $module_file = substr( current_filter(), strlen( 'uninstall_' ) );

            self::$_static_logger->info( 'module = ' . $module_file );

            define( 'MCMS_FS__UNINSTALL_MODE', true );

            $fs = self::get_instance_by_file( $module_file );

            if ( is_object( $fs ) ) {
                self::require_module_essentials();

                if ( is_module_active( $fs->_free_module_basename ) ||
                     is_module_active( $fs->premium_module_basename() )
                ) {
                    // Deleting Free or Premium module version while the other version still installed.
                    return;
                }

                $fs->_uninstall_module_event();
        ...
        /**
         * Load MandarinCMS core module.php essential module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.1
         */
        private static function require_module_essentials() {
            if ( ! function_exists( 'get_modules' ) ) {
                self::$_static_logger->log( 'Including mcms-admin/includes/module.php...' );
        ...
        /**
         * Load MandarinCMS core pluggable.php module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.2
         */
        private static function require_pluggable_essentials() {
            if ( ! function_exists( 'mcms_get_current_user' ) ) {
                require_once BASED_TREE_URI . 'mcms-includes/pluggable.php';
            }
        }
        ...
        function get_module_data() {
            if ( ! isset( $this->_module_data ) ) {
                self::require_module_essentials();

                if ( $this->is_module() ) {
                    /**
                     * @author Vova Feldman (@svovaf)
                     * @since  1.2.0 When using get_module_data() do NOT translate module data.
                     *
                     * @link   https://github.com/Freemius/mandarincms-sdk/issues/77
                     */
                    $module_data = get_module_data(
                        $this->_module_main_file_path,
                        false,
                        false
                    );
                } else {
                    ...
                    if ( $this->_module_basename !== $myskin_data->get_stylesheet() && is_child_myskin() ) {
                        $parent_myskin = $myskin_data->parent();

                        if ( ( $parent_myskin instanceof MCMS_mySkin ) && $this->_module_basename === $parent_myskin->get_stylesheet() ) {
                            $myskin_data = $parent_myskin;
                        }
                    }

                    $module_data = array(
                        'Name'        => $myskin_data->get( 'Name' ),
                        'Version'     => $myskin_data->get( 'Version' ),
                        'Author'      => $myskin_data->get( 'Author' ),
                        'Description' => $myskin_data->get( 'Description' ),
                        'ModuleURI'   => $myskin_data->get( 'mySkinURI' ),
                    );
                }

                $this->_module_data = $module_data;
            }
         ...
         * @since  1.2.1.5
         *
         * @return number Parent module ID (if parent exist).
         */
         ...
         * @since  1.0.1
         *
         * @return string Module public key.
         */
        function get_public_key() {
            return $this->_module->public_key;
        }
         ...
         * @since  1.0.4
         *
         * @return mixed Module secret key.
         */
        function get_secret_key() {
            return $this->_module->secret_key;
        }
        ...
        function has_secret_key() {
            return ! empty( $this->_module->secret_key );
        }
        ...
        function get_module_name() {
            ...
            if ( ! isset( $this->_module_name ) ) {
                $module_data = $this->get_module_data();

                // Get name.
                $this->_module_name = $module_data['Name'];

                // Check if module name contains "(Premium)" suffix and remove it.
                $suffix     = ' (premium)';
                $suffix_len = strlen( $suffix );

                if ( strlen( $module_data['Name'] ) > $suffix_len &&
                     $suffix === substr( strtolower( $module_data['Name'] ), - $suffix_len )
                ) {
                    $this->_module_name = substr( $module_data['Name'], 0, - $suffix_len );
                }

                $this->_logger->departure( 'Name = ' . $this->_module_name );
            }
        ...
        function get_module_version() {
            ...
            $this->_logger->departure( 'Version = ' . $module_data['Version'] );

            return $this->apply_filters( 'module_version', $module_data['Version'] );
        }
        ...
        function get_module_title() {
            ...
            return $this->apply_filters( 'module_title', $title );
        }
        ...
        function get_module_label( $lowercase = false ) {
            $label = $this->is_addon() ?
                ...
                ( $this->is_module() ?
                    $this->get_text_inline( 'Module', 'module' ) :
                    $this->get_text_inline( 'mySkin', 'myskin' ) );
        ...
        function get_module_basename() {
            if ( ! isset( $this->_module_basename ) ) {
                if ( $this->is_module() ) {
                    $this->_module_basename = module_basename( $this->_module_main_file_path );
                } else {
                    $this->_module_basename = basename( dirname( $this->_module_main_file_path ) );
                }
            }

            return $this->_module_basename;
        }

        function get_module_folder_name() {
            $this->_logger->entrance();

            $module_folder = $this->_module_basename;

            while ( '.' !== dirname( $module_folder ) ) {
                $module_folder = dirname( $module_folder );
            }

            $this->_logger->departure( 'Folder Name = ' . $module_folder );
        ...
        /**
         * Find module's slug by module's basename.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         * @param string $module_base_name
         *
         * @return false|string
         */
        private static function find_slug_by_basename( $module_base_name ) {
            $file_slug_map = self::$_accounts->get_option( 'file_slug_map', array() );

            if ( ! array( $file_slug_map ) || ! isset( $file_slug_map[ $module_base_name ] ) ) {
                return false;
            }

            return $file_slug_map[ $module_base_name ];
        }

        /**
         * Store the map between the module's basename to the slug.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         */
        private function store_file_slug_map() {
            ...
            if ( ! isset( $file_slug_map[ $this->_module_basename ] ) ||
                 $file_slug_map[ $this->_module_basename ] !== $this->_slug
            ) {
                $file_slug_map[ $this->_module_basename ] = $this->_slug;
                self::$_accounts->set_option( 'file_slug_map', $file_slug_map, true );
            }
        ...
        private static function get_account_option( $option_name, $module_type = null, $network_level_or_blog_id = null ) {
            if ( ! is_null( $module_type ) && MCMS_FS__MODULE_TYPE_PLUGIN !== $module_type ) {
        ...
        private static function set_account_option_by_module(
            ...
            if ( MCMS_FS__MODULE_TYPE_PLUGIN != $module_type ) {
                $option_name = $module_type . '_' . $option_name;
            }
        ...
        private static function get_all_licenses_by_module_type() {
            ...
            $licenses_by_module_type = array(
                MCMS_FS__MODULE_TYPE_PLUGIN => array(),
                MCMS_FS__MODULE_TYPE_THEME  => array()
            );
         ...
         * @since  1.0.6
         *
         * @return array<number,FS_Module[]>|false
         */
        ...
        function get_account_addons() {
            ...
            if ( ! is_array( $addons ) ||
                 ! isset( $addons[ $this->_module->id ] ) ||
                 ! is_array( $addons[ $this->_module->id ] ) ||
                 0 === count( $addons[ $this->_module->id ] )
            ) {
                return false;
            }

            return $addons[ $this->_module->id ];
        }
        ...
        private static function is_super_admin( $user_id ) {
            ...
            if ( $user instanceof FS_User && ! empty( $user->email ) ) {
                ...
                $mcms_user = get_user_by( 'email', $user->email );

                if ( $mcms_user instanceof MCMS_User ) {
                    $super_admins   = get_super_admins();
                    $is_super_admin = ( is_array( $super_admins ) && in_array( $mcms_user->user_login, $super_admins ) );
                }
            }
        ...
        /**
         * Check if running premium module code.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.5
         *
         ...
         */
        function is_premium() {
            return $this->_module->is_premium;
        }
        ...
        function get_plan() {
            ...
            return FS_Module_Plan::is_valid_id( $this->_site->plan_id ) ?
        ...
        function get_trial_plan() {
            ...
            if ( ! is_object( $trial_plan ) ) {
                 ...
                 */
                if ( $trial_plan instanceof FS_Module_Plan ) {
        ...
        /**
         * Check if the user has an activate, non-expired license on current module's install.
         *
         * @since 1.0.9
         *
         * @return bool
        ...
        /**
         * Check if user has any licenses associated with the module (including expired or blocking).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.7.3
         *
         ...
         *
         * @return FS_Module_License|false
         */
        ...
        /**
         * Sync local module plans with remote server.
         *
         ...
         * @since  1.0.5
         *
         * @return FS_Module_Plan[]|object
         */
        function _sync_plans() {
            $plans = $this->_fetch_module_plans();

            if ( $this->is_array_instanceof( $plans, 'FS_Module_Plan' ) ) {
                $plans_map = array();
         ...
         *
         * @return \FS_Module_Plan|object The plan entity or the API error object on failure.
         */
        private function sync_plan_if_not_exist( $plan_id ) {
            ...
            if ( $plan instanceof FS_Module_Plan ) {
                $this->_plans[] = $plan;
                $this->_store_plans();
        ...
        private function sync_license_if_not_exist( $license_id, $license_key ) {
            ...
            if ( $license instanceof FS_Module_License ) {
        ...
        private function get_plans_ids_associated_with_installs() {
            if ( ! $this->_is_network_active ) {
                if ( ! is_object( $this->_site ) ||
                     ! FS_Module_Plan::is_valid_id( $this->_site->plan_id )
                ) {
                    return array();
                }
            ...
            foreach ( $sites as $site ) {
                ...
                if ( ! is_object( $install ) ||
                     ! FS_Module_Plan::is_valid_id( $install->plan_id )
                ) {
                    continue;
                }
        ...
        private function get_license_ids_associated_with_installs() {
            if ( ! $this->_is_network_active ) {
                if ( ! is_object( $this->_site ) ||
                     ! FS_Module_License::is_valid_id( $this->_site->license_id )
                ) {
                    return array();
                }
            ...
            foreach ( $sites as $site ) {
                ...
                if ( ! is_object( $install ) ||
                     ! FS_Module_License::is_valid_id( $install->license_id )
                ) {
                    continue;
                }
         ...
         *
         * @return FS_Module_License[]|object
         */
        function _sync_licenses( $site_license_id = false, $blog_id = null ) {
            ...
            if ( $this->is_array_instanceof( $licenses, 'FS_Module_License' ) ) {
                $licenses_map = array();
...
//                }
        ...
        function _get_license_by_id( $id, $sync_licenses = true ) {
            ...
            if ( ! FS_Module_License::is_valid_id( $id ) ) {
                return false;
            }
         ...
         * @since  2.0.0
         *
         * @return \FS_Module_License|mixed
         */
        private function sync_site_license() {
            ...
            $license = $this->_update_site_license( new FS_Module_License( $result ) );
            $this->_store_licenses();
         ...
         *
         * @param \FS_User           $user
         * @param \FS_Module_License $license
         *
         * @return bool
         */
        private function try_activate_license_on_network( FS_User $user, FS_Module_License $license ) {
            $this->_logger->entrance();
...
//            $this->_store_licenses()
         ...
         * }
         */
        private function can_activate_license_on_network( FS_Module_License $license ) {
            $sites = self::get_sites();
            ...
            foreach ( $sites as $site ) {
                ...
                if ( is_object( $install ) ) {
                    if ( FS_Module_License::is_valid_id( $install->license_id ) ) {
        ...
        private function activate_license_on_many_installs(
            ...
            $result = $this->get_api_user_scope_by_user( $user )->call(
                "modules/{$this->_module->id}/installs.json",
                'PUT',
                $params
            );
         ...
         * @since  1.0.6
         *
         * @param FS_Module_License|null $new_license
         *
         * @return FS_Module_License|null
         */
         ...
         * @since  1.0.9
         *
         * @param FS_Module_License|null $license
         *
        ...
        /**
         * Check if module has any paid plans.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         *
        ...
        /**
         * Check if module has any plan with a trail.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
        ...
        /**
         * Check if module has any free plan, or is it premium only.
         *
         * Note: If no plans configured, assume module is free.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         *
         ...
         * or "Change License" link on the modules
         * page.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.1.9
        ...
        function _add_premium_version_upgrade_selection_dialog_box() {
            $modules_update = get_site_transient( $this->is_myskin() ? 'update_myskins' : 'update_modules' );
            if ( ! isset( $modules_update->response[ $this->_module_basename ] ) ) {
                return;
            }

            $vars = array(
                'id'          => $this->_module_id,
                'new_version' => is_object( $modules_update->response[ $this->_module_basename ] ) ?
                    $modules_update->response[ $this->_module_basename ]->new_version :
                    $modules_update->response[ $this->_module_basename ]['new_version']
            );
        ...
        /**
         * Displays the opt-out dialog box when the user clicks on the "Opt Out" link on the "Modules"
         * page.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.1.5
        ...
        function _add_license_activation() {
            ...
            if ( self::is_modules_page() ) {
                /**
                 * @since 1.2.0 Add license action link only on modules page.
                 */
        ...
        function _add_premium_version_upgrade_selection() {
            ...
            if ( self::is_updates_page() || ( $this->is_module() && self::is_modules_page() ) ) {
        ...
        function _activate_license_ajax_action() {
            ...
            $module_id = fs_request_get( 'module_id', '', 'post' );
            $fs        = ( $module_id == $this->_module_id ) ?
                $this :
                $this->get_addon_instance( $module_id );
        ...
        /**
         * Get current page or the referer if executing a MCMS AJAX request.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.1.8
         *
        ...
        /**
         * Helper method to check if user in the modules page.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.1.5
         *
         * @return bool
         */
        static function is_modules_page() {
            return ( 'modules.php' === self::get_current_page() );
        }
        ...
        /**
         * Helper method to check if user in the myskins page.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.2.6
         *
         * @return bool
         */
        static function is_myskins_page() {
            return ( 'myskins.php' === self::get_current_page() );
        }
        ...
        function has_affiliate_program() {
            ...
            return $this->_module->has_affiliate_program();
        }
        ...
        private function fetch_affiliate_terms() {
            if ( ! is_object( $this->module_affiliate_terms ) ) {
                $modules_api     = $this->get_api_module_scope();
                $affiliate_terms = $modules_api->get( '/aff.json?type=affiliation', false );

                if ( ! $this->is_api_result_entity( $affiliate_terms ) ) {
                    return;
                }

                $this->module_affiliate_terms = new FS_AffiliateTerms( $affiliate_terms );
            }
        }
        ...
        private function fetch_affiliate_and_custom_terms() {
            if ( ! empty( $this->_storage->affiliate_application_data ) ) {
                ...
                $result    = $users_api->get( "/modules/{$this->_module->id}/aff/{$this->module_affiliate_terms->id}/affiliates.json", $flush );
                if ( $this->is_api_result_object( $result, 'affiliates' ) ) {
                    if ( ! empty( $result->affiliates ) ) {
                        ...
                        if ( $affiliate->is_using_custom_terms ) {
                            $affiliate_terms = $users_api->get( "/modules/{$this->_module->id}/affiliates/{$affiliate->id}/aff/{$affiliate->custom_affiliate_terms_id}.json", $flush );
                            if ( $this->is_api_result_entity( $affiliate_terms ) ) {
        ...
        function _submit_affiliate_application() {
            ...
            $api    = $this->get_api_user_scope();
            $result = $api->call(
                ( "/modules/{$this->_module->id}/aff/{$this->module_affiliate_terms->id}/affiliates.json" ),
                'post',
                $affiliate
            );
         ...
         * @return string
         */
        function get_upgrade_url( $period = MCMS_FS__PERIOD_ANNUALLY, $is_trial = false ) {
        ...
        function get_trial_url() {
            return $this->get_upgrade_url( MCMS_FS__PERIOD_ANNUALLY, true );
        }

        /**
         * Module's pricing URL.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.4
         *
         ...
         * @return string
         */
        function pricing_url( $billing_cycle = MCMS_FS__PERIOD_ANNUALLY, $is_trial = false ) {
            $this->_logger->entrance();
        ...
        function checkout_url(
            $billing_cycle = MCMS_FS__PERIOD_ANNUALLY,
            $is_trial = false,
            $extra = array()
        ) {
            $this->_logger->entrance();
        ...
        /**
         * Check if module has any add-ons.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.5
         *
        ...
        /**
         * Check if module can work in anonymous mode.
         *
         * @author     Vova Feldman (@svovaf)
         * @since      1.0.9
         *
        ...
        /**
         * Check if module is premium only (no free plans).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.9
         *
        ...
        /**
         * Checks if the module's type is "module". The other type is "myskin".
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.2
         *
         ...
         */
        function is_module() {
            return ( MCMS_FS__MODULE_TYPE_PLUGIN === $this->_module_type );
        }
         ...
         * @return string
         */
        function get_module_main_file_path() {
            return $this->_module_main_file_path;
        }
         ...
         *
         * @link   http://mandarincms.stackexchange.com/questions/70676/how-to-check-if-i-am-in-admin-ajax
         */
        ...
        function is_ajax_action( $actions ) {
            ...
            // Verify the call is relevant for the module.
        ...
        /**
         * Construct module's settings page URL.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.4
         *
        ...
        function _get_admin_page_url( $page = '', $params = array(), $network = null ) {
            ...
            if ( empty( $page ) &&
                 $this->is_myskin() &&
                 // Show the opt-in as an overlay for free mcms.org myskins or myskins without any settings page.
                 ( $this->is_free_mcms_org_myskin() || ! $this->has_settings_menu() )
            ) {
                $params[ $this->get_unique_affix() . '_show_optin' ] = 'true';

                return add_query_arg(
                    $params,
                    $this->admin_url( 'myskins.php', 'admin', $network )
                );
            }
            ...
            if ( ! $this->has_settings_menu() ) {
                if ( ! empty( $page ) ) {
                    ...
                    if ( $this->is_activation_mode() ) {
                        ...
                        return add_query_arg(
                            $params,
                            $this->admin_url( 'modules.php', 'admin', $network )
                        );
                    }
                }
            }
        ...
        function is_active_for_site( $blog_id ) {
            if ( ! is_multisite() ) {
                // Not a multisite and this code is executed, means that the module is active.
                return true;
            }

            if ( $this->is_myskin() ) {
                // All myskins are site level activated.
                return true;
            }

            if ( $this->_is_network_active ) {
                // Module was network activated so it's active.
                return true;
            }

            return in_array( $this->_module_basename, (array) get_blog_option( $blog_id, 'active_modules', array() ) );
        }
        ...
        static function get_sites() {
            ...
            if ( function_exists( 'get_sites' ) ) {
                // For MCMS 4.6 and above.
                return get_sites( $args );
            } else if ( function_exists( 'mcms_get_sites' ) ) {
                // For MCMS 3.7 to MCMS 4.5.
                return mcms_get_sites( $args );
            } else {
                // For MCMS 3.6 and below.
        ...
        private static function is_site_active( $blog_id ) {
            ...
            $blog_info = $mcmsdb->get_row( $mcmsdb->prepare( "SELECT * FROM {$mcmsdb->blogs} WHERE blog_id = %d", $blog_id ) );
        ...
        private static function get_user_opted_in_module_ids_map( $fs_user_id ) {
            ...
            if ( ! is_multisite() ) {
                $installs = array_merge(
                    self::get_all_sites( MCMS_FS__MODULE_TYPE_PLUGIN ),
                    self::get_all_sites( MCMS_FS__MODULE_TYPE_THEME )
                );
            } else {
                $sites = self::get_sites();
                ...
                foreach ( $sites as $site ) {
                    ...
                    $installs = array_merge(
                        $installs,
                        self::get_all_sites( MCMS_FS__MODULE_TYPE_PLUGIN, $blog_id ),
                        self::get_all_sites( MCMS_FS__MODULE_TYPE_THEME, $blog_id )
                    );
                }
            }
            ...
            foreach ( $installs as $install ) {
                if ( is_object( $install ) &&
                    ...
                    $module_ids_map[ $install->module_id ] = true;
                }
            }
        ...
        function switch_to_blog( $blog_id, FS_Site $install = null ) {
            ...
            if ( is_object( $this->_site ) ) {
                ...
                $all_module_licenses = self::get_all_licenses( $this->_module_id );

                if ( ! empty( $all_module_licenses ) ) {
                    if ( ! FS_Module_License::is_valid_id( $this->_site->license_id ) ) {
                        $this->_license = null;
                    } else {
                        $license_found = false;
                        foreach ( $all_module_licenses as $license ) {
         ...
         * @since  2.0.0
         *
         * @param array|MCMS_Site $site
         *
         * @return int
         */
        static function get_site_blog_id( &$site ) {
            return ( $site instanceof MCMS_Site ) ?
                $site->blog_id :
         ...
         * @since  2.0.0
         *
         * @param array|MCMS_Site|null $site
         *
         * @return array
         */
        ...
        private function update_multisite_data_after_site_deactivation( $context_blog_id = 0 ) {
            ...
            if ( $this->is_sync_cron_scheduled() &&
                ...
                $this->schedule_sync_cron( MCMS_FS__SCRIPT_START_TIME, true, $context_blog_id );
            }
        ...
        /**
         * Check if currently on the myskin's setting page or
         ...
         * @return bool
         */
        function is_myskin_settings_page() {
        ...
        /**
         * Module's account page + sync license URL.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.9.1
         *
         * @param bool|number $module_id
         * @param bool        $add_action_nonce
         * @param array       $params
         *
         * @return string
         */
        function _get_sync_license_url( $module_id = false, $add_action_nonce = true, $params = array() ) {
            if ( is_numeric( $module_id ) ) {
        ...
        private static function _encrypt( $str ) {
             ...
             *
             * I used the same suggested hack by the myskin review team.
             * For more details, look at the function `Base64UrlDecode()`
             * in `./sdk/FreemiusBase.php`.
             *
             * @todo   Remove this hack once the base64 error is removed from the mySkin Check.
             *
             * @author Vova Feldman (@svovaf)
             * @since  1.2.2
             */
...
//				check_admin_referer( 'activate_' . $this->_module->public_key );

                // Verify matching module details.
                if ( $this->_module->id != fs_request_get( 'module_id' ) || $this->_slug != fs_request_get( 'module_slug' ) ) {
                    return;
                }
        ...
        private function _load_account() {
            ...
            if ( is_object( $site ) &&
                ...
                if ( ! is_array( $this->_plans ) || empty( $this->_plans ) ) {
                    ...
                    for ( $i = 0, $len = count( $this->_plans ); $i < $len; $i ++ ) {
                        if ( $this->_plans[ $i ] instanceof FS_Module_Plan ) {
            ...
            if ( is_object( $this->_site ) ) {
                ...
                if ( $this->_site->version != $this->get_module_version() ) {
                    // If stored install version is different than current installed module version,
                    // then update module version event.
                    $this->update_module_version_event();
                }
            }
        ...
        private function fetch_user_by_install() {
            ...
            $result = $api->get( $request_path, false, MCMS_FS__TIME_10_MIN_IN_SEC );
            ...
            if ( in_array( $error_code, array( 'invalid_unique_id', 'user_cannot_be_recovered' ) ) ) {
                 ...
                 */
                $api->update_cache_expiration( $request_path, MCMS_FS__TIME_WEEK_IN_SEC );
            }
        ...
        function get_opt_in_params( $override_with = array(), $network_level_or_blog_id = null ) {
            ...
            $params = array(
                ...
                'user_ip'                      => MCMS_FS__REMOTE_ADDR,
                'module_slug'                  => $this->_slug,
                'module_id'                    => $this->get_id(),
                'module_public_key'            => $this->get_public_key(),
                'module_version'               => $this->get_module_version(),
            ...
            if ( MCMS_FS__SKIP_EMAIL_ACTIVATION && $this->has_secret_key() ) {
                ...
                $params['ts']     = MCMS_FS__SCRIPT_START_TIME;
         ...
         *                                          data will be saved to the MCMS installation's database.
        ...
        function opt_in(
            ...
            } else if ( FS_Module_Plan::is_valid_id( $trial_plan_id ) ) {
            ...
            if ( isset( $params['license_key'] ) ) {
                ...
                if ( is_object( $fs_user ) ) {
                    /**
                     * If opting in with a context license and the context MCMS Admin user already opted in
            ...
            $request = array(
                'method'  => 'POST',
                'body'    => $params,
                'timeout' => MCMS_FS__DEBUG_SDK ? 60 : 30,
            );

            $url = MCMS_FS__ADDRESS . '/action/service/user/install/';
            $response = self::safe_remote_post( $url, $request );

            if ( is_mcms_error( $response ) ) {
                /**
                 * @var MCMS_Error $response
                 */
                $result = new stdClass();
            ...
            /**
             * When json_decode() executed on PHP 5.2 with an invalid JSON, it will throw a PHP warning. Unfortunately, the new mySkin Check doesn't allow PHP silencing and the myskin review team isn't open to change that, therefore, instead of using `@json_decode()` we had to use the method without the `@` directive.
             *
             * @author Vova Feldman (@svovaf)
             * @since  1.2.3
             * @link   https://myskins.trac.mandarincms.org/ticket/46134#comment:5
             * @link   https://myskins.trac.mandarincms.org/ticket/46134#comment:9
             * @link   https://myskins.trac.mandarincms.org/ticket/46134#comment:12
             * @link   https://myskins.trac.mandarincms.org/ticket/46134#comment:14
             */
        ...
        function setup_network_account(
            ...
            if ( $this->_storage->handle_gdpr_admin_notice &&
                 ...
                 */
                require_once MCMS_FS__DIR_INCLUDES . '/class-fs-user-lock.php';
...
//            $is_network_level_opt_in = self::is_ajax_action_static( 'network_activate', $this->_module_id );
            ...
            if ( $this->is_pending_activation() || ! $this->has_settings_menu() ) {
                ...
                // Remove module from pending activation mode.
                unset( $this->_storage->is_pending_activation );

                if ( ! $this->is_paying_or_trial() ) {
                    $this->_admin_notices->add_sticky(
                        sprintf( $this->get_text_inline( '%s activation was successfully completed.', 'module-x-activation-message' ), '<b>' . $this->get_module_name() . '</b>' ),
                        'activation_complete'
                    );
                }
            }
            ...
            $module_id = fs_request_get( 'module_id', false );

            // Store activation time ONLY for modules & myskins (not add-ons).
            if ( ! is_numeric( $module_id ) || ( $module_id == $this->_module->id ) ) {
                if ( empty( $this->_storage->activation_timestamp ) ) {
                    $this->_storage->activation_timestamp = MCMS_FS__SCRIPT_START_TIME;
                }
            }
            ...
            if ( is_numeric( $module_id ) ) {
                 ...
                 */
                if ( $this->is_anonymous() || $module_id != $this->_module->id ) {
                    // Add-on was installed - sync license right after install.
                    $next_page = $this->_get_sync_license_url( $module_id, true, $extra );
                }
            } else {
                /**
                 * @author Vova Feldman (@svovaf)
        ...
        /**
         * Install module with new user information after approval.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         */
        function _install_with_new_user() {
            ...
            if ( ( $this->is_module() && fs_request_is_action( $this->get_unique_affix() . '_activate_new' ) ) ||
                 // @todo This logic should be improved because it's executed on every load of a myskin.
                 $this->is_myskin()
            ) {
//				check_admin_referer( $this->_slug . '_activate_new' );
        ...
        private function install_many_with_new_user(
            ...
            $installs = array();
            while ( $left > 0 ) {
                $result = $this->get_api_user_scope()->get( "/modules/{$this->_module_id}/installs.json?ids=" . implode( ',', array_slice( $install_ids, $offset, 25 ) ) );
        ...
        /**
         * Install module with current logged MCMS user info.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         */
...
//				check_admin_referer( 'activate_existing_' . $this->_module->public_key );
        ...
        private function install_with_user(
            ...
            if ( empty( $sites ) ) {
                ...
                if ( ! $setup_account ) {
                    ...
                    if ( ! empty( $license_key ) && FS_Module_License::is_valid_id( $site->license_id ) ) {
        ...
        private function create_installs_with_user(
            ...
            // Install the module.
            $result = $this->get_api_user_scope_by_user( $user )->call(
                "/modules/{$this->get_id()}/installs.json",
                'post',
                $args
            );
            ...
            if ( ! $this->is_api_result_entity( $result ) &&
                ...
                if ( ! $silent ) {
                    $this->_admin_notices->add(
                        sprintf( $this->get_text_inline( 'Couldn\'t activate %s.', 'could-not-activate-x' ), $this->get_module_name() ) . ' ' .
                ...
                if ( $redirect ) {
                     ...
                     * context module doesn't have a settings menu and the default page is the `Modules` page,
                     * misleading module activation errors will be shown on the `Modules` page.
                     *
                     * @author Leo Fajardo (@leorw)
                     */
                    $this->_user = null;
        ...
        /**
         * Tries to activate add-on account based on parent module info.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.6
         *
        ...
        private function _activate_addon_account( Freemius $parent_fs ) {
            ...
            // Activate add-on with parent module credentials.
            $addon_install = $parent_fs->get_api_site_scope()->call(
                "/addons/{$this->_module->id}/installs.json",
                'post',
            ...
            // Get user information based on parent's module.
        ...
        private function activate_parent_account( Freemius $parent_fs ) {
            ...
            $parent_install = $this->get_api_user_scope()->call(
                "/modules/{$parent_fs->_module->id}/installs.json",
                'post',
...
//			}
        ...
        private function add_menu_action() {
            if ( $this->is_activation_mode() ) {
                if ( $this->is_module() || ( $this->has_settings_menu() && ! $this->is_free_mcms_org_myskin() ) ) {
                    $this->override_module_menu_with_activation();
                } else {
                    /**
                     * Handle myskin opt-in when the opt-in form shows as a dialog box in the myskins page.
                     */
                    if ( fs_request_is_action( $this->get_unique_affix() . '_activate_existing' ) ) {
                        add_action( 'load-myskins.php', array( &$this, '_install_with_current_user' ) );
                    } else if ( fs_request_is_action( $this->get_unique_affix() . '_activate_new' ) ||
                                fs_request_get_bool( 'pending_activation' )
                    ) {
                        add_action( 'load-myskins.php', array( &$this, '_install_with_new_user' ) );
                    }
                }
            } else {
                if ( ! $this->is_registered() ) {
                    ...
                    ( ! $this->has_settings_menu() || $this->is_free_mcms_org_myskin() )
                ) {
        ...
        /**
         * Remove module's all admin menu items & pages, and replace with activation page.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.1
         */
        private function override_module_menu_with_activation() {
            $this->_logger->entrance();
        ...
        /**
         * If a module was network activated and connected but don't have a network
        ...
        private function add_network_menu_when_missing() {
            ...
            if ( ! $this->_is_network_active ) {
                // Module wasn't activated on the network level.
                return;
            }
            ...
            if ( $this->_menu->has_network_menu() ) {
                // Module already has a network level menu.
                return;
            }
            ...
            if ( ! MCMS_FS__SHOW_NETWORK_EVEN_WHEN_DELEGATED ) {
                if ( $this->is_network_delegated_connection() ) {
         ...
         * @author Vova Feldman (@svovaf)
         * @since  1.0.0
         * @since  1.2.2.7 Also add submenu items when running in a free .org myskin so the tabs will be visible.
         */
        private function add_submenu_items() {
            ...
            } else if ( $this->is_free_mcms_org_myskin() && ! fs_is_network_admin() ) {
                // Also add submenu items when running in a free .org myskin so the tabs will be visible.
                $add_submenu_items = true;
            } else if ( $this->is_activation_mode() && ! $this->is_free_mcms_org_myskin() ) {
                $add_submenu_items = false;
            } else if ( fs_is_network_admin() ) {
                /**
                 * Add submenu items to network level when module was network
                 * activated and the super-admin did NOT delegated the connection
                 * of all sites to site admins.
                 */
                $add_submenu_items = (
                    $this->_is_network_active &&
                    ( MCMS_FS__SHOW_NETWORK_EVEN_WHEN_DELEGATED ||
                      ! $this->is_network_delegated_connection() )
                );
            } else {
            ...
            if ( $add_submenu_items ) {
                if ( $this->has_affiliate_program() ) {
                    ...
                    $this->add_submenu_item(
                        ...
                        $this->get_module_name() . ' &ndash; ' . $this->get_text_inline( 'Affiliation', 'affiliation' ),
                        'manage_options',
                        'affiliation',
                ...
                if ( ! MCMS_FS__DEMO_MODE && $this->is_registered() ) {
                    $show_account = (
                        $this->is_submenu_item_visible( 'account' ) &&
                        /**
                         * @since 1.2.2.7 Don't show the Account for free MCMS.org myskins without any paid plans.
                         */
                        ( ! $this->is_free_mcms_org_myskin() || $this->has_paid_plan() )
                    );
                    ...
                    $this->add_submenu_item(
                        ...
                        $this->get_module_name() . ' &ndash; ' . $this->get_text_inline( 'Account', 'account' ),
                        'manage_options',
                        'account',
                ...
                $this->add_submenu_item(
                    ...
                    $this->get_module_name() . ' &ndash; ' . $this->get_text_inline( 'Contact Us', 'contact-us' ),
                    'manage_options',
                    'contact',
                    'Freemius::_clean_admin_content_section',
                    MCMS_FS__DEFAULT_PRIORITY,
                ...
                if ( $this->has_addons() ) {
                    $this->add_submenu_item(
                        ...
                        $this->get_module_name() . ' &ndash; ' . $this->get_text_inline( 'Add-Ons', 'add-ons' ),
                        'manage_options',
                        'addons',
                        array( &$this, '_addons_page_load' ),
                        MCMS_FS__LOWEST_PRIORITY - 1,
                ...
                if ( ! MCMS_FS__DEMO_MODE ) {
                    ...
                    $this->add_submenu_item(
                        ...
                        $this->get_module_name() . ' &ndash; ' . $this->get_text_x_inline( 'Pricing', 'noun', 'pricing' ),
                        'manage_options',
                        'pricing',
                        'Freemius::_clean_admin_content_section',
                        MCMS_FS__LOWEST_PRIORITY,
            ...
            if ( 0 < count( $this->_menu_items ) ) {
                if ( ! $this->_menu->is_top_level() ) {
                    ...
                    // Append submenu items right after the module's submenu item.
         ...
         * in order to support sub-submenu items when the module's settings
         ...
         * are added right after the module's settings submenu item.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.4
         */
        private function order_sub_submenu_items() {
            ...
            /**
             * Before "admin_menu" fires, MandarinCMS will loop over the default submenus and remove pages for which the user
             * does not have permissions. So in case a module does not have top-level menu but does have submenus under any
             ...
             *
             * In case a module does not have submenus under any of the default menus but does have submenus under the menu
             * of another module, only users that have the right role can access its sub-submenus since we will use the
            ...
            foreach ( $top_level_menu as $submenu_id => $meta ) {
                if ( $found_submenu_item ) {
                    // Remove all submenu items after the module's submenu item.
            ...
            // Embed all module's new submenu items.
            $this->embed_submenu_items();
        ...
        function get_support_forum_url() {
            return $this->apply_filters( 'support_forum_url', "https://mandarincms.org/support/{$this->_module_type}/{$this->_slug}" );
        }
        ...
        function add_submenu_item(
            ...
            $priority = MCMS_FS__DEFAULT_PRIORITY,
            $show_submenu = true,
            $class = ''
        ) {
        ...
        public function get_action_tag( $tag ) {
            return self::get_action_tag_static( $tag, $this->_slug, $this->is_module() );
        }
         ...
         * @return string
         */
        static function get_action_tag_static( $tag, $slug = '', $is_module = true ) {
            $action = "fs_{$tag}";

            if ( ! empty( $slug ) ) {
                $action .= '_' . self::get_module_unique_affix( $slug, $is_module );
            }
        ...
        public function get_unique_affix() {
            return self::get_module_unique_affix( $this->_slug, $this->is_module() );
        }
         ...
         * @return string
         */
        static function get_module_unique_affix( $slug, $is_module = true ) {
            $affix = $slug;

            if ( ! $is_module ) {
                $affix .= '-' . MCMS_FS__MODULE_TYPE_THEME;
            }
        ...
        function get_ajax_security( $tag ) {
            return mcms_create_nonce( $this->get_ajax_action( $tag ) );
        }
        ...
        /**
         * Do action, specific for the current context module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.1
         *
        ...
        /**
         * Add action, specific for the current context module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.1
         *
        ...
        /**
         * Add AJAX action, specific for the current context module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.1
         *
        ...
        static function add_ajax_action_static(
            ...
            add_action(
                'mcms_ajax_' . self::get_ajax_action_static( $tag, $module_id ),
                $function_to_add,
                $priority,
                0
            );
        ...
        static function shoot_ajax_failure( $error = '' ) {
            ...
            mcms_send_json( $result );
        }

        /**
         * Apply filter, specific for the current context module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
        ...
        /**
         * Add filter, specific for the current context module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         * @uses   add_filter()
         */
        function add_filter( $tag, $function_to_add, $priority = MCMS_FS__DEFAULT_PRIORITY, $accepted_args = 1 ) {
            $this->_logger->entrance( $tag );
        ...
        /**
         * Update module's plans information.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.2
         *
         ...
         * @since  1.0.4
         *
         * @param FS_Module_Tag|null $update
         * @param bool               $store Flush to Database if true.
         * @param bool|number        $module_id
         */
        private function _store_update( $update, $store = true, $module_id = false ) {
            $this->_logger->entrance();

            if ( $update instanceof FS_Module_Tag ) {
                $update->updated = time();
            }
         ...
         * @since    1.0.6
         *
         * @param FS_Module[] $module_addons
         * @param bool        $store Flush to Database if true.
         */
        private function _store_addons( $module_addons, $store = true ) {
            $this->_logger->entrance();

            $addons                       = self::get_all_addons();
            $addons[ $this->_module->id ] = $module_addons;
            self::$_accounts->set_option( 'addons', $addons, $store );
        }

        /**
         * Delete module's associated add-ons.
         *
         * @author   Vova Feldman (@svovaf)
         * @since    1.0.8
         *
        ...
        private function _delete_account_addons( $store = true ) {
            ...
            if ( ! isset( $all_addons[ $this->_module->id ] ) ) {
                return false;
            }

            unset( $all_addons[ $this->_module->id ] );
        ...
        private function _store_account_addons( $addons, $store = true ) {
            ...
            $all_addons                       = self::get_all_account_addons();
            $all_addons[ $this->_module->id ] = $addons;
            self::$_accounts->set_option( 'account_addons', $all_addons, $store );
        }
            ...
            if ( isset( $user->id ) ) {
                ...
                if ( $user->is_verified &&
                    ...
                    $this->_admin_notices->add(
                        ...
                        // since the page will be auto redirected to the module's
                ...
                /**
                 * If account menu item is hidden, redirect to module's main settings page.
                 *
                 * @author Vova Feldman (@svovaf)
                 * @since  1.1.6
                 *
                 * @link   https://github.com/Freemius/mandarincms-sdk/issues/6
                 */
        ...
        private function _fetch_site_plan( $plan_id = false ) {
            ...
            return ! isset( $plan->error ) ? new FS_Module_Plan( $plan ) : $plan;
        }
         ...
         * @return FS_Module_Plan[]|object
         */
        private function _fetch_module_plans() {
            $this->_logger->entrance();
             ...
             */
            $result = $api->get( "/modules/{$this->_module_id}/plans.json?show_pending=" . ( $this->has_secret_key() ? 'true' : 'false' ), true );

            if ( $this->is_api_result_object( $result, 'plans' ) && is_array( $result->plans ) ) {
                for ( $i = 0, $len = count( $result->plans ); $i < $len; $i ++ ) {
                    $result->plans[ $i ] = new FS_Module_Plan( $result->plans[ $i ] );
                }
        ...
        private function fetch_plan_by_id( $plan_id ) {
            ...
            $result = $api->get( "/modules/{$this->_module_id}/plans/{$plan_id}.json", true );

            return $this->is_api_result_entity( $result ) ?
                new FS_Module_Plan( $result ) :
                $result;
        }
         ...
         * @param number|bool $module_id
         * @param number|bool $site_license_id
         * @param array       $foreign_licenses @since 2.0.0. This is used by network-activated modules.
         * @param number|null $blog_id
         *
         * @return FS_Module_License[]|object
         */
        private function _fetch_licenses(
            ...
            $user_licenses_endpoint = "/modules/{$module_id}/licenses.json";
            ...
            if ( ! $is_site_license_synced ) {
                ...
                if ( is_numeric( $site_license_id ) ) {
                    ...
                    if ( $this->is_api_result_object( $api_result, 'licenses' ) &&
                        ...
                        if ( ! empty( $licenses ) ) {
                            $result[] = new FS_Module_License( $licenses[0] );
                        }
                    } else {
                    ...
                    if ( ! $is_license_in_result ) {
                        ...
                        if ( $this->is_api_result_entity( $license ) ) {
                            $result[] = new FS_Module_License( $license );
                        } else {
                            $api_errors[] = $license;
                        }
                    }
         ...
         */
        function _fetch_payments( $module_id = false, $flush = false ) {
            $this->_logger->entrance();
            ...
            $result = $api->get( "/modules/{$module_id}/payments.json?include_addons=true", $flush );
        ...
        function _fetch_billing( $flush = false ) {
            require_once MCMS_FS__DIR_INCLUDES . '/entities/class-fs-billing.php';
         ...
         *
         * @return object|false New module tag info if exist.
         */
        private function _fetch_newer_version( $module_id = false, $flush = true, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
            $latest_tag = $this->_fetch_latest_version( $module_id, $flush, $expiration );
            ...
            $has_new_version =
                ...
                ( $this->_is_addon_id( $module_id ) && ! $this->is_addon_activated( $module_id ) ) ||
                // Compare versions.
                version_compare( $this->get_module_version(), $latest_tag->version, '<' );

            $this->_logger->departure( $has_new_version ? 'Found newer module version ' . $latest_tag->version : 'No new version' );
         ...
         * @param int         $expiration Since 1.2.2.7
         *
         * @return bool|FS_Module_Tag
         */
        function get_update( $module_id = false, $flush = true, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
            $this->_logger->entrance();
            ...
            $this->check_updates( true, $module_id, $flush, $expiration );
            $updates = $this->get_all_updates();

            return isset( $updates[ $module_id ] ) && is_object( $updates[ $module_id ] ) ? $updates[ $module_id ] : false;
        }
        ...
        /**
         * Checks if the current user can activate modules or switch myskins. Note that this method should only be used
        ...
        function is_user_admin() {
            ...
            return ( $this->is_module() && current_user_can( is_multisite() ? 'manage_options' : 'activate_modules' ) )
                   || ( $this->is_myskin() && current_user_can( 'switch_myskins' ) );
        }
        ...
        private function _sync_license( $background = false, $is_context_single_site = false ) {
            ...
            $module_id = fs_request_get( 'module_id', $this->get_id() );

            $is_addon_sync = ( ! $this->_module->is_addon() && $module_id != $this->get_id() );

            if ( $is_addon_sync ) {
                $this->_sync_addon_license( $module_id, $background );
            } else {
                $this->_sync_module_license( $background, true, $is_context_single_site );
            }
        ...
        private function _sync_addon_license( $addon_id, $background ) {
            ...
            if ( $this->is_array_instanceof( $licenses, 'FS_Module_License' ) ) {
                ...
                if ( ! $this->is_addon_installed( $addon->id ) && FS_License_Manager::has_premium_license( $licenses ) ) {
                    $plans_result = $this->get_api_site_or_module_scope()->get( "/addons/{$addon_id}/plans.json" );

                    if ( ! isset( $plans_result->error ) ) {
                        $plans = array();
                        foreach ( $plans_result->plans as $plan ) {
                            $plans[] = new FS_Module_Plan( $plan );
                        }
         ...
         */
        private function _sync_module_license(
            $background = false,
                ...
                if ( ! $is_valid ) {
                    ...
                    if ( ! $background || $this->is_paying() ) {
                        ...
                        if ( ! FS_Api::test() ) {
                             ...
                             *
                             * @author Vova Feldman (@svovaf)
                             * @since  1.1.6 Only show message related to one of the Freemius powered modules. Once it will be resolved it will fix the issue for all modules anyways. There's no point to scare users with multiple error messages.
                             */
                            $api = $this->get_api_site_scope();
                            ...
                            if ( ! self::$_global_admin_notices->has_sticky( 'api_blocked' ) ) {
                                self::$_global_admin_notices->add(
                                    sprintf(
                                        $this->get_text_x_inline( 'Your server is blocking the access to Freemius\' API, which is crucial for %1s synchronization. Please contact your host to whitelist %2s', '%1s - module title, %2s - API domain', 'server-blocking-access' ),
                                        $this->get_module_name(),
                    ...
                    case 'downgraded':
                        $this->_admin_notices->add_sticky(
                            ($this->has_free_plan() ?
                                ...
                                /* translators: %1$s: product title; %2$s, %3$s: wrapping HTML anchor element; %4$s: 'module', 'myskin', or 'add-on'. */
        ...
        protected function _activate_license( $background = false ) {
            ...
            if ( is_object( $this->_site ) &&
                 FS_Module_License::is_valid_id( $license_id ) &&
            ...
            $premium_license = FS_Module_License::is_valid_id( $license_id ) ?
            ...
            $premium_license = new FS_Module_License( $license );
            ...
            if ( is_array( $this->_licenses ) ) {
                for ( $i = 0, $len = count( $this->_licenses ); $i < $len; $i ++ ) {
                    if ( $license->id == $this->_licenses[ $i ]->id ) {
                        $this->_licenses[ $i ] = new FS_Module_License( $license );
                    }
                }
            }
        ...
        private function _downgrade_site() {
            ...
            if ( $this->is_api_result_entity( $site ) ) {
                ...
                $plan_downgraded = ( $plan instanceof FS_Module_Plan && $prev_plan_id != $plan->id ) ||
         ...
         * @return bool
         */
        private function _is_addon_id( $module_id ) {
            return is_numeric( $module_id ) && ( $this->get_id() != $module_id );
        }
         ...
         * @param int         $expiration Since 1.2.2.7
         *
         * @return object|false Module latest tag info.
         */
        function _fetch_latest_version(
            $addon_id = false,
            $flush = true,
            $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC
        ) {
            $this->_logger->entrance();
            ...
            /**
             * @since 1.1.7.3 Check for module updates from Freemius only if opted-in.
            ...
            $tag = $this->get_api_site_or_module_scope()->get(
        ...
        /**
         * Download latest module version, based on plan.
         *
         * Not like _download_latest(), this will redirect the page
         * to secure download url to prevent dual download (from FS to MCMS server,
         * and then from MCMS server to the client / browser).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         * @uses   mcms_redirect()
         */
        private function download_latest_directly( $module_id = false ) {
            $this->_logger->entrance();

            mcms_redirect( $this->get_latest_download_api_url( $module_id ) );
        }

        /**
         * Get latest module FS API download URL.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         * @return string
         */
        private function get_latest_download_api_url( $module_id = false ) {
            $this->_logger->entrance();

            return $this->get_api_site_scope()->get_signed_url(
                $this->_get_latest_version_endpoint( $module_id, 'zip' )
            );
        }
        ...
        /**
         * Get latest module download link.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         * @return string
         */
        private function get_latest_download_link( $label, $module_id = false ) {
            return sprintf(
                '<a target="_blank" href="%s">%s</a>',
                $this->_get_latest_download_local_url( $module_id ),
                $label
            );
        }

        /**
         * Get latest module download local URL.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
         ...
         * @return string
         */
        function _get_latest_download_local_url( $module_id = false ) {
            // Add timestamp to protect from caching.
            $params = array( 'ts' => MCMS_FS__SCRIPT_START_TIME );
        ...
        private function check_updates(
            ...
            $new_version = $this->_fetch_newer_version( $module_id, $flush, $expiration );

            $update = null;
            if ( is_object( $new_version ) ) {
                $update = new FS_Module_Tag( $new_version );
            ...
            $this->_store_update( $update, true, $module_id );
        }
        ...
        private function sync_addons( $flush = false ) {
            ...
            $api = $this->get_api_site_or_module_scope();
            ...
            $addons = array();
            if ( $this->is_api_result_object( $result, 'modules' ) &&
                 is_array( $result->modules )
            ) {
                for ( $i = 0, $len = count( $result->modules ); $i < $len; $i ++ ) {
                    $addons[ $i ] = new FS_Module( $result->modules[ $i ] );
                }
        ...
        private function update_email( $new_email ) {
            ...
            $api  = $this->get_api_user_scope();
            $user = $api->call( "?module_id={$this->_module->id}&fields=id,email,is_verified", 'put', array(
                'email'                   => $new_email,
        ...
        private function update_user_name() {
            ...
            $api  = $this->get_api_user_scope();
            $user = $api->call( "?module_id={$this->_module->id}&fields=id,first,last", 'put', array(
                'name' => $name,
            ) );
        ...
        function get_after_activation_url( $filter, $params = array(), $network = null ) {
            if ( $this->is_free_mcms_org_myskin() &&
            ...
            switch ( $action ) {
                ...
                case 'delete_account':
                    ...
                    if ( $module_id == $this->get_id() ) {
                        ...
                        if ( $this->is_addon_activated( $module_id ) ) {
                            $fs_addon = self::get_instance_by_id( $module_id );
                ...
                case 'downgrade_account':
                    ...
                    } else if ( $this->is_addon_activated( $module_id ) ) {
                        $fs_addon = self::get_instance_by_id( $module_id );
                ...
                case 'activate_license':
                    ...
                    $fs = $this;
                    if ( $module_id != $this->get_id() ) {
                        $fs = $this->is_addon_activated( $module_id ) ?
                            self::get_instance_by_id( $module_id ) :
                            null;
                    }
                ...
                case 'download_latest':
                    $this->download_latest_directly( $module_id );
        ...
        function _addons_page_load() {
            ...
            if ( ! $this->is_registered() && $this->is_org_repo_compliant() ) {
                $this->_admin_notices->add(
                    sprintf( $this->get_text_inline( 'Just letting you know that the add-ons information of %s is being pulled from an external server.', 'addons-info-external-message' ), '<b>' . $this->get_module_name() . '</b>' ),
        ...
        static function _clean_admin_content_section_hook() {
            ...
            // Hide footer.
            echo '<style>#mcmsfooter { display: none !important; }</style>';
        }
        ...
        /*		function _enqueue_script($handle, $src) {
					$url = modules_url( substr( MCMS_FS__DIR_JS, strlen( $this->_module_dir_path ) ) . '/assets/js/' . $src );

					$this->_logger->entrance( 'script = ' . $url );

					mcms_enqueue_script( $handle, $url );
				}*/
         ...
         * @return FS_Api
         */
        function get_api_module_scope() {
            if ( ! isset( $this->_module_api ) ) {
                $this->_module_api = FS_Api::instance(
                    $this->_module_id,
                    'module',
                    $this->_module->id,
                    $this->_module->public_key,
                    ! $this->is_live()
                );
            }

            return $this->_module_api;
        }

        /**
         * Get site API scope object (fallback to public module scope when not registered).
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.7
         *
         * @return FS_Api
         */
        function get_api_site_or_module_scope() {
            return $this->is_registered() ?
                $this->get_api_site_scope() :
                $this->get_api_module_scope();
        }
        ...
        function _add_trial_notice() {
            ...
            if ( $this->is_premium() && ! MCMS_FS__DEV_MODE ) {
            ...
            if ( ! $was_promotion_shown_before &&
                 $this->_storage->install_timestamp > ( time() - MCMS_FS__TIME_24_HOURS_IN_SEC )
            ) {
                return false;
            }
            ...
            if ( $was_promotion_shown_before &&
                 30 * MCMS_FS__TIME_24_HOURS_IN_SEC > time() - $last_time_trial_promotion_shown
            ) {
                return false;
            }
            ...
            $message = sprintf(
                ...
                sprintf( '<b>%s</b>', $this->get_module_name() ),
                $plans_string,
                $trial_period
            );
            ...
            $this->_storage->trial_promotion_shown = MCMS_FS__SCRIPT_START_TIME;
        ...
        function _add_affiliate_program_notice() {
            ...
            if ( $was_notice_shown_before ||
                 $this->_storage->install_timestamp > ( time() - ( MCMS_FS__TIME_24_HOURS_IN_SEC * 30 ) )
            ) {
                return false;
            }

            if ( ! $this->is_paying() &&
                 FS_Module::AFFILIATE_MODERATION_CUSTOMERS == $this->_module->affiliate_moderation
            ) {
            ...
            $message = sprintf(
                ...
                sprintf( '<strong>%s</strong>', $this->get_module_name() ),
            ...
            $this->_storage->affiliate_program_notice_shown = MCMS_FS__SCRIPT_START_TIME;
         ...
         * @since  1.2.2
         */
        function _show_myskin_activation_optin_dialog() {
            fs_enqueue_local_style( 'fs_connect', '/admin/connect.css' );

            add_action( 'admin_footer-myskins.php', array( &$this, '_add_fs_myskin_activation_dialog' ) );
        }
         ...
         * @since  1.2.2
         */
        function _add_fs_myskin_activation_dialog() {
            $vars = array( 'id' => $this->_module_id );
        ...
        /**
         * Hook to module action links filter.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.0
         */
        private function hook_module_action_links() {
            $this->_logger->entrance();
            ...
            add_filter( 'module_action_links_' . $this->_module_basename, array(
                &$this,
                '_modify_module_action_links_hook'
            ), MCMS_FS__DEFAULT_PRIORITY, 2 );
            add_filter( 'network_admin_module_action_links_' . $this->_module_basename, array(
                &$this,
                '_modify_module_action_links_hook'
            ), MCMS_FS__DEFAULT_PRIORITY, 2 );
        }
         ...
         * @param bool $key
         */
        function add_module_action_link( $label, $url, $external = false, $priority = MCMS_FS__DEFAULT_PRIORITY, $key = false ) {
            $this->_logger->entrance();
        ...
        /**
         * Adds Upgrade and Add-Ons links to the main Modules page link actions collection.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.0
         */
        ...
        /**
         * Adds "Activate License" or "Change License" link to the main Modules page link actions collection.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.1.9
         */
        ...
        /**
         * Adds "Opt in" or "Opt out" link to the main "Modules" page link actions collection.
         *
         * @author Leo Fajardo (@leorw)
         * @since  1.2.1.5
         */
        function _add_tracking_links() {
            ...
            if ( ( $this->is_module() && ! self::is_modules_page() ) ||
                ...
                // Only show tracking links on the modules and myskins pages.
                return;
            }
            ...
            if ( $this->is_registered() ) {
                ...
                $params = ! $this->is_anonymous() ?
                    array() :
                    array(
                        'nonce'     => mcms_create_nonce( $this->get_unique_affix() . '_reconnect' ),
        ...
        /**
         * Get the URL of the page that should be loaded right after the module activation.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.1.7.4
         *
         * @return string
         */
        function get_after_module_activation_redirect_url() {
            $url = false;
            ...
            if ( ! $this->is_addon() || ! $this->has_free_plan() ) {
                ...
                if ( $this->is_parent_module_installed() ) {
                    $module_fs = self::get_parent_instance();
                }

                if ( is_object( $module_fs ) ) {
                    if ( ! $module_fs->is_registered() ) {
                        // Forward to parent module connect when parent not registered.
                        $url = $module_fs->get_activation_url();
                    } else {
                        // Forward to account page.
                        $url = $module_fs->_get_admin_page_url( 'account' );
                    }
                }
            }
        ...
        function _redirect_on_activation_hook() {
            $url = $this->get_after_module_activation_redirect_url();
        ...
        /**
         * Modify module's page action links collection.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.0
         *
         ...
         * @return array
         */
        function _modify_module_action_links_hook( $links, $file ) {
            $this->_logger->entrance();
            ...
            if ( ! empty( $deactivate_link ) ) {
                /**
                 * This HTML element is used to identify the correct module when attaching an event to its Deactivate link.
                 *
        ...
        private function get_complete_upgrade_instructions( $plan_title = '' ) {
            ...
            return sprintf(
                ...
                '//bit.ly/upload-mcms-' . $this->_module_type . 's',
        ...
        private static function safe_remote_post(
            ...
            $response = (!MCMS_FS__DEBUG_SDK && ( false !== $cache_key )) ?
            ...
            if ( false === $response ) {
                ...
                $response = mcms_remote_post( $url, $request );

                if ( $response instanceof MCMS_Error ) {
                ...
                if ( false !== $cache_key ) {
                    set_transient(
                        $cache_key,
                        $response,
                        ( ( $response instanceof MCMS_Error ) ?
         ...
         * @return bool
         */
        function is_module_upgrade_mode() {
            return isset( $this->_storage->module_upgrade_mode ) ?
                $this->_storage->module_upgrade_mode :
                false;
        }

        /**
         * Turn module upgrade mode off.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         */
        function set_module_upgrade_complete() {
            $this->_storage->module_upgrade_mode = false;
        }
        ...
        /**
         * After upgrade callback to install and auto activate a module.
         * This code will only be executed on explicit request from the user,
         * following the practice Jetpack are using with their myskin installations.
         *
        ...
        function _install_premium_version_ajax_action() {
            ...
            $module_id = fs_request_get( 'target_module_id', $this->get_id() );
            ...
            if ( $module_id == $this->get_id() ) {
                ...
                if ( ! $this->has_release_on_freemius() ) {
                    // Module is a serviceware, no premium code version.
                    self::shoot_ajax_failure( array(
                        'message' => $this->get_text_inline( 'Module is a "Serviceware" which means it does not have a premium code version.', 'auto-install-error-serviceware' ),
                        'code'    => 'premium_version_missing',
                    ) );
                }
            } else {
                $addon = $this->get_addon( $module_id );
                ...
                if ( $this->is_addon_activated( $module_id, true ) ) {
            ...
            $updater = FS_Module_Updater::instance( $this );
            $result  = $updater->install_and_activate_module( $module_id );
        ...
        /**
         * Inject a JavaScript logic to capture the myskin tabs HTML.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.2.7
         */
        function _tabs_capture() {
            ...
            if ( ! $this->is_myskin_settings_page() ||
        ...
        /**
         * Cache myskin's tabs HTML for a week. The cache will also be set as expired
        ...
        function _store_tabs_ajax_action() {
            ...
            $tabs_html = $mcms_filesystem->get_contents( "php://input" );
            ...
            $this->_cache->set( 'tabs', $tabs_html, 7 * MCMS_FS__TIME_24_HOURS_IN_SEC );

            self::shoot_ajax_success();
        }

        /**
         * Cache myskin's settings page custom styles. The cache will also be set as expired
         * after version and type (free/premium) changes, in addition to the week period.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.2.7
        ...
        function _store_tabs_styles() {
            ...
            $myskin_styles_url = get_template_directory_uri();

            $stylesheets = array();
            foreach ( $mcms_styles->queue as $handler ) {
                ...
                /**
                 * @var _MCMS_Dependency $stylesheet
                 */
                $stylesheet = $mcms_styles->registered[ $handler ];

                if ( fs_starts_with( $stylesheet->src, $myskin_styles_url ) ) {
                    $stylesheets[] = $stylesheet->src;
                }
            }

            if ( ! empty( $stylesheets ) ) {
                $this->_cache->set( 'tabs_stylesheets', $stylesheets, 7 * MCMS_FS__TIME_24_HOURS_IN_SEC );
            }
        }
        ...
        private function should_page_include_tabs() {
            ...
            if ( ! $this->is_myskin() ) {
                // Only add tabs to myskins for now.
                return false;
            }

            if ( ! $this->has_paid_plan() && ! $this->has_addons() ) {
                // Only add tabs to monetizing myskins.
                return false;
            }

            if ( ! $this->is_myskin_settings_page() ) {
                // Only add tabs if browsing one of the myskin's setting pages.
                return false;
            }
        ...
        function _add_tabs_before_content() {
             ...
             * myskin settings page. That way, if the myskin settings has
            ...
            if ( is_array( $stylesheets ) ) {
                for ( $i = 0, $len = count( $stylesheets ); $i < $len; $i ++ ) {
                    mcms_enqueue_style( "fs_{$this->_module_id}_tabs_{$i}", $stylesheets[ $i ] );
                }
            }
        ...
        #--------------------------------------------------------------------------------
        #region Customizer Integration for mySkins
        #--------------------------------------------------------------------------------
         ...
         * @since  1.2.2.7
         *
         * @param MCMS_Customize_Manager $customizer
         */
        function _customizer_register( $customizer ) {
            $this->_logger->entrance();

            if ( $this->is_pricing_page_visible() ) {
                require_once MCMS_FS__DIR_INCLUDES . '/customizer/class-fs-customizer-upsell-control.php';
            ...
            if ( $this->is_page_visible( 'contact' ) || $this->is_page_visible( 'support' ) ) {
                require_once MCMS_FS__DIR_INCLUDES . '/customizer/class-fs-customizer-support-section.php';
        ...
        /**
         * If the myskin has a paid version, add some custom
         * styling to the myskin's premium version (if exists)
         * to highlight that it's the premium version of the
         * same myskin, making it easier for identification
         * after the user upgrades and upload it to the site.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.2.2.7
         */
        function _style_premium_myskin() {
            $this->_logger->entrance();

            if ( ! self::is_myskins_page() ) {
                // Only include in the myskins page.
                return;
            }
            ...
            fs_require_template( '/js/style-premium-myskin.php', $params );
        }

        /**
         * This method will return the absolute URL of the module's local icon.
         *
         * When you are running your module or myskin on a **localhost** environment, if the icon
         * is not found in the local assets folder, try to fetch the icon URL from Freemius. If not set and
         * it's a module hosted on MandarinCMS.org, try fetching the icon URL from mandarincms.org.
        ...
        function get_local_icon_url() {
             ...
             * @since 1.1.7.5
             */
            $local_path = $this->apply_filters( 'module_icon', false );

            if ( is_string( $local_path ) ) {
                $icons = array( $local_path );
            } else {
                $img_dir = MCMS_FS__DIR_IMG;

                // Locate the main assets folder.
                if ( 1 < count( $fs_active_modules->modules ) ) {
                    $module_or_myskin_img_dir = ( $this->is_module() ? MCMS_PLUGIN_DIR : get_myskin_root() );

                    foreach ( $fs_active_modules->modules as $sdk_path => &$data ) {
                        if ( $data->module_path == $this->get_module_basename() ) {
                            $img_dir = $module_or_myskin_img_dir
                                       . '/'
                                       . str_replace( '../myskins/', '', $sdk_path )
                                       . '/assets/img';
                ...
                if ( ! is_array( $icons ) || 0 === count( $icons ) ) {
                    if ( ! MCMS_FS__IS_LOCALHOST && $this->is_myskin() ) {
                        $icons = array(
                            fs_normalize_path( $img_dir . '/myskin-icon.png' )
                        );
                    } else {
                        $icon_found = false;
                         ...
                         * of the module in a local environment. The module icon file WILL
                         * already exist in the assets folder when the module is deployed to
                         * the repository.
                         */
                        if ( MCMS_FS__IS_LOCALHOST && $have_write_permissions ) {
                            // Fetch icon from Freemius.
                            $icon = $this->fetch_remote_icon_url();

                            // Fetch icon from MandarinCMS.org.
                            if ( empty( $icon ) && $this->is_module() && $this->is_org_repo_compliant() ) {
                                if ( ! function_exists( 'modules_api' ) ) {
                                    require_once BASED_TREE_URI . 'mcms-admin/includes/module-install.php';
                                }

                                $module_information = modules_api( 'module_information', array(
                                    'slug'   => $this->_slug,
                                    'fields' => array(
                                        'sections' => false,
                                        'tags'     => false,
                                ...
                                if (
                                    ! is_mcms_error( $module_information )
                                    && isset( $module_information->icons )
                                    && ! empty( $module_information->icons )
                                ) {
                                    /**
                                     ...
                                     * @since  1.2.2
                                     */
                                    $icon = end( $module_information->icons );
                                }
                            }
        ...
        private function fetch_module_info() {
            return $this->get_api_module_scope()->get( 'info.json', false, MCMS_FS__TIME_WEEK_IN_SEC );
        }
        ...
        function fetch_and_store_current_user_gdpr_anonymously() {
            ...
            if ( ! $this->get_api_module_scope()->is_valid_ping( $pong ) ) {
                return false;
            } else {
                FS_GDPR_Manager::instance()->store_is_required( $pong->is_gdpr_required );
         ...
         * @since  2.1.0
         *
         * @param array $user_modules
         *
         * @return string
         */
        private function get_gdpr_admin_notice_string( $user_modules ) {
            $this->_logger->entrance();

            $addons = self::get_all_addons();

            foreach ( $user_modules as $user_module ) {
                $has_addons = isset( $addons[ $user_module->id ] );

                if ( MCMS_FS__MODULE_TYPE_PLUGIN === $user_module->type && ! $has_addons ) {
                    if ( $this->_module_id == $user_module->id ) {
                        $addons     = $this->get_addons();
                        $has_addons = ( ! empty( $addons ) );
                    } else {
                        $module_api = FS_Api::instance(
                            $user_module->id,
                            'module',
                            $user_module->id,
                            $user_module->public_key,
                            ! $user_module->is_live
                        );

                        $addons_result = $module_api->get( '/addons.json?enriched=true', true );

                        if ( $this->is_api_result_object( $addons_result, 'modules' ) &&
                             is_array( $addons_result->modules ) &&
                             ! empty( $addons_result->modules )
                        ) {
                            $has_addons = true;
                        }
                    }
                ...
                $user_module->has_addons = $has_addons;
            }

            $is_single_parent_product = ( 1 === count( $user_modules ) );

            $multiple_products_text = '';

            if ( $is_single_parent_product ) {
                $single_parent_product = reset( $user_modules );

                $thank_you = sprintf(
                    "<span data-module-id='%d'>%s</span>",
                ...
                $already_opted_in = sprintf(
                    ...
                    ( MCMS_FS__MODULE_TYPE_THEME === $single_parent_product->type ) ? MCMS_FS__MODULE_TYPE_THEME : MCMS_FS__MODULE_TYPE_PLUGIN
                );
            } else {
         ...
         * modules and myskins they've opted in to.
         *
         * @author Leo Fajardo (@leorw)
         * @since 2.1.0
         *
         ...
         * @return array|false
         */
        private function fetch_user_marketing_flag_status_by_modules( $user_email, $license_key, $module_ids ) {
            $request = array(
                'method'  => 'POST',
                'body'    => array(),
                'timeout' => MCMS_FS__DEBUG_SDK ? 60 : 30,
            ...
            $url              = MCMS_FS__ADDRESS . '/action/service/user_module/';
            $total_module_ids = count( $module_ids );

            $module_ids_count_per_request = 10;
            for ( $i = 1; $i <= $total_module_ids; $i += $module_ids_count_per_request ) {
                $module_ids_set = array_slice( $module_ids, $i - 1, $module_ids_count_per_request );

                $request['body']['module_ids'] = $module_ids_set;

                $response = self::safe_remote_post(
                    $url,
                    $request,
                    MCMS_FS__TIME_24_HOURS_IN_SEC,
                    MCMS_FS__TIME_12_HOURS_IN_SEC
                );
        ...
        function _maybe_show_gdpr_admin_notice() {
            ...
            /**
             * Try to acquire a 60-sec lock based on the MCMS user and thread/process ID.
             */
            if ( ! $lock->try_lock( 60 ) ) {
                return;
            }
            ...
            /**
             * @var $current_mcms_user MCMS_User
             */
            $current_mcms_user = self::_get_current_mcms_user();

            /**
             * @var FS_User $current_fs_user
             */
            $current_fs_user = Freemius::_get_user_by_email( $current_mcms_user->user_email );

            $ten_years_in_sec = 10 * 365 * MCMS_FS__TIME_24_HOURS_IN_SEC;
            ...
            if ( $gdpr->is_opt_in_notice_shown() ) {
                // 30-day lock.
                $lock->lock( 30 * MCMS_FS__TIME_24_HOURS_IN_SEC );
            ...
            if ( $was_notice_shown_before &&
                 30 * MCMS_FS__TIME_24_HOURS_IN_SEC > time() - $last_time_notice_shown
            ) {
            ...
            /**
             * Find all module IDs that were installed by the current admin.
             */
            $module_ids_map = self::get_user_opted_in_module_ids_map( $current_fs_user->id );

            if ( empty( $module_ids_map )) {
                $lock->lock( $ten_years_in_sec );

                return;
            }

            $user_modules = $this->fetch_user_marketing_flag_status_by_modules(
                $current_fs_user->email,
                null,
                array_keys( $module_ids_map )
            );

            if ( empty( $user_modules ) ) {
                $lock->lock(
                    is_array($user_modules) ?
                        $ten_years_in_sec :
            ...
            foreach ( $user_modules as $user_module ) {
                if ( true == $user_module->is_marketing_allowed ) {
                    unset( $module_ids_map[ $user_module->module_id ] );
                }

                if ( ! $has_unset_marketing_optin && is_null( $user_module->is_marketing_allowed ) ) {
            ...
            $modules = array_merge(
                array_values( self::$_accounts->get_option( 'modules', array() ) ),
                array_values( self::$_accounts->get_option( 'myskins', array() ) )
            );

            foreach ( $modules as $module ) {
                if ( ! FS_Module::is_valid_id( $module->parent_module_id ) && isset( $module_ids_map[ $module->id ] ) ) {
                    $module_ids_map[ $module->id ] = $module;
                }
            }

            $module_title = null;
            if ( 1 === count( $module_ids_map ) ) {
                $module       = reset( $module_ids_map );
                $module_title = $module->title;
            }

            $gdpr->add_opt_in_sticky_notice(
                $this->get_gdpr_admin_notice_string( $module_ids_map ),
                $module_title
            );
        ...
        private function disable_opt_in_notice_and_lock_user() {
            ...
            // 10-year lock.
            FS_User_Lock::instance()->lock( 10 * 365 * MCMS_FS__TIME_24_HOURS_IN_SEC );
        }
        ...
        function _fetch_is_marketing_required_flag_value_ajax_action() {
            ...
            if ( ! is_array( $user_modules ) ||
                 empty($user_modules) ||
                 !isset($user_modules[0]->module_id) ||
                 $user_modules[0]->module_id != $this->_module_id
            ) {
                /**
            ...
            self::shoot_ajax_success( array( 'is_marketing_allowed' => $user_modules[0]->is_marketing_allowed ) );
        }
        ...
        function _gdpr_optin_ajax_action() {
            ...
            if ( ! fs_request_has( 'is_marketing_allowed' ) || ! fs_request_has( 'module_ids' ) ) {
                self::shoot_ajax_failure();
            }

            $current_mcms_user = self::_get_current_mcms_user();

            $module_ids = fs_request_get( 'module_ids', array() );
            ...
            $user_api = $this->get_api_user_scope_by_user( Freemius::_get_user_by_email( $current_mcms_user->user_email ) );

            foreach ( $modules as $module ) {
                $user_api->call( "?module_id={$module->id}", 'put', array(
        ...
        /**
         * Check if current user purchased any other modules before.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
