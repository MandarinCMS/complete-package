<?php
    ...
    class FS_Module_Info_Dialog {
        ...
        function __construct( Freemius $fs ) {
            ...
            $this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_' . $fs->get_slug() . '_info', MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );

            // Remove default module information action.
            remove_all_actions( 'install_modules_pre_module-information' );

            // Override action with custom modules function for add-ons.
            add_action( 'install_modules_pre_module-information', array( &$this, 'install_module_information' ) );

            // Override request for module information for Add-ons.
            add_filter(
                'fs_modules_api',
                array( &$this, '_get_addon_info_filter' ),
                MCMS_FS__DEFAULT_PRIORITY, 3 );
        }

        /**
         * Generate add-on module information.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.6
         *
        ...
        function _get_addon_info_filter( $data, $action = '', $args = null ) {
            ...
            $parent_module_id = fs_request_get( 'parent_module_id', false );

            if ( $this->_fs->get_id() != $parent_module_id ||
                 ( 'module_information' !== $action ) ||
                 ! isset( $args->slug )
            ) {
                return $data;
            }
            ...
            // Find add-on by slug.
            $selected_addon = $this->_fs->get_addon_by_slug( $args->slug, MCMS_FS__DEV_MODE );
            ...
            $result = $this->_fs->get_api_module_scope()->get( "/addons/{$selected_addon->id}/pricing.json?type=visible" );
            ...
            if ( ! isset( $result->error ) ) {
                ...
                if ( is_array( $plans ) ) {
                    for ( $i = 0, $len = count( $plans ); $i < $len; $i ++ ) {
                        ...
                        $plans[ $i ] = new FS_Module_Plan( $plans[ $i ] );
                        $plan        = $plans[ $i ];
            ...
            if ( ! $has_paid_plan && $selected_addon->is_mcms_org_compliant ) {
                $repo_data = FS_Module_Updater::_fetch_module_info_from_repository(
                    'module_information', (object) array(
                ...
                if ( ! empty( $repo_data ) ) {
                    $data                 = $repo_data;
                    $data->mcms_org_missing = false;
                } else {
                    // Couldn't find module on .org.
                    $selected_addon->is_mcms_org_compliant = false;

                    // Module is missing, not on Freemius nor MCMS.org.
                    $data->mcms_org_missing = true;
                }
            } else {
                $data->mcms_org_missing = false;
                ...
                $view_vars         = array( 'module' => $selected_addon );
                $data->sections    = array(
                    'description' => fs_get_template( '/module-info/description.php', $view_vars ),
                );
                ...
                if ( ! empty( $selected_addon->info->screenshots ) ) {
                    $view_vars                     = array(
                        ...
                        'module'      => $selected_addon,
                    );
                    $data->sections['screenshots'] = fs_get_template( '/module-info/screenshots.php', $view_vars );
                }
                ...
                if ( is_object( $latest ) ) {
                    ...
                    // Add message to developer to deploy the module through Freemius.
                }
            }
            ...
            if ( $has_pricing ) {
                ...
                if ( $has_features ) {
                    ...
                    $data->sections['features'] = fs_get_template( '/module-info/features.php', $view_vars );
                }
            }
            ...
            $data->is_mcms_org_compliant = $selected_addon->is_mcms_org_compliant;
         ...
         * @since  1.1.7
         *
         * @param FS_Module_Plan $plan
         *
         * @return string
         */
        private function get_billing_cycle( FS_Module_Plan $plan ) {
            $billing_cycle = null;
         ...
         */
        private function get_price_tag( FS_Module_Plan $plan, FS_Pricing $pricing ) {
            $price_tag = '';
            if ( isset( $pricing->annual_price ) ) {
        ...
        private function get_download_cta( $api, $is_primary = true ) {
            ...
            // Hosted on MandarinCMS.org.
            switch ( $status['status'] ) {
                case 'install':
                    if ( $api->is_mcms_org_compliant ||
                         ! $this->_fs->is_org_repo_compliant() ||
                         $this->_fs->is_premium()
                    ) {
                        /**
                         * Allow immediate installation if one of the following:
                         *  1. MandarinCMS.org add-on.
                         *  2. The core module is NOT mcms.org compliant.
                         *  3. The core module is running the premium version which is not mcms.org compliant.
                         */
                        if ( $status['url'] ) {
                            return $this->get_cta(
                                ( $has_paid_version ?
         ...
         *
         * Based on core install_module_information() function.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.6
         */
        function install_module_information() {
            ...
            $api = apply_filters( 'fs_modules_api', false, 'module_information', $args );
...
//			'tbody' => array(),
            ...
            $modules_section_titles = array(
                'description'  => fs_text_x_inline( 'Description', 'Module installer section title', 'description', $api->slug ),
                'installation' => fs_text_x_inline( 'Installation', 'Module installer section title', 'installation', $api->slug ),
                'faq'          => fs_text_x_inline( 'FAQ', 'Module installer section title', 'faq', $api->slug ),
                'screenshots'  => fs_text_inline( 'Screenshots', 'screenshots', $api->slug ),
                'changelog'    => fs_text_x_inline( 'Changelog', 'Module installer section title', 'changelog', $api->slug ),
                'reviews'      => fs_text_x_inline( 'Reviews', 'Module installer section title', 'reviews', $api->slug ),
                'other_notes'  => fs_text_x_inline( 'Other Notes', 'Module installer section title', 'other-notes', $api->slug ),
            );
...
//		}
            ...
            $modules_section_titles['features'] = fs_text_x_inline( 'Features & Pricing', 'Module installer section title', 'features-and-pricing', $api->slug );
            ...
            iframe_header( fs_text_inline( 'Module Install', 'module-install', $api->slug ) );
...
//	var_dump($api->banners);
        ...
        <div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
            <div class="fyi">
                <?php if ( $api->is_paid ) : ?>
                    <?php if ( isset( $api->plans ) ) : ?>
                        <div class="module-information-pricing">
                                ...
                                <div class="nav-tab-wrapper">
                                    ...
                                    <?php $i = 0;
                                        foreach ( $billing_cycles as $cycle ) : ?>
                                            ...
                                            <?php endif ?>
                                            <?php $i ++; endforeach ?>
                                    <?php mcms_enqueue_script( 'jquery' ) ?>
                                        ...
                                        (function ($, undef) {
                                            var
                                                ...
                                                _checkoutUrl            = function (plan, pricing, cycle) {
                                                    return '<?php echo esc_url_raw( remove_query_arg( 'billing_cycle', add_query_arg( array( 'module_id' => $plan->module_id ), $api->checkout_link ) ) ) ?>' +
                                                        '&plan_id=' + plan +
                                                        '&pricing_id=' + pricing +
                                                ...
                                                _updateCtaUrl           = function (plan, pricing, cycle) {
                                                    $('.module-information-pricing .button, #module-information-footer .button.fs-checkout-button').attr('href', _checkoutUrl(plan, pricing, cycle));
                                                };

                                            $(document).ready(function () {
                                                var $plan = $('.module-information-pricing .fs-plan[data-plan-id=<?php echo $plan->id ?>]');
                    ...
                    <ul>
                        <?php if ( ! empty( $api->version ) ) { ?>
                            ...
                            if ( ! empty( $api->author ) ) {
                                ?>
                                <li>
                                    <strong><?php fs_echo_x_inline( 'Author', 'as the module author', 'author', $api->slug ); ?>
                ...
                /* translators: %s: time period (e.g. "2 hours" ago) */
                    ...
                    human_time_diff( strtotime( $api->last_updated ) )
                            ...
                            if ( ! empty( $api->requires ) ) {
                                ?>
                                <li>
                                    <strong><?php fs_esc_html_echo_inline( 'Requires MandarinCMS Version', 'requires-mandarincms-version', $api->slug ) ?>
                            ...
                            if ( ! empty( $api->slug ) && empty( $api->is_mcms_org_compliant ) ) {
                                ?>
                                <li><a target="_blank"
                                       href="https://mandarincms.org/modules/<?php echo $api->slug; ?>/"><?php fs_esc_html_echo_inline( 'MandarinCMS.org Module Page', 'mcms-org-module-page', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                                <?php
                            }
                            if ( ! empty( $api->homepage ) ) {
                                ?>
                                <li><a target="_blank"
                                       href="<?php echo esc_url( $api->homepage ); ?>"><?php fs_esc_html_echo_inline( 'Module Homepage', 'module-homepage', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                                <?php
                            }
                            if ( ! empty( $api->donate_link ) && empty( $api->contributors ) ) {
                                ?>
                                <li><a target="_blank"
                                       href="<?php echo esc_url( $api->donate_link ); ?>"><?php fs_esc_html_echo_inline( 'Donate to this module', 'donate-to-module', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                            <?php } ?>
                    </ul>
                ...
                <?php if ( ! empty( $api->rating ) ) { ?>
					...
					<span class="counter-label"><a
                            href="https://mandarincms.org/support/view/module-reviews/<?php echo $api->slug; ?>?filter=<?php echo $key; ?>"
                            target="_blank"
                    ...
                    if ( ! empty( $api->contributors ) ) {
                            ...
                            <?php
                                foreach ( (array) $api->contributors as $contrib_username => $contrib_profile ) {
                                    ...
                                    if ( empty( $contrib_profile ) ) {
                                        echo "<li><img src='https://mandarincms.org/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' />{$contrib_username}</li>";
                                    } else {
                                        echo "<li><a href='{$contrib_profile}' target='_blank'><img src='https://mandarincms.org/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' />{$contrib_username}</a></li>";
                                    }
                                }
                            ?>
                        </ul>
                        <?php if ( ! empty( $api->donate_link ) ) { ?>
                            <a target="_blank"
                               href="<?php echo esc_url( $api->donate_link ); ?>"><?php fs_echo_inline( 'Donate to this module', 'donate-to-module', $api->slug ) ?>
                                &#187;</a>
                        <?php } ?>
                    <?php } ?>
            </div>
            ...
            <?php
            if ( ! empty( $api->tested ) && version_compare( substr( $GLOBALS['mcms_version'], 0, strlen( $api->tested ) ), $api->tested, '>' ) ) {
                echo '<div class="notice notice-warning"><p>' . '<strong>' . fs_text_inline( 'Warning', 'warning', $api->slug ) . ':</strong> ' . fs_text_inline( 'This module has not been tested with your current version of MandarinCMS.', 'not-tested-warning', $api->slug ) . '</p></div>';
            } else if ( ! empty( $api->requires ) && version_compare( substr( $GLOBALS['mcms_version'], 0, strlen( $api->requires ) ), $api->requires, '<' ) ) {
                echo '<div class="notice notice-warning"><p>' . '<strong>' . fs_text_inline( 'Warning', 'warning', $api->slug ) . ':</strong> ' . fs_text_inline( 'This module has not been marked as compatible with your version of MandarinCMS.', 'not-compatible-warning', $api->slug ) . '</p></div>';
            }

            foreach ( (array) $api->sections as $section_name => $content ) {
                $content = links_add_base_url( $content, 'https://mandarincms.org/modules/' . $api->slug . '/' );
                ...
                if ( 'description' === $section_name &&
                     ( ( $api->is_mcms_org_compliant && $api->mcms_org_missing ) ||
                       ( ! $api->is_mcms_org_compliant && $api->fs_missing ) )
                ) {
                    $missing_notice = array(
                        ...
                        'message' => $api->is_paid ?
                            ...
                            fs_text_inline( 'Add-on must be deployed to MandarinCMS.org or Freemius.', 'free-addon-not-deployed', $api->slug ),
                    );
