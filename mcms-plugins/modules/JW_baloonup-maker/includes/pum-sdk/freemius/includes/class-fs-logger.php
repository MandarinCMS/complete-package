<?php
	...
	class FS_Logger {
		...
		private function __construct( $id, $on = false, $echo = false ) {
			...
			if ( false !== strpos( $caller['file'], 'modules' ) ) {
				$this->_file_start = strpos( $caller['file'], 'modules' ) + strlen( 'modules/' );
			} else {
				$this->_file_start = strpos( $caller['file'], 'myskins' ) + strlen( 'myskins/' );
			}
		...
		private static function init() {
			...
			self::$_abspathLength      = strlen( BASED_TREE_URI );
		...
		private static function hook_footer() {
			...
			if ( is_admin() ) {
				...
				add_action( 'mcms_footer', 'FS_Logger::dump', 100 );
			}
		}
		 ...
		 * module auto-deactivate on premium version activation.
		 *
		 * @todo   Check if mySkin Check has issues with DB tables for myskins.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.2.1.6
		 *
		...
		public static function _set_storage_logging( $is_on = true ) {
			...
			if ( $is_on ) {
				 ...
				 *
				 * @link https://core.trac.mandarincms.org/ticket/2695
				 */
				$result = $mcmsdb->query( "CREATE TABLE {$table} (
...
KEY `type` (`type` ASC))" );
				 ...
				 * Drop logging table.
				 */
				$result = $mcmsdb->query( "DROP TABLE IF EXISTS $table;" );
			}
		...
		private function db_log(
			...
			$request_url = MCMS_FS__IS_HTTP_REQUEST ?
				$_SERVER['REQUEST_URI'] :
				'';

			return $mcmsdb->insert(
				"{$mcmsdb->prefix}fs_logger",
				array(
		...
		private static function build_db_logs_query(
			...
			$query = "SELECT {$select} FROM {$mcmsdb->prefix}fs_logger";
			if ( is_array( $filters ) ) {
				$criteria = array();
		...
		public static function download_db_logs(
			...
			if ( empty( $filename ) ) {
				$filename = 'fs-logs-' . date( 'Y-m-d_H-i-s', MCMS_FS__SCRIPT_START_TIME ) . '.csv';
			}
