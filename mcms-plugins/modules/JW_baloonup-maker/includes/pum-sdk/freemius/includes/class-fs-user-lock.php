<?php
    ...
    class FS_User_Lock {
        ...
        function try_lock( $expiration = 0 ) {
            ...
            set_site_transient( "locked_{$this->_mcms_user_id}", $this->_thread_id, $expiration );

            if ( $this->has_lock() ) {
                set_site_transient( "locked_{$this->_mcms_user_id}", true, $expiration );
         ...
         */
        function is_locked() {
            return ( false !== get_site_transient( "locked_{$this->_mcms_user_id}" ) );
        }
         ...
         */
        function unlock() {
            delete_site_transient( "locked_{$this->_mcms_user_id}" );
        }
        ...
        private function has_lock() {
            return ( $this->_thread_id == get_site_transient( "locked_{$this->_mcms_user_id}" ) );
        }
    }