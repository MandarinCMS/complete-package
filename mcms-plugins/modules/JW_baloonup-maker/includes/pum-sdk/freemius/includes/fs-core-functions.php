<?php
    ...
    if ( ! function_exists( 'fs_get_template_path' ) ) {
        function fs_get_template_path( $path ) {
            return MCMS_FS__DIR_TEMPLATES . '/' . trim( $path, '/' );
        }
     ...
     * is inside a module's folder or a myskin's folder.
     *
     * Examples:
     * 1. "myskins" folder
     *    Path: C:/xampp/htdocs/fsmcms/mcms-content/myskins/twentytwelve/freemius/assets/css/admin/common.css
     *    URL: http://fsmcms:8080/mcms-content/myskins/twentytwelve/freemius/assets/css/admin/common.css
     *
     * 2. "modules" folder
     *    Path: C:/xampp/htdocs/fsmcms/mcms-content/modules/rating-widget-premium/freemius/assets/css/admin/common.css
     *    URL: http://fsmcms:8080/mcms-content/modules/rating-widget-premium/freemius/assets/css/admin/common.css
     *
    ...
    function fs_asset_url( $asset_abs_path ) {
        $mcms_content_dir = fs_normalize_path( MCMS_CONTENT_DIR );
        $asset_abs_path = fs_normalize_path( $asset_abs_path );
        $asset_rel_path = str_replace( $mcms_content_dir, '', $asset_abs_path );
    ...
    function fs_enqueue_local_style( $handle, $path, $deps = array(), $ver = false, $media = 'all' ) {
        mcms_enqueue_style( $handle, fs_asset_url( MCMS_FS__DIR_CSS . '/' . trim( $path, '/' ) ), $deps, $ver, $media );
    }

    function fs_enqueue_local_script( $handle, $path, $deps = array(), $ver = false, $in_footer = 'all' ) {
        mcms_enqueue_script( $handle, fs_asset_url( MCMS_FS__DIR_JS . '/' . trim( $path, '/' ) ), $deps, $ver, $in_footer );
    }

    function fs_img_url( $path, $img_dir = MCMS_FS__DIR_IMG ) {
    ...
    if ( ! function_exists( 'fs_request_is_action_secure' ) ) {
        ...
        function fs_request_is_action_secure(
            ...
            if ( empty( $nonce ) ||
                 ( false === mcms_verify_nonce( $nonce, $action ) )
            ) {
                return false;
            }
    ...
    if ( ! function_exists( 'fs_is_module_page' ) ) {
        function fs_is_module_page( $page_slug ) {
    ...
    if ( ! function_exists( 'fs_get_raw_referer' ) ) {
         ...
         *
         * Do not use for redirects, use {@see mcms_get_referer()} instead.
         *
         * @since 1.2.3
         *
        ...
        function fs_get_raw_referer() {
            if ( function_exists( 'mcms_get_raw_referer' ) ) {
                return mcms_get_raw_referer();
            }
    ...
    function fs_ui_get_action_button(
        ...
        if ( is_string( $confirmation ) ) {
            return sprintf( '<form action="%s" method="%s"><input type="hidden" name="fs_action" value="%s">%s<a href="#" class="%s" onclick="if (confirm(\'%s\')) this.parentNode.submit(); return false;">%s</a></form>',
                ...
                $method,
                $action,
                mcms_nonce_field( $action, '_mcmsnonce', true, false ),
            ...
            return sprintf( '<a href="%s" class="%s">%s</a></form>',
                mcms_nonce_url( freemius( $module_id )->_get_admin_page_url( $page, array_merge( $params, array( 'fs_action' => $action ) ) ), $action ),
                'button' . ( ! empty( $button_class ) ? ' ' . $button_class : '' ) . ( $is_primary ? ' button-primary' : '' ) . ( $is_small ? ' button-small' : '' ),
                $title
            );
        }
    ...
    function fs_ui_action_link( $module_id, $page, $action, $title, $params = array() ) {
        ?><a class=""
             href="<?php echo mcms_nonce_url( freemius( $module_id )->_get_admin_page_url( $page, array_merge( $params, array( 'fs_action' => $action ) ) ), $action ) ?>"><?php echo $title ?></a><?php
    }
...
//		}
    ...
    if ( ! function_exists( 'fs_nonce_url' ) ) {
         ...
         *
         * Originally was using `mcms_nonce_url()` but the new version
         ...
         */
        function fs_nonce_url( $actionurl, $action = - 1, $name = '_mcmsnonce' ) {
            return add_query_arg( $name, mcms_create_nonce( $action ), $actionurl );
        }
    }
...
//		}
     ...
     * @author Vova Feldman (@svovaf)
     *
     * @since  1.2.2 Changed to usage of MCMS_Filesystem_Direct.
     *
    ...
    function fs_download_image( $from, $to ) {
        ...
        if ( ! class_exists( 'MCMS_Filesystem_Direct' ) ) {
            require_once BASED_TREE_URI . 'mcms-admin/includes/class-mcms-filesystem-base.php';
            require_once BASED_TREE_URI . 'mcms-admin/includes/class-mcms-filesystem-direct.php';
        }

        $fs      = new MCMS_Filesystem_Direct( '' );
        $tmpfile = download_url( $from );

        if ( $tmpfile instanceof MCMS_Error ) {
    ...
    if ( ! function_exists( 'fs_text' ) ) {
        ...
        function _fs_text_inline( $text, $key = '', $slug = 'freemius' ) {
            ...
            // Avoid misleading mySkin Check warning.
            $fn = 'translate';
    ...
    if ( ! function_exists( 'fs_is_module_uninstall' ) ) {
        /**
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
         */
        function fs_is_module_uninstall() {
            return (
                defined( 'MCMS_UNINSTALL_PLUGIN' ) ||
                ( 0 < did_action( 'update_option_uninstall_modules' ) )
            );
        }
    }
    ...
    if ( ! function_exists( 'fs_is_network_admin' ) ) {
         ...
         * the context execution is MCMS AJAX handler, and during module
         * uninstall.
         *
         * @author Vova Feldman (@svovaf)
         * @since  2.0.0
        ...
        function fs_is_network_admin() {
            return (
                MCMS_FS__IS_NETWORK_ADMIN ||
                ( is_multisite() && fs_is_module_uninstall() )
            );
        }
    }
    ...
    if ( ! function_exists( 'fs_apply_filter' ) ) {
        /**
         * Apply filter for specific module.
         *
         * @author Vova Feldman (@svovaf)
         * @since  1.0.9
         *
