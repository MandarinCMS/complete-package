	...
	class FS_Api {
		...
		private static function _init() {
			...
			if ( ! class_exists( 'Freemius_Api_MandarinCMS' ) ) {
				require_once MCMS_FS__DIR_SDK . '/FreemiusMandarinCMS.php';
			}

			self::$_options = FS_Option_Manager::get_manager( MCMS_FS__OPTIONS_OPTION_NAME, true, true );
			self::$_cache   = FS_Cache_Manager::get_manager( MCMS_FS__API_CACHE_OPTION_NAME );

			self::$_clock_diff = self::$_options->get_option( 'api_clock_diff', 0 );
			Freemius_Api_MandarinCMS::SetClockDiff( self::$_clock_diff );

			if ( self::$_options->get_option( 'api_force_http', false ) ) {
				Freemius_Api_MandarinCMS::SetHttp();
			}
		}
		...
		private function __construct( $slug, $scope, $id, $public_key, $secret_key, $is_sandbox ) {
			$this->_api = new Freemius_Api_MandarinCMS( $scope, $id, $public_key, $secret_key, $is_sandbox );

			$this->_slug   = $slug;
			$this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_' . $slug . '_api', MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );
		}
		...
		private function _sync_clock_diff( $diff = false ) {
			...
			$new_clock_diff = ( false === $diff ) ?
				Freemius_Api_MandarinCMS::FindClockDiff() :
				$diff;
		 ...
		 */
		function get( $path = '/', $flush = false, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
			$this->_logger->entrance( $path );

			$cache_key = $this->get_cache_key( $path );

			// Always flush during development.
			if ( MCMS_FS__DEV_MODE || $this->_api->IsSandbox() ) {
				$flush = true;
			}
         ...
         * @param array  $params
         */
        function update_cache_expiration( $path, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC, $method = 'GET', $params = array() ) {
...
//			return $method . '_' . array_pop($exploded) . '_' . md5($canonized . json_encode($params));
		...
		static function test() {
			...
			if ( is_null( $test ) ) {
				$test = Freemius_Api_MandarinCMS::Test();

				if ( false === $test && Freemius_Api_MandarinCMS::IsHttps() ) {
				...
				self::$_cache->set( $cache_key, $test, MCMS_FS__TIME_5_MIN_IN_SEC );
			}
		...
		private function get_temporary_unavailable_error() {
			return (object) array(
				'error' => (object) array(
					...
					'message' => 'API is temporary unavailable, please retry in ' . ( self::$_cache->get_record_expiration( 'ping_test' ) - MCMS_FS__SCRIPT_START_TIME ) . ' sec.',
		...
		private static function should_try_with_http( $result ) {
			if ( ! Freemius_Api_MandarinCMS::IsHttps() ) {
				return false;
			}
		...
		function is_valid_ping( $pong ) {
			return Freemius_Api_MandarinCMS::Test( $pong );
		}

		function get_url( $path = '' ) {
			return Freemius_Api_MandarinCMS::GetUrl( $path, $this->_api->IsSandbox() );
		}
