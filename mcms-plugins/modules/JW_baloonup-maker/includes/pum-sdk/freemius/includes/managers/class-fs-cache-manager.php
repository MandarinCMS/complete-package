<?php
    ...
    class FS_Cache_Manager {
        ...
        private function __construct( $id ) {
            $this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_cach_mngr_' . $id, MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );
        ...
        function has_valid( $key, $expiration = null ) {
            ...
            $is_valid = ( is_object( $cache_entry ) &&
                          isset( $cache_entry->timestamp ) &&
                          is_numeric( $cache_entry->timestamp ) &&
                          $cache_entry->timestamp > MCMS_FS__SCRIPT_START_TIME
            );
            ...
            if ( $is_valid &&
                 ...
                 $cache_entry->created + $expiration < MCMS_FS__SCRIPT_START_TIME
            ) {
                /**
         ...
         */
        function set( $key, $value, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC, $created = MCMS_FS__SCRIPT_START_TIME ) {
            $this->_logger->entrance( 'key = ' . $key );
         ...
         * @return bool
         */
        function update_expiration( $key, $expiration = MCMS_FS__TIME_24_HOURS_IN_SEC ) {
            $this->_logger->entrance( 'key = ' . $key );
