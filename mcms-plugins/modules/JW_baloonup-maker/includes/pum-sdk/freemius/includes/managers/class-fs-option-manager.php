<?php
     ...
     *      layer 3: Memory
     *      layer 2: Cache (if there's any caching module and if MCMS_FS__DEBUG_SDK is FALSE)
    ...
    class FS_Option_Manager {
        ...
        private function __construct( $id, $load = false, $network_level_or_blog_id = false ) {
            ...
            $this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_opt_mngr_' . $id, MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );
        ...
        function load( $flush = false ) {
            ...
            if ( $flush || ! isset( $this->_options ) ) {
                ...
                if ( ! MCMS_FS__DEBUG_SDK ) {

                    $this->_options = mcms_cache_get(
                        $option_name,
                        $cache_group
                    );
                }
...
//					$this->_logger->info('get_option = ' . var_export($this->_options, true));
                ...
                if ( ! MCMS_FS__DEBUG_SDK && ! $cached ) {
                    // Set non encoded cache.
                    mcms_cache_set( $option_name, $this->_options, $cache_group );
                }
            }
        }
        ...
        function store() {
            ...
            if ( ! MCMS_FS__DEBUG_SDK ) {
                mcms_cache_set( $option_name, $this->_options, $this->get_cache_group() );
            }
        }
