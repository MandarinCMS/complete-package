<?php
	...
	class FS_Admin_Menu_Manager {
		...
		protected function __construct( $module_id, $module_type, $module_unique_affix ) {
			$this->_logger = FS_Logger::get_logger( MCMS_FS__SLUG . '_' . $module_id . '_admin_menu', MCMS_FS__DEBUG_SDK, MCMS_FS__ECHO_DEBUG_SDK );
...
//					) );
		...
		/**
		 * Check if module's menu item is part of a custom top level menu.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.1.3
		 *
		 ...
		 * If module's menu slug is a file (e.g. CPT), uses module's slug as the menu slug.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.1.3
		 *
		...
		/**
		 * Get module's original menu slug.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.1.3
		 *
		...
		/**
		 * Is user on module's admin activation page.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.0.8
		 *
		...
		function is_main_settings_page() {
			if ( $this->_menu_exists &&
			     ( fs_is_module_page( $this->_menu_slug ) || fs_is_module_page( $this->_module_unique_affix ) )
			) {
				/**
			...
			global $pagenow;
			if ( ( MCMS_FS__MODULE_TYPE_THEME === $this->_module_type ) && Freemius::is_myskins_page() ) {
				/**
		...
		/**
		 * Find module's admin dashboard main menu item.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.0.2
		 *
		...
		private function find_top_level_menu() {
			...
			$hook_name = get_module_page_hookname( $menu_slug, '' );
		...
		/**
		 * Find module's admin dashboard main submenu item.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.2.1.6
		 *
		...
		private function find_main_submenu() {
			...
			$hook_name = get_module_page_hookname( $submenu_slug, '' );
		 ...
		 * @since  1.0.7
		 *
		 * @return bool If submenu with module's menu slug was found.
		 */
		private function remove_all_submenu_items() {
			...
			/**
			 * This method is NOT executed for MandarinCMS.org myskins.
			 * Since we maintain only one version of the SDK we added this small
			 * hack to avoid the error from mySkin Check since it's a false-positive.
			 *
			 * @author Vova Feldman (@svovaf)
			 * @since  1.2.2.7
			 */
		 ...
		 *
		 * @todo This method was only tested for mcms.org compliant myskins with a submenu item. Need to test for modules with top level, submenu, and CPT top level, menu items.
		 *
		 * @author Vova Feldman (@svovaf)
		 * @since  1.2.2.7
		 *
		...
		function override_menu_item( $function ) {
			...
			if ( ! $this->is_top_level() || ! $this->is_cpt() ) {
				$menu_slug = module_basename( $this->get_slug() );
		...
		function add_counter_to_menu_item( $counter = 1, $class = '' ) {
			...
			$mask = '%s <span class="update-modules %s count-%3$s" aria-hidden="true"><span>%3$s<span class="screen-reader-text">%3$s notifications</span></span></span>';
		 ...
		 *
		 *  This is a replication of `add_menu_page()` to avoid mySkin Check warning.
		 *
		 *  Why?
		 *  ====
		 ...
		 *
		 *  This method will not run for mcms.org myskins (only modules) since myskin
		 ...
		 *                                    * Pass 'none' to leave div.mcms-menu-image empty so an icon can be added
		 *                                    via CSS.
		 ...
		 * Note for MandarinCMS.org mySkin/Module reviewer:
		 *
		 *  This is a replication of `add_submenu_page()` to avoid mySkin Check warning.
		 *
		 *  Why?
		 *  ====
		 *  Freemius is an SDK for module and myskin developers. Since the core
