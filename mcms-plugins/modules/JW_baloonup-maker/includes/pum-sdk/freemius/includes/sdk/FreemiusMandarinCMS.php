<?php
	...
	if ( class_exists( 'Freemius_Api_MandarinCMS' ) ) {
		return;
	}

	class Freemius_Api_MandarinCMS extends Freemius_Api_Base {
		 ...
		 * @param array  $pMCMSRemoteArgs
		 *
		 * @return array
		 */
		function SignRequest( $pResourceUrl, $pMCMSRemoteArgs ) {
			$auth = $this->GenerateAuthorizationParams(
				$pResourceUrl,
				$pMCMSRemoteArgs['method'],
				! empty( $pMCMSRemoteArgs['body'] ) ? $pMCMSRemoteArgs['body'] : ''
			);

			$pMCMSRemoteArgs['headers']['Date']          = $auth['date'];
			$pMCMSRemoteArgs['headers']['Authorization'] = $auth['authorization'];

			if ( ! empty( $auth['content_md5'] ) ) {
				$pMCMSRemoteArgs['headers']['Content-MD5'] = $auth['content_md5'];
			}

			return $pMCMSRemoteArgs;
		}
        ...
        function GetSignedUrl( $pPath ) {
            ...
            return Freemius_Api_MandarinCMS::GetUrl(
                $pResourceUrl . '?' .
		 ...
		 * @return mixed
		 */
		private static function ExecuteRequest( $pUrl, &$pMCMSRemoteArgs ) {
			$start = microtime( true );

			$response = mcms_remote_request( $pUrl, $pMCMSRemoteArgs );

			if ( FS_API__LOGGER_ON ) {
				$end = microtime( true );

				$has_body      = ( isset( $pMCMSRemoteArgs['body'] ) && ! empty( $pMCMSRemoteArgs['body'] ) );
				$is_http_error = is_mcms_error( $response );
				...
				self::$_logger[] = array(
					...
					'end'       => $end,
					'total'     => ( $end - $start ),
					'method'    => $pMCMSRemoteArgs['method'],
					'path'      => $pUrl,
					'body'      => $has_body ? $pMCMSRemoteArgs['body'] : null,
		 ...
		 * @param string        $pMethod
		 * @param array         $pParams
		 * @param null|array    $pMCMSRemoteArgs
		...
		private static function MakeStaticRequest(
			...
			if ( empty( $pMCMSRemoteArgs ) ) {
				$user_agent = 'Freemius/MandarinCMS-SDK/' . Freemius_Api_Base::VERSION . '; ' .
				              home_url();

				$pMCMSRemoteArgs = array(
			...
			if ( ! isset( $pMCMSRemoteArgs['headers'] ) ||
			     ! is_array( $pMCMSRemoteArgs['headers'] )
			) {
				$pMCMSRemoteArgs['headers'] = array();
			}

			if ( in_array( $pMethod, array( 'POST', 'PUT' ) ) ) {
				if ( is_array( $pParams ) && 0 < count( $pParams ) ) {
					$pMCMSRemoteArgs['headers']['Content-type'] = 'application/json';
					$pMCMSRemoteArgs['body']                    = json_encode( $pParams );
				}
			}
            ...
            if ( FS_SDK__HAS_CURL ) {
                ...
                $pMCMSRemoteArgs['headers']['Expect'] = '';
            }

			if ( 'https' === substr( strtolower( $request_url ), 0, 5 ) ) {
				$pMCMSRemoteArgs['sslverify'] = false;
			}

			if ( false !== $pBeforeExecutionFunction &&
			     is_callable( $pBeforeExecutionFunction )
			) {
				$pMCMSRemoteArgs = call_user_func( $pBeforeExecutionFunction, $resource[0], $pMCMSRemoteArgs );
			}

			$result = self::ExecuteRequest( $request_url, $pMCMSRemoteArgs );

			if ( is_mcms_error( $result ) ) {
				/**
				 * @var MCMS_Error $result
				 */
...
//						    error_log('Invalid IPv6 configuration on server, Please disable or get native IPv6 on your server.');
								...
								add_action( 'http_api_curl', 'Freemius_Api_MandarinCMS::CurlResolveToIPv4', 10, 1 );
				...
				if ( is_mcms_error( $result ) ) {
					self::ThrowMCMSRemoteException( $result );
				}
			}
		 ...
		 * developers want to do fancier things or use something other than mcms_remote_request()
		 * to make the request.
		 *
		 * @param string     $pCanonizedPath The URL to make the request to
		 * @param string     $pMethod        HTTP method
		 * @param array      $pParams        The parameters to use for the POST body
		 * @param null|array $pMCMSRemoteArgs  mcms_remote_request options.
		 *
		 * @return object[]|object|null
		 *
		 * @throws Freemius_Exception
		...
		/**
		 * @param \MCMS_Error $pError
		 *
		 * @return bool
		 */
		private static function IsCurlError( MCMS_Error $pError ) {
		 ...
		 * @throws Freemius_Exception
		 */
		private static function ThrowMCMSRemoteException( MCMS_Error $pError ) {
