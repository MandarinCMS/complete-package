<?php
	 ...
	 */
    $slug      = $VARS['slug'];
    $module_id = $VARS['target_module_id'];
	...
	$title = $fs->get_module_title();

	if ( $module_id != $fs->get_id() ) {
		$addon = $fs->get_addon( $module_id );
	...
	// Pass unique auto installation URL if MCMS_Filesystem is needed.
	...
	<div class="fs-modal-dialog">
		...
		<div class="fs-modal-body">
			...
			<p class="fs-installing"
			   style="display: none"><?php echo sprintf( fs_esc_html_inline( 'The installation process has started and may take a few minutes to complete. Please wait until it is done - do not refresh this page.', 'installing-module-x', $slug ), $module_title ) ?></p>
		</div>
		...
		$(document).ready(function () {
			...
			var startAutoInstall = function () {
				...
				var data = {
                    ...
                    target_module_id: '<?php echo $module_id ?>'
				};
