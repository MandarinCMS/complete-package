...
<tr<?php if ( $odd ) {
    ...
    <?php if ( $is_addon_connected ) : ?>
        ...
        <!-- Version -->
        <td><?php echo $fs_addon->get_module_version() ?></td>
        <!--/ Version -->
        ...
        if ( $is_addon_activated ) {
            ...
            if ( $fs->is_addon_installed( $addon_id ) ) {
                ...
                $buttons[]  = sprintf(
                    ...
                    mcms_nonce_url( 'modules.php?action=activate&amp;module=' . $addon_file, 'activate-module_' . $addon_file ),
                ...
                if ( $fs->is_allowed_to_install() ) {
                    $buttons[] = sprintf(
                        ...
                        mcms_nonce_url( self_admin_url( 'update.php?action=install-module&module=' . $addon->slug ), 'install-module_' . $addon->slug ),
        ...
        if ( $show_upgrade ) {
            $buttons[] = sprintf( '<a href="%s" class="thickbox button button-small button-primary" aria-label="%s" data-title="%s"><i class="dashicons dashicons-cart"></i> %s</a>',
                esc_url( network_admin_url( 'module-install.php?tab=module-information&parent_module_id=' . $fs->get_id() . '&module=' . $addon->slug .
    ...
    <?php else : ?>
        ...
        <td colspan="4">
            <?php if ( $fs->is_addon_installed( $addon_id ) ) : ?>
                ...
                <a class="button button-primary"
                   href="<?php echo mcms_nonce_url( 'modules.php?action=activate&amp;module=' . $addon_file, 'activate-module_' . $addon_file ) ?>"
            ...
            <?php else : ?>
                <?php if ( $fs->is_allowed_to_install() ) : ?>
                    <a class="button button-primary"
                       href="<?php echo mcms_nonce_url( self_admin_url( 'update.php?action=install-module&module=' . $addon->slug ), 'install-module_' . $addon->slug ) ?>"><?php fs_esc_html_echo_inline( 'Install Now', 'install-now', $slug ) ?></a>
                <?php else : ?>
        ...
        <!--/ Action -->
    <?php endif ?>
    <?php if ( ! $is_paying && MCMS_FS__DEV_MODE ) : ?>
