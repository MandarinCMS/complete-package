<?php
     ...
     * @var array             $VARS
     * @var Freemius          $fs
     * @var FS_Module_License $main_license
     */
    $fs            = $VARS['freemius'];
    $slug          = $fs->get_slug();
    $site          = $VARS['site'];
    ...
    <tr class="fs-site-details" data-blog-id="<?php echo $blog_id ?>"<?php if ( $is_registered ) : ?> data-install-id="<?php echo $install->id ?>"<?php endif ?>>
        ...
        <td><?php if ( $is_registered ) : ?>
            ...
            <?php else : ?>
                ...
                <form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
                    ...
                    <?php mcms_nonce_field( trim( "{$action}:{$blog_id}", ':' ) ) ?>
        ...
        <td><?php if ( $has_paid_plan ) {
                ...
                if ( $is_registered ) {
                    ...
                    $has_license = FS_Module_License::is_valid_id( $install->license_id );
                    $license     = $has_license ?
        ...
        <td colspan="5">
            <table class="widefat fs-key-value-table">
                ...
                <tr <?php if ( 1 == $row_index % 2 ) {
                    ...
                    <td><?php if ( ! FS_Module_License::is_valid_id( $install->license_id ) ) : ?>
                        <!-- Toggle Usage Tracking -->
                        <?php $action = 'toggle_tracking' ?>
                        <form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
                            <input type="hidden" name="fs_action" value="<?php echo $action ?>">
                            <?php mcms_nonce_field( trim( "{$action}:{$blog_id}:{$install->id}", ':' ) ) ?>
