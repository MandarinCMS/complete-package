...
?>
<h1><?php echo fs_text_inline( 'Freemius Debug' ) . ' - ' . fs_text_inline( 'SDK' ) . ' v.' . $fs_active_modules->newest->version ?></h1>
<div>
    <!-- Debugging Switch -->
    ...
    <div class="switch <?php echo MCMS_FS__DEBUG_SDK ? 'off' : 'on' ?>">
    ...
    <tr>
        <td>
            ...
            <form action="" method="POST">
                ...
                <?php mcms_nonce_field( 'restart_freemius' ) ?>
        ...
        <td>
            ...
            <form action="" method="POST">
                ...
                <?php mcms_nonce_field( 'clear_updates_data' ) ?>
...
<?php
    ...
    $defines = array(
        array(
            'key' => 'MCMS_FS__REMOTE_ADDR',
            'val' => MCMS_FS__REMOTE_ADDR,
        ),
        array(
            'key' => 'MCMS_FS__ADDRESS_PRODUCTION',
            'val' => MCMS_FS__ADDRESS_PRODUCTION,
        ),
        array(
            'key' => 'FS_API__ADDRESS',
            'val' => FS_API__ADDRESS,
...
<table id="fs_sdks" class="widefat">
    ...
    <tbody>
    <?php foreach ( $fs_active_modules->modules as $sdk_path => &$data ) : ?>
        <?php $is_active = ( MCMS_FS__SDK_VERSION == $data->version ) ?>
        <tr<?php if ( $is_active ) {
            echo ' style="background: #E6FFE6; font-weight: bold"';
        } ?>>
            <td><?php echo $data->version ?></td>
            <td><?php echo $sdk_path ?></td>
            <td><?php echo $data->module_path ?></td>
...
<?php
    $module_types = array(
        MCMS_FS__MODULE_TYPE_PLUGIN,
        MCMS_FS__MODULE_TYPE_THEME
    );
?>
...
<?php foreach ( $module_types as $module_type ) : ?>
    ...
    <?php if ( is_array( $modules ) && count( $modules ) > 0 ) : ?>
        <h2><?php echo esc_html( ( MCMS_FS__MODULE_TYPE_PLUGIN == $module_type ) ? fs_text_inline( 'Modules', 'modules' ) : fs_text_inline( 'mySkins', 'myskins' ) ) ?></h2>
        <table id="fs_<?php echo $module_type ?>" class="widefat">
            ...
            <?php foreach ( $modules as $slug => $data ) : ?>
                <?php
                if ( MCMS_FS__MODULE_TYPE_THEME !== $module_type ) {
                    $is_active = is_module_active( $data->file );
                } else {
                    $current_myskin = mcms_get_myskin();
                    $is_active     = ( $current_myskin->stylesheet === $data->file );

                    if ( ! $is_active && is_child_myskin() ) {
                        $parent_myskin = $current_myskin->parent();

                        $is_active = ( ( $parent_myskin instanceof MCMS_mySkin ) && $parent_myskin->stylesheet === $data->file );
                    }
                }
                ?>
                        ...
                        <?php if ( $is_active ) : ?>
                            <?php if ( $fs->has_trial_plan() ) : ?>
                                <form action="" method="POST">
                                    ...
                                    <?php mcms_nonce_field( 'simulate_trial' ) ?>
                            ...
                            <?php if ( fs_is_network_admin() && ! $fs->is_network_upgrade_mode() ) : ?>
                                <form action="" method="POST">
                                    ...
                                    <input type="hidden" name="module_id" value="<?php echo $fs->get_id() ?>">
                                    <?php mcms_nonce_field( 'simulate_network_upgrade' ) ?>
...
<?php foreach ( $module_types as $module_type ) : ?>
    ...
    <?php if ( is_array( $sites_map ) && count( $sites_map ) > 0 ) : ?>
        <h2><?php echo esc_html( sprintf(
            /* translators: %s: 'module' or 'myskin' */
                fs_text_inline( '%s Installs', 'module-installs' ),
                ( MCMS_FS__MODULE_TYPE_PLUGIN === $module_type ? fs_text_inline( 'Module', 'module' ) : fs_text_inline( 'mySkin', 'myskin' ) )
            ...
            <?php foreach ( $sites_map as $slug => $sites ) : ?>
                ...
                <?php foreach ( $sites as $site ) : ?>
                    <tr>
                        ...
                        <td><?php
                                $plan_name = '';
                                if ( FS_Module_Plan::is_valid_id( $site->plan_id ) ) {
                                    if ( false === $all_plans ) {
                                        $option_name = 'plans';
                        ...
                        <td>
                            <form action="" method="POST">
                                ...
                                <?php mcms_nonce_field( 'delete_install' ) ?>
                                <input type="hidden" name="module_id" value="<?php echo $site->module_id ?>">
                                <?php if ( $is_multisite ) : ?>
...
<?php
    $addons = $VARS['addons'];
?>
<?php foreach ( $addons as $module_id => $module_addons ) : ?>
    <h2><?php echo esc_html( sprintf( fs_text_inline( 'Add Ons of module %s', 'addons-of-x' ), $module_id ) ) ?></h2>
            ...
            <tr>
                ...
                <td>
                    <form action="" method="POST">
                        ...
                        <?php mcms_nonce_field( 'delete_user' ) ?>
...
<?php foreach ( $module_types as $module_type ) : ?>
    ...
    <?php if ( is_array( $licenses ) && count( $licenses ) > 0 ) : ?>
        <h2><?php echo esc_html( sprintf( fs_text_inline( '%s Licenses', 'module-licenses' ), ( MCMS_FS__MODULE_TYPE_PLUGIN === $module_type ? fs_text_inline( 'Module', 'module' ) : fs_text_inline( 'mySkin', 'myskin' ) ) ) ) ?></h2>
        <table id="fs_<?php echo $module_type ?>_licenses" class="widefat">
            <thead>
            <tr>
                <th><?php fs_esc_html_echo_inline( 'ID', 'id' ) ?></th>
                <th><?php fs_esc_html_echo_inline( 'Module ID' ) ?></th>
                <th><?php fs_esc_html_echo_inline( 'User ID' ) ?></th>
            ...
            <?php foreach ( $licenses as $license ) : ?>
                <tr>
                    ...
                    <td><?php echo $license->module_id ?></td>
...
<?php if ( FS_Logger::is_storage_logging_on() ) : ?>
    ...
    <div id="fs_debug_filters">
        ...
        <select name="request_type">
            ...
            <option value="cron">MCMS Cron</option>
        </select>
        ...
        <div style="margin: 10px 0">
            ...
            <form action="" method="POST" style="float: left; margin-left: 10px;">
                ...
                <?php mcms_nonce_field( 'download_logs' ) ?>
