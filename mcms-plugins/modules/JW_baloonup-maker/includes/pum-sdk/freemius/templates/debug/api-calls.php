<?php
	...
	if ( class_exists( 'Freemius_Api_MandarinCMS' ) ) {
		$logger = Freemius_Api_MandarinCMS::GetLogger();
	} else {
		$logger = array();
	}
	 ...
	 * limitations of the current mySkin Check, it throws an error
	...
	$root_path_len = strlen( BASED_TREE_URI );
...
<table class="widefat">
	...
	<?php foreach ( $logger as $log ) : ?>
		<tr>
			...
			<td><?php echo number_format( 100 * ( $log['start'] - MCMS_FS__SCRIPT_START_TIME ), 2 ) . ' ' . $ms_text ?></td>
			<td><?php echo number_format( 100 * ( $log['end'] - MCMS_FS__SCRIPT_START_TIME ), 2 ) . ' ' . $ms_text ?></td>
		</tr>
	<?php endforeach ?>
	</tbody>
</table>