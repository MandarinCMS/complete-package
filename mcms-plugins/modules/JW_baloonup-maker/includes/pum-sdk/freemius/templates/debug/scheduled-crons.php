			...
			<tr>
				...
				<td><?php
						if ( is_numeric( $cron['last'] ) ) {
							$diff       = abs( MCMS_FS__SCRIPT_START_TIME - $cron['last'] );
							$human_diff = ( $diff < MINUTE_IN_SECONDS ) ?
								$diff . ' ' . $sec_text :
								human_time_diff( MCMS_FS__SCRIPT_START_TIME, $cron['last'] );

							echo esc_html( sprintf(
								( ( MCMS_FS__SCRIPT_START_TIME < $cron['last'] ) ?
									$in_x_text :
									$x_ago_text ),
								$human_diff
							) );
				...
				<td><?php
						if ( is_numeric( $cron['next'] ) ) {
							$diff       = abs( MCMS_FS__SCRIPT_START_TIME - $cron['next'] );
							$human_diff = ( $diff < MINUTE_IN_SECONDS ) ?
								$diff . ' ' . $sec_text :
								human_time_diff( MCMS_FS__SCRIPT_START_TIME, $cron['next'] );

							echo esc_html( sprintf(
								( ( MCMS_FS__SCRIPT_START_TIME < $cron['next'] ) ?
									$in_x_text :
									$x_ago_text ),
								$human_diff
							) );
