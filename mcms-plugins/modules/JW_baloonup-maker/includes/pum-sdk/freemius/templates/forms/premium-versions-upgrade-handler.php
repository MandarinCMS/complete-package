<?php
    ...
    $module_data     = $fs->get_module_data();
    $module_name     = $module_data['Name'];
    $module_basename = $fs->get_module_basename();
    ...
    $( document ).ready(function() {
        ...
        var modalContentHtml = <?php echo json_encode( $modal_content_html ) ?>,
            modalHtml        =
                ...
                + '</div>',
            $modal           = $( modalHtml ),
            isModulesPage    = <?php echo Freemius::is_modules_page() ? 'true' : 'false' ?>;
        ...
        function registerEventHandlers() {
            ...
            $( 'body' ).on( 'click', '[id*="select-all"]', function( evt ) {
                ...
                if ( 0 === $modulesWithNonActiveLicense.length ) {
                     ...
                     * ".license-expired" class if for example only the myskins table has such checkboxes and the user
                     * clicks on a "Select All" checkbox on the modules table which has no such checkboxes.
                     *
                ...
                /**
                 * Prevent the default MandarinCMS handler from checking all checkboxes.
                 *
                 * @author Leo Fajardo (@leorw)
                 */
                evt.stopImmediatePropagation();
        ...
        function showModal( $module ) {
            $modal.find( '#module_name' ).text( $module.data( 'module-name' ) );
