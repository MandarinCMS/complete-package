<?php
	...
	$reconnect_url = $fs->get_activation_url( array(
		'nonce'     => mcms_create_nonce( $fs->get_unique_affix() . '_reconnect' ),
		'fs_action' => ( $fs->get_unique_affix() . '_reconnect' ),
	) );

	$module_title                   = "<strong>{$fs->get_module()->title}</strong>";
	...
	$opt_out_message_usage_tracking = sprintf( fs_text_inline( "Usage tracking is done in the name of making %s better. Making a better user experience, prioritizing new features, and more good things. We'd really appreciate if you'll reconsider letting us continue with the tracking.", 'opt-out-message-usage-tracking', $slug ), $module_title );
	...
	(function( $ ) {
		$( document ).ready(function() {
			...
			function showOptInAppreciationMessageAndScrollToTop() {
				$adminNotice.insertAfter( $( '#mcmsbody-content' ).find( ' > .wrap > h1' ) );
				window.scrollTo(0, 0);
			}
			...
			<?php if ( $fs->is_myskin() ) : ?>
			/**
			 * Add opt-in/out button to the active myskin's buttons collection
			 * in the myskin's extended details overlay.
			 *
			 * @author Vova Feldman (@svovaf)
			 * @since 1.2.2.7
			 */
			$('.myskin-overlay').contentChange(function () {
				if (!$(this).find('.myskin-overlay').hasClass('active')) {
					// Add opt-in/out button only to the currently active myskin.
					return;
				}

				if ($('#fs_myskin_opt_in_out').length > 0){
				...
				$actionLink = $('<a id="fs_myskin_opt_in_out" href="' + encodeURI(href) + '" class="button" data-action="' + action + '">' + label + '</a>');

				$('.myskin-wrap .myskin-actions .active-myskin').append($actionLink);
