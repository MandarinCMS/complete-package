...
HTML;
	...
	$myskin_text      = fs_text_inline( 'mySkin', 'myskin', $slug );
...
(function ($) {
	var reasonsHtml = <?php echo json_encode( $reasons_list_items_html ); ?>,
        ...
        deletemySkinUpdateData = <?php echo $fs->is_myskin() && $fs->is_premium() && ! $fs->has_active_valid_license() ? 'true' : 'false' ?>;
	...
	function registerEventHandlers() {
		...
		<?php
		/**
		 * For "myskin" module type, the modal is shown when the current user clicks on
		 * the "Activate" button of any other myskin. The "Activate" button is actually
		 * a link to the "mySkins" page (/mcms-admin/myskins.php) containing query params
		 * that tell MandarinCMS to deactivate the current myskin and activate a different myskin.
		 *
		 * @author Leo Fajardo (@leorw)
		 * @since 1.2.2
		 *        
		 * @since 1.2.2.7 Don't trigger the deactivation feedback form if activating the premium version of the myskin.
		 */
		} else { ?>
		$('body').on('click', '.myskin-browser .myskin:not([data-slug=<?php echo $slug ?>-premium]) .myskin-actions .button.activate', function (evt) {
			evt.preventDefault();
		...
		$modal.on('click', '.fs-modal-footer .button', function (evt) {
			...
			if (_this.hasClass('allow-deactivate')) {
				...
				if (0 === $radio.length) {
				    if ( ! deletemySkinUpdateData ) {
                        // If no selected reason, just deactivate the module.
                        ...
                        $.ajax({
                            ...
                            method    : 'POST',
                            data      : {
                                action   : '<?php echo $fs->get_ajax_action( 'delete_myskin_update_data' ) ?>',
                                security : '<?php echo $fs->get_ajax_security( 'delete_myskin_update_data' ) ?>',
				...
				$.ajax({
					...
					complete  : function () {
						// Do not show the dialog box, deactivate the module.
						window.location.href = redirectLink;
					}
				});
		...
		$modal.on('click', 'input[type="radio"]', function () {
			...
			$modal.find('.button-deactivate').html('<?php echo esc_js( sprintf(
				...
				$fs->is_module() ?
					$deactivate_text :
					sprintf( $activate_x_text, $myskin_text )
			) ) ?>');
	...
	function resetModal() {
		 ...
		 * that clicking the deactivate button will actually deactivate the module.
		 */
