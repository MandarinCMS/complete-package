<?php
	...
	foreach ( $plans as $plan ) {
		...
		if ( ! empty( $plan->support_email ) ||
			...
			if ( ! isset( $features_plan_map['support'] ) ) {
				...
				$support_feature->title = fs_text_inline( 'Support', $module->slug );
	...
	$updates_feature->title = fs_text_inline( 'Unlimited Updates', 'unlimited-updates', $module->slug );
				...
				<th>
					...
					<span class="fs-price"><?php
							if ( empty( $plan->pricing ) ) {
								fs_esc_html_echo_inline( 'Free', 'free', $module->slug );
							} else {
								foreach ( $plan->pricing as $pricing ) {
									...
									if ( 1 == $pricing->licenses ) {
										if ( $pricing->has_annual() ) {
											echo "\${$pricing->annual_price} / " . fs_esc_html_x_inline( 'year', 'as annual period', 'year', $module->slug );
										} else if ( $pricing->has_monthly() ) {
											echo "\${$pricing->monthly_price} / " . fs_esc_html_x_inline( 'mo', 'as monthly period', 'mo', $module->slug );
										} else {
