<?php
	...
	$current_user = Freemius::_get_current_mcms_user();
	...
	$freemius_site_url = $freemius_site_www . '/' . ( $fs->is_premium() ?
			...
			'mandarincms/usage-tracking/' . $fs->get_id() . "/{$slug}/" );
	...
	$is_optin_dialog = (
		$fs->is_myskin() &&
		$fs->is_myskins_page() &&
		( ! $fs->has_settings_menu() || $fs->is_free_mcms_org_myskin() )
	);

	if ( $is_optin_dialog ) {
		$show_close_button             = false;
		$previous_myskin_activation_url = '';

		if ( ! $is_premium_code ) {
			$show_close_button = true;
		} else if ( $is_premium_only ) {
			$previous_myskin_activation_url = $fs->get_previous_myskin_activation_url();
			$show_close_button             = ( ! empty( $previous_myskin_activation_url ) );
		}
	}
...
<?php
	if ( $is_optin_dialog ) { ?>
<div id="fs_myskin_connect_wrapper">
	<?php
	...
	<div id="fs_connect"
		...
		<div class="fs-visual">
			<b class="fs-site-icon"><i class="dashicons dashicons-mandarincms"></i></b>
			<i class="dashicons dashicons-plus fs-first"></i>
			<?php
				$vars = array( 'id' => $fs->get_id() );
				fs_require_once_template( 'module-icon.php', $vars );
			?>
			...
			<p><?php
					...
					if ( $is_pending_activation ) {
						...
						$message = $fs->apply_filters(
							...
							sprintf( fs_text_inline( 'Thanks for purchasing %s! To get started, please enter your license key:', 'thanks-for-purchasing', $slug ), '<b>' . $fs->get_module_name() . '</b>' ),
							$first_name,
							$fs->get_module_name()
						);
					} else {
						...
						if ( $fs->is_module_update() ) {
							// If Freemius was added on a module update, set different
							// opt-in message.
							$default_optin_message = $is_gdpr_required ?
						...
						$message = $fs->apply_filters(
							...
							sprintf(
								...
								'<b>' . esc_html( $fs->get_module_name() ) . '</b>',
					...
					if ( $is_network_upgrade_mode ) {
                        ...
                        if ($is_premium_code){
                            ...
                            $message .= '<br><br>' . sprintf( fs_text_inline( 'If you\'d like to use the %s on those sites, please enter your license key below and click the activation button.', 'connect_message_network_upgrade-premium-activate-license', $slug ), $is_premium_only ? $fs->get_module_label( true ) : sprintf(
                                /* translators: %s: module type (module, myskin, or add-on) */
		...
		<div class="fs-actions">
			...
			<?php if ( $activate_with_current_user ) : ?>
				<form action="" method="POST">
					...
					<?php mcms_nonce_field( 'activate_existing_' . $fs->get_public_key() ) ?>
					<button class="button button-primary" tabindex="1"
					        type="submit"><?php echo esc_html( $button_label ) ?></button>
				</form>
			<?php else : ?>
				<form method="post" action="<?php echo MCMS_FS__ADDRESS ?>/action/service/user/install/">
			...
			$permissions = array(
				...
				'site'    => array(
					...
					'desc'       => $fs->get_text_inline( 'Site URL, MCMS version, PHP info, modules & myskins', 'permissions-site_desc' ),
					'priority'   => 10,
				),
				'notices' => array(
				...
				'events'  => array(
					'icon-class' => 'dashicons dashicons-admin-modules',
...
<script type="text/javascript">
	(function ($) {
		...
		if ( $is_optin_dialog ) {
		if ( $show_close_button ) { ?>
		var $myskinConnectWrapper = $('#fs_myskin_connect_wrapper');

		$myskinConnectWrapper.find('button.close').on('click', function () {
			<?php if ( ! empty( $previous_myskin_activation_url ) ) { ?>
			location.href = '<?php echo html_entity_decode( $previous_myskin_activation_url ); ?>';
			<?php } else { ?>
			$myskinConnectWrapper.remove();
		...
		$form.on('submit', function () {
			...
			if ( ajaxOptin ) {
				if (!hasContextUser || isNetworkUpgradeMode) {
					...
					// Update secret key if premium only module.
		...
		$primaryCta.on('click', function () {
			...
			$(this).html('<?php echo esc_js( $is_pending_activation ?
				...
				fs_text_x_inline( 'Activating', 'as activating module', 'activating', $slug )
			) ?>...');
		});
