<?php
	...
	if ( FS_Module_Plan::is_valid_id( $plan_id ) ) {
	...
	if ( $module_id == $fs->get_id() ) {
		...
		if ( $fs->is_addon_activated( $module_id ) ) {
			$fs_addon   = Freemius::get_instance_by_id( $module_id );
	...
	if ( $fs->is_payments_sandbox() ) {
		// Append module secure token for sandbox mode authentication.
	...
	$return_url = $fs->_get_sync_license_url( $module_id );

	$can_user_install = (
		( $fs->is_module() && current_user_can( 'install_modules' ) ) ||
		( $fs->is_myskin() && current_user_can( 'install_myskins' ) )
	);

	$query_params = array_merge( $context_params, $_GET, array(
		// Current module version.
		'module_version' => $fs->get_module_version(),
		'sdk_version'    => MCMS_FS__SDK_VERSION,
