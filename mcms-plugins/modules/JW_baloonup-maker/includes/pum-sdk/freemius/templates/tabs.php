<?php
    ...
    $is_free_mcms_org_myskin = $fs->is_free_mcms_org_myskin();

    $tabs = array();
    foreach ( $menu_items as $priority => $items ) {
        foreach ( $items as $item ) {
            if ( ! $item['show_submenu'] ) {
                if ( ! $is_free_mcms_org_myskin || ! $fs->is_submenu_item_visible( $item['menu_slug'], true ) ) {
                    continue;
                }
            }
...
<script type="text/javascript">
    (function ($) {
        $(document).ready(function () {
            ...
            if (0 === $wrap.length) {
                $wrap = $('<div class="wrap">');
                $wrap.insertBefore($('#mcmsbody-content .clear'));
            }
            ...
            <?php $is_support_tab = ( 'mcms-support-forum' == $tab['slug'] ) ?>
