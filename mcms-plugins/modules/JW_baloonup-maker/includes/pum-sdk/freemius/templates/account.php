<?php
	...
	/**
	 * @var FS_Module_Tag $update
	 */
	$update = $fs->get_update( false, false, MCMS_FS__TIME_24_HOURS_IN_SEC / 24 );
	...
	if ( is_object($update) ) {
         ...
         * @since  2.0.0
         */
		$updater = FS_Module_Updater::instance( $fs );
								...
								<ul>
									<?php if ( ! $is_paying ) : ?>
										<li>
											<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
												...
												<?php mcms_nonce_field( 'delete_account' ) ?>
									...
									<?php if ( $is_paying ) : ?>
										...
										<li>
											<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
												...
												<?php mcms_nonce_field( 'deactivate_license' ) ?>
										...
										<?php if ( ! $license->is_lifetime() &&
											...
											<li>
												<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
													...
													<?php mcms_nonce_field( 'downgrade_account' ) ?>
													<a href="#"
									...
									<?php elseif ( $is_paid_trial ) : ?>
										<li>
											<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
												...
												<?php mcms_nonce_field( 'cancel_trial' ) ?>
												<a href="#"
									...
									<li>
										<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
											...
											<?php mcms_nonce_field( $fs->get_unique_affix() . '_sync_license' ) ?>
									...
									<?php $odd = true;
										foreach ( $profile as $p ) : ?>
											...
											if ( 'plan' === $p['id'] && ! $has_paid_plan ) {
												// If module don't have any paid plans, there's no reason
													...
													<td class="fs-right">
														<?php if ( 'email' === $p['id'] && ! $user->is_verified() ) : ?>
															<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
																...
																<?php mcms_nonce_field( 'verify_email' ) ?>
																	...
																	<?php if ( $is_paying || $fs->is_trial() ) : ?>
																		...
																		<?php elseif ( is_object( $update ) ) : ?>
																			...
																			<a class="button button-primary"
																			   href="<?php echo mcms_nonce_url( self_admin_url( "update.php?action=upgrade-{$module_type}&{$module_type}=" . $fs->get_module_basename() ), "upgrade-{$module_type}_" . $fs->get_module_basename() ) ?>"><?php echo fs_esc_html_inline( 'Install Update Now', 'install-update-now', $slug ) . ' [' . $update->version . ']' ?></a>
																		<?php endif ?>
																	<?php endif; ?>
																</div>
															<?php endif ?>
														...
														( is_string( $user->secret_key ) && in_array( $p['id'], array(
															...
															<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST"
																...
																<input type="hidden" name="fs_<?php echo $p['id'] ?>_<?php echo $fs->get_unique_affix() ?>"
																       value="">
																<?php mcms_nonce_field( 'update_' . $p['id'] ) ?>
																<input type="submit" class="button button-small"
								...
								<div class="">
									...
									<table id="fs_addons" class="widefat">
										...
										<tr>
											...
											<th></th>
											<?php if ( defined( 'MCMS_FS__DEV_MODE' ) && MCMS_FS__DEV_MODE ) : ?>
												<th></th>
											<?php endif ?>
										</tr>
										</thead>
