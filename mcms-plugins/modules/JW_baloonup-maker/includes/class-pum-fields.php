	...
	*/
	...
	public function multicheck_callback( $args, $values = array() ) {
		...
		if ( ! empty( $args['options'] ) ) {
			foreach ( $args['options'] as $key => $option ) {
				...
				$option = mcms_parse_args( $option, array(
					'label' => '',
					'required' => false,
					'disabled' => false,
				) );
	...
	public function objectselect_callback( $args, $value = null ) {
		...
		if ( $args['multiple'] ) {
			...
			$args['name'] .= $args['as_array'] ? '[]' : '';
			$value = ! is_array( $value ) ? array( $value ) : $value;
			$value = mcms_parse_id_list( $value );
		}
    ...
    public function objectselect_sanitize( $value = array(), $args = array() ) {
        return mcms_parse_id_list( $value );
    }
...
* if ( $mcms_version >= 3.3 && function_exists( 'mcms_editor' ) ) {
			* ob_start();
			* mcms_editor( stripslashes( $value ), $this->options_key . '_' . $args['id'], array(
