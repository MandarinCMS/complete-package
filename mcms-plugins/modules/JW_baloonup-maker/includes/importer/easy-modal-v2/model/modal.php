...
if ( ! function_exists( "get_all_modals" ) ) {
	function get_all_modals( $where = "is_trash != 1" ) {
		...
		foreach ( $EModal_Model_Modal->load( "SELECT * FROM {$mcmsdb->prefix}em_modals" . ( $where ? " WHERE " . $where : '' ) ) as $modal ) {
			$modals[ $modal->id ] = $modal;
			$modal_ids[]          = $modal->id;
		}
		if ( count( $modals ) ) {
			foreach ( $EModal_Model_Modal_Meta->load( "SELECT * FROM {$mcmsdb->prefix}em_modal_metas WHERE modal_id IN (" . implode( ',', $modal_ids ) . ")" ) as $meta ) {
...
if ( ! function_exists( "count_all_modals" ) ) {
	function count_all_modals() {
		...
		return (int) $mcmsdb->get_var( "SELECT COUNT(*) FROM {$mcmsdb->prefix}em_modals WHERE is_trash = 0" );
	}
}
...
if ( ! function_exists( "count_deleted_modals" ) ) {
	function count_deleted_modals() {
		...
		return (int) $mcmsdb->get_var( "SELECT COUNT(*) FROM {$mcmsdb->prefix}em_modals WHERE is_trash = 1" );
	}
}