...
class EModal_Model {
	...
	public function __construct( $id = null, $limit = 1 ) {
		...
		if ( $id && is_numeric( $id ) ) {
			$row = $mcmsdb->get_row( "SELECT * FROM $table_name WHERE $this->_pk = $id LIMIT 1", ARRAY_A );
			if ( $row[ $this->_pk ] ) {
	...
	public function load( $query = null ) {
		...
		$rows = $mcmsdb->get_results( $query, ARRAY_A );
		if ( ! empty( $rows ) ) {
			$results = array();
			foreach ( $rows as $row ) {
	...
	public function save() {
		...
		$table_name = $mcmsdb->prefix . $this->_table_name;
		if ( $this->id ) {
			if ( ! $mcmsdb->update( $table_name, $this->serialized_values(), array( $this->_pk => $this->{$this->_pk} ) ) ) {
	...
	public function delete() {
		...
		return $mcmsdb->delete( $table_name, array( $this->_pk => $this->{$this->_pk} ) );
	}
