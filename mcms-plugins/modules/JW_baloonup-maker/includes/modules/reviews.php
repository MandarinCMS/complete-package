 ...
 *
 * This class adds a review request system for your module or myskin to the MCMS dashboard.
 */
class PUM_Modules_Reviews {
	 ...
	 * @var string
	 */
	public static $api_url = 'https://api.mcmsbaloonupmaker.com/mcms-json/pmapi/v1/review_action';
	...
	public static function init() {
		...
		add_action( 'mcms_ajax_pum_review_action', array( __CLASS__, 'ajax_handler' ) );
	}

	/**
	 * Hook into relevant MCMS actions.
	 */
	...
	public static function ajax_handler() {
		...
		if ( ! mcms_verify_nonce( $_REQUEST['nonce'], 'pum_review_action' ) ) {
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$time_message = __( 'Hi there! You\'ve been using BaloonUp Maker on your site for %s - I hope it\'s been helpful. If you\'re enjoying my module, would you mind rating it 5-stars to help spread the word?', 'baloonup-maker' );
			$triggers = array(
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'baloonup-maker' ) ),
							'conditions' => array(
								strtotime( self::installed_on() . ' +1 week' ) < time(),
							),
							'link'       => 'https://mandarincms.org/support/module/baloonup-maker/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'baloonup-maker' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'baloonup-maker' ) ),
							'conditions' => array(
			...
			$pri = 10;
			$open_message = __( 'Hi there! You\'ve recently hit %s baloonup views on your site – that’s awesome!! If you\'d like to celebrate this milestone, rate BaloonUp Maker 5-stars to help spread the word!', 'baloonup-maker' );
	...
	public static function admin_notices() {
		...
		$uuid = mcms_hash( home_url() . '-' . get_current_user_id() );
				...
				function dismiss(reason) {
					$.ajax({
						...
						data: {
							...
							nonce: '<?php echo mcms_create_nonce( 'pum_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
			...
			<ul>
				<li>
					<a class="pum-dismiss" target="_blank" href="<?php echo $trigger['link']; ?>>" data-reason="am_now">
						<strong><?php _e( 'Ok, you deserve it', 'baloonup-maker' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="#" class="pum-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'baloonup-maker' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="pum-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'baloonup-maker' ); ?>
					</a>
				</li>
			</ul>
