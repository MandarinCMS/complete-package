...
class PUM_DataStorage {
	...
	public static function get( $key, $default = null ) {
		global $mcmsdb;
		$value = $mcmsdb->get_var( $mcmsdb->prepare( "SELECT option_value FROM $mcmsdb->options WHERE option_name = '%s'", $key ) );
	...
	public static function write( $key, $value ) {
		...
		$mcmsdb->replace( $mcmsdb->options, $data, $formats );
	}
	...
	public static function delete( $key ) {
		...
		return $mcmsdb->delete( $mcmsdb->options, array( 'option_name' => $key ) );
	}
	...
	public static function delete_by_match( $pattern ) {
		...
		if ( "^[0-9a-z\\_]+" !== $pattern && ! empty( $pattern ) ) {
			$query = "DELETE FROM $mcmsdb->options WHERE option_name REGEXP %s";

			$result = $mcmsdb->query( $mcmsdb->prepare( $query, $pattern ) );
		} else {
			$result = false;
		}
