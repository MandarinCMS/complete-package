...
abstract class PUM_Abstract_Upgrade_Posts extends PUM_Abstract_Upgrade implements PUM_Interface_Upgrade_Posts {
	 ...
	 *
	 * When true will prefetch all post_ids from the query and cache them, stepping through that array. MCMS_Query is only called once.
	 *
	 * When false the stepping will occur via a new MCMS_Query with pagination.
	 *
	...
	public function pre_fetch() {
		...
		if ( false === $total_to_migrate ) {
			...
			$posts = mcms_parse_id_list( $posts );
	...
	public function query_args( $args = array() ) {
		...
		$defaults = mcms_parse_args( $this->custom_query_args(), array(
	 ...
	 *
	 * @return int|string|MCMS_Error Next step number, 'done', or a MCMS_Error object.
	 */
	public function process_step() {
		...
		// Deduplicate.
		$completed_post_ids = mcms_parse_id_list( $completed_post_ids );
	...
	public function get_message( $code ) {
		...
		switch ( $code ) {
			...
			case 'start':
				...
				$message = sprintf( _n( 'Updating %d %2$s.', 'Updating %d %3$s.', $total_count, 'baloonup-maker' ), number_format_i18n( $total_count ), $singular, $plural );
				break;

			case 'done':
				$final_count = $this->get_current_count();

				$message = sprintf( _n( '%s %2$s was updated successfully.', '%s %3$s were updated successfully.', $final_count, 'baloonup-maker' ), number_format_i18n( $final_count ), $singular, $plural );
				break;
	...
	protected function get_post_ids() {
		if ( ! isset( $this->post_ids ) || ! $this->post_ids ) {
			...
			$this->post_ids = mcms_parse_id_list( $this->post_ids );
		}
	...
	protected function get_completed_post_ids() {
		if ( ! isset( $this->completed_post_ids ) || ! $this->completed_post_ids ) {
			...
			$this->completed_post_ids = mcms_parse_id_list( $this->completed_post_ids );
		}
