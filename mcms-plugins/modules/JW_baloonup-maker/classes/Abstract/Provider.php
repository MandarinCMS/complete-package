...
abstract class PUM_Abstract_Provider implements PUM_Interface_Provider {
	 ...
	 *
	 * @param MCMS_Error $errors Errors object.
	 * @param array    $values Values.
	 *
	 * @return MCMS_Error
	 */
	 ...
	 * @return MCMS_Error
	 */
	public function process_form_validation( MCMS_Error $errors, $values = array() ) {
		if ( $this->id !== $values['provider'] && ( 'none' === $values['provider'] && PUM_Options::get( 'newsletter_default_provider' ) !== $this->id ) ) {
			return $errors;
		}
	 ...
	 * @param MCMS_Error $errors        Errors object.
	 */
	public function process_form_submission( $values, &$json_response, MCMS_Error &$errors ) {
		if ( $this->id !== $values['provider'] && ( 'none' === $values['provider'] && PUM_Options::get( 'newsletter_default_provider' ) !== $this->id ) ) {
			return;
		}
