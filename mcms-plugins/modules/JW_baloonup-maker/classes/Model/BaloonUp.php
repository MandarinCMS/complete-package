...
/**
 * Class PUM_Model_BaloonUp
 *
 * @since 1.4
 */
class PUM_Model_BaloonUp extends PUM_Model_Post {

	#region Properties

	/** @var string */
	protected $required_post_type = 'baloonup';
	 ...
	 *
	 * @uses filter `pum_baloonup_get_title`
	 *
	 * @return string
	 */
	public function get_title() {
		$title = $this->get_meta( 'baloonup_title' );

		return (string) apply_filters( 'pum_baloonup_get_title', $title, $this->ID );
	}

	/**
	 * Returns the content of a baloonup.
	 *
	 * @uses filter `pum_baloonup_content`
	 *
	 * @return string
	 */
	public function get_content() {
		return $this->content = apply_filters( 'pum_baloonup_content', $this->post_content, $this->ID );
	}
	...
	/**
	 * Returns array of all baloonup settings.
	 *
	 * @param bool $force
	 *
	 * @return array
	...
	public function get_settings( $force = false ) {
		if ( ! isset( $this->settings ) || $force ) {
			$this->settings = $this->get_meta( 'baloonup_settings' );
		...
		return apply_filters( 'pum_baloonup_settings', $this->settings, $this->ID );
	}

	/**
	 * Returns a specific baloonup setting with optional default value when not found.
	 *
	 * @param $key
	 * @param bool $default
	 *
	...
	public function update_setting( $key, $value ) {
		...
		return $this->update_meta( 'baloonup_settings', $settings );
	}
	...
	/**
	 * Returns cleansed public settings for a baloonup.
	 *
	 * @return array
	 */
	public function get_public_settings() {
		$settings = mcms_parse_args( $this->get_settings(), PUM_Admin_BaloonUps::defaults() );

		foreach ( $settings as $key => $value ) {
			$field = PUM_Admin_BaloonUps::get_field( $key );
		...
		return apply_filters( 'pum_baloonup_get_public_settings', $settings, $this );
	}
	...
	public function get_cookies() {
		...
		return apply_filters( 'pum_baloonup_get_cookies', $cookies, $this->ID );
	}
	...
	public function get_triggers() {
		...
		return apply_filters( 'pum_baloonup_get_triggers', $triggers, $this->ID );
	}
	...
	/**
	 * Returns this baloonups myskin id or the default id.
	 *
	 * @uses filter `pum_baloonup_get_myskin_id`
	 *
	 * @return int $myskin_id
	 */
	public function get_myskin_id() {
		// TODO replace usage of balooncreate_get_default_baloonup_myskin.
		$myskin_id = $this->get_setting( 'myskin_id', balooncreate_get_default_baloonup_myskin() );

		return (int) apply_filters( 'pum_baloonup_get_myskin_id', $myskin_id, $this->ID );
	}
	...
	public function _dep_get_settings_group( $group, $key = null ) {
		if ( ! $this->$group ) {
			...
			$group_values = $this->get_meta( "baloonup_$group" );
			...
			$deprecated_values = balooncreate_get_baloonup_meta_group( $group, $this->ID );
		...
		$values = apply_filters( "pum_baloonup_get_$group", $this->$group, $this->ID );
		...
		if ( ! isset( $value ) ) {
			$value = $this->get_meta( "baloonup_{$group}_{$key}" );
		}

		return apply_filters( "pum_baloonup_get_{$group}_" . $key, $value, $this->ID );
	}
	...
	/**
	 * Returns array of classes for this baloonup.
	 *
	 * @uses filter `pum_baloonup_get_classes`
	 *
	...
	public function get_classes( $element = 'overlay' ) {
		$classes = array(
			'overlay'   => array(
				'pum',
				'pum-overlay',
				'pum-myskin-' . $this->get_myskin_id(),
				'pum-myskin-' . get_post_field( 'post_name', $this->get_myskin_id() ),
				'balooncreate-overlay', // Backward Compatibility
			),
			'container' => array(
				'pum-container',
				'balooncreate', // Backward Compatibility
				'myskin-' . $this->get_myskin_id(), // Backward Compatibility
			),
			'title'     => array(
				'pum-title',
				'balooncreate-title', // Backward Compatibility
			),
			'content'   => array(
				'pum-content',
				'balooncreate-content', // Backward Compatibility
			),
			'close'     => array(
				'pum-close',
				'balooncreate-close' // Backward Compatibility
			),
		);
		...
		$classes = apply_filters( 'pum_baloonup_classes', $classes, $this->ID );

		if ( ! isset( $classes[ $element ] ) ) {
			$classes[ $element ] = array();
		}

		return apply_filters( "pum_baloonup_{$element}_classes", $classes[ $element ], $this->ID );
	}

	/**
	 * Returns array for data attribute of this baloonup.
	 *
	 * @uses filter `pum_baloonup_get_data_attr`
	 *
	 * @return array
	 */
	public function get_data_attr() {
		$data_attr = array(
			...
			'meta'            => array(
				...
				'click_open' => balooncreate_get_baloonup_click_open( $this->ID ),
			),
		);
		...
		return apply_filters( 'pum_baloonup_data_attr', $data_attr, $this->ID );
	}
	...
	public function close_text() {
		...
		$myskin_text = balooncreate_get_baloonup_myskin_close( $this->get_myskin_id(), 'text', false );

		if ( empty( $text ) && ! empty( $myskin_text ) ) {
			$text = $myskin_text;
		}

		return apply_filters( 'pum_baloonup_close_text', $text, $this->ID );
	}

	/**
	 * Returns true if the close button should be rendered.
	 *
	 * @uses apply_filters `pum_baloonup_show_close_button`
	 *
	 * @return bool
	 */
	public function show_close_button() {
		return (bool) apply_filters( 'pum_baloonup_show_close_button', true, $this->ID );
	}
	...
	public function get_conditions( $filters = array() ) {
		...
		$filters = mcms_parse_args( $filters, array(
			'php_only' => null,
			'js_only'  => null,
		) );
		...
		return apply_filters( 'pum_baloonup_get_conditions', $conditions, $this->ID, $filters );
	}
	...
	public function parse_condition( $condition ) {
		$condition = mcms_parse_args( $condition, array(
			'target' => '',
			'not_operand' => false,
			'settings' => array(),
		) );
	...
	/**
	 * Checks if this baloonup has any conditions.
	 *
	 * @param array $filters
	 *
	 * @return bool
	...
	/**
	 * Checks if the baloonup has a specific condition.
	 *
	...
	/**
	 * Returns whether or not the baloonup is visible in the loop.
	 *
	 * @return bool
	 */
	public function is_loadable() {
		...
		return apply_filters( 'pum_baloonup_is_loadable', $loadable, $this->ID );
	}
	...
	public function mobile_disabled() {
		...
		return (bool) apply_filters( 'pum_baloonup_mobile_disabled', $mobile_disabled, $this->ID );
	}
	...
	public function tablet_disabled() {
		...
		return (bool) apply_filters( 'pum_baloonup_tablet_disabled', $tablet_disabled, $this->ID );
	}
	...
	public function get_event_count( $event = 'open', $which = 'current' ) {
		...
		switch ( $which ) {
			case 'current' :
				$current = $this->get_meta( 'baloonup_' . $keys[0] . '_count' );

				// Save future queries by inserting a valid count.
				if ( $current === false || ! is_numeric( $current ) ) {
					$current = 0;
					$this->update_meta( 'baloonup_' . $keys[0] . '_count', $current );
				}

				return absint( $current );
			case 'total'   :
				$total = $this->get_meta( 'baloonup_' . $keys[0] . '_count_total' );

				// Save future queries by inserting a valid count.
				if ( $total === false || ! is_numeric( $total ) ) {
					$total = 0;
					$this->update_meta( 'baloonup_' . $keys[0] . '_count_total', $total );
				}
	...
	/**
	 * Increase baloonup event counts.
	 *
	 * @param string $event
	 */
	public function increase_event_count( $event = 'open' ) {
		...
		/**
		 * This section simply ensures that all keys exist before the below query runs. This should only ever cause extra queries once per baloonup, usually in the admin.
		 */
		...
		$this->update_meta( 'baloonup_' . $keys[0] . '_count', absint( $current ) );
		$this->update_meta( 'baloonup_' . $keys[0] . '_count_total', absint( $total ) );
		$this->update_meta( 'baloonup_last_' . $keys[1], current_time( 'timestamp', 0 ) );
	...
	public function set_event_defaults( $event ) {
		...
		$keys = PUM_Analytics::event_keys( $event );
		$last = $this->get_meta( 'baloonup_last_' . $keys[1] );

		if ( empty( $last ) || ! is_numeric( $last ) ) {
			$this->update_meta( 'baloonup_last_' . $keys[1], 0 );
		}
	}

	/**
	 * Log and reset baloonup open count to 0.
	 */
	public function reset_counts() {
		// Log the reset time and count.
		add_post_meta( $this->ID, 'baloonup_count_reset', array(
		...
		foreach ( array( 'open', 'conversion' ) as $event ) {
			$keys = PUM_Analytics::event_keys( $event );
			$this->update_meta( 'baloonup_' . $keys[0] . '_count', 0 );
			$this->update_meta( 'baloonup_last_' . $keys[1], 0 );
		}
	...
	public function get_last_count_reset() {
		$resets = $this->get_meta( 'baloonup_count_reset', false );
	...
	public function setup() {
		...
		if ( ! isset( $this->data_version ) ) {
			...
			if ( ! $this->data_version ) {
				$myskin = $this->get_meta( 'baloonup_myskin' );
				$display_settings = $this->get_meta( 'baloonup_display' );

				// If there are existing settings set the data version to 2 so they can be updated.
				// Otherwise set to the current version as this is a new baloonup.
				$is_v2  = ( ! empty( $display_settings ) && is_array( $display_settings ) ) || $myskin > 0;
		...
		if ( $this->data_version < $this->model_version && pum_passive_baloonups_enabled() ) {
			/**
			 * Process passive settings migration as each baloonup is loaded. The will only run each migration routine once for each baloonup.
			 */
	...
	public function passive_migration() {
		...
		for ( $i = $this->data_version; $this->data_version < $this->model_version; $i ++ ) {
			do_action_ref_array( 'pum_baloonup_passive_migration_' . $this->data_version, array( &$this ) );
			$this->data_version ++;
		...
		do_action_ref_array( 'pum_baloonup_passive_migration', array( &$this, $this->data_version ) );
