...
abstract class PUM_Model_Post {
	...
	/**
	 * Declare the default properities in MCMS_Post as we can't extend it
	 */
	...
	public function __construct( $post = null, $_args = array() ) {
		if ( ! isset( $post ) ) {
			$post = new MCMS_Post( $this );
		} elseif ( ! is_a( $post, 'MCMS_Post' ) ) {
			$post = MCMS_Post::get_instance( $post );
		}
	...
	protected function _setup( $post ) {
		if ( ! is_object( $post ) || ! is_a( $post, 'MCMS_Post' ) ) {
			$this->valid = false;
	...
	public static function instance( $post_id, $force = false ) {
		...
		if ( $post === false || $force ) {
			if ( ! ( $_post = MCMS_Post::get_instance( $post_id ) ) ) {
				// Post doesn't exist.
				return false;
			}
	 ...
	 *
	 * This method allows us to get posts via MCMS_Query, while also passing in key/value pairs and a 'meta_relation'
	 ...
	 * that allows any arguments necessary from MCMS_Query
	 *
	 * If more control is needed over the meta_query item, you can
	 *
	 *        - use self::get() (a more basic wrapper for MCMS_Query) and pass in the meta_query manually
	 *        - use the '_get_posts_by' hook to access the query arguments
	 *        - use a normal MCMS_Query or get_posts; and then for each $post, create a new Helping_Friendly_Post( $post )
	 *
	 * @param    array $args {
	 *
	 *        Arguments for getting posts.  Besides the keys given, any arguments for MCMS_Query can also be included.
	 *
	 *        'meta_relation'
	 *        'meta' => array(
	 *            'meta_key_1' => 'value1',
	...
	/**
	 * Get an array of new instances of this class (or an extension class), as a wrapper for a new MCMS_Query
	 *
	 * @param    array $mcms_query_args Arguments to use for the MCMS_Query
	 *
	 * @return    array
	 */
	public static function get( $mcms_query_args ) {
		$class = get_called_class();
		...
		$mcms_query_args = mcms_parse_args( $mcms_query_args, $defaults );

		$query = new MCMS_Query( $mcms_query_args );
	 ...
	 * @return    MCMS_Query
	 */
	public static function query( $mcms_query_args ) {
		$class = get_called_class();
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new MCMS_Error( 'post-invalid-property', sprintf( __( 'Can\'t get property %s' ), $key ) );
