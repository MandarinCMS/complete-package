...
class PUM_Triggers {
	...
	public function add_trigger( $trigger = array() ) {
		if ( ! empty( $trigger['id'] ) && ! isset ( $this->triggers[ $trigger['id'] ] ) ) {
			$trigger = mcms_parse_args( $trigger, array(
				'id'              => '',
				'name'            => '',
				'modal_title'     => '',
				'settings_column' => '',
	...
	public function register_triggers() {
		$triggers = apply_filters( 'pum_registered_triggers', array(
			'click_open' => array(
				'name'            => __( 'Click Open', 'baloonup-maker' ),
				'modal_title'     => __( 'Click Trigger Settings', 'baloonup-maker' ),
				'settings_column' => sprintf( '<strong>%1$s</strong>: %2$s', __( 'Extra Selectors', 'baloonup-maker' ), '{{data.extra_selectors}}' ),
				'fields'          => array(
					'general'  => array(
						'extra_selectors' => array(
							'label'       => __( 'Extra CSS Selectors', 'baloonup-maker' ),
							'desc'        => __( 'For more than one selector, separate by comma (,)', 'baloonup-maker' ) . '<br /><strong>eg:  </strong>' . __( ' .class-here, .class-2-here, #button_id', 'baloonup-maker' ),
							'placeholder' => __( '.class-here', 'baloonup-maker' ),
							'doclink'     => 'https://docs.mcmsbaloonupmaker.com/article/147-getting-css-selectors?utm_source=module-baloonup-editor=&utm_medium=inline-doclink&utm_campaign=ContextualHelp&utm_content=extra-selectors',
						),
					),
					'advanced' => array(
						'do_default' => array(
			...
			'auto_open'  => array(
				'name'            => __( 'Time Delay / Auto Open', 'baloonup-maker' ),
				'modal_title'     => __( 'Time Delay Settings', 'baloonup-maker' ),
				'settings_column' => sprintf( '<strong>%1$s</strong>: %2$s', __( 'Delay', 'baloonup-maker' ), '{{data.delay}}' ),
				'fields'          => array(
					'general' => array(
						'delay' => array(
							'type'  => 'rangeslider',
							'label' => __( 'Delay', 'baloonup-maker' ),
							'desc'  => __( 'The delay before the baloonup will open in milliseconds.', 'baloonup-maker' ),
							'std'   => 500,
							'min'   => 0,
							'max'   => 10000,
							'step'  => 500,
	...
	public function cookie_field() {
		...
		return apply_filters( 'pum_trigger_cookie_field', array(
			...
			'desc'     => __( 'Choose which cookies will disable this trigger?', 'baloonup-maker' ),
			'type'     => 'select',
			'multiple' => true,
			'as_array' => true,
			'select2'  => true,
			'priority' => 99,
			'options'  => array(
				'add_new' => __( 'Add New Cookie', 'baloonup-maker' ),
			),
		) );
	}
	...
	public function get_tabs() {
		...
		return apply_filters( 'pum_get_trigger_tabs', array(
			...
			'cookie'   => __( 'Cookie', 'baloonup-maker' ),
