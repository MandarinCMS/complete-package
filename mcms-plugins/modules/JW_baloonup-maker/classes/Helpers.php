...
class PUM_Helpers {
	...
	public static function taxonomy_selectlist_query( $taxonomies = array(), $args = array(), $include_total = false ) {
		...
		if ( ! isset( $queries[ $key ] ) ) {
			...
			$results = array(
				'items'       => $terms,
				'total_count' => $include_total ? mcms_count_terms( $taxonomies, $total_args ) : null,
			);
	...
	public static function baloonup_myskin_selectlist() {

		$myskins = array();

		foreach ( balooncreate_get_all_baloonup_myskins() as $myskin ) {
			$myskins[ $myskin->ID ] = $myskin->post_title;
		}
	...
	public static function baloonup_selectlist( $args = array() ) {
		$baloonup_list = array();

		$baloonups = PUM_BaloonUps::query( $args );

		foreach ( $baloonups->posts as $baloonup ) {
			if ( in_array( $baloonup->post_status, array( 'publish' ) ) ) {
				$baloonup_list[ (string) $baloonup->ID ] = $baloonup->post_title;
			}
		}
