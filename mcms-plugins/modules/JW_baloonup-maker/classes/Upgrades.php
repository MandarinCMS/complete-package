...
class PUM_Upgrades {
	...
	/**
	 * BaloonUp Maker upgraded from version.
	 *
	 * @var    string
	 */
	...
	/**
	 * BaloonUp Maker initial version.
	 *
	 * @var    string
	 */	
	...
	public function __construct() {
		// Update stored module version info.
		...
		add_action( 'mcms_ajax_pum_process_upgrade_request', array( $this, 'process_upgrade_request' ) );
	 ...
	 * Update version info.
	 */
	public static function update_module_version() {
		...
		if ( empty( self::$version ) ) {
			$deprecated_ver = get_site_option( 'balooncreate_version' );

			// set to the deprecated version or last version that didn't have the version option set
			self::$version = $deprecated_ver ? $deprecated_ver : BaloonUp_Maker::$VER; // Since we had versioning in v1 if there isn't one stored its a new install.
		}
		...
		if ( ! self::$initial_version ) {
			...
			$oldest_known = BaloonUp_Maker::$VER;
			...
			$dep_upgraded_from = get_option( 'balooncreate_version_upgraded_from' );
		...
		if ( version_compare( self::$version, BaloonUp_Maker::$VER, '<' ) ) {
			...
			update_option( 'pum_ver', BaloonUp_Maker::$VER );
			self::$upgraded_from = self::$version;
			self::$version       = BaloonUp_Maker::$VER;
		}

		// If no current db version, but prior install detected, set db version correctly.

		// Here for backward compatibility.
		if ( ! self::$db_version || self::$db_version < BaloonUp_Maker::$DB_VER ) {
			self::$db_version = BaloonUp_Maker::$DB_VER;
	...
	public function register_processes( PUM_Upgrade_Registry $registry ) {
		// v1.7 Upgrades
		$registry->add_upgrade( 'core-v1_7-baloonups', array(
			'rules' => array(
				version_compare( self::$initial_version, '1.7', '<' ),
			),
			'class' => 'PUM_Upgrade_v1_7_BaloonUps',
			'file'  => BaloonUp_Maker::$DIR . 'includes/batch/upgrade/class-upgrade-v1_7-baloonups.php',
		) );
		...
		$registry->add_upgrade( 'core-v1_7-settings', array(
			...
			'file'  => BaloonUp_Maker::$DIR . 'includes/batch/upgrade/class-upgrade-v1_7-settings.php',
		) );
	}
	...
	public function render_upgrade_notice() {
		...
		<p class="pum-upgrade-notice">
			...
			if ( empty( $resume_upgrade ) ) {
				_e( 'Changes in the latest version of BaloonUp Maker or a PM extension require changes your settings to ensure everything continues to work correctly.', 'baloonup-maker' );
			} else {
				_e( 'BaloonUp Maker needs to complete a the update of your settings that was previously started.', 'baloonup-maker' );
			} ?>
		</p>
		<?php
	}
	...
	public function render_form() {
		...
		if ( $resume_upgrade && is_array( $resume_upgrade ) ) {
			$args = mcms_parse_args( $resume_upgrade, $args );
		} ?>

		<form method="post" class="pum-form  pum-batch-form  pum-upgrade-form" data-ays="<?php _e( 'This can sometimes take a few minutes, are you ready to begin?', 'baloonup-maker' ); ?>" data-upgrade_id="<?php echo $args['upgrade_id']; ?>" data-step="<?php echo (int) $args['step']; ?>" data-nonce="<?php echo esc_attr( mcms_create_nonce( 'pum_upgrade_ajax_nonce' ) ); ?>">

			<p><small><?php _e( 'The button below will do process these changes automatically for you.', 'baloonup-maker' ); ?></small></p>
			<div class="pum-field  pum-field-button  pum-field-submit">
				<?php submit_button( ! empty( $resume_upgrade ) ? __( 'Finish Upgrades', 'baloonup-maker' ) : __( 'Process Changes', 'baloonup-maker' ), 'secondary', 'submit', false ); ?>
			</div>
	...
	public function process_upgrade_request() {
		...
		if ( ! $upgrade_id && ! $this->has_uncomplete_upgrades() ) {
			mcms_send_json_error( array(
				'error' => __( 'A batch process ID must be present to continue.', 'baloonup-maker' ),
			) );
		}
		...
		if ( ! check_ajax_referer( 'pum_upgrade_ajax_nonce', 'nonce' ) ) {
			mcms_send_json_error( array(
				'error' => __( 'You do not have permission to initiate this request. Contact an administrator for more information.', 'baloonup-maker' ),
			) );
		}
		...
		if ( $upgrade === false ) {
			mcms_send_json_error( array(
				'error' => sprintf( __( '%s is an invalid batch process ID.', 'baloonup-maker' ), esc_html( $upgrade_id ) ),
			) );
		}
		...
		/** @var int|string|MCMS_Error $step */
		$step = $upgrade->process_step();

		if ( ! is_mcms_error( $step ) ) {
			$response_data = array(
				'step' => $step,
				'next' => null,
			);
			...
			mcms_send_json_success( $response_data );
		} else {
			mcms_send_json_error( $step );
		}
	}
	...
	public function get_upgrade( $upgrade_id = '', $step = 1 ) {
		...
		if ( ! class_exists( $class ) && ! empty( $class_file ) && file_exists( $class_file ) ) {
			...
			mcms_send_json_error( array(
				'error' => sprintf( __( 'An invalid file path is registered for the %1$s batch process handler.', 'baloonup-maker' ), "<code>{$upgrade_id}</code>" ),
			) );
		}

		if ( empty( $class ) || ! class_exists( $class ) ) {
			mcms_send_json_error( array(
				'error' => sprintf( __( '%1$s is an invalid handler for the %2$s batch process. Please try again.', 'baloonup-maker' ), "<code>{$class}</code>", "<code>{$upgrade_id}</code>" ),
			) );
		}
	...
	public function tools_page_tabs( $tabs = array() ) {
		...
		if ( $this->has_uncomplete_upgrades() ) {
			$tabs['upgrades'] = __( 'Upgrades', 'baloonup-maker' );
		}
	...
	public function tools_page_tab_content() {
		if ( ! $this->has_uncomplete_upgrades() ) {
			_e( 'No upgrades currently required.', 'baloonup-maker' );
