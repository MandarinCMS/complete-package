...
class PUM_Admin_Settings {
	...
	public static function save() {
		if ( ! empty( $_POST['pum_settings'] ) && empty( $_POST['pum_license_activate'] ) && empty( $_POST['pum_license_deactivate'] ) ) {
			...
			if ( ! isset( $_POST['pum_settings_nonce'] ) || ! mcms_verify_nonce( $_POST['pum_settings_nonce'], basename( __FILE__ ) ) ) {
				return;
			}
			...
			if ( PUM_Options::update_all( $settings ) ) {
				self::$notices[] = array(
					'type'    => 'success',
					'message' => __( 'Settings saved successfully!', 'baloonup-maker' ),
				);
				...
				self::$notices[] = array(
					'type'    => 'error',
					'message' => __( 'There must have been an error, settings not saved successfully!', 'baloonup-maker' ),
				);
			}
			 ...
			 *
			 * // We store the key in mcms_options for use by the update & licensing system to keep things cleanly detached.
			 ...
			 * self::$notices[] = array(
			 * 'type'    => 'success',
			 * 'message' => __( 'License activated successfully!', 'baloonup-maker' ),
			 * );
			 * }
			 * }
			 * }
	...
	public static function fields() {
		...
		if ( ! isset( $fields ) ) {
				...
				$fields['subscriptions'] = array(
					'main' => apply_filters( 'pum_newsletter_settings', array(
						'newsletter_default_provider'        => array(
							'label'   => __( 'Default Newsletter Provider', 'baloonup-maker' ),
							'desc'    => __( 'The default mailing provider used for the subscription form.', 'baloonup-maker' ),
							'type'    => 'select',
						...
						'default_success_message'            => array(
							'label' => __( 'Success Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user when successfuly subscribed.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'You have been subscribed!', 'baloonup-maker' ),
						),
						'default_empty_email_message'        => array(
							'label' => __( 'Empty Email Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user when no email is entered.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'Please enter a valid email.', 'baloonup-maker' ),
						),
						'default_invalid_email_message'      => array(
							'label' => __( 'Invalid Email Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user when an invalid email is entered.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'Email provided is not a valid email address.', 'baloonup-maker' ),
						),
						'default_error_message'              => array(
							'label' => __( 'Error Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user when an error has occurred.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'Error occurred when subscribing. Please try again.', 'baloonup-maker' ),
						),
						'default_already_subscribed_message' => array(
							'label' => __( 'Already Subscribed Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user who is already subscribed.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'You are already a subscriber.', 'baloonup-maker' ),
						),
						'default_consent_required_message'   => array(
							'label' => __( 'Consent Required Message', 'baloonup-maker' ),
							'desc'  => __( 'Message to show user who is already subscribed.', 'baloonup-maker' ),
							'type'  => 'text',
							'std'   => __( 'You must agree to continue.', 'baloonup-maker' ),
						),
					) ),
				);
			}
			...
			$fields = array_merge( $fields, array(
				...
				'privacy'    => array(
					'main'  => array(
						'disable_baloonup_open_tracking' => array(
							'type'  => 'checkbox',
							'label' => __( 'Disables baloonup open tracking?', 'baloonup-maker' ),
							'desc'  => __( 'This will disable the built in analytics functionality.', 'baloonup-maker' ),
						),
					),
					'forms' => array(
						'forms_disclaimer'                     => array(
							'type'    => 'html',
							'content' => "<strong>" . __( 'Disclaimer', 'baloonup-maker' ) . ":</strong> " . __( 'These settings only pertain to usage of the BaloonUp Maker built in subscription form shortcode, not 3rd party form modules.', 'baloonup-maker' ),
						),
						'privacy_consent_always_enabled'       => array(
							'label'   => __( 'Always enable consent field on subscription forms.', 'baloonup-maker' ),
							'type'    => 'select',
							'options' => array(
						...
						'default_privacy_consent_label'        => array(
							...
							'type'         => 'text',
							'std'          => __( 'Notify me about related content and special offers.', 'baloonup-maker' ),
							'dependencies' => array(
								'privacy_consent_always_enabled' => 'yes',
							),
						),
						'default_privacy_consent_type'         => array(
							'label'        => __( 'Consent Field Type', 'baloonup-maker' ),
							'desc'         => __( 'Radio forces the user to make a choice, often resulting in more opt-ins.', 'baloonup-maker' ),
							'type'         => 'select',
							'options'      => array(
						...
						'default_privacy_usage_text'           => array(
							...
							'desc'         => function_exists( 'get_privacy_policy_url' ) ? sprintf( __( 'You can use %1$%2$s to insert a link to your privacy policy. To customize the link text use %1$s:Link Text%2$s', 'baloonup-maker' ), '{{privacy_link', '}}' ) : '',
							'type'         => 'text',
							'std'          => __( 'If you opt in above we use this information send related content, discounts and other special offers.', 'baloonup-maker' ),
							'dependencies' => array(
								'privacy_consent_always_enabled' => 'yes',
							),
						),
				...
				'misc' => array(
					'main'   => array(
						'disabled_admin_bar'                   => array(
							'type'  => 'checkbox',
							'label' => __( 'Disable BaloonUps Admin Bar', 'baloonup-maker' ),
							'desc'  => __( 'This will disable the admin BaloonUps menu item.', 'baloonup-maker' ),
						),
						'debug_mode'                           => array(
							'type'  => 'checkbox',
							'label' => __( 'Enable Debug Mode', 'baloonup-maker' ),
							'desc'  => __( 'This will turn on multiple debug tools used to quickly find issues.', 'baloonup-maker' ),
						),
						'enable_easy_modal_compatibility_mode' => array(
							'type'  => 'checkbox',
							'label' => __( 'Enable Easy Modal v2 Compatibility Mode', 'baloonup-maker' ),
							'desc'  => __( 'This will automatically make any eModal classes you have added to your site launch the appropriate BaloonUp after import.', 'baloonup-maker' ),
						),
						'disable_admin_support_widget'         => array(
							'type'  => 'checkbox',
							'label' => __( 'Hide Admin Support Widget', 'baloonup-maker' ),
							'desc'  => __( 'This will hide the support widget on all baloonup maker admin pages.', 'baloonup-maker' ),
						),
						'disable_baloonup_category_tag'           => array(
							'type'  => 'checkbox',
							'label' => __( 'Disable categories & tags?', 'baloonup-maker' ),
							'desc'  => __( 'This will disable the baloonup tags & categories.', 'baloonup-maker' ),
						),
						'disable_cache'                        => array(
							'type'  => 'checkbox',
							'label' => __( 'Disable object caching', 'baloonup-maker' ),
							'desc'  => __( 'If you are seeing issues with settings not saving or baloonups not rendering changes immediately, try this option.', 'baloonup-maker' ),
						),
						'disable_asset_caching'                => array(
							'type'  => 'checkbox',
							'label' => __( 'Disable asset caching.', 'baloonup-maker' ),
							'desc'  => __( 'By default BaloonUp Maker caches a single JS & CSS file in your Uploads folder. These files include core, extension & user customized styles & scripts in a single set of files.', 'baloonup-maker' ),
						),
						'disable_shortcode_ui'                 => array(
							'type'  => 'checkbox',
							'label' => __( 'Disable the BaloonUp Maker shortcode button', 'baloonup-maker' ),
						),
					),
					'assets' => array(
						'disable_google_font_loading'     => array(
							'type'  => 'checkbox',
							'label' => __( "'Don't Load Google Fonts", 'baloonup-maker' ),
							'desc'  => __( 'Check this disable loading of google fonts, useful if the fonts you chose are already loaded with your myskin.', 'baloonup-maker' ),
						),
						'disable_baloonup_maker_core_styles' => array(
							'type'  => 'checkbox',
							'label' => __( 'Don\'t load BaloonUp Maker core stylesheet.', 'baloonup-maker' ),
							'desc'  => __( 'Check this if you have copied the BaloonUp Maker core styles to your own stylesheet or are using custom styles.', 'baloonup-maker' ),
						),
						'disable_baloonup_myskin_styles'      => array(
							'type'  => 'checkbox',
							'label' => __( 'Don\'t load baloonup myskin styles to the head.', 'baloonup-maker' ),
							'desc'  => __( 'Check this if you have copied the baloonup myskin styles to your own stylesheet or are using custom styles.', 'baloonup-maker' ),
						),
						'output_pum_styles'               => array(
							'id'      => 'output_pum_styles',
							'type'    => 'html',
							'content' => balooncreate_output_pum_styles(),
						),
					),
				),
			) );
	...
	public static function user_role_options() {
		...
		$options = array();
		foreach ( $mcms_roles->roles as $role => $labels ) {
	...
	public static function page() {
		...
		<div class="wrap">
			...
			<form id="pum-settings" method="post" action="">
				...
				<?php mcms_nonce_field( basename( __FILE__ ), 'pum_settings_nonce' ); ?>

				<button class="right top button-primary"><?php _e( 'Save', 'baloonup-maker' ); ?></button>

				<h1><?php _e( 'BaloonUp Maker Settings', 'baloonup-maker' ); ?></h1>
				...
				<button class="button-primary bottom right"><?php _e( 'Save', 'baloonup-maker' ); ?></button>
	...
	public static function tabs() {
		...
		if ( ! isset( $tabs ) ) {
			$tabs = apply_filters( 'pum_settings_tabs', array(
				...
				'subscriptions' => __( 'Subscriptions', 'baloonup-maker' ),
				'extensions'    => __( 'Extensions', 'baloonup-maker' ),
				'licenses'      => __( 'Licenses', 'baloonup-maker' ),
				'privacy'       => __( 'Privacy', 'baloonup-maker' ),
				'misc'          => __( 'Misc', 'baloonup-maker' ),
			) );

			/** @deprecated 1.7.0 */
			$tabs = apply_filters( 'balooncreate_settings_tabs', $tabs );
		}
	...
	public static function sections() {
		return apply_filters( 'pum_settings_tab_sections', array(
			...
			'extensions'    => array(
				'main' => __( 'Extension Settings', 'baloonup-maker' ),
			),
			'licenses'      => array(
				'main' => __( 'Licenses', 'baloonup-maker' ),
			),
			'privacy'       => array(
				'main'  => __( 'General', 'baloonup-maker' ),
				'forms' => __( 'Subscription Forms', 'baloonup-maker' ),
			),
			'misc'          => array(
				'main'   => __( 'Misc', 'baloonup-maker' ),
				'assets' => __( 'Assets', 'baloonup-maker' ),
			),
		) );
	}
