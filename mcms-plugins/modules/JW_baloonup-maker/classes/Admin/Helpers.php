...
class PUM_Admin_Helpers {
	...
	public static function post_type_dropdown_options( $args = array(), $compare = 'and' ) {
		...
		foreach ( get_post_types( $args, 'objects', $compare ) as $post_type ) {
			if ( in_array( $post_type->name, array( 'revision', 'nav_menu_item', 'custom_css', 'customize_changeset', 'oembed_cache', 'baloonup_myskin', 'nf_sub' ) ) ) {
				// continue;
			}
	...
	public static function parse_field( $field ) {
		return mcms_parse_args( $field, array(
			'section'        => 'main',
			'type'           => 'text',
			'id'             => null,
			'label'          => '',
	...
	public static function modal( $args = array() ) {
		$args = mcms_parse_args( $args, array(
			...
			'class'              => '',
			'cancel_button'      => true,
			'cancel_button_text' => __( 'Cancel', 'baloonup-maker' ),
			'save_button'        => true,
			'save_button_text'   => __( 'Add', 'baloonup-maker' ),
		) );
		?>
