...
class PUM_Admin_Templates {
	...
	public static function custom_fields() {
		...
		<script type="text/html" id="tmpl-pum-field-editor">
			<textarea name="{{data.name}}" id="{{data.id}}" class="pum-mcmseditor {{data.size}}-text" {{{data.meta}}}>{{data.value}}</textarea>
		</script>
		...
		<script type="text/html" id="tmpl-pum-field-license_key">
			...
			<# if (data.value.key !== '') { #>
			<?php mcms_nonce_field( 'pum_license_activation', 'pum_license_activation_nonce' ); ?>
			<# if (data.value.status === 'valid') { #>
			<span class="pum-license-status"><?php _e( 'Active', 'baloonup-maker' ); ?></span>
			<input type="submit" class="button-secondary pum-license-deactivate" id="{{data.id}}_deactivate" name="pum_license_deactivate[{{data.id}}]" value="<?php _e( 'Deactivate License', 'baloonup-maker' ); ?>"/>
			<# } else { #>
			<span class="pum-license-status"><?php _e( 'Inactive', 'baloonup-maker' ); ?></span>
			<input type="submit" class="button-secondary pum-license-activate" id="{{data.id}}_activate" name="pum_license_activate[{{data.id}}]" value="<?php _e( 'Activate License', 'baloonup-maker' ); ?>"/>
			<# } #>
			<# } #>
		...
		<script type="text/html" id="tmpl-pum-field-ga_event_labels">
			...
			<table>
				...
				<tr>
					<td>
						<label for="{{data.id}}_category" style="padding-left: 3px;"><?php _e( 'Category', 'baloonup-maker' ); ?></label>
						<input type="text" style="width:100%;" id="{{data.id}}_category" name="{{data.name}}[category]" value="{{data.value.category}}"/>
					</td>
					<td>
						<label for="{{data.id}}_action" style="padding-left: 3px;"><?php _e( 'Action', 'baloonup-maker' ); ?></label>
						<input type="text" style="width:100%;" id="{{data.id}}_action" name="{{data.name}}[action]" value="{{data.value.action}}"/>
					</td>
					<td>
						<label for="{{data.id}}_label" style="padding-left: 3px;"><?php _e( 'Label', 'baloonup-maker' ); ?></label>
						<input type="text" style="width:100%;" id="{{data.id}}_label" name="{{data.name}}[label]" value="{{data.value.label}}"/>
					</td>
					<td>
						<label for="{{data.id}}_value" style="padding-left: 3px;"><?php _e( 'Value', 'baloonup-maker' ); ?></label>
	...
	public static function misc_fields() {
		...
		<script type="text/html" id="tmpl-pum-field-wrapper">
			...
			<label for="{{data.id}}">
				...
				<a href="{{data.doclink}}" title="<?php _e( 'Documentation', 'baloonup-maker' ); ?>: {{data.label}}" target="_blank" class="pum-doclink dashicons dashicons-editor-help"></a>
				<# } #>
			</label>
			<# } else { #>
			<# if (typeof data.doclink === 'string' && data.doclink !== '') { #>
				...
				<div class="pum-modal-wrap">
					<form class="pum-form">
						<div class="pum-modal-header">
							...
							<# } #>
							<button type="button" class="pum-modal-close" aria-label="<?php _e( 'Close', 'baloonup-maker' ); ?>"></button>
						</div>
	...
	public static function triggers_editor() {
		...
		<script type="text/html" id="tmpl-pum-trigger-editor">
			<div class="pum-baloonup-trigger-editor  <# if (data.triggers && data.triggers.length) { print('has-list-items'); } #>" data-field_name="{{data.name}}">
				<button type="button" class="button button-primary pum-add-new no-button"><?php _e( 'Add New Trigger', 'baloonup-maker' ); ?></button>

				<p>
					<strong>
						<?php _e( 'Triggers cause a baloonup to open.', 'baloonup-maker' ); ?>
						<a href="<?php echo esc_url( 'https://docs.mcmsbaloonupmaker.com/article/350-baloonup-settings-box-triggers-option-settings?utm_medium=inline-doclink&utm_campaign=ContextualHelp&utm_source=module-baloonup-editor&utm_content=triggers-option-settings' ); ?>" target="_blank"
						   class="pum-doclink dashicons dashicons-editor-help" title="<?php esc_attr_e( sprintf( __( 'Learn more about %s', 'baloonup-maker' ), __( 'Triggers', 'baloonup-maker' ) ) ); ?>"></a>
					</strong>
				</p>
				...
				<table class="list-table form-table">
					<thead>
					<tr>
						<th><?php _e( 'Type', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Cookie', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Settings', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Actions', 'baloonup-maker' ); ?></th>
					</tr>
					</thead>
					<tbody>
					<#
				...
				<!--						<label for="pum-first-trigger">--><?php //_e( 'Choose a type of trigger to get started.', 'baloonup-maker' ); ?><!--</label>-->
				<!--						<# print(PUM_Admin.triggers.template.selectbox({id: 'pum-first-trigger', name: "", placeholder: "--><?php //_e( 'Select a trigger type.', 'baloonup-maker' ); ?><!--"})); #>-->
				<!--					</div>-->
				<!--				</div>-->
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-pum-trigger-row">
			<tr data-index="{{data.index}}">
				<td class="type-column">
					<button type="button" class="edit no-button link-button" aria-label="<?php _e( 'Edit this trigger', 'baloonup-maker' ); ?>">{{PUM_Admin.triggers.getLabel(data.type)}}</button>
					<input class="baloonup_triggers_field_type" type="hidden" name="{{data.name}}[{{data.index}}][type]" value="{{data.type}}"/>
					<input class="baloonup_triggers_field_settings" type="hidden" name="{{data.name}}[{{data.index}}][settings]" value="{{JSON.stringify(data.settings)}}"/>
				</td>
				...
				<td class="list-item-actions">
					<button type="button" class="edit dashicons dashicons-edit no-button" aria-label="<?php _e( 'Edit this trigger', 'baloonup-maker' ); ?>"></button>
					<button type="button" class="remove dashicons dashicons-no no-button" aria-label="<?php _e( 'Delete` this trigger', 'baloonup-maker' ); ?>"></button>
				</td>
			</tr>
		</script>
		...
		$presets = apply_filters( 'pum_click_selector_presets', array(
			'a[href="exact_url"]'    => __( 'Link: Exact Match', 'baloonup-maker' ),
			'a[href*="contains"]'    => __( 'Link: Containing', 'baloonup-maker' ),
			'a[href^="begins_with"]' => __( 'Link: Begins With', 'baloonup-maker' ),
			'a[href$="ends_with"]'   => __( 'Link: Ends With', 'baloonup-maker' ),
		) );
		?>

		<script type="text/html" id="tmpl-pum-click-selector-presets">
			<div class="pum-click-selector-presets">
				<span class="dashicons dashicons-arrow-left" title="<?php _e( 'Insert Preset', 'baloonup-maker' ); ?>"></span>
				<ul>
		...
		<script type="text/html" id="tmpl-pum-trigger-add-type">
			...
			var form_args = <?php echo PUM_Utils_Array::safe_json_encode( array(
				'id'     => 'pum-add-trigger',
				'fields' => array(
					'baloonup_trigger_add_type'         => array(
						'id'      => 'baloonup_trigger_add_type',
						'name'    => "",
						'label'   => __( 'Choose what type of trigger to add?', 'baloonup-maker' ),
						'type'    => 'select',
						'options' => PUM_Triggers::instance()->dropdown_list(),
					),
					'baloonup_trigger_add_cookie'       => array(
						'id'    => 'baloonup_trigger_add_cookie',
						'name'  => "",
						'type'  => 'checkbox',
						'label' => __( 'Would you like to set up a cookie as well?', 'baloonup-maker' ),
					),
					'baloonup_trigger_add_cookie_event' => array(
						'id'           => 'baloonup_trigger_add_cookie_event',
						'name'         => "",
						'type'         => 'select',
						'label'        => __( 'When will the cookie be set?', 'baloonup-maker' ),
						'options'      => PUM_Cookies::instance()->dropdown_list(),
						'dependencies' => array(
							'baloonup_trigger_add_cookie' => true,
						),
					),
				),
			) ); ?>,
			...
			title: '<?php _e( 'Choose what type of trigger to add?', 'baloonup-maker' ); ?>',
			content: content,
			save_button: pum_admin_vars.I10n.add || '<?php __( 'Add', 'baloonup-maker' ); ?>'
			}));
			#>
		</script>
	...
	public static function conditions_editor() {
		...
		<script type="text/html" id="tmpl-pum-condition-editor">
			<div class="facet-builder <# if (data.groups && data.groups.length) { print('has-conditions'); } #>">
				<p>
					<strong>
						<?php _e( 'Conditions allow you to show your baloonup to a targeted segment of your sites users.', 'baloonup-maker' ); ?>

						<?php printf( '%2$s<i class="dashicons dashicons-editor-help" title="%1$s"></i>%3$s', sprintf( __( 'Learn more about %s', 'baloonup-maker' ), __( 'Targeting Conditions', 'baloonup-maker' ) ), '<a href="https://docs.mcmsbaloonupmaker.com/article/351-baloonup-settings-box-targeting-option-settings?utm_medium=inline-doclink&utm_campaign=ContextualHelp&utm_source=module-baloonup-editor&utm_content=targeting-option-settings" target="_blank">', '</a>' ); ?>
					</strong>
				</p>

				<ul class="ul-disc">
					<li><?php _e( 'By default, this baloonup will be loaded on every page of your site for all users.', 'baloonup-maker' ); ?></li>
					<li><?php _e( 'Target the baloonup to a specific segment by adding conditions below.', 'baloonup-maker' ); ?></li>
					<li><?php printf( __( 'Click the %s button for any condition to check the opposite of the chosen condition. The button will turn red %s when active.', 'baloonup-maker' ), '<i style="font-size: 1em; width: 1em; height: 1em; line-height:1.5em;" class="dashicons dashicons-warning"></i>', '<i style="width: 1em; height: 1em; font-size: 1em; line-height:1.5em; color:#a00;" class="dashicons dashicons-warning"></i>' ); ?></li>
				</ul>
				...
				<div class="no-facet-groups">
					<label for="pum-first-condition"><?php _e( 'Choose a condition to target your baloonup to specific content or various other segments.', 'baloonup-maker' ); ?></label>
					<div class="facet-target">
						<button type="button" class="pum-not-operand dashicons-before dashicons-warning no-button" aria-label="<?php _e( 'Enable the Not Operand', 'baloonup-maker' ); ?>">
							<input type="checkbox" id="pum-first-facet-operand" value="1"/>
						</button>
						<# print(PUM_Admin.conditions.template.selectbox({id: 'pum-first-condition', name: "", placeholder: "<?php _e( 'Choose a condition', 'baloonup-maker' ); ?>"})); #>
					</div>
				</div>
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-pum-condition-group">
			...
			<div class="facet-group-wrap" data-index="{{data.index}}">
				<section class="facet-group">
					...
					<div class="add-or">
						<button type="button" class="add add-facet no-button" aria-label="<?php _ex( 'Add another OR condition', 'aria-label for add new OR condition button', 'baloonup-maker' ); ?>"><?php _e( 'or', 'baloonup-maker' ); ?></button>
					</div>
				</section>
				<p class="and">
					<button type="button" class="add-facet no-button" aria-label="<?php _ex( 'Add another AND condition group', 'aria-label for add new AND condition button', 'baloonup-maker' ); ?>"><?php _e( 'and', 'baloonup-maker' ); ?></button>
				</p>
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-pum-condition-facet">
			<div class="facet" data-index="{{data.index}}" data-target="{{data.target}}">
				<i class="or"><?php _e( 'or', 'baloonup-maker' ); ?></i>
				<div class="facet-col facet-target <# if (typeof data.not_operand !== 'undefined' && data.not_operand == '1') print('not-operand-checked'); #>">
					<button type="button" class="pum-not-operand dashicons-before dashicons-warning no-button" aria-label="<?php _e( 'Enable the Not Operand', 'baloonup-maker' ); ?>">
						<input type="checkbox" name="baloonup_settings[conditions][{{data.group}}][{{data.index}}][not_operand]" value="1"
						<# if (typeof data.not_operand !== 'undefined') print(PUM_Admin.utils.checked(data.not_operand, true, true)); #> />
					</button>
					<# print(PUM_Admin.conditions.template.selectbox({index: data.index, group: data.group, value: data.target, placeholder: "<?php _e( 'Choose a condition', 'baloonup-maker' ); ?>"})); #>
				</div>
				...
				<div class="facet-actions">
					<button type="button" class="remove remove-facet dashicons dashicons-dismiss no-button" aria-label="<?php _e( 'Remove Condition', 'baloonup-maker' ); ?>"></button>
				</div>
			</div>
		</script>
		<?php
	...
	public static function cookies_editor() {
		...
		<script type="text/html" id="tmpl-pum-cookie-editor">
			<div class="pum-baloonup-cookie-editor  <# if (data.cookies && data.cookies.length) { print('has-list-items'); } #>" data-field_name="{{data.name}}">
				<button type="button" class="button button-primary pum-add-new no-button"><?php _e( 'Add New Cookie', 'baloonup-maker' ); ?></button>

				<p>
					<strong>
						<?php _e( 'Cookies control the repeat display of a baloonup.', 'baloonup-maker' ); ?>
						<a href="<?php echo esc_url( 'https://docs.mcmsbaloonupmaker.com/article/358-baloonup-settings-box-cookies-option-settings?utm_medium=inline-doclink&utm_campaign=ContextualHelp&utm_source=module-baloonup-editor&utm_content=baloonup-settings-box-cookies-option-settings' ); ?>"
						   target="_blank" class="pum-doclink dashicons dashicons-editor-help" title="<?php esc_attr_e( sprintf( __( 'Learn more about %s', 'baloonup-maker' ), __( 'Cookies', 'baloonup-maker' ) ) ); ?>"></a>
					</strong>
				</p>
				...
				<table class="list-table form-table">
					<thead>
					<tr>
						<th><?php _e( 'Event', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Name', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Settings', 'baloonup-maker' ); ?></th>
						<th><?php _e( 'Actions', 'baloonup-maker' ); ?></th>
					</tr>
					</thead>
				...
				<!--					<div class="pum-field pum-field-select pum-field-select2">-->
				<!--						<label for="pum-first-cookie">--><?php //_e( 'Choose when you want to set a cookie to get started.', 'baloonup-maker' ); ?><!--</label>-->
				<!--						<# print(PUM_Admin.cookies.template.selectbox({id: 'pum-first-cookie', name: "", placeholder: "--><?php //_e( 'Select an event.', 'baloonup-maker' ); ?><!--"})); #>-->
				<!--					</div>-->
				<!--				</div>-->
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-pum-cookie-row">
			<tr data-index="{{data.index}}">
				<td class="event-column">
					<button type="button" class="edit no-button link-button" aria-label="<?php _e( 'Edit this cookie', 'baloonup-maker' ); ?>">{{PUM_Admin.cookies.getLabel(data.event)}}</button>
					<input class="baloonup_cookies_field_event" type="hidden" name="{{data.name}}[{{data.index}}][event]" value="{{data.event}}"/>
					<input class="baloonup_cookies_field_settings" type="hidden" name="{{data.name}}[{{data.index}}][settings]" value="{{JSON.stringify(data.settings)}}"/>
				</td>
				...
				<td class="list-item-actions">
					<button type="button" class="edit dashicons dashicons-edit no-button" aria-label="<?php _e( 'Edit this cookie', 'baloonup-maker' ); ?>"></button>
					<button type="button" class="remove dashicons dashicons-no no-button" aria-label="<?php _e( 'Delete` this cookie', 'baloonup-maker' ); ?>"></button>
				</td>
			</tr>
		</script>
		...
		<script type="text/html" id="tmpl-pum-cookie-add-event">
			...
			title: '<?php _e( 'When should your cookie be created?', 'baloonup-maker' ); ?>',
			content: PUM_Admin.cookies.template.selectbox({id: 'baloonup_cookie_add_event', name: "", placeholder: "<?php _e( 'Select a cookie type.', 'baloonup-maker' ); ?>"}),
			save_button: pum_admin_vars.I10n.add || '<?php __( 'Add', 'baloonup-maker' ); ?>'
			}));
			#>
		</script>
		...
		<script type="text/html" id="tmpl-pum-field-cookie_key">
			<div class="cookie-key">
				<button type="button" class="reset dashicons-before dashicons-image-rotate" title="<?php _e( 'Reset Cookie Key', 'baloonup-maker' ); ?>"></button>
