...
class PUM_Admin_Ajax {
	...
	public static function init() {
		add_action( 'mcms_ajax_pum_object_search', array( __CLASS__, 'object_search' ) );
		add_action( 'mcms_ajax_pum_process_batch_request', array( __CLASS__, 'process_batch_request' ) );
		// add_action( 'mcms_ajax_pum_process_batch_import', array( __CLASS__, 'process_batch_import' ) );
	}
	...
	public static function object_search() {
		...
		switch ( $object_type ) {
			case 'post_type':
				...
				$include = ! empty( $_REQUEST['include'] ) ? mcms_parse_id_list( $_REQUEST['include'] ) : null;
				$exclude = ! empty( $_REQUEST['exclude'] ) ? mcms_parse_id_list( $_REQUEST['exclude'] ) : null;
	...
	public static function process_batch_request() {
		...
		// Nonce.
		if ( ! isset( $_REQUEST['nonce'] ) || ( isset( $_REQUEST['nonce'] ) && false === mcms_verify_nonce( $_REQUEST['nonce'], "{$batch_id}_step_nonce" ) ) ) {
			mcms_send_json_error( array(
		...
		if ( $batch === false ) {
			mcms_send_json_error( array(
				'error' => sprintf( __( '%s is an invalid batch process ID.', 'baloonup-maker' ), esc_html( $_REQUEST['batch_id'] ) ),
			) );
		}
		...
		if ( empty( $class_file ) || ! file_exists( $class_file ) ) {
			mcms_send_json_error( array(
				'error' => sprintf( __( 'An invalid file path is registered for the %1$s batch process handler.', 'baloonup-maker' ), "<code>{$batch_id}</code>" ),
			) );
		} else {
		...
		if ( empty( $class ) || ! class_exists( $class ) ) {
			mcms_send_json_error( array(
				'error' => sprintf( __( '%1$s is an invalid handler for the %2$s batch process. Please try again.', 'baloonup-maker' ), "<code>{$class}</code>", "<code>{$batch_id}</code>" ),
			) );
		}
				...
				if ( method_exists( $process, 'can_export' ) ) {
					...
					if ( ! $process->is_empty ) {
						$response_data['url'] = pum_admin_url( 'tools', array(
							'step'       => $step,
							'nonce'      => mcms_create_nonce( 'pum-batch-export' ),
							'batch_id'   => $batch_id,
	 ...
	 * if ( ! function_exists( 'mcms_handle_upload' ) ) {
	 * require_once( BASED_TREE_URI . 'mcms-admin/includes/file.php' );
	 * }
	 *
	 * require_once AFFILIATEMCMS_PLUGIN_DIR . 'includes/admin/tools/import/class-batch-import-csv.php';
	 *
	 * if ( ! mcms_verify_nonce( $_REQUEST['pum_import_nonce'], 'pum_import_nonce' ) ) {
	 * mcms_send_json_error( array( 'error' => __( 'Nonce verification failed', 'baloonup-maker' ) ) );
	 * }
	 *
	 * if ( empty( $_FILES['pum-import-file'] ) ) {
	 * mcms_send_json_error( array( 'error' => __( 'Missing import file. Please provide an import file.', 'baloonup-maker' ), 'request' => $_REQUEST ) );
	 * }
	 *
	 ...
	 * mcms_send_json_error( array( 'error' => __( 'The file you uploaded does not appear to be a CSV file.', 'baloonup-maker' ), 'request' => $_REQUEST ) );
	 * }
	 *
	 * if ( ! file_exists( $_FILES['pum-import-file']['tmp_name'] ) ) {
	 * mcms_send_json_error( array( 'error' => __( 'Something went wrong during the upload process, please try again.', 'baloonup-maker' ), 'request' => $_REQUEST ) );
	 * }
	 *
	 * // Let MandarinCMS import the file. We will remove it after import is complete
	 * $import_file = mcms_handle_upload( $_FILES['pum-import-file'], array( 'test_form' => false ) );
	 *
	 ...
	 *
	 * // Attempt to retrieve the batch attributes from memory.
	 * if ( $batch_id && false === $batch = affiliate_mcms()->utils->batch->get( $batch_id ) ) {
	 * mcms_send_json_error( array(
	 * 'error' => sprintf( __( '%s is an invalid batch process ID.', 'baloonup-maker' ), esc_html( $_REQUEST['batch_id'] ) ),
	 * ) );
	 * }
	 ...
	 * mcms_send_json_error( array( 'error' => __( 'You do not have permission to import data', 'baloonup-maker' ) ) );
	 * }
	 *
	 ...
	 * 'nonce'     => mcms_create_nonce( "{$batch_id}_step_nonce" ),
	 * ) );
	 *
	 * } else {
	 *
	 * /**
	 * Error generated by _mcms_handle_upload()
	 *
	 * @see _mcms_handle_upload() in mcms-admin/includes/file.php
	 *
	 *
	 * mcms_send_json_error( array( 'error' => $import_file['error'] ) );
	 * }
	 *
	 * exit;
	 * }
