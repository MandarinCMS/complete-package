...
class PUM_Cookies {
	...
	public function register_cookies() {
		$cookies = apply_filters( 'pum_registered_cookies', array(
			'on_baloonup_close' => array(
				'name' => __( 'On BaloonUp Close', 'baloonup-maker' ),
			),
			'on_baloonup_open'  => array(
				'name' => __( 'On BaloonUp Open', 'baloonup-maker' ),
			),
			'pum_sub_form_success'             => array(
				'name' => __( 'Subscription Form: Successful', 'baloonup-maker' ),
			),
			'pum_sub_form_already_subscribed' => array(
				'name' => __( 'Subscription Form: Already Subscribed', 'baloonup-maker' ),
			),
			'manual'         => array(
				'name' => __( 'Manual JavaScript', 'baloonup-maker' ),
			),
		) );
	...
	public function add_cookie( $cookie = null ) {
		if ( ! empty( $cookie['id'] ) && ! isset ( $this->cookies[ $cookie['id'] ] ) ) {
			$cookie = mcms_parse_args( $cookie, array(
				'id'              => '',
				'name'            => '',
				'modal_title'     => __( 'Cookie Settings', 'baloonup-maker' ),
				'settings_column' => sprintf( '%s%s%s', '<# if (typeof data.session === "undefined" || data.session !== "1") { print(data.time); } else { print("', __( 'Sessions', 'baloonup-maker' ), '"); } #>' ),
				'priority'        => 10,
				'tabs'            => $this->get_tabs(),
	...
	public function cookie_fields() {
		return apply_filters( 'pum_get_cookie_fields', array(
			'general'  => array(
				'name' => array(
					'label'       => __( 'Cookie Name', 'baloonup-maker' ),
					'placeholder' => __( 'Cookie Name ex. balooncreater-123', 'baloonup-maker' ),
					'desc'        => __( 'The name that will be used when checking for or saving this cookie.', 'baloonup-maker' ),
					'std'         => '',
					'priority'    => 1,
				),
				'time' => array(
					'label'       => __( 'Cookie Time', 'baloonup-maker' ),
					'placeholder' => __( '364 days 23 hours 59 minutes 59 seconds', 'baloonup-maker' ),
					'desc'        => __( 'Enter a plain english time before cookie expires.', 'baloonup-maker' ),
					'std'         => '1 month',
					'priority'    => 2,
				),
			),
			'advanced' => array(
				'session' => array(
					'label'    => __( 'Use Session Cookie?', 'baloonup-maker' ),
					'desc'     => __( 'Session cookies expire when the user closes their browser.', 'baloonup-maker' ) . ' ' . sprintf( __( '%sNote%s: Modern browsers that reopen your last browser session\'s tabs do not properly clear session cookies', 'baloonup-maker' ), '<strong>', '</strong>' ),
					'type'     => 'checkbox',
					'std'      => false,
					'priority' => 1,
				),
				'path'    => array(
					'label'    => __( 'Sitewide Cookie', 'baloonup-maker' ),
					'desc'     => __( 'This will prevent the baloonup from triggering on all pages until the cookie expires.', 'baloonup-maker' ),
					'type'     => 'checkbox',
					'std'      => true,
					'priority' => 2,
				),
				'key'     => array(
					'label'    => __( 'Cookie Key', 'baloonup-maker' ),
					'desc'     => __( 'Changing this will cause all existing cookies to be invalid.', 'baloonup-maker' ),
					'type'     => 'cookie_key',
					'std'      => '',
					'priority' => 3,
				),
