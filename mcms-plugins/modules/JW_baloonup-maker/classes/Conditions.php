...
class PUM_Conditions {
	...
	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			...
			self::$instance->preload_posts = balooncreate_is_admin_baloonup_page();
		}
	...
	public function condition_sort_order() {
		if ( ! $this->condition_sort_order ) {
			...
			$order = array(
				__( 'General', 'baloonup-maker' )    => 1,
				__( 'Pages', 'baloonup-maker' )      => 5,
				__( 'Posts', 'baloonup-maker' )      => 5,
				__( 'Categories', 'baloonup-maker' ) => 14,
				__( 'Tags', 'baloonup-maker' )       => 14,
				__( 'Format', 'baloonup-maker' )     => 16,
			);
	...
	public function generate_post_type_conditions() {
		...
		foreach ( $post_types as $name => $post_type ) {
			...
			if ( $name == 'baloonup' || $name == 'baloonup_myskin' ) {
				continue;
			}
			...
			if ( $post_type->has_archive ) {
				$conditions[ $name . '_index' ] = array(
					...
					'name'     => sprintf( _x( '%s Archive', 'condition: post type plural label ie. Posts: All', 'baloonup-maker' ), $post_type->labels->name ),
			...
			$conditions[ $name . '_all' ] = array(
				'group'    => $post_type->labels->name,
				'name'     => sprintf( _x( 'All %s', 'condition: post type plural label ie. Posts: All', 'baloonup-maker' ), $post_type->labels->name ),
				'callback' => array( 'PUM_ConditionCallbacks', 'post_type' ),
			);

			$conditions[ $name . '_selected' ] = array(
				'group'    => $post_type->labels->name,
				'name'     => sprintf( _x( '%s: Selected', 'condition: post type plural label ie. Posts: Selected', 'baloonup-maker' ), $post_type->labels->name ),
				'fields'   => array(
					'selected' => array(
						'placeholder' => sprintf( _x( 'Select %s.', 'condition: post type plural label ie. Select Posts', 'baloonup-maker' ), strtolower( $post_type->labels->name ) ),
						'type'        => 'postselect',
						'post_type'   => $name,
						'multiple'    => true,
						'as_array'    => true,
			...
			$conditions[ $name . '_ID' ] = array(
				'group'    => $post_type->labels->name,
				'name'     => sprintf( _x( '%s: ID', 'condition: post type plural label ie. Posts: ID', 'baloonup-maker' ), $post_type->labels->name ),
				'fields'   => array(
					'selected' => array(
						'placeholder' => sprintf( _x( '%s IDs: 128, 129', 'condition: post type singular label ie. Posts IDs', 'baloonup-maker' ), strtolower( $post_type->labels->singular_name ) ),
						'type'        => 'text',
					),
				),
				'callback' => array( 'PUM_ConditionCallbacks', 'post_type' ),
			...
			if ( is_post_type_hierarchical( $name ) ) {
				$conditions[ $name . '_children' ] = array(
					'group'    => $post_type->labels->name,
					'name'     => sprintf( _x( '%s: Child Of', 'condition: post type plural label ie. Posts: ID', 'baloonup-maker' ), $post_type->labels->name ),
					'fields'   => array(
						'selected' => array(
							'placeholder' => sprintf( _x( 'Select %s.', 'condition: post type plural label ie. Select Posts', 'baloonup-maker' ), strtolower( $post_type->labels->name ) ),
							'type'        => 'postselect',
				...
				$conditions[ $name . '_ancestors' ] = array(
					'group'    => $post_type->labels->name,
					'name'     => sprintf( _x( '%s: Ancestor Of', 'condition: post type plural label ie. Posts: ID', 'baloonup-maker' ), $post_type->labels->name ),
					'fields'   => array(
						'selected' => array(
							'placeholder' => sprintf( _x( 'Select %s.', 'condition: post type plural label ie. Select Posts', 'baloonup-maker' ), strtolower( $post_type->labels->name ) ),
							'type'        => 'postselect',
			...
			if ( $name == 'page' && ! empty( $templates ) ) {
				$conditions[ $name . '_template' ] = array(
					'group'    => $post_type->labels->name,
					'name'     => sprintf( _x( '%s: With Template', 'condition: post type plural label ie. Pages: With Template', 'baloonup-maker' ), $post_type->labels->name ),
					'fields'   => array(
						'selected' => array(
							'type'     => 'select',
							'select2'  => true,
							'multiple' => true,
							'as_array' => true,
							'options'  => array_merge( array( 'default' => __( 'Default', 'baloonup-maker' ) ), $templates ),
						),
					),
					'callback' => array( 'PUM_ConditionCallbacks', 'post_type' ),
				);
	...
	public function generate_post_type_tax_conditions( $name ) {
		...
		foreach ( $taxonomies as $tax_name => $taxonomy ) {
			...
			$conditions[ $name . '_w_' . $tax_name ] = array(
				'group'    => $post_type->labels->name,
				'name'     => sprintf( _x( '%1$s: With %2$s', 'condition: post type plural and taxonomy singular label ie. Posts: With Category', 'baloonup-maker' ), $post_type->labels->name, $taxonomy->labels->singular_name ),
				'fields'   => array(
					'selected' => array(
						'placeholder' => sprintf( _x( 'Select %s.', 'condition: post type plural label ie. Select categories', 'baloonup-maker' ), strtolower( $taxonomy->labels->name ) ),
						'type'        => 'taxonomyselect',
						'taxonomy'    => $tax_name,
						'multiple'    => true,
						'as_array'    => true,
	...
	public function generate_taxonomy_conditions() {
		...
		foreach ( $taxonomies as $tax_name => $taxonomy ) {
			...
			$conditions[ 'tax_' . $tax_name . '_all' ] = array(
				...
				'name'     => sprintf( _x( '%s: All', 'condition: taxonomy plural label ie. Categories: All', 'baloonup-maker' ), $taxonomy->labels->name ),
				'callback' => array( 'PUM_ConditionCallbacks', 'taxonomy' ),
			);

			$conditions[ 'tax_' . $tax_name . '_selected' ] = array(
				'group'    => $taxonomy->labels->name,
				'name'     => sprintf( _x( '%s: Selected', 'condition: taxonomy plural label ie. Categories: Selected', 'baloonup-maker' ), $taxonomy->labels->name ),
				'fields'   => array(
					'selected' => array(
						'placeholder' => sprintf( _x( 'Select %s.', 'condition: taxonomy plural label ie. Select Categories', 'baloonup-maker' ), strtolower( $taxonomy->labels->name ) ),
						'type'        => 'taxonomyselect',
						'taxonomy'    => $tax_name,
						'multiple'    => true,
						'as_array'    => true,
			...
			$conditions[ 'tax_' . $tax_name . '_ID' ] = array(
				'group'    => $taxonomy->labels->name,
				'name'     => sprintf( _x( '%s: IDs', 'condition: taxonomy plural label ie. Categories: Selected', 'baloonup-maker' ), $taxonomy->labels->name ),
				'fields'   => array(
					'selected' => array(
						'placeholder' => sprintf( _x( '%s IDs: 128, 129', 'condition: taxonomy plural label ie. Category IDs', 'baloonup-maker' ), strtolower( $taxonomy->labels->singular_name ) ),
						'type'        => 'text',
					),
				),
				'callback' => array( 'PUM_ConditionCallbacks', 'taxonomy' ),
	...
	public function register_conditions() {
		...
		$conditions['is_front_page'] = array(
			'group'    => __( 'General', 'baloonup-maker' ),
			'name'     => __( 'Home Page', 'baloonup-maker' ),
			'callback' => 'is_front_page',
			'priority' => 2,
		);

		$conditions['is_home'] = array(
			'group'    => __( 'Posts', 'baloonup-maker' ),
			'name'     => __( 'Blog Index', 'baloonup-maker' ),
			'callback' => 'is_home',
			'priority' => 1,
		);
		...
		$conditions['is_search'] = array(
			'group'    => __( 'General', 'baloonup-maker' ),
			'name'     => __( 'Search Result Page', 'baloonup-maker' ),
			'callback' => 'is_search',
		);

		$conditions['is_404'] = array(
			'group'    => __( 'General', 'baloonup-maker' ),
			'name'     => __( '404 Error Page', 'baloonup-maker' ),
			'callback' => 'is_404',
		);
	...
	public static function allowed_user_roles() {
		...
		if ( ! isset( $roles ) && is_object( $mcms_roles ) ) {
			$roles = apply_filters( 'pum_user_roles', $mcms_roles->role_names );
