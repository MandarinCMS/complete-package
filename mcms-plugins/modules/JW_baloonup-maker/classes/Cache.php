<?php
/**
 * @copyright   Copyright (c) 2017, Jungle Modules
 * @author      Daniel Iser
 */
...
class PUM_Cache {
	...
	public static function add( $key, $data, $group = '' ) {
		...
		return mcms_cache_add( $key, $data, self::prefix_( $group ), self::get_timeout( $key, $group ) );
	}
	...
	public static function replace( $key, $data, $group = '' ) {
		...
		return mcms_cache_replace( $key, $data, self::prefix_( $group ), self::get_timeout( $key, $group ) );
	}
	...
	public static function set( $key, $data, $group = '' ) {
		...
		return mcms_cache_set( $key, $data, self::prefix_( $group ), self::get_timeout( $key, $group ) );
	}
	...
	public static function get( $key, $group = '', $force = false, &$found = null ) {
		...
		return mcms_cache_get( $key, self::prefix_( $group ), $force, $found );
	}
	...
	public static function delete( $key, $group = '' ) {
		...
		return mcms_cache_delete( $key, self::prefix_( $group ) );
	}
	...
	public static function delete_group( $group = '' ) {
		...
		if ( ! function_exists( 'mcms_cache_delete_group' ) ) {
			return false;
		}

		return mcms_cache_delete_group( self::prefix_( $group ) );
	}
	...
	public static function incr( $key, $offset = 1, $group = '' ) {
		...
		return mcms_cache_incr( $key, $offset, self::prefix_( $group ) );
	}
	...
	public static function decr( $key, $offset = 1, $group = '' ) {
		...
		return mcms_cache_decr( $key, $offset, self::prefix_( $group ) );
	}
