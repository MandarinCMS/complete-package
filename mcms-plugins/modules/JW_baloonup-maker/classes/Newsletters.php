...
class PUM_Newsletters {
	...
	public static function init() {
		add_action( 'modules_loaded', array( __CLASS__, 'delayed_init' ), - 100 );
	}
	...
	public static function delayed_init() {
		...
		require_once BaloonUp_Maker::$DIR . 'includes/functions/newsletter.php';

		do_action( 'pum_newsletter_init' );

		PUM_Shortcode_Subscribe::init();

		add_action( 'mcms_ajax_pum_sub_form', array( __CLASS__, 'ajax_request' ) );
		add_action( 'mcms_ajax_nopriv_pum_sub_form', array( __CLASS__, 'ajax_request' ) );
	...
	public static function ajax_request() {
		self::$errors = new MCMS_Error;

		$values = isset( $_REQUEST['values'] ) ? $_REQUEST['values'] : array();

		if ( empty( $values['baloonup_id'] ) && ! empty( $values['pum_form_baloonup_id'] ) ) {
			$values['baloonup_id'] = absint( $values['pum_form_baloonup_id'] );
		}
	 ...
	 *
	 * @param       $errors MCMS_Error
	 * @param array $extra_response_args
	 */
	public static function send_errors( MCMS_Error $errors, $extra_response_args = array() ) {
		if ( ! $errors || ! is_mcms_error( $errors ) ) {
			$errors = self::$errors;
		}
	...
	public static function send_success( $response = array() ) {
		mcms_send_json_success( array_filter( $response ) );
		die;
	}
	 ...
	 *
	 * @param MCMS_Error $_errors
	 *
	 * @return array
	 */
	public static function prepare_errors( MCMS_Error $_errors ) {
		if ( ! $_errors || ! is_mcms_error( $_errors ) ) {
			$_errors = self::$errors;
		}
	...
	public static function record_submission( $values = array() ) {
		$data = mcms_parse_args( $values, array(
			'uuid'         => self::uuid(),
	...
	public static function sanitization( $values = array() ) {
		$values = mcms_parse_args( $values, array(
		...
		$values['consent_args'] = mcms_parse_args( $values['consent_args'], array(
			'enabled'  => 'no',
			'required' => false,
			'text'     => '',
		) );
		...
		if ( $values['consent_args']['enabled'] === 'yes' && ! $values['consent_args']['required'] && $values['consent'] === 'no' ) {
			...
			$values['fname'] = '';
			$values['lname'] = '';
			$values['email']   = function_exists( 'mcms_privacy_anonymize_data' ) ? mcms_privacy_anonymize_data( 'email', $values['email'] ) : 'deleted@site.invalid';
		}
