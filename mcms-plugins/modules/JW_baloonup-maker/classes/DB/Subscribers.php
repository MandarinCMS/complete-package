...
class PUM_DB_Subscribers extends PUM_Abstract_Database {
	...
	public function create_table() {
		...
		$sql = "CREATE TABLE " . $this->table_name() . " (
			...
			`baloonup_id` BIGINT(20) NOT NULL,
	...
	public function query( $args = array(), $return_type = 'OBJECT' ) {
		...
		if ( $args['s'] && ! empty( $args['s'] ) ) {
			...
			$search = mcms_unslash( trim( $args['s'] ) );
		...
		if ( ! empty( $args['orderby'] ) ) {
			$query .= " ORDER BY `" . mcms_unslash( trim( $args['orderby'] ) ) . '`';
		...
		if ( strpos( $query, '%s' ) || strpos( $query, '%d' ) ) {
			$query = $mcmsdb->prepare( $query, $values );
		}

		if ( $return_type != 'model' ) {
			$results = $mcmsdb->get_results( $query, $return_type );
		}
