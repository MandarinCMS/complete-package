...
class PUM_ConditionCallbacks {
	...
	public static function post_type( $condition = array() ) {
		...
		switch ( $modifier ) {
			...
			case 'ID':
			case 'selected':
				if ( self::is_post_type( $post_type ) && is_singular( $post_type ) && in_array( $post->ID, mcms_parse_id_list( $selected ) ) ) {
					return true;
				}
				break;
			...
			case 'children':
				...
				// Chosen parents.
				$selected = mcms_parse_id_list( $selected );
	...
	public static function taxonomy( $condition = array() ) {
		...
		switch ( $modifier ) {
			...
			case 'selected':
				...
				if ( is_tax( $taxonomy, mcms_parse_id_list( $selected ) ) ) {
					return true;
				}
				break;
		}
	...
	public static function category( $condition = array() ) {
		...
		switch ( $modifier ) {
			...
			case 'selected':
				$selected = ! empty( $condition['settings']['selected'] ) ? $condition['settings']['selected'] : array();
				if ( is_category( mcms_parse_id_list( $selected ) ) ) {
					return true;
				}
				break;
		}
	...
	public static function post_tag( $condition = array() ) {
		...
		switch ( $modifier ) {
			...
			case 'selected':
				$selected = ! empty( $condition['settings']['selected'] ) ? $condition['settings']['selected'] : array();
				if ( is_tag( mcms_parse_id_list( $selected ) ) ) {
					return true;
				}
				break;
		}
	...
	public static function post_type_tax( $condition = array() ) {
		...
		$selected = ! empty( $condition['settings']['selected'] ) ? $condition['settings']['selected'] : array();
		if ( self::is_post_type( $post_type ) && has_term( mcms_parse_id_list( $selected ), $taxonomy ) ) {
			return true;
		}
	...
	public static function post_type_category( $condition = array() ) {
		...
		$selected = ! empty( $condition['settings']['selected'] ) ? $condition['settings']['selected'] : array();
		if ( self::is_post_type( $post_type ) && has_category( mcms_parse_id_list( $selected ) ) ) {
			return true;
		}
	...
	public static function post_type_tag( $condition = array() ) {
		...
		$selected = ! empty( $condition['settings']['selected'] ) ? $condition['settings']['selected'] : array();
		if ( self::is_post_type( $post_type ) && has_tag( mcms_parse_id_list( $selected ) ) ) {
			return true;
		}
