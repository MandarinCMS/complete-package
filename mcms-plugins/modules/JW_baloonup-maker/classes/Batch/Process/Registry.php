...
class PUM_Batch_Process_Registry extends PUM_Abstract_Registry {
	 ...
	 * }
	 *
	 * @return MCMS_Error|true True on successful registration, otherwise a MCMS_Error object.
	 */
	public function register_process( $batch_id, $process_args ) {
		$process_args = mcms_parse_args( $process_args,  array_fill_keys( array( 'class', 'file' ), '' ) );

		if ( empty( $process_args['class'] ) ) {
			return new MCMS_Error( 'invalid_batch_class', __( 'A batch process class must be specified.', 'baloonup-maker' ) );
		}

		if ( empty( $process_args['file'] ) ) {
			return new MCMS_Error( 'missing_batch_class_file', __( 'No batch class handler file has been supplied.', 'baloonup-maker' ) );
		}

		// 2 if Windows path.
		if ( ! in_array( validate_file( $process_args['file'] ), array( 0, 2 ), true ) ) {
			return new MCMS_Error( 'invalid_batch_class_file', __( 'An invalid batch class handler file has been supplied.', 'baloonup-maker' ) );
		}
