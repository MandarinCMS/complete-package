(function ($, document, undefined) {
    ...
    $(document)
        .on('click', '#baloonup_reset_open_count', function () {
            var $this = $(this);
...
(function ($) {
    ...
    var conditions = {
        get_conditions: function () {
            return window.pum_baloonup_settings_editor.conditions_selectlist;
        },
        ...
        template: {
            ...
            settings: function (args, values) {
                ...
                if (!data.fields.length && pum_baloonup_settings_editor.conditions[args.target] !== undefined) {
                    data.fields = pum_baloonup_settings_editor.conditions[args.target].fields;
                }
                ...
                _.each(data.fields, function (field, fieldID) {
                    ...
                    field.name = 'baloonup_settings[conditions][' + data.group + '][' + data.index + '][settings][' + fieldID + ']';

                    if (field.id === '') {
                        field.id = 'baloonup_settings_conditions_' + data.group + '_' + data.index + '_settings_' + fieldID;
                    }
            ...
            selectbox: function (args) {
                ...
                if (data.id === null) {
                    data.id = 'baloonup_settings_conditions_' + data.group + '_' + data.index + '_target';
                }

                if (data.name === null) {
                    data.name = 'baloonup_settings[conditions][' + data.group + '][' + data.index + '][target]';
                }
            ...
            $('.facet-builder .facet-group-wrap').each(function () {
                ...
                $group
                    ...
                    $facet
                        ...
                        this.name = this.name.replace(/baloonup_settings\[conditions\]\[\d*?\]\[\d*?\]/, "baloonup_settings[conditions][" + groupIndex + "][" + facetIndex + "]");
                        this.id = this.id.replace(/baloonup_settings_conditions_\d*?_\d*?_/, "baloonup_settings_conditions_" + groupIndex + "_" + facetIndex + "_");
                    });
                });
            });
        }
...
(function ($, document, undefined) {
    ...
    var I10n = pum_admin_vars.I10n,
        ...
        cookies = {
            get_cookies: function () {
                return window.pum_baloonup_settings_editor.cookies;
            },
            ...
            refreshDescriptions: function () {
                $('.pum-baloonup-cookie-editor table.list-table tbody tr').each(function () {
                    var $row = $(this),
                        event = $row.find('.baloonup_cookies_field_event').val(),
                        values = JSON.parse($row.find('.baloonup_cookies_field_settings:first').val());
            ...
            template: {
                ...
                selectbox: function (args) {
                    ...
                    if (data.id === null) {
                        data.id = 'baloonup_settings_cookies_' + data.index + '_event';
                    }

                    if (data.name === null) {
                        data.name = 'baloonup_settings[cookies][' + data.index + '][event]';
                    }
            ...
            rows: {
                ...
                remove: function ($cookie) {
                    var $editor = $cookie.parents('.pum-baloonup-cookie-editor');
    ...
    $(document)
        ...
        .on('select2:select pumselect2:select', '#pum-first-cookie', function () {
            var $this = $(this),
                $editor = $this.parents('.pum-baloonup-cookie-editor'),
                event = $this.val(),
                values = {
        ...
        .on('click', '.pum-baloonup-cookie-editor .pum-add-new', function () {
            current_editor = $(this).parents('.pum-baloonup-cookie-editor');
            var template = mcms.template('pum-cookie-add-event');
            PUM_Admin.modals.reload('#pum_cookie_add_event_modal', template({I10n: I10n}));
        })
        .on('click', '.pum-baloonup-cookie-editor .edit', function (e) {
            var $this = $(this),
                $editor = $this.parents('.pum-baloonup-cookie-editor'),
                $row = $this.parents('tr:first'),
                event = $row.find('.baloonup_cookies_field_event').val(),
                values = _.extend({}, JSON.parse($row.find('.baloonup_cookies_field_settings:first').val()), {
        ...
        .on('click', '.pum-baloonup-cookie-editor .remove', function (e) {
            var $this = $(this),
                $row = $this.parents('tr:first');
        ...
        .on('submit', '#pum_cookie_add_event_modal .pum-form', function (e) {
            var $editor = current_editor,
                event = $('#baloonup_cookie_add_event').val(),
                values = {
            ...
            cookies.template.form(event, values, function (e) {
                ...
                if (typeof PUM_Admin.triggers !== 'undefined' && PUM_Admin.triggers.new_cookie !== false && PUM_Admin.triggers.new_cookie >= 0) {
                    var $trigger = PUM_Admin.triggers.current_editor.find('tbody tr').eq(PUM_Admin.triggers.new_cookie).find('.baloonup_triggers_field_settings:first'),
...
(function ($, document, undefined) {
    ...
    var triggers = {
        ...
        get_triggers: function () {
            return window.pum_baloonup_settings_editor.triggers;
        },
        ...
        rows: {
            ...
            remove: function ($trigger) {
                var $editor = $trigger.parents('.pum-baloonup-trigger-editor');
        ...
        template: {
            form: function (type, values, callback) {
                ...
                $cookies.each(function () {
                    var settings = JSON.parse($(this).find('.baloonup_cookies_field_settings:first').val());
            ...
            selectbox: function (args) {
                ...
                if (data.id === null) {
                    data.id = 'baloonup_settings_triggers_' + data.index + '_type';
                }

                if (data.name === null) {
                    data.name = 'baloonup_settings[triggers][' + data.index + '][type]';
                }
        ...
        renumber: function () {
            $('.pum-baloonup-trigger-editor table.list-table tbody tr').each(function () {
                var $this = $(this),
                    index = $this.parent().children().index($this);
        ...
        refreshDescriptions: function () {
            $('.pum-baloonup-trigger-editor table.list-table tbody tr').each(function () {
                var $row = $(this),
                    type = $row.find('.baloonup_triggers_field_type').val(),
                    values = JSON.parse($row.find('.baloonup_triggers_field_settings:first').val()),
    ...
    $(document)
        ...
        .on('select2:select pumselect2:select', '#pum-first-trigger', function () {
            var $this = $(this),
                $editor = $this.parents('.pum-baloonup-trigger-editor'),
                type = $this.val(),
                values = {};
            ...
            triggers.template.form(type, values, function (event) {
                ...
                if (trigger_settings.cookie_name !== undefined && trigger_settings.cookie_name !== null && (trigger_settings.cookie_name === 'add_new' || trigger_settings.cookie_name.indexOf('add_new') >= 0)) {
                    ...
                    $('#pum-baloonup-settings-container .pum-baloonup-cookie-editor button.pum-add-new').trigger('click');
                }
            });
        ...
        // Add New Triggers
        .on('click', '.pum-baloonup-trigger-editor .pum-add-new', function () {
            PUM_Admin.triggers.current_editor = $(this).parents('.pum-baloonup-trigger-editor');
            var template = mcms.template('pum-trigger-add-type');
            PUM_Admin.modals.reload('#pum_trigger_add_type_modal', template({I10n: I10n}));
        })
        .on('click', '.pum-baloonup-trigger-editor .edit', function (event) {
            var $this = $(this),
                $editor = $this.parents('.pum-baloonup-trigger-editor'),
                $row = $this.parents('tr:first'),
                type = $row.find('.baloonup_triggers_field_type').val(),
                values = _.extend({}, JSON.parse($row.find('.baloonup_triggers_field_settings:first').val()), {
                    index: $row.parent().children().index($row),
                    type: type
                });
        ...
        .on('click', '.pum-baloonup-trigger-editor .remove', function (event) {
            var $this = $(this),
                $editor = $this.parents('.pum-baloonup-trigger-editor'),
                $row = $this.parents('tr:first');
        ...
        .on('submit', '#pum_trigger_add_type_modal .pum-form', function (event) {
            var $editor = PUM_Admin.triggers.current_editor,
                $cookie_editor = $editor.parents('#pum-baloonup-settings-triggers-subtabs_main').find('.pum-field-cookies .pum-baloonup-cookie-editor'),
                type = $('#baloonup_trigger_add_type').val(),
                add_cookie = $('#baloonup_trigger_add_cookie').is(':checked'),
                add_cookie_event = $('#baloonup_trigger_add_cookie_event').val(),
                values = {};
...
(function ($) {
    ...
    window.pum_baloonup_settings_editor = window.pum_baloonup_settings_editor || {
        form_args: {},
        current_values: {}
    };
    ...
    $(document)
        .ready(function () {
            ...
            var $container = $('#pum-baloonup-settings-container'),
                args = pum_baloonup_settings_editor.form_args || {},
                values = pum_baloonup_settings_editor.current_values || {};
            ...
            $('a.page-title-action')
                .clone()
                .attr('target', '_blank')
                .attr('href', pum_admin_vars.homeurl + '?baloonup_preview=true&baloonup=' + $('#post_ID').val())
                .text(pum_admin_vars.I10n.preview_baloonup)
        ...
        .on('keydown', '#baloonup-title', function (event) {
        ...
        .on('keydown', '#title, #baloonup-title', function (event) {
            var keyCode = event.keyCode || event.which,
                target;
            if (!event.shiftKey && 9 === keyCode) {
                event.preventDefault();
                target = $(this).attr('id') === 'title' ? '#baloonup-title' : '#insert-media-button';
                $(target).focus();
            }
        })
        .on('keydown', '#baloonup-title, #insert-media-button', function (event) {
            var keyCode = event.keyCode || event.which,
                target;
            if (event.shiftKey && 9 === keyCode) {
                event.preventDefault();
                target = $(this).attr('id') === 'baloonup-title' ? '#title' : '#baloonup-title';
                $(target).focus();
            }
        });
}(jQuery));