	...
	$(document).ready(function(){
		...
		window.ULTHotspotContainerView = window.VcColumnView.extend({
			buildDesignHelpers: function() {
				...
				_.isEmpty(hotspot_image) || ( container_element.find("> .mcmsb_element_wrapper .mcmsb_column_container").attr("data-hotspot-image", hotspot_image), container_element.find("> .mcmsb_element_wrapper .mcmsb_column_container").attr("data-hotspot-size", hotspot_size), container_element.find("> .mcmsb_element_wrapper .mcmsb_column_container").attr("data-hotspot-custom", hotspot_custom_size), container_element.attr("data-hotspot-image", hotspot_image), container_element.attr("data-hotspot-size", hotspot_size), container_element.attr("data-hotspot-custom", hotspot_custom_size), $.ajax({
					type: "POST",
					url: window.ajaxurl,
					data: {
						action: "mcmsb_single_image_src",
					...
					success: function(img_link){
						container_element.find("> .mcmsb_element_wrapper .mcmsb_column_container").attr("data-hotspot-link", img_link);
