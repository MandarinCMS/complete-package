...
class BSF_Envato_Activate {
	...
	public function envato_register( $args ) {
		...
		if ( ! empty( $bundled ) ) {
			...
			if ( $is_active ) {
				...
				$html  .=  '<p>' . sprintf( 
							'This module is came bundled with the <i>%1$s</i>. For receiving updates, you need to activate license of <i>%2$s</i> <a href="%3$s">here</a>.', 
							$parent_name, 
							$parent_name, 
							$registration_page 
						). '</p>';
		...
		if ( $is_active != true ) {
			$form_action = get_api_site() . 'envato-validation-callback/?mcms-envato-validate';
		} else {
	...
	public function envato_activation_url( $form_data ) {
		...
		$url 				= get_api_site() . 'envato-validation-callback/?mcms-envato-validate';
