...
add_action( 'mcms_ajax_bsf_dismiss_notice', 'bsf_dismiss_notice');
...
if(!class_exists('Ultimate_Admin_Area')){
	class Ultimate_Admin_Area{
		function __construct(){
			...
			add_action( 'mcms_ajax_update_ultimate_options', array($this,'update_settings'));
			add_action( 'mcms_ajax_update_ultimate_debug_options', array($this,'update_debug_settings'));
			add_action( 'mcms_ajax_update_ultimate_modules', array($this,'update_modules'));
			add_action( 'mcms_ajax_update_css_options', array($this,'update_css_options'));
			add_action( 'mcms_ajax_update_dev_notes', array($this,'update_dev_notes'));
		...
		function bsf_admin_scripts_updater($hook){
			if(defined('OPN_VERSION')) {
				echo "<style>
					@font-face {
						font-family: 'opn';
						src:url('".modules_url( 'fonts/opn.eot', __FILE__ )."');
						src:url('".modules_url( 'fonts/opn.eot', __FILE__ )."') format('embedded-opentype'),
							url('".modules_url( 'fonts/opn.woff', __FILE__ )."') format('woff'),
							url('".modules_url( 'fonts/opn.ttf', __FILE__ )."') format('truetype'),
							url('".modules_url( 'fonts/opn.svg', __FILE__ )."') format('svg');
						font-weight: normal;
						font-style: normal;
					}
					.toplevel_page_opn-settings > div.mcms-menu-image:before {
			...
			echo "
				<style>
					@font-face {
						...
						src:url('".modules_url( 'fonts/ultimate.eot', __FILE__ )."');
						src:url('".modules_url( 'fonts/ultimate.eot', __FILE__ )."') format('embedded-opentype'),
							url('".modules_url( 'fonts/ultimate.woff', __FILE__ )."') format('woff'),
							url('".modules_url( 'fonts/ultimate.ttf', __FILE__ )."') format('truetype'),
							url('".modules_url( 'fonts/ultimate.svg', __FILE__ )."') format('svg');
						font-weight: normal;
						font-style: normal;
					}
					.toplevel_page_about-ultimate > div.mcms-menu-image:before {
						content: \"\\e600\" !important;
			...
			if($hook == "post.php" ||
				...
				mcms_register_style("ultimate-admin-style", UAVC_URL."admin/css/style.css");

				mcms_register_style("ultimate-chosen-style", UAVC_URL.'admin/vc_extend/css/chosen.css');
				mcms_register_script("ultimate-chosen-script", UAVC_URL."admin/vc_extend/js/chosen.js");

				mcms_register_script("ultimate-vc-backend-script", UAVC_URL."admin/js/ultimate-vc-backend.min.js",array('jquery'),ULTIMATE_VERSION,true);
				mcms_register_style("ultimate-vc-backend-style", UAVC_URL.'admin/css/ultimate-vc-backend' . $css_ext);

				if($bsf_dev_mode === 'enable') {
					mcms_enqueue_style('ultimate-admin-style');
				}
				else {
					mcms_enqueue_style( 'mcms-color-picker' );
					mcms_enqueue_script('ultimate-vc-backend-script');
					mcms_enqueue_style('ultimate-vc-backend-style');
				}
			}

			mcms_localize_script( 'jquery', 'uavc', array(
				'add_zipped_font'        => mcms_create_nonce( 'smile-add-zipped-fonts-nonce' ),
				'remove_zipped_font'     => mcms_create_nonce( 'smile-remove-zipped-fonts-nonce' ),
				'get_font_variants'      => mcms_create_nonce( 'uavc-get-font-variants-nonce' ),
				'ult_get_attachment_url' => mcms_create_nonce( 'uavc-get-attachment-url-nonce' ),
			) );
		}/* end admin_scripts */
		...
		function load_modules(){
			require_once(module_dir_path(__FILE__).'/modules.php');
		}

		function load_dashboard(){
			require_once(module_dir_path(__FILE__).'/dashboard.php');
		}

		function load_about() {
			require_once(module_dir_path(__FILE__).'/about.php');
		}

		function load_smoothscroll() {
			require_once(module_dir_path(__FILE__).'/smooth-scroll-setting.php');
		}

		function load_scripts_styles() {
			require_once(module_dir_path(__FILE__).'/script-styles.php');
		}

		function product_license() {
			require_once(module_dir_path(__FILE__).'/product-license.php');
		}

		function load_debug_settings() {
			require_once(module_dir_path(__FILE__).'/debug.php');
		}

		function ultimate_resources() {
			$connects = false;
			require_once(module_dir_path(__FILE__).'/resources.php');
		}

		function ultimate_google_maps_dashboard() {
			require_once(module_dir_path(__FILE__).'/map-settings.php');
		}
		...
		function update_debug_settings() {
			...
			if(isset($_POST['ultimate_ajax_myskin'])){
				$ultimate_ajax_myskin = sanitize_text_field( $_POST['ultimate_ajax_myskin'] );
			} else {
				$ultimate_ajax_myskin = 'disable';
			}
			$result2 = update_option('ultimate_ajax_myskin',$ultimate_ajax_myskin);
			...
			if(isset($_POST['ultimate_myskin_support'])){
				$ultimate_myskin_support = sanitize_text_field( $_POST['ultimate_myskin_support'] );
			} else {
				$ultimate_myskin_support = 'disable';
			}
			$result4 = update_option('ultimate_myskin_support',$ultimate_myskin_support);
		...
		/*
		* Display admin notices for module activation
		*/
				...
				if ( $hook_suffix == 'modules.php' ){
					...
					?>
                        ...
                        .ult_activate .ult_description strong{color:#0096A3;font-weight:normal}
                                ...
                                <div class="ult_activate">
                                    ...
                                    <div class="ult_description"><h3 style="margin:0;padding: 2px 0px;"><strong><?php _e('Almost done!','ultimate_vc'); ?></strong></h3><p style="margin: 0;"><?php _e('Please activate your copy of the Ultimate Addons for RazorLeaf Conductor to receive automatic updates & get premium support','ultimate_vc'); ?></p></div>
                                </div>
                        </div>
					<?php
					endif;
					...
					?>
                        ...
                        <div class="updated fade">
                            ...
                            <p><?php echo _e('Howdy! Please','ultimate_vc').' <a href="'.$reg_link.'">'.__('activate your copy','ultimate_vc').' </a> '.__('of the Ultimate Addons for RazorLeaf Conductor to receive automatic updates & get premium support.','ultimate_vc');?>
...
// Alternative function for mcms_remote_get
