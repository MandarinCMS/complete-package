...
if(!class_exists('WooComposer_Cat_Carousel')){
	class WooComposer_Cat_Carousel
	{
		...
		function woocomposer_carousel_shortcode($atts){
			...
			// get terms and workaround MCMS bug with parents/pad counts
			$args = array(
				'orderby'    => $orderby,
				'order'      => $order,
			...
			if ( $parent !== "" ) {
				$product_categories = mcms_list_filter( $product_categories, array( 'parent' => $parent ) );
			}
			...
			if ( $product_categories ) {
				...
				foreach ( $product_categories as $category ) {
					$vc_span = '';
					if($columns == "2") $vc_span = 'vc_span6 mcmsb_column column_container';
					elseif($columns == "3") $vc_span = 'vc_span4 mcmsb_column column_container';
					elseif($columns == "4") $vc_span = 'vc_span3 mcmsb_column column_container';
