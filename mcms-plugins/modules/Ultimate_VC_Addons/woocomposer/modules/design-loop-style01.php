...
function WooComposer_Loop_style01($atts,$element){
	...
	if($query->have_posts()):
		while ( $query->have_posts() ) : $query->the_post();
			...
			$uid = uniqid();
			$output .= '<div id="product-'.$uid.'" style="'.$style.'" class="'.$class.' mcmsb_column column_container wooproduct" data-animation="animated '.$product_animation.'">';
			if($element == 'carousel'){
			...
			$product_img = mcms_get_attachment_image_src( get_post_thumbnail_id($product_id),$image_size);
				...
				$output .= "\n\t".'<div class="wcmp-product-image">';
					if(empty($attachment_ids) && count($attachment_ids) > 1 && $product_img_disp == "carousel"){
						...
						foreach($attachment_ids as $attachment_id){
							$product_img = mcms_get_attachment_image_src( $attachment_id,$image_size);
							if($lazy_images == "enable"){
								$src = UAVC_URL.'assets/img/loader.gif';
							} else {
								$src = $product_img[0];
