...
if(!class_exists("WooComposer")){
	class WooComposer{
		...
		{
			$this->module_dir = module_dir_path( __FILE__ ).'modules/';
			add_action('admin_enqueue_scripts',array($this,'admin_scripts'));
			add_action('mcms_enqueue_scripts',array($this,'front_scripts'),1);
		...
		function generate_shortcode_params(){
			...
			if(defined('MCMSB_VC_VERSION') && version_compare(MCMSB_VC_VERSION, 4.8) >= 0) {
				...
				{
					vc_add_shortcode_param('woocomposer', array($this,'woo_query_builder'), modules_url("admin/js/mapping.js",__FILE__));
				...
				if(function_exists('add_shortcode_param')) {
					add_shortcode_param('woocomposer', array($this,'woo_query_builder'), modules_url("admin/js/mapping.js",__FILE__));
		...
		function woo_query_builder($settings, $value)
		{
			...
			$output .= mcms_dropdown_categories( array('taxonomy'=>'product_cat','selected'=>$cat_id,'echo' => false,)).'</div>';
			$output .= '<!-- '.$value.' -->';
			$output .= "<input type='hidden' name='".$param_name."' value='".$value."' class='mcmsb_vc_param_value ".$param_name." ".$type." ".$class."' id='shortcode'>";
			return $output;
		...
		function woo_product_search($settings, $value){
			...
			$products_array = new MCMS_Query(array(
								'post_type' => 'product',
								'posts_per_page' => -1,
								'post_status' => 'publish'
							));
			$output = '';
			$output .= '<select id="products" name="'.$param_name.'" class="mcmsb_vc_param_value '.$param_name.' '.$type.' '.$class.'">';
			...
			// 					if ( jQuery(".mcmsb_el_type_product_search #products").data("select2") ) {
			// 						jQuery(".mcmsb_el_type_product_search #products").select2("destroy");
			// 					}

			// 					// WooComposer is depracated.
			// 					// jQuery(".mcmsb_el_type_product_search #products").select2({
		...
		function woo_product_categories($settings, $value){
			...
			$output .= "<input type='hidden' name='".$param_name."' value='".$value."' class='mcmsb_vc_param_value ".$param_name." ".$type." ".$class."' id='sel_cat'>";
			...
			if($hook == "post.php" || $hook == "post-new.php"){
				if(defined('WOOCOMMERCE_VERSION') && version_compare( '2.1.0', WOOCOMMERCE_VERSION, '<' )) {
					...
					if($bsf_dev_mode === 'enable') {
						mcms_register_style("woocomposer-admin",modules_url("admin/css/admin.css",__FILE__));
						mcms_register_style("woocomposer-select2-bootstrap",modules_url("admin/css/select2-bootstrap.css",__FILE__));
						mcms_register_style("woocomposer-select2",modules_url("admin/css/select2.css",__FILE__));
						mcms_enqueue_style("woocomposer-admin");

						// Disable WooComposer select2 js to break incompatibility with WooCommerce 3.0.0 +
						// mcms_enqueue_style("woocomposer-select2-bootstrap");
						// mcms_enqueue_style("woocomposer-select2");						
						// mcms_register_script("woocomposer-select2-js",modules_url("admin/js/select2.js",__FILE__),array('jquery'),'',true);
						// mcms_enqueue_script("woocomposer-select2-js");
					}
					else {

						// Disable WooComposer select2 js to break incompatibility with WooCommerce 3.0.0 +
						// mcms_register_style("woocomposer-admin-style",modules_url("admin/css/ultimate-woocomposer-backend.min.css",__FILE__));
						// mcms_enqueue_style("woocomposer-admin-style");

						// mcms_enqueue_script("woocomposer-admin-script",modules_url("admin/js/ultimate-woocomposer-backend.min.js",__FILE__),array('jquery'),'',true);
						// mcms_enqueue_style("woocomposer-admin-script");
					}
				}
			}
		} /* end admin scripts */
		function front_scripts($post)
		{
			if(!is_404() && !is_search() && !is_archive()){
				...
				if ( NULL !== $post ) {
					...
					mcms_register_script('woocomposer-unveil',modules_url('assets/js/unveil.js',__FILE__),array('jquery'),WOOCOMPOSER_VERSION,true);
					mcms_register_script('woocomposer-slick',modules_url('assets/js/slick.js',__FILE__),array('jquery'),WOOCOMPOSER_VERSION,true);
					mcms_register_script('woocomposer-js',modules_url('assets/js/custom.js',__FILE__),array('jquery','woocomposer-slick'),WOOCOMPOSER_VERSION,true);

					mcms_register_style('woocomposer-front',modules_url('assets/css/style.css',__FILE__),array(),WOOCOMPOSER_VERSION);
					mcms_register_style('woocomposer-front-wooicon',modules_url('assets/css/wooicon.css',__FILE__),array(),WOOCOMPOSER_VERSION);
					mcms_register_style('woocomposer-front-slick',modules_url('assets/css/slick.css',__FILE__),array(),WOOCOMPOSER_VERSION);
					mcms_register_style('woocomposer-animate',modules_url('assets/css/animate.min.css',__FILE__),array(),WOOCOMPOSER_VERSION);

					mcms_register_script('woocomposer-script',modules_url('assets/js/woocomposer.min.js',__FILE__),array('jquery'),WOOCOMPOSER_VERSION,true);
					mcms_register_style('woocomposer-style',modules_url('assets/css/woocomposer.min.css',__FILE__),array(),WOOCOMPOSER_VERSION);
					...
					if(defined('WOOCOMMERCE_VERSION') && version_compare( '2.1.0', WOOCOMMERCE_VERSION, '<' ) && $count !== 0) {
						...
						if($ultimate_css == "enable" && $bsf_dev_mode !== 'enable'){
							mcms_enqueue_style("woocomposer-style");
						}
						else {
							mcms_enqueue_style("woocomposer-front");
							mcms_enqueue_style("woocomposer-front-wooicon");
							mcms_enqueue_style("woocomposer-front-slick");
							mcms_enqueue_style("woocomposer-animate");
						}

						$ultimate_js = get_option('ultimate_js');
						mcms_enqueue_script('jquery');
						if($ultimate_js == 'enable') {
							mcms_enqueue_script("woocomposer-script");
						}
						else {
							mcms_enqueue_script("woocomposer-unveil");
							mcms_enqueue_script("woocomposer-slick");
							mcms_enqueue_script("woocomposer-js");
						}
					}
				}
	...
	function woocomposer_admin_notice_for_version()
	{
		...
		if(($is_multisite && $is_network_admin) || !$is_multisite)
			echo '<div class="updated"><p>'.__('The','ultimate_vc').' <strong>WooComposer</strong> '.__('module requires','ultimate_vc').' <strong>RazorLeaf Conductor</strong> '.__('version 3.7.2 or greater.','ultimate_vc').'</p></div>';
	}
	function woocomposer_admin_notice_for_vc_activation()
	{
		...
		$is_network_admin = is_network_admin();
		if(($is_multisite && $is_network_admin) || !$is_multisite)
			echo '<div class="updated"><p>'.__('The','ultimate_vc').' <strong>WooComposer</strong> '.__('module requires','ultimate_vc').' <strong>RazorLeaf Conductor</strong> '.__('Module installed and activated.').'</p></div>';
	}
}
