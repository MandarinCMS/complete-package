...
if(!class_exists('ULT_HotSpot')) {
	class ULT_HotSpot {
		...
		function __construct() {
			...
			// Register CSS and JS
			add_action( 'mcms_enqueue_scripts', array( $this, 'ult_hotspot_scripts' ), 1 );
		...
		function ultimate_hotspot_param_callback($settings, $value)
		{
			...
			$output = '<div class="ult-hotspot-image-wrapper '.esc_attr($class).'">';
				...
				$output .= '<input type="hidden" name="'.esc_attr($settings['param_name']).'" value="'.esc_attr($value).'" class="ult-hotspot-positions mcmsb_vc_param_value" '.$dependency.'/>';
			$output .= '</div>';
			return $output;
		}
		...
		function enqueue_admin_assets($hook)
		{
			if($hook == "post.php" || $hook == "post-new.php" || $hook == "edit.php"){
				...
				if($bsf_dev_mode === 'enable') {
					mcms_register_script('hotspt-admin-js', UAVC_URL.'admin/vc_extend/js/admin_enqueue_js.js',array( 'jquery', 'jquery-ui-core', 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ),ULTIMATE_VERSION,true);
					mcms_enqueue_script('hotspt-admin-js');
				}
			}
		}
		...
		function ult_hotspot_callback( $atts, $content = null ) {
			...
			$content = mcmsb_js_remove_mcmsautop($content, true); // fix unclosed/unwanted paragraph tags in $content
		...
		function ult_hotspot_items_callback( $atts, $content = null ){
		   global $hotspot_icon, $hotspot_icon_bg_color, $hotspot_icon_color, $hotspot_icon_size;
			...
			//$content = mcmsb_js_remove_mcmsautop($content, false); // fix unclosed/unwanted paragraph tags in $content
			...
			if($tooltip_position!='')       { $data .= 'data-arromcmsosition="'.esc_attr($tooltip_position).'"';}
				...
				vc_map( array(
					  ...
					  "params" => array(
							...
							array(
							  ...
							  "value" => "#4c4c4c",
							  /*"dependency" => array("element" => "tooltip_myskin", "value" => "custom"),*/
							  "group" => "Tooltip",
							  "dependency" => Array("element" => "link_style","value" => "tooltip"),
	...
	if ( class_exists( 'MCMSBakeryShortCodesContainer' ) && !class_exists( 'MCMSBakeryShortCode_ult_hotspot' ) ) {
	  class MCMSBakeryShortCode_ult_hotspot extends MCMSBakeryShortCodesContainer {
	  }
	}
	if ( class_exists( 'MCMSBakeryShortCode' ) && !class_exists( 'MCMSBakeryShortCode_ult_hotspot_items' ) ) {
	  class MCMSBakeryShortCode_ult_hotspot_items extends MCMSBakeryShortCode {
	  }
	}
