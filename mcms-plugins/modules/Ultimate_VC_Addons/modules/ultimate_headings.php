...
if(!class_exists("Ultimate_Headings")){
	class Ultimate_Headings{
		...
		function __construct(){
			...
			add_action("mcms_enqueue_scripts", array($this, "register_headings_assets"),1);
		}
				...
				vc_map(
					array(
					   ...
					   "params" => array(
							...
							array(
								"type" => "ult_param_heading",
								"text" => "<span style='display: block;'><a href='http://jiiworks.net/8v9sy' target='_blank' rel='noopener'>".__("Watch Video Tutorial","ultimate_vc")." &nbsp; <span class='dashicons dashicons-video-alt3' style='font-size:30px;vertical-align: middle;color: #e52d27;'></span></a></span>",
		...
		function ultimate_headings_shortcode($atts, $content = null){
			...
			else if($spacer == 'image_only')
			{
				...
				$spacer_inline = '';
				//$icon_inline = mcms_get_attachment_image( $spacer_img, $siwidth, false, array("class"=>"ultimate-headings-icon-image") );
	...
	if(class_exists('MCMSBakeryShortCode') && !class_exists('MCMSBakeryShortCode_ultimate_heading'))
	{
		class MCMSBakeryShortCode_ultimate_heading extends MCMSBakeryShortCode {
		}
	}
}