...
Add-on URI: https://jiiworks.com/demos/parallax/
...
if(!class_exists('VC_Ultimate_Parallax')){
	class VC_Ultimate_Parallax{
		function __construct(){
			...
			/*add_action('mcms_enqueue_scripts',array($this,'front_scripts'),9999); */
			add_action('admin_init',array($this,'parallax_init'));
			$ultimate_row = get_option('ultimate_row');
			//echo MCMSB_VC_VERSION;
			if($ultimate_row == "enable"){
				if(defined('MCMSB_VC_VERSION') && version_compare( MCMSB_VC_VERSION, '4.4', '>=' )) {
		...
		public static function parallax_shortcode($atts, $content){
			...
			if((get_option('ultimate_row') == "enable" && $bg_type !== "") || $parallax_content != '' || $fadeout_row != ''){
				...
				if($ultimate_js != 'enable') :
					...
					{
						//mcms_enqueue_script('ultimate-row-bg',modules_url('../assets/min-js/',__FILE__).'ultimate_bg.min.js');
						//mcms_enqueue_script('ultimate-custom');
					}
					else if($bg_type != 'no_bg' && ($parallax_content != '' || $fadeout_row != ''))
					{
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ultimate-row-bg', $js_path.'ultimate_bg'.$ext.'.js');
						mcms_enqueue_script('ultimate-custom');
					}
					else if($bg_type != 'no_bg' && ($parallax_content == '' || $fadeout_row == ''))
					{
				...
				$ultimate_myskin_support = get_option('ultimate_myskin_support');

				if(defined('MCMSB_VC_VERSION'))
					$vc_version = MCMSB_VC_VERSION;
				...
				$commom_data_attributes .= ' data-myskin-support="'.esc_attr($ultimate_myskin_support).'" ';
				...
				elseif($bg_type == "image"){
					...
					if($parallax_style == 'vcpb-fs-jquery' || $parallax_style=="vcpb-mlvp-jquery"){
						...
						foreach ($imgs as $value) {
							$layer_image[] = mcms_get_attachment_image_src($value,'full');
						}
				...
				elseif ($bg_type=='u_iframe') {
					//mcms_enqueue_script('jquery.tublar',modules_url('../assets/js/tubular.js',__FILE__));
		...
		function parallax_init(){
			...
			$modulename = dirname(dirname(module_basename( __FILE__ )));

			$patterns_path = realpath(dirname(module_dir_path(__FILE__)).'/assets/images/patterns');
			...
			/*
			$separator_path = realpath(dirname(module_dir_path(__FILE__)).'/assets/images/separator');
			$separator_list = glob($separator_path.'/*.*');
			$separator_imgs = array();

			foreach($separator_list as $separator)
				$separator_imgs[basename($separator)] = modules_url().'/'.$modulename.'/assets/images/separator/'.basename($separator);*/
			if(function_exists('vc_add_param')){
				...
				vc_add_param('vc_row',array(
						...
						"value" => "",
						/*"description" => __("Enter your video URL. You can upload a video through <a href='".home_url()."/mcms-admin/media-new.php' target='_blank'>MandarinCMS Media Library</a>, if not done already.", "upb_parallax"),*/
				...
				vc_add_param('vc_row',array(
						...
						"description" => __("By default, the background will be given to the RazorLeaf Conductor row. However, in some cases depending on your myskin's CSS - it may not fit well to the container you are wishing it would. In that case you will have to select the appropriate value here that gets you desired output..", "ultimate_vc"),
				...
				vc_add_param('vc_row',array(
						"type" => "number",
						"class" => "",
						"heading" => __("Viemcmsort Position", "ultimate_vc"),
				...
				vc_add_param(
					...
					array(
						...
						'options' => $patterns,
						/*'options' => array(
							'image-1' => modules_url('../assets/images/patterns/01.png',__FILE__),
							'image-2' => modules_url('../assets/images/patterns/12.png',__FILE__),
						),*/
						'css' => array(
							'width' => '40px',
							'height' => '35px',
				...
				vc_add_param(
					...
					array(
						...
						'text' => __('In order for Effects below to work, you must select something except "default" in background tab.','ultimate_vc').' '.__('May be single color.').' <br> '.__('Screenshot','ultimate_vc').' - <a href="https://jiiworks.net/cc1J8ZlcdZW" target="_blank" rel="noopener">https://jiiworks.net/cc1J8ZlcdZW</a>',
		...
		function admin_scripts($hook){
			if($hook == "post.php" || $hook == "post-new.php"){
				mcms_register_script("ult-colorpicker",UAVC_URL."admin/js/jquery-colorpicker.js ",array('jquery'),ULTIMATE_VERSION);
				mcms_register_script("ult-classygradient",UAVC_URL."admin/js/jquery-classygradient-min.js",array('jquery'),ULTIMATE_VERSION);
				...
				$bsf_dev_mode = bsf_get_option('dev_mode');
				if($bsf_dev_mode === 'enable') {
					mcms_enqueue_script('ult-colorpicker');
					mcms_enqueue_script('ult-classygradient');

					mcms_enqueue_style('ult-classycolorpicker-style');
					mcms_enqueue_style('ult-classygradient-style');
				}
				//mcms_enqueue_style("ultimate-admin-style",modules_url("../admin/css/style.css",__FILE__));
			}
		}/* end admin_scripts */
		static function front_scripts(){
			/* mcms_enqueue_script('jquery.video_bg',modules_url('../assets/js/ultimate_bg.js',__FILE__),'1.0',array('jQuery')); */
			$bsf_dev_mode = bsf_get_option('dev_mode');
			if($bsf_dev_mode === 'enable') {
				$js_path = '../assets/js/';
				$css_path = '../assets/css/';
			...
			if($ultimate_css != "enable" || $bsf_dev_mode === 'enable') {
				...
				mcms_enqueue_style( 'ult-background-style' );
			}
		} /* end front_scripts */
	}
...
$ultimate_row = get_option('ultimate_row');
if((defined('MCMSB_VC_VERSION') && (!version_compare( MCMSB_VC_VERSION, '4.4', '>=' ))) && $ultimate_row == "enable"){
	if ( !function_exists( 'vc_myskin_after_vc_row' ) ) {
		function vc_myskin_after_vc_row($atts, $content = null) {
