...
if(!class_exists('Ultimate_Google_Font_Manager'))
{
	class Ultimate_Google_Font_Manager
	{
		function __construct()
		{
			...
			add_action('mcms_ajax_ultimate_google_fonts_refresh', array($this, 'refresh_google_fonts_list'));
			add_action('mcms_ajax_get_google_fonts', array($this, 'get_google_fonts_list'));
			add_action('mcms_ajax_add_google_font', array($this, 'add_selected_google_font'));
			add_action('mcms_ajax_delete_google_font', array($this, 'delete_selected_google_font'));
			add_action('mcms_ajax_update_google_font', array($this, 'update_selected_google_font'));
			add_action('mcms_ajax_get_font_variants', array($this, 'get_font_variants_callback'));
		...
		{
			mcms_register_script('ultimate-google-fonts-script', UAVC_URL.'admin/js/google-fonts-admin.js',array('jquery'));
			mcms_enqueue_script('ultimate-google-fonts-script');
			mcms_localize_script( 'jquery', 'uavc', array(
				'google_font_nonce'   => mcms_create_nonce( 'uavc-google-font-nonce' ),
				'get_google_fonts'    => mcms_create_nonce( 'uavc-get-google-fonts-nonce' ),
				'add_google_fonts'    => mcms_create_nonce( 'uavc-add-google-fonts-nonce' ),
				'delete_google_fonts' => mcms_create_nonce( 'uavc-delete-google-fonts-nonce' ),
				'update_google_fonts' => mcms_create_nonce( 'uavc-update-google-fonts-nonce' ),
			) );

			Ultimate_VC_Addons::ultimate_register_style( 'ultimate-google-fonts-style', UAVC_URL.'admin/css/google-fonts-admin.css', true );

			mcms_enqueue_style('ultimate-google-fonts-style');
		}
		function enqueue_selected_ultimate_google_fonts() {
			...
			if(!empty($selected_fonts)) {
				...
				mcms_register_style('ultimate-selected-google-fonts-style',$font_api_call, array(), null);
				mcms_enqueue_style('ultimate-selected-google-fonts-style');
			}
		}
		...
		function refresh_google_fonts_list()
		{
			...
			if($error == true || count($fonts) == 0)
			{
				$error = false;
				try{
					$fonts =	mcms_remote_get($filename = 'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyD_6TR2RyX2VRf8bABDRXCcVqdMXB5FQvs');
			...
			{
				$response['count'] = 0;
				$response['message'] = __('Fonts could not be downloaded as there might be some issue with file_get_contents or mcms_remote_get due to your server configuration.','ultimate_vc');
			}
			...
			foreach($enqueue_fonts as $key => $efont)
			{
				...
				if(isset($font_arr[1]))
				{
					...
					if(isset($font_call_arr[1]) && $font_call_arr[1] != '')
					{
						...
						if (!mcms_script_is( 'ultimate-'.$eq_name, 'registered' ))
						{
							mcms_register_style('ultimate-'.$eq_name,$link);
							mcms_enqueue_style('ultimate-'.$eq_name);
						}
						else if(mcms_script_is( 'ultimate-'.$eq_name, 'registered' ))
						{
							mcms_enqueue_style('ultimate-'.$eq_name);
						}
					}
...
if(!function_exists('enquque_ultimate_google_fonts_optimzed'))
{
	function enquque_ultimate_google_fonts_optimzed($enqueue_fonts)
	{
		...
		if(!empty($enqueue_fonts))
		{
			...
			if($font_count > 0)
				mcms_enqueue_style('ultimate-google-fonts', $font_api_call, array(), null);
		}
	}
}