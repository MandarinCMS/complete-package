<?php
/*
* Add-on Name: Swatch Book for RazorLeaf Conductor
* Add-on URI: http://.jiiworks.com/demos/ultimate/swatch-book
*/
if(!class_exists('Ultimate_Swatch_Book')){
	class Ultimate_Swatch_Book{
		...
		function __construct(){
			...
			add_action("mcms_enqueue_scripts", array($this, "register_swatch_assets"),1);
 ...
 = $swatch_width = $swatch_height = $swatch_trans_bg_img = $swatch_main_strip_text = $swatch_main_strip_highlight_text = $swatch_main_strip_font_size = $swatch_main_strip_font_style = $swatch_main_strip_color = $swatch_main_strip_highlight_font_size = $swatch_main_strip_highlight_font_weight = $swatch_main_strip_highlight_color = $swatch_main_strip_bg_color = $main_strip_font_family = $main_strip_font_style = '';
			...
			if($swatch_trans_bg_img !== ''){
				// $img = mcms_get_attachment_image_src( $swatch_trans_bg_img, 'large');
				// $img = $img[0];
		...
		function swatch_item($atts,$content=null){
			...
			if($this->swatch_trans_bg_img !== ''){
				// $img = mcms_get_attachment_image_src( $this->swatch_trans_bg_img, 'large');
				// $img = $img[0];
...
{
	if(!class_exists('MCMSBakeryShortCode_swatch_container'))
	{
		class MCMSBakeryShortCode_swatch_container extends MCMSBakeryShortCodesContainer {
	...
	if(!class_exists('MCMSBakeryShortCode_swatch_item'))
	{
		class MCMSBakeryShortCode_swatch_item extends MCMSBakeryShortCode {
			function content( $atts, $content = null ) {
	            global $Ultimate_Swatch_Book;
