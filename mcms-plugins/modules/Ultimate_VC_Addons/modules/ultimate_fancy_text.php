...
if(!class_exists('Ultimate_FancyText')){
	class Ultimate_FancyText{
		...
		function __construct(){
			...
			add_action('mcms_enqueue_scripts', array($this, 'register_fancytext_assets'),1);
		}
				...
				vc_map(
					array(
					   ...
					   "params" => array(
							...
							array(
								"type" => "ult_param_heading",
								"text" => "<span style='display: block;'><a href='http://jiiworks.net/t5ir4' target='_blank' rel='noopener'>".__("Watch Video Tutorial","ultimate_vc")." &nbsp; <span class='dashicons dashicons-video-alt3' style='font-size:30px;vertical-align: middle;color: #e52d27;'></span></a></span>",
			...
			$output = '<'.$fancytext_tag.' id="'.esc_attr( $fancy_text_id ).'" '.$data_list.' class="uvc-type-wrap '.esc_attr($css_design_style).' '.esc_attr($is_vc_49_plus).' ult-responsive '.esc_attr($ex_class).' uvc-wrap-'.esc_attr($id).'" style="'.esc_attr($string_inline_style).'">';
				...
				if($fancytext_effect == 'ticker' || $fancytext_effect == 'ticker-down')
				{
					if($ultimate_js != 'enable')
						mcms_enqueue_script('ultimate-easy-ticker-js');
					if($strings_font_size != '')
				...
				{
					if($ultimate_js != 'enable')
						mcms_enqueue_script('ultimate-typed-js');
	...
	if(class_exists('MCMSBakeryShortCode') && !class_exists('MCMSBakeryShortCode_ultimate_fancytext'))
	{
		class MCMSBakeryShortCode_ultimate_fancytext extends MCMSBakeryShortCode {
		}
	}
