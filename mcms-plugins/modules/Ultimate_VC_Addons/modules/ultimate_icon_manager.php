...
if ( ! class_exists( 'AIO_Icon_Manager' ) ) {
	class AIO_Icon_Manager {
		...
		function __construct() {
			...
			$this->vc_fonts_dir     = module_dir_path( __FILE__ ) . '../assets/fonts/';
			//font file extract by ajax function
			add_action( 'mcms_ajax_smile_ajax_add_zipped_font', array( $this, 'add_zipped_font' ) );
			add_action( 'mcms_ajax_smile_ajax_remove_zipped_font', array( $this, 'remove_zipped_font' ) );
		...
		function admin_scripts() {
			...
			mcms_enqueue_script( 'aio-admin-media', $this->admin_js . 'admin-media.js', array( 'jquery' ) );
			
			mcms_enqueue_script( 'media-upload' );
			mcms_enqueue_media();
			mcms_enqueue_style( 'aio-icon-manager-admin', $this->admin_css . 'icon-manager-admin.css' );
		...
		public function add_zipped_font() {
			...
			$cap = apply_filters( 'avf_file_upload_capability', 'update_modules' );
		...
		function zip_flatten( $zipfile, $filter ) {
			@ini_set( 'memory_limit', apply_filters( 'admin_memory_limit', MCMS_MAX_MEMORY_LIMIT ) );
		...
		function create_config() {
			...
			//$response 	= mcms_remote_get( $this->paths['tempurl'].$this->svg_file );
			$response = mcms_remote_fopen( trailingslashit( $this->paths['tempurl'] ) . $this->svg_file );
			//if mandarincms wasnt able to get the file which is unlikely try to fetch it old school
			...
			if ( ! is_mcms_error( $json ) && ! empty( $json ) ) {
		...
		function delete_all_fonts() {
			...
			delete_option( 'smile_fonts' );
			mcms_redirect( admin_url( 'admin.php?page=bsf-font-icon-manager' ) );
			exit;
		}
	...
	/*
	* creates a folder for the myskin framework
	*/
	if ( ! function_exists( 'smile_backend_create_folder' ) ) {
		function smile_backend_create_folder( &$folder, $addindex = true ) {
			...
			$created = mcms_mkdir_p( trailingslashit( $folder ) );
			@chmod( $folder, 0777 );
			if ( $addindex == false ) {
				return $created;
			}
...
add_action( 'mcms_ajax_smile_ajax_add_zipped_font', array( $AIO_Icon_Manager, 'add_zipped_font' ) );
add_action( 'mcms_ajax_smile_ajax_remove_zipped_font', array( $AIO_Icon_Manager, 'remove_zipped_font' ) );