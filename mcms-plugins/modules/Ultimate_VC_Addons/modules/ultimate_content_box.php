  ...
  if(!class_exists('Ult_Content_Box')) {
	class Ult_Content_Box {
		function __construct() {
			...
			add_action( 'mcms_enqueue_scripts', array( $this, 'ult_content_box_scripts' ), 1 );
		}
		  	...
		  	if($bg_type!='') {
		  		switch ($bg_type) {
		  			...
		  			case 'bg_image': 	if($bg_image != '') {
											$img = mcms_get_attachment_image_src( $bg_image, 'large');
	...
	new Ult_Content_Box;
		if ( class_exists( 'MCMSBakeryShortCodesContainer' ) && !class_exists( 'MCMSBakeryShortCode_ult_content_box' ) ) {
			class MCMSBakeryShortCode_ult_content_box extends MCMSBakeryShortCodesContainer {
			}
		}
  }
