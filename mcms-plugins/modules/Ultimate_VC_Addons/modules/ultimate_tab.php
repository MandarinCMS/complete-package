...
* Add-on URI: http://dev.jiiworks.com
*/
...
if(!class_exists('ULT_TAB_ELEMENT'))
{
...

	{
		function __construct()
		{
			...
			add_action( 'mcms_enqueue_scripts', 'ultimate_tabs',1 );
...
function ultimate_single_tab($atts ,$content = null)
 ...
 //echo $content = mcmsb_js_remove_mcmsautop($content, true);
			global $tabarr;
			...
			$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'mcmsb_tab ui-tabs-panel mcmsb_ui-tabs-hide vc_clearfix ult_back','single_tab', $atts );
...
$element = 'mcmsb_tabs';
//if ( 'vc_tour' == $this->shortcode ) $element = 'mcmsb_tour';
...
foreach ($tabarr as $key => $value) {
	...
	$accontaint=mcmsb_js_remove_mcmsautop( $accontaint );
...
$newtabcontain .=mcmsb_js_remove_mcmsautop( $content );
$newtabcontain .='</div>';
$newtabcontain .='</div>';
$op='';
...
$op .=mcmsb_js_remove_mcmsautop( $content );
$op .='</div>';
$op .='</div>';
$op .='</div>';
//$op .='</div>';
...
function ult_tab_init() {
	...
	$settings = array(
		...
		"category" => __("Ultimate Conductor","ultimate_vc"),
		...
		'icon' => 'icon-mcmsb-ui-tab-content ult_tab_eleicon',
		'params'                  => array(
...
'custom_markup' => '<div class="mcmsb_tabs_holder mcmsb_holder vc_container_for_children">
...
vc_map( array(
	...
	//'admin_enqueue_js'  => preg_replace( '/\s/', '%20', modules_url( 'vc_extend/js/single_element_js.js', __FILE__ ) ),
	...
	'params' => array(
		...
		array(
			'type' => 'textfield',
			"edit_field_class" => " vc_col-sm-12 vc_column mcmsb_el_type_textfield vc_shortcode-param",
...
function ultimate_tabs_admin() {
	...
	if($screen_id !== 'post')
		return false;
	mcms_register_script('tab-js-1', UAVC_URL.'admin/vc_extend/js/ult_tab_admin_enqueue_js.js',array( 'jquery'),ULTIMATE_VERSION,true);
	mcms_register_script('tab-js-2', UAVC_URL.'admin/vc_extend/js/single_element_js.js',array( 'jquery'),ULTIMATE_VERSION,true);

	mcms_enqueue_script('tab-js-1');
	mcms_enqueue_script('tab-js-2');
	//mcms_enqueue_style('css-1',modules_url( '../admin/vc_extend/css/sub-tab.css', __FILE__ ));
}
}
...
if ( class_exists( "MCMSBakeryShortCode" ) ) {
		// Class Name should be MCMSBakeryShortCode_Your_Short_Code
		// See more in vc_composer/includes/classes/shortcodes/shortcodes.php
		class MCMSBakeryShortCode_ULT_TAB_ELEMENT extends MCMSBakeryShortCode {
		...
		public function contentAdmin( $atts, $content = null ) {
					...
					//$elem = str_ireplace('%mcmsb_element_content%', $iner, $elem);
			...
			public function getTabTemplate() {
					return '<div class="mcmsb_template">' . do_shortcode( '[single_tab title="Tab" tab_id=""   icon_type="" icon="" icon_color="" icon_hover_color="" icon_size="15px" icon_background_color=""][/single_tab]' ) . '</div>';
				}
...
if(class_exists('MCMSBakeryShortCode_VC_Column')){
class MCMSBakeryShortCode_SINGLE_TAB extends MCMSBakeryShortCode_VC_Column {
					...
					public function mainHtmlBlockParams( $width, $i ) {
						return 'data-element_type="' . esc_attr($this->settings["base"]) . '" class="mcmsb_' . esc_attr($this->settings['base']) . ' mcmsb_sortable mcmsb_content_holder"' . $this->customAdminBlockParams();
					}
