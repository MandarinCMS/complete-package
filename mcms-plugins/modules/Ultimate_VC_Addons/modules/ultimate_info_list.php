<?php
/*
* Add-on Name: Info List for RazorLeaf Conductor
...
if(!class_exists('AIO_Info_list'))
{
	class AIO_Info_list
	{
		...
		function info_list_item($atts,$content = null)
		{
			...
			//$content =  mcmsb_js_remove_mcmsautop($content);
			...
			$output .= '<div class="icon_description_text ult-responsive" '.$info_list_desc_data_list.' style="'.esc_attr($desc_style).'">'.mcmsb_js_remove_mcmsautop($content, true).'</div>';
			$output .= '</div>';
				...
				array(
				   ...
				   "params" => array(
						...
						array(
							...
							"text" => "<span style='display: block;'><a href='http://jiiworks.net/v9k0x' target='_blank' rel='noopener'>".__("Watch Video Tutorial","ultimate_vc")." &nbsp; <span class='dashicons dashicons-video-alt3' style='font-size:30px;vertical-align: middle;color: #e52d27;'></span></a></span>",
...
global $AIO_Info_list; // MCMSB: Beter to create singleton in AIO_Info_list I think, but it also work
if(class_exists('MCMSBakeryShortCodesContainer'))
{
	if ( ! class_exists( 'MCMSBakeryShortCode_info_list' ) ) {
		class MCMSBakeryShortCode_info_list extends MCMSBakeryShortCodesContainer {
	...
	if ( ! class_exists( 'MCMSBakeryShortCode_info_list_item' ) ) {
		class MCMSBakeryShortCode_info_list_item extends MCMSBakeryShortCode {
	        function content($atts, $content = null ) {
	            global $AIO_Info_list;
