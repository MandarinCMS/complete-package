...
if(!class_exists("Ultimate_Google_Maps")){
	class Ultimate_Google_Maps{
		function __construct(){
			...
			add_action('mcms_enqueue_scripts', array($this, 'ultimate_google_map_script'),1);
		}
		function ultimate_google_map_script()
		{
			...
			mcms_register_script("googleapis",$api,null,null,false);
		}
				...
				vc_map( array(
					...
					"params" => array(
						...
						array(
							...
							"value" => array(__("Use Google Default", "ultimate_vc") => "default", __("Use Module's Default", "ultimate_vc") => "default_self", __("Upload Custom", "ultimate_vc") => "custom"),
							"group" => "Marker"
						),
						array(
							"type" => "ult_img_single",
						...
						array(
							...
							"description" => __("By default, the map will be given to the RazorLeaf Conductor row. However, in some cases depending on your myskin's CSS - it may not fit well to the container you are wishing it would. In that case you will have to select the appropriate value here that gets you desired output..", "ultimate_vc"),
							"group" => "General Settings"
						),
						array(
						...
						array(
							...
							"text" => "<span style='display: block;'><a href='http://jiiworks.net/f57sh' target='_blank' rel='noopener'>".__("Watch Video Tutorial","ultimate_vc")." &nbsp; <span class='dashicons dashicons-video-alt3' style='font-size:30px;vertical-align: middle;color: #e52d27;'></span></a></span>",
		...
		function display_ultimate_map($atts,$content = null){
			...
			$output .= "<div id='".esc_attr($wrap_id)."' class='ultimate-map-wrapper ".esc_attr($is_vc_49_plus)." ".esc_attr($el_class)."' style='".esc_attr($gmap_design_css)." ".($map_height!="" ? "height:" . $map_height . ";" : "")."'><div id='" . esc_attr($id) . "' data-map_override='".esc_attr($map_override)."' class='ultimate_google_map mcmsb_content_element ".esc_attr($margin_css)."'" . ($width!="" || $map_height!="" ? " style='".esc_attr($border_css) . ($width!="" ? "width:" . esc_attr($width) . ";" : "") . ($map_height!="" ? "height:" . esc_attr($map_height) . ";" : "") . "'" : "") . "></div></div>";
	...
	if(class_exists('MCMSBakeryShortCode') && !class_exists('MCMSBakeryShortCode_ultimate_google_map'))
	{
		class MCMSBakeryShortCode_ultimate_google_map extends MCMSBakeryShortCode {
		}
	}
}