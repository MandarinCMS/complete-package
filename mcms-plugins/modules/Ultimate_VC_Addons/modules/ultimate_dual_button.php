<?php
/*
* Add-on Name: Just Dual Button for RazorLeaf Conductor
...
if(!class_exists('AIO_Dual_Button'))
{
	class AIO_Dual_Button
	{
		function __construct()
		{
			...
			add_action( 'mcms_enqueue_scripts', array( $this, 'dualbutton_scripts') );
		...
		function dualbutton_backend_scripts($hook){
			if($hook == "post.php" || $hook == "post-new.php"){
				...
				if($bsf_dev_mode === 'enable') {
					mcms_register_script("jquery_dualbtn_new",UAVC_URL.'admin/js/dualbtnbackend.js',array('jquery'),ULTIMATE_VERSION);
					mcms_enqueue_script('jquery_dualbtn_new');
				}
			}
		}
			...
			if(function_exists('vc_map'))
			{
				vc_map(
					array(
					  ...
					  // "front_enqueue_js" => array(modules_url("../assets/min-js/dual_button.min.js",__FILE__),array('jquery'),ULTIMATE_VERSION),
					  // "front_enqueue_js" =>  preg_replace( '/\s/', '%20', modules_url( '../admin/js/dualbtnfront.js', __FILE__ ) ),
...
if(class_exists('MCMSBakeryShortCode') && !class_exists('MCMSBakeryShortCode_ult_dualbutton'))
{
	class MCMSBakeryShortCode_ult_dualbutton extends MCMSBakeryShortCode {
	}
}
