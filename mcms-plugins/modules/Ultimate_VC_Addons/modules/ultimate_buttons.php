...
if(!class_exists("Ultimate_Buttons")){
	class Ultimate_Buttons{
		function __construct(){
			...
			add_action( "mcms_enqueue_scripts", array( $this, "advanced_button_scripts"),1 );
		}
		...
		function button_admin_scripts($hook){
			if($hook == "post.php" || $hook == "post-new.php"){
				...
				if($bsf_dev_mode === 'enable') {
					...
					$ext = '';
					mcms_enqueue_style( 'ult-button', $css_path.'advanced-buttons'.$ext.'.css' );
				}
			}
		}
		function ult_buttons_shortcode($atts){
			...
			if($enable_tooltip == "yes"){
				mcms_enqueue_script('ultimate-tooltip');
						...
						"params" => array(
							...
							*/
							...
							array(
								...
								"text" => "<span style='display: block;'><a href='http://jiiworks.net/0n-7p' target='_blank' rel='noopener'>".__("Watch Video Tutorial","ultimate_vc")." &nbsp; <span class='dashicons dashicons-video-alt3' style='font-size:30px;vertical-align: middle;color: #e52d27;'></span></a></span>",
	...
	if(class_exists('MCMSBakeryShortCode') && !class_exists('MCMSBakeryShortCode_ult_buttons'))
	{
		class MCMSBakeryShortCode_ult_buttons extends MCMSBakeryShortCode {
		}
	}
