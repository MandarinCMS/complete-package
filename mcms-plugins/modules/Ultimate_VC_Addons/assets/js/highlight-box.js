	...
	function resize_call_to_action()
	{
		$('.ultimate-call-to-action').each(function(i,element){
			...
			if(override != 0)
			{
				...
				if($(element).parents('.mcmsb_row').length > 0)
					var ancenstor = $(element).parents('.mcmsb_row');
				else if($(element).parents('.mcmsb_column').length > 0)
					var ancenstor = $(element).parents('.mcmsb_column');
				else
