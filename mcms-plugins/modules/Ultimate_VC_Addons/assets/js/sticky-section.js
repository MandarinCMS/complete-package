  ...
  var row_elements = $('div.ult-sticky-anchor');
    ...
    row_elements.each(function(){
      ...
      //if ( stick_behaviour == 'stick_with_scroll_row' ) {
        ...
        var parent = $this.closest(".mcmsb_column").closest('.vc_row');//parent();//$this.closest('.vc_row');
...
/*function inside_row () {
    ...
    row_elements.each(function(){
      ...
      //if ( stick_behaviour == 'stick_with_scroll_row' ) {
        ...
        gutter = ult_explode_offset( global_height, gutter, gutter_class, gutter_id );
        var parent = $this.closest(".mcmsb_column").parent();//$this.closest('.vc_row');
        ...
        if( _windowsize < 768 && mobile == 'no'){
          ...
          $this.stick_in_parent({parent:".ult_s_container", offset_top:gutter, spacer: ult_row_spacer})
            .on("sticky_kit:stick", function(e) {
              ...
              //console.log( $(e.target).closest(".mcmsb_column").parent().css("display", "flex") );
              $(e.target).css("top", "0");
              //console.log($(e.target).offset().top);
              $(e.target).closest(".mcmsb_column").parent().css("display", "flex");
            })
            .on("sticky_kit:unstick", function(e) {
              //console.log("has unstuck!", e.target);
              //$(e.target).closest(".vc_row").css("display", "")

              $(e.target).closest(".mcmsb_column").parent().css("display", "")
            })
...
/****** inside row end *******/
  ...
  function sticky_relocate() {
    ...
    elements.each(function() { 
      ...
      //   var parent_height = $this.closest(".mcmsb_column").parent().parent().innerHeight();
      //   var parent_offset = $this.closest(".mcmsb_column").parent().parent().offset().top;
     ...
     if ( stick_behaviour == 'stick_permanent' && !$this.hasClass('ult-permanent-flag') ) {
        ...
        //$this.parents('.mcmsb_column').css('min-height', '0px');
      }
