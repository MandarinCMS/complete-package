      ...
      if($content!='' && $content!='null|null') {
        ...
        if($data!='') {
          switch ($data) {
            case 'url':     // First  - Priority for ID
                            if( !empty($mainArr['id']) && $mainArr['id'] != 'null' ) {
                              ...
                              if( preg_match('/^\d/', $size) === 1 ) {
                                ...
                                //  resize image using vc helper function - mcmsb_resize
                                $img = mcmsb_resize( $mainArr['id'], null, $size[0], $size[1], true );
                                if ( $img ) {
                                ...
                                $hasImage = mcms_get_attachment_image_src( $mainArr['id'], $size ); // returns an array
            ...
            break;
            case 'title':
            	$final = isset($mainArr['title']) ? $mainArr['title'] : get_post_meta($mainArr['id'], '_mcms_attachment_image_title', true);
            break;
            case 'caption':
            	$final = isset($mainArr['caption']) ? $mainArr['caption'] : get_post_meta($mainArr['id'], '_mcms_attachment_image_caption', true);
            break;
            case 'alt':
            	$final = isset($mainArr['alt']) ? $mainArr['alt'] : get_post_meta($mainArr['id'], '_mcms_attachment_image_alt', true);
            break;
            case 'description':
            	$final = isset($mainArr['description']) ? $mainArr['description'] : get_post_meta($mainArr['id'], '_mcms_attachment_image_description', true);
            break;
            case 'json':
