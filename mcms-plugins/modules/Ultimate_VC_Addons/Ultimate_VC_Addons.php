<?php
/*
Module Name: Ultimate Addons for Conductor
Version: 3.16.20
Description: Includes conductor addon elements like Icon, Info Box, Interactive Banner, Flip Box, Info List & Counter. Best of all - provides A Font Icon Manager allowing users to upload / delete custom icon fonts.
...
if ( ! defined( 'ULTIMATE_URL' ) ) {
	define( 'ULTIMATE_URL', module_dir_url( __FILE__ ) );
}
...
if ( ! class_exists( 'Ultimate_VC_Addons' ) ) {
	...
	class Ultimate_VC_Addons {
		...
		function __construct() {
			...
			if ( ! defined( 'MCMSB_VC_VERSION' ) ) {
				add_action( 'admin_init', array( $this, 'init_addons' ) );
				return;
			}

			// Activation hook
			register_activation_hook( __FILE__, array( $this, 'uvc_module_activate' ) );

			$module = module_basename( __FILE__ );

			define( 'UAVC_DIR', module_dir_path( __FILE__ ) );
			define( 'UAVC_URL', modules_url('/', __FILE__ ) );
			...
			add_filter( 'module_action_links_' . $module, array( $this, 'ultimate_modules_page_link' ) );
			...
			add_action( 'after_setup_myskin', array( $this, 'aio_init' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'aio_admin_scripts' ) );
			add_action( 'mcms_enqueue_scripts', array( $this, 'aio_front_scripts' ), 99 );
			...
			add_action( 'mcms_head', array( $this, 'ultimate_init_vars' ) );
		...
		function uvc_module_activate() {
			...
			if ( $memory_limit - $peak_memory <= 14436352 && ! defined( 'MCMS_CLI' ) ) {
				$msg  = __('Unfortunately, module could not be activated as the memory allocated by your host has almost exhausted. <i>Ultimate Addons for RazorLeaf Conductor</i> module recommends that your site should have 15M PHP memory remaining. ', 'ultimate_vc');
				$msg .= '<br/><br/>' . __('Please check ', 'ultimate_vc') . '<a target="_blank" href="https://docs.jiiworks.com/increasing-memory-limit/">' . __('this article', 'ultimate_vc') . '</a> ';
				$msg .= __(' for solution or contact ', 'ultimate_vc') . '<a target="_blank" href="http://support.jiiworks.com">' . __(' support', 'ultimate_vc') . '</a>.';
				$msg .= '<br/><br/><a class="button button-primary" href="'.network_admin_url( 'modules.php' ). '">' . __('Return to Modules Page', 'ultimate_vc') . '</a>';

				deactivate_modules( module_basename( __FILE__ ) );
				mcms_die( $msg );
			}

			// myskin depend custom row class
			$myskins        = array(
				'X'         => 'x-content-band',
				'HighendMCMS' => 'vc_row',
			...
			$site_myskin    = mcms_get_myskin();
			$current_myskin = $site_myskin->get( 'Name' );

			if ( array_key_exists( $current_myskin, $myskins ) ) {

				if ( ! get_option( 'ultimate_custom_vc_row' ) || get_option( 'ultimate_custom_vc_row' ) == '' ) {
					update_option( 'ultimate_custom_vc_row', $myskins[ $current_myskin ] );
				}
			}
		...
		function init_addons() {
			...
			if ( version_compare( $required_vc, 'MCMSB_VC_VERSION', '>' ) ) {
		...
		function ultimate_modules_page_link( $links ) {
			return $links;
		}
		...
		function admin_notice_for_version() {
			...
			if ( ( $is_multisite && $is_network_admin ) || ! $is_multisite ) {
				echo '<div class="updated"><p>' . __( 'The', 'ultimate_vc' ) . ' <strong>Ultimate addons for RazorLeaf Conductor</strong> ' . __( 'module requires', 'ultimate_vc' ) . ' <strong>RazorLeaf Conductor</strong> ' . __( 'version 3.7.2 or greater.', 'ultimate_vc' ) . '</p></div>';
			}
		}
		...
		function admin_notice_for_vc_activation() {
			...
			$is_network_admin = is_network_admin();
			if ( ( $is_multisite && $is_network_admin ) || ! $is_multisite ) {
				echo '<div class="updated"><p>' . __( 'The', 'ultimate_vc' ) . ' <strong>Ultimate addons for RazorLeaf Conductor</strong> ' . __( 'module requires', 'ultimate_vc' ) . ' <strong>RazorLeaf Conductor</strong> ' . __( 'Module installed and activated.', 'ultimate_vc' ) . '</p></div>';
			}
		}
		...
		{
			load_module_textdomain('ultimate_vc', FALSE, dirname(module_basename(__FILE__)).'/languages/');
		}
		...
		function woocomposer_admin_notice_for_woocommerce(){
			echo '<div class="error"><p>'._('The','ultimate_vc').' <strong>WooComposer</strong> '.__('module requires','ultimate_vc').' <strong>WooCommerce</strong> '.__('module installed and activated with version greater than 2.1.0.', 'ultimate_vc').'</p></div>';
		}
		function aio_admin_scripts($hook)
		{
			...
			if($hook == "post.php" || $hook == "post-new.php" || $hook == 'visual-composer_page_vc-roles'){
				...
				if($bsf_dev_mode === 'enable') {
					mcms_enqueue_style('ult-animate',$this->assets_css.'animate.css');
					mcms_enqueue_style('aio-icon-manager',$this->admin_css.'icon-manager.css');
				}
				if(mcms_script_is( 'vc_inline_custom_view_js', 'enqueued' ))
					mcms_enqueue_script('vc-inline-editor',$this->assets_js.'vc-inline-editor.js',array('vc_inline_custom_view_js'),'1.5',true);
				...
				if(is_array($fonts))
				{
					foreach($fonts as $font => $info)
					{
						if(strpos($info['style'], 'http://' ) !== false) {
							mcms_enqueue_style('bsf-'.$font,$info['style']);
						} else {
							mcms_enqueue_style('bsf-'.$font,trailingslashit($this->paths['fonturl']).$info['style']);
						}
					}
				}
			}
		...
		public static function ultimate_register_style( $handle, $slug, $full_path = false, $deps = array(), $ver = ULTIMATE_VERSION ) {
			...
			mcms_register_style( $handle, $file_path, $deps, $ver );
		}
		...
		public static function ultimate_register_script( $handle, $slug, $full_path = false, $deps = array(), $ver = ULTIMATE_VERSION, $footer = true ) {
			...
			mcms_register_script( $handle, $file_path, $deps, $ver, $footer);
		}

		function aio_front_scripts()
		{
			$isAjax = false;
			$ultimate_ajax_myskin = get_option('ultimate_ajax_myskin');
			if($ultimate_ajax_myskin == 'enable')
				$isAjax = true;
			...
			// register js
			mcms_register_script('ultimate-script',UAVC_URL.'assets/min-js/ultimate.min.js', array('jquery', 'jquery-ui-core' ), ULTIMATE_VERSION, false);
			mcms_register_script('ultimate-appear', $js_path.'jquery-appear'.$ext.'.js',array('jquery'), ULTIMATE_VERSION);
			mcms_register_script('ultimate-custom', $js_path.'custom'.$ext.'.js',array('jquery'), ULTIMATE_VERSION);
			mcms_register_script('ultimate-vc-params', $js_path.'ultimate-params'.$ext.'.js',array('jquery'), ULTIMATE_VERSION);
			if($ultimate_smooth_scroll_compatible === 'enable') {
			...
			mcms_register_script('ultimate-smooth-scroll',UAVC_URL.'assets/min-js/'.$smoothScroll,array('jquery'),ULTIMATE_VERSION,true);
			mcms_register_script("ultimate-modernizr", $js_path.'modernizr-custom'.$ext.'.js',array('jquery'),ULTIMATE_VERSION);
			mcms_register_script("ultimate-tooltip", $js_path.'tooltip'.$ext.'.js',array('jquery'),ULTIMATE_VERSION);
			...
			if($ultimate_smooth_scroll == "enable" || $ultimate_smooth_scroll_compatible === 'enable') {
				...
				mcms_enqueue_script('ultimate-smooth-scroll');
				if($ultimate_smooth_scroll == "enable") {	
					mcms_localize_script( 'ultimate-smooth-scroll', 'php_vars', $options );
				}
			}

			if(function_exists('vc_is_editor')){
				if(vc_is_editor()){
					mcms_enqueue_style('vc-fronteditor',UAVC_URL.'assets/min-css/vc-fronteditor.min.css');
				}
			}
			$fonts = get_option('smile_fonts');
			if(is_array($fonts))
			...
			if($ultimate_global_scripts === 'enable') {
				mcms_enqueue_script('ultimate-modernizr');
				mcms_enqueue_script('jquery_ui');
				mcms_enqueue_script('masonry');
				if(defined('DISABLE_ULTIMATE_GOOGLE_MAP_API') && (DISABLE_ULTIMATE_GOOGLE_MAP_API == true || DISABLE_ULTIMATE_GOOGLE_MAP_API == 'true'))
					$load_map_api = false;
				else
					$load_map_api = true;
				if($load_map_api)
					mcms_enqueue_script('googleapis');
				/* Range Slider Dependecy */
				mcms_enqueue_script('jquery-ui-mouse');
				mcms_enqueue_script('jquery-ui-widget');
				mcms_enqueue_script('jquery-ui-slider');
				mcms_enqueue_script('ult_range_tick');
				/* Range Slider Dependecy */
				mcms_enqueue_script('ultimate-script');
				mcms_enqueue_script('ultimate-modal-all');
				mcms_enqueue_script('jquery.shake', $js_path.'jparallax'.$ext.'.js');
				mcms_enqueue_script('jquery.vhparallax', $js_path.'vhparallax'.$ext.'.js');

				mcms_enqueue_style('ultimate-style-min');
				mcms_enqueue_style("ult-icons");
				mcms_enqueue_style('ultimate-vidcons', UAVC_URL.'assets/fonts/vidcons.css');
				mcms_enqueue_script('jquery.ytplayer', $js_path.'mb-YTPlayer'.$ext.'.js');
			...
			if(!is_404() && !is_search()){
				...
				if(($ultimate_js == 'enable' || $isAjax == true) && ($bsf_dev_mode != 'enable') )
				{
					...
					if( stripos( $post_content, '[ult_range_slider') ) {
						...
						mcms_enqueue_script('jquery-ui-slider');
						mcms_enqueue_script('ult_range_tick');
						mcms_enqueue_script('ult_ui_touch_punch');
					}
					...
					if( stripos( $post_content, '[ultimate_modal') ) {
						...
						//if($modal_fixer === 'enable')
							//mcms_enqueue_script('ultimate-modal-all-switched');
						//else
							mcms_enqueue_script('ultimate-modal-all');
					}
				}
				...
				{
					mcms_enqueue_script('ultimate-vc-params');
					...
					if(
						...
						mcms_enqueue_script('ultimate-custom');
					}
					if(
						stripos( $post_content, '[just_icon')
						|| stripos( $post_content, '[ult_animation_block')
						...
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ultimate-custom');
					}
					if( stripos( $post_content, '[ultimate_heading') ) {
						mcms_enqueue_script("ultimate-headings-script");
					}
					if( stripos( $post_content, '[ultimate_carousel') ) {
						mcms_enqueue_script('ult-slick');
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ult-slick-custom');
					}
					if( stripos( $post_content, '[ult_countdown') ) {
						mcms_enqueue_script('jquery.timecircle');
						mcms_enqueue_script('jquery.countdown');
					}
					if( stripos( $post_content, '[icon_timeline') ) {
						mcms_enqueue_script('masonry');
					}
					if( stripos( $post_content, '[ultimate_info_banner') ) {
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('utl-info-banner-script');
					}
					if( stripos( $post_content, '[ultimate_google_map') ) {
						if(defined('DISABLE_ULTIMATE_GOOGLE_MAP_API') && (DISABLE_ULTIMATE_GOOGLE_MAP_API == true || DISABLE_ULTIMATE_GOOGLE_MAP_API == 'true'))
							$load_map_api = false;
					...
					if( stripos( $post_content, '[swatch_container') ) {
						mcms_enqueue_script('ultimate-modernizr');
						mcms_enqueue_script('swatchbook-js');
					}
					if( stripos( $post_content, '[ult_ihover') ) {
						mcms_enqueue_script('ult_ihover_js');
					}
					if( stripos( $post_content, '[ult_hotspot') ) {
						mcms_enqueue_script('ult_hotspot_tooltipster_js');
						mcms_enqueue_script('ult_hotspot_js');
					}
					if( stripos( $post_content, '[ult_content_box') ) {
						mcms_enqueue_script('ult_content_box_js');
					}
					if( stripos( $post_content, '[bsf-info-box') ) {
						mcms_enqueue_script('info_box_js');
					}
					if( stripos( $post_content, '[icon_counter') ) {
						mcms_enqueue_script('flip_box_js');
					}
					if( stripos( $post_content, '[ultimate_ctation') ) {
						mcms_enqueue_script('utl-ctaction-script');
					}
					if( stripos( $post_content, '[stat_counter') ) {
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ult-stats-counter-js');
						//mcms_enqueue_script('ult-slick-custom');
						mcms_enqueue_script('ultimate-custom');
						array_push($dependancy,'stats-counter-js');
					}
					if( stripos( $post_content, '[ultimate_video_banner') ) {
						mcms_enqueue_script('ultimate-video-banner-script');
					}
					if( stripos( $post_content, '[ult_dualbutton') ) {
						mcms_enqueue_script('jquery.dualbtn');

					}
					if( stripos( $post_content, '[ult_createlink') ) {
						mcms_enqueue_script('jquery.ult_cllink');
					}
					if( stripos( $post_content, '[ultimate_img_separator') ) {
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ult-easy-separator-script');
						mcms_enqueue_script('ultimate-custom');
					}

					if( stripos( $post_content, '[ult_tab_element') ) {
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('ult_tabs_rotate');
						mcms_enqueue_script('ult_tabs_acordian_js');
					}
					if( stripos( $post_content, '[ultimate_exp_section') ) {
						mcms_enqueue_script('jquery_ui');
						mcms_enqueue_script('jquery_ultimate_expsection');
					}

					if( stripos( $post_content, '[info_circle') ) {
						mcms_enqueue_script('jquery_ui');
						mcms_enqueue_script('ultimate-appear');
						mcms_enqueue_script('info-circle');
						//mcms_enqueue_script('info-circle-ui-effect');
					}
					...
					if( stripos( $post_content, '[ultimate_modal') ) {
						...
						if($bsf_dev_mode == true || $bsf_dev_mode == 'true') {
							mcms_enqueue_script('ultimate-modal-customizer');
							mcms_enqueue_script('ultimate-modal-classie');
							mcms_enqueue_script('ultimate-modal-froogaloop2');
							mcms_enqueue_script('ultimate-modal-snap-svg');
							mcms_enqueue_script('ultimate-modal');
						} else {
							mcms_enqueue_script('ultimate-modal-all');
						}
					}
					...
					if( stripos( $post_content, '[ult_sticky_section') ) {
						mcms_enqueue_script('ult_sticky_js');
						mcms_enqueue_script('ult_sticky_section_js');
					}

					if( stripos( $post_content, '[ult_team') ) {
						mcms_enqueue_script('ultimate-team');
					}
					...
					if( stripos( $post_content, '[ult_range_slider') ) {
						...
						mcms_enqueue_script('jquery-ui-slider');
						mcms_enqueue_script('ult_range_tick');
						mcms_enqueue_script('ult_range_slider_js');
						mcms_enqueue_script('ult_ui_touch_punch');
					}
				}
				...
				if($ultimate_css == "enable"){
					mcms_enqueue_style('ultimate-style-min');
					if( is_rtl() ) {
						mcms_enqueue_style( 'ult_hotspot_rtl_css' );
					}
					if( stripos( $post_content, '[ultimate_carousel') ) {
						mcms_enqueue_style("ult-icons");
					}
					...
					mcms_enqueue_style('ultimate-style');

					if( stripos( $post_content, '[ult_animation_block') ) {
						mcms_enqueue_style('ultimate-animate');
					}
					if( stripos( $post_content, '[icon_counter') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ult-flip-style');
					}
					if( stripos( $post_content, '[ult_countdown') ) {
						mcms_enqueue_style('ult-countdown');
					}
					if( stripos( $post_content, '[ultimate_icon_list') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ultimate-tooltip');
					}
					if( stripos( $post_content, '[ultimate_carousel') ) {
						mcms_enqueue_style("ult-slick");
						mcms_enqueue_style("ult-icons");
						mcms_enqueue_style("ultimate-animate");
					}
					if( stripos( $post_content, '[ultimate_fancytext') ) {
						mcms_enqueue_style('ultimate-fancytext-style');
					}
					if( stripos( $post_content, '[ultimate_ctation') ) {
						mcms_enqueue_style('utl-ctaction-style');
					}
					if( stripos( $post_content, '[ult_buttons') ) {
						mcms_enqueue_style( 'ult-btn' );
					}
					if( stripos( $post_content, '[ultimate_heading') ) {
						mcms_enqueue_style("ultimate-headings-style");
					}
					...
					if( stripos( $post_content, '[ult_ihover') ) {
						 mcms_enqueue_style( 'ult_ihover_css' );
					}
					if( stripos( $post_content, '[ult_hotspot') ) {
						mcms_enqueue_style( 'ult_hotspot_css' );
						mcms_enqueue_style( 'ult_hotspot_tooltipster_css' );
						if( is_rtl() ) {
							mcms_enqueue_style( 'ult_hotspot_rtl_css' );
						}
					}
					if( stripos( $post_content, '[ult_content_box') ) {
						mcms_enqueue_style('ult_content_box_css');
					}
					if( stripos( $post_content, '[bsf-info-box') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('info-box-style');
					}
					if( stripos( $post_content, '[info_circle') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('info-circle');
					}
					if( stripos( $post_content, '[ultimate_info_banner') ) {
						mcms_enqueue_style('utl-info-banner-style');
						mcms_enqueue_style('ultimate-animate');
					}
					if( stripos( $post_content, '[icon_timeline') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ultimate-timeline-style');
					}
					...
					if( $ib_found && !$ib_2_found ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ult-interactive-banner');
					}
					else if( !$ib_found && $ib_2_found ) {
						mcms_enqueue_style('ult-ib2-style');
					}
					else if($ib_found && $ib_2_found) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ult-interactive-banner');
					...
					if( stripos( $post_content, '[ultimate_modal') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ultimate-modal');
					}
					if( stripos( $post_content, '[ultimate_info_table') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style("ultimate-pricing");
					}
					...
					if( stripos( $post_content, '[swatch_container') ) {
						mcms_enqueue_style('swatchbook-css');
					}
					if( stripos( $post_content, '[stat_counter') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ult-stats-counter-style');
					}
					if( stripos( $post_content, '[ultimate_video_banner') ) {
						mcms_enqueue_style('ultimate-video-banner-style');
					}
					if( stripos( $post_content, '[ult_dualbutton') ) {
						mcms_enqueue_style('ult-dualbutton');
					}
					if( stripos( $post_content, '[ult_createlink') ) {
						mcms_enqueue_style('ult_cllink');
					}
					if( stripos( $post_content, '[ultimate_img_separator') ) {
						mcms_enqueue_style('ultimate-animate');
						mcms_enqueue_style('ult-easy-separator-style');
					}
					if( stripos( $post_content, '[ult_tab_element') ) {
						mcms_enqueue_style('ult_tabs');
						mcms_enqueue_style('ult_tabs_acordian');
					}
					if( stripos( $post_content, '[ultimate_exp_section') ) {
						mcms_enqueue_style('style_ultimate_expsection');
					}
					if( stripos( $post_content, '[ult_sticky_section') ) {
						mcms_enqueue_style('ult_sticky_section_css');
					}
					if( stripos( $post_content, '[ult_team') ) {
						mcms_enqueue_style('ultimate-team');
					}
					if( stripos( $post_content, '[ult_range_slider') ) {
						mcms_enqueue_style('ult_range_slider_css');
					}
				}
			}
		...
		function toggle_updater(){
			...
			if(get_option('ultimate_vc_addons_redirect') == true)
			{
				...
				if(!is_multisite()) :
					mcms_redirect(admin_url('admin.php?page=about-ultimate'));
				endif;
			}
    ...
    add_action( 'modules_loaded', 'uavc_module_init' );

    function uavc_module_init() {
