...
if(!class_exists('Ultimate_ColorPicker_Param'))
{
	class Ultimate_ColorPicker_Param
	{
		...
		function colorpicker_alpha_gen($settings, $value)
		{
			...
			$output = '
                <input id="alpha_val'. esc_attr( $uni ).'" class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . ' vc_column_alpha" value="'. esc_attr( $value ).'" '.$dependency.' data-uniqid="'.esc_attr( $uni ).'" data-opacity="'.esc_attr( $opacity ).'" data-hex-code="'.esc_attr( $base ).'"/>
';
			$output .= '
<input class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr($type) . ' ' . esc_attr($class) . '" '.$dependency.' name="'.esc_attr($param_name).'" value="'.esc_attr($value).'" style="display:none"/>
