...
if(!class_exists('Ultimate_DateTime_Picker_Param'))
{
	class Ultimate_DateTime_Picker_Param
	{
		...
		function datetimepicker($settings, $value)
		{
			...
			$output = '<div id="ult-date-time'.esc_attr( $uni ).'" class="ult-datetime"><input data-format="yyyy/MM/dd hh:mm:ss" readonly class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . '" name="' . esc_attr( $param_name ) . '" style="width:258px;" value="'. esc_attr( $value ).'" '.$dependency.'/><div class="add-on" >  <i data-time-icon="Defaults-calendar-o" data-date-icon="Defaults-calendar-o"></i></div></div>';
