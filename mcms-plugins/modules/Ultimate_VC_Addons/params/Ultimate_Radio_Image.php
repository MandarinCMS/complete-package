...
if(!class_exists('Ultimate_Radio_Image_Param'))
{
	class Ultimate_Radio_Image_Param
	{
		...
		function radio_image_settings_field($settings, $value)
		{
			...
			$output = '';
			$output = '<input id="radio_image_setting_val_'.esc_attr( $uni ).'" class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . ' '.esc_attr( $value ).' vc_ug_gradient" name="' . esc_attr( $param_name ) . '"  style="display:none"  value="'.esc_attr( $value ).'" '.$dependency.'/>';
