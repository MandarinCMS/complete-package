...
if(!class_exists('Ultimate_Navigation'))
{
	class Ultimate_Navigation
	{
		...
		function icon_settings_field($settings, $value)
		{
			...
			$output = '<input type="hidden" name="'.esc_attr( $param_name ).'" class="mcmsb_vc_param_value '.esc_attr( $param_name ).' '.esc_attr( $type ).' '.esc_attr( $class ).'" value="'.esc_attr( $value ).'" id="trace-'.esc_attr( $uid ).'"/>';
