...
if(!class_exists('Ult_Image_Single'))
{
  class Ult_Image_Single
  {
    function __construct()
    {
      ...
      add_action('mcms_ajax_ult_get_attachment_url', array($this, 'get_attachment_url_init') );
    }
    function get_attachment_url_init() {
      ...
      $id = $_POST['attach_id'];
      $thumb = mcms_get_attachment_image_src( $id, 'thumbnail' );
    ...
    function ult_img_single_callback($settings, $value)
    {
        ...
        $html .= '  <input type="hidden" name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ult-image_single-value '. esc_attr( $settings['param_name'] ).' '. esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.' />';
        $html .= '</div>';
      return $html;
    }
    ...
    function image_single_scripts() {
      mcms_enqueue_media();
      mcms_enqueue_style( 'ultimate_image_single_css', UAVC_URL.'admin/vc_extend/css/ultimate_image_single.css');
    }
  }
}
