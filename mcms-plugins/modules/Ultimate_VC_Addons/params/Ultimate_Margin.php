	...
	class Ultimate_Margin_Param
	{
		...
		function ultimate_margins_param($settings, $value)
		{
			...
			$html = '<div class="ultimate-margins">
						<input type="hidden" name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ultimate-margin-value '. esc_attr( $settings['param_name'] ).' '. esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.'/>';
