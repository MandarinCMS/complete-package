...
if(!class_exists('Ultimate_Border'))
{
  class Ultimate_Border
  {
    ...
    function ultimate_border_callback($settings, $value)
    {
        ...
        $html .= '      <input name="" class="ultimate-colorpicker cs-mcms-color-picker" data-id="border-color" type="text" value="" />';
        $html .= '    </div>';
        $html .= '  </div>';

        $html .= '  <input type="hidden" data-unit="'. esc_attr( $unit ) .'" name="'. esc_attr( $settings['param_name'] ) .'" class="mcmsb_vc_param_value ultimate-border-value '. esc_attr( $settings['param_name'] ).' '. esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.' />';
        $html .= '</div>';
      return $html;
    }
    ...
    function ultimate_border_param_scripts($hook) {
        mcms_register_style("ultimate-border-style", UAVC_URL.'admin/vc_extend/css/ultimate_border.css');

        if($hook == "post.php" || $hook == "post-new.php"){
          $bsf_dev_mode = bsf_get_option('dev_mode');
          if( $bsf_dev_mode === 'enable' ) {
            mcms_enqueue_style( 'mcms-color-picker' );
            mcms_enqueue_style( 'ultimate-border-style');
            mcms_enqueue_style( 'ultimate-chosen-style');

            mcms_enqueue_script('ultimate-chosen-script');
          }
        }
    }
  }
