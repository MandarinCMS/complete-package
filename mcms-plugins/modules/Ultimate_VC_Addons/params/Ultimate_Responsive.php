...
if(!class_exists('Ultimate_Responsive'))
{
	class Ultimate_Responsive
	{
		...
		function ultimate_responsive_callback($settings, $value)
		{
			...
			$html .= '  <input type="hidden" data-unit="'.esc_attr( $unit ).'"  name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ultimate-responsive-value '. esc_attr( $settings['param_name'] ).' '.esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.' />';
		...
		function ultimate_admin_responsive_param_scripts($hook) {
			if($hook == "post.php" || $hook == "post-new.php"){
				...
				if($bsf_dev_mode === 'enable') {
					...
					// mcms_register_style('ultimate_responsive_param_css', modules_url('../admin/vc_extend/css/ultimate_responsive.min.css', __FILE__ ));
					mcms_enqueue_style( 'ultimate_responsive_param_css');
				}
			}
		}
	}
