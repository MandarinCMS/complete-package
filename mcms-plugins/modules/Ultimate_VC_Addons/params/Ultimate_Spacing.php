...
if(!class_exists('Ult_Spacing'))
{
  class Ult_Spacing
  {
    ...
    function ultimate_spacing_callback($settings, $value)
    {
        ...
        $html .= '  <input type="hidden" data-unit="'.esc_attr( $unit ).'" name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ultimate-spacing-value '.esc_attr( $settings['param_name'] ).' '.esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.' />';
        $html .= '</div>';
      return $html;
    }
    ...
    function ultimate_spacing_param_scripts($hook) {
        if($hook == "post.php" || $hook == "post-new.php"){
          ...
          if($bsf_dev_mode === 'enable') {
            mcms_register_style( 'ultimate_spacing_css', UAVC_URL.'admin/vc_extend/css/ultimate_spacing.css');
            mcms_enqueue_style( 'ultimate_spacing_css');
          }
        }
    }
  }
