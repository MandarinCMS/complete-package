...
if(!class_exists('Ultimate_BoxShadow'))
{
  class Ultimate_BoxShadow
  {
    ...
    function ultimate_boxshadow_callback($settings, $value) {
        ...
        //$html .= '    <div class="label mcmsb_element_label">';
        //$html .=        $label;
        //$html .= '    </div>';
        ...
        if($enable_color) {
          ...
          $html .= '      <input name="" class="ultbs-colorpicker cs-mcms-color-picker" type="text" value="" />';
          $html .= '    </div>';
          $html .= '  </div>';
        }

        $html .= '  <input type="hidden" data-unit="'. esc_attr( $unit ) .'" name="'. esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ultbs-result-value '. esc_attr( $settings['param_name'] ) .' '. esc_attr( $settings['type'] ) .'_field" value="'. esc_attr( $value ) .'" '.$dependency.' />';
        $html .= '</div>';
      return $html;
    }
    ...
    function ultimate_boxshadow_param_scripts($hook) {
      if($hook == "post.php" || $hook == "post-new.php"){
        ...
        if($bsf_dev_mode === 'enable') {
          ...
          mcms_register_style( 'ultimate_boxshadow_param_css', UAVC_URL.'admin/vc_extend/css/vc_param_boxshadow.css');
          mcms_enqueue_style( 'ultimate_boxshadow_param_css');
          //mcms_register_script('ultimate_boxshadow_param_js',modules_url( '../admin/vc_extend/js/vc-box-shadow-param.js', __FILE__ ));
          //mcms_enqueue_style( 'ultimate_boxshadow_param_choosen_css', modules_url('../admin/vc_extend/css/chosen.css', __FILE__ ));
          //mcms_enqueue_script('ultimate_boxshadow_param_choosen_js',modules_url( '../admin/vc_extend/js/chosen.js', __FILE__ ));
        }
      }
    }
  }
