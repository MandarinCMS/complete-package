...
if(!class_exists('Ultimate_Gradient_Param'))
{
	class Ultimate_Gradient_Param
	{
		...
		function gradient_picker($settings, $value)
		{
			...
			//$output .= '<div class="mcmsb_element_label" style="margin-top: 10px;">'.__('Gradient Type','upb_parallax').'</div>
			...
			$output .= '<div class="mcmsb_element_label" style="margin-top: 10px;">'.__('Choose Colors','ultimate_vc').'</div>';
			$output .= '<div class="grad_hold" id="grad_hold'.esc_attr( $uni ).'"></div>';
			$output .= '<div class="grad_trgt" id="grad_target'.esc_attr( $uni ).'"></div>';

			$output .= '<input id="grad_val'.esc_attr( $uni ).'" class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . ' vc_ug_gradient" name="' . esc_attr( $param_name ) . '"  style="display:none"  value="'.esc_attr( $value ).'" '.$dependency.'/></div>';
							...
							jQuery('.vc_ug_control').each(function(){
								...
								jQuery(hid).ClassyGradient({
							        ...
							        onChange: function(stringGradient,cssGradient) {
							        	...
							        	if(jQuery(tid).parents('.mcmsb_el_type_gradient').css('display')=='none'){
											//jQuery(tid).val('');
											cssGradient='';
										}
										if(depend)
