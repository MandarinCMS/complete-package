...
if(!class_exists('Ultimate_Switch_Param'))
{
	class Ultimate_Switch_Param
	{
		...
		function checkbox_param($settings, $value){
			...
			if(is_array($options) && !empty($options)){
				foreach($options as $key => $opts){
					...
					$output .= '<div class="ult-onoffswitch">
							<input type="checkbox" name="'.esc_attr( $param_name ).'" value="'.esc_attr( $value ).'" '.$dependency.' class="mcmsb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . ' '.esc_attr( $dependency ).' ult-onoffswitch-checkbox chk-switch-'.esc_attr( $un ).'" id="switch'.esc_attr( $uid ).'" '.$checked.'>
			...
			//$output .= '<input type="hidden" id="chk-switch-'.$un.'" class="mcmsb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" value="'.$value.'" />';
