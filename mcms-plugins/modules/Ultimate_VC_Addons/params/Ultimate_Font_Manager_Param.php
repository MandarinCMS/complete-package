	...
	class Ultimate_Font_Manager_Param
	{
		...
		function ultimate_google_fonts_settings($settings, $value)
		{
			...
			$html = '<div class="ultimate_google_font_param_block">';
				$html .= '<input type="hidden" name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value vc-ultimate-google-font '.esc_attr( $settings['param_name'] ).' '.esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.'/>';
		...
		{
			$dependency = '';
			$html = '<input type="hidden" name="'.esc_attr( $settings['param_name'] ).'" class="mcmsb_vc_param_value ugfont-style-value '.esc_attr( $settings['param_name'] ).' '. esc_attr( $settings['type'] ).'_field" value="'.esc_attr( $value ).'" '.$dependency.'/>';
