...
class JIICP_license extends JIICP {
  // hook things up
  static function init() {
    if (is_admin()) {
      add_filter('pre_set_site_transient_update_modules', array(__CLASS__, 'update_filter'));
      add_filter('modules_api', array(__CLASS__, 'update_details'), 100, 3);
    }
  } // init


  // get module info for lightbox
  static function update_details($result, $action, $args) {
    ...
    $module = 'JW_maintenance-mode';

    if ($action != 'module_information' || empty($args->slug) || ($args->slug != $module)) {
      return $result;
    }

    if(empty($response) || is_mcms_error($response)) {
      $request_params = array('sslverify' => false, 'timeout' => 15, 'redirection' => 2);
      $request_args = array('action' => 'module_information',
    ...
    if (is_mcms_error($response) || !mcms_remote_retrieve_body($response)) {
      $res = new MCMS_Error('modules_api_failed', __('An unexpected HTTP error occurred during the API request.', 'JW_maintenance-mode'), $response->get_error_message());
    } else {
      $res = json_decode(mcms_remote_retrieve_body($response), false);

      if (!is_object($res)) {
        $res = new MCMS_Error('modules_api_failed', __('Invalid API respone.', 'JW_maintenance-mode'), mcms_remote_retrieve_body($response));
      } else {
  ...
  // get info on new module version if one exists
  static function update_filter($current) {
    ...
    static $response = false;
    $options = parent::get_options();
    $module = 'JW_maintenance-mode/under-construction.php';
    ...
    if (!is_mcms_error($response) && mcms_remote_retrieve_body($response)) {
      $data = json_decode(mcms_remote_retrieve_body($response), false);
      if (empty($current)) {
  ...
  static function query_licensing_server($action, $data = array(), $method = 'GET', $array_response = true) {
    ...
    if (is_mcms_error($response) || !($body = mcms_remote_retrieve_body($response)) || !($result = @json_decode($body, $array_response))) {
