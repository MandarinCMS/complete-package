	...
	if(!$no_sliders){
		...
		foreach($arrSliders as $slider){
			...
			try{
				...
				$strSource = __("Gallery",'thunderslider');
				...
				$slider_type = 'gallery';
				if($isFromPosts == true){
					$strSource = __('Posts','thunderslider');
					$preicon ="revicon-doc";
				...
				$editLink = self::getViewUrl(ThunderSliderAdmin::VIEW_SLIDER,"id=$id");
				
				$editSlidesLink = self::getViewUrl(ThunderSliderAdmin::VIEW_SLIDE,"id=$first_slide_id");

				$showTitle = ThunderSliderFunctions::getHtmlLink($editLink, $showTitle);
			...
			<li class="tls-slide tls-stype-all tls-stype-<?php echo $slider_type; ?>" data-favorit="<?php echo ($is_favorite) ? 'a' : 'b'; ?>" data-id="<?php echo $id; ?>" data-name="<?php echo $title; ?>" data-type="<?php echo $slider_type; ?>">
				...
				<div class="tls-hover-metas">
					...
					<span class="button-primary rs-embed-slider" ><i class="eg-icon-plus"></i><?php _e("Embed Slider",'thunderslider'); ?></span>
					<?php if(!RS_DEMO){ ?>
						<a class="button-primary  export_slider_overview" id="export_slider_<?php echo $id; ?>" href="javascript:void(0);" ><i class="revicon-export"></i><?php _e("Export",'thunderslider'); ?></a>
						<?php
						$operations = new ThunderSliderOperations();
						$general_settings = $operations->getGeneralSettingsValues();
						
						$show_dev_export = ThunderSliderBase::getVar($general_settings, 'show_dev_export', 'off');
						
						if($show_dev_export == 'on'){
							?>
							<a class="button-primary  export_slider_standalone" id="export_slider_standalone_<?php echo $id; ?>" href="javascript:void(0);" ><i class="revicon-export"></i><?php _e("Export to HTML",'thunderslider'); ?></a>
							<?php
						}
						?>
					<?php } ?>
					<a class="button-primary  button_delete_slider" id="button_delete_<?php echo $id; ?>" href='javascript:void(0)'><i class="revicon-trash"></i><?php _e("Delete",'thunderslider'); ?></a>
					<a class="button-primary  button_duplicate_slider" id="button_duplicate_<?php echo $id; ?>" href='javascript:void(0)'><i class="revicon-picture"></i><?php _e("Duplicate",'thunderslider'); ?></a>
					<div id="button_preview_<?php echo $id; ?>" class="button_slider_preview button-primary revgray"><i class="revicon-search-1"></i><?php _e("Preview",'thunderslider'); ?></div>
				</div>
	...
	<li class="tls-slide tls-addnewslider">
		<a href='<?php echo $addNewLink; ?>'>
			<span class="tls-main-metas">
				...
				<span class="tls-title-wrapper">			
					<span class="tls-title"><?php _e("New Slider",'thunderslider'); ?></span>					
				</span>
			</span>
		</a>
	</li>
	<li class="tls-slide tls-addnewslider">
		<a href="javascript:void(0);" id="button_import_template_slider">
			<span class="tls-main-metas">
				...
				<span class="tls-title-wrapper">			
					<span class="tls-title"><?php _e("Add Slider From Template",'thunderslider'); ?></span>					
				</span>
			</span>
		</a>
	</li>
	<?php if(!ThunderSliderFunctionsMCMS::isAdminUser() && apply_filters('thunderslider_restrict_role', true)){ }else{ ?>
		<li class="tls-slide tls-addnewslider">
			<a href="javascript:void(0);" id="button_import_slider">
				<span class="tls-main-metas">
					...
					<span class="tls-title-wrapper">			
						<span class="tls-title"><?php _e("Import Slider",'thunderslider'); ?></span>					
					</span>
				</span>
			</a>		
		</li>
