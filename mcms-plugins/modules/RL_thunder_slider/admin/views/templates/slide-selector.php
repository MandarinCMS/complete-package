...
if($the_slidertype == 'hero'){
	...
	$exists = ThunderSliderSlide::isSlideByID($active_slide);
	...
	<div class="inner_wrapper p10 boxsized">
		<ul class="list_slide_links">
			...
			<li class="<?php echo $staticclass; ?> eg-drag-disabled">
				...
				if(!$slide->isStaticSlide()){
					?>
					<a href="<?php echo self::getViewUrl(ThunderSliderAdmin::VIEW_SLIDE,"id=static_$sID"); ?>" class="add_slide">
					<?php
				}
				...
				<div class="slide-link-content alwaysbluebg" style="background:#3498DB !important; color:#fff">
					<span class="slide-link" style="width:100%;text-align: center;"><?php _e("Static / Global Layers",'thunderslider'); ?></span>
				</div>
				<?php 
				if(!$slide->isStaticSlide()){
					?>
				...
				if($slide->isStaticSlide()){
					...
					?>
						<span style="position:absolute; top:13px;left:0px; text-align: center">
							<span class="setting_text_3"><?php _e("Show Layers from Slide:",'thunderslider'); ?></span>
							<select name="rev_show_the_slides">
								...
								foreach($all_slides as $c_slide){
									...
									?>
									<option value="<?php echo $c_slide->getID(); ?>"><?php echo stripslashes(ThunderSliderFunctions::getVal($c_params, 'title', 'Slide')).' (ID: '.$c_slide->getID().')'; ?></option>
									<?php
								}
								?>
							</select>
			...
			foreach($arrSlides as $t_slide) {
				...
				$titleclass = '';
				$c_topclass = '';
				$urlEditSlide = self::getViewUrl(ThunderSliderAdmin::VIEW_SLIDE,"id=$slidelistID");
					...
					<div class="slide-link-content">		
						...
						<div class="slide-link-toolbar">
							...
							if($slidelistID != $slideID && !in_array($slideID, $arrChildrenIDs)){
								?>
								<a class="slide-link-toolbar-button slide-moveto" href="#"><span class=""><i class="eg-icon-forward"></i><span><?php _e("Copy / Move",'thunderslider'); ?></span></span></a>
								<?php
							}
							?>
							<a class="slide-link-toolbar-button slide-duplicate" href="#"><span class=""><i class="eg-icon-picture"></i><span><?php _e("Duplicate",'thunderslider'); ?></span></span></a>
							<a class="slide-link-toolbar-button slide-add-as-template" href="#"><span class=""><i class="eg-icon-menu"></i><span><?php _e("Add to Templates",'thunderslider'); ?></span></span></a>
							<a class="slide-link-toolbar-button slide-remove" href="#"><span class=""><i class="eg-icon-trash"></i><span><?php _e("Delete",'thunderslider'); ?></span></span></a>
						</div>
					</div>
			...
			<li class="eg-drag-disabled">
				<a href="javascript:void(0);" class="add_slide">
					...
					<div class="slide-link-content">
						<span class="slide-link" style="width:100%;text-align: center;font-weight:600;"><?php _e("Add Slide",'thunderslider'); ?></span>
					</div>
				</a>
				<div class="slide-link-content">
					<div class="slide-link-toolbar">
						<a id="link_add_slide" href="javascript:void(0);" class="slide-link-toolbar-button"><span class="slide-add"><i class="eg-icon-picture-1" style="margin-right:5px"></i><span><?php _e("Add Blank Slide", 'thunderslider'); ?></span></span></a>
						<a id="link_add_bulk_slide" href="javascript:void(0);" class="slide-link-toolbar-button"><span class="slide-add"><i class="eg-icon-picture" style="margin-right:5px"></i><span><?php _e("Add Bulk Slides", 'thunderslider'); ?></span></span></a>								
						<a id="rs_copy_slide_from_slider" href="javascript:void(0);" class="slide-link-toolbar-button">
							<span class="slide-copy-from-slider"><i class="eg-icon-menu" style="margin-right:5px"></i><span><?php _e("Add from Template", 'thunderslider'); ?></span></span>
						</a>
					</div>
					<span class="slide-link" style="text-align:center">
						<?php _e("Add Slide", 'thunderslider'); ?>
					</span>
				</div>
	...
	jQuery("document").ready(function() {
		...
		jQuery('.slidetitleinput').on("change",function() {
			...
			if(UniteAdminRev.sanitize_input(slide_title) == ''){
				alert('<?php _e('Slide name should not be empty', 'thunderslider'); ?>');
				return false;
			}
