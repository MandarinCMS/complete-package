	...
	$(document).ready(function(){
		//hover toggles buttons
			...
			$('.rs-reload-shop,#rev_check_version').click(function(){
				showWaitAMinute({fadeIn:300,text:thunder_slider_addon.please_wait_a_moment});
			});
		
		//click event to install module
			$(".rs-addon-not-installed").click(function(){
				showWaitAMinute({fadeIn:300,text:thunder_slider_addon.please_wait_a_moment});
				$this = $(this);
				//console.log("install process");
				$.ajax({
				url : thunder_slider_addon.ajax_url,
				type : 'post',
				data : {
					action : 'install_module',
					nonce: 	$('#ajax_thunder_slider_addon_nonce').text(),// The security nonce
					module: $this.data("module")
				},
				success : function( response ) {
					switch(response){
						case "0":
								showWaitAMinute({fadeOut:300,text:thunder_slider_addon.please_wait_a_moment});
		...
		//click event to activate module
		...
		//click event to deactivate module
