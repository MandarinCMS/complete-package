...
if(typeof(window.InlineShortcodeView) !== 'undefined'){
	...
	jQuery(window).on('vc_build',function(){
		vc.add_element_block_view.$el.find('[data-element="thunder_slider"]').click(function(){ rs_show_frontend_overlay = true; });
	});
	
	window.InlineShortcodeView_thunder_slider = window.InlineShortcodeView.extend({
		render: function() {
			rs_cur_vc_obj = this;
			var params = this.model.get('params');
			...
			window.InlineShortcodeView_thunder_slider.__super__.render.call(this);
		...
		update: function(model) {
			...
			window.InlineShortcodeView_thunder_slider.__super__.update.call(this,model);
...
function rs_vc_show_overlay(params){
	...
	thunderslider_is_vc = true; //set for the saving that we are visual composer

	jQuery('.mcmsb-element-edit-modal').hide(); //hide the normal VC window and use own (old vc version)
	jQuery('#vc_properties-panel').hide(); //hide the normal VC window and use own (new vc version)
	
	var thunderslider_vc_variables = {};
	
	thunderslider_vc_variables['alias'] = (typeof(params.alias) !== 'undefined') ? params.alias : '';
	thunderslider_vc_variables['order'] = (typeof(params.order) !== 'undefined') ? params.order : ''; //.replace(/\'/g, '"')
	...
	if(thunderslider_vc_variables['alias'] !== ''){ //only slider with alias
		jQuery('select[name="thunderslider-existing-slider"] option').each(function(){
			if(jQuery(this).val() == thunderslider_vc_variables['alias']){
		...
		if(thunderslider_vc_variables['order'] != ''){
			...
			if(!Array.isArray(thunderslider_vc_variables['order'])){
				thunderslider_vc_variables['order'] = thunderslider_vc_variables['order'].split(',');
			}
			...
			if(typeof(thunder_sliders_info) !== 'undefined'){
				for(var key in thunder_sliders_info){
					if(key == sel.data('sliderid')){
						...
						my_entry = jQuery.extend(true,{},my_entry, thunder_sliders_info[key]);																		
						var my_entries = [];
						var act_slide = -1;
						
						for(var okey in thunderslider_vc_variables['order']){
							for(var sl in my_entry){
								if(my_entry[sl]['slider_type'] == 'hero'){
									if(thunderslider_vc_variables['order'][okey] == my_entry[sl]['id']){
