...
class ThunderSliderSlide extends ThunderSliderElementsBase{
	...
	public function initByData($record){
		$record = apply_filters('thunderslider_slide_initByData', $record);
		...
		$layers = ThunderSliderFunctions::convertStdClassToArray($layers);
		...
		$imageID = ThunderSliderFunctions::getVal($params, "image_id");
		
		$imgResolution = ThunderSliderFunctions::getVal($params, 'image_source_type', 'full');
		
		//get image url and thumb url
		if(!empty($imageID)){
			$this->imageID = $imageID;
			
			$imageUrl = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($imageID, $imgResolution);
			if(empty($imageUrl)){
				$imageUrl = ThunderSliderFunctions::getVal($params, "image");
				
				$imgID = ThunderSliderBase::get_image_id_by_url($imageUrl);
				if($imgID !== false){
					$imageUrl = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($imgID, $imgResolution);
				}
			}
			
			$this->imageThumb = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($imageID,ThunderSliderFunctionsMCMS::THUMB_MEDIUM);
		...
		$this->imageFilepath = ThunderSliderFunctionsMCMS::getImagePathFromURL($this->imageUrl);
		$realPath = ThunderSliderFunctionsMCMS::getPathContent().$this->imageFilepath;
	...
	public function setImageByID($imageID, $size = 'full'){
		$a = apply_filters('thunderslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
		
		$imageUrl = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($a['imageID'], $a['size']);
		
		if(!empty($imageUrl)){
			$this->imageID = $a['imageID'];
			$this->imageUrl = $imageUrl;
			$this->imageThumb = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($a['imageID'],ThunderSliderFunctionsMCMS::THUMB_MEDIUM);
			$this->imageFilename = basename($this->imageUrl);
			$this->imageFilepath = ThunderSliderFunctionsMCMS::getImagePathFromURL($this->imageUrl);
			$realPath = ThunderSliderFunctionsMCMS::getPathContent().$this->imageFilepath;
	...
	public function setBackgroundType($new_param){
		$new_param = apply_filters('thunderslider_slide_setBackgroundType', $new_param, $this);
	 ...
	 */
	public function initBySlide(ThunderSliderSlide $slide){
		$slide = apply_filters('thunderslider_slide_initBySlide', $slide, $this);
	...
	public function initByStreamData($postData, $slideTemplate, $sliderID, $sourceType, $additions){
		$a = apply_filters('thunderslider_slide_initByStreamData', array('postData' => $postData, 'slideTemplate' => $slideTemplate, 'sliderID' => $sliderID, 'sourceType' => $sourceType, 'additions' => $additions), $this);
		...
		switch($a['sourceType']){
			...
			break;
			default:
				$return = apply_filters('thunderslider_slide_initByStreamData_sourceType', false, $a, $this);
				if($return === false)
					ThunderSliderFunctions::throwError(__("Source must be from Stream", 'thunderslider'));
			break;
		}
	}
	...
	private function initByFacebook($sliderID, $additions){
		$this->postData = apply_filters('thunderslider_slide_initByFacebook_pre', $this->postData, $sliderID, $additions, $this);
		
		//set some slide params
		$this->id = ThunderSliderFunctions::getVal($this->postData, 'id');
		
		$this->params["title"] = ThunderSliderFunctions::getVal($this->postData, 'name');
		
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = ThunderSliderFunctions::getVal($this->postData, 'link');
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			if($additions['fb_type'] == 'album'){
				$this->imageUrl = 'https://graph.facebook.com/'.ThunderSliderFunctions::getVal($this->postData, 'id').'/picture';
				$this->imageThumb = ThunderSliderFunctions::getVal($this->postData, 'picture');
			}else{
		...
		$this->postData = apply_filters('thunderslider_slide_initByFacebook_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByTwitter($sliderID, $additions){
		$this->postData = apply_filters('thunderslider_slide_initByTwitter_pre', $this->postData, $sliderID, $additions, $this);
		
		//set some slide params
		$this->id = ThunderSliderFunctions::getVal($this->postData, 'id');

		$this->params["title"] = ThunderSliderFunctions::getVal($this->postData, 'title');
		
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = 'https://twitter.com/'.$additions['twitter_user'].'/status/'.ThunderSliderFunctions::getVal($this->postData, 'id_str');
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
		}
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamtwitter' || $this->params["background_type"] == 'streamtwitterboth'){ //if image is choosen, use featured image as background
			$img_sizes = ThunderSliderBase::get_all_image_sizes('twitter');
			
			$imgResolution = ThunderSliderFunctions::getVal($this->params, 'image_source_type', reset($img_sizes));
			$this->imageID = ThunderSliderFunctions::getVal($this->postData, 'id');
			if(!isset($img_sizes[$imgResolution])) $imgResolution = key($img_sizes);
			
			$image_url_array = ThunderSliderFunctions::getVal($this->postData, 'media');
			$image_url_large = ThunderSliderFunctions::getVal($image_url_array, 'large');
			
			$img = ThunderSliderFunctions::getVal($image_url_large, 'media_url', '');
			$entities = ThunderSliderFunctions::getVal($this->postData, 'entities');
			
			if($img == ''){
				$image_url_array = ThunderSliderFunctions::getVal($entities, 'media');
				if(is_array($image_url_array) && isset($image_url_array[0])){
					if(is_ssl()){
						$img = ThunderSliderFunctions::getVal($image_url_array[0], 'media_url_https');
					}else{
						$img = ThunderSliderFunctions::getVal($image_url_array[0], 'media_url');
					}
					
				}
			}
			
			$urls = ThunderSliderFunctions::getVal($entities, 'urls');
			if(is_array($urls) && isset($urls[0])){
				$display_url = ThunderSliderFunctions::getVal($urls[0], 'display_url');
			...
			$image_url_array = ThunderSliderFunctions::getVal($entities, 'media');
			if(is_array($image_url_array) && isset($image_url_array[0])){
				$video_info = ThunderSliderFunctions::getVal($image_url_array[0], 'video_info');
				$variants = ThunderSliderFunctions::getVal($video_info, 'variants');
				if(is_array($variants) && isset($variants[0])){
					$mp4 = ThunderSliderFunctions::getVal($variants[0], 'url');

					$this->params["slide_bg_html_mpeg"] = $mp4; //set video for background video
				}
			}
			
			$entities = ThunderSliderFunctions::getVal($this->postData, 'extended_entities');
			if($img == ''){
				$image_url_array = ThunderSliderFunctions::getVal($entities, 'media');
				if(is_array($image_url_array) && isset($image_url_array[0])){
					if(is_ssl()){
		...
		$this->postData = apply_filters('thunderslider_slide_initByTwitter_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByInstagram($sliderID){
		$this->postData = apply_filters('thunderslider_slide_initByInstagram_pre', $this->postData, $sliderID, $this);
		...
		$caption = ThunderSliderFunctions::getVal($this->postData, 'caption');
		
		$this->params["title"] = ThunderSliderFunctions::getVal($caption, 'text');
		
		$link = ThunderSliderFunctions::getVal($this->postData, 'link');

		if(empty($link)) $link = 'https://www.instagram.com/p/' . ThunderSliderFunctions::getVal($this->postData, 'code');
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streaminstagram' || $this->params["background_type"] == 'streaminstagramboth'){ //if image is choosen, use featured image as background
			$img_sizes = ThunderSliderBase::get_all_image_sizes('instagram');

			$imgResolution = ThunderSliderFunctions::getVal($this->params, 'image_source_type', reset($img_sizes));
			if(!isset($img_sizes[$imgResolution])) $imgResolution = key($img_sizes);
			
			$this->imageID = ThunderSliderFunctions::getVal($this->postData, 'id');
			$imgs = ThunderSliderFunctions::getVal($this->postData, 'images', array());
			$is = array();
			...
			if(isset($is[$imgResolution])){
				...
				$this->imageUrl = ThunderSliderFunctions::getVal($this->postData, 'display_src');
				$this->imageThumb = ThunderSliderFunctions::getVal($this->postData, 'thumbnail_src');
			}
		...
		$videos = ThunderSliderFunctions::getVal($this->postData, 'videos');
		...
		$this->postData = apply_filters('thunderslider_slide_initByInstagram_post', $this->postData, $sliderID, $this);
	...
	private function initByFlickr($sliderID){
		$this->postData = apply_filters('thunderslider_slide_initByFlickr_pre', $this->postData, $sliderID, $this);
		...
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = 'http://flic.kr/p/'.$this->base_encode(ThunderSliderFunctions::getVal($this->postData, 'id'));
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
		}
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			...
			$img_sizes = ThunderSliderBase::get_all_image_sizes('flickr');
			...
			$is = @array(
				'square' 		=> 	ThunderSliderFunctions::getVal($this->postData, 'url_sq'),
				'large-square' 	=> 	ThunderSliderFunctions::getVal($this->postData, 'url_q'),
				'thumbnail' 	=> 	ThunderSliderFunctions::getVal($this->postData, 'url_t'),
				'small' 		=> 	ThunderSliderFunctions::getVal($this->postData, 'url_s'),
				'small-320' 	=> 	ThunderSliderFunctions::getVal($this->postData, 'url_n'),
				'medium' 		=> 	ThunderSliderFunctions::getVal($this->postData, 'url_m'),
				'medium-640' 	=> 	ThunderSliderFunctions::getVal($this->postData, 'url_z'),
				'medium-800' 	=> 	ThunderSliderFunctions::getVal($this->postData, 'url_c'),
				'large' 		=>	ThunderSliderFunctions::getVal($this->postData, 'url_l'),
				'original'		=>	ThunderSliderFunctions::getVal($this->postData, 'url_o')
			);
		...
		$this->postData = apply_filters('thunderslider_slide_initByFlickr_post', $this->postData, $sliderID, $this);
		//replace placeholders in layers:
	...
	private function initByYoutube($sliderID, $additions){
		$this->postData = apply_filters('thunderslider_slide_initByYoutube_pre', $this->postData, $sliderID, $additions, $this);
		
		//set some slide params
		$snippet = ThunderSliderFunctions::getVal($this->postData, 'snippet');
		$resource = ThunderSliderFunctions::getVal($snippet, 'resourceId');
		
		if($additions['yt_type'] == 'channel'){
			$link_raw = ThunderSliderFunctions::getVal($this->postData, 'id');
			$link = ThunderSliderFunctions::getVal($link_raw, 'videoId');
		}else{
			$link_raw = ThunderSliderFunctions::getVal($snippet, 'resourceId');
			$link = ThunderSliderFunctions::getVal($link_raw, 'videoId');
		}
		...
		switch($additions['yt_type']){
			case 'channel':
				$id = ThunderSliderFunctions::getVal($this->postData, 'id');
				$this->id = ThunderSliderFunctions::getVal($id, 'videoId');
			break;
			case 'playlist':
				$this->id = ThunderSliderFunctions::getVal($resource, 'videoId');
			break;
		}
		if($this->id == '') $this->id = 'not-found';
		
		$this->params["title"] = ThunderSliderFunctions::getVal($snippet, 'title');
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamyoutube' || $this->params["background_type"] == 'streamyoutubeboth'){ //if image is choosen, use featured image as background
			//facebook check which image size is choosen
			$img_sizes = ThunderSliderBase::get_all_image_sizes('youtube');
			
			$imgResolution = ThunderSliderFunctions::getVal($this->params, 'image_source_type', reset($img_sizes));

			$this->imageID = ThunderSliderFunctions::getVal($resource, 'videoId');
			if(!isset($img_sizes[$imgResolution])) $imgResolution = key($img_sizes);
			
			$thumbs = ThunderSliderFunctions::getVal($snippet, 'thumbnails');
			$is = array();
			if(!empty($thumbs)){
				foreach($thumbs as $name => $vals){
					$is[$name] = ThunderSliderFunctions::getVal($vals, 'url');
				}
			}
		...
		$this->postData = apply_filters('thunderslider_slide_initByYoutube_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByVimeo($sliderID, $additions){
		...
		$this->postData = apply_filters('thunderslider_slide_initByVimeo_pre', $this->postData, $sliderID, $additions, $this);
		...
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = ThunderSliderFunctions::getVal($this->postData, 'url');
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
		}
		
		$this->params["slide_bg_vimeo"] = ThunderSliderFunctions::getVal($this->postData, 'url');
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamvimeo' || $this->params["background_type"] == 'streamvimeoboth'){ //if image is choosen, use featured image as background
			//facebook check which image size is choosen
			$img_sizes = ThunderSliderBase::get_all_image_sizes('vimeo');
			$imgResolution = ThunderSliderFunctions::getVal($this->params, 'image_source_type', reset($img_sizes));
			...
			foreach($img_sizes as $handle => $name){
				$is[$handle] = ThunderSliderFunctions::getVal($this->postData, $handle);
			}
		...
		$this->postData = apply_filters('thunderslider_slide_initByVimeo_post', $this->postData, $sliderID, $additions, $this);
	...
	private function setLayersByStreamData($sliderID, $stream_type, $additions = array()){
		...
		$a = apply_filters('thunderslider_slide_setLayersByStreamData_pre', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
		...
		foreach($this->arrLayers as $key=>$layer){
			...
			$text = apply_filters('thunderslider_mod_stream_meta', $text, $sliderID, $stream_type, $this->postData); //option to add your own filter here to modify meta to your likings
		...
		$a = apply_filters('thunderslider_slide_setLayersByStreamData_post', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
		$img_sizes = ThunderSliderBase::get_all_image_sizes($stream_type);
		
		$text = apply_filters('thunderslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
		
		$title = ($stream_type == 'twitter' && $is_action === false) ? ThunderSliderBase::add_wrap_around_url($attr['title']) : $attr['title'];
		$excerpt = ($stream_type == 'twitter' && $is_action === false) ? ThunderSliderBase::add_wrap_around_url($attr['excerpt']) : $attr['excerpt'];
		$content = ($stream_type == 'twitter' && $is_action === false) ? ThunderSliderBase::add_wrap_around_url($attr['content']) : $attr['content'];
		...
		foreach($arrMatches as $matched){
			foreach($matched as $match) {
				...
				$meta = str_replace('_THUNDERSLIDER_', '-', $meta);
				$vals = explode(':', $meta);
		...
		return apply_filters('thunderslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	public function return_stream_data($stream_type, $additions = array()){
		...
		$attr = apply_filters('thunderslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
		
		switch($stream_type){
			case 'facebook':
				if($additions['fb_type'] == 'album'){
					$attr['title'] = ThunderSliderFunctions::getVal($this->postData, 'name');
					$attr['content'] = ThunderSliderFunctions::getVal($this->postData, 'name');
					$attr['link'] = ThunderSliderFunctions::getVal($this->postData, 'link');
					$attr['date'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($this->postData, 'created_time'), true);
					$attr['date_modified'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($this->postData, 'updated_time'), true);
					$author_name_raw = ThunderSliderFunctions::getVal($this->postData, 'from');
					$attr['author_name'] = $author_name_raw->name;
					$likes_data = ThunderSliderFunctions::getVal($this->postData, 'likes');
					$attr['likes'] = count(ThunderSliderFunctions::getVal($likes_data, 'data'));
					$fb_img_thumbnail = ThunderSliderFunctions::getVal($this->postData, 'picture');
					$fb_img = 'https://graph.facebook.com/'.ThunderSliderFunctions::getVal($this->postData, 'id').'/picture';
					...
					$attr['title'] = ThunderSliderFunctions::getVal($this->postData, 'message');
					$attr['content'] = ThunderSliderFunctions::getVal($this->postData, 'message');
					$post_url = explode('_', ThunderSliderFunctions::getVal($this->postData, 'id'));
					...
					$likes_data = ThunderSliderFunctions::getVal($likes_data, 'summary');
					$likes_data = ThunderSliderFunctions::getVal($likes_data, 'total_count');
			...
			break;
			case 'twitter':
				$user = ThunderSliderFunctions::getVal($this->postData, 'user');
				$attr['title'] = ThunderSliderFunctions::getVal($this->postData, 'text');
				$attr['content'] = ThunderSliderFunctions::getVal($this->postData, 'text');
				$attr['link'] = 'https://twitter.com/'.$additions['twitter_user'].'/status/'.ThunderSliderFunctions::getVal($this->postData, 'id_str');
				$attr['date'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($this->postData, 'created_at'), true);
				$attr['author_name'] = ThunderSliderFunctions::getVal($user, 'screen_name');
				$attr['retweet_count'] = ThunderSliderFunctions::getVal($this->postData, 'retweet_count', '0');
				$attr['favorite_count'] = ThunderSliderFunctions::getVal($this->postData, 'favorite_count', '0');
				$image_url_array = ThunderSliderFunctions::getVal($this->postData, 'media');
				$image_url_large = ThunderSliderFunctions::getVal($image_url_array, 'large');
				$img = ThunderSliderFunctions::getVal($image_url_large, 'media_url', '');
				if($img == ''){
				...
				if($img !== ''){
					$w = ThunderSliderFunctions::getVal($image_url_large, 'w', '');
					$h = ThunderSliderFunctions::getVal($image_url_large, 'h', '');
			...
			case 'instagram':
				$caption = ThunderSliderFunctions::getVal($this->postData, 'caption');
				$user = ThunderSliderFunctions::getVal($this->postData, 'owner');

				$attr['title'] = ThunderSliderFunctions::getVal($caption, 'text');
				$attr['content'] = ThunderSliderFunctions::getVal($caption, 'text');
				$attr['link'] = 'https://www.instagram.com/p/' . ThunderSliderFunctions::getVal($this->postData, 'code');
				$attr['date'] = ThunderSliderFunctions::getVal($this->postData, 'date');
				$attr['date'] = date_i18n(get_option('date_format').' '.get_option('time_format'), $attr['date']);

				$attr['author_name'] = ThunderSliderFunctions::getVal($user, 'id');
				$attr['author_name'] = empty($attr['author_name']) ? "" : $attr['author_name'];
				
				$likes_raw = ThunderSliderFunctions::getVal($this->postData, 'likes');
				$attr['likes'] = ThunderSliderFunctions::getVal($likes_raw, 'count');
				
				$comments_raw = ThunderSliderFunctions::getVal($this->postData, 'comments');
				$attr['num_comments'] = ThunderSliderFunctions::getVal($comments_raw, 'count');
				
				$inst_img = ThunderSliderFunctions::getVal($this->postData, 'images', array());
			...
			break;
			case 'flickr':
				$attr['title'] = ThunderSliderFunctions::getVal($this->postData, 'title');
				$tc = ThunderSliderFunctions::getVal($this->postData, 'description');
				$attr['content'] = ThunderSliderFunctions::getVal($tc, '_content');
				$attr['date'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($this->postData, 'datetaken'));
				$attr['author_name'] = ThunderSliderFunctions::getVal($this->postData, 'ownername');
				$attr['link'] = 'http://flic.kr/p/'.$this->base_encode(ThunderSliderFunctions::getVal($this->postData, 'id'));
				$attr['views'] = ThunderSliderFunctions::getVal($this->postData, 'views');
				
				$attr['img_urls'] = @array(
					'square' 		=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_sq'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_sq').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_sq').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_sq').'" data-no-retina />'),
					'large-square' 	=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_q'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_q').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_q').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_q').'"  data-no-retina />'),
					'thumbnail' 	=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_t'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_t').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_t').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_t').'"  data-no-retina />'),
					'small' 		=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_s'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_s').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_s').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_s').'"  data-no-retina />'),
					'small-320' 	=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_n'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_n').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_n').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_n').'"  data-no-retina />'),
					'medium' 		=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_m'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_m').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_m').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_m').'"  data-no-retina />'),
					'medium-640' 	=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_z'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_z').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_z').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_z').'"  data-no-retina />'),
					'medium-800' 	=> 	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_c'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_c').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_c').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_c').'"  data-no-retina />'),
					'large' 		=>	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_l'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_l').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_l').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_l').'"  data-no-retina />'),
					'original'		=>	array('url' => ThunderSliderFunctions::getVal($this->postData, 'url_o'), 'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, 'url_o').'" width="'.ThunderSliderFunctions::getVal($this->postData, 'width_o').'" height="'.ThunderSliderFunctions::getVal($this->postData, 'height_o').'"  data-no-retina />')
				);
			break;
			case 'youtube':
				$snippet = ThunderSliderFunctions::getVal($this->postData, 'snippet');
				$attr['title'] = ThunderSliderFunctions::getVal($snippet, 'title');
				$attr['excerpt'] = ThunderSliderFunctions::getVal($snippet, 'description');
				$attr['content'] = ThunderSliderFunctions::getVal($snippet, 'description');
				$attr['date'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($snippet, 'publishedAt'));
				
				if($additions['yt_type'] == 'channel'){
					$link_raw = ThunderSliderFunctions::getVal($this->postData, 'id');
					$attr['link'] = ThunderSliderFunctions::getVal($link_raw, 'videoId');
					if($attr['link'] !== '') $attr['link'] = '//youtube.com/watch?v='.$attr['link'];
				}else{
					$link_raw = ThunderSliderFunctions::getVal($this->postData, 'resourceId');
					$attr['link'] = ThunderSliderFunctions::getVal($link_raw, 'videoId');
					if($attr['link'] !== '') $attr['link'] = '//youtube.com/watch?v='.$attr['link'];
				}
				...
				if(!empty($thumbs)){
					foreach($thumbs as $name => $vals){
						$attr['img_urls'][$name] = array(
							'url' => ThunderSliderFunctions::getVal($vals, 'url'),
						);
						switch($additions['yt_type']){
							case 'channel':
								$attr['img_urls'][$name]['tag'] = '<img src="'.ThunderSliderFunctions::getVal($vals, 'url').'" data-no-retina />';
							break;
							case 'playlist':
								$attr['img_urls'][$name]['tag'] = '<img src="'.ThunderSliderFunctions::getVal($vals, 'url').'" width="'.ThunderSliderFunctions::getVal($vals, 'width').'" height="'.ThunderSliderFunctions::getVal($vals, 'height').'" data-no-retina />';
							break;
						}
					}
				}
			...
			case 'vimeo':
				$attr['title'] = ThunderSliderFunctions::getVal($this->postData, 'title');
				$attr['excerpt'] = ThunderSliderFunctions::getVal($this->postData, 'description');
				$attr['content'] = ThunderSliderFunctions::getVal($this->postData, 'description');
				$attr['date'] = ThunderSliderFunctionsMCMS::convertPostDate(ThunderSliderFunctions::getVal($this->postData, 'upload_date'));
				$attr['likes'] = ThunderSliderFunctions::getVal($this->postData, 'stats_number_of_likes');
				$attr['views'] = ThunderSliderFunctions::getVal($this->postData, 'stats_number_of_plays');
				$attr['num_comments'] = ThunderSliderFunctions::getVal($this->postData, 'stats_number_of_comments');
				$attr['link'] = ThunderSliderFunctions::getVal($this->postData, 'url');
				$attr['author_name'] = ThunderSliderFunctions::getVal($this->postData, 'user_name');
				
				$attr['img_urls'] = array();
				if(!empty($img_sizes)){
					foreach($img_sizes as $name => $vals){
						$attr['img_urls'][$name] = array(
							'url' => ThunderSliderFunctions::getVal($this->postData, $name),
							'tag' => '<img src="'.ThunderSliderFunctions::getVal($this->postData, $name).'" data-no-retina />'
						);
					}
				}
		...
		return apply_filters('thunderslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function get_facebook_timeline_image(){
		...
		$object_id = ThunderSliderFunctions::getVal($this->postData, 'object_id', '');
		$picture = ThunderSliderFunctions::getVal($this->postData, 'picture', '');
		if(!empty($object_id)){
			$return = 'https://graph.facebook.com/'.ThunderSliderFunctions::getVal($this->postData, 'object_id', '').'/picture';//$photo->picture;
		}elseif(!empty($picture)) {
			
			$image_url = $this->decode_facebook_url(ThunderSliderFunctions::getVal($this->postData, 'picture', ''));
	        ...
	        if(strpos($image_url[0], 'fbstaging') !== false) {
	            ...
	            $new_url = ThunderSliderFunctions::getVal($this->postData, 'picture', '');
		...
		return apply_filters('thunderslider_slide_get_facebook_timeline_image', $return, $object_id, $picture, $this);
	}
	 ...
	 * init slide by post data
	 */
	public function initByPostData($postData, ThunderSliderSlide $slideTemplate, $sliderID){
		//global $rs_slide_template;
		
		$this->postData = apply_filters('thunderslider_slide_initByPostData', $postData, $slideTemplate, $sliderID, $this);
		...
		if(!empty($slideTemplateID) && is_numeric($slideTemplateID)){
			...
			try{
				...
				$slideTemplateLocal = new ThunderSliderSlide();
		...
		//set some slide params
		$this->id = $postID;
		$this->params["title"] = ThunderSliderFunctions::getVal($postData, "post_title");
		
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = get_permalink($postID);
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
			$this->params["link"] = str_replace('-', '_THUNDERSLIDER_', $this->params["link"]);
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			//set image
			$thumbID = ThunderSliderFunctionsMCMS::getPostThumbID($postID);
	...
	private function setLayersByPostData($postData,$sliderID){
		$postData = apply_filters('thunderslider_slide_setLayersByPostData_pre', $postData, $sliderID, $this);
		
		$postID = $postData["ID"];
		
		$attr = array();
		$attr['title'] = ThunderSliderFunctions::getVal($postData, "post_title");
		
		//check if we are woocommerce or not
		$st = $this->getSliderParam($sliderID,"source_type",'gallery');
		
		if($st == 'woocommerce'){
			$excerpt_limit = $this->getSliderParam($sliderID,"excerpt_limit_product",55,ThunderSlider::VALIDATE_NUMERIC);
			$excerpt_limit = (int)$excerpt_limit;
		}else{
			$excerpt_limit = $this->getSliderParam($sliderID,"excerpt_limit",55,ThunderSlider::VALIDATE_NUMERIC);
			$excerpt_limit = (int)$excerpt_limit;
		}
		
		$attr['excerpt'] = ThunderSliderFunctionsMCMS::getExcerptById($postID, $excerpt_limit);
		
		$attr['alias'] = ThunderSliderFunctions::getVal($postData, "post_name");
		
		$attr['content'] = ThunderSliderFunctions::getVal($postData, "post_content");
		
		$attr['link'] = get_permalink($postID);
		
		$postDate = ThunderSliderFunctions::getVal($postData, "post_date_gmt");
		$attr['postDate'] = ThunderSliderFunctionsMCMS::convertPostDate($postDate);
		
		$dateModified = ThunderSliderFunctions::getVal($postData, "post_modified");
		$attr['dateModified'] = ThunderSliderFunctionsMCMS::convertPostDate($dateModified);
		
		$authorID = ThunderSliderFunctions::getVal($postData, "post_author");
		$attr['authorName'] = ThunderSliderFunctionsMCMS::getUserDisplayName($authorID);
		$attr['authorID'] = $authorID;
		$attr['authorPage'] = ThunderSliderFunctionsMCMS::getUserPage($authorID);
		$attr['authorPostsPage'] = ThunderSliderFunctionsMCMS::getUserPostsPage($authorID);
		
		$postCatsIDs = $postData["post_category"];
		$attr['catlist'] = ThunderSliderFunctionsMCMS::getCategoriesHtmlList($postCatsIDs);
		$attr['catlist_raw'] = strip_tags(ThunderSliderFunctionsMCMS::getCategoriesHtmlList($postCatsIDs));
		$attr['taglist'] = ThunderSliderFunctionsMCMS::getTagsHtmlList($postID);
		...
		$attr['img_urls'] = array();
		foreach($img_sizes as $img_handle => $img_name){
			$featured_image_url = mcms_get_attachment_image_src($ptid, $img_handle);
		...
		$attr['numComments'] = ThunderSliderFunctions::getVal($postData, "comment_count");
		
		$attr = apply_filters('thunderslider_slide_setLayersByPostData_post', $attr, $postData, $sliderID, $this);
		
		foreach($this->arrLayers as $key=>$layer){
			
			$text = ThunderSliderFunctions::getVal($layer, "text");
			$text = apply_filters('thunderslider_mod_meta', $text, $postID, $postData); //option to add your own filter here to modify meta to your likings
	...
	public function set_post_data($text, $attr, $post_id){
		...
		//remove thunder_slider shortcodes from content ( no inception ;)
		$content = preg_replace('/\\[thunder_slider.*?\\]/', '', $content, -1 );
		$content = str_replace("[/thunder_slider]", "", $content);
		
		//add filter for addon metas
		$text = apply_filters( 'thunder_slider_insert_meta', $text, $post_id );
		...
		//process meta tags:
		$text = str_replace('-', '_THUNDERSLIDER_', $text);
		...
		if(ThunderSliderEventsManager::isEventsExists()){
			$eventData = ThunderSliderEventsManager::getEventPostData($post_id);
			if(!empty($eventData)){
				foreach($eventData as $eventKey=>$eventValue){
					$eventPlaceholder = "%event_".$eventKey."%";
					$eventPlaceholderNew = "{{event_".$eventKey."}}";
					if($eventKey == 'start_date' || $eventKey == 'end_date') $eventValue = ThunderSliderFunctionsMCMS::convertPostDate($eventValue);
					$text = str_replace(array($eventPlaceholder, $eventPlaceholderNew), $eventValue , $text);
				}
			}
		}
		
		if(ThunderSliderWooCommerce::isWooCommerceExists()){
			$is_30 = ThunderSliderWooCommerce::version_check('3.0');
			...
			if($product !== false){
				...
				if(strpos($text, 'wc_add_to_cart_button') !== false){
					...
					$assets_path          	= str_replace( array( 'http:', 'https:' ), '', WC()->module_url() ) . '/assets/';
					$frontend_script_path 	= $assets_path . 'js/frontend/';
					
					if ( $ajax_cart_en ){
						mcms_enqueue_script( 'wc-add-to-cart', $frontend_script_path . 'add-to-cart' . $suffix . '.js', array( 'jquery' ), WC_VERSION, true );
						
						global $wc_is_localized;
						if($wc_is_localized === false){ //load it only one time
							mcms_localize_script( 'wc-add-to-cart', 'wc_add_to_cart_params', apply_filters( 'wc_add_to_cart_params', array(
	...
	public function getDataByID($slideid){
		...
		if(strpos($slideid, 'static_') !== false){
			...
			$record = $this->db->fetch(ThunderSliderGlobals::$table_static_slides, $this->db->prepare("slider_id = %s", array($sliderID)));
			if(!empty($record)){
				$return = $record[0];
			}
			//$return = false;
		}else{
			$record = $this->db->fetchSingle(ThunderSliderGlobals::$table_slides, $this->db->prepare("id = %d", array($slideid)));
			$return = $record;
		}
		
		return apply_filters('thunderslider_slide_getDataByID', $return, $slideid, $this);
	}
	...
	public function initByID($slideid){
		try{
			if(strpos($slideid, 'static_') !== false){
				...
				ThunderSliderFunctions::validateNumeric($sliderID,"Slider ID");
				...
				ThunderSliderFunctions::validateNumeric($slideid,"Slide ID");
				$record = $this->db->fetchSingle(ThunderSliderGlobals::$table_slides, $this->db->prepare("id = %d", array($slideid)));
	...
	public static function isSlideByID($slideid){
		...
		try{
			if(strpos($slideid, 'static_') !== false){
				...
				$record = $db->fetch(ThunderSliderGlobals::$table_static_slides, $db->prepare("slider_id = %s", array($sliderID)));
				...
				$record = $db->fetchSingle(ThunderSliderGlobals::$table_slides, $db->prepare("id = %s", array($slideid)));
	...
	public function initByStaticID($slideid){
		...
		ThunderSliderFunctions::validateNumeric($slideid,"Slide ID");
		$record = $this->db->fetchSingle(ThunderSliderGlobals::$table_static_slides, $this->db->prepare("id = %s", array($slideid)));
	...
	private function setImageByImageID($imageID){
		...
		$imageID = apply_filters('thunderslider_slide_setImageByImageID', $imageID, $this);
		
		$imgResolution = ThunderSliderFunctions::getVal($this->params, 'image_source_type', 'full');
		
		$this->imageID = $imageID;
		
		$this->imageUrl = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($imageID, $imgResolution);
		$this->imageThumb = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($imageID,ThunderSliderFunctionsMCMS::THUMB_MEDIUM);
	...
	public function getArrChildren(){
		...
		return apply_filters('thunderslider_slide_getArrChildren', $this->arrChildren, $this);
	}
	...
	public function getLayers(){
		$this->validateInited();
		return apply_filters('thunderslider_getLayers', $this->arrLayers, $this);
	}
	...
	public function getLayerID_by_unique_id($unique_id, $static_slide){
		...
		if(strpos($unique_id, 'static-') !== false){
			...
			if(!empty($layers)){
				foreach($layers as $l){
					$uid = ThunderSliderFunctions::getVal($l, 'unique_id');
					if($uid == $unique_id){
						return ThunderSliderFunctions::getVal($l, 'attrID');
					}
				}
			}
		}else{
	...
	public function saveLayers(){
		$this->validateInited();
		$table = ($this->static_slide) ? ThunderSliderGlobals::$table_static_slides : ThunderSliderGlobals::$table_slides;
		
		$this->arrLayers = apply_filters('thunderslider_slide_saveLayers', $this->arrLayers, $this->static_slide, $this);
	...
	public function saveParams(){
		...
		$this->params = apply_filters('thunderslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function getLayersForExport($useDummy = false){
		...
		$arrLayersNew = array();
		foreach($this->arrLayers as $key=>$layer){
			$imageUrl = ThunderSliderFunctions::getVal($layer, "image_url");
			if(!empty($imageUrl))
				$layer["image_url"] = ThunderSliderFunctionsMCMS::getImagePathFromURL($layer["image_url"]);
			
			$arrLayersNew[] = $layer;
		}
		
		return apply_filters('thunderslider_slide_getLayersForExport', $arrLayersNew, $this);
	}
	...
	public function getParamsForExport(){
		...
		$urlImage = ThunderSliderFunctions::getVal($arrParams, "image");
		if(!empty($urlImage))
			$arrParams["image"] = ThunderSliderFunctionsMCMS::getImagePathFromURL($urlImage);
		
		//check if we are transparent or solid and remove unneeded image
		$bgtype = ThunderSliderFunctions::getVal($arrParams, "background_type", 'transparent');
		switch($bgtype){
			case 'transparent':
			case 'trans':
			case 'solid':
		...
		return apply_filters('thunderslider_slide_getParamsForExport', $arrParams, $this);
	}
	...
	public function getLayersNormalizeText(){
		...
		return apply_filters('thunderslider_slide_getLayersNormalizeText', $arrLayersNew, $this);
	}
	...
	public function getParams(){
		$this->validateInited();
		return apply_filters('thunderslider_slide_getParams', $this->params, $this);
	}
	...
	public function getSettings(){
		$this->validateInited();
		return apply_filters('thunderslider_slide_getSettings', $this->settings, $this);
	}
	...
	function getParam($name,$default=null){
		...
		return ThunderSliderFunctions::getVal($this->params, $name, $default);
	}
	...
	private function validateInited(){
		if(empty($this->id))
			ThunderSliderFunctions::throwError("The slide is not initialized!!!");
	}
	...
	public function createSlide($sliderID,$obj="",$static = false){
		...
		if(is_array($obj)){
			$urlImage = ThunderSliderFunctions::getVal($obj, "url");
			$imageID = ThunderSliderFunctions::getVal($obj, "id");
		}else{
			$urlImage = $obj;
		}
		...
		$arrInsert = apply_filters('thunderslider_slide_createSlide', $arrInsert, $sliderID, $static, $this);
		
		if(!$static)
			$slideID = $this->db->insert(ThunderSliderGlobals::$table_slides, $arrInsert);
		else
			$slideID = $this->db->insert(ThunderSliderGlobals::$table_static_slides, $arrInsert);
	...
	public function updateSlideImageFromData($data){
		...
		$slideID = ThunderSliderFunctions::getVal($data, "slide_id");
		$urlImage = ThunderSliderFunctions::getVal($data, "url_image");
		ThunderSliderFunctions::validateNotEmpty($urlImage);
		$imageID = ThunderSliderFunctions::getVal($data, "image_id");
		if($slider->isSlidesFromPosts()){
			
			if(!empty($imageID))
				ThunderSliderFunctionsMCMS::updatePostThumbnail($slideID, $imageID);
	...
	protected function updateParamsInDB($arrUpdate = array(), $static = false){
		...
		$this->params = apply_filters('thunderslider_slide_updateParamsInDB', array_merge($this->params,$arrUpdate), $this);
		
		$jsonParams = json_encode($this->params);
		
		$arrDBUpdate = array("params"=>$jsonParams);
		if($static === false){
			$this->db->update(ThunderSliderGlobals::$table_slides,$arrDBUpdate,array("id"=>$this->id));
		}else{
			
			$this->db->update(ThunderSliderGlobals::$table_static_slides,$arrDBUpdate,array("id"=>$static));
		}
	}
	...
	protected function updateLayersInDB($arrLayers = null, $static = false){
		...
		$arrLayers = apply_filters('thunderslider_slide_updateLayersInDB', $arrLayers, $this);
	...
	private function sortLayersByOrder($layer1,$layer2){
		...
		$order1 = ThunderSliderFunctions::getVal($layer1, "order",1);
		$order2 = ThunderSliderFunctions::getVal($layer2, "order",2);
		if($order1 == $order2)
			return(0);
	...
	private function normalizeParams($params){
		...
		$urlImage = ThunderSliderFunctions::getVal($params, "image_url");
		
		//init the id if absent
		$params["image_id"] = ThunderSliderFunctions::getVal($params, "image_id");
		
		$params["image"] = $urlImage;
		unset($params["image_url"]);
		
		if(isset($params["video_description"]))
			$params["video_description"] = ThunderSliderFunctions::normalizeTextareaContent($params["video_description"]);
	...
	public function updateSlideFromData($data){
		...
		$slideID = ThunderSliderFunctions::getVal($data, "slideid");
		$this->initByID($slideID);						
		
		//treat params
		$params = ThunderSliderFunctions::getVal($data, "params");
		...
		//treat layers
		$layers = ThunderSliderFunctions::getVal($data, "layers");
		...
		if(gettype($layers) == "string"){
			...
			$layers = ThunderSliderFunctions::convertStdClassToArray($layersDecoded);
		}
		...
		$settings = ThunderSliderFunctions::getVal($data, "settings");
		...
		$arrUpdate = apply_filters('thunderslider_slide_updateSlideFromData_pre', $arrUpdate, $data, $this);
		
		$this->db->update(ThunderSliderGlobals::$table_slides,$arrUpdate,array("id"=>$this->id));
		
		do_action('thunderslider_slide_updateSlideFromData_post', $arrUpdate, $data, $this);
		//ThunderSliderOperations::updateDynamicCaptions();
	}
	...
	public function updateStaticSlideFromData($data){
		...
		$arrUpdate = apply_filters('thunderslider_slide_updateStaticSlideFromData', $arrUpdate, $data, $this);
		
		$this->db->update(ThunderSliderGlobals::$table_static_slides,$arrUpdate,array("id"=>$this->id));
		
		do_action('thunderslider_slide_updateStaticSlideFromData_post', $arrUpdate, $data, $this);
		//ThunderSliderOperations::updateDynamicCaptions();
	}
	...
	public function deleteSlide(){
		...
		$this->db->delete(ThunderSliderGlobals::$table_slides, $this->db->prepare("id = %s", array($this->id)));
		
		do_action('thunderslider_slide_deleteSlide', $this->id);
	}
	...
	public function updateTitleByID($data){
		...
		$arrUpdate = apply_filters('thunderslider_slide_updateTitleByID', $arrUpdate, $data, $this);
	...
	public function toggleSlideStatFromData($data){
		...
		$arrUpdate = apply_filters('thunderslider_slide_toggleSlideStatFromData', $arrUpdate, $data, $this);
	...
	private function updateLangFromData($data){
		...
		$lang = ThunderSliderFunctions::getVal($data, "lang");
		...
		$response = array();
		$response["url_icon"] = ThunderSliderWpml::getFlagUrl($lang);
		$response["title"] = ThunderSliderWpml::getLangTitle($lang);
	...
	private function addLangFromData($data){
		...
		$urlIcon = ThunderSliderWpml::getFlagUrl($lang);
		$title = ThunderSliderWpml::getLangTitle($lang);
		
		$newSlide = new RevSlide();
		$newSlide->initByID($slideID);
		$arrLangCodes = $newSlide->getArrChildLangCodes();
		$isAll = ThunderSliderWpml::isAllLangsInArray($arrLangCodes);
	...
	public function doSlideLangOperation($data){
		...
		$operation = ThunderSliderFunctions::getVal($data, "operation");
		switch($operation){
			case "add":
	...
	public function getUrlImageThumb(){
		...
		if(!empty($this->imageID)){
			$urlImage = ThunderSliderFunctionsMCMS::getUrlAttachmentImage($this->imageID, ThunderSliderFunctionsMCMS::THUMB_MEDIUM);
		}else{
			//get from cache
			if(!empty($this->imageFilepath)){
				$urlImage = ThunderSliderBase::getImageUrl($this->imageFilepath,200,100,true);
			}
			else 
		...
		$urlImage = apply_filters('thunderslider_slide_getUrlImageThumb', $urlImage, $this);
	...
	public function get_image_attributes($slider_type){
		...
		$bgPosition = ThunderSliderBase::getVar($params, "bg_position","center top");
		...
		$thumb = '';
		$thumb_on = ThunderSliderBase::getVar($params, "thumb_for_admin", 'off');
		...
		switch($slider_type){
			case 'gallery':
				...
				if($thumb_on == 'on'){
					$thumb = ThunderSliderBase::getVar($params, "slide_thumb", '');
				}
		...
		return apply_filters('thunderslider_slide_get_image_attributes', array(
	...
	public function replaceImageUrls($urlFrom, $urlTo, $static = false){
		...
		foreach($check as $param){
			$urlImage = ThunderSliderFunctions::getVal($this->params, $param, '');
		...
		$isUpdated = false;
		foreach($this->arrLayers as $key=>$layer){
			$type =  ThunderSliderFunctions::getVal($layer, "type");
			...
			if(isset($type) && ($type == 'video' || $type == 'audio')){
				...
				if(!empty($check)){
					foreach($check as $param){
						$url = ThunderSliderFunctions::getVal($video_data, $param);
		...
		do_action('thunderslider_slide_replaceImageUrls', $this);
	}
	...
	public function getUsedFonts($full = false){
		...
		$op = new ThunderSliderOperations();
		$fonts = array();
		...
		if(!empty($this->arrLayers)){
			foreach($this->arrLayers as $key=>$layer){
				$def = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());
				$font = ThunderSliderFunctions::getVal($def, 'font-family', '');
				$static = (array) ThunderSliderFunctions::getVal($layer, 'static_styles', array());
				...
				foreach($all_fonts as $f){
					if(strtolower(str_replace(array('"', "'", ' '), '', $f['label'])) == strtolower(str_replace(array('"', "'", ' '), '', $font)) && $f['type'] == 'googlefont'){
						...
						if($full){ //if full, add all.
							...
							$fw = (array) ThunderSliderFunctions::getVal($static, 'font-weight', '400'); 
							$fs = ThunderSliderFunctions::getVal($def, 'font-style', '');
		...
		return apply_filters('thunderslider_slide_getUsedFonts', $fonts, $this);
	}
	...
	public function replaceCssClass($css_from, $css_to){
		...
		if(!empty($this->arrLayers)){
			foreach($this->arrLayers as $key=>$layer){
				$caption = ThunderSliderFunctions::getVal($layer, 'style');
				if($caption == $css_from){
		...
		do_action('thunderslider_slide_replaceCssClass', $css_from, $css_to, $this);
	}
	...
	public static function translateIntoSizes(){
		...
		return apply_filters('thunderslider_slide_translateIntoSizes', array(
			'align_hor',
			'align_vert',
			'top',
			'left',
 ...
 * @since: 5.0
 **/
class RevSlide extends ThunderSliderSlide {}
?>