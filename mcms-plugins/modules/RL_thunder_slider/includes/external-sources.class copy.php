...
class ThunderSliderFacebook {
	...
	*/
	...
	public function get_photo_sets($user_id,$item_count=10,$app_id,$app_secret){
		//photoset params
		$oauth = mcms_remote_fopen("https://graph.facebook.com/oauth/access_token?type=client_cred&client_id=".$app_id."&client_secret=".$app_secret);
    $oauth = json_decode($oauth);
		$url = "https://graph.facebook.com/$user_id/albums?access_token=".$oauth->access_token;
		$photo_sets_list = json_decode(mcms_remote_fopen($url));
		//echo '<pre>';
    ...
    $url = "https://graph.facebook.com/$photo_set_id/photos?fields=photos&access_token=".$oauth->access_token."&fields=id,from,message,picture,link,name,icon,privacy,type,status_type,object_id,application,created_time,updated_time,is_hidden,is_expired,comments.limit(1).summary(true),likes.limit(1).summary(true)";
		...
		$transient_name = 'thunderslider_' . md5($url);

		if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name)))
			return ($data);

		$photo_set_photos = json_decode(mcms_remote_fopen($url));
    ...
    */
    ...
    $url = "https://graph.facebook.com/$user/feed?access_token=".$oauth->access_token."&fields=id,from,message,picture,link,name,icon,privacy,type,status_type,object_id,application,created_time,updated_time,is_hidden,is_expired,comments.limit(1).summary(true),likes.limit(1).summary(true)";
		...
		$feed = json_decode(mcms_remote_fopen($url));
...
class ThunderSliderTwitter {
  ...
  public function get_public_photos($twitter_account,$include_rts,$exclude_replies,$count,$imageonly){
    ...
    //require_once( 'class-mcms-twitter-api.php' );
	...
	$twitter_api = new ThunderSliderTwitterApi( $credentials , $this->transient_sec);
...
/**
* Class MandarinCMS Twitter API
*
* https://github.com/micc83/Twitter-API-1.1-Client-for-Wordpress/blob/master/class-mcms-twitter-api.php
* @version 1.0.0
*/
class ThunderSliderTwitterApi {
        ...
        /**
         * MandarinCMS Twitter API Constructor
         *
         * @param array $args
         */
        ...
        private function get_bearer_token() {
                ...
                $response = mcms_remote_post( 'https://api.twitter.com/oauth2/token', $args );
                
                if ( is_mcms_error( $response ) || 200 != $response['response']['code'] )
                        return $this->bail( __( 'Can\'t get the bearer token, check your credentials', 'mcms_twitter_api' ), $response );
        ...
        public function query( $query, $query_args = array(), $stop = false ) {
                ...
                $response = mcms_remote_get( 'https://api.twitter.com/1.1/' . $this->query_args['type'] . '.json?' . $query, $args );
                if ( is_mcms_error( $response ) || 200 != $response['response']['code'] ){
                        ...
                        if ( !$stop ){
                                ...
                                return $this->bail( __( 'Bearer Token is good, check your query', 'mcms_twitter_api' ), $response );
                        }
         ...
         * Let's manage errors
         *
         * MCMS_DEBUG has to be set to true to show errors
         *
         * @param string $error_text Error message
         * @param string $error_object Server response or mcms_error
         */
        private function bail( $error_text, $error_object = '' ) {
                
                $this->has_error = true;
                
                if ( is_mcms_error( $error_object ) ){
  ...
  private $transient_sec;
    ...
    if(!empty($search_user_id) && !empty($this->api_key)){
      ...
      $url = 'https://api.instagram.com/v1/users/'.$search_user_id.'/media/recent/?&access_token='.$this->api_key.'&count='.$count;
    		...
    		$rsp = json_decode(mcms_remote_fopen($url));
