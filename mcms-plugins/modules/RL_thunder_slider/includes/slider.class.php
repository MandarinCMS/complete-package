...
class ThunderSliderSlider extends ThunderSliderElementsBase{
	...
	private function validateInited(){
		if(empty($this->id))
			ThunderSliderFunctions::throwError("The slider is not initialized!");
	}
	...
	public function initByDBData($arrData){
		...
		$params = ThunderSliderBase::translate_settings_to_v5($params);
	...
	public function initByID($sliderID){
		...
		try{
			$sliderData = $this->db->fetchSingle(ThunderSliderGlobals::$table_sliders, $this->db->prepare("id = %s", array($sliderID)));
		}catch(Exception $e){
	...
	public function initByAlias($alias){
		...
		try{
			...
			$sliderData = $this->db->fetchSingle(ThunderSliderGlobals::$table_sliders,$where);
			...
			ThunderSliderFunctions::throwError($errorMessage);
		}
	...
	function getParam($name,$default=null,$validateType = null,$title=""){
		...
		$value = ThunderSliderFunctions::getVal($this->arrParams, $name,$default);
		...
		switch($validateType){
			...
			case self::VALIDATE_EMPTY:
				...
				if($value !== "0" && $value !== 0 && empty($value))
					ThunderSliderFunctions::throwError("The param <strong>$paramTitle</strong> should not be empty.");
			break;
			case self::VALIDATE_NUMERIC:
				$paramTitle = !empty($title)?$title:$name;
				if(!is_numeric($value))
					ThunderSliderFunctions::throwError("The param <strong>$paramTitle</strong> should be numeric. Now it's: $value");
			break;
	...
	public function getShortcode(){
		$shortCode = '[thunder_slider alias="'.$this->alias.'"]';
		return($shortCode);
	}
	...
	public function isAliasExistsInDB($alias){
		...
		$response = $this->db->fetch(ThunderSliderGlobals::$table_sliders,$where);
	...
	public static function isAliasExists($alias){
		...
		$response = $mcmsdb->get_row($mcmsdb->prepare("SELECT * FROM ".ThunderSliderGlobals::$table_sliders." WHERE alias = %s AND `type` != 'template'", $alias));
	...
	private function validateInputSettings($title,$alias,$params){
		ThunderSliderFunctions::validateNotEmpty($title,"title");
		ThunderSliderFunctions::validateNotEmpty($alias,"alias");
		
		if($this->isAliasExistsInDB($alias))
			ThunderSliderFunctions::throwError("Some other slider with alias '$alias' already exists");
	...
	public function setHeroSlide($data){
		...
		ThunderSliderFunctions::validateNotEmpty($sliderID,"Slider ID");
		$this->initByID($sliderID);

		$new_slide_id = ThunderSliderFunctions::getVal($data, "slide_id");
		ThunderSliderFunctions::validateNotEmpty($new_slide_id,"Hero Slide ID");
	...
	private function createUpdateSliderFromOptions($options, $sliderID = null){
		...
		$arrMain = ThunderSliderFunctions::getVal($options, "main");
		$params = ThunderSliderFunctions::getVal($options, "params");
		
		//trim all input data
		$arrMain = ThunderSliderFunctions::trimArrayItems($arrMain);
		
		$params = ThunderSliderFunctions::trimArrayItems($params);
		
		$params = array_merge($arrMain,$params);
		
		$title = sanitize_text_field(ThunderSliderFunctions::getVal($arrMain, "title"));
		$alias = sanitize_text_field(ThunderSliderFunctions::getVal($arrMain, "alias"));
		...
		if(empty($sliderID)){	//create slider	
			...
			$sliderID = $this->db->insert(ThunderSliderGlobals::$table_sliders,$arrData);
			return($sliderID);
			...
			$sliderID = $this->db->update(ThunderSliderGlobals::$table_sliders,$arrData,array("id"=>$sliderID));
		}
	}
	...
	public function deleteSlider(){
		...
		//delete slider
		$this->db->delete(ThunderSliderGlobals::$table_sliders, $this->db->prepare("id = %s", array($this->id)));
	...
	private function deleteAllSlides(){
		...
		$this->db->delete(ThunderSliderGlobals::$table_slides, $this->db->prepare("slider_id = %s", array($this->id)));
		
		do_action('thunderslider_slider_deleteAllSlides', $this->id);
	}
	...
	public function deleteStaticSlide(){
		...
		$this->db->delete(ThunderSliderGlobals::$table_static_slides, $this->db->prepare("slider_id = %s", array($this->id)));
	}
	...
	public function getArrSlideChildren($slideID){
		...
		if(!isset($arrSlides[$slideID]))
			ThunderSliderFunctions::throwError("Slide with id: $slideID not found in the main slides of the slider. Maybe it's child slide.");
	...
	private function duplicateSlider($title = false, $prefix = false){
		...
		//insert a new slider
		$sqlSelect = $this->db->prepare("select ".ThunderSliderGlobals::FIELDS_SLIDER." from ".ThunderSliderGlobals::$table_sliders." where id = %s", array($this->id));
		$sqlInsert = "insert into ".ThunderSliderGlobals::$table_sliders." (".ThunderSliderGlobals::FIELDS_SLIDER.") ($sqlSelect)";
		
		$this->db->runSql($sqlInsert);
		$lastID = $this->db->getLastInsertID();
		ThunderSliderFunctions::validateNotEmpty($lastID);
		...
		if($title === false){
			//get slider number:
			$response = $this->db->fetch(ThunderSliderGlobals::$table_sliders);
			...
			$sqlTitle = $this->db->fetch(ThunderSliderGlobals::$table_sliders, $this->db->prepare("alias = %s", array($newSliderAlias)));
			if(!empty($sqlTitle)){
				$response = $this->db->fetch(ThunderSliderGlobals::$table_sliders);
				$numSliders = count($response);
				$newSliderSerial = $numSliders+1;
		...
		$params["shortcode"] = "[thunder_slider alias=\"". $newSliderAlias ."\"]";
		...
		$this->db->update(ThunderSliderGlobals::$table_sliders, $arrUpdate, array("id"=>$lastID));
		
		//duplicate Slides
		$slides = $this->db->fetch(ThunderSliderGlobals::$table_slides, $this->db->prepare("slider_id = %s", array($this->id)));
		if(!empty($slides)){
			foreach($slides as $slide){
				$slide['slider_id'] = $lastID;
				$myID = $slide['id'];
				unset($slide['id']);
				$last_id = $this->db->insert(ThunderSliderGlobals::$table_slides,$slide);
		...
		$static_id = 0;
		if($staticID !== false){
			$record = $this->db->fetchSingle(ThunderSliderGlobals::$table_static_slides, $this->db->prepare("id = %s", array($staticID)));
			unset($record['id']);
			$record['slider_id'] = $lastID;
			
			$static_id = $this->db->insert(ThunderSliderGlobals::$table_static_slides, $record);
		}
		
		
		//update actions
		$slides = $this->db->fetch(ThunderSliderGlobals::$table_slides, $this->db->prepare("slider_id = %s", array($lastID)));
		if($static_id > 0){
			$slides_static = $this->db->fetch(ThunderSliderGlobals::$table_static_slides, $this->db->prepare("id = %s", array($static_id)));
			...
			foreach($slides as $slide){
				...
				if($did_change === true){
					...
					if($slide['id'] == $static_id){
						$this->db->update(ThunderSliderGlobals::$table_static_slides,$arrCreate,array("id"=>$static_id));
					}else{
						$this->db->update(ThunderSliderGlobals::$table_slides,$arrCreate,array("id"=>$slide['id']));
					}
		...
		$c_slider = new ThunderSliderSlider();
	...
	public function duplicateSlide($slideID){
		...
		//do duplication
		$sqlSelect = $this->db->prepare("select ".ThunderSliderGlobals::FIELDS_SLIDE." from ".ThunderSliderGlobals::$table_slides." where id = %s", array(intval($slideID)));
		$sqlInsert = "insert into ".ThunderSliderGlobals::$table_slides." (".ThunderSliderGlobals::FIELDS_SLIDE.") ($sqlSelect)";
		...
		$this->db->update(ThunderSliderGlobals::$table_slides,$arrUpdate, array("id"=>$lastID));
	...
	private function copyMoveSlide($slideID,$targetSliderID,$operation){
		...
		if($operation == "move"){
			...
			$targetSlider = new ThunderSlider();
			...
			foreach($arrChildren as $child){
				$childID = $child->getID();
				$this->db->update(ThunderSliderGlobals::$table_slides,$arrUpdate,array("id"=>$childID));
			}
			
			$this->db->update(ThunderSliderGlobals::$table_slides,$arrUpdate,array("id"=>$slideID));
	...
	private function shiftOrder($fromOrder){
		...
		$sql = "update ".ThunderSliderGlobals::$table_slides." set slide_order=(slide_order+1) where $where";
		$this->db->runSql($sql);
	...
	public function exportSlider($useDummy = false){
		...
		if(!empty($cfw) && count($cfw) > 0){
			...
			$d = apply_filters('thunderslider_exportSlider_usedMedia', $d, $cfw, $sliderParams, $useDummy); //  $arrSlides, $arrStaticSlide, 
		...
		//$strExportAnim = serialize(ThunderSliderOperations::getFullCustomAnimations());
		...
		if(!empty($usedNavigations)){
			$navs = ThunderSliderNavigation::export_navigation($usedNavigations);
		...
		if(!empty($usedAnimations)){
			...
			foreach($usedAnimations as $anim => $val){
				$anima = ThunderSliderOperations::getFullCustomAnimationByID($anim);
		...
		if(class_exists('ZipArchive')){
			...
			if($success !== true)
				throwError("Can't create zip file: ".ThunderSliderGlobals::$uploadsUrlExportZip);
		...
		if(!empty($usedSVG)){
			...
			$content_path = BASED_TREE_URI . 'mcms-content';
			$ud = mcms_upload_dir();
			$up_dir = $ud['baseurl'];
			foreach($usedSVG as $file => $val){
				if(strpos($file, 'http') !== false){ //remove all up to mcms-content folder
					...
					if(is_file($content_path.$checkpath)){
						$strExport = str_replace($file, str_replace('/thunderslider/assets/svg', '', $checkpath2), $strExport);
					}
				}
			}
		}
		...
		$enable_slider_pack = apply_filters('thunderslider_slider_pack_export', false);
	...
	public function importSliderFromPost($updateAnim = true, $updateStatic = true, $exactfilepath = false, $is_template = false, $single_slide = false, $updateNavigation = true){
		...
		try{
			...
			$sliderID = ThunderSliderFunctions::getPostVariable("sliderid");
			...
			if($exactfilepath !== false){
				...
				switch ($_FILES['import_file']['error']) {
					...
					case UPLOAD_ERR_NO_FILE:
						ThunderSliderFunctions::throwError(__('No file sent.', 'thunderslider'));
					case UPLOAD_ERR_INI_SIZE:
					case UPLOAD_ERR_FORM_SIZE:
						ThunderSliderFunctions::throwError(__('Exceeded filesize limit.', 'thunderslider'));
			...
			if(file_exists($filepath) == false)
				ThunderSliderFunctions::throwError(__('Import file not found!!!', 'thunderslider'));
			...
			if( !is_mcms_error($unzipfile) ){
				...
				$content = ( $mcms_filesystem->exists( $d_path.'slider_export.txt' ) ) ? $mcms_filesystem->get_contents( $d_path.'slider_export.txt' ) : '';
				if($content == ''){
					ThunderSliderFunctions::throwError(__('slider_export.txt does not exist!', 'thunderslider'));
				}
				$animations = ( $mcms_filesystem->exists( $d_path.'custom_animations.txt' ) ) ? $mcms_filesystem->get_contents( $d_path.'custom_animations.txt' ) : '';
				$dynamic = ( $mcms_filesystem->exists( $d_path.'dynamic-captions.css' ) ) ? $mcms_filesystem->get_contents( $d_path.'dynamic-captions.css' ) : '';
				//$static = ( $mcms_filesystem->exists( $d_path.'static-captions.css' ) ) ? $mcms_filesystem->get_contents( $d_path.'static-captions.css' ) : '';
				$navigations = ( $mcms_filesystem->exists( $d_path.'navigation.txt' ) ) ? $mcms_filesystem->get_contents( $d_path.'navigation.txt' ) : '';
				
				$uid_check = ( $mcms_filesystem->exists( $d_path.'info.cfg' ) ) ? $mcms_filesystem->get_contents( $d_path.'info.cfg' ) : '';
				$version_check = ( $mcms_filesystem->exists( $d_path.'version.cfg' ) ) ? $mcms_filesystem->get_contents( $d_path.'version.cfg' ) : '';
				
				if($is_template !== false){
					if($uid_check != $is_template){
						return(array("success"=>false,"error"=>__('Please select the correct zip file, checksum failed!', 'thunderslider')));
					}
					...
					if($uid_check !== ''){
						...
						$tmpl_slider = $tmpl->getMandarinCMSTemplateSliders();
				...
				if(!empty($animations)){
					foreach($animations as $key => $animation){ //$animation['id'], $animation['handle'], $animation['params']
						$exist = $db->fetch(ThunderSliderGlobals::$table_layer_anims, $db->prepare("handle = %s", array($animation['handle'])));
						if(!empty($exist)){ //update the animation, get the ID
							if($updateAnim == "true"){ //overwrite animation if exists
								$arrUpdate = array();
								$arrUpdate['params'] = stripslashes(json_encode(str_replace("'", '"', $animation['params'])));
								$db->update(ThunderSliderGlobals::$table_layer_anims, $arrUpdate, array('handle' => $animation['handle']));
								...
								$anim_id = $db->insert(ThunderSliderGlobals::$table_layer_anims, $arrInsert);
							}
					...
					//dmp(__("animations imported!",'thunderslider'));
				}
				
				//overwrite/append static-captions.css
				/*if(!empty($static)){
					if($updateStatic == "true"){ //overwrite file
						ThunderSliderOperations::updateStaticCss($static);
					}elseif($updateStatic == 'none'){
						//do nothing
					}else{//append
						$static_cur = ThunderSliderOperations::getStaticCss();
						$static = $static_cur."\n".$static;
						ThunderSliderOperations::updateStaticCss($static);
					}
				}*/
				...
				$dynamicCss = ThunderSliderCssParser::parseCssToArray($dynamic);
				if(is_array($dynamicCss) && $dynamicCss !== false && count($dynamicCss) > 0){
					...
					//dmp(__("dynamic styles imported!",'thunderslider'));
				}
				...
				if(!empty($navigations)){
					...
					foreach($navigations as $key => $navigation){
						$exist = $db->fetch(ThunderSliderGlobals::$table_navigation, $db->prepare("handle = %s", array($navigation['handle'])));
						unset($navigation['id']);
						...
						if(!empty($exist)){ //create new navigation, get the ID
							if($updateNavigation == "true"){ //overwrite navigation if exists
								...
								$db->update(ThunderSliderGlobals::$table_navigation, $navigation, array('handle' => $rh));
								...
								$navi_id = $db->insert(ThunderSliderGlobals::$table_navigation, $navigation);
					...
					//dmp(__("navigations imported!",'thunderslider'));
				}
			}else{
				$message = $unzipfile->get_error_message();
				
				$mcms_filesystem->delete($rem_path, true);
				
				return(array("success"=>false,"error"=>$message));
			}
			
			//$content = preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", $content); //clear errors in string //deprecated in newest php version
			$content = preg_replace_callback('!s:(\d+):"(.*?)";!', array('ThunderSliderSlider', 'clear_error_in_string') , $content); //clear errors in string
			
			$arrSlider = @unserialize($content);
			if(empty($arrSlider)){
				$mcms_filesystem->delete($rem_path, true);
				ThunderSliderFunctions::throwError(__('Wrong export slider file format! Please make sure that the uploaded file is either a zip file with a correct slider_export.txt in the root of it or an valid slider_export.txt file.', 'thunderslider'));
			}
			...
			if(isset($sliderParams["background_image"]))
				$sliderParams["background_image"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($sliderParams["background_image"]);
			...
			if($sliderExists){
				...
				$this->db->update(ThunderSliderGlobals::$table_sliders,$arrUpdate,array("id"=>$sliderID));
			}else{	//new slider
				$arrInsert = array();
				...
				$arrInsert['title'] = ThunderSliderFunctions::getVal($sliderParams, 'title', 'Slider1');
				$arrInsert['alias'] = ThunderSliderFunctions::getVal($sliderParams, 'alias', 'slider1');	
				...
				$sliderID = $this->db->insert(ThunderSliderGlobals::$table_sliders,$arrInsert);
			}
			...
			foreach($arrSlides as $sl_key => $slide){
				...
				if($importZip === true){ //we have a zip, check if exists
					...
					if(isset($params["image"])){
						$params["image"] = ThunderSliderBase::check_file_in_zip($d_path, $params["image"], $sliderParams["alias"], $alreadyImported);
						$params["image"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($params["image"]);
					}
					
					if(isset($params["background_image"])){
						$params["background_image"] = ThunderSliderBase::check_file_in_zip($d_path, $params["background_image"], $sliderParams["alias"], $alreadyImported);
						$params["background_image"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($params["background_image"]);
					}
					
					if(isset($params["slide_thumb"])){
						$params["slide_thumb"] = ThunderSliderBase::check_file_in_zip($d_path, $params["slide_thumb"], $sliderParams["alias"], $alreadyImported);
						$params["slide_thumb"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($params["slide_thumb"]);
					}
					
					if(isset($params["show_alternate_image"])){
						$params["show_alternate_image"] = ThunderSliderBase::check_file_in_zip($d_path, $params["show_alternate_image"], $sliderParams["alias"], $alreadyImported);
						$params["show_alternate_image"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($params["show_alternate_image"]);
					}
					if(isset($params['background_type']) && $params['background_type'] == 'html5'){
						if(isset($params['slide_bg_html_mpeg']) && $params['slide_bg_html_mpeg'] != ''){
							$params['slide_bg_html_mpeg'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_mpeg"], $sliderParams["alias"], $alreadyImported, true));
						}
						if(isset($params['slide_bg_html_webm']) && $params['slide_bg_html_webm'] != ''){
							$params['slide_bg_html_webm'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_webm"], $sliderParams["alias"], $alreadyImported, true));
						}
						if(isset($params['slide_bg_html_ogv'])  && $params['slide_bg_html_ogv'] != ''){
							$params['slide_bg_html_ogv'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_ogv"], $sliderParams["alias"], $alreadyImported, true));
						}
					}
				}
				...
				foreach($layers as $key=>$layer){					
					...
					if($importZip === true){ //we have a zip, check if exists
						if(isset($layer["image_url"])){
							$layer["image_url"] = ThunderSliderBase::check_file_in_zip($d_path, $layer["image_url"], $sliderParams["alias"], $alreadyImported);
							$layer["image_url"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($layer["image_url"]);
						}
						if(isset($layer["bgimage_url"])){
							$layer["bgimage_url"] = ThunderSliderBase::check_file_in_zip($d_path, $layer["bgimage_url"], $sliderParams["alias"], $alreadyImported);
							$layer["bgimage_url"] = ThunderSliderFunctionsMCMS::getImageUrlFromPath($layer["bgimage_url"]);
						}
						if(isset($layer['type']) && ($layer['type'] == 'video' || $layer['type'] == 'audio')){
							...
							if(!empty($video_data) && isset($video_data['video_type']) && $video_data['video_type'] == 'html5'){
								...
								if(isset($video_data['urlPoster']) && $video_data['urlPoster'] != ''){
									$video_data['urlPoster'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["urlPoster"], $sliderParams["alias"], $alreadyImported));
								}
								
								if(isset($video_data['urlMp4']) && $video_data['urlMp4'] != ''){
									$video_data['urlMp4'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["urlMp4"], $sliderParams["alias"], $alreadyImported, true));
								}
								if(isset($video_data['urlWebm']) && $video_data['urlWebm'] != ''){
									$video_data['urlWebm'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["urlWebm"], $sliderParams["alias"], $alreadyImported, true));
								}
								if(isset($video_data['urlOgv']) && $video_data['urlOgv'] != ''){
									$video_data['urlOgv'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["urlOgv"], $sliderParams["alias"], $alreadyImported, true));
								}
								
							}elseif(!empty($video_data) && isset($video_data['video_type']) && $video_data['video_type'] != 'html5'){ //video cover image
								if($video_data['video_type'] == 'audio'){
									if(isset($video_data['urlAudio']) && $video_data['urlAudio'] != ''){
										$video_data['urlAudio'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["urlAudio"], $sliderParams["alias"], $alreadyImported, true));
									}
								}else{
									if(isset($video_data['previewimage']) && $video_data['previewimage'] != ''){
										$video_data['previewimage'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $video_data["previewimage"], $sliderParams["alias"], $alreadyImported));
									}
								}
							}
							...
							if(isset($layer['video_image_url']) && $layer['video_image_url'] != ''){
								$layer['video_image_url'] = ThunderSliderFunctionsMCMS::getImageUrlFromPath(ThunderSliderBase::check_file_in_zip($d_path, $layer["video_image_url"], $sliderParams["alias"], $alreadyImported));
							}
						}
				...
				$d = apply_filters('thunderslider_importSliderFromPost_modify_data', $d, 'normal', $d_path);
				...
				$last_id = $this->db->insert(ThunderSliderGlobals::$table_slides,$arrCreate);
			...
			//change for MCMSML the parent IDs if necessary
			if(!empty($slider_map)){
				foreach($arrSlides as $sl_key => $slide){
					if(isset($slide['params']['parentid']) && isset($slider_map[$slide['params']['parentid']])){
						...
						$this->db->update(ThunderSliderGlobals::$table_slides,$arrCreate,array("id"=>$update_id));
					}
					...
					foreach($slide['layers'] as $key => $value){
						...
						$link_slide = ThunderSliderFunctions::getVal($value, 'link_slide', false);
						if($link_slide != false && $link_slide !== 'nothing'){ //link to slide/scrollunder is set, move it to actions
							...
							switch($link_slide){
								case 'link':
									$link = ThunderSliderFunctions::getVal($value, 'link');
									$link_open_in = ThunderSliderFunctions::getVal($value, 'link_open_in');
								...
								break;
								case 'scroll_under':
									$scrollunder_offset = ThunderSliderFunctions::getVal($value, 'scrollunder_offset');
						...
						if($did_change === true){
							...
							$this->db->update(ThunderSliderGlobals::$table_slides,$arrCreate,array("id"=>$slider_map[$slide['id']]));
						}
					}
				}
			}
			...
			if(isset($arrSlider['static_slides']) && !empty($arrSlider['static_slides']) && $import_statics){
				...
				foreach($static_slide as $slide){
					...
					if(isset($params["image"])){
						...
						if(strpos($params["image"], 'http') !== false){
							...
							if(trim($params["image"]) !== ''){
								if($importZip === true){ //we have a zip, check if exists
									$image = $mcms_filesystem->exists( $d_path.'images/'.$params["image"] );
									if(!$image){
										echo $params["image"].__(' not found!<br>', 'thunderslider');
									}else{
										if(!isset($alreadyImported['images/'.$params["image"]])){
											$importImage = ThunderSliderFunctionsMCMS::import_media($d_path.'images/'.$params["image"], $sliderParams["alias"].'/');
					...
					foreach($layers as $key=>$layer){
						if(isset($layer["image_url"])){
							...
							if(trim($layer["image_url"]) !== ''){
								if(strpos($layer["image_url"], 'http') !== false){
									...
									if($importZip === true){ //we have a zip, check if exists
										$image_url = $mcms_filesystem->exists( $d_path.'images/'.$layer["image_url"] );
										if(!$image_url){
											echo $layer["image_url"].__(' not found!<br>');
										}else{
											if(!isset($alreadyImported['images/'.$layer["image_url"]])){
												$importImage = ThunderSliderFunctionsMCMS::import_media($d_path.'images/'.$layer["image_url"], $sliderParams["alias"].'/');
						...
						if(isset($layer["bgimage_url"])){
							...
							if(trim($layer["bgimage_url"]) !== ''){
								if(strpos($layer["bgimage_url"], 'http') !== false){
									...
									if($importZip === true){ //we have a zip, check if exists
										$bgimage_url = $mcms_filesystem->exists( $d_path.'images/'.$layer["bgimage_url"] );
										if(!$bgimage_url){
											echo $layer["bgimage_url"].__(' not found!<br>');
										}else{
											if(!isset($alreadyImported['images/'.$layer["bgimage_url"]])){
												$importImage = ThunderSliderFunctionsMCMS::import_media($d_path.'images/'.$layer["bgimage_url"], $sliderParams["alias"].'/');
						...
						$link_slide = ThunderSliderFunctions::getVal($layer, 'link_slide', false);
						if($link_slide != false && $link_slide !== 'nothing'){ //link to slide/scrollunder is set, move it to actions
							if(!isset($layer['layer_action'])) $layer['layer_action'] = new stdClass();
							
							switch($link_slide){
								case 'link':
									$link = ThunderSliderFunctions::getVal($layer, 'link');
									$link_open_in = ThunderSliderFunctions::getVal($layer, 'link_open_in');
					...
					$d = apply_filters('thunderslider_importSliderFromPost_modify_data', $d, 'static', $d_path);
					...
					if($sliderExists){
						...
						$this->db->update(ThunderSliderGlobals::$table_static_slides,$arrCreate,array("slider_id"=>$sliderID));
					}else{
						$this->db->insert(ThunderSliderGlobals::$table_static_slides,$arrCreate);
					}
				}
			}
			...
			ThunderSliderModuleUpdate::update_css_styles(); //set to version 5
			ThunderSliderModuleUpdate::add_animation_settings_to_layer($c_slider); //set to version 5
			ThunderSliderModuleUpdate::add_style_settings_to_layer($c_slider); //set to version 5
			ThunderSliderModuleUpdate::change_settings_on_layers($c_slider); //set to version 5
			ThunderSliderModuleUpdate::add_general_settings($c_slider); //set to version 5
			ThunderSliderModuleUpdate::change_general_settings_5_0_7($c_slider); //set to version 5.0.7
			ThunderSliderModuleUpdate::change_layers_svg_5_2_5_5($c_slider); //set to version 5.2.5.5
			...
			if($is_template !== false){ //duplicate the slider now, as we just imported the "template"
				if($single_slide !== false){ //add now one Slide to the current Slider
					...
					if($changed){
						...
						return(array("success"=>false,"error"=>__('could not find correct Slide to copy, please try again.', 'thunderslider'),"sliderID"=>$sliderID));
					}
					
				}else{
					$mslider = new ThunderSlider();
					$title = ThunderSliderFunctions::getVal($sliderParams, 'title', 'slider1');	
					$talias = $title;
					$ti = 1;
					while($this->isAliasExistsInDB($talias)){ //set a new alias and title if its existing in database
						$talias = $title . $ti;
		...
		do_action('thunderslider_slider_imported', $real_slider_id);
	...
	public function updateSliderFromOptions($options){
		...
		$sliderID = ThunderSliderFunctions::getVal($options, "sliderid");
		ThunderSliderFunctions::validateNotEmpty($sliderID,"Slider ID");
	...
	public function updateParam($arrUpdate){
		...
		$this->db->update(ThunderSliderGlobals::$table_sliders,$arrUpdateDB,array("id"=>$this->id));
	}
	...
	public function duplicateSliderFromData($data){
		...
		ThunderSliderFunctions::validateNotEmpty($sliderID,"Slider ID");
		$this->initByID($sliderID);
		$slider_id = $this->duplicateSlider(ThunderSliderFunctions::getVal($data, "title"));
		return $slider_id;
	}
	...
	public function duplicateSliderPackageFromData($data){
		...
		$slider_uid = ThunderSliderFunctions::getVal($data, "slideruid");
		
		$uids = $tmpl->get_package_uids($slider_uid);
		
		foreach($uids as $sid => $uid){
			if($sid < 0){ //one or more still needs to be downloaded...
				return __('Please install Package first to use this feature', 'thunderslider');
			}
		}
		...
		foreach($uids as $sliderID => $uid){
			...
			$slider_id = $slider->duplicateSlider(ThunderSliderFunctions::getVal($data, "title"), true);
		}
	...
	public function duplicateSlideFromData($data){
		...
		ThunderSliderFunctions::validateNotEmpty($slideID,"Slide ID");
		$newSlideID = $this->duplicateSlide($slideID);
	...
	public function copySlideToSlider($data){
		...
		$sliderID = intval(ThunderSliderFunctions::getVal($data, "slider_id"));
		ThunderSliderFunctions::validateNotEmpty($sliderID,"Slider ID");
		$slideID = intval(ThunderSliderFunctions::getVal($data, "slide_id"));
		ThunderSliderFunctions::validateNotEmpty($slideID,"Slide ID");
		...
		//check if ID exists
		$add_to_slider = $mcmsdb->get_row($mcmsdb->prepare("SELECT * FROM $tableSliders WHERE id = %s", $sliderID), ARRAY_A);
		
		if(empty($add_to_slider))
			return __('Slide could not be duplicated', 'thunderslider');
		
		//get last slide in slider for the order
		$slide_order = $mcmsdb->get_row($mcmsdb->prepare("SELECT * FROM $tableSlides WHERE slider_id = %s ORDER BY slide_order DESC", $sliderID), ARRAY_A);
		$order = (empty($slide_order)) ? 1 : $slide_order['slide_order'] + 1;
		
		$slide_to_copy = $mcmsdb->get_row($mcmsdb->prepare("SELECT * FROM $tableSlides WHERE id = %s", $slideID), ARRAY_A);
		...
		$response = $mcmsdb->insert($tableSlides, $slide_to_copy);
		
		if($response === false) return __('Slide could not be copied', 'thunderslider');
	...
	public function copyMoveSlideFromData($data){
		...
		$targetSliderID = ThunderSliderFunctions::getVal($data, "targetSliderID");
		ThunderSliderFunctions::validateNotEmpty($sliderID,"Target Slider ID");
		$this->initByID($sliderID);
		
		if($targetSliderID == $sliderID)
			ThunderSliderFunctions::throwError("The target slider can't be equal to the source slider");
	...
	public function createSlideFromData($data,$returnSlideID = false){
		...
		$sliderID = ThunderSliderFunctions::getVal($data, "sliderid");
		$obj = ThunderSliderFunctions::getVal($data, "obj");
	...
	public function updateSlidesOrderFromData($data){
		$sliderID = ThunderSliderFunctions::getVal($data, "sliderID");
		$arrIDs = ThunderSliderFunctions::getVal($data, "arrIDs");
		ThunderSliderFunctions::validateNotEmpty($arrIDs,"slides");
		...
		foreach($arrIDs as $index=>$slideID){
			...
			if($isFromPosts){
				ThunderSliderFunctionsMCMS::updatePostOrder($slideID, $order);
			}else{
				
				$arrUpdate = array("slide_order"=>$order);
				$where = array("id"=>$slideID);
				$this->db->update(ThunderSliderGlobals::$table_slides,$arrUpdate,$where);
			}							
		}//end foreach
		...
		if($isFromPosts){
			$arrUpdate = array();
			$arrUpdate["post_sortby"] = ThunderSliderFunctionsMCMS::SORTBY_MENU_ORDER;
	...
	public function getUsedFonts($full = false){
		...
		return apply_filters('thunderslider_getUsedFonts', $gf);
	}
	...
	/**
	 * get slides from gallery respecting mcmsml
	 * force from gallery - get the slide from the gallery only
	 */
	public function getSlidesMCMSML($publishedOnly = false, $slide){
		
		$arrSlides = $this->getSlides($publishedOnly);
		
		$mslide_list = array();
		
		//check if MCMSML is active and change the ID of Slide depending on that.
		if(ThunderSliderWpml::isWpmlExists() && $this->getParam('use_mcmsml', 'off') == 'on'){
	...
	public function getSlidesFromPosts($publishedOnly = false, $gal_ids = array()){
		...
		$slideTemplates = ThunderSliderFunctions::assocToArray($slideTemplates);
		...
		switch($sourceType){
			...
			break;
			default:
				ThunderSliderFunctions::throwError("getSlidesFromPosts error: This source type must be from posts.");
			break;
		}
	...
	public function getSlidesFromStream($publishedOnly = false){
		...
		switch($sourceType){
			case "facebook":
				$facebook = new ThunderSliderFacebook($this->getParam('facebook-transient','1200'));
			...
			break;
			case "twitter":
				$twitter = new ThunderSliderTwitter($this->getParam('twitter-consumer-key'),$this->getParam('twitter-consumer-secret'),$this->getParam('twitter-access-token'),$this->getParam('twitter-access-secret'),$this->getParam('twitter-transient','1200'));
			...
			break;
			case "instagram":
				$instagram = new ThunderSliderInstagram($this->getParam('instagram-transient','1200'));
			...
			break;
			case "flickr":
				$flickr = new ThunderSliderFlickr($this->getParam('flickr-api-key'),$this->getParam('flickr-transient','1200'));
			...
			case 'youtube':
				...
				$youtube = new ThunderSliderYoutube($this->getParam('youtube-api'),$channel_id,$this->getParam('youtube-transient','1200'));
				...
				break;
			case 'vimeo':
				$vimeo = new ThunderSliderVimeo($this->getParam('vimeo-transient','1200'));
				...
				break;
			default:
				ThunderSliderFunctions::throwError("getSlidesFromStream error: This source type must be from stream.");
			break;
		}
		
		if($max_posts < 0) $max_posts *= -1;
		
		$arrPosts = apply_filters('thunderslider_pre_mod_stream_data', $arrPosts, $sourceType, $this->id);
		
		while(count($arrPosts) > $max_posts || count($arrPosts) > $max_allowed){
			array_pop($arrPosts);
		}
		
		$arrPosts = apply_filters('thunderslider_post_mod_stream_data', $arrPosts, $sourceType, $this->id);
		...
		if(empty($arrPosts)) ThunderSliderFunctions::throwError(__('Failed to load Stream', 'thunderslider'));
	...
	public function getFirstSlideIdFromGallery(){
		...
		$arrSlides = array();
		$arrSlideRecords = $this->db->fetch(ThunderSliderGlobals::$table_slides,$this->db->prepare("slider_id = %s", array($this->id)),"slide_order",'',' LIMIT 0,1');
	 ...
	 */
	public function getSlidesFromGallery($publishedOnly = false, $allmcmsml = false, $first = false){
		//global $rs_slide_template;
		$this->validateInited();
		
		$arrSlides = array();
		$arrSlideRecords = $this->db->fetch(ThunderSliderGlobals::$table_slides,$this->db->prepare("slider_id = %s", array($this->id)),"slide_order");
		
		//add Slides set by postsettings, so slide_template
		/*if(!empty($rs_slide_template)){
			foreach($rs_slide_template as $rs_s_t){
				$rs_s_t_d = $this->db->fetch(ThunderSliderGlobals::$table_slides,$this->db->prepare("id = %s", array($rs_s_t)),"slide_order");
	...
	public function getSlidesCountFromGallery($publishedOnly = false){
		...
		$sqlSelect = $this->db->prepare("select `id` from ".ThunderSliderGlobals::$table_slides." where `slider_id` = %s", array($this->id));
	...
	public function getSlidesForOutput($publishedOnly = false, $lang = 'all',$gal_ids = array()){
		...
		if(ThunderSliderWpml::isWpmlExists() && $this->getParam('use_mcmsml', 'off') == 'on'){
			$cur_lang = apply_filters( 'mcmsml_current_language', null );
			$mcmsml_current_language = apply_filters( 'mcmsml_current_language', null );
			do_action( 'mcmsml_switch_language', $lang );
		...
		if(ThunderSliderWpml::isWpmlExists() && $this->getParam('use_mcmsml', 'off') == 'on'){ //switch language back
			do_action( 'mcmsml_switch_language', $cur_lang );
		}
	...
	private function getParamsForExport(){
		...
		$urlImage = ThunderSliderFunctions::getVal($exportParams, "background_image");
		if(!empty($urlImage))
	...
	public function getSlidesForExport($useDummy = false){
		...
		return apply_filters('thunderslider_getSlidesForExport', $arrSlidesExport);
	}
	...
	public function getStaticSlideForExport($useDummy = false){
		...
		return apply_filters('thunderslider_getStaticSlideForExport', $arrSlidesExport);
	}
	...
	public function getArrSliders($orders = false, $templates = 'neither'){
		...
		$response = $this->db->fetch(ThunderSliderGlobals::$table_sliders,$where,$do_order,'',$order_direction);
	...
	public function getAllSliderAliases(){
		...
		$response = $this->db->fetch(ThunderSliderGlobals::$table_sliders,$where,"id");
	...
	public function getMaxOrder(){
		$this->validateInited();
		$maxOrder = 0;
		$arrSlideRecords = $this->db->fetch(ThunderSliderGlobals::$table_slides,$this->db->prepare("slider_id = %s", array($this->id)),"slide_order desc","","limit 1");
	...
	private function getPostsFromCategories($publishedOnly = false){
		...
		$data = ThunderSliderFunctionsMCMS::getCatAndTaxData($catIDs);
		...
		if($sortBy == ThunderSliderFunctionsMCMS::SORTBY_MENU_ORDER)
			$sortDir = ThunderSliderFunctionsMCMS::ORDER_DIRECTION_ASC;
		
		//Events integration
		$arrAddition = array();
		if($publishedOnly == true)			
			$arrAddition["post_status"] = ThunderSliderFunctionsMCMS::STATE_PUBLISHED;
		
		if(ThunderSliderEventsManager::isEventsExists()){
			
			$filterType = $this->getParam("events_filter",ThunderSliderEventsManager::DEFAULT_FILTER);				
			$arrAddition = ThunderSliderEventsManager::getMCMSQuery($filterType, $sortBy);
		}
		
		$slider_id = $this->getID();
		$arrPosts = ThunderSliderFunctionsMCMS::getPostsByCategory($slider_id, $catIDs,$sortBy,$sortDir,$maxPosts,$postTypes,$taxonomies,$arrAddition);
	...
	public function getPostsFromRelated(){
		...
		$get_relateds = apply_filters('thunderslider_get_related_posts', $query, $post_id);
	...
	public function getPostsFromPopular($max_posts = false){
		...
		$args = apply_filters('thunderslider_get_popular_posts', $args, $post_id);
	...
	public function getPostsFromRecent($max_posts = false){
		...
		$args = apply_filters('thunderslider_get_latest_posts', $args, $post_id);
	...
	private function getProductsFromCategories($publishedOnly = false){
		...
		$arrAddition = array_merge($arrAddition, ThunderSliderWooCommerce::getMetaQuery($this->getParams()));
	...
	private function getPostsFromSpecificList($gal_ids = array()){
		...
		if(!empty($gal_ids) && $gal_ids[0]){
			$strPosts = $gal_ids;
			$strPosts = apply_filters('thunderslider_set_posts_list_gal', $strPosts, $this->getID());
			$is_gal = true;
		}else{
			if(isset($gal_ids[0])){
				unset($gal_ids[0]);
			...
			$strPosts = apply_filters('thunderslider_set_posts_list', $strPosts, $this->getID());
		}
		
		$slider_id = $this->getID();
		
		$arrPosts = ThunderSliderFunctionsMCMS::getPostsByIDs($strPosts, $slider_id, $is_gal, $additional);
	...
	public function updatePostsSortbyFromData($data){
		...
		$sliderID = ThunderSliderFunctions::getVal($data, "sliderID");
		$sortBy = ThunderSliderFunctions::getVal($data, "sortby");
		ThunderSliderFunctions::validateNotEmpty($sortBy,"sortby");
	...
	public function replaceImageUrlsFromData($data){
		...
		$sliderID = ThunderSliderFunctions::getVal($data, "sliderid");
		$urlFrom = ThunderSliderFunctions::getVal($data, "url_from");
		ThunderSliderFunctions::validateNotEmpty($urlFrom,"url from");
		$urlTo = ThunderSliderFunctions::getVal($data, "url_to");
 ...
 * @since: 5.0
 **/
class ThunderSlider extends ThunderSliderSlider {}
?>