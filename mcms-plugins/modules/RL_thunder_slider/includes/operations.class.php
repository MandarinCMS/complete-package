...
class ThunderSliderOperations extends ThunderSliderElementsBase{
	...
	public static function getArrAnimations($all = true){
		...
		$arrAnimations['custom'] = array('handle' => __('## Custom Animation ##', 'thunderslider'));
		$arrAnimations['vSFXs'] = array('handle' => '-----------------------------------');
		$arrAnimations['vSFX'] = array('handle' => __('- SPECIAL EFFECTS -', 'thunderslider'));
		...
		$arrAnimations['v5'] = array('handle' => __('- VERSION 5.0 ANIMATIONS -', 'thunderslider'));
		...
		$arrAnimations['v4'] = array('handle' => __('- VERSION 4.0 ANIMATIONS -', 'thunderslider'));
		...
		if($all){
			...
			$arrAnimations['vs'] = array('handle' => __('- SAVED CUSTOM ANIMATIONS -', 'thunderslider'));
			$arrAnimations['vse'] = array('handle' => '--------------------------------------');
		
			//$custom = ThunderSliderOperations::getCustomAnimations('customin');
			$custom = ThunderSliderOperations::getCustomAnimationsFullPre('customin');
	...
	public static function getArrEndAnimations($all = true){
		$arrAnimations = array();
		$arrAnimations['custom'] = array('handle' => __('## Custom Animation ##', 'thunderslider'));
		$arrAnimations['auto'] = array('handle' => __('Automatic Reverse', 'thunderslider'));
		$arrAnimations['vSFXs'] = array('handle' => '-----------------------------------');
		$arrAnimations['vSFX'] = array('handle' => __('- SPECIAL EFFECTS -', 'thunderslider'));
		$arrAnimations['vSFXe'] = array('handle' => '-----------------------------------');
		...
		if($all){
			...
			$arrAnimations['vs'] = array('handle' => __('- SAVED CUSTOM ANIMATIONS -', 'thunderslider'));
			$arrAnimations['vse'] = array('handle' => '--------------------------------------');
			//$custom = ThunderSliderOperations::getCustomAnimations('customout');
			$custom = ThunderSliderOperations::getCustomAnimationsFullPre('customout');
	...
	public static function insertCustomAnim($anim){
		if(isset($anim['handle'])) {
			...
			$result = $db->insert(ThunderSliderGlobals::$table_layer_anims, $arrInsert);
		}

		$arrAnims['customin'] = ThunderSliderOperations::getCustomAnimations();
		$arrAnims['customout'] = ThunderSliderOperations::getCustomAnimations('customout');
		$arrAnims['customfull'] = ThunderSliderOperations::getFullCustomAnimations();
	...
	public static function updateCustomAnim($anim){
		...
		if(isset($anim['handle'])) {
			...
			$result = $db->update(ThunderSliderGlobals::$table_layer_anims, $arrUpdate, array('id' => $id));
		}
	...
	public static function updateCustomAnimName($anim){
		if(isset($anim['handle'])) {
			...
			$result = $db->update(ThunderSliderGlobals::$table_layer_anims, $anim, array('id' => $id));
		}
	...
	public static function deleteCustomAnim($rawID){
		...
		if(trim($rawID) != '') {
			...
			$db->delete(ThunderSliderGlobals::$table_layer_anims, $db->prepare("id = %s", array(intval($id))));
		}
	...
	public static function fillAnimations(){
		if(empty(self::$animations)){
			...
			$result = $db->fetch(ThunderSliderGlobals::$table_layer_anims);
			if(!empty($result)){
	...
	public function getArrCaptionClasses($contentCSS){
		...
		$parser = new ThunderSliderCssParser();
	...
	public function getArrFontFamilys($slider = false){
		...
		$fonts = array(
			// GOOGLE Loaded Fonts
			array('type' => 'websafe', 'version' => __('Loaded Google Fonts', 'thunderslider'), 'label' => 'Dont Show Me'),

			//Serif Fonts
			array('type' => 'websafe', 'version' => __('Serif Fonts', 'thunderslider'), 'label' => 'Georgia, serif'),
			array('type' => 'websafe', 'version' => __('Serif Fonts', 'thunderslider'), 'label' => '"Palatino Linotype", "Book Antiqua", Palatino, serif'),
			array('type' => 'websafe', 'version' => __('Serif Fonts', 'thunderslider'), 'label' => '"Times New Roman", Times, serif'),

			//Sans-Serif Fonts
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => 'Arial, Helvetica, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => '"Arial Black", Gadget, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => '"Comic Sans MS", cursive, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => 'Impact, Charcoal, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => '"Lucida Sans Unicode", "Lucida Grande", sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => 'Tahoma, Geneva, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => '"Trebuchet MS", Helvetica, sans-serif'),
			array('type' => 'websafe', 'version' => __('Sans-Serif Fonts', 'thunderslider'), 'label' => 'Verdana, Geneva, sans-serif'),

			//Monospace Fonts
			array('type' => 'websafe', 'version' => __('Monospace Fonts', 'thunderslider'), 'label' => '"Courier New", Courier, monospace'),
			array('type' => 'websafe', 'version' => __('Monospace Fonts', 'thunderslider'), 'label' => '"Lucida Console", Monaco, monospace')
		);
		...
		/*if($slider !== false){
			...
			if(is_array($font_custom)){
				foreach($font_custom as $key => $curFont){
					...
					if($font != false)
						$font_custom[$key] = array('version' => __('Depricated Google Fonts', 'thunderslider'), 'label' => $font);
					else
		...
		foreach($googlefonts as $f => $val){
			$fonts[] = array('type' => 'googlefont', 'version' => __('Google Fonts', 'thunderslider'), 'label' => $f, 'variants' => $val['variants'], 'subsets' => $val['subsets']);
		}
		
		return apply_filters('thunderslider_operations_getArrFontFamilys', $fonts);
	}
	...
	private function getHtmlSelectCaptionClasses($contentCSS){
		...
		$htmlSelect = ThunderSliderFunctions::getHTMLSelect($arrCaptions,"","id='layer_caption' name='layer_caption'",true);
		return($htmlSelect);
	}
	...
	public static function fillCSS(){
		if(empty(self::$css)){
			...
			$customCss = array();
			$result = $db->fetch(ThunderSliderGlobals::$table_css);
			if(!empty($result)){
				$customCss = $result;
			}
	...
	public function getCaptionsContent(){
		...
		$result = self::$css;
		$contentCSS = ThunderSliderCssParser::parseDbArrayToCss($result);
		return($contentCSS);
	}
	...
	public static function getCaptionsContentArray($handle = false){
		...
		$result = self::$css;
		$contentCSS = ThunderSliderCssParser::parseDbArrayToArray($result, $handle);
		return($contentCSS);
	}
	...
	public static function getStaticCss(){
		/*if ( is_multisite() ){
			if(!get_site_option('thunderslider-static-css')){
			...
			$contentCSS = get_site_option('thunderslider-static-css', '');
		}else{*/
			if(!get_option('thunderslider-static-css')){
				if(file_exists(RS_PLUGIN_PATH.'public/assets/css/static-captions.css')){
					$contentCSS = @file_get_contents(RS_PLUGIN_PATH.'public/assets/css/static-captions.css');
					self::updateStaticCss($contentCSS);
				}
			}
			$contentCSS = get_option('thunderslider-static-css', '');
		//}
	...
	public static function updateStaticCss($content){
		...
		/*if ( is_multisite() ){
			$c = get_site_option('thunderslider-static-css', '');
			$c = update_site_option('thunderslider-static-css', $content);
		}else{*/
			$c = get_option('thunderslider-static-css', '');
			$c = ThunderSliderFunctionsMCMS::update_option('thunderslider-static-css', $content, 'off');
		//}
	...
	public function insertCaptionsContentData($content){
		...
		$result = $db->insert(ThunderSliderGlobals::$table_css, $arrInsert);

		//output captions array
		$arrCaptions = ThunderSliderCssParser::get_captions_sorted();
	...
	public function updateCaptionsContentData($content){
		...
		$styles = $db->fetchSingle(ThunderSliderGlobals::$table_css, $db->prepare('`handle` = %s', array('.tp-caption.'.$content['handle'])));
		...
		$result = $db->update(ThunderSliderGlobals::$table_css, $arrUpdate, array('handle' => '.tp-caption.'.$handle));
	...
	public function updateAdvancedCssData($data){
		...
		//get current styles
		$styles = $db->fetchSingle(ThunderSliderGlobals::$table_css, $db->prepare('`handle` = %s', array($data['handle'])));
		...
		if(!empty($styles)){
			...
			$result = $db->update(ThunderSliderGlobals::$table_css, $arrUpdate, array('handle' => $data['handle']));
	...
	public function deleteCaptionsContentData($handle){
		...
		$db->delete(ThunderSliderGlobals::$table_css, $db->prepare("handle= %s", array(".tp-caption.".$handle)));
	...
	public static function updateDynamicCaptions($full = false){
		if($full){
			$captions = array();
			$captions = ThunderSliderOperations::getCaptionsContentArray();

			$styles = ThunderSliderCssParser::parseArrayToCss($captions, "\n");
		}else{
			...
			if(!empty($classes)){
				...
				foreach($classes as $class => $val){
					$captionCheck = ThunderSliderOperations::getCaptionsContentArray($class);
	...
	public static function getCaptionsCssContentArray(){
		if(file_exists(RS_PLUGIN_PATH.'public/assets/css/captions.css'))
			...
			ThunderSliderFunctions::throwError("No captions.css found! This installation is incorrect, please make sure to reupload the RazorLeaf ThunderSlider module and try again!");
		
		$result = ThunderSliderCssParser::parseCssToArray($contentCSS);
	...
	public static function importCaptionsCssContentArray(){
		...
		if(is_array($css) && $css !== false && count($css) > 0){
			foreach($css as $class => $styles){
				...
				$result = $db->fetch(ThunderSliderGlobals::$table_css, $db->prepare("handle = %s", array($class)));

				if(!empty($result)){ //update
					$db->update(ThunderSliderGlobals::$table_css, $arrInsert, array('handle' => $class));
				}else{ //insert
					$arrInsert["handle"] = $class;
					$db->insert(ThunderSliderGlobals::$table_css, $arrInsert);
				}
			}
		}

		if(!empty($static)){ //save static into static-captions.css
			$css = ThunderSliderCssParser::parseStaticArrayToCss($static);
			$static_cur = ThunderSliderOperations::getStaticCss(); //get the open sans line!
			$css = $static_cur."\n".$css;
	...
	public static function moveOldCaptionsCss(){
		if(file_exists(ThunderSliderGlobals::$filepath_captions_original))
			$success = @rename(ThunderSliderGlobals::$filepath_captions_original, ThunderSliderGlobals::$filepath_backup.'/captions-original.css');

		if(file_exists(ThunderSliderGlobals::$filepath_captions))
			$success = @rename(ThunderSliderGlobals::$filepath_captions, ThunderSliderGlobals::$filepath_backup.'/captions.css');
	}
	...
	public function previewOutput($sliderID,$output = null){
		...
		if($isWpmlExists && $useWpml == "on"){
			$mcmsmlActive = true;
			$arrLanguages = ThunderSliderWpml::getArrLanguages(false);

			//set current lang to output
			$currentLang = ThunderSliderFunctions::getPostGetVariable("lang");

			if(empty($currentLang))
				$currentLang = ThunderSliderWpml::getCurrentLang();
			...
			$selectLangChoose = ThunderSliderFunctions::getHTMLSelect($arrLanguages,$currentLang,"id='select_langs'",true);
		}
		...
		//put the output html
		$urlModule = RS_PLUGIN_URL.'public/assets/';
		$urlPreviewPattern = ThunderSliderBase::$url_ajax_actions."&client_action=preview_slider&sliderid=".$sliderID."&lang=[lang]&nonce=[nonce]";
		...
		<html>
			<head>
				<link rel='stylesheet' href='<?php echo $urlModule; ?>css/settings.css?rev=<?php echo ThunderSliderGlobals::SLIDER_REVISION; ?>' type='text/css' media='all' />
				<link rel='stylesheet' href='<?php echo $urlModule; ?>fonts/font-awesome/css/font-awesome.css?rev=<?php echo ThunderSliderGlobals::SLIDER_REVISION; ?>' type='text/css' media='all' />
				<link rel='stylesheet' href='<?php echo $urlModule; ?>fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css?rev=<?php echo ThunderSliderGlobals::SLIDER_REVISION; ?>' type='text/css' media='all' />
				<?php
				$db = new ThunderSliderDB();
				...
				echo '<style type="text/css">'.ThunderSliderCssParser::compress_css($custom_css).'</style>';
				...
				if(is_array($googleFont)){
					...
					//$gfonts .= ThunderSliderOperations::getCleanFontImport($googleFont);
				}
				...
				foreach($gf as $gfk => $gfv){
					...
					//$gfonts .= ThunderSliderOperations::getCleanFontImport($gfk, '', '', $variants, $subsets);
				}
				
				echo ThunderSliderOperations::printCleanFontImport();
				//echo $gfonts;
				?>

				<script type='text/javascript' src='<?php echo $setBase; ?>code.jquery.com/jquery-latest.min.js'></script>

				<script type='text/javascript' src='<?php echo $urlModule; ?>js/jquery.myskinpunch.tools.min.js?rev=<?php echo ThunderSliderGlobals::SLIDER_REVISION; ?>'></script>
				<script type='text/javascript' src='<?php echo $urlModule; ?>js/jquery.myskinpunch.revolution.min.js?rev=<?php echo ThunderSliderGlobals::SLIDER_REVISION; ?>'></script>
				<?php
				$setRevStartSize = ThunderSliderFront::add_setREVStartSize();
				echo $setRevStartSize;
				?>
				<?php
				do_action('thunderslider_preview_slider_head');
				?>
			</head>
			<body style="padding:0px;margin:0px;width:100%;height:100%;position:relative;">
				<?php
				if($mcmsmlActive == true){
					?>
					<div style="margin-bottom:10px;text-align:center;">
					<?php _e("Choose language",'thunderslider'); ?>: <?php echo $selectLangChoose; ?>
					</div>
				...
				<?php
					...
					do_action('thunderslider_preview_slider_footer', $slider, $output);
				?>
			</body>
		</html>
		<?php
	...
	/*
	 * show only the markup for jQuery version of module
	 */
	public function previewOutputMarkup($sliderID){
		...
		if(file_exists(ThunderSliderGlobals::$uploadsUrlExportZip)){
			@unlink(ThunderSliderGlobals::$uploadsUrlExportZip); //delete file to start with a fresh one
		}
		
		$usepcl = false;
		if(class_exists('ZipArchive')){
			$zip = new ZipArchive;
			$success = $zip->open(ThunderSliderGlobals::$uploadsUrlExportZip, ZIPARCHIVE::CREATE | ZipArchive::OVERWRITE);
			
			if($success !== true){
				echo __("No write permissions. Can't create zip file: ", 'thunderslider').ThunderSliderGlobals::$uploadsUrlExportZip;
				exit;
			}
		}else{
			//fallback to pclzip
			require_once(BASED_TREE_URI . 'mcms-admin/includes/class-pclzip.php');
			
			$pclzip = new PclZip(ThunderSliderGlobals::$uploadsUrlExportZip);
		...
		if($sliderID == "empty_output"){
			echo __("Wrong request!", 'thunderslider');
			exit;
		}
		...
		$static_css = ThunderSliderOperations::getStaticCss();
	...
	<head>
		...
		<title><?php echo $slider->getTitle(); ?> - RazorLeaf ThunderSlider</title>
		<meta name="description" content="RazorLeaf ThunderSlider Example" />
		<meta name="keywords" content="fullscreen image, grid layout, flexbox grid, transition" />
		<meta name="author" content="MandarinCMS" />
		...
		if($export_real){ 
			...
			if($static_css !== ''){
				...
				echo ThunderSliderCssParser::compress_css($static_css);
				echo '</style>'."\n";
			}
		}else{
			?>
		...
		?>
		<!-- REVOLUTION JS FILES -->
		<script type="text/javascript" src="<?php echo $path_js; ?>jquery.myskinpunch.tools.min.js"></script>
		<script type="text/javascript" src="<?php echo $path_js; ?>jquery.myskinpunch.revolution.min.js"></script>
	...
	<body>
		...
		<article class="content">
			...
			<section class="header">
				...
				<a class="button" style="float:right" target="_blank" href="http://www.jiiworks.net/thundersliderjquery-doc/slider-revolution-jquery-5-x-documentation/"><i class="pe-7s-help2"></i>Online Documentation</a>
		...
		?>
		  <article class="small-history"> 
            <h2 class="textaligncenter" style="margin-bottom:25px;">Your RazorLeaf ThunderSlider jQuery Module</h2>
            <p>RazorLeaf ThunderSlider is an innovative, responsive Slider Module that displays your content the beautiful way. Whether it's a <strong>Slider, Carousel, Hero Scene</strong> or even a whole <strong>Front Page</strong>.<br>The <a href="https://jiiworks.net/item/slider-revolution-jquery-visual-editor-addon/13934907" target="_blank">visual drag &amp; drop editor</a> will help you to create your Sliders and tell your own stories in no time!</p>
        </article>
		<?php
		}
		?>
	...
	<?php if(!$export_real){ ?>
        ...
        <article class="small-history bottom-history">
            ...
            <p>We would always recommend to use our<a target="_blank" href="http://www.jiiworks.net/thundersliderjquery-doc/slider-revolution-jquery-5-x-documentation/"> online documentation</a> however you can find also our embeded local documentation zipped in the Documentation folder. Online Documentation and FAQ Page is regulary updated. You will find More examples, Visit us also at <a href="http://jiiworks.net">http://jiiworks.net</a> ! </p>
    ...
    <footer>
        <div class="footer_inner">
            <div class="footerwidget">
                <h3>RazorLeaf ThunderSlider</h3>
                <a href="http://jiiworks.net/jquery/#features" target="_self">Features</a>
                <a href="http://jiiworks.net/examples-jquery/" target="_self">Usage Examples</a>
                <a href="http://www.jiiworks.net/thundersliderjquery-doc/slider-revolution-jquery-5-x-documentation/" target="_blank">Online Documentation</a>
            </div>
            <div class="footerwidget">
                <h3>Resources</h3>
                <a href="http://www.jiiworks.net/support-center/" target="_blank">FAQ Database</a>
                <a href="http://jiiworks.net" target="_blank">MandarinCMS.com</a>
                <a href="http://myskinpunch.us9.list-manage.com/subscribe?u=a5738148e5ec630766e28de16&amp;id=3e718acc63" target="_blank">Newsletter</a>
                <a href="http://www.jiiworks.net/products/" target="_blank">Modules</a>
                <a href="http://www.jiiworks.net/products/" target="_blank">Themes</a>
            </div>
            <div class="footerwidget">
                <h3>More Versions</h3>
                <a href="http://jiiworks.net" target="_blank">MandarinCMS</a>
                <a href="http://jiiworks.net/item/slider-revolution-responsive-prestashop-module/7140939?ref=myskinpunch" target="_blank">Prestashop</a>
                <a href="http://jiiworks.net/item/slider-revolution-responsive-magento-extension/9332896?ref=myskinpunch" target="_blank">Magento</a>
                <a href="http://jiiworks.net/item/slider-revolution-responsive-opencart-module/9994648?ref=myskinpunch" target="_blank">OpenCart</a>
                <a href="http://jiiworks.net/item/slider-revolution-responsive-drupal-module/12041755?ref=myskinpunch" target="_blank">Drupal</a>
            </div>
            <div class="footerwidget social">
                <h3>Follow Us</h3>
                <ul>
                    <li><a href="https://www.facebook.com/myskinpunchofficial" target="_blank" class="so_facebook" data-rel="tooltip" data-animation="false" data-placement="bottom" data-original-title="Facebook"><i class="s_icon fa-icon-facebook 
						"></i></a>
                    </li>
                    <li><a href="https://twitter.com/myskinpunch" target="_blank" class="so_twitter" data-rel="tooltip" data-animation="false" data-placement="bottom" data-original-title="Twitter"><i class="s_icon fa-icon-twitter"></i></a>
                    </li>
                    <li><a href="https://plus.google.com/+MandarinCMS/posts" target="_blank" class="so_gplus" data-rel="tooltip" data-animation="false" data-placement="bottom" data-original-title="Google+"><i class="s_icon fa-icon-google-plus"></i></a>
                    </li>
                </ul>
            </div>
            <div class="clearfix"></div>
...
ob_end_clean();
		...
		$upload_dir = ThunderSliderFunctionsMCMS::getPathUploads();
		$upload_dir_multisiteless = mcms_upload_dir();
		...
		$search = apply_filters('thunderslider_html_export_replace_urls', $search);
		...
		if($export_real){ //only include if real export
			...
			if(!$usepcl){
				...
				$zip->addFile(RS_PLUGIN_PATH.'/public/assets/js/jquery.myskinpunch.enablelog.js', 'js/jquery.myskinpunch.enablelog.js');
				$zip->addFile(RS_PLUGIN_PATH.'/public/assets/js/jquery.myskinpunch.revolution.min.js', 'js/jquery.myskinpunch.revolution.min.js');
				$zip->addFile(RS_PLUGIN_PATH.'/public/assets/js/jquery.myskinpunch.tools.min.js', 'js/jquery.myskinpunch.tools.min.js');
				...
				$pclzip->add(RS_PLUGIN_PATH.'public/assets/js/jquery.myskinpunch.enablelog.js', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				$pclzip->add(RS_PLUGIN_PATH.'public/assets/js/jquery.myskinpunch.revolution.min.js', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
				$pclzip->add(RS_PLUGIN_PATH.'public/assets/js/jquery.myskinpunch.tools.min.js', PCLZIP_OPT_REMOVE_PATH, RS_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
			...
			$notice_text = "";
			$notice_text .= __('Using this data is only allowed with a valid licence of the jQuery RazorLeaf ThunderSlider Module, which can be found at CodeCanyon: http://jiiworks.net/item/slider-revolution-responsive-jquery-module/2580848?ref=myskinpunch', 'thunderslider');
		...
		header("Expires: 0");
		readfile(ThunderSliderGlobals::$uploadsUrlExportZip);
		
		@unlink(ThunderSliderGlobals::$uploadsUrlExportZip); //delete file after sending it to user
		exit();
	}
	...
	public function loadingMessageOutput(){
		?>
		<div class="message_loading_preview"><?php _e("Loading Preview...",'thunderslider')?></div>
		<?php
	}
	...
	public function putSlidePreviewByData($data){
		...
		$data = ThunderSliderFunctions::jsonDecodeFromClientSide($data);
	...
	public function updateGeneralSettings($data){
		...
		update_option('thunderslider-global-settings', $data);
	...
	static function getGeneralSettingsValues(){
		...
		$arrValues = get_option('thunderslider-global-settings', '');
	...
	public static function getPostTypesWithCatsForClient(){
		$arrPostTypes = ThunderSliderFunctionsMCMS::getPostTypesWithCats();
	...
	public static function setCleanFontImport($font, $class = '', $url = '', $variants = array(), $subsets = array()){
		global $thunderslider_fonts;
		
		$ret = '';
		
		if(!isset($thunderslider_fonts)) $thunderslider_fonts = array(); //if this is called without thunderslider.php beeing loaded
		
		$do_print = false;
		$tcf = '';
		if(!empty($variants) || !empty($subsets)){
			if(!isset($thunderslider_fonts[$font])) $thunderslider_fonts[$font] = array();
			if(!isset($thunderslider_fonts[$font]['variants'])) $thunderslider_fonts[$font]['variants'] = array();
			if(!isset($thunderslider_fonts[$font]['subsets'])) $thunderslider_fonts[$font]['subsets'] = array();
			
			if(!empty($variants)){
				foreach($variants as $k => $v){
					if(!in_array($v, $thunderslider_fonts[$font]['variants'])){
						$thunderslider_fonts[$font]['variants'][] = $v;
			...
			if(!empty($subsets)){
				foreach($subsets as $k => $v){
					if(!in_array($v, $thunderslider_fonts[$font]['subsets'])){
						$thunderslider_fonts[$font]['subsets'][] = $v;
					}else{ //already included somewhere, so do not call it anymore
						unset($subsets[$k]);
					}
				}
			...
			/*if(in_array($font, $thunderslider_fonts)){
				$ret = '';
				$do_print = false;
			}else{
				$do_print = true;
		...
		/*if($do_print){
			...
			if(!isset($thunderslider_fonts[$font])){
				$thunderslider_fonts[$font] = array();
			}
			if(strpos($font, "href=") === false){ //fallback for old versions
				$url = ThunderSliderFront::modify_punch_url($setBase . 'fonts.googleapis.com/css?family=');
		...
		return apply_filters('thunderslider_getCleanFontImport', $ret, $font, $class, $url, $variants, $subsets);*/
	}
	...
	public static function printCleanFontImport(){
		...
		if(!empty($thunderslider_fonts)){
			foreach($thunderslider_fonts as $f_n => $f_s){
				if($f_n !== ''){
		...
		return apply_filters('thunderslider_printCleanFontImport', $ret);
	}
	...
	public function checkPurchaseVerification($data){
		...
		do{	
			...
			$response = mcms_remote_post($url.'/activate.php', array(
		...
		if($version_info == 'valid'){
			...
			update_option('thunderslider-code', $data['code']);
			//update_option('thunderslider-email', $data['email']);
			update_option('thunderslider-temp-active-notice', 'false');
			return true;
		}elseif($version_info == 'exist'){
			return 'exist';
			//ThunderSliderFunctions::throwError(__('Purchase Code already registered!', 'thunderslider'));
		}elseif($version_info == 'temp_valid'){ //only temporary active, rechecking needs to be done soon on the myskinpunch servers (envato API may be down)
			update_option('thunderslider-valid', 'true');
			update_option('thunderslider-code', $data['code']);
			//update_option('thunderslider-email', $data['email']);
			update_option('thunderslider-temp-active', 'true');
			update_option('thunderslider-temp-active-notice', 'false');
			return 'temp';
		}else{
			return false;
	...
	public function doPurchaseDeactivation($data){
		...
		do{	
			$url 		= $rslb->get_url('updates');
			$response	= mcms_remote_post($url.'/deactivate.php', array(
				'user-agent' => 'MandarinCMS/'.$mcms_version.'; '.get_bloginfo('url'),
				'body' => array(
					'code' => urlencode($code),
					'product' => urlencode(RS_PLUGIN_SLUG)
		...
		if($version_info == 'valid'){
			...
			update_option('thunderslider-code', '');
			return true;
		}else{
			return false;
		}
	...
	public static function get_slider_speed($sliderID){
		//$data = mcms_get_attachment_metadata($cur_img_id);
			...
			foreach($slides as $key => $slide){
				...
				if(isset($slide['params']['image']) && $slide['params']['image'] != ''){
					...
					if(isset($slide['params']['image_id'])){
						...
						$urlImage = mcms_get_attachment_image_src($slide['params']['image_id'], $slide['params']['image_source_type']);
					}
					if($urlImage === false){
						$cur_img_id = ThunderSliderFunctionsMCMS::get_image_id_by_url($slide['params']['image']);
						if($cur_img_id !== false){
							$urlImage = mcms_get_attachment_image_src($cur_img_id, $slide['params']['image_source_type']);
						}
					}
					...
					if(strpos($slide_id, 'static_') !== false){
						$infos['url'] = ThunderSliderBaseAdmin::getViewUrl(ThunderSliderAdmin::VIEW_SLIDE, 'id=static_'.$sliderID);
					}else{
						$infos['url'] = ThunderSliderBaseAdmin::getViewUrl(ThunderSliderAdmin::VIEW_SLIDE, 'id='.$slide_id);
					}
				...
				if(isset($slide['layers']) && !empty($slide['layers']) && count($slide['layers']) > 0){
					...
					foreach($slide['layers'] as $lKey => $layer){
						switch($layer['type']){
							case 'image':
								...
								if(isset($layer['image_url']) && trim($layer['image_url']) != ''){
									...
									$cur_img_id = ThunderSliderFunctionsMCMS::get_image_id_by_url($layer['image_url']);
									if($cur_img_id !== false){
										if(!isset($layer['layer-image-size']) || $layer['layer-image-size'] == 'auto') $layer['layer-image-size'] = $slide['params']['image_source_type'];
										
										$urlImage = mcms_get_attachment_image_src($cur_img_id, $layer['layer-image-size']);
							...
							case 'video':
								...
								if(!empty($poster_img)){
									foreach($poster_img as $img){
										...
										$cur_img_id = ThunderSliderFunctionsMCMS::get_image_id_by_url($img);
											...
											case 'html5':
												...
												if(!empty($video_arr)){
													foreach($video_arr as $type => $url){
														$cur_id = ThunderSliderFunctionsMCMS::get_image_id_by_url($url);
						...
						if(isset($layer['layer_action']) && !empty($layer['layer_action'])){
							...
							$a_action = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($layer['layer_action'], 'action', array()));
							$a_link_type = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($layer['layer_action'], 'link_type', array()));
		...
		<span class="tp-monitor-performance-title"><?php echo __("Overall Slider Performance",'thunderslider'); ?></span>
		<span class="tp-monitor-performace-wrap">
			...
			<span class="tp-monitor-slow"><?php echo __("Slow",'thunderslider'); ?></span>
			<span class="tp-monitor-ok"><?php echo __("Ok",'thunderslider'); ?></span>
			<span class="tp-monitor-fast"><?php echo __("Fast",'thunderslider'); ?></span>
		</span>
		...
		<span  class="tp-monitor-speed-table tp-monitor-single-speed">
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo __("Load Speed UMTS:",'thunderslider'); ?></span>
				<span class="tp-monitor-total-subsize" id="umts-speed">%umtsspeed-single%</span>
			</span>
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo __("Load Speed DSL:",'thunderslider'); ?></span>
				<span class="tp-monitor-total-subsize" id="dsl-speed">%dslspeed-single%</span>
			</span>
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo __("Load Speed T1:",'thunderslider'); ?></span>
		...
		<span style="float:left;width:165px">
			<span class="tp-monitor-smalllabel"><?php echo __("Total Slider Size:",'thunderslider'); ?></span>
		...
		<span style="float:right; width:165px">
			<span class="tp-monitor-smalllabel"><?php echo __("Preloaded Slides Size:",'thunderslider'); ?></span>
		...
		<div id="monitor-problems" style="display:none">
			<span class="tp-monitor-performance-title"><?php echo __("Need Some Attention",'thunderslider'); ?></span>			
			<span class="tp-clearfix" style="height:25px"></span>
		...
		if(!empty($used_images)){
			?>
			<!-- THE IMAGE PERFORMANCE MESSING -->
			<span class="tp-monitor-performance-title"><?php echo __("Image Performance",'thunderslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<span style="float:left;width:40%">
				<span class="tp-monitor-smalllabel"><?php echo __("Images Loaded:",'thunderslider'); ?></span>
			...
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-image-details" data-open="</span><?php echo __("Hide Details",'thunderslider'); ?>" data-close="</span><?php echo __("Show Details",'thunderslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details",'thunderslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
			<!-- THE IMAGE LIST -->
				...
				foreach($used_images as $path => $image){		
					...
					if(isset($image['size'])) {
						...
						if($image['id'] == 'external'){
							$_li .=   '<span class="tp-monitor-neutral"></span><span class="tp-monitor-size">'.__('extern', 'thunderslider').'</span>';
						}else{
							$_li .=   '<span class="tp-monitor-warning"></span><span class="tp-monitor-size">'.__('missing', 'thunderslider').'</span>';
						}
					}
		...
		?>
			...
			<span class="tp-monitor-performance-title"><?php echo __("Video Performance",'thunderslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo __("Videos Loaded (max):",'thunderslider'); ?></span>
				<?php if ($video_size>0) {?>				
					<span class="tp-monitor-imageicon"></span>
					<span id="video_sub_size" class="tp-monitor-total-subsize"><?php echo size_format($video_size,2); ?></span>
				<?php } else {?>
					<span class="tp-monitor-imageicon"></span>
					<span class="tp-monitor-total-subsize"><?php echo __("Unknown",'thunderslider'); ?></span>
				<?php } ?>
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-video-details" data-open="</span><?php echo __("Hide Details",'thunderslider'); ?>" data-close="</span><?php echo __("Show Details",'thunderslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details",'thunderslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
		...
		<!-- THE IMAGE PERFORMANCE MESSING -->
		<span class="tp-monitor-performance-title"><?php echo __("CSS Performance",'thunderslider'); ?></span>
		<span class="tp-monitor-performace-wrap">
		...
		<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
		<span style="float:left;width:40%">
			<span class="tp-monitor-smalllabel"><?php echo __("CSS Loaded:",'thunderslider'); ?></span>
			<span class="tp-monitor-cssicon"></span><span id="css_sub_size" class="tp-monitor-total-subsize">%css_size%</span>
			</span>
		<span style="float:left;width:60%; text-align:right;">
			<span class="tp-monitor-showdetails" data-target="#monitor-CSS-details" data-open="</span><?php echo __("Hide Details",'thunderslider'); ?>" data-close="</span><?php echo __("Show Details",'thunderslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details",'thunderslider'); ?></span></span>
		</span>
		<span class="tp-clearfix" style="height:15px"></span>
		...
		if(file_exists( RS_PLUGIN_PATH . '/public/assets/css/settings.css' )){
			...
			$_li .= '<span class="tp-monitor-file">';
			$_li .= __('css/settings.css','thunderslider');
			$_li .= '</span>';
		...
		$_li .= '<span class="tp-monitor-file">';
		$_li .= __('Static Styles','thunderslider');
		$_li .= '</span>';
		...
		*/
		...
		if(!empty($used_captions)){
			...
			foreach($used_captions as $class => $val){
				$cap = ThunderSliderOperations::getCaptionsContentArray($class);
				if(!empty($cap))
					$captions[] = $cap;
			}
			$styles = ThunderSliderCssParser::parseArrayToCss($captions, "\n");
			...
			$_li .=  '<span class="tp-monitor-file">';
			$_li .=  __('Dynamic Styles','thunderslider');
			$_li .=  '</span>';
		...
		<span class="tp-clearfix" style="height:15px"></span>
			...
			<span class="tp-monitor-performance-title"><?php echo __("jQuery Performance",'thunderslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo __("jQuery Loaded:",'thunderslider'); ?></span>				
				<span class="tp-monitor-imageicon"></span><span id="jquery_sub_size" class="tp-monitor-total-subsize">%js_size%</span>				
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-jquery-details" data-open="</span><?php echo __("Hide Details",'thunderslider'); ?>" data-close="</span><?php echo __("Show Details",'thunderslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details",'thunderslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
		...
		$jsfiles = array(
			'jquery.myskinpunch.tools.min.js' => RS_PLUGIN_PATH . '/public/assets/js/jquery.myskinpunch.tools.min.js',
			'jquery.myskinpunch.revolution.min.js' => RS_PLUGIN_PATH . '/public/assets/js/jquery.myskinpunch.revolution.min.js',
		...
		<span class="tp-clearfix" style="height:15px"></span>
			...
			<span class="tp-monitor-performance-title"><?php echo __("Google Fonts Performance",'thunderslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo __("Fonts Loaded:",'thunderslider'); ?></span>				
				<span class="tp-monitor-jsicon"></span><span class="tp-monitor-total-subsize">%font_size%</span>				
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-fonts-details" data-open="</span><?php echo __("Hide Details",'thunderslider'); ?>" data-close="</span><?php echo __("Show Details",'thunderslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details",'thunderslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
		...
		if(!empty($my_fonts)){
			foreach($my_fonts as $c_font){
				$fcount = ThunderSliderBase::get_font_weight_count($c_font);
		...
		$overall = ThunderSliderOperations::get_performance($total_size/$slide_counter, 0,400000); // 400KB / Slide is ok
		$image = ThunderSliderOperations::get_performance($img_size/$img_counter, 0,100000); // 100KB Image OK
		$css = ThunderSliderOperations::get_performance($css_size, 0,150000); // 150KB CSS OK
		$js = ThunderSliderOperations::get_performance($js_size, 0,250000); // 250KB Image OK
		$font = ThunderSliderOperations::get_performance($all_font_count, 0,15); // 250KB Image OK
	...
	public static function get_preset_settings(){
		...
		//MandarinCMS default presets are added here directly
...
$presets[] = array (
	...
	//data.layer - Layer as jQuery Object
      ...
      'custom_javascript' => '',
		...
		$customer_presets = get_option('thunderslider_presets', array());
		
		
		$presets = array_merge($presets, $customer_presets);
		
		$presets = apply_filters('thunderslider_slider_presets', $presets);
		
		foreach($presets as $key => $preset){
			if(intval($preset['settings']['image']) > 0){
				$img = mcms_get_attachment_image_src(esc_attr($preset['settings']['image']), 'medium');
	...
	public static function add_preset_setting($data){
		...
		if(!isset($data['settings']) || !isset($data['values'])) return __('Missing values to add preset', 'thunderslider');
		...
		ThunderSliderFunctionsMCMS::update_option('thunderslider_presets', $customer_presets, 'off');
	...
	public static function remove_preset_setting($data){
		...
		if(!isset($data['name'])) return __('Missing values to remove preset', 'thunderslider');
	...
	public static function update_preset_setting($data){
		...
		if(!isset($data['name'])) return __('Missing values to update preset', 'thunderslider');
	...
	/**
	 * @since: 5.3.0
	 * create a page with thunderslider shortcodes included
	 **/
	public static function create_slider_page($added){
		
		$new_page_id = 0;
		
		if(!is_array($added)) return apply_filters('thunderslider_create_slider_page', $new_page_id, $added);
		...
		foreach($added as $sid){
			...
			if($alias !== ''){
				$content .= '[thunder_slider alias="'.$alias.'"][/thunder_slider]'; //this way we will reorder as last comes first
			}
		}
		
		if($content !== ''){
			$new_page_id = mcms_insert_post(
				array(
					'post_title'    => mcms_strip_all_tags( 'ThunderSlider Page '.$page_id ), //$title
					'post_content'  => $content,
					'post_type'   	=> 'page',
					'post_status'   => 'draft',
					'page_template' => '../public/views/thunderslider-page-template.php'
				)
			);
			
			if(is_mcms_error($new_page_id)) $new_page_id = 0; //fallback to 0
			
			$page_id++;
			update_option('rs_import_page_id', $page_id);
		}
		
		return apply_filters('thunderslider_create_slider_page', $new_page_id, $added);
	}
	 ...
	 * get cache modules
	 **/
	public function get_installed_cache_modules(){
		//get all modules
		$modules = get_modules();

		//arrays for found cache related modules
		$known_cache_modules = array();
		$unknown_cache_modules = array();

		//run through all modules
		foreach ($modules as $module_key => $module_values) {
			switch($module_key){
				//check if W3TC or MCMS Super Cache or MCMS Rocket
				//add to known modules
				case "mcms-rocket/mcms-rocket.php":
					$known_cache_modules[$module_values['Name']] = "https://www.jiiworks.net/faq/updating-make-sure-clear-caches/#mcmsrocket"; 
					break;
				case "mcms-super-cache/mcms-cache.php":
					$known_cache_modules[$module_values['Name']] = "https://www.jiiworks.net/faq/updating-make-sure-clear-caches/#mcmssc";
					break;
				case "w3-total-cache/w3-total-cache.php":
					$known_cache_modules[$module_values['Name']] = "https://www.jiiworks.net/faq/updating-make-sure-clear-caches/#w3tc";
					break;
				//check if cache in slug
				default:
					if(strpos($module_key,"cache")){
						//add to unknown modules
						$unknown_cache_modules[$module_values['Name']] = $module_values['ModuleURI'];
					}
					break; 
			}
		}
		
		$all = array_merge($known_cache_modules, $unknown_cache_modules);
		
		return apply_filters('thunderslider_get_installed_cache_modules', $all);
 ...
 * @since: 5.0
 **/
class RevOperations extends ThunderSliderOperations {}
?>