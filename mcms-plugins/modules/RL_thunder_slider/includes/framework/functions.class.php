...
class ThunderSliderFunctions{
	...
	public static function trimArrayItems($arr){
		if(gettype($arr) != "array")
			ThunderSliderFunctions::throwError("trimArrayItems error: The type must be array");
	...
	public static function get_responsive_size($slider){
		...
		if($adv_resp_sizes == true){
			$width = $slider->slider->getParam("width", 1240, ThunderSlider::FORCE_NUMERIC);
			$def = $width;
			$width .= ',';
			if($enable_custom_size_notebook == 'on'){
				$width.= $slider->slider->getParam("width_notebook", 1024, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("width_notebook", 1024, ThunderSlider::FORCE_NUMERIC);
			}else{
				$width.= $def;
			}
			$width.= ',';
			if($enable_custom_size_tablet == 'on'){
				$width.= $slider->slider->getParam("width_tablet", 778, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("width_tablet", 778, ThunderSlider::FORCE_NUMERIC);
			}else{
				$width.= $def;
			}
			$width.= ',';
			if($enable_custom_size_iphone == 'on'){
				$width.= $slider->slider->getParam("width_mobile", 480, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("width_mobile", 480, ThunderSlider::FORCE_NUMERIC);
			}else{
				$width.= $def;
			}
			
			$height = $slider->slider->getParam("height", 1240, ThunderSlider::FORCE_NUMERIC);
			$def = $height;
			$height .= ',';
			if($enable_custom_size_notebook == 'on'){
				$height.= $slider->slider->getParam("height_notebook", 1024, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("height_notebook", 1024, ThunderSlider::FORCE_NUMERIC);
			}else{
				$height.= $def;
			}
			$height.= ',';
			if($enable_custom_size_tablet == 'on'){
				$height.= $slider->slider->getParam("height_tablet", 778, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("height_tablet", 778, ThunderSlider::FORCE_NUMERIC);
			}else{
				$height.= $def;
			}
			$height.= ',';
			if($enable_custom_size_iphone == 'on'){
				$height.= $slider->slider->getParam("height_mobile", 480, ThunderSlider::FORCE_NUMERIC);
				$def = $slider->slider->getParam("height_mobile", 480, ThunderSlider::FORCE_NUMERIC);
			}else{
				$height.= $def;
			}
			...
			return array(
				...
				'height' => $slider->slider->getParam("height", "868", ThunderSlider::FORCE_NUMERIC),
				'width' => $slider->slider->getParam("width", "1240", ThunderSlider::FORCE_NUMERIC)
			);
		}
	}
 ...
 * @since: 5.0
 **/
class UniteFunctionsRev extends ThunderSliderFunctions {}
?>