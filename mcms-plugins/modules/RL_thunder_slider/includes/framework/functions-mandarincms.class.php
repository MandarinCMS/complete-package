...
class ThunderSliderFunctionsMCMS {
	...
	public static function isDBTableExists($tableName){
		...
		if(empty($tableName))
			ThunderSliderFunctions::throwError("Empty table name!!!");
		
		$sql = "show tables like '$tableName'";
		
		$table = $mcmsdb->get_var($sql);
	...
	/**
	 * 
	 * get mandarincms base path
	 */
	public static function getPathBase(){
		return BASED_TREE_URI;
	}
	
	/**
	 * 
	 * get mcms-content path
	 */
	public static function getPathUploads(){	
		...
		if(self::isMultisite()){
			if(!defined("BLOGUPLOADDIR")){
				...
				//$pathContent = $pathBase."mcms-content/uploads/";
				$pathContent = $pathBase."mcms-content/uploads/sites/{$mcmsdb->blogid}/";
			}else
			  $pathContent = BLOGUPLOADDIR;
		}else{
			$pathContent = MCMS_CONTENT_DIR;
	...
	public static function import_media($file_url, $folder_name) {
		...
		$ul_dir = mcms_upload_dir();
		$artDir = 'thunderslider/';
		...
		if(@fclose(@fopen($file_url, "r"))){ //make sure the file actually exists
			...
			if($atc_id == false || $atc_id == NULL){
				...
				//create an array of attachment data to insert into mcms_posts table
				$artdata = array(
					'post_author' => 1, 
				...
				$attach_id = mcms_insert_attachment($artdata, $artDir.$folder_name.$filename);
			}else{
				$attach_id = $atc_id;
			}

			//generate metadata and thumbnails
			if($attach_data = mcms_generate_attachment_metadata($attach_id, $saveDir)) mcms_update_attachment_metadata($attach_id, $attach_data);
	...
	public static function getImageRealPathFromUrl($urlImage){
		...
		$realPath = ThunderSliderFunctionsMCMS::getPathUploads().$filepath;
		return($realPath);
	}
	...
	public static function getPostsByIDs($strIDs, $slider_id, $is_gal, $additional = array()){
		...
		$query = apply_filters('thunderslider_get_posts', $query, $slider_id);
		
		$objQuery = new MCMS_Query($query);
	...
	public static function getPostsByCategory($slider_id,$catID,$sortBy = self::SORTBY_ID,$direction = self::ORDER_DIRECTION_DESC,$numPosts=-1,$postTypes="any",$taxonomies="category",$arrAddition = array()){
		...
		if(ThunderSliderWpml::isWpmlExists()){ //translate categories to languages
			$newcat = array();
			foreach($catID as $id){
				//$newcat[] = icl_object_id($id, 'category', true);
				$newcat[] = apply_filters( 'mcmsml_object_id', $id, 'category', true );
			}
			$catID = $newcat;
		}
	...
	public static function getPostCategories($postID,$arrTax){
		...
		$arrCats = mcms_get_post_terms( $postID, $arrTax);
		$arrCats = ThunderSliderFunctions::convertStdClassToArray($arrCats);
		return($arrCats);
	}
	...
	public static function getPost($postID){
		...
		if(empty($post))
			ThunderSliderFunctions::throwError("Post with id: $postID not found");
	...
	public static function getUrlPostImage($postID,$size = self::THUMB_FULL){
		...
		$arrImage = mcms_get_attachment_image_src($post_thumbnail_id,$size);
		if(empty($arrImage))
			return("");
	...
	public static function getAttachmentImage($thumbID,$size = self::THUMB_FULL){
		...
		$arrImage = mcms_get_attachment_image_src($thumbID,$size);
		if(empty($arrImage))
			return(false);
		
		$output = array();
		$output["url"] = ThunderSliderFunctions::getVal($arrImage, 0);
		$output["width"] = ThunderSliderFunctions::getVal($arrImage, 1);
		$output["height"] = ThunderSliderFunctions::getVal($arrImage, 2);
	...
	public static function getUrlAttachmentImage($thumbID,$size = self::THUMB_FULL){
		...
		$url = ThunderSliderFunctions::getVal($arrImage, 0);
		return($url);
	}
	...
	public static function deletePost($postID){
		$success = mcms_delete_post($postID,false);
		if($success == false)
			ThunderSliderFunctions::throwError("Could not delete post: $postID");
	}
	...
	public static function getIntroFromContent($text){
		...
		if(!empty($text)){
			...
			$intro = ThunderSliderFunctions::getVal($arrExtended, "main");
	...
	public static function getExcerptById($postID, $limit=55){
		...
		$excerpt = ThunderSliderFunctions::getTextIntro($excerpt, $limit);

		return apply_filters('thunderslider_getExcerptById', $excerpt, $post, $limit);
	}		
	...
	/**
	 * get categories list, copy the code from default mcms functions
	 */
	public static function getCategoriesHtmlList($catIDs,$strTax = null){
		...
		$arrErrors = ThunderSliderFunctions::getVal($categories, "errors");
		...
		if(!empty($arrErrors)){
			...
			ThunderSliderFunctions::throwError("getCategoriesHtmlList error: ".$strErrors);
		}
		...
		foreach ( $categories as $category ) {
			...
			if(!empty($link))
				$thelist .= '<a href="' . esc_url( $link ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", 'thunderslider'), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
			else
				$thelist .= $catName;
	...
	public static function getPathContent(){		
		if(self::isMultisite()){
			if(!defined("BLOGUPLOADDIR")){
				$pathBase = self::getPathBase();
				$pathContent = $pathBase."mcms-content/";
			}else
			  $pathContent = BLOGUPLOADDIR;
		}else{
			$pathContent = MCMS_CONTENT_DIR;
	...
	public static function getCatAndTaxData($catIDs){
		...
		foreach($catIDs as $cat){
			...
			$pos = strrpos($cat,"_");
			if($pos === false)
				ThunderSliderFunctions::throwError("The category is in wrong format");
...
ThunderSliderFunctionsMCMS::initStaticVars();

/**
 * old classname extends new one (old classnames will be obsolete soon)
 * @since: 5.0
 **/
class UniteFunctionsMCMSRev extends ThunderSliderFunctionsMCMS {}
?>