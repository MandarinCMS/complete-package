...
class ThunderSliderModuleUpdate {
	...
	public static function get_version(){
		$real_version = get_option('thunderslider_update_version', 1.0);
	...
	public static function set_version($set_to){
		...
		update_option('thunderslider_update_version', $set_to);
	...
	public static function add_v5_styles(){
		...
		foreach($v5 as $v5class){
			$result = $db->fetch(ThunderSliderGlobals::$table_css, $db->prepare("handle = %s", array($v5class['handle'])));
			if(empty($result)){
				//add v5 style
				$db->insert(ThunderSliderGlobals::$table_css, $v5class);
			}
		}
	...
	public static function update_css_styles(){
		...
		$css = new ThunderSliderCssParser();
		$db = new ThunderSliderDB();
		
		$styles = $db->fetch(ThunderSliderGlobals::$table_css);
		$default_classes = ThunderSliderCssParser::default_css_classes();
		...
		$cs = array_merge($cs, ThunderSliderCssParser::get_deformation_css_tags());
		...
		foreach($styles as $key => $attr){
			$ret = $db->update(ThunderSliderGlobals::$table_css, array('settings' => $styles[$key]['settings'], 'params' => $styles[$key]['params'], 'hover' => $styles[$key]['hover'], 'advanced' => $styles[$key]['advanced']), array('id' => $attr['id']));
		}
	...
	public static function check_settings_table(){
		...
		if($mcmsdb->get_var("SHOW TABLES LIKE '".ThunderSliderGlobals::$table_settings."'") == ThunderSliderGlobals::$table_settings) {
			$result = $mcmsdb->get_row("SELECT `general` FROM ".ThunderSliderGlobals::$table_settings, ARRAY_A);
			if(isset($result['general'])){
				update_option('thunderslider-global-settings', $result['general']);
			}
		}
				...
				if($template_id > 0){ //initialize slider to see if it exists. Then copy over the Template Sliders Slides to the Post Based Slider
					foreach($tempSliders as $t_slider){
						if($t_slider->getID() === $template_id){ //copy over the slides
							...
							if($static_id !== false){
								$record = $db->fetchSingle(ThunderSliderGlobals::$table_static_slides, $db->prepare("id = %s", array($static_id)));
								unset($record['id']);
								$record['slider_id'] = $slider_id;
								
								$db->insert(ThunderSliderGlobals::$table_static_slides, $record);
							}
	...
	public static function add_animation_settings_to_layer($sliders = false){
		...
		$inAnimations = ThunderSliderOperations::getArrAnimations(true);
		$outAnimations = ThunderSliderOperations::getArrEndAnimations(true);
					...
					foreach($slides as $slide){
						...
						if(!empty($layers) && is_array($layers)){
							foreach($layers as $lk => $layer){
								if(ThunderSliderFunctions::getVal($layer, 'x_start', false) === false){ //values are not set, set them now through
									$animation = ThunderSliderFunctions::getVal($layer, 'animation', 'tp-fade');
									$endanimation = ThunderSliderFunctions::getVal($layer, 'endanimation', 'tp-fade');
									...
									$layers[$lk]['x_start'] = ThunderSliderFunctions::getVal($anim_values, 'movex', 'inherit');
									$layers[$lk]['x_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'movex', 'inherit');
									$layers[$lk]['y_start'] = ThunderSliderFunctions::getVal($anim_values, 'movey', 'inherit');
									$layers[$lk]['y_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'movey', 'inherit');
									$layers[$lk]['z_start'] = ThunderSliderFunctions::getVal($anim_values, 'movez', 'inherit');
									$layers[$lk]['z_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'movez', 'inherit');
									
									$layers[$lk]['x_rotate_start'] = ThunderSliderFunctions::getVal($anim_values, 'rotationx', 'inherit');
									$layers[$lk]['x_rotate_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'rotationx', 'inherit');
									$layers[$lk]['y_rotate_start'] = ThunderSliderFunctions::getVal($anim_values, 'rotationy', 'inherit');
									$layers[$lk]['y_rotate_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'rotationy', 'inherit');
									$layers[$lk]['z_rotate_start'] = ThunderSliderFunctions::getVal($anim_values, 'rotationz', 'inherit');
									$layers[$lk]['z_rotate_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'rotationz', 'inherit');
									
									$layers[$lk]['scale_x_start'] = ThunderSliderFunctions::getVal($anim_values, 'scalex', 'inherit');
									if(intval($layers[$lk]['scale_x_start']) > 10) $layers[$lk]['scale_x_start'] /= 100;
									$layers[$lk]['scale_x_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'scalex', 'inherit');
									if(intval($layers[$lk]['scale_x_end']) > 10) $layers[$lk]['scale_x_end'] /= 100;
									$layers[$lk]['scale_y_start'] = ThunderSliderFunctions::getVal($anim_values, 'scaley', 'inherit');
									if(intval($layers[$lk]['scale_y_start']) > 10) $layers[$lk]['scale_y_start'] /= 100;
									$layers[$lk]['scale_y_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'scaley', 'inherit');
									if(intval($layers[$lk]['scale_y_end']) > 10) $layers[$lk]['scale_y_end'] /= 100;
									
									$layers[$lk]['skew_x_start'] = ThunderSliderFunctions::getVal($anim_values, 'skewx', 'inherit');
									$layers[$lk]['skew_x_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'skewx', 'inherit');
									$layers[$lk]['skew_y_start'] = ThunderSliderFunctions::getVal($anim_values, 'skewy', 'inherit');
									$layers[$lk]['skew_y_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'skewy', 'inherit');
									
									$layers[$lk]['opacity_start'] = ThunderSliderFunctions::getVal($anim_values, 'captionopacity', 'inherit');
									$layers[$lk]['opacity_end'] = ThunderSliderFunctions::getVal($anim_endvalues, 'captionopacity', 'inherit');
							...
							foreach($layers as $lk => $layer){
								...
								if($link_slide != false && $link_slide !== 'nothing'){ //link to slide/scrollunder is set, move it to actions
									...
									switch($link_slide){
										...
										break;
										case 'scroll_under':
											$scrollunder_offset = ThunderSliderFunctions::getVal($layer, 'scrollunder_offset');
			...
			foreach($sliders as $slider){
				...
				if(!empty($slides) && is_array($slides)){
					foreach($slides as $slide){
						...
						if(!empty($layers) && is_array($layers)){
							foreach($layers as $lk => $layer){
								$static_styles = (array) ThunderSliderFunctions::getVal($layer, 'static_styles', array());
								$def_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());
								$defh_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation-hover', array());
								
								if(empty($def_val)){
									
									//add parallax always!
									$def_val['parallax'] = ThunderSliderFunctions::getVal($layer, 'parallax_level', '-');
									$layers[$lk]['deformation'] = $def_val;
									
									//check for selected style in styles, then add all deformations to the layer
									$cur_style = ThunderSliderFunctions::getVal($layer, 'style', '');
									...
									if(isset($wws['params']['background-color'])){
										if(ThunderSliderFunctions::isrgb($wws['params']['background-color'])){
											$def_val['background-color'] = ThunderSliderFunctions::rgba2hex($wws['params']['background-color']);
										}else{
									...
									if(isset($wws['params']['background-transparency'])){
										...
										if(isset($wws['params']['background-color'])) $def_val['background-transparency'] = ThunderSliderFunctions::get_trans_from_rgba($wws['params']['background-color'], true);
									}
									
									if(isset($wws['params']['border-color'])){
										if(ThunderSliderFunctions::isrgb($wws['params']['border-color'])){
											$def_val['border-color'] = ThunderSliderFunctions::rgba2hex($wws['params']['border-color']);
										}else{
									...
									if(isset($wws['params']['color'])){
										if(ThunderSliderFunctions::isrgb($wws['params']['color'])){
											$static_styles['color'] = ThunderSliderFunctions::rgba2hex($wws['params']['color']);
										}else{
									...
									if(isset($wws['hover']['color'])){
										if(ThunderSliderFunctions::isrgb($wws['hover']['color'])){
											$defh_val['color'] = ThunderSliderFunctions::rgba2hex($wws['hover']['color']);
										}else{
									...
									if(isset($wws['hover']['background-color'])){
										if(ThunderSliderFunctions::isrgb($wws['hover']['background-color'])){
											$defh_val['background-color'] = ThunderSliderFunctions::rgba2hex($wws['hover']['background-color']);
										}else{
									...
									if(isset($wws['hover']['background-transparency'])){
										...
										if(isset($wws['hover']['background-color'])) $defh_val['background-transparency'] = ThunderSliderFunctions::get_trans_from_rgba($wws['hover']['background-color'], true);
									}
									if(isset($wws['hover']['border-color'])){
										if(ThunderSliderFunctions::isrgb($wws['hover']['border-color'])){
											$defh_val['border-color'] = ThunderSliderFunctions::rgba2hex($wws['hover']['border-color']);
										}else{
	...
	public static function change_layers_svg_5_2_5_5($sliders = false){
		...
		$path = $upload_dir['baseurl'].'/thunderslider/assets/svg/';
 ...
 * @since: 5.0
 **/
class UniteModuleUpdateRev extends ThunderSliderModuleUpdate {}
?>