...
class ThunderSliderCssParser{
					...
					foreach($styles as $name => $style){
						...
						if(!is_array($name) && isset($transparency[$name])){ //the style can have transparency!
							if(isset($styles[$transparency[$name]]) && $style !== 'transparent'){
								$style = ThunderSliderFunctions::hex2rgba($style, $styles[$transparency[$name]] * 100);
							}
						}
						if(!is_array($name) && isset($check_parameters[$name])){
							$style = ThunderSliderFunctions::add_missing_val($style, $check_parameters[$name]);
						}
						...
						foreach($hover as $name => $style){
							...
							if(!is_array($name) && isset($transparency[$name])){ //the style can have transparency!
								if(isset($hover[$transparency[$name]]) && $style !== 'transparent'){
									$style = ThunderSliderFunctions::hex2rgba($style, $hover[$transparency[$name]] * 100);
								}
							}
							if(!is_array($name) && isset($check_parameters[$name])){
								$style = ThunderSliderFunctions::add_missing_val($style, $check_parameters[$name]);
	...
	public static function parseStaticArrayToCss($cssArray, $nl = "\n"){
		$css = ThunderSliderCssParser::parseSimpleArrayToCss();
		return $css;
	}
	...
	public static function default_css_classes(){
		...
		$default = apply_filters('thunderslider_mod_default_css_handles', $default);
	...
	public static function get_captions_sorted(){
		...
		$styles = $db->fetch(ThunderSliderGlobals::$table_css, '', 'handle ASC');
 ...
 * @since: 5.0
 **/
class UniteCssParserRev extends ThunderSliderCssParser {}
?>