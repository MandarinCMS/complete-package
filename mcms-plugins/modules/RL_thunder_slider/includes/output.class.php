...
class ThunderSliderOutput {
	...
	public $icon_sets = array();//ThunderSliderBase::set_icon_sets(array());
	public $ignore_styles = array('font-family', 'color', 'font-weight', 'font-style', 'text-decoration');
	public $customAnimations = array();//ThunderSliderOperations::getCustomAnimations('customin'); //get all custom animations
	public $customEndAnimations = array();//ThunderSliderOperations::getCustomAnimations('customout'); //get all custom animations
	public $startAnimations = array();//ThunderSliderOperations::getArrAnimations(false); //only get the standard animations
	public $endAnimations = array();//ThunderSliderOperations::getArrEndAnimations(false); //only get the standard animations
	public $slideID = 0;
	public $slide;
	public $layers;
	...
	public static function putSlider($sliderID,$putIn="", $gal_ids = array(), $settings = array(), $order = array()){
		$settings = apply_filters('thunderslider_modify_slider_settings', $settings, $sliderID);
	 ...
	 */
	private function getSlideFullWidthVideoData(ThunderSliderSlide $slide){
		...
		$response['autoplay'] = ThunderSliderFunctions::strToBool($videoAutoplay);
		$response['cover'] = ThunderSliderFunctions::strToBool($videoCover);
		$response['autoplayonlyfirsttime'] = ThunderSliderFunctions::strToBool($videoAutoplayOnlyFirstTime);
		$response['previewimage'] = ThunderSliderFunctions::strToBool($previewimage);
		$response['nextslide'] = ($videoNextslide == 'on') ? true : false;
		$response['mute'] = ThunderSliderFunctions::strToBool($mute);
	...
	private function filterOneSlide($slides){
		...
		if(strpos($oneSlideID, 'static_') !== false){
			...
			$tmp_slides = $mcmsdb->get_results($mcmsdb->prepare("SELECT * FROM ".ThunderSliderGlobals::$table_static_slides." WHERE slider_id = %s", array($sliderID)), ARRAY_A);
			
			if(!empty($tmp_slides)){
				$n_slides = new ThunderSliderSlide();
		...
		$oneSlideParams = ThunderSliderFunctions::getVal($this->oneSlideData, 'params');
		$oneSlideLayers = ThunderSliderFunctions::getVal($this->oneSlideData, 'layers');
		...
		if(!empty($oneSlideLayers))
			$oneSlideLayers = ThunderSliderFunctions::convertStdClassToArray($oneSlideLayers);
	...
	private function putSlides($gal_ids = array(), $order = array()){
		...
		if($this->previewMode == true && $this->oneSlideMode == true){
			$previewSlideID = ThunderSliderFunctions::getVal($this->oneSlideData, 'slideid');
		...
		$mobile = (strstr($usragent,'Android') || strstr($usragent,'webOS') || strstr($usragent,'iPhone') ||strstr($usragent,'iPod') || strstr($usragent,'iPad') || strstr($usragent,'Windows Phone') || mcms_is_mobile()) ? true : false;
		...
		if(empty($slides)){
			...
			<div class="no-slides-text">
				...
				if($this->slider->isSlidesFromPosts()){
					_e('No slides found, please add at least one Slide Template to the choosen language.', 'thunderslider');
				}else{
					_e('No slides found, please add some slides', 'thunderslider');
				}
				?>
			</div>
			<?php
		...
		$do_static = apply_filters('thunderslider_enable_static_layers', true);
		...
		foreach($slides as $slide){
			...
			if($isExternal != 'external'){
				...
				switch($alt_type){
					case 'media_library':
						$alt = get_post_meta( $img_id, '_mcms_attachment_image_alt', true );
					break;
					case 'file_name':
				...
				if($img_id !== false){
					$img_data = mcms_get_attachment_metadata( $img_id );
			...
			if($enableLink == 'true'){
				...
				switch($linkType){
					...
					//---- link to slide
					case 'slide':
						$slideLink = ThunderSliderFunctions::getVal($params, 'slide_link');
						if(!empty($slideLink) && $slideLink != 'nothing'){
							//get slide index from id
							if(is_numeric($slideLink))
								$slideLink = ThunderSliderFunctions::getVal($this->slidesNumIndex, $slideLink);
				...
				//set link position:
				$linkPos = ThunderSliderFunctions::getVal($params, 'link_pos', 'front');
				if($linkPos == 'back')
			...
			for($mi=1;$mi<=10;$mi++){
				...
				if($pa !== ''){
					$pa_limit = $slide->getParam('params_'.$mi.'_chars',10,ThunderSlider::FORCE_NUMERIC);
					$pa = strip_tags($pa);
			...
			do_action('thunderslider_add_li_data', $this->slider, $slide);
			...
			$this->icon_sets = ThunderSliderBase::set_icon_sets(array());
			$this->customAnimations = ThunderSliderOperations::getCustomAnimations('customin'); //get all custom animations
			$this->customEndAnimations = ThunderSliderOperations::getCustomAnimations('customout'); //get all custom animations
			$this->startAnimations = ThunderSliderOperations::getArrAnimations(false); //only get the standard animations
			$this->endAnimations = ThunderSliderOperations::getArrEndAnimations(false); //only get the standard animations
			...
			do_action('thunderslider_add_layer_html', $this->slider, $slide);
		...
		if($do_static && !empty($this->static_slide)){
			...
			if(!empty($layers)){
				...
				do_action('thunderslider_add_static_layer_html', $this->slider);
	...
	public function putBackgroundVideo($slide){
		...
		if($this->adv_resp_sizes == true){
			if(is_object($videoWidth)){
				$videoWidth = ThunderSliderFunctions::normalize_device_settings($videoWidth, $this->enabled_sizes, 'html-array');
			}
			if(is_object($videoHeight)){
				$videoHeight = ThunderSliderFunctions::normalize_device_settings($videoHeight, $this->enabled_sizes, 'html-array');
			}
		}else{
			if(is_object($videoWidth)) $videoWidth = ThunderSliderFunctions::get_biggest_device_setting($videoWidth, $this->enabled_sizes);
			if(is_object($videoHeight)) $videoHeight = ThunderSliderFunctions::get_biggest_device_setting($videoHeight, $this->enabled_sizes);
		}
		...
		switch($videoType){
			...
			case 'twitter':
				...
				if($youtube_id !== ''){
					$arguments = $slide->getParam('video_arguments', ThunderSliderGlobals::DEFAULT_YOUTUBE_ARGUMENTS);
					$speed = $slide->getParam('video_speed', '1');
				
					if(empty($arguments))
						$arguments = ThunderSliderGlobals::DEFAULT_YOUTUBE_ARGUMENTS;
					...
					$arguments = $slide->getParam('video_arguments_vim', ThunderSliderGlobals::DEFAULT_VIMEO_ARGUMENTS);
				
					if(empty($arguments))
						$arguments = ThunderSliderGlobals::DEFAULT_VIMEO_ARGUMENTS;
	...
	private function putCreativeLayer($static_slide = false){
		...
		$layers = apply_filters('thunderslider_putCreativeLayer', $this->layers, $this, $static_slide);
	...
	public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false){
		$layer = apply_filters('thunderslider_putLayer_pre', $layer, $this, $row_group_uid, $static_slide, $special_type);
		if(!isset($layer['deformation'])) $layer['deformation'] = array();
		if(!isset($layer['deformation-hover'])) $layer['deformation-hover'] = array();
		
		$unique_id = ThunderSliderFunctions::getVal($layer, 'unique_id');
		...
		//$visible = ThunderSliderFunctions::getVal($layer, 'visible', true);
		//if($visible == false) continue;
		
		$type = ThunderSliderFunctions::getVal($layer, 'type', 'text');
		$svg_val = '';
		...
		$ids = ThunderSliderFunctions::getVal($layer, 'attrID');
		$wrapper_ids = ThunderSliderFunctions::getVal($layer, 'attrWrapperID');
		$classes = ThunderSliderFunctions::getVal($layer, 'attrClasses');
		$wrapper_classes = ThunderSliderFunctions::getVal($layer, 'attrWrapperClasses');
		$title = ThunderSliderFunctions::getVal($layer, 'attrTitle');
		$tabindex = ThunderSliderFunctions::getVal($layer, 'attrTabindex');
		$rel = ThunderSliderFunctions::getVal($layer, 'attrRel');
		$frames = ThunderSliderFunctions::getVal($layer, 'frames', false);
		$frame_start = ThunderSliderFunctions::getVal($frames, 'frame_0', false);
		$frame_end = ThunderSliderFunctions::getVal($frames, 'frame_999', false);
		...
		//$frame_start['to']
		$blurfilter = ThunderSliderFunctions::getVal($layer['deformation'], 'blurfilter', 0);
		$grayscalefilter = ThunderSliderFunctions::getVal($layer['deformation'], 'grayscalefilter', 0);
		$brightnessfilter = ThunderSliderFunctions::getVal($layer['deformation'], 'brightnessfilter', 100);
		...
		if(isset($layer['deformation-hover'])){
			$hover_blurfilter = ThunderSliderFunctions::getVal($layer['deformation-hover'], 'blurfilter', 0);
			$hover_grayscalefilter = ThunderSliderFunctions::getVal($layer['deformation-hover'], 'grayscalefilter', 0);
			$hover_brightnessfilter = ThunderSliderFunctions::getVal($layer['deformation-hover'], 'brightnessfilter', 100);
		}
		
		//$frame_start['from']
		$anim_blurfilter_start = ThunderSliderFunctions::getVal($layer, 'blurfilter_start', 0);
		$anim_grayscalefilter_start = ThunderSliderFunctions::getVal($layer, 'grayscalefilter_start', 0);
		$anim_brightnessfilter_start = ThunderSliderFunctions::getVal($layer, 'brightnessfilter_start', 100);

		//$frame_end
		$anim_blurfilter_end = ThunderSliderFunctions::getVal($layer, 'blurfilter_end', 0);
		$anim_grayscalefilter_end = ThunderSliderFunctions::getVal($layer, 'grayscalefilter_end', 0);
		$anim_brightnessfilter_end = ThunderSliderFunctions::getVal($layer, 'brightnessfilter_end', 100);
		...
		switch($type){
			case 'video':
				...
				$videoData = ThunderSliderFunctions::getVal($layer, 'video_data');
				if(!empty($videoData)){
					$videoData = (array)$videoData;
					$isFullWidthVideo = ThunderSliderFunctions::getVal($videoData, 'fullwidth');
					$isFullWidthVideo = ThunderSliderFunctions::strToBool($isFullWidthVideo);
				}else
					$videoData = array();
			break;
			case 'audio':
			...
			case 'group':
				...
				$classes .= ' rev_group';
				$group_overflow = ThunderSliderFunctions::getVal($layer['deformation'], 'overflow'); 
		...
		$class = ThunderSliderFunctions::getVal($layer, 'style');
		
		if(trim($class) !== ''){
			$this->class_include['.'.trim($class)] = true; //add classname for style inclusion
			//get class styles for further compare usage
			
			if(!isset($this->in_class_usage[trim($class)])) $this->in_class_usage[trim($class)] = ThunderSliderOperations::getCaptionsContentArray(trim($class));
		}
		...
		**/
		$do_remove_inline = apply_filters('thunderslider_remove_inline', true);
		...
		$animation = ThunderSliderFunctions::getVal($frame_start, 'animation', ThunderSliderFunctions::getVal($layer, 'animation', 'tp-fade'));
		...
		$tcin = ThunderSliderOperations::parseCustomAnimationByArray($layer, 'start', $frame_start);
		...
		$do_mask_in = ThunderSliderFunctions::getVal($layer, 'mask_start',false);
		if($do_mask_in){
			$tmask = ThunderSliderOperations::parseCustomMaskByArray($layer, 'start');
			if($tmask !== ''){
		...
		$layer_2d_rotation = intval(ThunderSliderFunctions::getVal($layer, '2d_rotation', '0'));
		
		$internal_class = ThunderSliderFunctions::getVal($layer, 'internal_class', '');

		$layer_selectable = ThunderSliderFunctions::getVal($layer, 'layer-selectable', 'default');
		
		$outputClass = 'tp-caption '. trim($class);
		
		$d_hovers = (array) ThunderSliderFunctions::getVal($layer, 'deformation-hover', array());
		$pointerevents = ThunderSliderFunctions::getVal($d_hovers, 'pointer_events', 'auto');
		...
		//$speed = ThunderSliderFunctions::getVal($frame_start, 'speed',ThunderSliderFunctions::getVal($layer, 'speed', 300));
		$time = ThunderSliderFunctions::getVal($frame_start, 'time', ThunderSliderFunctions::getVal($layer, 'time', 0));
		$time_relative = ThunderSliderFunctions::getVal($frame_start, 'time_relative', ThunderSliderFunctions::getVal($layer, 'timedelay', 'none'));
		
		if($time_relative !== 'none'){
			$time = ($time_relative !== $time) ? '+'.$time_relative : $time_relative;
		}
		
		$frame_start['speed'] = ThunderSliderFunctions::getVal($frame_start, 'speed', ThunderSliderFunctions::getVal($layer, 'speed', '300')); //fallback
		$frame_start['easing'] = ThunderSliderFunctions::getVal($frame_start, 'easing', ThunderSliderFunctions::getVal($layer, 'easing', 'easeOutExpo')); //fallback
				
		if (ThunderSliderFunctions::getVal($frame_start, 'use_text_c')==1) 
			$frame_start['color'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_start, 'text_c', 'transparent'));

		if (ThunderSliderFunctions::getVal($frame_start, 'use_bg_c')==1) 
			$frame_start['bgcolor'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_start, 'bg_c', 'transparent'));
		...
		$frame_start['sfx_effect'] = ThunderSliderFunctions::getVal($frame_start, 'sfx_effect', ThunderSliderFunctions::getVal($layer, 'sfx_effect', '')); //fallback
		$frame_start['sfxcolor'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_start, 'sfxcolor', ThunderSliderFunctions::getVal($layer, 'sfxcolor', '#ffffff'))); //fallback
		
		$randomRotate = ThunderSliderFunctions::getVal($layer, 'random_rotation', 'false');
		$randomRotate = ThunderSliderFunctions::boolToStr($randomRotate);

		$splitin = ThunderSliderFunctions::getVal($frame_start, 'split', ThunderSliderFunctions::getVal($layer, 'split', 'none'));
		$splitout = ThunderSliderFunctions::getVal($frame_end, 'split', ThunderSliderFunctions::getVal($layer, 'endsplit', 'none'));
		$splitindir = ThunderSliderFunctions::getVal($frame_start, 'split_direction', ThunderSliderFunctions::getVal($layer, 'split_direction', 'forward'));
		$splitoutdir = ThunderSliderFunctions::getVal($frame_end, 'split_direction', ThunderSliderFunctions::getVal($layer, 'endsplit_direction', 'forward'));
		
		$elementdelay = intval(ThunderSliderFunctions::getVal($frame_start, 'splitdelay', ThunderSliderFunctions::getVal($layer, 'splitdelay', 0)));		
		$endelementdelay = intval(ThunderSliderFunctions::getVal($frame_end, 'splitdelay', ThunderSliderFunctions::getVal($layer, 'endsplitdelay', 0)));
		

		$basealign = ThunderSliderFunctions::getVal($layer, 'basealign', 'grid');
		...
		$text = ThunderSliderFunctions::getVal($layer, 'text');
		...
		$text_toggle = ThunderSliderFunctions::getVal($layer, 'texttoggle');
		if(function_exists('qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage')){ //use qTranslate
		...
		$videoType = '';
		$cover = false;
		$toggle_allow = ThunderSliderFunctions::getVal($layer, 'toggle',false);
		$toggle_inverse_content = ThunderSliderFunctions::getVal($layer, 'toggle_inverse_content', false);
		...
		switch($type){
			...
			case 'group':
			case 'column':
				$urlImage = ThunderSliderFunctions::getVal($layer, 'bgimage_url', '');
				//add background image 
				if($urlImage !== ''){
					
					$imglibsize = (array)ThunderSliderFunctions::getVal($layer, 'image_librarysize');
					...
					$layer_bg_size = ThunderSliderFunctions::getVal($layer, 'layer_bg_size', 'cover');
					$layer_bg_position = ThunderSliderFunctions::getVal($layer, 'layer_bg_position', 'center center');
					$layer_bg_repeat = ThunderSliderFunctions::getVal($layer, 'layer_bg_repeat', 'no-repeat');
					
					$def_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());
					$bg_trans = ThunderSliderFunctions::getVal($def_val, 'background-transparency', '1');
		...
		switch($type){
			...
			break;
			case 'svg':
				$max_width = ThunderSliderFunctions::getVal($layer, 'max_width', 'auto');
				$max_height = ThunderSliderFunctions::getVal($layer, 'max_height', 'auto');										
				$max_width = (is_object($max_width)) ? ThunderSliderFunctions::get_biggest_device_setting($max_width, $this->enabled_sizes) : $max_width;
				$max_height = (is_object($max_height)) ? ThunderSliderFunctions::get_biggest_device_setting($max_height, $this->enabled_sizes) : $max_height;
				
				if($max_width !== 'auto'){
					$max_width = ($max_width !== 'none') ? ThunderSliderFunctions::add_missing_val($max_width) : $max_width;
					$inline_styles .= ' min-width: '.$max_width.'; max-width: '.$max_width.';';
				}
				if($max_height !== 'auto'){
					$max_height = ($max_height !== 'none') ? ThunderSliderFunctions::add_missing_val($max_height) : $max_height;
					$inline_styles .= ' max-width: '.$max_height.'; max-width: '.$max_height.';';
				}

				$svg_val = ThunderSliderFunctions::getVal($layer, 'svg', false);

				$static_styles = ThunderSliderFunctions::getVal($layer, 'static_styles', array());
				...
				if(!empty($static_styles)){
					...
					if(!empty($static_styles['color'])){
						if(is_object($static_styles['color'])){
							$use_color = ThunderSliderFunctions::get_biggest_device_setting($static_styles['color'], $this->enabled_sizes);
						}else{
							$use_color = $static_styles['color'];
						}
						
						$def_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());						
						$color_trans = ThunderSliderFunctions::getVal($def_val, 'color-transparency', false);
			...
			case 'button':
				...
				foreach($this->icon_sets as $is){
					if(strpos($html, $is) !== false){ //include default Icon Sets if used
						...
						$$font_var = true;
						//mcms_enqueue_style('rs-icon-set-'.$is);
					}
				}
				
				$max_width = ThunderSliderFunctions::getVal($layer, 'max_width', 'auto');
				$max_height = ThunderSliderFunctions::getVal($layer, 'max_height', 'auto');
				$white_space = ThunderSliderFunctions::getVal($layer, 'whitespace', 'nowrap');
				
				$max_width = (is_object($max_width)) ? ThunderSliderFunctions::get_biggest_device_setting($max_width, $this->enabled_sizes) : $max_width;
				$max_height = (is_object($max_height)) ? ThunderSliderFunctions::get_biggest_device_setting($max_height, $this->enabled_sizes) : $max_height;
				$white_space = (is_object($white_space)) ? ThunderSliderFunctions::get_biggest_device_setting($white_space, $this->enabled_sizes) : $white_space;
				...
				if(!empty($static_styles)){
					...
					if(!empty($static_styles['font-size'])){
						$static_styles['font-size'] = ThunderSliderFunctions::add_missing_val($static_styles['font-size'], 'px');
						if(is_object($static_styles['font-size'])){
							$mcfs = ThunderSliderFunctions::get_biggest_device_setting($static_styles['font-size'], $this->enabled_sizes);
						}else{
					...
					if(!empty($static_styles['line-height'])){
						$static_styles['line-height'] = ThunderSliderFunctions::add_missing_val($static_styles['line-height'], 'px');
						if(is_object($static_styles['line-height'])){
							$mclh = ThunderSliderFunctions::get_biggest_device_setting($static_styles['line-height'], $this->enabled_sizes);
						}else{
					...
					if(!empty($static_styles['font-weight'])){
						if(is_object($static_styles['font-weight'])){
							$mcfw = ThunderSliderFunctions::get_biggest_device_setting($static_styles['font-weight'], $this->enabled_sizes);
						}else{
					...
					if(!empty($static_styles['letter-spacing'])){
						$static_styles['letter-spacing'] = ThunderSliderFunctions::add_missing_val($static_styles['letter-spacing'], 'px');
						if(is_object($static_styles['letter-spacing'])){
							$mcls = ThunderSliderFunctions::get_biggest_device_setting($static_styles['letter-spacing'], $this->enabled_sizes);
						}else{
			...
			case 'image':
				...
				$cover_mode = ThunderSliderFunctions::getVal($layer, 'cover_mode', array());
				...
				$scaleX = ThunderSliderFunctions::getVal($layer, 'scaleX');
				$scaleY = ThunderSliderFunctions::getVal($layer, 'scaleY');
				...
				if($this->adv_resp_sizes == true){
					...
					if($scaleX !== ''){
						if(!is_object($scaleX)){
							...
							$myScaleX = ThunderSliderFunctions::normalize_device_settings($scaleX, $this->enabled_sizes, 'html-array', array('NaNpx' => '', 'auto' => ''));
							if($myScaleX == "['','','','']") $myScaleX = '';
							
							if(strpos($myScaleX, '[') !== false) $y_is_single = false;
						}
						if($y_is_single){ //force to array if voffset is also array
							if(!isset($myScaleX)) $myScaleX = ThunderSliderFunctions::get_biggest_device_setting($scaleX, $this->enabled_sizes);
					...
					if($scaleY !== ''){
						if(!is_object($scaleY)){
							...
							$myScaleY = ThunderSliderFunctions::normalize_device_settings($scaleY, $this->enabled_sizes, 'html-array', array('NaNpx' => '', 'auto' => ''));
							if($myScaleY == "['','','','']") $myScaleY = '';
							
							if(strpos($myScaleY, '[') !== false) $y_is_single = false;
						}
						if($y_is_single){ //force to array if voffset is also array
							if(!isset($myScaleY)) $myScaleY = ThunderSliderFunctions::get_biggest_device_setting($scaleY, $this->enabled_sizes);
					...
					$myScaleX = (is_object($scaleX)) ? ThunderSliderFunctions::get_biggest_device_setting($scaleX, $this->enabled_sizes) : $scaleX;
					if(trim($myScaleX) == '' || $myScaleX == 'NaNpx') $myScaleX = 'auto';
					
					$myScaleY = (is_object($scaleY)) ? ThunderSliderFunctions::get_biggest_device_setting($scaleY, $this->enabled_sizes) : $scaleY;
					if(trim($myScaleY) == '' || $myScaleY == 'NaNpx') $myScaleY = 'auto';
				...
				$urlImage = ThunderSliderFunctions::getVal($layer, 'image_url');
				
				$do_image_change = ThunderSliderFunctions::getVal($layer, 'image-size', 'auto');
				...
				$cur_img_id = ThunderSliderFunctionsMCMS::get_image_id_by_url($urlImage);
				
				if($img_size !== 'full'){
					if($cur_img_id !== false && !empty($cur_img_id)){
						$urlImage = mcms_get_attachment_image_src($cur_img_id, $img_size);
						$urlImage = $urlImage[0];
					}
				}
				...
				$img_h = '';
				if($cur_img_id !== false && !empty($cur_img_id)){
					$img_data = mcms_get_attachment_metadata( $cur_img_id );
					if($img_data !== false && !empty($img_data)){
						if($img_size !== 'full'){
							if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
					...
					$msx = ThunderSliderFunctions::get_biggest_device_setting($scaleX, $this->enabled_sizes);
					$msy = ThunderSliderFunctions::get_biggest_device_setting($scaleY, $this->enabled_sizes);
					$imglibsize = (array)ThunderSliderFunctions::getVal($layer, 'image_librarysize');
				...
				$alt = '';
				$alt_option = ThunderSliderFunctions::getVal($layer, 'alt_option', 'media_library');
				
				switch($alt_option){
					case 'media_library':
						if($cur_img_id !== false){
							$alt = get_post_meta( $cur_img_id, '_mcms_attachment_image_alt', true );
						}
					break;
					case 'file_name':
					...
					break;
					case 'custom':
						$alt = ThunderSliderFunctions::getVal($layer, 'alt');
					break;
				}
				...
				$do_ll = ThunderSliderFunctions::getVal($layer, "lazy-load", 'auto');
				...
				if($lazyLoad != 'none' || $do_ll == 'force' && $do_ll !== 'ignore'){
					$seo_opti = ThunderSliderFunctions::getVal($layer, 'seo-optimized', false);
				...
				$imageLink = ThunderSliderFunctions::getVal($layer, "link","");
			...
			break;
			case 'audio':
				$videoType = trim(ThunderSliderFunctions::getVal($layer, 'video_type'));
				$videoWidth = ThunderSliderFunctions::getVal($layer, 'video_width');
				$videoHeight = ThunderSliderFunctions::getVal($layer, 'video_height');
				$videoArgs = trim(ThunderSliderFunctions::getVal($videoData, 'args'));
				$v_controls = ThunderSliderFunctions::getVal($videoData, 'controls');
				$v_controls = ThunderSliderFunctions::strToBool($v_controls);
				$v_visible = ThunderSliderFunctions::getVal($videoData, 'video_show_visibility');
				$v_visible = ThunderSliderFunctions::strToBool($v_visible);
				$videopreload = ThunderSliderFunctions::getVal($videoData, "preload_audio");
				$videopreloadwait = ThunderSliderFunctions::getVal($videoData, "preload_wait");
				
				$start_at = ThunderSliderFunctions::getVal($videoData, 'start_at');
				$htmlStartAt = ($start_at !== '') ? ' data-videostartat="'.$start_at.'"' : '';
				$end_at = ThunderSliderFunctions::getVal($videoData, 'end_at');
				$htmlEndAt = ($end_at !== '') ? ' data-videoendat="'.$end_at.'"' : '';
				
				$rewind = ThunderSliderFunctions::getVal($videoData, 'forcerewind');
				$rewind = ThunderSliderFunctions::strToBool($rewind);
				$htmlRewind = ($rewind == true) ? ' data-forcerewind="on"' : '';
				
				$volume = ThunderSliderFunctions::getVal($videoData, "volume", '100');
				...
				$videoloop = ThunderSliderFunctions::getVal($videoData, "videoloop");
				...
				$urlAudio = esc_attr(ThunderSliderFunctions::getVal($videoData, 'urlAudio'));
				if(!empty($urlAudio)) $add_data .= ' data-videomp4="'.$urlAudio.'"';
				
				
				if(ThunderSliderFunctions::strToBool($videoloop) == true){ //fallback
					$add_data .= ' data-videoloop="loop"';
				}else{
				...
				if(array_key_exists("autoplayonlyfirsttime", $videoData)){
					$autoplayonlyfirsttime = ThunderSliderFunctions::strToBool(ThunderSliderFunctions::getVal($videoData, "autoplayonlyfirsttime"));
					if($autoplayonlyfirsttime == true) $videoAutoplay = '1sttime';
				}
				
				if($videoAutoplay == false){
					if(array_key_exists("autoplay", $videoData))
						$videoAutoplay = ThunderSliderFunctions::getVal($videoData, "autoplay");
					else	//backword compatability
						$videoAutoplay = ThunderSliderFunctions::getVal($layer, "video_autoplay");
				}
				...
				$videoNextSlide = ThunderSliderFunctions::getVal($videoData, "nextslide");
				$videoNextSlide = ThunderSliderFunctions::strToBool($videoNextSlide);

				if($videoNextSlide == true)
					$htmlVideoNextSlide = '			data-nextslideatend="true"'." \n";

				$stopallvideo = ThunderSliderFunctions::getVal($videoData, "stopallvideo");
				$stopallvideo = ThunderSliderFunctions::strToBool($stopallvideo);
				$htmlDisableOnMobile .= ($stopallvideo)	? '			data-stopallvideos="true"'." \n" : '';
				
			break;
			case 'video':
				$videoType = trim(ThunderSliderFunctions::getVal($layer, 'video_type'));
				$videoID = trim(ThunderSliderFunctions::getVal($layer, 'video_id'));
				$videoWidth = ThunderSliderFunctions::getVal($layer, 'video_width');
				$videoHeight = ThunderSliderFunctions::getVal($layer, 'video_height');
				$videoArgs = trim(ThunderSliderFunctions::getVal($layer, 'video_args'));
				$v_controls = ThunderSliderFunctions::getVal($videoData, 'controls');
				$v_controls = ThunderSliderFunctions::strToBool($v_controls);
				...
				$show_cover_pause = ThunderSliderFunctions::getVal($videoData, 'show_cover_pause');
				$show_cover_pause = ThunderSliderFunctions::strToBool($show_cover_pause);
				...
				$only_poster_on_mobile = ThunderSliderFunctions::strToBool($only_poster_on_mobile);
				...
				$cover = ThunderSliderFunctions::getVal($videoData, 'cover');
				$cover = ThunderSliderFunctions::strToBool($cover);
				
				$videoloop = ThunderSliderFunctions::getVal($videoData, "videoloop");
				
				$mute = ThunderSliderFunctions::getVal($videoData, "mute");
				$mute = ThunderSliderFunctions::strToBool($mute);
				...
				switch($videoType){
					...
					case 'youtube':
						...
						if(empty($videoArgs))
							$videoArgs = ThunderSliderGlobals::DEFAULT_YOUTUBE_ARGUMENTS;
						...
						$videospeed = ThunderSliderFunctions::getVal($videoData, 'videospeed', '1');
						$play_inline = ThunderSliderFunctions::getVal($videoData, 'video_play_inline', false);
						$play_inline = ThunderSliderFunctions::strToBool($play_inline);
					...
					case 'vimeo':
						...
						if(empty($videoArgs))
							$videoArgs = ThunderSliderGlobals::DEFAULT_VIMEO_ARGUMENTS;
					...
					case 'streaminstagramboth':
					case 'html5':
						$urlPoster = ThunderSliderFunctions::getVal($videoData, "urlPoster");
						$urlMp4 = ThunderSliderFunctions::getVal($videoData, "urlMp4");
						$urlWebm = ThunderSliderFunctions::getVal($videoData, "urlWebm");
						$urlOgv = ThunderSliderFunctions::getVal($videoData, "urlOgv");
						$videopreload = ThunderSliderFunctions::getVal($videoData, "preload");
						$leave_on_pause = ThunderSliderFunctions::getVal($videoData, "leave_on_pause");
						$leave_on_pause = ThunderSliderFunctions::strToBool($leave_on_pause);
						...
						$large_controls = ThunderSliderFunctions::strToBool(ThunderSliderFunctions::getVal($videoData, "large_controls"));
						if(!$large_controls){
					...
					break;
					default:
						ThunderSliderFunctions::throwError("wrong video type: $videoType");
					break;
				}
				...
				if($cover == true){
					$dotted = ThunderSliderFunctions::getVal($videoData, "dotted");
					if($dotted !== 'none')
						$add_data .= ' data-dottedoverlay="'.$dotted.'"';
						
					$add_data .=  ' data-forceCover="1"';
						
					$ratio = ThunderSliderFunctions::getVal($videoData, "ratio");
						if(!empty($ratio))
							$add_data .= ' data-aspectratio="'.$ratio.'"';
				}
				...
				$disable_on_mobile = ThunderSliderFunctions::getVal($videoData, "disable_on_mobile");
				$disable_on_mobile = ThunderSliderFunctions::strToBool($disable_on_mobile);
				$htmlDisableOnMobile = ($disable_on_mobile)	? '			data-disablevideoonmobile="1"'." \n" : '';
				$stopallvideo = ThunderSliderFunctions::getVal($videoData, "stopallvideo");
				$stopallvideo = ThunderSliderFunctions::strToBool($stopallvideo);
				$allowfullscreenvideo = ThunderSliderFunctions::getVal($videoData, "allowfullscreen");
				$allowfullscreenvideo = ThunderSliderFunctions::strToBool($allowfullscreenvideo);
				$htmlDisableOnMobile .= ($stopallvideo)	? '			data-stopallvideos="true"'." \n" : '';
				$htmlDisableOnMobile .= ($allowfullscreenvideo)	? '			data-allowfullscreenvideo="true"'." \n" : '';
			break;
			case 'column':
				$size_raw = explode('/', ThunderSliderFunctions::getVal($layer, 'column_size'));
				...
				$col_verticalalign = ThunderSliderFunctions::getVal($layer['deformation'], 'vertical-align','top');
		...
		if($type == 'row' || $type == 'column' || $row_group_uid !== false){
			$margins = ThunderSliderFunctions::getVal($layer, 'margin');
			$rl_margin = array();
			if(is_object($margins)){
				$margin_vals = ThunderSliderFunctions::normalize_device_settings($margins, $this->enabled_sizes, 'obj');
		...
		$text_align = ThunderSliderFunctions::getVal($layer, 'text-align');
		if(empty($text_align)){ //backwards compatability, get the info from deformation
			$text_align = (object)$text_align;
			$text_align->desktop = ThunderSliderFunctions::getVal($layer['deformation'], 'text-align'); 
		}
		
		if(is_object($text_align)){
			$text_align_vals = ThunderSliderFunctions::normalize_device_settings($text_align, $this->enabled_sizes, 'obj');
		...
		$paddings = ThunderSliderFunctions::getVal($layer, 'padding');
		if(empty($paddings)){ //backwards compatability, get the info from deformation
			$paddings = (object)$paddings;
			$paddings->desktop = ThunderSliderFunctions::getVal($layer['deformation'], 'padding'); 
		}
		
		$rl_padding = array();
		if(is_object($paddings)){
			$padding_vals = ThunderSliderFunctions::normalize_device_settings($paddings, $this->enabled_sizes, 'obj');
		...
		$blendmode = ThunderSliderFunctions::getVal($layer, "layer_blend_mode", 'normal');
		if ($blendmode!=="normal") {
		...
		switch($this->container_mode){
			case 'column': // we are a layer inside of a container, so add the display: block/inline-block
				$inline_styles.= (ThunderSliderFunctions::getVal($layer, 'displaymode')) ? ' display: block;' : ' display: inline-block;';
		...
		foreach($this->layers as $cl){
			...
			$all_actions = ThunderSliderFunctions::getVal($cl, 'layer_action', array());
			if(!empty($all_actions)){
				$a_action = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'action', array()));
				$a_layer_target = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'layer_target', array()));
		...
		if($has_trigger){
			$animation_overwrite = ThunderSliderFunctions::getVal($layer, 'animation_overwrite', 'wait');
			$trigger_memory = ThunderSliderFunctions::getVal($layer, 'trigger_memory', 'keep');
		...
		if($animation_overwrite == 'waitout' || $animation_overwrite == 'wait'){
			...
			$endTime = trim(ThunderSliderFunctions::getVal($frame_end, 'time', ThunderSliderFunctions::getVal($layer, 'endtime', 0)));
			$end_time_relative = ThunderSliderFunctions::getVal($frame_end, 'time_relative', ThunderSliderFunctions::getVal($layer, 'endtimedelay', 'none'));
			if($end_time_relative !== 'none'){
				$endTime = ($endTime !== $end_time_relative) ? '+'.$end_time_relative : $end_time_relative;
			}
			
			$endWithSlide = ThunderSliderFunctions::getVal($layer, 'endWithSlide', false);			
			$version = ThunderSliderFunctions::getVal($layer, 'version', false);
			...
			if($version === false || intval($version) < 530) {
				...
				if($realEndTime !== false){
					$endSpeed = trim(ThunderSliderFunctions::getVal($frame_end, 'speed', UniteFunctionsRev::getVal($layer, 'endspeed')));
					$speed = UniteFunctionsRev::getVal($frame_start, 'speed', ThunderSliderFunctions::getVal($layer, 'speed', 300));
				...
				//endtime - endspeed
				$es = ThunderSliderFunctions::getVal($frame_end, 'speed', ThunderSliderFunctions::getVal($layer, 'endspeed', 0));
				...
				//end - start + speed
				$speed = UniteFunctionsRev::getVal($frame_start, 'speed', ThunderSliderFunctions::getVal($layer, 'speed', 300));
				$time = UniteFunctionsRev::getVal($frame_start, 'time', ThunderSliderFunctions::getVal($layer, 'time', 0));
		...
		$show_on_hover = ThunderSliderFunctions::getVal($layer, 'show-on-hover', false);
		...
		$columnbreak = '';
		if ($type==="row") {
			$columnbreak = ThunderSliderFunctions::getVal($layer, 'column_break_at');			
		...
		$endAnimation = trim(ThunderSliderFunctions::getVal($frame_end, 'animation', ThunderSliderFunctions::getVal($layer, 'endanimation')));
		...
		if($endAnimation !== 'auto'){
			$tcout = ThunderSliderOperations::parseCustomAnimationByArray($layer, 'end', $frame_end);
		}else{ //automatic reverse
			$tcout .= 'auto:auto;';
		}
		
		$es = ThunderSliderFunctions::getVal($frame_end, 'speed', ThunderSliderFunctions::getVal($layer, 'endspeed'));
		$ee = trim(ThunderSliderFunctions::getVal($frame_end, 'easing', ThunderSliderFunctions::getVal($layer, 'endeasing')));
		
		if (ThunderSliderFunctions::getVal($frame_end, 'use_text_c')==1) 
			$frame_end['color'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_end, 'text_c', 'transparent'));

		if (ThunderSliderFunctions::getVal($frame_end, 'use_bg_c')==1) 
			$frame_end['bgcolor'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_end, 'bg_c', 'transparent'));
		...
		$frame_end['sfx_effect'] = ThunderSliderFunctions::getVal($frame_end, 'sfx_effect', ThunderSliderFunctions::getVal($layer, 'sfx_effect', '')); 
		$frame_end['sfxcolor'] = TPColorpicker::get(ThunderSliderFunctions::getVal($frame_end, 'sfxcolor', ThunderSliderFunctions::getVal($layer, 'sfxcolor', '#ffffff'))); //fallback
		...
		$do_mask_out = ThunderSliderFunctions::getVal($layer, 'mask_end',false);
		if($do_mask_out){
			$tmask = ThunderSliderOperations::parseCustomMaskByArray($layer, 'end');
			if($tmask !== ''){
		...
		$htmlHidden = '';
		$layerHidden = ThunderSliderFunctions::getVal($layer, 'hiddenunder');
		if($layerHidden == 'true' || $layerHidden == '1')
			$htmlHidden = '			data-captionhidden="on"'." \n";
		
		$toggle_use_hover = ThunderSliderFunctions::getVal($layer, 'toggle_use_hover', false);
		...
		$alignHor = ThunderSliderFunctions::getVal($layer, 'align_hor', 'left');
		$alignVert = ThunderSliderFunctions::getVal($layer, 'align_vert', 'top');
		$left = ThunderSliderFunctions::getVal($layer, 'left', 0);
		$top = ThunderSliderFunctions::getVal($layer, 'top', 0);
		...
		if($this->adv_resp_sizes == true){
			...
			if(!is_object($left) ){
				...
				$myLeft = ThunderSliderFunctions::normalize_device_settings($left, $this->enabled_sizes, 'html-array');
			...
			if(!is_object($top)){
				...
				$myTop = ThunderSliderFunctions::normalize_device_settings($top, $this->enabled_sizes, 'html-array');
			...
			if(!is_object($alignHor)){
				...
				$myHor = ThunderSliderFunctions::normalize_device_settings($alignHor, $this->enabled_sizes, 'html-array');
			...
			if(!is_object($alignVert)){
				...
				$myVer = ThunderSliderFunctions::normalize_device_settings($alignVert, $this->enabled_sizes, 'html-array');
			...
			if(!empty($static_styles)){
				...
				if(!empty($static_styles['font-size'])){
					if(is_object($static_styles['font-size'])){
						$ss_fs = ThunderSliderFunctions::normalize_device_settings($static_styles['font-size'], $this->enabled_sizes, 'html-array');
						if(strpos($ss_fs, '[') !== false) $static_data .= str_replace('px', '', '			data-fontsize="'.$ss_fs.'"')."\n";
					}
				}
				if(!empty($static_styles['line-height'])){
					if(is_object($static_styles['line-height'])){
						$ss_lh = ThunderSliderFunctions::normalize_device_settings($static_styles['line-height'], $this->enabled_sizes, 'html-array');
						if(strpos($ss_lh, '[') !== false) $static_data .= str_replace('px', '', '			data-lineheight="'.$ss_lh.'"')."\n";
					}
				}
				if(!empty($static_styles['font-weight'])){
					if(is_object($static_styles['font-weight'])){
						$ss_fw = ThunderSliderFunctions::normalize_device_settings($static_styles['font-weight'], $this->enabled_sizes, 'html-array');
				...
				if(!empty($static_styles['color'])){
					if(is_object($static_styles['color'])){
						...
						$ss_c = ThunderSliderFunctions::normalize_device_settings($static_styles['color'], $this->enabled_sizes, 'html-array');
				...
				if(!empty($static_styles['letter-spacing'])){
					if(is_object($static_styles['letter-spacing'])){
						$ss_ls = ThunderSliderFunctions::normalize_device_settings($static_styles['letter-spacing'], $this->enabled_sizes, 'html-array');
			...
			if(is_object($max_width)){
				$max_width = ThunderSliderFunctions::normalize_device_settings($max_width, $this->enabled_sizes);
			...
			if(is_object($max_height)){
				$max_height = ThunderSliderFunctions::normalize_device_settings($max_height, $this->enabled_sizes);
			...
			if(is_object($white_space)){
				$white_space = ThunderSliderFunctions::normalize_device_settings($white_space, $this->enabled_sizes);
			...
			if(is_object($alignHor)) $alignHor = ThunderSliderFunctions::get_biggest_device_setting($alignHor, $this->enabled_sizes);
			if(is_object($alignVert)) $alignVert = ThunderSliderFunctions::get_biggest_device_setting($alignVert, $this->enabled_sizes);
			if(is_object($left)) $left = ThunderSliderFunctions::get_biggest_device_setting($left, $this->enabled_sizes);
			if(is_object($top)) $top = ThunderSliderFunctions::get_biggest_device_setting($top, $this->enabled_sizes);
		...
		$vis_desktop = ThunderSliderFunctions::getVal($layer, 'visible-desktop', true);
		$vis_notebook = ThunderSliderFunctions::getVal($layer, 'visible-notebook', true);
		$vis_tablet = ThunderSliderFunctions::getVal($layer, 'visible-tablet', true);
		$vis_mobile = ThunderSliderFunctions::getVal($layer, 'visible-mobile', true);
		...
		$il_style = (array)ThunderSliderFunctions::getVal($layer, 'inline', array());
		$adv_style = (array)ThunderSliderFunctions::getVal($layer, 'advanced', array());
		
		//add deformation and hover deformation to the layers
		$def_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());
		//get hover stuff, because of css_cursor
		$def_val_h = (array) ThunderSliderFunctions::getVal($layer, 'deformation-hover', array());
		
		$def = array();
		$st_idle = array();
		$def['o'] = array(ThunderSliderFunctions::getVal($def_val, 'opacity', '0'), '0');
		$def['sX'] = array(ThunderSliderFunctions::getVal($def_val, 'scalex', '1'), '1');
		$def['sY'] = array(ThunderSliderFunctions::getVal($def_val, 'scaley', '1'), '1');
		$def['skX'] = array(ThunderSliderFunctions::getVal($def_val, 'skewx', '0'), '0');
		$def['skY'] = array(ThunderSliderFunctions::getVal($def_val, 'skewy', '0'), '0');
		$def['rX'] = array(ThunderSliderFunctions::getVal($def_val, 'xrotate', '0'), '0');
		$def['rY'] = array(ThunderSliderFunctions::getVal($def_val, 'yrotate', '0'), '0');
		$def['rZ'] = array(ThunderSliderFunctions::getVal($layer, '2d_rotation', '0'), '0');		
		$orix = ThunderSliderFunctions::getVal($def_val, '2d_origin_x', '50%');
		if(strpos($orix, '%') === false) $orix .= '%';
		$oriy = ThunderSliderFunctions::getVal($def_val, '2d_origin_y', '50%');
		if(strpos($oriy, '%') === false) $oriy .= '%';
		$def['tO'] = array($orix . ' ' . $oriy, '50% 50%');
		$def['tP'] = array(ThunderSliderFunctions::getVal($def_val, 'pers', '600'), '600');
		$def['z'] = array(ThunderSliderFunctions::getVal($def_val, 'z', '0'), '0');
		
		$st_idle['font-family'] = array(str_replace('"', "'", ThunderSliderFunctions::getVal($def_val, 'font-family', '')), str_replace('"', "'", $dff));
		//$st_idle['text-align'] 		= array(ThunderSliderFunctions::getVal($def_val, 'text-align', 'inherit'), 'ALWAYS'); //$dta
		$st_idle['text-transform'] 	= array(ThunderSliderFunctions::getVal($def_val, 'text-transform', 'left'), $dttr);
		
		//styling
		$mfs = ThunderSliderFunctions::getVal($def_val, 'font-style', 'off');
		$font_style = ($mfs == 'on' || $mfs == 'italic') ? 'italic' : 'normal';
		
		$st_idle['font-style'] = array($font_style, $dfs);
		$st_idle['text-decoration'] = array(ThunderSliderFunctions::getVal($def_val, 'text-decoration', 'none'), $dtd);
		
		
		
		$bg_color = ThunderSliderFunctions::getVal($def_val, 'background-color', $dbc);						
		$bg_trans = ThunderSliderFunctions::getVal($def_val, 'background-transparency', $dbt);		
		...
		/*$my_padding = ThunderSliderFunctions::getVal($def_val, 'padding', array('0px','0px','0px','0px'));
		if(!empty($my_padding)){
		...
		$border_color = ThunderSliderFunctions::getVal($def_val, 'border-color', $dboc);		
		//if($border_color !== 'transparent'){
			$border_trans = ThunderSliderFunctions::getVal($def_val, 'border-transparency', $dbot);			
			$dbot = intval($dbot);
			$st_idle['border-color'] = array(TPColorpicker::convert($border_color, $border_trans), TPColorpicker::convert($dboc, $dbot)); //'ALWAYS'
		//}
		
		$st_idle['border-style'] = array(ThunderSliderFunctions::getVal($def_val, 'border-style', 'none'), $dbs);
		
		$bdw = ThunderSliderFunctions::getVal($def_val, 'border-width', '0px');
		$bdw = ThunderSliderFunctions::add_missing_val($bdw, 'px');
		if($bdw == '0px' || $bdw == '0px 0px 0px 0px') unset($st_idle['border-style']);
		$st_idle['border-width'] = array($bdw, $dbw);
		
		$my_border = ThunderSliderFunctions::getVal($def_val, 'border-width', array('0px','0px','0px','0px'));
		if(!empty($my_border) && is_array($my_border)){
			$my_border = ThunderSliderFunctions::add_missing_val($bdw, 'px');
		...
		$my_border = ThunderSliderFunctions::getVal($def_val, 'border-radius', array('0px','0px','0px','0px'));
		if(!empty($my_border)){
			$my_border = implode(' ', $my_border);
			if(trim($my_border) != '')
		...
		$css_cursor = ThunderSliderFunctions::getVal($def_val_h, 'css_cursor', 'auto');
		...
		$pointer_events = ThunderSliderFunctions::getVal($def_val_h, 'pointer_events', 'auto');
		...
		$is_hover_active = ThunderSliderFunctions::getVal($layer, 'hover', '0');
		...
		if($is_hover_active){
			...
			$def_h['o'] = array(ThunderSliderFunctions::getVal($def_val_h, 'opacity', '0'), '0');
			$def_h['sX'] = array(ThunderSliderFunctions::getVal($def_val_h, 'scalex', '1'), '1');
			$def_h['sY'] = array(ThunderSliderFunctions::getVal($def_val_h, 'scaley', '1'), '1');
			$def_h['skX'] = array(ThunderSliderFunctions::getVal($def_val_h, 'skewx', '0'), '0');
			$def_h['skY'] = array(ThunderSliderFunctions::getVal($def_val_h, 'skewy', '0'), '0');
			$def_h['rX'] = array(ThunderSliderFunctions::getVal($def_val_h, 'xrotate', '0'), '0');
			$def_h['rY'] = array(ThunderSliderFunctions::getVal($def_val_h, 'yrotate', '0'), '0');
			$def_h['rZ'] = array(ThunderSliderFunctions::getVal($def_val_h, '2d_rotation', '0'), 'inherit');
			$def_h['z'] = array(ThunderSliderFunctions::getVal($def_val_h, 'z', '0'), '0');
			
			$frame_hover['frame'] = 'hover';
			$frame_hover['speed'] = ThunderSliderFunctions::getVal($def_val_h, 'speed', '300');
			$frame_hover['ease'] = ThunderSliderFunctions::getVal($def_val_h, 'easing', 'easeOutExpo');
			
			/*$fr_force_hover = ThunderSliderFunctions::getVal($layer, 'force_hover', '');
			if($fr_force_hover !== ''){
				$frame_hover['force'] = $fr_force_hover;
			}*/
			
			//style
			$font_color = ThunderSliderFunctions::getVal($def_val_h, 'color', '#000');
			if($font_color !== 'transparent'){
				$font_trans = ThunderSliderFunctions::getVal($def_val_h, 'color-transparency', false);				
				$st_hover['c'] = array(TPColorpicker::convert($font_color, $font_trans), 'ALWAYS');
			}else{
				$st_hover['c'] = array($font_color, 'ALWAYS');
			}
			$mfs = ThunderSliderFunctions::getVal($def_val_h, 'font-style', 'off');
			$font_style = ($mfs == 'on' || $mfs == 'italic') ? 'italic' : 'normal';
			$st_hover['fs'] = array($font_style, 'normal');
			$st_hover['td'] = array(ThunderSliderFunctions::getVal($def_val_h, 'text-decoration', 'none'), 'none');
			$bg_color = ThunderSliderFunctions::getVal($def_val_h, 'background-color', 'transparent');
			//if($bg_color !== 'transparent'){
				$bg_trans = ThunderSliderFunctions::getVal($def_val_h, 'background-transparency', false);				
				$st_hover['bg'] = array(TPColorpicker::convert($bg_color, $bg_trans), 'ALWAYS');
			//}
			
			$st_hover['zi'] = array(ThunderSliderFunctions::getVal($def_val_h, 'zindex', 'auto'), 'auto'); 
			
			$border_color = ThunderSliderFunctions::getVal($def_val_h, 'border-color', 'transparent');
			//if($border_color !== 'transparent'){
				$border_trans = ThunderSliderFunctions::getVal($def_val_h, 'border-transparency', false);				
				$st_hover['bc'] = array(TPColorpicker::convert($border_color, $border_trans), 'ALWAYS');
			//}
			
			$st_hover['bs'] = array(ThunderSliderFunctions::getVal($def_val_h, 'border-style', 'none'), 'none');
			$my_border_width = ThunderSliderFunctions::getVal($def_val_h, 'border-width', array('0px','0px','0px','0px'));
			if(!empty($my_border_width) && is_array($my_border_width)){
				...
				$my_border_width = ThunderSliderFunctions::add_missing_val($my_border_width, 'px');
				$brsthoverdw = (isset($st_hover['border-width']) && $st_hover['border-width'] !== '0px 0px 0px 0px') ? 'ALWAYS' : '0px 0px 0px 0px';
				$st_hover['bw'] = array($my_border_width.' '.$my_border_width.' '.$my_border_width.' '.$my_border_width, $brsthoverdw);
			}
			
			
			$my_border = ThunderSliderFunctions::getVal($def_val_h, 'border-radius', array('0px','0px','0px','0px'));
			if(!empty($my_border)){
				$my_border = implode(' ', $my_border);
		...
		if($type == "text" || $type == "button"){
			$cornerdef = ThunderSliderFunctions::getVal($layer, "deformation");
			$cornerLeft = ThunderSliderFunctions::getVal($cornerdef, "corner_left");
			$cornerRight = ThunderSliderFunctions::getVal($cornerdef, "corner_right");
			switch($cornerLeft){
				case "curved":
		...
		//add resizeme class
		$resize_full = ThunderSliderFunctions::getVal($layer, "resize-full", true);
		if($resize_full === true || $resize_full == 'true' || $resize_full == '1'){
			$resizeme = ThunderSliderFunctions::getVal($layer, "resizeme", true);
		...
		$parallax_class = '';
		if($use_parallax == 'on'){
			$ldef = ThunderSliderFunctions::getVal($layer, 'deformation', array());
			$slide_level = ThunderSliderFunctions::getVal($ldef, 'parallax', '-');				
			$parallax_ddd = $this->slider->getParam("ddd_parallax","off");
			
			$parallax_ddd_bgfreeze=$this->slider->getParam("ddd_parallax_bgfreeze","off");
			$ddd_z = ThunderSliderFunctions::getVal($layer, 'parallax_layer_ddd_zlevel', ''); //bg , layers 
		...
		//check for actions
		$all_actions = ThunderSliderFunctions::getVal($layer, 'layer_action', array());
		
		$a_tooltip_event = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'tooltip_event', array()));
		$a_action = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'action', array()));
		$a_image_link = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'image_link', array()));
		$a_link_open_in = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'link_open_in', array()));
		$a_jump_to_slide = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'jump_to_slide', array()));
		$a_scrolloffset = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'scrollunder_offset', array()));
		$a_actioncallback = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'actioncallback', array()));
		$a_target = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'layer_target', array()));
		$a_action_delay = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'action_delay', array()));
		$a_action_speed = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'action_speed', array()));
		$a_action_easing = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'action_easing', array()));
		$a_link_type = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'link_type', array()));
		$a_link_follow = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'link_follow', array()));
		$a_toggle_layer_type = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'toggle_layer_type', array()));
		$a_toggle_class = ThunderSliderFunctions::cleanStdClassToArray(ThunderSliderFunctions::getVal($all_actions, 'toggle_class', array()));
				...
				switch($action){
							...
							*/
							...
							if($html_simple_link == ''){ //adds the link to the layer
								...
								*/
								...
								$def_val = (array) ThunderSliderFunctions::getVal($layer, 'deformation', array());
								$te_de = ThunderSliderFunctions::getVal($def_val, 'text-decoration', 'none');
								if($dtd === $te_de){ //was not written before, so write it now
									$idle_style .= 'text-decoration: '.$te_de.';';
								}
								$te_de_h = ThunderSliderFunctions::getVal($def_val_h, 'text-decoration', 'none');
								if($te_de_h === 'none'){
					...
					case 'scroll_under': //ok
						...
						$a_events[] = array(
							'event' => $a_tooltip_event[$num],
							'action' => 'scrollbelow',
							'offset' => ThunderSliderFunctions::add_missing_val($a_scrolloffset[$num], 'px'),
							'delay' => $a_action_delay[$num],
		...
		$do_loop = ThunderSliderFunctions::getVal($layer, 'loop_animation', 'none');
		$loop_data = '';
		$loop_class = '';
		...
		if($do_loop !== 'none'){
			...
			switch($do_loop){
				case 'rs-pendulum':
					$loop_data.= ' data-easing="'.ThunderSliderFunctions::getVal($layer, 'loop_easing', 'Power3.easeInOut').'"';
					$loop_data.= ' data-startdeg="'.ThunderSliderFunctions::getVal($layer, 'loop_startdeg', '-20').'"';
					$loop_data.= ' data-enddeg="'.ThunderSliderFunctions::getVal($layer, 'loop_enddeg', '20').'"';
					$loop_data.= ' data-speed="'.ThunderSliderFunctions::getVal($layer, 'loop_speed', '2').'"';
					$loop_data.= ' data-origin="'.ThunderSliderFunctions::getVal($layer, 'loop_xorigin', '50').'% '.ThunderSliderFunctions::getVal($layer, 'loop_yorigin', '50').'%"';
				break;
				case 'rs-rotate':
					$loop_data.= ' data-easing="'.ThunderSliderFunctions::getVal($layer, 'loop_easing', 'Power3.easeInOut').'"';
					$loop_data.= ' data-startdeg="'.ThunderSliderFunctions::getVal($layer, 'loop_startdeg', '-20').'"';
				...
				case 'rs-slideloop':
					$loop_data.= ' data-easing="'.ThunderSliderFunctions::getVal($layer, 'loop_easing', 'Power3.easeInOut').'"';
					$loop_data.= ' data-speed="'.ThunderSliderFunctions::getVal($layer, 'loop_speed', '1').'"';
					$loop_data.= ' data-xs="'.ThunderSliderFunctions::getVal($layer, 'loop_xstart', '0').'"';
					$loop_data.= ' data-xe="'.ThunderSliderFunctions::getVal($layer, 'loop_xend' ,'0').'"';
					$loop_data.= ' data-ys="'.ThunderSliderFunctions::getVal($layer, 'loop_ystart', '0').'"';
					$loop_data.= ' data-ye="'.ThunderSliderFunctions::getVal($layer, 'loop_yend', '0').'"';
				break;
				case 'rs-pulse':
					$loop_data.= ' data-easing="'.ThunderSliderFunctions::getVal($layer, 'loop_easing', 'Power3.easeInOut').'"';
					$loop_data.= ' data-speed="'.ThunderSliderFunctions::getVal($layer, 'loop_speed', '1').'"';
					$loop_data.= ' data-zoomstart="'.ThunderSliderFunctions::getVal($layer, 'loop_zoomstart', '1').'"';
					$loop_data.= ' data-zoomend="'.ThunderSliderFunctions::getVal($layer, 'loop_zoomend', '1').'"';
				break;
				case 'rs-wave':
					$loop_data.= ' data-speed="'.ThunderSliderFunctions::getVal($layer, 'loop_speed', '1').'"';
					$loop_data.= ' data-angle="'.ThunderSliderFunctions::getVal($layer, 'loop_angle', '0').'"';
					$loop_data.= ' data-radius="'.ThunderSliderFunctions::getVal($layer, 'loop_radius', '10').'"';
					$loop_data.= ' data-origin="'.ThunderSliderFunctions::getVal($layer, 'loop_xorigin', '50').'% '.ThunderSliderFunctions::getVal($layer, 'loop_yorigin', '50').'%"';
				break;
			}
		}
		...
		$use_tag = ThunderSliderFunctions::getVal($layer, 'html_tag', 'div');
		...
		do_action('thunderslider_add_layer_attributes', $layer, $this->slide, $this->slider);
		...
		$ro = ThunderSliderFunctions::getVal($layer, 'responsive_offset', true);
		...
		if($static_slide){
			if($this->slider->getParam('template', 'false') != "true" && $this->slider->getParam('slider-type') !== 'hero'){
				$start_on_slide = intval(ThunderSliderFunctions::getVal($layer,"static_start",1)) - 1;
				$end_on_slide = ThunderSliderFunctions::getVal($layer,"static_end",2);
		...
		$zi = ThunderSliderFunctions::getVal($layer, 'zIndex', false);
		...
		if($special_type === false){
			echo apply_filters('thunderslider_layer_content', stripslashes($html), $html, $this->slider->getID(), $this->slide, $layer)." ";
		}
		if ($toggle_allow!=false) {
			echo '</div>';
			echo '<div class="';
	...
	public function set_placeholders($text){
		...
		if(isset($post->ID)) $text = apply_filters( 'thunderslider_gallery_set_placeholders', $text , $post->ID );
		return $text;
	}
	...
	public function putCreativeZones(){
		...
		foreach($zones as $zs => $zone){ //go through all three zones
			$lzone = array();
			foreach($layers as $layer){
				if(ThunderSliderFunctions::getVal($layer, 'type', 'text') !== 'row') continue; //we only want to handle rows here to get the zones we need to create
				
				$this->row_zindex = 0;
				
				$layer_zone = ThunderSliderFunctions::getVal($layer, 'align_vert');
				$layer_zone = (is_object($layer_zone)) ? ThunderSliderFunctions::get_biggest_device_setting($layer_zone, $enabled_sizes) : $layer_zone;
	...
	public function putCreativeRows($layers, $current_zone){
		...
		$go = 9999;
		foreach($layers as $layer){
			if(ThunderSliderFunctions::getVal($layer, 'type', 'text') !== 'row') continue; //we only want to handle rows here of the current zone and add them as a rows
			...
			$groupOrder = ThunderSliderFunctions::getVal($layer, 'groupOrder');
			if($groupOrder === ''){
				$groupOrder = $go;
				$go++;
			}
			
			$zi = ThunderSliderFunctions::getVal($layer, 'zIndex', false); // set the z-index so that the wrapper gains the highest one
			$zi = ($zi === false) ? $this->zIndex : $zi;
	...
	public function putCreativeColumn($layers, $unique_id){
		...
		foreach($layers as $layer){
			...
			if(ThunderSliderFunctions::getVal($layer, 'type', 'text') !== 'column') continue; //we only want to handle columns here of the current row
			if(ThunderSliderFunctions::getVal($layer, 'p_uid', -1) !== $unique_id) continue; //has the wrong row ID
			
			$this->putLayer($layer, true, false, 'column');
			$cuid = ThunderSliderFunctions::getVal($layer, 'unique_id', -1);
			
			//add layers here
			$group_layers = array();
			$go = 9999;
			foreach($layers as $nlayer){
				$uid = ThunderSliderFunctions::getVal($nlayer, 'p_uid', -1);
				if($uid !== $cuid) continue;
				
				$groupOrder = ThunderSliderFunctions::getVal($nlayer, 'groupOrder');
				if($groupOrder === ''){
					$groupOrder = $go;
					$go++;
				}
	...
	public function putCreativeGroups(){
		...
		foreach($layers as $layer){
			if(ThunderSliderFunctions::getVal($layer, 'type', 'text') !== 'group') continue; //we only want to handle groups here to get the zones we need to create
	...
	public function putCreativeGroupLayer($layers, $u_id){
		...
		foreach($layers as $layer){
			$unique_id = ThunderSliderFunctions::getVal($layer, 'p_uid');
	...
	private function putJS($markup_export = false){
		...
		$hideSliderAtLimit = $this->slider->getParam("hide_slider_under","0",ThunderSlider::VALIDATE_NUMERIC);
		if(!empty($hideSliderAtLimit))
			$hideSliderAtLimit++;

		$hideCaptionAtLimit = $this->slider->getParam("hide_defined_layers_under","0",ThunderSlider::VALIDATE_NUMERIC);;
		if(!empty($hideCaptionAtLimit))
			$hideCaptionAtLimit++;

		$hideAllCaptionAtLimit = $this->slider->getParam("hide_all_layers_under","0",ThunderSlider::VALIDATE_NUMERIC);;
		...
		$swipe_velocity = $this->slider->getParam("swipe_velocity","0.7",ThunderSlider::VALIDATE_NUMERIC);
		$swipe_min_touches = $this->slider->getParam("swipe_min_touches","1",ThunderSlider::VALIDATE_NUMERIC);
		...
		$do_delay = apply_filters('thunderslider_add_js_delay', $do_delay);
		...
		$csizes = ThunderSliderFunctions::get_responsive_size($this);
...
<?php
	...
	echo '		thunderslider_showDoubleJqueryError("#'.$this->sliderHtmlID.'");'."\n";
	echo '	}else{'."\n";
	...
	echo '			delay:'.esc_attr($this->slider->getParam("delay","9000",ThunderSlider::FORCE_NUMERIC)) .','."\n";
	
	do_action('thunderslider_fe_javascript_option_output', $this->slider);
	...
	if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')){
		...
		if($enable_arrows == 'on'){
			...
			if($hide_arrows_on_mobile === 'true') {
				echo '					hide_under:'. esc_attr($this->slider->getParam('arrows_under_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			if($hide_arrows_over === 'true') {
				echo '					hide_over:'. esc_attr($this->slider->getParam('arrows_over_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					hide_onleave:'.$arrows_always_on.','."\n";
			if($arrows_always_on === 'true'){
				echo '					hide_delay:'. esc_attr($this->slider->getParam('hide_arrows','200',ThunderSlider::FORCE_NUMERIC)).','."\n";
				echo '					hide_delay_mobile:'. esc_attr($this->slider->getParam('hide_arrows_mobile','1200',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					tmp:\'';
			echo preg_replace( "/\r|\n/", "", $arr_tmp);
			echo '\','."\n";
			...
			echo '						h_offset:'. esc_attr($this->slider->getParam('leftarrow_offset_hor','20',ThunderSlider::FORCE_NUMERIC)) .','."\n";
			echo '						v_offset:'. esc_attr($this->slider->getParam('leftarrow_offset_vert','0',ThunderSlider::FORCE_NUMERIC))."\n";										
			echo '					},'."\n";
			echo '					right: {'."\n";
			...
			echo '						h_offset:'. esc_attr($this->slider->getParam('rightarrow_offset_hor','20',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '						v_offset:'. esc_attr($this->slider->getParam('rightarrow_offset_vert','0',ThunderSlider::FORCE_NUMERIC))."\n";					
			echo '					}'."\n";
			echo '				}'."\n";
		}
		...
		if($enable_bullets == 'on'){
			...
			if($hide_bullets_on_mobile === 'true'){
				echo '					hide_under:'. esc_attr($this->slider->getParam('bullets_under_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			if($hide_bullets_over === 'true'){
				echo '					hide_over:'. esc_attr($this->slider->getParam('bullets_over_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					style:"'. esc_attr($navigation_bullets_style).'",'."\n";
			echo '					hide_onleave:'.$bullets_always_on.','."\n";
			if($bullets_always_on === 'true'){
				echo '					hide_delay:'. esc_attr($this->slider->getParam('hide_bullets','200',ThunderSlider::FORCE_NUMERIC)).','."\n";
				echo '					hide_delay_mobile:'. esc_attr($this->slider->getParam('hide_bullets_mobile','1200',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			...
			echo '					h_offset:'. esc_attr($this->slider->getParam('bullets_offset_hor','20',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					v_offset:'. esc_attr($this->slider->getParam('bullets_offset_vert','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					space:'. esc_attr($this->slider->getParam('bullets_space','5',ThunderSlider::FORCE_NUMERIC)).','."\n";					
			echo '					tmp:\'';
			echo preg_replace( "/\r|\n/", "", $bul_tmp);
			echo '\''."\n";
			echo '				}'."\n";
		...
		if($enable_thumbnails == 'on'){
			...
			echo '					width:'. esc_attr($this->slider->getParam('thumb_width','100',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					height:'. esc_attr($this->slider->getParam('thumb_height','50',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					min_width:'. esc_attr($this->slider->getParam('thumb_width_min','100',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					wrapper_padding:'. esc_attr($this->slider->getParam('thumbnails_padding','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			...
			echo preg_replace( "/\r|\n/", "", $thumbs_tmp);
			echo '\','."\n";
			echo '					visibleAmount:'. esc_attr($this->slider->getParam('thumb_amount','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					hide_onmobile:'.$hide_thumbs_on_mobile.','."\n";
			if($hide_thumbs_on_mobile === 'true'){
				echo '					hide_under:'. esc_attr($this->slider->getParam('thumbs_under_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			if($hide_thumbs_over === 'true'){
				echo '					hide_over:'. esc_attr($this->slider->getParam('thumbs_over_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					hide_onleave:'.$thumbs_always_on.','."\n";
			if($thumbs_always_on === 'true'){
				echo '					hide_delay:'. esc_attr($this->slider->getParam('hide_thumbs','200',ThunderSlider::FORCE_NUMERIC)).','."\n";
				echo '					hide_delay_mobile:'. esc_attr($this->slider->getParam('hide_thumbs_mobile','1200',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			...
			echo '					space:'. esc_attr($this->slider->getParam('thumbnails_space','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			...
			echo '					h_offset:'. esc_attr($this->slider->getParam('thumbnails_offset_hor','20',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					v_offset:'. esc_attr($this->slider->getParam('thumbnails_offset_vert','0',ThunderSlider::FORCE_NUMERIC))."\n";					
			echo '				}'."\n";
		}
		if($enable_tabs == 'on'){
			...
			echo '					width:'. esc_attr($this->slider->getParam('tabs_width','100',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					height:'. esc_attr($this->slider->getParam('tabs_height','50',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					min_width:'. esc_attr($this->slider->getParam('tabs_width_min','100',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					wrapper_padding:'. esc_attr($this->slider->getParam('tabs_padding','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			...
			echo preg_replace( "/\r|\n/", "", $tabs_tmp);
			echo '\','."\n";
			echo '					visibleAmount: '. esc_attr($this->slider->getParam('tabs_amount','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					hide_onmobile: '.$hide_tabs_on_mobile.','."\n";
			if($hide_tabs_on_mobile === 'true'){
				echo '					hide_under:'. esc_attr($this->slider->getParam('tabs_under_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			if($hide_tabs_over === 'true'){
				echo '					hide_over:'. esc_attr($this->slider->getParam('tabs_over_hidden','0',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					hide_onleave:'.$tabs_always_on.','."\n";
			if($tabs_always_on === 'true'){
				echo '					hide_delay:'. esc_attr($this->slider->getParam('hide_tabs','200',ThunderSlider::FORCE_NUMERIC)).','."\n";
				echo '					hide_delay_mobile:'. esc_attr($this->slider->getParam('hide_tabs_mobile','1200',ThunderSlider::FORCE_NUMERIC)).','."\n";
			}
			echo '					hide_delay:'. esc_attr($this->slider->getParam('hide_tabs','200',ThunderSlider::FORCE_NUMERIC)).','."\n";
			...
			echo '					space:'. esc_attr($this->slider->getParam('tabs_space','5',ThunderSlider::FORCE_NUMERIC)).','."\n";
			...
			echo '					h_offset:'. esc_attr($this->slider->getParam('tabs_offset_hor','20',ThunderSlider::FORCE_NUMERIC)).','."\n";
			echo '					v_offset:'. esc_attr($this->slider->getParam('tabs_offset_vert','0',ThunderSlider::FORCE_NUMERIC))."\n";											
			echo '				}'."\n";
		}
		echo '			},'."\n";
	...
	if($slider_type == 'carousel'){
		...
		$car_space = $this->slider->getParam('carousel_space',0,ThunderSlider::FORCE_NUMERIC);
		$car_stretch = $this->slider->getParam('carousel_stretch','off');
		$car_all_layers = $this->slider->getParam('showalllayers_carousel','off');
		$car_maxitems = $this->slider->getParam('carousel_maxitems',5,ThunderSlider::FORCE_NUMERIC);
		...
		$car_speed = $this->slider->getParam('carousel_speed','800',ThunderSlider::FORCE_NUMERIC);
		...
		$car_maxrotation = $this->slider->getParam('carousel_maxrotation',90,ThunderSlider::FORCE_NUMERIC);
		...
		$car_scaledown = $this->slider->getParam('carousel_scaledown',50,ThunderSlider::FORCE_NUMERIC);
		if($car_scaledown > 100) $car_scaledown = 100;
		
		$car_borderr = $this->slider->getParam('carousel_borderr',0,ThunderSlider::FORCE_NUMERIC);
		$car_borderr_unit = $this->slider->getParam('carousel_borderr_unit','px');
		
		$car_padding_top = $this->slider->getParam('carousel_padding_top',0,ThunderSlider::FORCE_NUMERIC);
		$car_padding_bottom = $this->slider->getParam('carousel_padding_bottom',0,ThunderSlider::FORCE_NUMERIC);
	...
	$viewport_area = $this->slider->getParam('viewport_area', 80, ThunderSlider::FORCE_NUMERIC);
	...
	echo '	}'."\n";
	echo '	';
	do_action('thunderslider_fe_javascript_output', $this->slider, $this->sliderHtmlID);
	echo ''."\n";
	echo '});	/*ready*/'."\n";
	?>
</script>
		...
		if($js_to_footer && $this->previewMode == false && $markup_export == false){
			...
			add_action('mcms_print_footer_scripts', array($this, 'add_inline_js'), 100);
		}
		...
		switch($use_spinner){
			...
			case '2':
				if(!is_admin()){?><script>
					...
					var htmlDiv = document.getElementById('rs-module-settings-inline-css');
					if(htmlDiv) {
		...
		if($this->slider->getParam("custom_css", '') !== ''){
			if(!is_admin()){
				?><script>
					var htmlDivCss = unescape("<?php echo ThunderSliderCssParser::compress_css(rawurlencode(stripslashes($this->slider->getParam('custom_css', ''))));?>");
					var htmlDiv = document.getElementById('rs-module-settings-inline-css');
					if(htmlDiv) {
						htmlDiv.innerHTML = htmlDiv.innerHTML + htmlDivCss;
					}
				...
				?><style type="text/css"><?php echo ThunderSliderCssParser::compress_css(stripslashes($this->slider->getParam('custom_css', '')));?></style><?php	
				if($markup_export === true){
					echo '<!-- /STYLE -->';
				}
			}
		...
		if(trim($nav_css) !== ''){
			if(!is_admin()){
				?><script>
					var htmlDivCss = unescape("<?php echo ThunderSliderCssParser::compress_css(rawurlencode($nav_css));?>");
					var htmlDiv = document.getElementById('rs-module-settings-inline-css');
					if(htmlDiv) {
						htmlDiv.innerHTML = htmlDiv.innerHTML + htmlDivCss;
					}
				...
				?>
				<style type="text/css"><?php echo ThunderSliderCssParser::compress_css(($nav_css));?></style>
				<?php
				if($markup_export === true){
					echo '<!-- /STYLE -->';
				}
		...
		if(!$markup_export){ //not needed for html markup export
			if(has_action('mcms_footer', array($this, 'add_inline_double_jquery_error')) === false){
				add_action('mcms_footer', array($this, 'add_inline_double_jquery_error'));
			}
		}
	}
	...
	/**
	 * Output thunderslider_showDoubleJqueryError
	 */
	public function add_inline_double_jquery_error(){
		...
		if($rs_double_jquery_script === false){
			?>
			<script type="text/javascript">
				function thunderslider_showDoubleJqueryError(sliderID) {
					var errorMessage = "RazorLeaf ThunderSlider Error: You have some jquery.js library include that comes after the revolution files js include.";
	...
	public function add_inline_styles(){
		...
		if(!is_admin()){
			echo '<script>var htmlDiv = document.getElementById("rs-module-settings-inline-css"); var htmlDivCss="';
		} 
		else echo "<style>";
	...
	public function putErrorMessage($message){
		...
		<div style="width:800px;height:300px;margin-bottom:10px;margin:0px auto;">
			<div style="margin: auto; line-height: 40px; font-size: 14px; color: #FFF; padding: 15px; background: #E74C3C; margin: 20px 0px;">
				<?php _e("RazorLeaf ThunderSlider Error",'thunderslider'); ?>: <?php echo $message; ?>
			</div>
		</div>
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery(".thunder_slider").show();
			});
		</script>
		<?php
	}
	...
	public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array()){
		...
		try{
			...
			do_action('thunderslider_modify_core_settings', $this->slider);
			...
			//set slider language
			$isWpmlExists = ThunderSliderWpml::isWpmlExists();
			$useWpml = $this->slider->getParam("use_mcmsml","off");
			if(	$isWpmlExists && $useWpml == "on"){
				if($this->previewMode == false)
					$this->sliderLang = ThunderSliderFunctionsMCMS::getCurrentLangCode();
			}
			...
			if($this->slider->getParam("load_googlefont","false") == "true"){
				...
				if(is_array($googleFont)){
					...
					//$htmlBeforeSlider .= ThunderSliderOperations::getCleanFontImport($googleFont);
					ThunderSliderOperations::setCleanFontImport($googleFont);
				}
			}
			...
			if(!empty($gfonts) && is_array($gfonts)){
				foreach($gfonts as $gf){
					...
					$htmlBeforeSlider .= ThunderSliderOperations::getCleanFontImport($gf);
					ThunderSliderOperations::setCleanFontImport($gf);
				}
			}*/
			...
			foreach($gf as $gfk => $gfv){
				...
				//$htmlBeforeSlider .= ThunderSliderOperations::getCleanFontImport($gfk, '', '', $variants, $subsets);
				ThunderSliderOperations::setCleanFontImport($gfk, '', '', $variants, $subsets);
			}
			
			$htmlBeforeSlider .= ThunderSliderOperations::printCleanFontImport();
			...
			if($this->slider->getParam("js_to_body","false") == "true"){
				...
				if($enable_logs == 'on'){
					$urlIncludeJS = RS_PLUGIN_URL.'public/assets/js/jquery.myskinpunch.enablelog.js?rev='. $slver;
					$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				}

				$urlIncludeJS = RS_PLUGIN_URL.'public/assets/js/jquery.myskinpunch.tools.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				$urlIncludeJS = RS_PLUGIN_URL.'public/assets/js/jquery.myskinpunch.revolution.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
			...
			$bannerWidth = $this->slider->getParam("width",null,ThunderSlider::VALIDATE_NUMERIC,"Slider Width");
			$bannerHeight = $this->slider->getParam("height",null,ThunderSlider::VALIDATE_NUMERIC,"Slider Height");
			...
			if(trim($slider_id) !== ''){
				...
				$this->sliderHtmlID = 'thunder_slider_'.$sliderID.'_'.self::$sliderSerial;
			}
			...
			if($sliderType != 'fullscreen'){
				...
				if($sliderPosition != 'center'){
					$containerStyle .= 'margin-left:'.esc_attr($this->slider->getParam('margin_left', '0', ThunderSlider::FORCE_NUMERIC)).'px;';
					$containerStyle .= 'margin-right:'.esc_attr($this->slider->getParam('margin_right', '0', ThunderSlider::FORCE_NUMERIC)).'px;';
				}

				$containerStyle .= 'margin-top:'.esc_attr($this->slider->getParam('margin_top', '0', ThunderSlider::FORCE_NUMERIC)).'px;';
				$containerStyle .= 'margin-bottom:'.esc_attr($this->slider->getParam('margin_bottom', '0', ThunderSlider::FORCE_NUMERIC)).'px;';
			}
			...
			$sliderWrapperClass = "thunder_slider_wrapper";
			$sliderClass = "thunder_slider";
			...
			$maxWidth = $this->slider->getParam('max_width', '0', ThunderSlider::FORCE_NUMERIC);
			...
			echo '<!-- START REVOLUTION SLIDER '. $thunderSliderVersion .' '. $sliderType .' mode -->'."\n";

			echo '	<div id="'. $this->sliderHtmlID .'"';
			echo ' class="'. $sliderClass . $do_overflow .'"';
			echo ' style="'. $bannerStyle .'"';
			echo ' data-version="'.$thunderSliderVersion.'">'."\n";
	...
	public function add_custom_navigation_css($slides){
		...
		if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on')){
			if(!empty($slides)){
				...
				if(!is_admin()){
					echo addslashes(ThunderSliderCssParser::compress_css($this->rev_custom_navigation_css)).'";
						if(htmlDiv) {
							htmlDiv.innerHTML = htmlDiv.innerHTML + htmlDivCss;
						}else{
							var htmlDiv = document.createElement("div");
	...
	public function is_in_group_or_row($layer){
		$puid = ThunderSliderFunctions::getVal($layer, 'p_uid');
