...
class ThunderSliderTinyBox {
	...
	public static function add_tinymce_editor(){
		...
		if((isset($_GET['action']) && $_GET['action'] == 'edit') || (isset($_GET['vc_action']) && $_GET['vc_action'] == 'vc_inline') || $pagenow == 'post-new.php'){
			...
			if(get_user_option('rich_editing') == 'true'){
				add_filter('mce_external_modules', array('ThunderSliderTinyBox', 'add_tinymce_shortcode_editor_module'));
				add_filter('mce_buttons', array('ThunderSliderTinyBox', 'add_tinymce_shortcode_editor_button'));
			}
			
			add_action('in_admin_footer', array('ThunderSliderTinyBox', 'add_tiny_mce_shortcode_dialog'));
		}else{
			return;
		}
	}
	...
	/**
	 * Allow for VC to use this module
	 */
	public static function visual_composer_include(){
		if(@is_user_logged_in()){
			...
			if('vc_inline' === vc_action() || is_admin()){
				add_action( 'init', array('ThunderSliderTinyBox', 'add_to_VC' ));
			}
		}
	}
	...
	public static function add_to_VC() {
		...
		//mcms_enqueue_script('thunderslider-admin-script', RS_PLUGIN_URL.'admin/assets/js/admin.js', array('jquery'), ThunderSliderGlobals::SLIDER_REVISION );
		mcms_enqueue_script('mcmsdialogs', 'jquery-ui-sortable', 'jquery-ui-dialog');
		mcms_enqueue_style('mcms-jquery-ui-dialog');
		
		vc_map( array(
			'name' => __('RazorLeaf ThunderSlider 5', 'thunderslider'),
			'base' => 'thunder_slider',
			'icon' => 'icon-mcmsb-thunderslider',
			'category' => __('Content', 'thunderslider'),
			...
			'params' => array(
				array(
					'type' => 'thunder_slider_shortcode',
					'heading' => __('Alias', 'thunderslider'),
					'param_name' => 'alias',
					'admin_label' => true,
					'value' => ''
				),
				array(
					'type' => 'thunder_slider_shortcode',
					'heading' => __('Order', 'thunderslider'),
					'param_name' => 'order',
					'admin_label' => true,
					'value' => ''
				)
		...
		if(version_compare(MCMSB_VC_VERSION, '4.4', '>=')){ //use if 4.4 or newer
			vc_add_shortcode_param('thunder_slider_shortcode', array('ThunderSliderTinyBox', 'thunderslider_shortcode_settings_field'));
		}else{ //use if older than 4.4
			add_shortcode_param('thunder_slider_shortcode', array('ThunderSliderTinyBox', 'thunderslider_shortcode_settings_field'));
		}
		
		//if ( vc_is_frontend_ajax() || vc_is_frontend_editor() ) {
		//	remove_filter( 'vc_thunderslider_shortcode', array( 'Vc_Vendor_Revslider', 'setId' ) );
		//}
		
	}
	
	//public function setId( $output ) {
	//	return preg_replace( '/thunder_slider_(\d+)_(\d+)/', 'thunder_slider_$1_$2' . time() . '_' . self::$instanceIndex ++, $output );
	//}
	 ...
	 * @since: 5.1.1
	 */
	public static function thunderslider_shortcode_settings_field($settings, $value) {
		
		return '<div class="thunder_slider_shortcode_block">'
	 ...
	 * @since: 5.1.1
	 */
	public static function add_tinymce_shortcode_editor_module($module_array){
	
		$module_array['thunderslider_sc_button'] = RS_PLUGIN_URL . 'admin/assets/js/tinymce-shortcode-script.js';
	...
	public static function add_tinymce_shortcode_editor_button($buttons){
		...
		array_push($buttons, "thunderslider_sc_button");
	...
	public static function add_tiny_mce_shortcode_dialog(){
		?>
		<div id="thunderslider-tiny-mce-dialog" tabindex="-1" action="" title="" style="display: none; ">
			<form id="thunderslider-tiny-mce-settings-form" action="">
				<!-- STEP 1 -->
				<div id="thunderslider-tiny-dialog-step-1">					
					<p class="thunderslider-quicktitle"></p>
					<?php
						self::add_step_1_markup();
					?>
					<div class="thunderslider-stepnavigator">
						<span class="thunderslider-currentstep"><strong><?php _e('STEP 1', 'thunderslider'); ?></strong><?php _e('Select / Create Slider', 'thunderslider'); ?></span>
						<span class="thunderslider-step-actions-wrapper">
							<a href="javascript:void(0);" class="button-primary nonclickable" id="rs-modify-predefined-slider"><span class="dashicons dashicons-admin-generic"></span><?php _e('Quick Modify Slider', 'thunderslider'); ?></a>
							<a href="javascript:void(0);" class="button-primary nonclickable" id="rs-add-predefined-slider"><span class="dashicons dashicons-migrate"></span><?php _e('Add Selected Slider', 'thunderslider'); ?></a>							
						</span>
				...
				<!-- STEP 1.5 -->
				<div id="thunderslider-tiny-dialog-step-1-5" style="display: none;">
					<p class="thunderslider-quicktitle"></p>
					<div id="thunderslider-tiny-settings-wrap" class="thunderslider-quick-inner-wrapper" style="padding-right:0px;padding-bottom:0px;">
							<ul class="rs-mod-slides-wrapper">
								
							</ul>
					</div>					
					<div class="thunderslider-stepnavigator"><span class="thunderslider-currentstep"><strong><?php _e('STEP 2', 'thunderslider'); ?></strong><?php _e('Quick Modify / Add Slider', 'thunderslider'); ?></span>
						<span class="thunderslider-step-actions-wrapper">
							<a href="javascript:void(0);" class="button-primary  rs-goto-step-1"><span class="dashicons dashicons-arrow-left-alt"></span><?php _e('Select Slider', 'thunderslider'); ?></a>
							<a href="javascript:void(0);" class="button-primary " id="thunderslider-add-custom-shortcode-modify"><span class="dashicons dashicons-migrate"></span><?php _e('Add Selected Slider', 'thunderslider'); ?></a>							
						</span>
					</div>
				</div>
			</form>
	...
	public static function add_step_1_markup(){
		...
		?>
		<div class="thunderslider-quick-inner-wrapper" style="padding-right:0px;padding-bottom:0px;">
			<select name="thunderslider-existing-slider" id="thunderslider-existing-slider">
				<option value="-1" selected="selected"><?php _e('--- Choose Slider ---', 'thunderslider'); ?></option>
				<?php
				$sl = array();
				$sliders_info = array();
				if(!empty($sliders)){
			...
			<ul id="rs-shortcode-select-wrapper">
				<li class="rs-slider-modify-li rs-slider-modify-new-slider">								
					<a href="<?php echo ThunderSliderBaseAdmin::getViewUrl(ThunderSliderAdmin::VIEW_SLIDER); ?>" target="_blank" class="button-primary revgreen" id="rs-create-predefined-slider"><span class="dashicons dashicons-plus"></span><span class="rs-slider-modify-title"><?php _e('Create Slider', 'thunderslider'); ?></span></a>
				</li>
				<?php
			...
			<script type="text/javascript">
				var thunder_sliders_info = jQuery.parseJSON(<?php echo ThunderSliderFunctions::jsonEncodeForClientSide($sliders_info); ?>);
			</script>
		</div>
		<?php
	}
 ...
 * @since: 5.0
 **/
class ThunderSlider_TinyBox extends ThunderSliderTinyBox {}
?>