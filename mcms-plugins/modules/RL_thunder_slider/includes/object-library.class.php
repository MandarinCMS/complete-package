...
class ThunderSliderObjectLibrary {
	
	private $library_list		= 'library.php';
	private $library_download	= 'download.php';
	
	private $object_thumb_path	= '/thunderslider/objects/thumbs/';
	private $object_orig_path	= '/thunderslider/objects/';
	...
	public function _get_list($force = false){
		...
		$last_check	= get_option('thunderslider-library-check');
		
		if($last_check == false){ //first time called
			$last_check = 1296001;
			update_option('thunderslider-library-check',  time());
		}
		...
		if(time() - $last_check > 1296000 || $force == true){ //30 days
			...
			do{	
				...
				$request	= mcms_remote_post($url.'/'.$this->library_list, array(
	...
	public function _get_object_thumb($object_handle, $type){
		...
		if($download && mcms_mkdir_p( $upload_dir['basedir'].$path ) ) {
			
			$curl = ($this->check_curl_connection()) ? new MCMS_Http_Curl() : false;
			...
			if(!file_exists($file) || isset($temp['push_image'])){
				...
				if($curl !== false){
					...
					if($validated == 'false' && $type != 'thumb'){
						$error = __('Module not activated', 'thunderslider');
					}else{
						$code = ($validated == 'false') ? '' : get_option('thunderslider-code', '');
						
						$done	= false;
						$count	= 0;
						do{	
							$url		= $rslb->get_url('library');
							$image_data = mcms_remote_post($url.'/'.$this->library_download, array(
								'user-agent' => 'MandarinCMS/'.$mcms_version.'; '.get_bloginfo('url'),
								'body' => array(
									'code' => urlencode($code),
							...
							$response_code = mcms_remote_retrieve_response_code( $image_data );
							if($response_code == 200){
								$done = true;
							}else{
								$rslb->move_server_list();
						...
						if(!is_mcms_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
							...
							$image_data = false;
							$error = __('Error downloading object', 'thunderslider');
						}
					}
				}else{
					//cant download file
	...
	public function write_markup(){
		...
		<div id="dialog_addobj" class="dialog-addobj" title="<?php _e("Add Object Layer",'thunderslider'); ?>" style="display:none">
				...
				<div id="addobj-list-of-items">
					<div id="addobj-dialog-header">
						<div class="object_library_search_wrapper">
							<input type="text" id="obj_library_search" placeholder="<?php _e("Search for Objects...",'thunderslider'); ?>" /><span id="obj_library_search_trigger"><i class="eg-icon-search"></i></span>
						</div>												
						<div id="object_library_type_list_new">
							<span id="obj_lib_main_cat_filt_all" data-value="all" class="obj_library_cats_filter"><?php _e("ALL",'thunderslider'); ?></span>
							<span id="obj_lib_main_cat_filt_allimages" data-value="allimages" class="obj_library_cats_filter all_img_cat"><?php _e("ALL IMAGES",'thunderslider'); ?></span>
							<span id="obj_lib_main_cat_filt_svg" data-value="svg" class="obj_library_cats_filter svg_cat"><?php _e("SVG",'thunderslider'); ?></span>
							<span id="obj_lib_main_cat_filt_icon" data-value="icon" class="obj_library_cats_filter fonticon_cat"><?php _e("ICON",'thunderslider'); ?></span>							
							<span id="obj_lib_main_cat_filt_image" data-value="image" class="obj_library_cats_filter png_cat"><?php _e("PNG",'thunderslider'); ?></span>
							<span id="obj_lib_main_cat_filt_bgimage" data-value="bgimage" class="obj_library_cats_filter jpg_cat"><?php _e("JPG",'thunderslider'); ?></span>
						</div>			
						<div id="up-lic-ob-lib">																
							<div id="licence_obect_library"><i class="fa-icon-copyright"></i><?php _e("License Info",'thunderslider'); ?></div>
							<div id="update_obect_library"><i class="eg-icon-arrows-ccw"></i><?php _e("Update Object Library",'thunderslider'); ?></div>
						</div>
						<div id="object-tag-list" class="object-tag-list"><span id="obj_library_cats_favorite" class="obj_library_cats" data-tag="favorite" data-image="true" data-bgimages="true" data-svg="true"><i class="fa-icon-star-o" style="margin-right: 5px;"></i><?php _e("Favorite",'thunderslider'); ?></span><span id="obj_library_cats_allico" class="obj_library_cats" data-tag="allicon" data-icon="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php _e("All Icons",'thunderslider'); ?></span><span id="obj_library_cats_allpng" class="obj_library_cats" data-tag="allpng" data-image="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php _e("All PNG",'thunderslider'); ?></span><span id="obj_library_cats_alljpg" class="obj_library_cats" data-tag="alljpg" data-image="false" data-bgimages="true" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php _e("All JPG",'thunderslider'); ?></span><span id="obj_library_cats_allsvg" class="obj_library_cats" data-tag="allsvg" data-image="false" data-bgimages="false" data-svg="true"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php _e("All SVG",'thunderslider'); ?></span></div>
					</div>
			...
			<div id="bg-vs-layer-wrapper">
				<span id="add_objimage_as_layer"><?php _e("As Layer",'thunderslider'); ?></span><span class="addthisasbg" id="obj-layer-bg-switcher"></span><span id="add_objimage_as_slidebg"><?php _e("As Slide BG",'thunderslider'); ?></span>
			</div>
			...
			function createFontIcons() {
				...
				if (sheets)
					jQuery.each(sheets,function(index,sheet) {
						var found = false,
							markup = "";	
						if (sheet && sheet.href && sheet.href.indexOf("modules/thunderslider/public/assets/fonts/")>=0) {							
							try{
	...
	public function load_objects(){
		...
		$svgs = ThunderSliderBase::get_svg_sets_full();
	...
	public function create_image_dimensions($handle, $force = false){
		$img_editor_test = mcms_image_editor_supports(array('methods' => array('resize', 'save')));
		...
		$image = mcms_get_image_editor($image_path);
		$imgsize = getimagesize($image_path);
		
		if(!is_mcms_error($image) && $imgsize !== false) {
