...
const Device = (function Device() {
  ...
  if (device.os && device.os === 'ios') {
    ...
    const metaViemcmsort = doc.querySelector('meta[name="viemcmsort"]');
    device.minimalUi =
      !device.webView &&
      (ipod || iphone) &&
      (osVersionArr[0] * 1 === 7 ? osVersionArr[1] * 1 >= 1 : osVersionArr[0] * 1 > 7) &&
      metaViemcmsort && metaViemcmsort.getAttribute('content').indexOf('minimal-ui') >= 0;
  }
...
var addClasses = function () {
  ...
  // MCMS8 Touch Events Fix
  if (Support.pointerEvents || Support.prefixedPointerEvents) {
    suffixes.push(`mcms8-${params.direction}`);
  }
...
const Keyboard = {
  handle(event) {
    ...
    if (swiper.params.keyboard.onlyInViemcmsort && (kc === 37 || kc === 39 || kc === 38 || kc === 40)) {
      let inView = false;
