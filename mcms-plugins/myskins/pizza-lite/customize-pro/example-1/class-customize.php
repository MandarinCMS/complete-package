<?php
/**
 * Singleton class for handling the myskin's customizer integration.
 *
 * @since  1.0.0
 * @access public
 */
final class pizza_lite_Example_1_Customize {
		...
		$manager->add_section(
			new pizza_lite_Example_1_Customize_Section_Pro(
				...
				array(
					...
					'pro_url'  => 'https://www.sktmyskins.net/shop/pizza-ordering-mandarincms-myskin/',
					'priority'   => 1,
				)
			)
		);
	...
	public function pizza_lite_enqueue_control_scripts() {
		...
		mcms_enqueue_script( 'pizza_lite', trailingslashit( get_template_directory_uri() ) . 'customize-pro/example-1/customize-controls.js', array( 'customize-controls' ) );

		mcms_enqueue_style( 'pizza_lite', trailingslashit( get_template_directory_uri() ) . 'customize-pro/example-1/customize-controls.css' );
	}
}
