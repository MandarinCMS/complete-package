...
class jmd_magandanow_new_posts_focus extends MCMS_Widget {
	function __construct() {
		parent::__construct(
			'jmd_magandanow_new_posts_focus', esc_html_x('MH Posts Focus [lite]', 'widget name', 'jmd-magandanow-new'),
			array(
				'classname' => 'jmd_magandanow_new_posts_focus',
				'description' => esc_html__('MH Posts Focus widget to display 5 posts with focus on large post in the middle (if placed in full-width widget area).', 'jmd-magandanow-new'),
	...
	function widget($args, $instance) {
		...
		$widget_posts = new MCMS_Query($query_args);
										...
										<figure class="mh-posts-focus-thumb mh-posts-focus-thumb-large">
											<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
												if (has_post_thumbnail()) {
													the_post_thumbnail('jmd-magandanow-new-large');
												} else {
													echo '<img class="mh-image-placeholder" src="' . get_template_directory_uri() . '/images/placeholder-large.png' . '" alt="' . esc_html__('No Image', 'jmd-magandanow-new') . '" />';
												} ?>
											</a>
										</figure>
    ...
    function form($instance) {
        ...
        $instance = mcms_parse_args($instance, $defaults); ?>
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'jmd-magandanow-new'); ?></label>
