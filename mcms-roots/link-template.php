<?php
/**
 * MandarinCMS Link Template Functions
 *
 * @package MandarinCMS
 * @subpackage Template
 */
 ...
 *
 * @param int|MCMS_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|MCMS_Post $post      Post ID, MCMS_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $mcms_rewrite;
	if ( $mcms_rewrite->use_trailing_slashes )
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param MCMS_Post $post      The post in question.
	...
	if ( '' != $permalink && !in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ) ) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( $cats ) {
				$cats = mcms_list_sort( $cats, array(
					'term_id' => 'ASC',
				) );
				 ...
				 * @since 3.5.0
				 *
				 * @param MCMS_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (MCMS_Term objects) associated with the post.
				 * @param MCMS_Post  $post The post in question.
				 */
			...
			if ( empty($category) ) {
				...
				if ( $default_category && ! is_mcms_error( $default_category ) ) {
 ...
 * @global MCMS_Rewrite $mcms_rewrite
 *
 * @param int|MCMS_Post $id        Optional. Post ID or post object. Default is the global `$post`.
 ...
 * @return string|MCMS_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $mcms_rewrite->get_extra_permastruct($post->post_type);
 ...
 * @since 1.5.0
 *
 * @param int|MCMS_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $mcms_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the MandarinCMS Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global MCMS_Rewrite $mcms_rewrite
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $mcms_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $mcms_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $mcms_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $mcms_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $mcms_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $mcms_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $mcms_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @param MCMS_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $mcms_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $mcms_rewrite->front . $struct;
		else
			$struct = $mcms_rewrite->root . $struct;
 ...
 * @since 4.4.0
 *
 * @param int|MCMS_Post $post         Optional. Post ID or `MCMS_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the MandarinCMS loop or outside of it. Can be used with
 ...
 *
 * Can be used within the MandarinCMS loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int|MCMS_Post $id           Optional. Post ID or post object. Default is the global `$post`.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', mcms_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @since 2.3.0
 *
 * @param int|MCMS_Comment $comment_id Optional. Comment ID or MCMS_Comment object.
 ...
 * @return null|string|MCMS_Post Post object if successful. Null if global $post is not set. Empty string if no
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		...
		if ( $in_same_term ) {
			$join .= " INNER JOIN $mcmsdb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $mcmsdb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$where .= $mcmsdb->prepare( "AND tt.taxonomy = %s", $taxonomy );

			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) )
				return '';
			$term_array = mcms_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_mcms_error( $term_array ) )
				return '';
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $mcmsdb->term_relationships tr LEFT JOIN $mcmsdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( ',', array_map( 'intval', $excluded_terms ) ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( (array) $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $mcmsdb->prepare( " OR p.post_status = %s", $state );
			} else {
				$where .= $mcmsdb->prepare( " OR (p.post_author = %d AND p.post_status = %s)", $user_id, $state );
			}
		}
		$where .= " )";
	} else {
	 ...
	 * @param MCMS_Post $post           MCMS_Post object.
	 */
	 ...
	 * @param MCMS_Post $post           MCMS_Post object.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $mcmsdb->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms, $taxonomy, $post );
	...
	$query = "SELECT p.ID FROM $mcmsdb->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5( $query );
	$result = mcms_cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result )
	...
	$result = $mcmsdb->get_var( $query );
	if ( null === $result )
		$result = '';

	mcms_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like 'mcms_head'. Do not call this directly in modules
 * or myskin templates.
 *
 * @since 3.0.0
 *
 ...
 */
function adjacent_posts_rel_link_mcms_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param MCMS_Post $post     The adjacent post.
...
function get_pagenum_link($pagenum = 1, $escape = true ) {
	...
	if ( !$mcms_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$mcms_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^' . preg_quote( $mcms_rewrite->index, '|' ) . '|i', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $mcms_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= $mcms_rewrite->index . '/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $mcms_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $mcms_query;

	if ( !$max_page )
		$max_page = $mcms_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $mcms_query->max_num_pages;
 ...
 * @since 4.1.0
 *
 * @global MCMS_Query $mcms_query MandarinCMS Query object.
 *
 * @param array $args {
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['mcms_query']->max_num_pages > 1 ) {
		$args = mcms_parse_args( $args, array(
	...
	if ( 'newest' == get_option('default_comments_page') ) {
		if ( $pagenum != $max_page ) {
			if ( $mcms_rewrite->using_permalinks() )
				$result = user_trailingslashit( trailingslashit($result) . $mcms_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged');
			else
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $mcms_query->max_num_comment_pages;
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $mcms_rewrite->using_permalinks() )
		$defaults['base'] = user_trailingslashit(trailingslashit(get_permalink()) . $mcms_rewrite->comments_pagination_base . '-%#%', 'commentpaged');
...
function get_home_url( $blog_id = null, $path = '', $scheme = null ) {
	...
	if ( ! in_array( $scheme, array( 'http', 'https', 'relative' ) ) ) {
		if ( is_ssl() && ! is_admin() && 'signin.php' !== $pagenow )
			$scheme = 'https';
		else
...
/**
 * Retrieves the URL for the current site where MandarinCMS application files
 * (e.g. components.php or the mcms-admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where MandarinCMS application files
 * (e.g. components.php or the mcms-admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url($blog_id, 'mcms-admin/', $scheme);
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string $path Path relative to the URL to the mcms-roots directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( MCMS_CONTENT_URL );
...
/**
 * Retrieves a URL within the modules or mu-modules directory.
 *
 * Defaults to the modules directory URL if no arguments are supplied.
 *
 * @since 2.6.0
 *
 * @param  string $path   Optional. Extra path appended to the end of the URL, including
 *                        the relative directory if $module is supplied. Default empty.
 * @param  string $module Optional. A full path to a file inside a module or mu-module.
 *                        The URL will be relative to its directory. Default empty.
 *                        Typically this is done by passing `__FILE__` as the argument.
 * @return string Modules URL link with optional paths appended.
 */
function modules_url( $path = '', $module = '' ) {

	$path = mcms_normalize_path( $path );
	$module = mcms_normalize_path( $module );
	$mu_module_dir = mcms_normalize_path( MCMSMU_PLUGIN_DIR );

	if ( !empty($module) && 0 === strpos($module, $mu_module_dir) )
		$url = MCMSMU_PLUGIN_URL;
	else
		$url = MCMS_PLUGIN_URL;
	...
	if ( !empty($module) && is_string($module) ) {
		$folder = dirname(module_basename($module));
		if ( '.' != $folder )
			$url .= '/' . ltrim($folder, '/');
	}
	...
	/**
	 * Filters the URL to the modules directory.
	 *
	 * @since 2.8.0
	 *
	 * @param string $url    The complete URL to the modules directory including scheme and path.
	 * @param string $path   Path relative to the URL to the modules directory. Blank string
	 *                       if no path is specified.
	 * @param string $module The module file path to be relative to. Blank string if no module
	 *                       is specified.
	 */
	return apply_filters( 'modules_url', $url, $path, $module );
}
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url('mcms-admin/network/', $scheme);
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url('mcms-admin/user/', $scheme);
 ...
 *                      been published yet.
 */
function mcms_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use mcms_get_canonical_url().
 */
 ...
 *
 * This function exists to provide a shortlink tag that all myskins and modules can target.
 * A module must hook in to provide the actual shortlinks. Default shortlink support is
 * limited to providing ?p= style links for posts. Modules can short-circuit this function
 ...
 * @param bool   $allow_slugs Optional. Whether to allow post slugs in the shortlink. It is up to the module how
 *                            and whether to honor this. Default true.
 * @return string A shortlink or an empty string if no shortlink exists for the requested resource or if shortlinks
 *                are not enabled.
 */
function mcms_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'mcms_head'} action.
 *
 * @since 3.0.0
 */
function mcms_shortlink_mcms_head() {
	$shortlink = mcms_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the {@see 'mcms'} action.
 *
 * @since 3.0.0
 */
function mcms_shortlink_header() {
	if ( headers_sent() )
		return;
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = mcms_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof MCMS_User ) {
		// User Object
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof MCMS_Post ) {
		// Post Object
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof MCMS_Comment ) {
		/**
		...
		if ( ( ! $user || is_mcms_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
...
/**
 * Retrieves the URL of a file in the myskin.
 *
 * Searches in the stylesheet directory before the template directory so myskins
 * which inherit from a parent myskin can just override one file.
 *
 * @since 4.7.0
 *
 ...
 */
function get_myskin_file_uri( $file = '' ) {
	$file = ltrim( $file, '/' );
	...
	/**
	 * Filters the URL to a file in the myskin.
	 *
	 * @since 4.7.0
	 *
	 ...
	 */
	return apply_filters( 'myskin_file_uri', $url, $file );
}

/**
 * Retrieves the URL of a file in the parent myskin.
 *
 * @since 4.7.0
 *
 ...
 * @return string The URL of the file.
 */
function get_parent_myskin_file_uri( $file = '' ) {
	$file = ltrim( $file, '/' );
	...
	/**
	 * Filters the URL to a file in the parent myskin.
	 *
	 * @since 4.7.0
	 *
	 * @param string $url  The file URL.
	 * @param string $file The requested file to search for.
	 */
	return apply_filters( 'parent_myskin_file_uri', $url, $file );
}

/**
 * Retrieves the path of a file in the myskin.
 *
 * Searches in the stylesheet directory before the template directory so myskins
 * which inherit from a parent myskin can just override one file.
 *
 ...
 */
function get_myskin_file_path( $file = '' ) {
	$file = ltrim( $file, '/' );
	...
	/**
	 * Filters the path to a file in the myskin.
	 *
	 * @since 4.7.0
	 *
	 ...
	 * @param string $file The requested file to search for.
	 */
	return apply_filters( 'myskin_file_path', $path, $file );
}

/**
 * Retrieves the path of a file in the parent myskin.
 *
 * @since 4.7.0
 *
 ...
 * @return string The path of the file.
 */
function get_parent_myskin_file_path( $file = '' ) {
	$file = ltrim( $file, '/' );
	...
	/**
	 * Filters the path to a file in the parent myskin.
	 *
	 * @since 4.7.0
	 *
	 * @param string $path The file path.
	 * @param string $file The requested file to search for.
	 */
	return apply_filters( 'parent_myskin_file_path', $path, $file );
}
