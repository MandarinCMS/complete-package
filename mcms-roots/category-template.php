 ...
 * @return string|MCMS_Error A list of category parents on success, MCMS_Error on failure.
 */
 ...
 * @return array Array of MCMS_Term objects, one for each category assigned to the post.
 */
function get_the_category( $id = false ) {
	$categories = get_the_terms( $id, 'category' );
	if ( ! $categories || is_mcms_error( $categories ) )
		$categories = array();
 ...
 * @return string|MCMS_Error Category name on success, MCMS_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	...
	$rel = ( is_object( $mcms_rewrite ) && $mcms_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of MandarinCMS, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the MandarinCMS Loop.
 ...
 *     Optional. Array or string of arguments to generate a categories drop-down element. See MCMS_Term_Query::__construct()
 ...
 */
function mcms_dropdown_categories( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'orderby'           => 'id',
	...
	if ( empty( $categories ) && ! $r['hide_if_empty'] && ! empty( $r['show_option_none'] ) ) {
		 ...
		 * @param MCMS_Term|null $category The category object, or null if there's no corresponding category.
		 */
	 ...
	 */
	$output = apply_filters( 'mcms_dropdown_cats', $output, $r );
 ...
 */
function mcms_list_categories( $args = '' ) {
	$defaults = array(
		'child_of'            => 0,
		'current_category'    => 0,
		'depth'               => 0,
	...
	if ( true == $r['hierarchical'] ) {
		...
		if ( $r['exclude_tree'] ) {
			$exclude_tree = array_merge( $exclude_tree, mcms_parse_id_list( $r['exclude_tree'] ) );
		}

		if ( $r['exclude'] ) {
			$exclude_tree = array_merge( $exclude_tree, mcms_parse_id_list( $r['exclude'] ) );
		}
	 ...
	 */
	$html = apply_filters( 'mcms_list_categories', $output, $args );
...
function mcms_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_mcms_error( $tags ) )
		return;
	...
	$return = mcms_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
	 ...
	 */
	$return = apply_filters( 'mcms_tag_cloud', $return, $args );
 ...
 *                                                via {@see 'mcms_generate_tag_cloud'}. Default 1|true.
 ...
 */
function mcms_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_mcms_object_name_sort_cb' );
			} else {
				uasort( $tags, '_mcms_object_count_sort_cb' );
			}
	 ...
	 * myskins set up the Tag Cloud to display all tags with the same font size (setting
	 ...
	 */
	$tags_data = apply_filters( 'mcms_generate_tag_cloud_data', $tags_data );
	...
	switch ( $args['format'] ) {
		...
		case 'list' :
			 ...
			 */
			$return = "<ul class='mcms-tag-cloud' role='list'>\n\t<li>";
			$return .= join( "</li>\n\t<li>", $a );
			$return .= "</li>\n</ul>\n";
			break;
		default :
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in mcms_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
		 * @see mcms_generate_tag_cloud()
		 *
		 * @param array|string $return String containing the generated HTML tag cloud output
		 *                             or an array of tag links if the 'format' argument
		 *                             equals 'array'.
		 * @param array        $tags   An array of terms used in the tag cloud.
		 * @param array        $args   An array of mcms_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'mcms_generate_tag_cloud', $return, $tags, $args );
	}
 ...
 */
function _mcms_object_name_sort_cb( $a, $b ) {
 ...
 */
function _mcms_object_count_sort_cb( $a, $b ) {
 ...
 * @return array|false|MCMS_Error Array of tag objects on success, false on failure.
 */
 ...
 * @return string|false|MCMS_Error A list of tags on success, false if there are no terms, MCMS_Error on failure.
 */
...
function term_description( $term = 0, $deprecated = null ) {
	...
	return is_mcms_error( $description ) ? '' : $description;
}
 ...
 * @return array|false|MCMS_Error Array of MCMS_Term objects on success, false if there are no terms
 *                              or the post does not exist, MCMS_Error on failure.
 */
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = mcms_get_object_terms( $post->ID, $taxonomy );
		if ( ! is_mcms_error( $terms ) ) {
			$term_ids = mcms_list_pluck( $terms, 'term_id' );
			mcms_cache_add( $post->ID, $term_ids, $taxonomy . '_relationships' );
		}
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param array|MCMS_Error $terms    List of attached terms, or MCMS_Error on failure.
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return string|false|MCMS_Error A list of terms on success, false if there are no terms, MCMS_Error on failure.
 */
 ...
 * }
 * @return string|MCMS_Error A list of term parents on success, MCMS_Error or empty string on failure.
 */
function get_term_parents_list( $term_id, $taxonomy, $args = array() ) {
	...
	foreach ( array( 'link', 'inclusive' ) as $bool ) {
		$args[ $bool ] = mcms_validate_boolean( $args[ $bool ] );
	}
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return false|void False on MandarinCMS error.
 */
function the_terms( $id, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_mcms_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of MandarinCMS, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the MandarinCMS Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 2.6.0
