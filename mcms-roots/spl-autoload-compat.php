 ...
 *
 * See https://core.trac.mandarincms.com/ticket/41134
 *
 * @package PHP
 * @access private
 */
...
if ( ! function_exists( 'spl_autoload_register' ) ) {
	...
	function __autoload( $classname ) {
		global $_mcms_spl_autoloaders;
		foreach ( $_mcms_spl_autoloaders as $autoloader ) {
	...
	function spl_autoload_register( $autoload_function, $throw = true, $prepend = false ) {
		...
		if ( in_array( $autoload_function, $_mcms_spl_autoloaders ) ) {
			return;
		}

		if ( $prepend ) {
			array_unshift( $_mcms_spl_autoloaders, $autoload_function );
		} else {
			$_mcms_spl_autoloaders[] = $autoload_function;
		}
	}
	...
	function spl_autoload_functions() {
		return $GLOBALS['_mcms_spl_autoloaders'];
	}
}
