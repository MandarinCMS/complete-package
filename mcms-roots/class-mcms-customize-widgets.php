<?php
/**
 * MandarinCMS Customize Widgets classes
 *
 * @package MandarinCMS
 ...
 */
final class MCMS_Customize_Widgets {
	 ...
	 * @since 3.9.0
	 *
	 * @param MCMS_Customize_Manager $manager Customize manager bootstrap instance.
	 */
	public function __construct( $manager ) {
		$this->manager = $manager;

		// See https://github.com/xmcms/mcms-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L420-L449
		...
		add_action( 'mcms_loaded',                               array( $this, 'override_sidebars_widgets_for_myskin_switch' ) );
	 ...
	 *
	 * If the myskin does not support the customize-selective-refresh-widgets feature,
	 ...
	 *
	 * @return array Mapping of id_base to support. If myskin doesn't support
	 *               selective refresh, an empty array is returned.
	 */
	public function get_selective_refreshable_widgets() {
		global $mcms_widget_factory;
		if ( ! current_myskin_supports( 'customize-selective-refresh-widgets' ) ) {
			return array();
		}
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			$this->selective_refreshable_widgets = array();
			foreach ( $mcms_widget_factory->widgets as $mcms_widget ) {
				$this->selective_refreshable_widgets[ $mcms_widget->id_base ] = ! empty( $mcms_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( mcms_unslash( $_REQUEST['widget-id'] ) );
		}
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the MCMS_Customize_Setting constructor.
	...
	protected function get_post_value( $name, $default = null ) {
		...
		return mcms_unslash( $_POST[ $name ] );
	}

	/**
	 * Override sidebars_widgets for myskin switch.
	 *
	 * When switching a myskin via the Customizer, supply any previously-configured
	 * sidebars_widgets from the target myskin as the initial sidebars_widgets
	 * setting. Also store the old myskin's existing settings so that they can
	 * be passed along for storing in the sidebars_widgets myskin_mod when the
	 * myskin gets switched.
	 *
	 * @since 3.9.0
	 *
	 ...
	 */
	public function override_sidebars_widgets_for_myskin_switch() {
		global $sidebars_widgets;

		if ( $this->manager->doing_ajax() || $this->manager->is_myskin_active() ) {
			return;
		}

		$this->old_sidebars_widgets = mcms_get_sidebars_widgets();
		...
		add_filter( 'option_sidebars_widgets', array( $this, 'filter_option_sidebars_widgets_for_myskin_switch' ), 1 );
		// reset global cache var used by mcms_get_sidebars_widgets()
		unset( $GLOBALS['_mcms_sidebars_widgets'] );
	}

	/**
	 * Filters old_sidebars_widgets_data Customizer setting.
	 *
	 * When switching myskins, filter the Customizer setting old_sidebars_widgets_data
	 * to supply initial $sidebars_widgets before they were overridden by retrieve_widgets().
	 * The value for old_sidebars_widgets_data gets set in the old myskin's sidebars_widgets
	 * myskin_mod.
	 *
	 * @since 3.9.0
	 *
	 * @see MCMS_Customize_Widgets::handle_myskin_switch()
	 *
	...
	/**
	 * Filters sidebars_widgets option for myskin switch.
	 *
	 * When switching myskins, the retrieve_widgets() function is run when the Customizer initializes,
	 ...
	 * @return array
	 */
	public function filter_option_sidebars_widgets_for_myskin_switch( $sidebars_widgets ) {
	 ...
	 *
	 * Note: these actions are also fired in mcms_ajax_update_widget().
	 *
	 * @since 3.9.0
	 */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after MandarinCMS is loaded
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'mcms', array( $this, 'customize_register' ) );
		}
	}
	...
	public function customize_register() {
		global $mcms_registered_widgets, $mcms_registered_widget_controls, $mcms_registered_sidebars;

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'mcms_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $mcms_registered_sidebars ), array() ),
			mcms_get_sidebars_widgets()
		);
		 ...
		 * via a module (like Widget Visibility).
		 */
		foreach ( array_keys( $mcms_registered_widgets ) as $widget_id ) {
		...
		/*
		 * Add a setting which will be supplied for the myskin's sidebars_widgets
		 * myskin_mod when the myskin is switched.
		 */
		...
		$this->manager->add_panel( 'widgets', array(
			'type'            => 'widgets',
			'title'           => __( 'Widgets' ),
			'description'     => __( 'Widgets are independent sections of content that can be placed into widgetized areas provided by your myskin (commonly called sidebars).' ),
			'priority'        => 110,
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'mcms_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title' => $mcms_registered_sidebars[ $sidebar_id ]['name'],
						'description' => $mcms_registered_sidebars[ $sidebar_id ]['description'],
						'priority' => array_search( $sidebar_id, array_keys( $mcms_registered_sidebars ) ),
						'panel' => 'widgets',
					...
					$section = new MCMS_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control = new MCMS_Widget_Area_Customize_Control( $this->manager, $setting_id, array(
						'section'    => $section_id,
						'sidebar_id' => $sidebar_id,
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				// Skip widgets that may have gone away due to a module being deactivated.
				if ( ! $is_active_sidebar || ! isset( $mcms_registered_widgets[$widget_id] ) ) {
					continue;
				}

				$registered_widget = $mcms_registered_widgets[$widget_id];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $mcms_registered_widget_controls[$widget_id]['id_base'];

				$control = new MCMS_Widget_Form_Customize_Control( $this->manager, $setting_id, array(
					...
					'priority'       => $i,
					'width'          => $mcms_registered_widget_controls[$widget_id]['width'],
					'height'         => $mcms_registered_widget_controls[$widget_id]['height'],
	 ...
	 * @since 4.4.0
	 *
	 * @see MCMS_Customize_Panel::$active_callback
	 *
	 * @global array $mcms_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $mcms_registered_sidebars );
	}
	 ...
	 * @return MCMS_Error|array Array containing a widget's id_base and number components,
	 *                        or a MCMS_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new MCMS_Error( 'widget_setting_invalid_id' );
		}
	 ...
	 * allow custom styles from modules.
	 *
	 * @since 3.9.0
	 */
	 ...
	 * allow custom scripts from modules.
	 *
	 * @since 3.9.0
	 */
	...
	public function enqueue_scripts() {
		global $mcms_scripts, $mcms_registered_sidebars, $mcms_registered_widgets;

		mcms_enqueue_style( 'customize-widgets' );
		mcms_enqueue_script( 'customize-widgets' );
		 ...
		 * since modules need templates to be in the DOM.
		 */
		...
		$some_non_rendered_areas_messages[1] = html_entity_decode(
			__( 'Your myskin has 1 other widget area, but this particular page doesn&#8217;t display it.' ),
			ENT_QUOTES,
			get_bloginfo( 'charset' )
		);
		$registered_sidebar_count = count( $mcms_registered_sidebars );
		for ( $non_rendered_count = 2; $non_rendered_count < $registered_sidebar_count; $non_rendered_count++ ) {
			$some_non_rendered_areas_messages[ $non_rendered_count ] = html_entity_decode( sprintf(
				...
				_n(
					'Your myskin has %s other widget area, but this particular page doesn&#8217;t display it.',
					'Your myskin has %s other widget areas, but this particular page doesn&#8217;t display them.',
					$non_rendered_count
				),
		...
		if ( 1 === $registered_sidebar_count ) {
			$no_areas_shown_message = html_entity_decode( sprintf(
				__( 'Your myskin has 1 widget area, but this particular page doesn&#8217;t display it.' )
			), ENT_QUOTES, get_bloginfo( 'charset' ) );
		} else {
			$no_areas_shown_message = html_entity_decode( sprintf(
				/* translators: %s: the total number of widget areas registered */
				_n(
					'Your myskin has %s widget area, but this particular page doesn&#8217;t display it.',
					'Your myskin has %s widget areas, but this particular page doesn&#8217;t display them.',
		...
		$settings = array(
			'registeredSidebars'   => array_values( $mcms_registered_sidebars ),
			'registeredWidgets'    => $mcms_registered_widgets,
		...
		$mcms_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _mcmsCustomizeWidgetsSettings = %s;', mcms_json_encode( $settings ) )
		);
	}
	...
	public function get_setting_args( $id, $overrides = array() ) {
		...
		if ( preg_match( $this->setting_id_patterns['sidebar_widgets'], $id, $matches ) ) {
			...
			$args['transport'] = current_myskin_supports( 'customize-selective-refresh-widgets' ) ? 'postMessage' : 'refresh';
	...
	public function get_available_widgets() {
		...
		global $mcms_registered_widgets, $mcms_registered_widget_controls;
		require_once BASED_TREE_URI . '/mcms-admin/includes/widgets.php'; // for next_widget_id_number()

		$sort = $mcms_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $mcms_registered_widget_controls[$widget['id']]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $mcms_registered_widget_controls[$widget['id']]['id_base'];
				...
				if ( $sidebar && 'mcms_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = mcms_list_widget_controls_dynamic_sidebar( array( 0 => $args, 1 => $widget['params'][0] ) );
			...
			$available_widget = array_merge( $available_widget, array(
				...
				'width'        => $mcms_registered_widget_controls[$widget['id']]['width'],
				'height'       => $mcms_registered_widget_controls[$widget['id']]['height'],
	...
	public function get_widget_control( $args ) {
		...
		ob_start();
		call_user_func_array( 'mcms_widget_control', $args );
	...
	public function customize_preview_init() {
		add_action( 'mcms_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'mcms_print_styles',    array( $this, 'print_preview_css' ), 1 );
		add_action( 'mcms_footer',          array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = mcms_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because mcms_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * mcms_convert_widget_settings()) and can set global variable `$_mcms_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		mcms_enqueue_script( 'customize-preview-widgets' );
	}

	/**
	 * Inserts default style for highlighted widget at early point so myskin
	 ...
	 * and at the very end of the mcms_footer,
	 *
	 * @since 3.9.0
     *
	 * @global array $mcms_registered_sidebars
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _mcmsWidgetCustomizerPreviewSettings = <?php echo mcms_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return mcms_hash( $serialized_instance );
	}
	 ...
	 *
	 * One example where this might happen is when a module orphans a widget
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $mcms_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param  string $widget_id Widget ID.
	 * @return MCMS_Error|array Array containing the updated widget information.
	 *                        A MCMS_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $mcms_registered_widget_updates, $mcms_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new MCMS_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new MCMS_Error( 'widget_setting_unsanitized' );
			}
			...
			if ( ! is_null( $parsed_id['number'] ) ) {
				...
				$_REQUEST[$key] = $_POST[$key] = mcms_slash( $value );
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new MCMS_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new MCMS_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from MCMS_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $mcms_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from mcms_ajax_save_widget().
	 *
	 * @since 3.9.0
	 *
	 * @see mcms_ajax_save_widget()
	 */
	public function mcms_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			mcms_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			mcms_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_mcms_error( $updated_widget ) ) {
			mcms_send_json_error( $updated_widget->get_error_code() );
		}

		$form = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		mcms_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'mcms_kses_allowed_html', array( $this, 'filter_mcms_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see MCMS_Customize_Nav_Menus_Partial_Refresh::filter_mcms_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mcms_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_mcms_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through mcms_kses() when printed.
	 *
	 * @since 4.5.0
	 *
	 ...
	 */
	public function filter_mcms_kses_allowed_data_attributes( $allowed_html ) {
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in mcms-roots/option.php */
