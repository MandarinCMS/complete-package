<?php
/**
 * Session API: MCMS_Session_Tokens class
 *
 * @package MandarinCMS
 * @subpackage Session
 * @since 4.7.0
 ...
 * @since 4.0.0
 */
abstract class MCMS_Session_Tokens {
	 ...
	 *
	 * This method contains a {@see 'session_token_manager'} filter, allowing a module to swap out
	 * the session manager for a subclass of `MCMS_Session_Tokens`.
	 *
	 * @since 4.0.0
	 * @static
	 *
	 * @param int $user_id User whose session to manage.
	 * @return MCMS_User_Meta_Session_Tokens MCMS_User_Meta_Session_Tokens class instance by default.
	 */
	final public static function get_instance( $user_id ) {
		 ...
		 *                        Default 'MCMS_User_Meta_Session_Tokens'.
		 */
		$manager = apply_filters( 'session_token_manager', 'MCMS_User_Meta_Session_Tokens' );
	...
	final public function create( $expiration ) {
		...
		if ( ! empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
			$session['ua'] = mcms_unslash( $_SERVER['HTTP_USER_AGENT'] );
		}

		// Timestamp
		$session['login'] = time();

		$token = mcms_generate_password( 43, false, false );
	...
	final public static function destroy_all_for_all_users() {
		/** This filter is documented in mcms-roots/class-mcms-session-tokens.php */
		$manager = apply_filters( 'session_token_manager', 'MCMS_User_Meta_Session_Tokens' );
