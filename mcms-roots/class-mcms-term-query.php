...
/**
 * Taxonomy API: MCMS_Term_Query class.
 *
 * @package MandarinCMS
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see MCMS_Term_Query::__construct() for accepted arguments.
 */
class MCMS_Term_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query MCMS_Term_Query arguments. See MCMS_Term_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$query = mcms_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param MCMS_Term_Query $this Current instance of MCMS_Term_Query.
		 */
	...
	public function get_terms() {
		...
		if ( ! empty( $include ) ) {
			$exclude = '';
			$exclude_tree = '';
			$inclusions = implode( ',', mcms_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree = mcms_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( mcms_parse_id_list( $exclude ), $exclusions );
		}
		...
		if ( ! empty( $args['term_taxonomy_id'] ) ) {
			if ( is_array( $args['term_taxonomy_id'] ) ) {
				...
				$this->sql_clauses['where']['term_taxonomy_id'] = $mcmsdb->prepare( "tt.term_taxonomy_id = %d", $args['term_taxonomy_id'] );
			}
		}

		if ( ! empty( $args['name__like'] ) ) {
			$this->sql_clauses['where']['name__like'] = $mcmsdb->prepare( "t.name LIKE %s", '%' . $mcmsdb->esc_like( $args['name__like'] ) . '%' );
		}

		if ( ! empty( $args['description__like'] ) ) {
			$this->sql_clauses['where']['description__like'] = $mcmsdb->prepare( "tt.description LIKE %s", '%' . $mcmsdb->esc_like( $args['description__like'] ) . '%' );
		}
		...
		$join .= " INNER JOIN $mcmsdb->term_taxonomy AS tt ON t.term_id = tt.term_id";

		if ( ! empty( $this->query_vars['object_ids'] ) ) {
			$join .= " INNER JOIN {$mcmsdb->term_relationships} AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id";
		}
		...
		$this->sql_clauses['from']    = "FROM $mcmsdb->terms AS t $join";
		...
		$key = md5( serialize( mcms_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) ) ) . serialize( $taxonomies ) . $this->request );
		...
		if ( 'count' == $_fields ) {
			$count = $mcmsdb->get_var( $this->request );
			mcms_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}

		$terms = $mcmsdb->get_results( $this->request );
		...
		if ( empty( $terms ) ) {
			mcms_cache_add( $cache_key, array(), 'terms', DAY_IN_SECONDS );
			return array();
		}
		...
		mcms_cache_add( $cache_key, $terms, 'terms', DAY_IN_SECONDS );
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', mcms_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to MCMS_Term_Query.
	...
	protected function get_search_sql( $string ) {
		...
		return $mcmsdb->prepare( '((t.name LIKE %s) OR (t.slug LIKE %s))', $like, $like );
	}
}
