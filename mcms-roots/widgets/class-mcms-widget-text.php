<?php
/**
 * Widget API: MCMS_Widget_Text class
 *
 * @package MandarinCMS
 * @subpackage Widgets
 * @since 4.4.0
 ...
 * @see MCMS_Widget
 */
class MCMS_Widget_Text extends MCMS_Widget {
	...
	public function _register_one( $number = -1 ) {
		...
		mcms_add_inline_script( 'text-widgets', sprintf( 'mcms.textWidgets.idBases.push( %s );', mcms_json_encode( $this->id_base ) ) );

		if ( $this->is_preview() ) {
			add_action( 'mcms_enqueue_scripts', array( $this, 'enqueue_preview_scripts' ) );
		}

		// Note that the widgets component in the customizer will also do the 'admin_print_scripts-widgets.php' action in MCMS_Customize_Widgets::print_scripts().
		add_action( 'admin_print_scripts-widgets.php', array( $this, 'enqueue_admin_scripts' ) );

		// Note that the widgets component in the customizer will also do the 'admin_footer-widgets.php' action in MCMS_Customize_Widgets::print_footer_scripts().
		add_action( 'admin_footer-widgets.php', array( 'MCMS_Widget_Text', 'render_control_template_scripts' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in mcms-roots/widgets/class-mcms-widget-pages.php */
		...
		/*
		 * Suspend legacy module-supplied do_shortcode() for 'widget_text' filter for the visual Text widget to prevent
		 * shortcodes being processed twice. Now do_shortcode() is added to the 'widget_text_content' filter in core itself
		 * and it applies after mcmsautop() to prevent corrupting HTML output added by the shortcode. When do_shortcode() is
		 ...
		 * @since 4.8.1 The `$this` param may now be a `MCMS_Widget_Custom_HTML` object in addition to a `MCMS_Widget_Text` object.
		 *
		 * @param string                               $text     The widget content.
		 * @param array                                $instance Array of settings for the current widget.
		 * @param MCMS_Widget_Text|MCMS_Widget_Custom_HTML $this     Current Text widget instance.
		 */
		...
		if ( $is_visual_text_widget ) {
			 ...
			 *
			 * By default a subset of the_content filters are applied, including mcmsautop and mcmstexturize.
			 *
			 * @since 4.8.0
			 *
			 * @param string         $text     The widget content.
			 * @param array          $instance Array of settings for the current widget.
			 * @param MCMS_Widget_Text $this     Current Text widget instance.
			 */
			 ...
			 * in core by adding do_shortcode() to the 'widget_text_content' filter to apply after mcmsautop().
			 * Since the legacy Text widget runs mcmsautop() after 'widget_text' filters are applied, the widget in
			 * legacy mode here manually applies do_shortcode() on the content unless the default
			 * core filter for 'widget_text_content' has been removed, or if do_shortcode() has already
			 * been applied via a module adding do_shortcode() to 'widget_text' filters.
			 */
		...
		// Undo suspension of legacy module-supplied shortcode handling.
		if ( $should_suspend_legacy_shortcode_support ) {
	 ...
	 * @since 4.9.0
	 *
	 * @see MCMS_Widget_Media_Video::inject_video_max_width_style()
	...
	public function update( $new_instance, $old_instance ) {
		$new_instance = mcms_parse_args( $new_instance, array(
			'title' => '',
			'text' => '',
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['text'] = mcms_kses_post( $new_instance['text'] );
		}
	...
	public function enqueue_preview_scripts() {
		...
		mcms_playlist_scripts( 'audio' );
		mcms_playlist_scripts( 'video' );
	}
	...
	public function enqueue_admin_scripts() {
		mcms_enqueue_editor();
		mcms_enqueue_media();
		mcms_enqueue_script( 'text-widgets' );
		mcms_add_inline_script( 'text-widgets', 'mcms.textWidgets.init();', 'after' );
	}
	 ...
	 * @see MCMS_Widget_Visual_Text::render_control_template_scripts()
	...
	public function form( $instance ) {
		...
		<?php if ( ! $this->is_legacy_instance( $instance ) ) : ?>
			...
			/** This filter is documented in mcms-roots/class-mcms-editor.php */
	...
	public static function render_control_template_scripts() {
		...
		<script type="text/html" id="tmpl-widget-text-control-fields">
			...
			<?php if ( ! in_array( 'text_widget_custom_html', $dismissed_pointers, true ) ) : ?>
				<div hidden class="mcms-pointer custom-html-widget-pointer mcms-pointer-top">
					<div class="mcms-pointer-content">
						...
						<?php endif; ?>
						<div class="mcms-pointer-buttons">
							<a class="close" href="#"><?php _e( 'Dismiss' ); ?></a>
						</div>
					</div>
					<div class="mcms-pointer-arrow">
						<div class="mcms-pointer-arrow-inner"></div>
					</div>
				</div>
			<?php endif; ?>

			<?php if ( ! in_array( 'text_widget_paste_html', $dismissed_pointers, true ) ) : ?>
				<div hidden class="mcms-pointer paste-html-pointer mcms-pointer-top">
					<div class="mcms-pointer-content">
			...
			<p>
				...
				<textarea id="{{ elementIdPrefix }}text" class="widefat text mcms-editor-area" style="height: 200px" rows="16" cols="20"></textarea>
			</p>
		</script>
		<?php
	}
