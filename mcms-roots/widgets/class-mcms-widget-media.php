<?php
/**
 * Widget API: MCMS_Media_Widget class
 *
 * @package MandarinCMS
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see MCMS_Widget
 */
abstract class MCMS_Widget_Media extends MCMS_Widget {
	 ...
	 * @param array  $widget_options  Optional. Widget options. See mcms_register_sidebar_widget() for
	 *                                information on accepted arguments. Default empty array.
	 * @param array  $control_options Optional. Widget control options. See mcms_register_widget_control()
	 *                                for information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = mcms_parse_args( $widget_options, array(
			'description' => __( 'A media item.' ),
			'customize_selective_refresh' => true,
			'mime_type' => '',
		) );

		$control_opts = mcms_parse_args( $control_options, array() );

		$l10n_defaults = array(
			'no_media_selected' => __( 'No media selected' ),
			'add_media' => _x( 'Upload&nbsp;Attachment', 'label for button in the media widget' ),
	...
	public function get_instance_schema() {
		 ...
		 * @param MCMS_Widget_Media $this   Widget object.
		 */
	 ...
	 * @since 4.8.0
	 *
	 * @param int|MCMS_Post $attachment Attachment post ID or object.
	...
	public function is_attachment_with_mime_type( $attachment, $mime_type ) {
		...
		return mcms_attachment_is( $mime_type, $attachment );
	}
	 ...
	 * @since 4.8.0
	 *
	 * @see MCMS_Widget::widget()
	 *
	 * @param array $args     Display arguments including before_title, after_title, before_widget, and after_widget.
	 * @param array $instance Saved setting from the database.
	 */
	public function widget( $args, $instance ) {
		$instance = mcms_parse_args( $instance, mcms_list_pluck( $this->get_instance_schema(), 'default' ) );
	...
	public function update( $new_instance, $instance ) {
		...
		foreach ( $schema as $field => $field_schema ) {
			...
			if ( is_mcms_error( $value ) ) {
				continue; // Handle case when rest_sanitize_value_from_schema() ever returns MCMS_Error as its phpdoc @return tag indicates.
			}
	 ...
	 * @since 4.8.0
	 *
	 * @see \MCMS_Widget_Media::render_control_template_scripts() Where the JS template is located.
	 * @param array $instance Current settings.
	 * @return void
	 */
	final public function form( $instance ) {
		$instance_schema = $this->get_instance_schema();
		$instance = mcms_array_slice_assoc(
			mcms_parse_args( (array) $instance, mcms_list_pluck( $instance_schema, 'default' ) ),
		...
		foreach ( $instance as $name => $value ) : ?>
			<input
				...
				id="<?php echo esc_attr( $this->get_field_id( $name ) ); // Needed specifically by mcmsWidgets.appendTitle(). ?>"
	...
	public function enqueue_admin_scripts() {
		mcms_enqueue_media();
		mcms_enqueue_script( 'media-widgets' );
	}
