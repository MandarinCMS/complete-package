<?php
/**
 * Widget API: MCMS_Widget_Media_Image class
 *
 * @package MandarinCMS
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see MCMS_Widget
 */
class MCMS_Widget_Media_Image extends MCMS_Widget_Media {
	...
	public function render_media( $instance ) {
		...
		$instance = mcms_parse_args( $instance, array(
			'size' => 'thumbnail',
		) );
		...
		if ( $attachment ) {
			...
			$image_attributes = array(
				'class' => sprintf( 'image mcms-image-%d %s', $attachment->ID, $instance['image_classes'] ),
			...
			$image = mcms_get_attachment_image( $attachment->ID, $size, false, $image_attributes );

			$caption_size = _mcms_get_image_size_from_meta( $instance['size'], mcms_get_attachment_metadata( $attachment->ID ) );
		...
		if ( 'file' === $instance['link_type'] ) {
			$url = $attachment ? mcms_get_attachment_url( $attachment->ID ) : $instance['url'];
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-mcms-media-widget-image-fields">
		...
		<script type="text/html" id="tmpl-mcms-media-widget-image-preview">
