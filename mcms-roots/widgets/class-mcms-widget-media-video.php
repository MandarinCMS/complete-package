<?php
/**
 * Widget API: MCMS_Widget_Media_Video class
 *
 * @package MandarinCMS
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see MCMS_Widget
 */
class MCMS_Widget_Media_Video extends MCMS_Widget_Media {
	...
	public function __construct() {
		...
		$this->l10n = array_merge( $this->l10n, array(
			...
			'unsupported_file_type' => sprintf( __( 'Sorry, we can&#8217;t load the video at the supplied URL. Please check that the URL is for a supported video file (%s) or stream (e.g. YouTube and Vimeo).' ), '<code>.' . implode( '</code>, <code>.', mcms_get_video_extensions() ) . '</code>' ),
		) );
	}
	...
	public function get_instance_schema() {
		...
		foreach ( mcms_get_video_extensions() as $video_extension ) {
	...
	public function render_media( $instance ) {
		...
		$src = $instance['url'];
		if ( $attachment ) {
			$src = mcms_get_attachment_url( $attachment->ID );
		}
		...
		if ( $attachment || preg_match( $youtube_pattern, $src ) || preg_match( $vimeo_pattern, $src ) ) {
			add_filter( 'mcms_video_shortcode', array( $this, 'inject_video_max_width_style' ) );

			echo mcms_video_shortcode(
				array_merge(
					$instance,
					compact( 'src' )
				),
			...
			remove_filter( 'mcms_video_shortcode', array( $this, 'inject_video_max_width_style' ) );
		} else {
			echo $this->inject_video_max_width_style( mcms_oembed_get( $src ) );
		}
	}
	...
	public function enqueue_preview_scripts() {
		...
		if ( 'mediaelement' === apply_filters( 'mcms_video_shortcode_library', 'mediaelement' ) ) {
	...
	public function enqueue_admin_scripts() {
		...
		foreach ( $this->get_instance_schema() as $field => $field_schema ) {
			$exported_schema[ $field ] = mcms_array_slice_assoc( $field_schema, array( 'type', 'default', 'enum', 'minimum', 'format', 'media_prop', 'should_preview_update' ) );
		}
		...
		mcms_add_inline_script(
			...
			sprintf(
				'
					...
					mcms.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n = _.extend( {}, mcms.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n, %3$s );
				',
				mcms_json_encode( $this->id_base ),
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-mcms-media-widget-video-preview">
