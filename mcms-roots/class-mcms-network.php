<?php
/**
 * Network API: MCMS_Network class
 *
 * @package MandarinCMS
 ...
 *
 * This class is most useful in MandarinCMS multi-network installations where the
...
class MCMS_Network {
	 ...
	 * @return MCMS_Network|bool The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = mcms_cache_get( $network_id, 'networks' );

		if ( ! $_network ) {
			$_network = $mcmsdb->get_row( $mcmsdb->prepare( "SELECT * FROM {$mcmsdb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_mcms_error( $_network ) ) {
				return false;
			}

			mcms_cache_add( $network_id, $_network, 'networks' );
		}

		return new MCMS_Network( $_network );
	}

	/**
	 * Create a new MCMS_Network object.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @param MCMS_Network|object $network A network object.
	 */
	...
	private function get_main_site_id() {
		 ...
		 * @param MCMS_Network $network      The network object for which the main site was detected.
		 */
		...
		if ( $site->domain === $this->domain && $site->path === $this->path ) {
			...
			$main_site_id = mcms_cache_get( $cache_key, 'site-options' );
			if ( false === $main_site_id ) {
				...
				mcms_cache_add( $cache_key, $main_site_id, 'site-options' );
			}
		}
	 ...
	 * @return MCMS_Network|bool Network object if successful. False when no network is found.
	 */
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		...
		if ( mcms_using_ext_object_cache() ) {
			$using_paths = mcms_cache_get( 'networks_have_paths', 'site-options' );
			if ( false === $using_paths ) {
				...
				mcms_cache_add( 'networks_have_paths', $using_paths, 'site-options'  );
			}
		}
		 ...
		 * an object from mcms_get_network().
		 *
		 * @since 3.9.0
		 *
