<?php
/**
 * Deprecated functions from MandarinCMS MU and the multisite feature. You shouldn't
 ...
 * Dashboard blog functionality was removed in MandarinCMS 3.1, replaced by the user admin.
 *
 * @since MU (3.0.0)
 * @deprecated 3.1.0 Use get_site()
 * @see get_site()
 *
 * @return MCMS_Site Current site object.
 */
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use mcms_generate_password()
 * @see mcms_generate_password()
 *
 * @param int $len Optional. The length of password to generate. Default 8.
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mcms_generate_password()' );
	return mcms_generate_password( $len );
}

/**
 * Determine if user is a site admin.
 *
 * Modules should use is_multisite() instead of checking if this function exists
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use mcms_die()
 * @see mcms_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mcms_die()' );
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use mcms_get_sites()
 * @see mcms_get_sites()
 *
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mcms_get_sites()' );

	global $mcmsdb;
	$blogs = $mcmsdb->get_results( $mcmsdb->prepare( "SELECT blog_id, domain, path FROM $mcmsdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", get_current_network_id() ), ARRAY_A );

	$blog_list = array();
	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $mcmsdb->get_var( "SELECT COUNT(ID) FROM " . $mcmsdb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.3.0 Use mcms_redirect()
 * @see mcms_redirect()
 *
 * @param string $url Optional. Redirect URL. Default empty.
 */
function mcmsmu_admin_do_redirect( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0', 'mcms_redirect()' );
	...
	if ( $ref ) {
		$ref = mcmsmu_admin_redirect_add_updated_param( $ref );
		mcms_redirect( $ref );
		exit();
	}
	if ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		mcms_redirect( $_SERVER['HTTP_REFERER'] );
		exit();
	}

	$url = mcmsmu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) ) {
		if ( substr( $_GET['redirect'], 0, 2 ) == 's_' )
			$url .= '&action=blogs&s='. esc_html( substr( $_GET['redirect'], 2 ) );
	} elseif ( isset( $_POST['redirect'] ) ) {
		$url = mcmsmu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	mcms_redirect( $url );
	exit();
}
 ...
 * @return string
 */
function mcmsmu_admin_redirect_add_updated_param( $url = '' ) {
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	// Need to back up mcmsdb table names, and create a new mcms_blogs entry for new blog.
	// Need to get blog_id from mcms_blogs, and create new table names.
...
function get_admin_users_for_domain( $domain = '', $path = '' ) {
	...
	if ( $network_id )
		return $mcmsdb->get_results( $mcmsdb->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $mcmsdb->users AS u, $mcmsdb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $network_id ), ARRAY_A );
 ...
 * }
 * @return array An empty array if the installation is considered "large" via mcms_is_large_network(). Otherwise,
 ...
 */
function mcms_get_sites( $args = array() ) {
...
function is_user_option_local( $key, $user_id = 0, $blog_id = 0 ) {
	...
	$local_key = $mcmsdb->get_blog_prefix( $blog_id ) . $key;
