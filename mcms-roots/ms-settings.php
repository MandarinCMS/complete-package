 ...
 *
 * Allows for some configuration in database-settings.php (see ms-default-constants.php)
 *
 * @package MandarinCMS
...
/** MCMS_Network class */
require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-network.php' );

/** MCMS_Site class */
require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-site.php' );

/** Multisite loader */
require_once( BASED_TREE_URI . MCMSINC . '/ms-load.php' );

/** Default Multisite constants */
require_once( BASED_TREE_URI . MCMSINC . '/ms-default-constants.php' );

if ( defined( 'SUNRISE' ) ) {
	include_once( MCMS_CONTENT_DIR . '/sunrise.php' );
}
...
if ( !isset( $current_site ) || !isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/mcms-admin/.*#', '$1/', $path );
	}
	...
	mcms_load_core_site_options( $site_id );
}

$mcmsdb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php
$mcmsdb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix = $mcmsdb->get_blog_prefix();
$_mcms_switched_stack = array();
$switched = false;

// need to init cache again after blog_id is set
mcms_start_object_cache();

if ( ! $current_site instanceof MCMS_Network ) {
	$current_site = new MCMS_Network( $current_site );
}

if ( ! $current_blog instanceof MCMS_Site ) {
	$current_blog = new MCMS_Site( $current_blog );
}
