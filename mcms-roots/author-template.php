<?php
/**
 * Author Template functions for use in myskins.
 *
 * These functions must be used within the MandarinCMS Loop.
 *
 * @link https://dev.mandarincms.com/Author_Templates
 *
 * @package MandarinCMS
 * @subpackage Template
 */
 ...
 * the value from get_the_author() and as an result of any old myskin that might
 ...
 * @since 0.71
 * @see get_the_author()
 * @link https://dev.mandarincms.com/Template_Tags/the_author
 *
 * @param string $deprecated Deprecated.
 ...
 * author's name.
 *
 * @link https://dev.mandarincms.com/Template_Tags/the_author_link
 *
 * @since 2.1.0
 */
 ...
 *
 * @link https://dev.mandarincms.com/Template_Tags/the_author_posts
 * @since 0.71
 */
...
function get_author_posts_url( $author_id, $author_nicename = '' ) {
	...
	$link = $mcms_rewrite->get_author_permastruct();
 ...
 *
 * @link https://dev.mandarincms.com/Template_Tags/mcms_list_authors
 *
 * @since 1.2.0
 *
 ...
 */
function mcms_list_authors( $args = '' ) {
	global $mcmsdb;
	...
	$query_args = mcms_array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	$query_args['fields'] = 'ids';
	$authors = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $mcmsdb->get_results( "SELECT DISTINCT post_author, COUNT(ID) AS count FROM $mcmsdb->posts WHERE " . get_private_posts_cap_sql( 'post' ) . " GROUP BY post_author" ) as $row ) {
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = get_transient( 'is_multi_author' ) ) ) {
		$rows = (array) $mcmsdb->get_col("SELECT DISTINCT post_author FROM $mcmsdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
