...
final class MCMS_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.mcms-menu-image empty
	 ...
	 *
	 * Custom controllers must extend MCMS_REST_Controller.
	 *
	 * @since 4.7.4
	...
	public function add_rewrite_rules() {
		...
		if ( false !== $this->query_var && $mcms && is_post_type_viewable( $this ) ) {
			$mcms->add_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite && ( is_admin() || '' != get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $mcms_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $mcms_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $mcms_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $mcms_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$mcms_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_mcms_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $mcms, $mcms_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$mcms->remove_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite ) {
			...
			foreach ( $mcms_rewrite->extra_rules_top as $regex => $query ) {
				if ( false !== strpos( $query, "index.php?post_type=$this->name" ) ) {
					unset( $mcms_rewrite->extra_rules_top[ $regex ] );
				}
			}
		}
