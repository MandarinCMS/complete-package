<?php
/**
 * Network API: MCMS_Network_Query class
 *
 * @package MandarinCMS
 ...
 * @since 4.6.0
 *
 * @see MCMS_Network_Query::__construct() for accepted arguments.
 */
class MCMS_Network_Query {
	 ...
	 *
	 *
	 * @param string|array $query MCMS_Network_Query arguments. See MCMS_Network_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param MCMS_Network_Query $this The MCMS_Network_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_network_query', array( &$this ) );
	}

	/**
	 * Sets up the MandarinCMS query for retrieving networks.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param MCMS_Network_Query $this Current instance of MCMS_Network_Query (passed by reference).
		 */
		...
		$last_changed = mcms_cache_get_last_changed( 'networks' );

		$cache_key = "get_network_ids:$key:$last_changed";
		$cache_value = mcms_cache_get( $cache_key, 'networks' );
		...
		if ( false === $cache_value ) {
			...
			mcms_cache_add( $cache_key, $cache_value, 'networks' );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param array            $_networks An array of MCMS_Network objects.
		 * @param MCMS_Network_Query $this      Current instance of MCMS_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to MCMS_Network instances
	...
	protected function get_network_ids() {
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$mcmsdb->site.id";
		}

		// Parse network IDs for an IN clause.
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$mcmsdb->site.id IN ( " . implode( ',', mcms_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$mcmsdb->site.id NOT IN ( " . implode( ',', mcms_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $mcmsdb->prepare( "$mcmsdb->site.domain = %s", $this->query_vars['domain'] );
		}

		// Parse network domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "$mcmsdb->site.domain IN ( '" . implode( "', '", $mcmsdb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse network domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "$mcmsdb->site.domain NOT IN ( '" . implode( "', '", $mcmsdb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $mcmsdb->prepare( "$mcmsdb->site.path = %s", $this->query_vars['path'] );
		}

		// Parse network path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "$mcmsdb->site.path IN ( '" . implode( "', '", $mcmsdb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse network path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "$mcmsdb->site.path NOT IN ( '" . implode( "', '", $mcmsdb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			$this->sql_clauses['where']['search'] = $this->get_search_sql(
				...
				array( "$mcmsdb->site.domain", "$mcmsdb->site.path" )
			);
		}
		...
		$this->sql_clauses['from']    = "FROM $mcmsdb->site $join";
		...
		$network_ids = $mcmsdb->get_col( $this->request );
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param MCMS_Network_Query $network_query        The `MCMS_Network_Query` instance.
			 */
			$found_networks_query = apply_filters( 'found_networks_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_networks = (int) $mcmsdb->get_var( $found_networks_query );
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $orderby == 'network__in' ) {
			...
			$parsed = "FIELD( {$mcmsdb->site}.id, $network__in )";
		} elseif ( $orderby == 'domain_length' || $orderby == 'path_length' ) {
			$field = substr( $orderby, 0, -7 );
			$parsed = "CHAR_LENGTH($mcmsdb->site.$field)";
		} elseif ( in_array( $orderby, $allowed_keys ) ) {
			$parsed = "$mcmsdb->site.$orderby";
		}
