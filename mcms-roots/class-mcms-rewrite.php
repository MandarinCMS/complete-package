<?php
/**
 * Rewrite API: MCMS_Rewrite class
 *
 * @package MandarinCMS
 * @subpackage Rewrite
 * @since 1.5.0
 ...
 *
 * The MandarinCMS Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * MandarinCMS Query class.
 *
 * The Rewrite along with MCMS class function as a front controller for MandarinCMS.
...
class MCMS_Rewrite {
	 ...
	 * `MCMS_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see MCMS_Rewrite::init()
	 * @see MCMS_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to MandarinCMS' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for MandarinCMS into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see MCMS_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to MandarinCMS 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells MCMS::parse_request() to check if a URL matching the page
	 ...
	 * `MCMS_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in MCMS_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see MCMS_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see MCMS_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var array
	 */
	...
	public function page_uri_index() {
		...
		$pages = $mcmsdb->get_results("SELECT ID, post_name, post_parent FROM $mcmsdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'");
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $mcmsdb->get_results( $mcmsdb->prepare( "SELECT ID, post_name, post_parent FROM $mcmsdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 * @since 1.5.0
	 *
	 * @see MCMS_Rewrite::$rewritecode
	 * @see MCMS_Rewrite::$rewritereplace
	 * @see MCMS_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main MCMS_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of MCMS_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see MCMS_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current MCMS_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*mcms-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*mcms-app\.php(/.*)?$' => $this->index . '?error=403',
		);
		...
		if ( is_multisite() && is_main_site() ) {
			...
			$registration_pages['.*register-activation.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*mcms-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param MCMS_Rewrite $this Current MCMS_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and MCMS_Rewrite::rewrite_rules() is that
	 ...
	 */
	public function mcms_rewrite_rules() {
		$this->rules = get_option('rewrite_rules');
		if ( empty($this->rules) ) {
			$this->matches = 'matches';
			$this->rewrite_rules();
			if ( ! did_action( 'mcms_loaded' ) ) {
				add_action( 'mcms_loaded', array( $this, 'flush_rules' ) );
				return $this->rules;
			}
	 ...
	 *
	 * Will add the non_mcms_rules property rules to the .htaccess file before
	 * the MandarinCMS rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to MCMS's index.php (and thus shouldn't be handled by MCMS at all).
		foreach ( (array) $this->non_mcms_rules as $match => $query) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="MandarinCMS: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	...
	public function add_external_rule( $regex, $query ) {
		$this->non_mcms_rules[ $regex ] = $query;
	}
	 ...
	 * query strings. The new permastruct is added to the MCMS_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by MCMS_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to MCMS_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how MCMS_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `MCMS_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls MCMS_Rewrite::mcms_rewrite_rules() after removing the 'rewrite_rules' option.
	...
	public function init() {
		$this->extra_rules = $this->non_mcms_rules = $this->endpoints = array();
	 ...
	 * MCMS_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls MCMS_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls MCMS_Rewrite::init() after
