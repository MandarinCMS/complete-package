<?php
/**
 * MandarinCMS Customize Manager classes
 *
 * @package MandarinCMS
 ...
 *
 * Sets up the myskin-switching process if a myskin other than the active one is
 ...
 * @since 3.4.0
 */
final class MCMS_Customize_Manager {
	/**
	 * An instance of the myskin being previewed.
	 *
	 * @since 3.4.0
	 * @var MCMS_MySkin
	 */
	...
	/**
	 * The directory name of the previously active myskin (within the myskin_root).
	 *
	 * @since 3.4.0
	 * @var string
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @var MCMS_Customize_Selective_Refresh
	 */
	public $selective_refresh;

	/**
	 * Registered instances of MCMS_Customize_Setting.
	 *
	 * @since 3.4.0
	 * @var array
	 */
	...
	/**
	 * Sorted top-level instances of MCMS_Customize_Panel and MCMS_Customize_Section.
	 *
	 * @since 4.0.0
	 * @var array
	 */
	...
	/**
	 * Registered instances of MCMS_Customize_Panel.
	 *
	 * @since 4.0.0
	 * @var array
	 */
	...
	/**
	 * Registered instances of MCMS_Customize_Section.
	 *
	 * @since 3.4.0
	 * @var array
	 */
	...
	/**
	 * Registered instances of MCMS_Customize_Control.
	 *
	 * @since 3.4.0
	 * @var array
	 */
	 ...
	 *                                                 then the changeset UUID will be determined during `after_setup_myskin`: when the
	 ...
	 *     @type string            $myskin              MySkin to be previewed (for myskin switch). Defaults to customize_myskin or myskin query params.
	...
	public function __construct( $args = array() ) {
		...
		$args = array_merge(
			array_fill_keys( array( 'changeset_uuid', 'myskin', 'messenger_channel', 'settings_previewed', 'autosaved', 'branching' ), null ),
			$args
		);

		// Note that the UUID format will be validated in the setup_myskin() method.
		if ( ! isset( $args['changeset_uuid'] ) ) {
			$args['changeset_uuid'] = mcms_generate_uuid4();
		}

		// The myskin and messenger_channel should be supplied via $args, but they are also looked at in the $_REQUEST global here for back-compat.
		if ( ! isset( $args['myskin'] ) ) {
			if ( isset( $_REQUEST['customize_myskin'] ) ) {
				$args['myskin'] = mcms_unslash( $_REQUEST['customize_myskin'] );
			} elseif ( isset( $_REQUEST['myskin'] ) ) { // Deprecated.
				$args['myskin'] = mcms_unslash( $_REQUEST['myskin'] );
			}
		}
		if ( ! isset( $args['messenger_channel'] ) && isset( $_REQUEST['customize_messenger_channel'] ) ) {
			$args['messenger_channel'] = sanitize_key( mcms_unslash( $_REQUEST['customize_messenger_channel'] ) );
		}

		$this->original_stylesheet = get_stylesheet();
		$this->myskin = mcms_get_myskin( 0 === validate_file( $args['myskin'] ) ? $args['myskin'] : null );
		...
		require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-panel.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-section.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-control.php' );

		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-color-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-media-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-upload-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-image-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-background-image-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-background-position-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-cropped-image-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-site-icon-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-header-image-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-myskin-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-code-editor-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-widget-area-customize-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-widget-form-customize-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-item-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-location-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-name-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-locations-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-auto-add-control.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-new-menu-control.php' ); // @todo Remove in 5.0. See #42364.

		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menus-panel.php' );

		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-myskins-panel.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-myskins-section.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-sidebar-section.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-section.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-new-menu-section.php' ); // @todo Remove in 5.0. See #42364.

		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-custom-css-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-filter-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-header-image-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-background-image-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-item-setting.php' );
		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-nav-menu-setting.php' );
		 ...
		 * during the {@see 'modules_loaded'} action, so it cannot be added
		 * in a myskin.
		 *
		 * @since 4.4.0
		 *
		 * @see MCMS_Customize_Manager::__construct()
		 *
		 * @param array                $components List of core components to load.
		 * @param MCMS_Customize_Manager $this       MCMS_Customize_Manager instance.
		 */
		$components = apply_filters( 'customize_loaded_components', $this->components, $this );

		require_once( BASED_TREE_URI . MCMSINC . '/customize/class-mcms-customize-selective-refresh.php' );
		$this->selective_refresh = new MCMS_Customize_Selective_Refresh( $this );

		if ( in_array( 'widgets', $components, true ) ) {
			require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-widgets.php' );
			$this->widgets = new MCMS_Customize_Widgets( $this );
		}

		if ( in_array( 'nav_menus', $components, true ) ) {
			require_once( BASED_TREE_URI . MCMSINC . '/class-mcms-customize-nav-menus.php' );
			$this->nav_menus = new MCMS_Customize_Nav_Menus( $this );
		}

		add_action( 'setup_myskin', array( $this, 'setup_myskin' ) );
		add_action( 'mcms_loaded',   array( $this, 'mcms_loaded' ) );

		// Do not spawn cron (especially the alternate cron) while running the Customizer.
		remove_action( 'init', 'mcms_cron' );
		...
		add_action( 'mcms_ajax_customize_save',                     array( $this, 'save' ) );
		add_action( 'mcms_ajax_customize_trash',                    array( $this, 'handle_changeset_trash_request' ) );
		add_action( 'mcms_ajax_customize_refresh_nonces',           array( $this, 'refresh_nonces' ) );
		add_action( 'mcms_ajax_customize_load_myskins',              array( $this, 'handle_load_myskins_request' ) );
		...
		add_action( 'mcms_ajax_customize_override_changeset_lock',  array( $this, 'handle_override_changeset_lock_request' ) );
		add_action( 'mcms_ajax_customize_dismiss_autosave_or_lock', array( $this, 'handle_dismiss_autosave_or_lock_request' ) );
		...
		// Export the settings to JS via the _mcmsCustomizeSettings variable.
		add_action( 'customize_controls_print_footer_scripts', array( $this, 'customize_pane_settings' ), 1000 );

		// Add myskin update notices.
		if ( current_user_can( 'install_myskins' ) || current_user_can( 'update_myskins' ) ) {
			require_once BASED_TREE_URI . '/mcms-admin/includes/update.php';
			add_action( 'customize_controls_print_footer_scripts', 'mcms_print_admin_notice_templates' );
		}
	}
	...
	public function doing_ajax( $action = null ) {
		...
		if ( ! $action ) {
			...
			/*
			 * Note: we can't just use doing_action( "mcms_ajax_{$action}" ) because we need
			 * to check before admin-ajax.php gets to that point.
			 */
			return isset( $_REQUEST['action'] ) && mcms_unslash( $_REQUEST['action'] ) === $action;
		}
	}

	/**
	 * Custom mcms_die wrapper. Returns either the standard message for UI
	 * or the Ajax message.
	 *
	 * @since 3.4.0
	 *
	 ...
	 */
	protected function mcms_die( $ajax_message, $message = null ) {
		if ( $this->doing_ajax() ) {
			mcms_die( $ajax_message );
		}
		...
		if ( $this->messenger_channel ) {
			ob_start();
			mcms_enqueue_scripts();
			mcms_print_scripts( array( 'customize-base' ) );

			$settings = array(
				'messengerArgs' => array(
					'channel' => $this->messenger_channel,
					'url' => mcms_customize_url(),
				),
				'error' => $ajax_message,
			);
			?>
			...
			} )( mcms.customize, <?php echo mcms_json_encode( $settings ) ?> );
			</script>
			<?php
	...
	/**
	 * Return the Ajax mcms_die() handler if it's a customized request.
	 *
	 * @since 3.4.0
	 * @deprecated 4.7.0
	 *
	 ...
	 */
	public function mcms_die_handler() {
		_deprecated_function( __METHOD__, '4.7.0' );

		if ( $this->doing_ajax() || isset( $_POST['customized'] ) ) {
			return '_ajax_mcms_die_handler';
		}

		return '_default_mcms_die_handler';
	}

	/**
	 * Start preview and customize myskin.
	 *
	 * Check if customize query variable exist. Init filters to filter the current myskin.
	 *
	 * @since 3.4.0
	 *
	 * @global string $pagenow
	 */
	public function setup_myskin() {
		global $pagenow;
		...
		if ( isset( $this->_changeset_uuid ) && false !== $this->_changeset_uuid && ! mcms_is_uuid( $this->_changeset_uuid ) ) {
			$this->mcms_die( -1, __( 'Invalid changeset UUID' ) );
		}

		/*
		 * Clear incoming post data if the user lacks a CSRF token (nonce). Note that the customizer
		 * application will inject the customize_preview_nonce query parameter into all Ajax requests.
		 * For similar behavior elsewhere in MandarinCMS, see rest_cookie_check_errors() which logs out
		...
		if ( ! current_user_can( 'customize' ) && ! $this->changeset_post_id() ) {
			$this->mcms_die( $this->messenger_channel ? 0 : -1, __( 'Non-existent changeset UUID.' ) );
		}
		...
		if ( $this->is_myskin_active() ) {
			// Once the myskin is loaded, we'll validate it.
			add_action( 'after_setup_myskin', array( $this, 'after_setup_myskin' ) );
		} else {
			// If the requested myskin is not the active myskin and the user doesn't have the
			// switch_myskins cap, bail.
			if ( ! current_user_can( 'switch_myskins' ) ) {
				$this->mcms_die( -1, __( 'Sorry, you are not allowed to edit myskin options on this site.' ) );
			}

			// If the myskin has errors while loading, bail.
			if ( $this->myskin()->errors() ) {
				$this->mcms_die( -1, $this->myskin()->errors()->get_error_message() );
			}

			// If the myskin isn't allowed per multisite settings, bail.
			if ( ! $this->myskin()->is_allowed() ) {
				$this->mcms_die( -1, __( 'The requested myskin does not exist.' ) );
			}
		}

		// Make sure changeset UUID is established immediately after the myskin is loaded.
		add_action( 'after_setup_myskin', array( $this, 'establish_loaded_changeset' ), 5 );

		/*
		 * Import myskin starter content for fresh installations when landing in the customizer.
		 * Import starter content at after_setup_myskin:100 so that any
		 * add_myskin_support( 'starter-content' ) calls will have been made.
		 */
		if ( get_option( 'fresh_site' ) && 'customize.php' === $pagenow ) {
			add_action( 'after_setup_myskin', array( $this, 'import_myskin_starter_content' ), 100 );
		}

		$this->start_previewing_myskin();
	}

	/**
	 * Establish the loaded changeset.
	 *
	 * This method runs right at after_setup_myskin and applies the 'customize_changeset_branching' filter to determine
	...
	public function establish_loaded_changeset() {
		...
		if ( empty( $this->_changeset_uuid ) ) {
			...
			if ( ! $this->branching() && $this->is_myskin_active() ) {
				...
				if ( ! empty( $unpublished_changeset_post ) && mcms_is_uuid( $unpublished_changeset_post->post_name ) ) {
			...
			if ( empty( $changeset_uuid ) ) {
				$changeset_uuid = mcms_generate_uuid4();
			}
	...
	/**
	 * Callback to validate a myskin once it is loaded
	 *
	 * @since 3.4.0
	 */
	public function after_setup_myskin() {
		$doing_ajax_or_is_customized = ( $this->doing_ajax() || isset( $_POST['customized'] ) );
		if ( ! $doing_ajax_or_is_customized && ! validate_current_myskin() ) {
			mcms_redirect( 'myskins.php?broken=true' );
			exit;
		}
	}

	/**
	 * If the myskin to be previewed isn't the active myskin, add filter callbacks
	 * to swap it out at runtime.
	 *
	 * @since 3.4.0
	 */
	public function start_previewing_myskin() {
		...
		if ( ! $this->is_myskin_active() ) {
			...
			add_filter( 'pre_option_current_myskin', array( $this, 'current_myskin' ) );

			// @link: https://core.trac.mandarincms.com/ticket/20027
			add_filter( 'pre_option_stylesheet', array( $this, 'get_stylesheet' ) );
			add_filter( 'pre_option_template', array( $this, 'get_template' ) );

			// Handle custom myskin roots.
			add_filter( 'pre_option_stylesheet_root', array( $this, 'get_stylesheet_root' ) );
			add_filter( 'pre_option_template_root', array( $this, 'get_template_root' ) );
		}

		/**
		 * Fires once the Customizer myskin preview has started.
		 *
		 * @since 3.4.0
		 *
		 * @param MCMS_Customize_Manager $this MCMS_Customize_Manager instance.
		 */
		do_action( 'start_previewing_myskin', $this );
	}

	/**
	 * Stop previewing the selected myskin.
	 *
	 * Removes filters to change the current myskin.
	 *
	 * @since 3.4.0
	 */
	public function stop_previewing_myskin() {
		if ( ! $this->is_preview() ) {
			return;
		}
		...
		if ( ! $this->is_myskin_active() ) {
			...
			remove_filter( 'pre_option_current_myskin', array( $this, 'current_myskin' ) );
		...
		/**
		 * Fires once the Customizer myskin preview has stopped.
		 *
		 * @since 3.4.0
		 *
		 * @param MCMS_Customize_Manager $this MCMS_Customize_Manager instance.
		 */
		do_action( 'stop_previewing_myskin', $this );
	}
	 ...
	 *
	 * @since 4.9.0
	 * @see MCMS_Customize_Setting::preview()
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @since 4.9.0
	 * @see MCMS_Customize_Manager::changeset_data()
	 *
	 ...
	 *
	 * @since 4.9.0
	 * @see MCMS_Customize_Manager::establish_loaded_changeset()
	 *
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of MandarinCMS users,
		 * they are the default. For MandarinCMS sites that have heavy site management in the Customizer
		 ...
		 * @param MCMS_Customize_Manager $mcms_customize    Manager instance.
		 */
	...
	/**
	 * Get the myskin being customized.
	 *
	 * @since 3.4.0
	 *
	 * @return MCMS_MySkin
	...
	/**
	 * Checks if the current myskin is active.
	 *
	 * @since 3.4.0
	 *
	 * @return bool
	 */
	public function is_myskin_active() {
	 ...
	 * @since 3.4.0
	 */
	public function mcms_loaded() {

		// Unconditionally register core types for panels, sections, and controls in case module unhooks all customize_register actions.
		$this->register_panel_type( 'MCMS_Customize_Panel' );
		$this->register_panel_type( 'MCMS_Customize_MySkins_Panel' );
		$this->register_section_type( 'MCMS_Customize_Section' );
		$this->register_section_type( 'MCMS_Customize_Sidebar_Section' );
		$this->register_section_type( 'MCMS_Customize_MySkins_Section' );
		$this->register_control_type( 'MCMS_Customize_Color_Control' );
		$this->register_control_type( 'MCMS_Customize_Media_Control' );
		$this->register_control_type( 'MCMS_Customize_Upload_Control' );
		$this->register_control_type( 'MCMS_Customize_Image_Control' );
		$this->register_control_type( 'MCMS_Customize_Background_Image_Control' );
		$this->register_control_type( 'MCMS_Customize_Background_Position_Control' );
		$this->register_control_type( 'MCMS_Customize_Cropped_Image_Control' );
		$this->register_control_type( 'MCMS_Customize_Site_Icon_Control' );
		$this->register_control_type( 'MCMS_Customize_MySkin_Control' );
		$this->register_control_type( 'MCMS_Customize_Code_Editor_Control' );
		$this->register_control_type( 'MCMS_Customize_Date_Time_Control' );

		/**
		 * Fires once MandarinCMS has loaded, allowing scripts and styles to be initialized.
		 *
		 * @since 3.4.0
		 *
		 * @param MCMS_Customize_Manager $this MCMS_Customize_Manager instance.
	...
	/**
	 * Prevents Ajax requests from following redirects when previewing a myskin
	 ...
	 * @return int
	 */
	public function mcms_redirect_status( $status ) {
	...
	public function find_changeset_post_id( $uuid ) {
		...
		if ( ! empty( $changeset_post_query->posts ) ) {
			...
			mcms_cache_set( $uuid, $changeset_post_id, $cache_group );
			return $changeset_post_id;
		}
	 ...
	 * }
	 * @return MCMS_Post[] Auto-draft changesets.
	 */
	...
	protected function get_changeset_post_data( $post_id ) {
		...
		if ( function_exists( 'json_last_error' ) && json_last_error() ) {
			return new MCMS_Error( 'json_parse_error', '', json_last_error() );
		}
		if ( ! is_array( $changeset_data ) ) {
			return new MCMS_Error( 'expected_array' );
		}
	...
	public function changeset_data() {
		...
		if ( ! $changeset_post_id ) {
			...
			if ( $this->autosaved() && is_user_logged_in() ) {
				$autosave_post = mcms_get_post_autosave( $changeset_post_id, get_current_user_id() );
				if ( $autosave_post ) {
	...
	/**
	 * Import myskin starter content into the customized state.
	 *
	 * @since 4.7.0
	 *
	 * @param array $starter_content Starter content. Defaults to `get_myskin_starter_content()`.
	 */
	function import_myskin_starter_content( $starter_content = array() ) {
		if ( empty( $starter_content ) ) {
			$starter_content = get_myskin_starter_content();
		}
		...
		if ( $this->changeset_post_id() ) {
			 ...
			 * This will need to be revisited in the future once myskin switching
			 * is allowed with drafted/scheduled changesets, since switching to
			 * another myskin could result in more starter content being applied.
		...
		$myskin_mods = isset( $starter_content['myskin_mods'] ) ? $starter_content['myskin_mods'] : array();
		...
		$all_post_slugs = array_merge(
			mcms_list_pluck( $attachments, 'post_name' ),
			mcms_list_pluck( $posts, 'post_name' )
		);
		 ...
		 */
		$post_types = array_filter( array_merge( array( 'attachment' ), mcms_list_pluck( $posts, 'post_type' ) ) );

		// Re-use auto-draft starter content posts referenced in the current customized state.
		$existing_starter_content_posts = array();
		if ( ! empty( $starter_content_auto_draft_post_ids ) ) {
			$existing_posts_query = new MCMS_Query( array(
			...
			foreach ( $attachments as $symbol => $attachment ) {
				...
				if ( isset( $existing_starter_content_posts[ 'attachment:' . $attachment['post_name'] ] ) ) {
					...
					if ( empty( $attached_file ) || ! file_exists( $attached_file ) ) {
						$attachment_id = null;
						$attached_file = null;
					} elseif ( $this->get_stylesheet() !== get_post_meta( $attachment_post->ID, '_starter_content_myskin', true ) ) {

						// Re-generate attachment metadata since it was previously generated for a different myskin.
						$metadata = mcms_generate_attachment_metadata( $attachment_post->ID, $attached_file );
						mcms_update_attachment_metadata( $attachment_id, $metadata );
						update_post_meta( $attachment_id, '_starter_content_myskin', $this->get_stylesheet() );
					}
				}
				...
				if ( ! $attachment_id ) {
					...
					// Copy file to temp location so that original file won't get deleted from myskin after sideloading.
					$temp_file_name = mcms_tempnam( basename( $file_path ) );
					...
					$attachment_post_data = array_merge(
						mcms_array_slice_assoc( $attachment, array( 'post_title', 'post_content', 'post_excerpt' ) ),
						array(
		...
		if ( ! empty( $posts ) ) {
			foreach ( array_keys( $posts ) as $post_symbol ) {
				...
				if ( ! empty( $posts[ $post_symbol ]['template'] ) ) {
					$posts[ $post_symbol ]['meta_input']['_mcms_page_template'] = $posts[ $post_symbol ]['template'];
				}

				$r = $this->nav_menus->insert_auto_draft_post( $posts[ $post_symbol ] );
				if ( $r instanceof MCMS_Post ) {
					$posts[ $post_symbol ]['ID'] = $r->ID;
				}
			}

			$starter_content_auto_draft_post_ids = array_merge( $starter_content_auto_draft_post_ids, mcms_list_pluck( $posts, 'ID' ) );
		}
		...
		// MySkin mods.
		foreach ( $myskin_mods as $name => $value ) {
			if ( preg_match( '/^{{(?P<symbol>.+)}}$/', $value, $matches ) ) {
				if ( isset( $posts[ $matches['symbol'] ] ) ) {
					$value = $posts[ $matches['symbol'] ]['ID'];
				} elseif ( isset( $attachment_ids[ $matches['symbol'] ] ) ) {
			...
			if ( 'header_image' === $name ) {
				...
				$metadata = mcms_get_attachment_metadata( $value );
				if ( empty( $metadata ) ) {
					continue;
				}
				$value = array(
					'attachment_id' => $value,
					'url' => mcms_get_attachment_url( $value ),
					'height' => $metadata['height'],
					'width' => $metadata['width'],
				);
			} elseif ( 'background_image' === $name ) {
				$value = mcms_get_attachment_url( $value );
			}
	...
	protected function prepare_starter_content_attachments( $attachments ) {
		...
		foreach ( $attachments as $symbol => $attachment ) {
			...
			if ( file_exists( $attachment['file'] ) ) {
				$file_path = $attachment['file']; // Could be absolute path to file in module.
			} elseif ( is_child_myskin() && file_exists( get_stylesheet_directory() . '/' . $attachment['file'] ) ) {
			...
			$checked_filetype = mcms_check_filetype( $file_name );
	 ...
	 * 1. If the myskin is not currently active, then the base array is any stashed
	 *    myskin mods that were modified previously but never published.
	 ...
	 * 4. Any programmatically-set post values via `MCMS_Customize_Manager::set_post_value()`.
	 *
	 ...
	 *
	 * @since 4.1.1
	 * @since 4.7.0 Added $args param and merging with changeset values and stashed myskin mods.
	 *
	 * @param array $args {
	 *     Args.
	 *
	...
	public function unsanitized_post_values( $args = array() ) {
		...
		// Let default values be from the stashed myskin mods if doing a myskin switch and if no changeset is present.
		if ( ! $this->is_myskin_active() ) {
			$stashed_myskin_mods = get_option( 'customize_stashed_myskin_mods' );
			$stylesheet = $this->get_stylesheet();
			if ( isset( $stashed_myskin_mods[ $stylesheet ] ) ) {
				$values = array_merge( $values, mcms_list_pluck( $stashed_myskin_mods[ $stylesheet ], 'value' ) );
			}
		}
		...
		if ( ! $args['exclude_changeset'] ) {
			foreach ( $this->changeset_data() as $setting_id => $setting_params ) {
				...
				if ( isset( $setting_params['type'] ) && 'myskin_mod' === $setting_params['type'] ) {

					// Ensure that myskin mods values are only used if they were saved under the current myskin.
		...
		if ( ! $args['exclude_post_data'] ) {
			if ( ! isset( $this->_post_values ) ) {
				if ( isset( $_POST['customized'] ) ) {
					$post_values = json_decode( mcms_unslash( $_POST['customized'] ), true );
				} else {
					$post_values = array();
				}
	 ...
	 *
	 * @see MCMS_REST_Server::dispatch()
	 * @see MCMS_REST_Request::sanitize_params()
	 * @see MCMS_REST_Request::has_valid_params()
	 *
	 * @param MCMS_Customize_Setting $setting A MCMS_Customize_Setting derived object.
	...
	public function post_value( $setting, $default = null ) {
		...
		if ( is_null( $value ) || is_mcms_error( $value ) ) {
			return $default;
		}
		return $value;
	}
	 ...
	 * @since 4.2.0
	 *
	 * @param string $setting_id ID for the MCMS_Customize_Setting instance.
	...
	public function set_post_value( $setting_id, $value ) {
		 ...
		 *
		 * Fires when the MCMS_Customize_Manager::set_post_value() method is called.
		 *
		 ...
		 * Fires when the MCMS_Customize_Manager::set_post_value() method is called.
		 *
		 * This is useful for `MCMS_Customize_Setting` instances to watch
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and MandarinCMS would
		...
		add_action( 'mcms_head', 'mcms_no_robots' );
		add_filter( 'mcms_headers', array( $this, 'filter_iframe_security_headers' ) );
		...
		if ( $this->messenger_channel && ! current_user_can( 'customize' ) ) {
			$this->mcms_die( -1, __( 'Unauthorized. You may remove the customize_messenger_channel param to preview as frontend.' ) );
			return;
		}

		$this->prepare_controls();

		add_filter( 'mcms_redirect', array( $this, 'add_state_query_params' ) );

		mcms_enqueue_script( 'customize-preview' );
		mcms_enqueue_style( 'customize-preview' );
		add_action( 'mcms_head', array( $this, 'customize_preview_loading_style' ) );
		add_action( 'mcms_head', array( $this, 'remove_frameless_preview_messenger_channel' ) );
		add_action( 'mcms_footer', array( $this, 'customize_preview_settings' ), 20 );
	 ...
	 * @since 4.7.0
	 * @see mcms_redirect()
	 * @see MCMS_Customize_Manager::get_allowed_url()
	 *
	 * @param string $url URL.
	 * @return string URL.
	 */
	public function add_state_query_params( $url ) {
		$parsed_original_url = mcms_parse_url( $url );
		$is_allowed = false;
		foreach ( $this->get_allowed_urls() as $allowed_url ) {
			$parsed_allowed_url = mcms_parse_url( $allowed_url );
			$is_allowed = (
		...
		if ( $is_allowed ) {
			...
			if ( ! $this->is_myskin_active() ) {
				$query_params['customize_myskin'] = $this->get_stylesheet();
			}
			if ( $this->messenger_channel ) {
	...
	public function customize_preview_loading_style() {
		?><style>
			body.mcms-customizer-unloading {
				opacity: 0.25;
	...
	public function customize_preview_settings() {
		...
		$self_url = empty( $_SERVER['REQUEST_URI'] ) ? home_url( '/' ) : esc_url_raw( mcms_unslash( $_SERVER['REQUEST_URI'] ) );
		...
		foreach ( $allowed_urls as $allowed_url ) {
			$parsed = mcms_parse_url( $allowed_url );
		...
		<script type="text/javascript">
			var _mcmsCustomizeSettings = <?php echo mcms_json_encode( $settings ); ?>;
			_mcmsCustomizeSettings.values = {};
			(function( v ) {
				<?php
				/*
				 * Serialize settings separately from the initial _mcmsCustomizeSettings
				...
				foreach ( $this->settings as $id => $setting ) {
					if ( $setting->check_capabilities() ) {
						printf(
							"v[%s] = %s;\n",
							mcms_json_encode( $id ),
							mcms_json_encode( $setting->js_value() )
						);
					}
				}
				?>
	 ...
	 * @deprecated 4.7.0
	 *
	 * @param mixed $return Value passed through for {@see 'mcms_die_handler'} filter.
	 * @return mixed Value passed through for {@see 'mcms_die_handler'} filter.
	 */
	...
	/**
	 * Retrieve the template name of the previewed myskin.
	 *
	 * @since 3.4.0
	 *
	...
	public function get_template() {
		return $this->myskin()->get_template();
	}

	/**
	 * Retrieve the stylesheet name of the previewed myskin.
	 *
	 * @since 3.4.0
	 *
	...
	public function get_stylesheet() {
		return $this->myskin()->get_stylesheet();
	}

	/**
	 * Retrieve the template root of the previewed myskin.
	 *
	 * @since 3.4.0
	 *
	...
	public function get_template_root() {
		return get_raw_myskin_root( $this->get_template(), true );
	}

	/**
	 * Retrieve the stylesheet root of the previewed myskin.
	 *
	 * @since 3.4.0
	 *
	 * @return string MySkin root.
	...
	public function get_stylesheet_root() {
		return get_raw_myskin_root( $this->get_stylesheet(), true );
	}

	/**
	 * Filters the current myskin and return the name of the previewed myskin.
	 *
	 * @since 3.4.0
	 *
	 * @param $current_myskin {@internal Parameter is not used}
	 * @return string MySkin name.
	 */
	public function current_myskin( $current_myskin ) {
		return $this->myskin()->display('Name');
	}
	 ...
	 * to values that pass validation, and values that become null or `MCMS_Error`
	 * after sanitizing are marked invalid.
	 *
	 * @since 4.6.0
	 *
	 * @see MCMS_REST_Request::has_valid_params()
	 * @see MCMS_Customize_Setting::validate()
	 *
	 ...
	 * }
	 * @return array Mapping of setting IDs to return value of validate method calls, either `true` or `MCMS_Error`.
	 */
	public function validate_setting_values( $setting_values, $options = array() ) {
		$options = mcms_parse_args( $options, array(
		...
		foreach ( $setting_values as $setting_id => $unsanitized_value ) {
			...
			if ( ! $setting ) {
				if ( $options['validate_existence'] ) {
					$validities[ $setting_id ] = new MCMS_Error( 'unrecognized', __( 'Setting does not exist or is unrecognized.' ) );
				}
				continue;
			}
			if ( $options['validate_capability'] && ! current_user_can( $setting->capability ) ) {
				$validity = new MCMS_Error( 'unauthorized', __( 'Unauthorized to modify setting due to capability.' ) );
			} else {
			...
			if ( ! is_mcms_error( $validity ) ) {
				/** This filter is documented in mcms-roots/class-mcms-customize-setting.php */
				$late_validity = apply_filters( "customize_validate_{$setting->id}", new MCMS_Error(), $unsanitized_value, $setting );
			...
			if ( ! is_mcms_error( $validity ) ) {
				...
				if ( is_null( $value ) ) {
					$validity = false;
				} elseif ( is_mcms_error( $value ) ) {
					$validity = $value;
				}
			}
			if ( false === $validity ) {
				$validity = new MCMS_Error( 'invalid_value', __( 'Invalid value.' ) );
			}
	 ...
	 *
	 * Converts `MCMS_Error` instance into array suitable for passing into the
	 * `mcms.customize.Notification` JS model.
	 *
	 * @since 4.6.0
	 *
	 * @param true|MCMS_Error $validity Setting validity.
	 * @return true|array If `$validity` was a MCMS_Error, the error codes will be array-mapped
	...
	/**
	 * Handle customize_save MCMS Ajax request to save/update a changeset.
	 *
	 * @since 3.4.0
	...
	public function save() {
		if ( ! is_user_logged_in() ) {
			mcms_send_json_error( 'unauthenticated' );
		}

		if ( ! $this->is_preview() ) {
			mcms_send_json_error( 'not_preview' );
		}

		$action = 'save-customize_' . $this->get_stylesheet();
		if ( ! check_ajax_referer( $action, 'nonce', false ) ) {
			mcms_send_json_error( 'invalid_nonce' );
		}
		...
		if ( $is_new_changeset ) {
			if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->create_posts ) ) {
				mcms_send_json_error( 'cannot_create_changeset_post' );
			}
		} else {
			if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->edit_post, $changeset_post_id ) ) {
				mcms_send_json_error( 'cannot_edit_changeset_post' );
			}
		}

		if ( ! empty( $_POST['customize_changeset_data'] ) ) {
			$input_changeset_data = json_decode( mcms_unslash( $_POST['customize_changeset_data'] ), true );
			if ( ! is_array( $input_changeset_data ) ) {
				mcms_send_json_error( 'invalid_customize_changeset_data' );
			}
		} else {
		...
		if ( isset( $_POST['customize_changeset_title'] ) ) {
			$changeset_title = sanitize_text_field( mcms_unslash( $_POST['customize_changeset_title'] ) );
		}
		...
		if ( isset( $_POST['customize_changeset_status'] ) ) {
			$changeset_status = mcms_unslash( $_POST['customize_changeset_status'] );
			if ( ! get_post_status_object( $changeset_status ) || ! in_array( $changeset_status, array( 'draft', 'pending', 'publish', 'future' ), true ) ) {
				mcms_send_json_error( 'bad_customize_changeset_status', 400 );
			}
			...
			if ( $is_publish && ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->publish_posts ) ) {
				mcms_send_json_error( 'changeset_publish_unauthorized', 403 );
			}
		}

		/*
		 * Validate changeset date param. Date is assumed to be in local time for
		 * the MCMS if in MySQL format (YYYY-MM-DD HH:MM:SS). Otherwise, the date
		...
		if ( isset( $_POST['customize_changeset_date'] ) ) {
			$changeset_date = mcms_unslash( $_POST['customize_changeset_date'] );
			if ( preg_match( '/^\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d$/', $changeset_date ) ) {
				...
				$valid_date = mcms_checkdate( $mm, $jj, $aa, $changeset_date );
				if ( ! $valid_date ) {
					mcms_send_json_error( 'bad_customize_changeset_date', 400 );
				}
		...
		if ( ! $is_new_changeset ) {
			$lock_user_id = mcms_check_post_lock( $this->changeset_post_id() );
		}
		...
		if ( is_mcms_error( $r ) ) {
			...
			if ( 'publish' === $response['changeset_status'] || 'trash' === $response['changeset_status'] ) {
				$response['next_changeset_uuid'] = mcms_generate_uuid4();
			}
		}
	 ...
	 * }
	 *
	 * @return array|MCMS_Error Returns array on success and MCMS_Error with array data on error.
	 */
	function save_changeset_post( $args = array() ) {
		...
		if ( $changeset_post_id ) {
			...
			if ( 'publish' === $existing_status || 'trash' === $existing_status ) {
				return new MCMS_Error(
					...
					array(
						'next_changeset_uuid' => mcms_generate_uuid4(),
					)
				);
			}

			$existing_changeset_data = $this->get_changeset_post_data( $changeset_post_id );
			if ( is_mcms_error( $existing_changeset_data ) ) {
				return $existing_changeset_data;
			}
		}

		// Fail if attempting to publish but publish hook is missing.
		if ( 'publish' === $args['status'] && false === has_action( 'transition_post_status', '_mcms_customize_publish_changeset' ) ) {
			return new MCMS_Error( 'missing_publish_callback' );
		}
		...
		if ( $args['date_gmt'] ) {
			...
			if ( ! $is_future_dated ) {
				return new MCMS_Error( 'not_future_date', __( 'You must supply a future date to schedule.' ) ); // Only future dates are allowed.
			}

			if ( ! $this->is_myskin_active() && ( 'future' === $args['status'] || $is_future_dated ) ) {
				return new MCMS_Error( 'cannot_schedule_myskin_switches' ); // This should be allowed in the future, when myskin is a regular setting.
			}
			...
			if ( $will_remain_auto_draft ) {
				return new MCMS_Error( 'cannot_supply_date_for_auto_draft_changeset' );
			}
			...
			if ( mysql2date( 'U', $changeset_post->post_date_gmt, false ) <= mysql2date( 'U', $now, false ) ) {
				return new MCMS_Error( 'not_future_date', __( 'You must supply a future date to schedule.' ) );
			}
		}
		...
		// Validate autosave param. See _mcms_post_revision_fields() for why these fields are disallowed.
		if ( $args['autosave'] ) {
			if ( $args['date_gmt'] ) {
				return new MCMS_Error( 'illegal_autosave_with_date_gmt' );
			} elseif ( $args['status'] ) {
				return new MCMS_Error( 'illegal_autosave_with_status' );
			} elseif ( $args['user_id'] && get_current_user_id() !== $args['user_id'] ) {
				return new MCMS_Error( 'illegal_autosave_with_non_current_user' );
			}
		}

		// The request was made via mcms.customize.previewer.save().
		...
		// Note that in addition to post data, this will include any stashed myskin mods.
		...
		foreach ( $post_values as $setting_id => $setting_value ) {
			...
			if ( $setting && 'myskin_mod' === $setting->type ) {
		 ...
		 *
		 * Modules can add just-in-time {@see 'customize_validate_{$this->ID}'} filters
		...
		$invalid_setting_count = count( array_filter( $setting_validities, 'is_mcms_error' ) );
		...
		if ( $update_transactionally && $invalid_setting_count > 0 ) {
			...
			return new MCMS_Error( 'transaction_fail', '', $response );
		}
		...
		foreach ( $args['data'] as $setting_id => $setting_params ) {
			...
			if ( isset( $setting_validities[ $setting_id ] ) && is_mcms_error( $setting_validities[ $setting_id ] ) ) {
				continue;
			}

			$changeset_setting_id = $setting_id;
			if ( 'myskin_mod' === $setting->type ) {
		...
		$filter_context = array(
			...
			'previous_data' => is_mcms_error( $original_changeset_data ) ? array() : $original_changeset_data,
			'manager' => $this,
		);
		 ...
		 *
		 * Modules may amend additional data (such as additional meta for settings) into the changeset with this filter.
		 *
		 * @since 4.7.0
		 *
		 ...
		 *     @type MCMS_Customize_Manager $manager       Manager instance.
		 * }
		 */
		$data = apply_filters( 'customize_changeset_save_data', $data, $filter_context );

		// Switch myskin if publishing changes now.
		if ( 'publish' === $args['status'] && ! $this->is_myskin_active() ) {
			// Temporarily stop previewing the myskin to allow switch_myskins() to operate properly.
			$this->stop_previewing_myskin();
			switch_myskin( $this->get_stylesheet() );
			update_option( 'myskin_switched_via_customizer', true );
			$this->start_previewing_myskin();
		}

		// Gather the data for mcms_insert_post()/mcms_update_post().
		$json_options = 0;
		...
		$json_options |= JSON_PRETTY_PRINT; // Also introduced in PHP 5.4, but MCMS defines constant for back compat. See MCMS Trac #30139.
		$post_array = array(
			'post_content' => mcms_json_encode( $data, $json_options ),
		);
		if ( $args['title'] ) {
		...
		add_filter( 'mcms_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ), 5, 3 );

		// Update the changeset post. The publish_customize_changeset action will cause the settings in the changeset to be saved via MCMS_Customize_Setting::save().
		$has_kses = ( false !== has_filter( 'content_save_pre', 'mcms_filter_post_kses' ) );
		if ( $has_kses ) {
			kses_remove_filters(); // Prevent KSES from corrupting JSON in post_content.
		}

		// Note that updating a post with publish status will trigger MCMS_Customize_Manager::publish_changeset_values().
		if ( $changeset_post_id ) {
			if ( $args['autosave'] && 'auto-draft' !== get_post_status( $changeset_post_id ) ) {
				// See _mcms_translate_postdata() for why this is required as it will use the edit_post meta capability.
				add_filter( 'map_meta_cap', array( $this, 'grant_edit_post_capability_for_changeset' ), 10, 4 );
				$post_array['post_ID'] = $post_array['ID'];
				$post_array['post_type'] = 'customize_changeset';
				$r = mcms_create_post_autosave( mcms_slash( $post_array ) );
				...
				$r = mcms_update_post( mcms_slash( $post_array ), true );

				// Delete autosave revision for user when the changeset is updated.
				if ( ! empty( $args['user_id'] ) ) {
					$autosave_draft = mcms_get_post_autosave( $changeset_post_id, $args['user_id'] );
					if ( $autosave_draft ) {
						mcms_delete_post( $autosave_draft->ID, true );
					}
				}
			}
		} else {
			$r = mcms_insert_post( mcms_slash( $post_array ), true );
			if ( ! is_mcms_error( $r ) ) {
		...
		$this->_changeset_data = null; // Reset so MCMS_Customize_Manager::changeset_data() will re-populate with updated contents.

		remove_filter( 'mcms_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ) );
		...
		if ( is_mcms_error( $r ) ) {
			...
			return new MCMS_Error( 'changeset_post_save_failure', '', $response );
		}
	 ...
	 *
	 * The following re-formulates the logic from `mcms_trash_post()` as done in
	 * `mcms_publish_post()`. The reason for bypassing `mcms_trash_post()` is that it
	 ...
	 * @see mcms_trash_post()
	 *
	 * @param int|MCMS_Post $post The changeset post.
	 * @return mixed A MCMS_Post object for the trashed post or an empty value on failure.
	 */
	public function trash_changeset_post( $post ) {
		...
		if ( ! ( $post instanceof MCMS_Post ) ) {
			return $post;
		}
		$post_id = $post->ID;

		if ( ! EMPTY_TRASH_DAYS ) {
			return mcms_delete_post( $post_id, true );
		}
		...
		/** This filter is documented in mcms-roots/post.php */
		$check = apply_filters( 'pre_trash_post', null, $post );
		if ( null !== $check ) {
			return $check;
		}

		/** This action is documented in mcms-roots/post.php */
		do_action( 'mcms_trash_post', $post_id );

		add_post_meta( $post_id, '_mcms_trash_meta_status', $post->post_status );
		add_post_meta( $post_id, '_mcms_trash_meta_time', time() );

		$old_status = $post->post_status;
		$new_status = 'trash';
		$mcmsdb->update( $mcmsdb->posts, array( 'post_status' => $new_status ), array( 'ID' => $post->ID ) );
		clean_post_cache( $post->ID );

		$post->post_status = $new_status;
		mcms_transition_post_status( $new_status, $old_status, $post );
		...
		/** This action is documented in mcms-roots/post.php */
		do_action( 'mcms_insert_post', $post->ID, $post, true );

		mcms_trash_post_comments( $post_id );
	 ...
	 * the 'customize' meta cap which then maps to 'edit_myskin_options'. This is currently
	 * required in core for `mcms_create_post_autosave()` because it will call
	 * `_mcms_translate_postdata()` which in turn will check if a user can 'edit_post', but the
	 ...
	 *
	 * @since 4.9.0
	 * @link https://core.trac.mandarincms.com/ticket/40922
	 * @see MCMS_Customize_Manager::save_changeset_post()
	 * @see _mcms_translate_postdata()
	 *
	...
	public function check_changeset_lock_with_heartbeat( $response, $data, $screen_id ) {
		...
		if (
			...
			$lock_user_id = mcms_check_post_lock( $changeset_post_id );
	...
	public function handle_override_changeset_lock_request() {
		if ( ! $this->is_preview() ) {
			mcms_send_json_error( 'not_preview', 400 );
		}
		...
		mcms_send_json_success( 'changeset_taken_over' );
	}
	 ...
	 * @param MCMS_Post $last_revision    The last revision post object.
	 ...
	 * `_mcms_customize_publish_changeset()` when a customize_changeset post is
	 * transitioned to the `publish` status. As such, this method should not be
	 * called directly and instead `mcms_publish_post()` should be used.
	 *
	 * Please note that if the settings in the changeset are for a non-activated
	 * myskin, the myskin must first be switched to (via `switch_myskin()`) before
	 * invoking this method.
	 *
	 * @since 4.7.0
	 * @see _mcms_customize_publish_changeset()
	 * @global mcmsdb $mcmsdb
	 *
	 * @param int $changeset_post_id ID for customize_changeset post. Defaults to the changeset for the current manager instance.
	 * @return true|MCMS_Error True or error info.
	 */
	public function _publish_changeset_values( $changeset_post_id ) {
		...
		if ( is_mcms_error( $publishing_changeset_data ) ) {
		 ...
		 * on the $mcms_customize object passed to hooks during the save logic.
		 */
		...
		// Parse changeset data to identify myskin mod settings and user IDs associated with settings to be saved.
		$setting_user_ids = array();
		$myskin_mod_settings = array();
		$namespace_pattern = '/^(?P<stylesheet>.+?)::(?P<setting_id>.+)$/';
		$matches = array();
		foreach ( $this->_changeset_data as $raw_setting_id => $setting_params ) {
			$actual_setting_id = null;
			$is_myskin_mod_setting = (
				isset( $setting_params['value'] )
				&&
				isset( $setting_params['type'] )
				&&
				'myskin_mod' === $setting_params['type']
				&&
				preg_match( $namespace_pattern, $raw_setting_id, $matches )
			);
			if ( $is_myskin_mod_setting ) {
				if ( ! isset( $myskin_mod_settings[ $matches['stylesheet'] ] ) ) {
					$myskin_mod_settings[ $matches['stylesheet'] ] = array();
				}
				$myskin_mod_settings[ $matches['stylesheet'] ][ $matches['setting_id'] ] = $setting_params;
			...
			// Keep track of the user IDs for settings actually for this myskin.
		...
		/**
		 * Fires once the myskin has switched in the Customizer, but before settings
		 * have been saved.
		 *
		 * @since 3.4.0
		 *
		 * @param MCMS_Customize_Manager $manager MCMS_Customize_Manager instance.
		 */
		...
		foreach ( $changeset_setting_ids as $setting_id ) {
			...
			if ( $setting ) {
				 ...
				 * when a scheduled changeset publishes via MCMS Cron.
				 */
				if ( isset( $setting_user_ids[ $setting_id ] ) ) {
					mcms_set_current_user( $setting_user_ids[ $setting_id ] );
				} else {
					mcms_set_current_user( $original_user_id );
				}
		...
		// Update the stashed myskin mod settings, removing the active myskin's stashed settings, if activated.
		if ( did_action( 'switch_myskin' ) ) {
			$other_myskin_mod_settings = $myskin_mod_settings;
			unset( $other_myskin_mod_settings[ $this->get_stylesheet() ] );
			$this->update_stashed_myskin_mod_settings( $other_myskin_mod_settings );
		}
		 ...
		 */
		$revisions = mcms_get_post_revisions( $changeset_post_id, array( 'check_enabled' => false ) );
	...
	/**
	 * Update stashed myskin mod settings.
	 *
	 * @since 4.7.0
	 *
	 * @param array $inactive_myskin_mod_settings Mapping of stylesheet to arrays of myskin mod settings.
	 * @return array|false Returns array of updated stashed myskin mods or false if the update failed or there were no changes.
	 */
	protected function update_stashed_myskin_mod_settings( $inactive_myskin_mod_settings ) {
		$stashed_myskin_mod_settings = get_option( 'customize_stashed_myskin_mods' );
		if ( empty( $stashed_myskin_mod_settings ) ) {
			$stashed_myskin_mod_settings = array();
		}

		// Delete any stashed myskin mods for the active myskin since they would have been loaded and saved upon activation.
		unset( $stashed_myskin_mod_settings[ $this->get_stylesheet() ] );

		// Merge inactive myskin mods with the stashed myskin mod settings.
		foreach ( $inactive_myskin_mod_settings as $stylesheet => $myskin_mod_settings ) {
			if ( ! isset( $stashed_myskin_mod_settings[ $stylesheet ] ) ) {
				$stashed_myskin_mod_settings[ $stylesheet ] = array();
			}

			$stashed_myskin_mod_settings[ $stylesheet ] = array_merge(
				$stashed_myskin_mod_settings[ $stylesheet ],
				$myskin_mod_settings
			);
		}

		$autoload = false;
		$result = update_option( 'customize_stashed_myskin_mods', $stashed_myskin_mod_settings, $autoload );
		if ( ! $result ) {
			return false;
		}
		return $stashed_myskin_mod_settings;
	}
	...
	public function refresh_nonces() {
		...
		mcms_send_json_success( $this->get_nonces() );
	}
	...
	public function handle_dismiss_autosave_or_lock_request() {
		// Calls to dismiss_user_auto_draft_changesets() and mcms_get_post_autosave() require non-zero get_current_user_id().
		if ( ! is_user_logged_in() ) {
			mcms_send_json_error( 'unauthenticated', 401 );
		}
		...
		if ( ! check_ajax_referer( 'customize_dismiss_autosave_or_lock', 'nonce', false ) ) {
			mcms_send_json_error( 'invalid_nonce', 403 );
		}
		...
		if ( $dismiss_autosave ) {
			if ( empty( $changeset_post_id ) || 'auto-draft' === get_post_status( $changeset_post_id ) ) {
				...
				if ( $dismissed > 0 ) {
					mcms_send_json_success( 'auto_draft_dismissed' );
				} else {
					mcms_send_json_error( 'no_auto_draft_to_delete', 404 );
				}
			} else {
				$revision = mcms_get_post_autosave( $changeset_post_id, get_current_user_id() );

				if ( $revision ) {
					if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->delete_post, $changeset_post_id ) ) {
						mcms_send_json_error( 'cannot_delete_autosave_revision', 403 );
					}

					if ( ! mcms_delete_post( $revision->ID, true ) ) {
						mcms_send_json_error( 'autosave_revision_deletion_failure', 500 );
					} else {
						mcms_send_json_success( 'autosave_revision_deleted' );
					}
				} else {
					mcms_send_json_error( 'no_autosave_revision_to_delete', 404 );
				}
			}
		}

		mcms_send_json_error( 'unknown_error', 500 );
	}
	 ...
	 *
	 * @since 3.4.0
	 * @since 4.5.0 Return added MCMS_Customize_Setting instance.
	 *
	 * @param MCMS_Customize_Setting|string $id   Customize Setting object, or ID.
	 * @param array                       $args {
	 *  Optional. Array of properties for the new MCMS_Customize_Setting. Default empty array.
	 *
	 *  @type string       $type                  Type of the setting. Default 'myskin_mod'.
	 *                                            Default 160.
	 *  @type string       $capability            Capability required for the setting. Default 'edit_myskin_options'
	 *  @type string|array $myskin_supports        MySkin features required to support the panel. Default is none.
	 *  @type string       $default               Default value for the setting. Default is empty string.
	 *  @type string       $transport             Options for rendering the live preview of changes in MySkin Customizer.
	 *                                            Using 'refresh' makes the change visible by reloading the whole preview.
	 *                                            Using 'postMessage' allows a custom JavaScript to handle live changes.
	 *                                            @link https://developer.mandarincms.com/myskins/customize-api
	 *                                            Default is 'refresh'
	 ...
	 * }
	 * @return MCMS_Customize_Setting             The instance of the setting that was added.
	 */
	public function add_setting( $id, $args = array() ) {
		if ( $id instanceof MCMS_Customize_Setting ) {
			$setting = $id;
		} else {
			$class = 'MCMS_Customize_Setting';
	 ...
	 * on the front end and have been sent to MandarinCMS in `$_POST['customized']`. When MCMS
	 ...
	 * @return array The MCMS_Customize_Setting objects added.
	 */
	public function add_dynamic_settings( $setting_ids ) {
		...
		foreach ( $setting_ids as $setting_id ) {
			...
			$setting_args = false;
			$setting_class = 'MCMS_Customize_Setting';
			 ...
			 * the MCMS_Customize_Setting constructor.
			 *
			 * @since 4.2.0
			 *
			 * @param false|array $setting_args The arguments to the MCMS_Customize_Setting constructor.
			...
			/**
			 * Allow non-statically created settings to be constructed with custom MCMS_Customize_Setting subclass.
			 *
			 * @since 4.2.0
			 *
			 * @param string $setting_class MCMS_Customize_Setting or a subclass.
			 * @param string $setting_id    ID for dynamic setting, usually coming from `$_POST['customized']`.
			 * @param array  $setting_args  MCMS_Customize_Setting or a subclass.
			 */
	 ...
	 * @return MCMS_Customize_Setting|void The setting, if set.
	 */
	 ...
	 *
	 * @since 4.0.0
	 * @since 4.5.0 Return added MCMS_Customize_Panel instance.
	 *
	 * @param MCMS_Customize_Panel|string $id   Customize Panel object, or Panel ID.
	 * @param array                     $args {
	 *  Optional. Array of properties for the new Panel object. Default empty array.
	 *  @type int          $priority              Priority of the panel, defining the display order of panels and sections.
	 *                                            Default 160.
	 *  @type string       $capability            Capability required for the panel. Default `edit_myskin_options`
	 *  @type string|array $myskin_supports        MySkin features required to support the panel.
	 *  @type string       $title                 Title of the panel to show in UI.
	 *  @type string       $description           Description to show in the UI.
	 *  @type string       $type                  Type of the panel.
	 *  @type callable     $active_callback       Active callback.
	 * }
	 * @return MCMS_Customize_Panel             The instance of the panel that was added.
	 */
	public function add_panel( $id, $args = array() ) {
		if ( $id instanceof MCMS_Customize_Panel ) {
			$panel = $id;
		} else {
			$panel = new MCMS_Customize_Panel( $this, $id, $args );
		}
	 ...
	 * @return MCMS_Customize_Panel|void Requested panel instance, if set.
	 */
	...
	public function remove_panel( $id ) {
		...
		if ( in_array( $id, $this->components, true ) ) {
			...
			$message = sprintf( __( 'Removing %1$s manually will cause PHP warnings. Use the %2$s filter instead.' ),
				$id,
				'<a href="' . esc_url( 'https://developer.mandarincms.com/reference/hooks/customize_loaded_components/' ) . '"><code>customize_loaded_components</code></a>'
			);
	 ...
	 *
	 * @param string $panel Name of a custom panel which is a subclass of MCMS_Customize_Panel.
	 */
	 ...
	 *
	 * @since 3.4.0
	 * @since 4.5.0 Return added MCMS_Customize_Section instance.
	 *
	 * @param MCMS_Customize_Section|string $id   Customize Section object, or Section ID.
	 * @param array                     $args {
	 ...
	 *  @type string       $capability            Capability required for the section. Default 'edit_myskin_options'
	 *  @type string|array $myskin_supports        MySkin features required to support the section.
	 ...
	 * }
	 * @return MCMS_Customize_Section             The instance of the section that was added.
	 */
	public function add_section( $id, $args = array() ) {
		if ( $id instanceof MCMS_Customize_Section ) {
			$section = $id;
		} else {
			$section = new MCMS_Customize_Section( $this, $id, $args );
		}
	 ...
	 * @return MCMS_Customize_Section|void The section, if set.
	 */
	 ...
	 *
	 * @param string $section Name of a custom section which is a subclass of MCMS_Customize_Section.
	 */
	 ...
	 *
	 * @since 3.4.0
	 * @since 4.5.0 Return added MCMS_Customize_Control instance.
	 *
	 * @param MCMS_Customize_Control|string $id   Customize Control object, or ID.
	 * @param array                       $args {
	 ...
	 *                                            IDs in the array correspond to the ID of a registered `MCMS_Customize_Setting`.
	 ...
	 * }
	 * @return MCMS_Customize_Control             The instance of the control that was added.
	 */
	public function add_control( $id, $args = array() ) {
		if ( $id instanceof MCMS_Customize_Control ) {
			$control = $id;
		} else {
			$control = new MCMS_Customize_Control( $this, $id, $args );
		}
	 ...
	 * @return MCMS_Customize_Control|void The control object, if set.
	 */
	...
	public function render_control_templates() {
				...
				<div class="notification-message customize-changeset-locked-message">
					...
					<p class="action-buttons">
						...
						<# if ( data.allowOverride ) { #>
							<button class="button button-primary mcms-tab-last customize-notice-take-over-button"><?php _e( 'Take over' ); ?></button>
						<# } #>
					</p>
				</div>
			</li>
		...
		<?php
		/* The following template is obsolete in core but retained for modules. */
		?>
	 ...
	 *
	 * @since 3.4.0
	 * @deprecated 4.7.0 Use mcms_list_sort()
	 *
	 * @param MCMS_Customize_Panel|MCMS_Customize_Section|MCMS_Customize_Control $a Object A.
	 * @param MCMS_Customize_Panel|MCMS_Customize_Section|MCMS_Customize_Control $b Object B.
	 * @return int
	 */
	protected function _cmp_priority( $a, $b ) {
		_deprecated_function( __METHOD__, '4.7.0', 'mcms_list_sort' );
	...
	public function prepare_controls() {
		...
		$controls = array();
		$this->controls = mcms_list_sort( $this->controls, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		...
		// Prepare sections.
		$this->sections = mcms_list_sort( $this->sections, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		$sections = array();
		...
		foreach ( $this->sections as $section ) {
			...
			$section->controls = mcms_list_sort( $section->controls, array(
				'priority'        => 'ASC',
				'instance_number' => 'ASC',
			) );
		...
		// Prepare panels.
		$this->panels = mcms_list_sort( $this->panels, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		$panels = array();
		...
		foreach ( $this->panels as $panel ) {
			...
			$panel->sections = mcms_list_sort( $panel->sections, array(
				'priority'        => 'ASC',
				'instance_number' => 'ASC',
			), 'ASC', true );
		...
		$this->containers = mcms_list_sort( $this->containers, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
	}
	...
	public function enqueue_control_scripts() {
		...
		if ( ! is_multisite() && ( current_user_can( 'install_myskins' ) || current_user_can( 'update_myskins' ) || current_user_can( 'delete_myskins' ) ) ) {
	...
	public function is_ios() {
		return mcms_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] );
	}
	...
	public function set_preview_url( $preview_url ) {
		...
		$this->preview_url = mcms_validate_redirect( $preview_url, home_url( '/' ) );
	}
	...
	public function is_cross_domain() {
		$admin_origin = mcms_parse_url( admin_url() );
		$home_origin = mcms_parse_url( home_url() );
	 ...
	 * ssl certs. Domain mapping modules can allow other urls in these conditions
	...
	public function set_return_url( $return_url ) {
		...
		$return_url = remove_query_arg( mcms_removable_query_args(), $return_url );
		$return_url = mcms_validate_redirect( $return_url );
	...
	public function set_autofocus( $autofocus ) {
		$this->autofocus = array_filter( mcms_array_slice_assoc( $autofocus, array( 'panel', 'section', 'control' ) ), 'is_string' );
	}
	...
	public function get_nonces() {
		$nonces = array(
			'save' => mcms_create_nonce( 'save-customize_' . $this->get_stylesheet() ),
			'preview' => mcms_create_nonce( 'preview-customize_' . $this->get_stylesheet() ),
			'switch_myskins' => mcms_create_nonce( 'switch_myskins' ),
			'dismiss_autosave_or_lock' => mcms_create_nonce( 'customize_dismiss_autosave_or_lock' ),
			'override_lock' => mcms_create_nonce( 'customize_override_changeset_lock' ),
			'trash' => mcms_create_nonce( 'trash_customize_changeset' ),
		);
	...
	public function customize_pane_settings() {
		...
		if ( ! $this->saved_starter_content_changeset && ! $this->autosaved() ) {
			if ( $changeset_post_id ) {
				if ( is_user_logged_in() ) {
					$autosave_revision_post = mcms_get_post_autosave( $changeset_post_id, get_current_user_id() );
				}
			} else {
		...
		$settings = array(
			...
			'myskin'    => array(
				'stylesheet'  => $this->get_stylesheet(),
				'active'      => $this->is_myskin_active(),
				'_canInstall' => current_user_can( 'install_myskins' ),
			),
			'url'      => array(
			...
			'l10n' => array(
				'confirmDeleteMySkin' => __( 'Are you sure you want to delete this myskin?' ),
				/* translators: %d: number of myskin search results, which cannot currently consider singular vs. plural forms */
				'myskinSearchResults' => __( '%d myskins found' ),
				/* translators: %d: number of myskins being displayed, which cannot currently consider singular vs. plural forms */
				'announceMySkinCount' => __( 'Displaying %d myskins' ),
				/* translators: %s: myskin name */
				'announceMySkinDetails' => __( 'Showing details for myskin: %s' ),
			),
		);
		...
		if ( 'direct' !== $filesystem_method && ! $filesystem_credentials_are_stored ) {
			$settings['myskin']['_filesystemCredentialsNeeded'] = true;
		}
		...
		<script type="text/javascript">
			var _mcmsCustomizeSettings = <?php echo mcms_json_encode( $settings ); ?>;
			_mcmsCustomizeSettings.initialClientTimestamp = _.now();
			_mcmsCustomizeSettings.controls = {};
			_mcmsCustomizeSettings.settings = {};
			<?php
			...
			foreach ( $this->settings() as $setting ) {
				if ( $setting->check_capabilities() ) {
					printf(
						"s[%s] = %s;\n",
						mcms_json_encode( $setting->id ),
						mcms_json_encode( $setting->json() )
					);
				}
			}
			echo "})( _mcmsCustomizeSettings.settings );\n";
			...
			foreach ( $this->controls() as $control ) {
				if ( $control->check_capabilities() ) {
					printf(
						"c[%s] = %s;\n",
						mcms_json_encode( $control->id ),
						mcms_json_encode( $control->json() )
					);
				}
			}
			echo "})( _mcmsCustomizeSettings.controls );\n";
		?>
		</script>
		<?php
	}
	...
	public function get_previewable_devices() {
		 ...
		 * @since 4.5.0
		 *
		 * @see MCMS_Customize_Manager::get_previewable_devices()
		 *
	...
	public function register_controls() {
		...
		/* MySkins (controls are loaded via ajax) */

		$this->add_panel( new MCMS_Customize_MySkins_Panel( $this, 'myskins', array(
			'title'       => $this->myskin()->display( 'Name' ),
			'description' => (
				'<p>' . __( 'Looking for a myskin? You can search or browse the MandarinCMS.org myskin directory, install and preview myskins, then activate them right here.' ) . '</p>' .
				'<p>' . __( 'While previewing a new myskin, you can continue to tailor things like widgets and menus, and explore myskin-specific options.' ) . '</p>'
			),
			'capability'  => 'switch_myskins',
			'priority'    => 0,
		) ) );

		$this->add_section( new MCMS_Customize_MySkins_Section( $this, 'installed_myskins', array(
			'title'       => __( 'Installed myskins' ),
			'action'      => 'installed',
			'capability'  => 'switch_myskins',
			'panel'       => 'myskins',
			'priority'    => 0,
		...
		if ( ! is_multisite() ) {
			$this->add_section( new MCMS_Customize_MySkins_Section( $this, 'mcmsorg_myskins', array(
				'title'       => __( 'MandarinCMS.org myskins' ),
				'action'      => 'mcmsorg',
				'filter_type' => 'remote',
				'capability'  => 'install_myskins',
				'panel'       => 'myskins',
				'priority'    => 5,
			) ) );
		}

		// MySkins Setting (unused - the myskin is considerably more fundamental to the Customizer experience).
		$this->add_setting( new MCMS_Customize_Filter_Setting( $this, 'active_myskin', array(
			'capability' => 'switch_myskins',
		) ) );
		...
		// Add a setting to hide header text if the myskin doesn't support custom headers.
		if ( ! current_myskin_supports( 'custom-header', 'header-text' ) ) {
			$this->add_setting( 'header_text', array(
				'myskin_supports'    => array( 'custom-logo', 'header-text' ),
				'default'           => 1,
		...
		$this->add_control( new MCMS_Customize_Site_Icon_Control( $this, 'site_icon', array(
			'label'       => __( 'Site Icon' ),
			'description' => sprintf(
				'<p>' . __( 'Site Icons are what you see in browser tabs, bookmark bars, and within the MandarinCMS mobile apps. Upload one here!' ) . '</p>' .
		...
		$this->add_setting( 'custom_logo', array(
			'myskin_supports' => array( 'custom-logo' ),
			'transport'      => 'postMessage',
		) );

		$custom_logo_args = get_myskin_support( 'custom-logo' );
		$this->add_control( new MCMS_Customize_Cropped_Image_Control( $this, 'custom_logo', array(
			'label'         => __( 'Logo' ),
			'section'       => 'title_tagline',
			'priority'      => 8,
		...
		$this->add_setting( 'header_textcolor', array(
			'myskin_supports' => array( 'custom-header', 'header-text' ),
			'default'        => get_myskin_support( 'custom-header', 'default-text-color' ),
		...
		$this->add_control( new MCMS_Customize_Color_Control( $this, 'header_textcolor', array(
		...
		$this->add_setting( 'background_color', array(
			'default'        => get_myskin_support( 'custom-background', 'default-color' ),
			'myskin_supports' => 'custom-background',

			'sanitize_callback'    => 'sanitize_hex_color_no_hash',
			'sanitize_js_callback' => 'maybe_hash_hex_color',
		) );

		$this->add_control( new MCMS_Customize_Color_Control( $this, 'background_color', array(
		...
		if ( current_myskin_supports( 'custom-header', 'video' ) ) {
			$title = __( 'Header Media' );
			$description = '<p>' . __( 'If you add a video, the image will be used as a fallback while the video loads.' ) . '</p>';

			$width = absint( get_myskin_support( 'custom-header', 'width' ) );
			$height = absint( get_myskin_support( 'custom-header', 'height' ) );
			if ( $width && $height ) {
				$control_description = sprintf(
					/* translators: 1: .mp4, 2: header size in pixels */
					__( 'Upload your video in %1$s format and minimize its file size for best results. Your myskin recommends dimensions of %2$s pixels.' ),
					'<code>.mp4</code>',
				...
				$control_description = sprintf(
					...
					__( 'Upload your video in %1$s format and minimize its file size for best results. Your myskin recommends a width of %2$s pixels.' ),
					'<code>.mp4</code>',
				...
				$control_description = sprintf(
					...
					__( 'Upload your video in %1$s format and minimize its file size for best results. Your myskin recommends a height of %2$s pixels.' ),
					'<code>.mp4</code>',
		...
		$this->add_section( 'header_image', array(
			...
			'myskin_supports' => 'custom-header',
			'priority'       => 60,
		) );

		$this->add_setting( 'header_video', array(
			'myskin_supports'    => array( 'custom-header', 'video' ),
			'transport'         => 'postMessage',
			'sanitize_callback' => 'absint',
		...
		$this->add_setting( new MCMS_Customize_Filter_Setting( $this, 'header_image', array(
			'default'        => sprintf( get_myskin_support( 'custom-header', 'default-image' ), get_template_directory_uri(), get_stylesheet_directory_uri() ),
			'myskin_supports' => 'custom-header',
		) ) );

		$this->add_setting( new MCMS_Customize_Header_Image_Setting( $this, 'header_image_data', array(
			'myskin_supports' => 'custom-header',
		) ) );
		...
		$this->add_control( new MCMS_Customize_Media_Control( $this, 'header_video', array(
			'myskin_supports' => array( 'custom-header', 'video' ),
		...
		$this->add_control( new MCMS_Customize_Header_Image_Control( $this ) );

		$this->selective_refresh->add_partial( 'custom_header', array(
			'selector'            => '#mcms-custom-header',
		...
		$this->add_setting( 'background_image', array(
			'default'        => get_myskin_support( 'custom-background', 'default-image' ),
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) );

		$this->add_setting( new MCMS_Customize_Background_Image_Setting( $this, 'background_image_thumb', array(
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) ) );

		$this->add_control( new MCMS_Customize_Background_Image_Control( $this ) );

		$this->add_setting( 'background_preset', array(
			'default'        => get_myskin_support( 'custom-background', 'default-preset' ),
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) );
		...
		$this->add_setting( 'background_position_x', array(
			'default'        => get_myskin_support( 'custom-background', 'default-position-x' ),
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) );

		$this->add_setting( 'background_position_y', array(
			'default'        => get_myskin_support( 'custom-background', 'default-position-y' ),
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) );

		$this->add_control( new MCMS_Customize_Background_Position_Control( $this, 'background_position', array(
		...
		$this->add_setting( 'background_size', array(
			'default'        => get_myskin_support( 'custom-background', 'default-size' ),
			'myskin_supports' => 'custom-background',
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
		) );
		...
		$this->add_setting( 'background_repeat', array(
			'default'           => get_myskin_support( 'custom-background', 'default-repeat' ),
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
			'myskin_supports'    => 'custom-background',
		) );
		...
		$this->add_setting( 'background_attachment', array(
			'default'           => get_myskin_support( 'custom-background', 'default-attachment' ),
			'sanitize_callback' => array( $this, '_sanitize_background_setting' ),
			'myskin_supports'    => 'custom-background',
		) );
		...
		// If the myskin is using the default background callback, we can update
		// the background CSS using postMessage.
		if ( get_myskin_support( 'custom-background', 'mcms-head-callback' ) === '_custom_background_cb' ) {
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.mandarincms.com/ticket/19627 which introduces the static-front-page myskin_support.
		...
		$section_description .= sprintf(
			...
			esc_url( __( 'https://dev.mandarincms.com/CSS' ) ),
		...
		if ( 'false' !== mcms_get_current_user()->syntax_highlighting ) {
			$section_description .= '<p>';
			$section_description .= sprintf(
		...
		$custom_css_setting = new MCMS_Customize_Custom_CSS_Setting( $this, sprintf( 'custom_css[%s]', get_stylesheet() ), array(
			'capability' => 'edit_css',
			'default' => '',
		) );
		$this->add_setting( $custom_css_setting );

		$this->add_control( new MCMS_Customize_Code_Editor_Control( $this, 'custom_css', array(
			'label'       => __( 'CSS code' ),
			'section'     => 'custom_css',
	...
	/**
	 * Load myskins into the myskin browsing/installation UI.
	 *
	 * @since 4.9.0
	 */
	public function handle_load_myskins_request() {
		check_ajax_referer( 'switch_myskins', 'nonce' );

		if ( ! current_user_can( 'switch_myskins' ) ) {
			mcms_die( -1 );
		}

		if ( empty( $_POST['myskin_action'] ) ) {
			mcms_send_json_error( 'missing_myskin_action' );
		}
		$myskin_action = sanitize_key( $_POST['myskin_action'] );
		$myskins = array();
		$args = array();
		...
		if ( ! array_key_exists( 'search', $_POST ) ) {
			...
			$args['search'] = sanitize_text_field( mcms_unslash( $_POST['search'] ) );
		}

		if ( ! array_key_exists( 'tags', $_POST ) ) {
			$args['tag'] = '';
		} else {
			$args['tag'] = array_map( 'sanitize_text_field', mcms_unslash( (array) $_POST['tags'] ) );
		}
		...
		if ( 'installed' === $myskin_action ) {

			// Load all installed myskins from mcms_prepare_myskins_for_js().
			$myskins = array( 'myskins' => mcms_prepare_myskins_for_js() );
			foreach ( $myskins['myskins'] as &$myskin ) {
				$myskin['type'] = 'installed';
				$myskin['active'] = ( isset( $_POST['customized_myskin'] ) && $_POST['customized_myskin'] === $myskin['id'] );
			}

		} elseif ( 'mcmsorg' === $myskin_action ) {

			// Load MandarinCMS.org myskins from the .org API and normalize data to match installed myskin objects.
			...
			$mcmsorg_args = array(
				...
				'fields' => array(
					...
					'tags' => true,
					'parent' => true,
					// 'extended_author' => true, @todo: MandarinCMS.org throws a 500 server error when this is here.
				),
			);

			$args = array_merge( $mcmsorg_args, $args );

			if ( '' === $args['search'] && '' === $args['tag'] ) {
				$args['browse'] = 'new'; // Sort by latest myskins by default.
			}

			// Load myskins from the .org API.
			$myskins = myskins_api( 'query_myskins', $args );
			if ( is_mcms_error( $myskins ) ) {
				mcms_send_json_error();
			}

			// This list matches the allowed tags in mcms-admin/includes/myskin-install.php.
			$myskins_allowedtags = array_fill_keys(
				array( 'a', 'abbr', 'acronym', 'code', 'pre', 'em', 'strong', 'div', 'p', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'img' ),
				array()
			);
			$myskins_allowedtags['a'] = array_fill_keys( array( 'href', 'title', 'target' ), true );
			$myskins_allowedtags['acronym']['title'] = true;
			$myskins_allowedtags['abbr']['title'] = true;
			$myskins_allowedtags['img'] = array_fill_keys( array( 'src', 'class', 'alt' ), true );

			// Prepare a list of installed myskins to check against before the loop.
			$installed_myskins = array();
			$mcms_myskins = mcms_get_myskins();
			foreach ( $mcms_myskins as $myskin ) {
				$installed_myskins[] = $myskin->get_stylesheet();
			}
			$update_php = network_admin_url( 'update.php?action=install-myskin' );

			// Set up properties for myskins available on MandarinCMS.org.
			foreach ( $myskins->myskins as &$myskin ) {
				...
				$myskin->tags        = implode( ', ', $myskin->tags );
				$myskin->stars       = mcms_star_rating( array(
					'rating' => $myskin->rating,
					'type' => 'percent',
					'number' => $myskin->num_ratings,
					'echo' => false,
				) );
				$myskin->num_ratings = number_format_i18n( $myskin->num_ratings );
				$myskin->preview_url = set_url_scheme( $myskin->preview_url );

				// Handle myskins that are already installed as installed myskins.
				if ( in_array( $myskin->slug, $installed_myskins, true ) ) {
					$myskin->type = 'installed';
				} else {
					$myskin->type = $myskin_action;
				}

				// Set active based on customized myskin.
				$myskin->active = ( isset( $_POST['customized_myskin'] ) && $_POST['customized_myskin'] === $myskin->slug );

				// Map available myskin properties to installed myskin properties.
				$myskin->id           = $myskin->slug;
				$myskin->screenshot   = array( $myskin->screenshot_url );
				$myskin->authorAndUri = $myskin->author;
				// The .org API can return the full parent myskin details if passed the 'parent' arg, or if passed the 'template' option it'll return that in the event it's a child myskin.
				if ( isset( $myskin->parent ) ) {
					$myskin->parent = $myskin->parent['slug'];
				} else {
					$myskin->parent = false;
				}
				unset( $myskin->slug );
				unset( $myskin->screenshot_url );
				unset( $myskin->author );
			} // End foreach().
		} // End if().

		/**
		 * Filters the myskin data loaded in the customizer.
		 *
		 * This allows myskin data to be loading from an external source,
		 * or modification of data loaded from `mcms_prepare_myskins_for_js()`
		 * or MandarinCMS.org via `myskins_api()`.
		 *
		 * @since 4.9.0
		 *
		 * @see mcms_prepare_myskins_for_js()
		 * @see myskins_api()
		 * @see MCMS_Customize_Manager::__construct()
		 *
		 * @param array                $myskins  Nested array of myskin data.
		 * @param array                $args    List of arguments, such as page, search term, and tags to query for.
		 * @param MCMS_Customize_Manager $manager Instance of Customize manager.
		 */
		$myskins = apply_filters( 'customize_load_myskins', $myskins, $args, $this );

		mcms_send_json_success( $myskins );
	}
	...
	public function _sanitize_header_textcolor( $color ) {
		...
		if ( empty( $color ) )
			$color = get_myskin_support( 'custom-header', 'default-text-color' );
	 ...
	 * @param MCMS_Customize_Setting $setting Setting.
	 * @return string|MCMS_Error Background value or validation error.
	 */
	public function _sanitize_background_setting( $value, $setting ) {
		if ( 'background_repeat' === $setting->id ) {
			if ( ! in_array( $value, array( 'repeat-x', 'repeat-y', 'repeat', 'no-repeat' ) ) ) {
				return new MCMS_Error( 'invalid_value', __( 'Invalid value for background repeat.' ) );
			}
		} elseif ( 'background_attachment' === $setting->id ) {
			if ( ! in_array( $value, array( 'fixed', 'scroll' ) ) ) {
				return new MCMS_Error( 'invalid_value', __( 'Invalid value for background attachment.' ) );
			}
		} elseif ( 'background_position_x' === $setting->id ) {
			if ( ! in_array( $value, array( 'left', 'center', 'right' ), true ) ) {
				return new MCMS_Error( 'invalid_value', __( 'Invalid value for background position X.' ) );
			}
		} elseif ( 'background_position_y' === $setting->id ) {
			if ( ! in_array( $value, array( 'top', 'center', 'bottom' ), true ) ) {
				return new MCMS_Error( 'invalid_value', __( 'Invalid value for background position Y.' ) );
			}
		} elseif ( 'background_size' === $setting->id ) {
			if ( ! in_array( $value, array( 'auto', 'contain', 'cover' ), true ) ) {
				return new MCMS_Error( 'invalid_value', __( 'Invalid value for background size.' ) );
			}
			...
			return new MCMS_Error( 'unrecognized_setting', __( 'Unrecognized background setting.' ) );
		}
		return $value;
	}
	 ...
	 * @param MCMS_Customize_Selective_Refresh $selective_refresh Selective refresh component.
	 ...
	 * argument. When MCMS no longer supports PHP 5.3, this method can be removed
	 * in favor of an anonymous function.
	 *
	 * @see MCMS_Customize_Manager::register_controls()
	 *
	 * @since 4.5.0
	 *
