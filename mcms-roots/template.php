 ...
 * extension. It will also check the parent myskin, if the file exists, with
...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof MCMS_Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
...
function get_author_template() {
	...
	if ( $author instanceof MCMS_User ) {
 ...
 *
 * Searches in the STYLESHEETPATH before TEMPLATEPATH and mcms-roots/myskin-compat
 * so that myskins which inherit from a parent myskin can just overload one file.
 *
 * @since 2.7.0
 *
...
function locate_template($template_names, $load = false, $require_once = true ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists(STYLESHEETPATH . '/' . $template_name)) {
			...
			break;
		} elseif ( file_exists( BASED_TREE_URI . MCMSINC . '/myskin-compat/' . $template_name ) ) {
			$located = BASED_TREE_URI . MCMSINC . '/myskin-compat/' . $template_name;
			break;
		}
	}
...
/**
 * Require the template file with MandarinCMS environment.
 *
 * The globals are set up for the template file to ensure that the MandarinCMS
...
function load_template( $_template_file, $require_once = true ) {
	global $posts, $post, $mcms_did_header, $mcms_query, $mcms_rewrite, $mcmsdb, $mcms_version, $mcms, $id, $comment, $user_ID;

	if ( is_array( $mcms_query->query_vars ) ) {
		extract( $mcms_query->query_vars, EXTR_SKIP );
	}
