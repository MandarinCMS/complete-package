<?php
/**
 * MandarinCMS Customize Setting classes
 *
 * @package MandarinCMS
...
class MCMS_Customize_Setting {
	...
	/**
	 * Feature a myskin is required to support to enable this setting.
	 *
	 * @since 3.4.0
	 * @var string
	 */
	public $myskin_supports = '';
	...
	/**
	 * Options for rendering the live preview of changes in MySkin Customizer.
	 *
	 ...
	 * @since 3.4.0
	 *
	 * @param MCMS_Customize_Manager $manager
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( 'option' === $this->type || 'myskin_mod' === $this->type ) {
		...
		switch ( $this->type ) {
			case 'myskin_mod' :
				if ( ! $is_multidimensional ) {
					add_filter( "myskin_mod_{$id_base}", array( $this, '_preview_filter' ) );
				} else {
					if ( empty( self::$aggregated_multidimensionals[ $this->type ][ $id_base ]['previewed_instances'] ) ) {
						// Only add this filter once for this ID base.
						add_filter( "myskin_mod_{$id_base}", $multidimensional_filter );
					}
			...
			default :
				...
				/**
				 * Fires when the MCMS_Customize_Setting::preview() method is called for settings
				 * not handled as myskin_mods or options.
				 *
				 * The dynamic portion of the hook name, `$this->id`, refers to the setting ID.
				 *
				 * @since 3.4.0
				 *
				 * @param MCMS_Customize_Setting $this MCMS_Customize_Setting instance.
				 */
	 ...
	 * that `MCMS_Customize_Setting::_multidimensional_preview_filter()`
	 * is called for this setting.
	 *
	 * @since 4.4.0
	 *
	 * @see MCMS_Customize_Manager::set_post_value()
	 * @see MCMS_Customize_Setting::_multidimensional_preview_filter()
	 */
	...
	/**
	 * Callback function to filter non-multidimensional myskin mods and options.
	 *
	...
	/**
	 * Callback function to filter multidimensional myskin mods and options.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @see MCMS_Customize_Setting::$aggregated_multidimensionals
	...
	/**
	 * Checks user capabilities and myskin supports, and then saves
	...
	final public function save() {
		...
		/**
		 * Fires when the MCMS_Customize_Setting::save() method is called.
		 *
	 ...
	 * @return string|array|null|MCMS_Error Sanitized value, or `null`/`MCMS_Error` if invalid.
	 */
	 ...
	 * @return true|MCMS_Error True if the input was validated, otherwise MCMS_Error.
	 */
	public function validate( $value ) {
		...
		if ( is_null( $value ) ) {
			return new MCMS_Error( 'invalid_value', __( 'Invalid value.' ) );
		}

		$validity = new MCMS_Error();

		/**
		 * Validates a Customize setting value.
		 *
		 * Modules should amend the `$validity` object via its `MCMS_Error::add()` method.
		 *
		 * The dynamic portion of the hook name, `$this->ID`, refers to the setting ID.
		 *
		 * @since 4.6.0
		 *
		 * @param MCMS_Error             $validity Filtered from `true` to `MCMS_Error` when invalid.
		 * @param mixed                $value    Value of the setting.
		 * @param MCMS_Customize_Setting $this     MCMS_Customize_Setting instance.
		 */
		$validity = apply_filters( "customize_validate_{$this->id}", $validity, $value, $this );

		if ( is_mcms_error( $validity ) && empty( $validity->errors ) ) {
			$validity = true;
		}
		return $validity;
	}
	...
	protected function get_root_value( $default = null ) {
		...
		} elseif ( 'myskin_mod' === $this->type ) {
			return get_myskin_mod( $id_base, $default );
		} else {
			/*
			 * Any MCMS_Customize_Setting subclass implementing aggregate multidimensional
	...
	protected function update( $value ) {
		...
		if ( 'option' === $this->type || 'myskin_mod' === $this->type ) {
			...
			/**
			 * Fires when the MCMS_Customize_Setting::update() method is called for settings
			 * not handled as myskin_mods or options.
			 *
	 ...
	 */
	protected function _update_myskin_mod() {
	...
	public function value() {
		$id_base = $this->id_data['base'];
		$is_core_type = ( 'option' === $this->type || 'myskin_mod' === $this->type );
		...
		if ( ! $is_core_type && ! $this->is_multidimensional_aggregated ) {
			...
			/**
			 * Filters a Customize setting value not handled as a myskin_mod or option.
			 *
			 * The dynamic portion of the hook name, `$id_base`, refers to
			 * the base slug of the setting name, initialized from `$this->id_data['base']`.
			 *
			 * For settings handled as myskin_mods or options, see those corresponding
			 ...
			 * @param MCMS_Customize_Setting $this    The setting instance.
			 */
	...
	/**
	 * Validate user capabilities whether the myskin supports the setting.
	 *
	 * @since 3.4.0
	 *
	 * @return bool False if myskin doesn't support the setting or user can't change setting, otherwise true.
	 */
	final public function check_capabilities() {
		if ( $this->capability && ! call_user_func_array( 'current_user_can', (array) $this->capability ) )
			return false;

		if ( $this->myskin_supports && ! call_user_func_array( 'current_myskin_supports', (array) $this->myskin_supports ) )
			return false;
