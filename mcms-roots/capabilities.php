...
function map_meta_cap( $cap, $user_id ) {
	...
	case 'delete_page':
		...
		if ( $post->post_author && $user_id == $post->post_author ) {
			...
			if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
				...
				$status = get_post_meta( $post->ID, '_mcms_trash_meta_status', true );
		 ...
		 */
		if ( (int) get_option( 'mcms_page_for_privacy_policy' ) === $post->ID ) {
	...
	case 'edit_myskins':
		...
		if ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT )
			$caps[] = 'do_not_allow';
		elseif ( ! mcms_is_file_mod_allowed( 'capability_edit_myskins' ) )
			$caps[] = 'do_not_allow';
		elseif ( is_multisite() && ! is_super_admin( $user_id ) )
			$caps[] = 'do_not_allow';
		else
	...
	case 'update_myskins':
	case 'delete_myskins':
	case 'install_myskins':
	case 'upload_myskins':
	case 'update_core':
		// Disallow anything that creates, deletes, or updates core, module, or myskin files.
		// Files in uploads are excepted.
		if ( ! mcms_is_file_mod_allowed( 'capability_update_core' ) ) {
			$caps[] = 'do_not_allow';
		} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
			$caps[] = 'do_not_allow';
		} elseif ( 'upload_myskins' === $cap ) {
			$caps[] = 'install_myskins';
		} elseif ( 'upload_modules' === $cap ) {
			$caps[] = 'install_modules';
		} else {
			$caps[] = $cap;
		}
		break;
	...
	case 'deactivate_module':
		$caps[] = 'activate_modules';
		if ( is_multisite() ) {
			// update_, install_, and delete_ are handled above with is_super_admin().
			$menu_perms = get_site_option( 'menu_items', array() );
			if ( empty( $menu_perms['modules'] ) )
				$caps[] = 'manage_network_modules';
		}
		break;
	case 'delete_user':
	case 'delete_users':
	...
	case 'manage_network_modules':
	case 'manage_network_myskins':
 ...
 *                           "Meta" capabilities, e.g. 'edit_post', 'edit_user', etc., are capabilities used
 *                           by map_meta_cap() to map to other "primitive" capabilities, e.g. 'edit_posts',
 *                           'edit_others_posts', etc. Accessed via func_get_args() and passed to MCMS_User::has_cap(),
 *                           then map_meta_cap().
 ...
 * @since 3.1.0
 *
 * @param int|MCMS_User $user       User ID or object.
 * @param string      $capability Capability name.
 * @return bool Whether the user has the given capability.
 */
...
/**
 * Retrieves the global MCMS_Roles instance and instantiates it if necessary.
 *
 * @since 4.3.0
 *
 * @global MCMS_Roles $mcms_roles MCMS_Roles global instance.
 *
 * @return MCMS_Roles MCMS_Roles global instance if not already instantiated.
 */
function mcms_roles() {
	global $mcms_roles;
 ...
 * @return MCMS_Role|null MCMS_Role object if found, null if the role does not exist.
 */
function get_role( $role ) {
	return mcms_roles()->get_role( $role );
}
 ...
 * @return MCMS_Role|null MCMS_Role object if role is added, null if already exists.
 */
function add_role( $role, $display_name, $capabilities = array() ) {
	if ( empty( $role ) ) {
		return;
	}
	return mcms_roles()->add_role( $role, $display_name, $capabilities );
}
 ...
 *
 * A user must have at least one out of the 'update_core', 'install_modules', and
 * 'install_myskins' capabilities to qualify for 'install_languages'.
 *
 * @since 4.9.0
 *
 ...
 */
function mcms_maybe_grant_install_languages_cap( $allcaps ) {
	if ( ! empty( $allcaps['update_core'] ) || ! empty( $allcaps['install_modules'] ) || ! empty( $allcaps['install_myskins'] ) ) {
