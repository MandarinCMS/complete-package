<?php
/**
 * Customize API: MCMS_Customize_Nav_Menu_Item_Setting class
 *
 * @package MandarinCMS
 ...
 *
 * Subclass of MCMS_Customize_Setting to represent a nav_menu taxonomy term, and
 ...
 */
class MCMS_Customize_Nav_Menu_Item_Setting extends MCMS_Customize_Setting {
	 ...
	 * @var array
	 *
	 * @see mcms_setup_nav_menu_item()
	 */
	public $default = array(
		// The $menu_item_data for mcms_update_nav_menu_item().
		'object_id'        => 0,
		...
		'status'           => 'publish',
		'original_title'   => '',
		'nav_menu_term_id' => 0, // This will be supplied as the $menu_id arg for mcms_update_nav_menu_item().
		'_invalid'         => false,
	);
	...
	/**
	 * Storage of pre-setup menu item to prevent wasted calls to mcms_setup_nav_menu_item().
	 *
	 * @since 4.3.0
	 * @var array
	 */
	 ...
	 * @var int
	 *
	 * @see MCMS_Customize_Nav_Menu_Item_Setting::update()
	 * @see MCMS_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by mcms_update_nav_menu_item() when setting is updated.
	 *
	 * @since 4.3.0
	 * @var MCMS_Error
	 *
	 ...
	 * @since 4.3.0
	 *
	 * @param MCMS_Customize_Manager $manager Bootstrap Customizer instance.
	 * @param string               $id      An specific ID of the setting. Can be a
	 *                                      myskin mod or option name.
	 * @param array                $args    Optional. Setting arguments.
	 *
	 * @throws Exception If $id is not valid for this setting type.
	 */
	public function __construct( MCMS_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected MCMS_Customize_Manager::$nav_menus to be set.' );
		}
		...
		add_action( 'mcms_update_nav_menu_item', array( $this, 'flush_cached_value' ), 10, 2 );
		...
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->post_id > 0 ) {
				...
				if ( $post && self::POST_TYPE === $post->post_type ) {
					...
					$value = (array) mcms_setup_nav_menu_item( $post );
					if ( $is_title_empty ) {
						$value['title'] = '';
					}
				}
			...
			// Cache the value for future calls to avoid having to re-call mcms_setup_nav_menu_item().
			$this->value = $value;
			$this->populate_value();
			$value = $this->value;
		}
	...
	protected function get_original_title( $item ) {
		...
		if ( 'post_type' === $item->type && ! empty( $item->object_id ) ) {
			...
			if ( ! is_mcms_error( $original_term_title ) ) {
	 ...
	 *
	 * Translates some properties added by mcms_setup_nav_menu_item() and removes others.
	 *
	 * @since 4.3.0
	 *
	 * @see MCMS_Customize_Nav_Menu_Item_Setting::value()
	 */
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = mcms_get_post_terms( $this->post_id, MCMS_Customize_Nav_Menu_Setting::TAXONOMY, array(
				'fields' => 'ids',
			) );
			if ( ! empty( $menus ) ) {
	 ...
	 *
	 * @see MCMS_Customize_Manager::post_value()
	 *
	...
	public function preview() {
		...
		add_filter( 'mcms_get_nav_menu_items', array( $this, 'filter_mcms_get_nav_menu_items' ), 10, 3 );

		$sort_callback = array( __CLASS__, 'sort_mcms_get_nav_menu_items' );
		if ( ! has_filter( 'mcms_get_nav_menu_items', $sort_callback ) ) {
			add_filter( 'mcms_get_nav_menu_items', array( __CLASS__, 'sort_mcms_get_nav_menu_items' ), 1000, 3 );
		}

		// @todo Add get_post_metadata filters for modules to add their data.

		return true;
	}

	/**
	 * Filters the mcms_get_nav_menu_items() result to supply the previewed menu items.
	 *
	 * @since 4.3.0
	 *
	 * @see mcms_get_nav_menu_items()
	 *
	 ...
	 */
	public function filter_mcms_get_nav_menu_items( $items, $menu, $args ) {
		...
		if ( $should_update ) {
			foreach ( $items as $item ) {
				if ( $item->db_id === $this->post_id ) {
					foreach ( get_object_vars( $this->value_as_mcms_post_nav_menu_item() ) as $key => $value ) {
						$item->$key = $value;
					}
					$mutated = true;
				}
			...
			if ( ! $mutated ) {
				$items[] = $this->value_as_mcms_post_nav_menu_item();
			}
		}
	...
	/**
	 * Re-apply the tail logic also applied on $items by mcms_get_nav_menu_items().
	 *
	 * @since 4.3.0
	 * @static
	 *
	 ...
	 * @return array Array of menu items,
	 */
	public static function sort_mcms_get_nav_menu_items( $items, $menu, $args ) {
	...
	/**
	 * Get the value emulated into a MCMS_Post and set up as a nav_menu_item.
	 *
	 * @since 4.3.0
	 *
	 * @return MCMS_Post With mcms_setup_nav_menu_item() applied.
	 */
	public function value_as_mcms_post_nav_menu_item() {
		...
		$post = new MCMS_Post( (object) $item );
		...
		/** This filter is documented in mcms-roots/nav-menu.php */
		$post->description = apply_filters( 'nav_menu_description', mcms_trim_words( $post->description, 200 ) );

		/** This filter is documented in mcms-roots/nav-menu.php */
		$post = apply_filters( 'mcms_setup_nav_menu_item', $post );
	 ...
	 * Sanitize an input.
	 *
	 * Note that parent::sanitize() erroneously does mcms_unslash() on $value, but
	 * we remove that in this override.
	 *
	 * @since 4.3.0
	 *
	 * @param array $menu_item_value The value to sanitize.
	 * @return array|false|null|MCMS_Error Null or MCMS_Error if an input isn't valid. False if it is marked for deletion.
	...
	public function sanitize( $menu_item_value ) {
		...
		$menu_item_value = mcms_array_slice_assoc( $menu_item_value, array_keys( $default ) );
		...
		// Apply the same filters as when calling mcms_insert_post().

		/** This filter is documented in mcms-roots/post.php */
		$menu_item_value['title'] = mcms_unslash( apply_filters( 'title_save_pre', mcms_slash( $menu_item_value['title'] ) ) );

		/** This filter is documented in mcms-roots/post.php */
		$menu_item_value['attr_title'] = mcms_unslash( apply_filters( 'excerpt_save_pre', mcms_slash( $menu_item_value['attr_title'] ) ) );

		/** This filter is documented in mcms-roots/post.php */
		$menu_item_value['description'] = mcms_unslash( apply_filters( 'content_save_pre', mcms_slash( $menu_item_value['description'] ) ) );

		if ( '' !== $menu_item_value['url'] ) {
			$menu_item_value['url'] = esc_url_raw( $menu_item_value['url'] );
			if ( '' === $menu_item_value['url'] ) {
				return new MCMS_Error( 'invalid_url', __( 'Invalid URL.' ) ); // Fail sanitization if URL is invalid.
			}
		}
	 ...
	 *                           entirely. See MCMS_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = mcms_delete_post( $this->post_id, true );

				if ( false === $r ) {
					$this->update_error  = new MCMS_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof MCMS_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( $nav_menu_setting->previous_term_id !== intval( $value['nav_menu_term_id'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof MCMS_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( $parent_nav_menu_item_setting->previous_post_id !== intval( $value['menu_item_parent'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MCMS_Error( 'unexpected_previous_post_id' );
					return;
				}
			...
			$r = mcms_update_nav_menu_item(
				$value['nav_menu_term_id'],
				$is_placeholder ? 0 : $this->post_id,
				mcms_slash( $menu_item_data )
			);
	 ...
	 * @see MCMS_Customize_Nav_Menu_Item_Setting::update()
	 *
	 * @param array $data Additional information passed back to the 'saved' event on `mcms.customize`.
