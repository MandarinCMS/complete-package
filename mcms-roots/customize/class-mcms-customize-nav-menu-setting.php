<?php
/**
 * Customize API: MCMS_Customize_Nav_Menu_Setting class
 *
 * @package MandarinCMS
 ...
 * @since 4.3.0
 *
 * @see mcms_get_nav_menu_object()
 * @see MCMS_Customize_Setting
 */
class MCMS_Customize_Nav_Menu_Setting extends MCMS_Customize_Setting {
	 ...
	 * @var int
	 *
	 * @see MCMS_Customize_Nav_Menu_Setting::update()
	 * @see MCMS_Customize_Nav_Menu_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by mcms_update_nav_menu_object() when setting is updated.
	 *
	 * @since 4.3.0
	 * @var MCMS_Error
	 *
	...
	public function value() {
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->term_id > 0 ) {
				$term = mcms_get_nav_menu_object( $this->term_id );

				if ( $term ) {
					$value = mcms_array_slice_assoc( (array) $term, array_keys( $this->default ) );
	...
	public function preview() {
		...
		add_filter( 'mcms_get_nav_menus', array( $this, 'filter_mcms_get_nav_menus' ), 10, 2 );
		add_filter( 'mcms_get_nav_menu_object', array( $this, 'filter_mcms_get_nav_menu_object' ), 10, 2 );
	...
	/**
	 * Filters the mcms_get_nav_menus() result to ensure the inserted menu object is included, and the deleted one is removed.
	 *
	 * @since 4.3.0
	 *
	 * @see mcms_get_nav_menus()
	 *
	 * @param array $menus An array of menu objects.
	 * @param array $args  An array of arguments used to retrieve menu objects.
	 * @return array
	 */
	public function filter_mcms_get_nav_menus( $menus, $args ) {
		...
		if ( ! $is_delete && ! empty( $args['orderby'] ) ) {
			$menus = mcms_list_sort( $menus, array(
				$args['orderby'] => 'ASC',
			) );
		}
	 ...
	 * @var string
	 *
	 * @see MCMS_Customize_Nav_Menu_Setting::filter_mcms_get_nav_menus()
	 * @see MCMS_Customize_Nav_Menu_Setting::_sort_menus_by_orderby()
	 */
	...
	/**
	 * Filters the mcms_get_nav_menu_object() result to supply the previewed menu object.
	 *
	 ...
	 * @see mcms_get_nav_menu_object()
	 *
	 * @param object|null $menu_obj Object returned by mcms_get_nav_menu_object().
	 * @param string      $menu_id  ID of the nav_menu term. Requests by slug or name will be ignored.
	 * @return object|null
	 */
	public function filter_mcms_get_nav_menu_object( $menu_obj, $menu_id ) {
		$ok = (
	...
	public function sanitize( $value ) {
		...
		$value = mcms_array_slice_assoc( $value, array_keys( $default ) );

		$value['name']        = trim( esc_html( $value['name'] ) ); // This sanitization code is used in mcms-admin/nav-menus.php.
	 ...
	 * @since 4.3.0
	 *
	 * @see mcms_update_nav_menu_object()
	 *
	 * @param array|false $value {
	 *     The value to update. Note that slug cannot be updated via mcms_update_nav_menu_object().
	...
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = mcms_delete_nav_menu( $this->term_id );
			...
			$menu_data = mcms_array_slice_assoc( $value, array( 'description', 'parent' ) );
			$menu_data['menu-name'] = $value['name'];

			$menu_id = $is_placeholder ? 0 : $this->term_id;
			$r = mcms_update_nav_menu_object( $menu_id, mcms_slash( $menu_data ) );
			$original_name = $menu_data['menu-name'];
			$name_conflict_suffix = 1;
			while ( is_mcms_error( $r ) && 'menu_exists' === $r->get_error_code() ) {
				$name_conflict_suffix += 1;
		...
		if ( 'inserted' === $this->update_status ) {
			...
			foreach ( array_keys( $this->manager->unsanitized_post_values() ) as $setting_id ) {
				...
				$widget_instance = $nav_menu_widget_setting->post_value(); // Note that this calls MCMS_Customize_Widgets::sanitize_widget_instance().
	 ...
	 * @since 4.3.0
	 *
	 * @see MCMS_Customize_Nav_Menu_Setting::filter_nav_menu_options()
	 * @see MCMS_Customize_Nav_Menu_Setting::update()
	 *
