<?php
/**
 * REST API: MCMS_REST_Request class
 *
 * @package MandarinCMS
 * @subpackage REST_API
 * @since 4.4.0
 ...
 *
 * Note: When using features provided by ArrayAccess, be aware that MandarinCMS deliberately
 ...
 * MCMS_REST_Request::get_body_params(), MCMS_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
 * @see ArrayAccess
 */
class MCMS_REST_Request implements ArrayAccess {
	...
	protected function get_parameter_order() {
		 ...
		 * }
		 * @param MCMS_REST_Request $this The request object.
		 */
	 ...
	 * @return true|MCMS_Error True if the JSON data was passed or no JSON data was provided, MCMS_Error if invalid JSON was passed.
	 */
	protected function parse_json_params() {
		 ...
		 *
		 * Note that due to MCMS's JSON compatibility functions, json_last_error
		 * might not be defined: https://core.trac.mandarincms.com/ticket/27799
		 */
		if ( null === $params && ( ! function_exists( 'json_last_error' ) || JSON_ERROR_NONE !== json_last_error() ) ) {
			...
			return new MCMS_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	...
	protected function parse_body_params() {
		 ...
		 *
		 * NOTE: Do not refactor to use `mcms_unslash`.
		 */
	 ...
	 * @since 4.4.0
	 *
	 * @return true|MCMS_Error True if parameters were sanitized, MCMS_Error if an error occurred during sanitization.
	 */
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				if ( is_mcms_error( $sanitized_value ) ) {
		...
		if ( $invalid_params ) {
			return new MCMS_Error( 'rest_invalid_param', sprintf( __( 'Invalid parameter(s): %s' ), implode( ', ', array_keys( $invalid_params ) ) ), array( 'status' => 400, 'params' => $invalid_params ) );
		}
	 ...
	 * @since 4.4.0
	 *
	 * @return bool|MCMS_Error True if there are no parameters to validate or if all pass validation,
	 *                       MCMS_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		// If JSON data was passed, check for errors.
		$json_error = $this->parse_json_params();
		if ( is_mcms_error( $json_error ) ) {
			return $json_error;
		}
		...
		if ( ! empty( $required ) ) {
			return new MCMS_Error( 'rest_missing_callback_param', sprintf( __( 'Missing parameter(s): %s' ), implode( ', ', $required ) ), array( 'status' => 400, 'params' => $required ) );
		}
	...
	/**
	 * Retrieves a MCMS_REST_Request object from a full URL.
	 *
	 * @static
	 * @since 4.5.0
	 *
	 * @param string $url URL with protocol, domain, path and query args.
	 * @return MCMS_REST_Request|false MCMS_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			mcms_parse_str( $bits['query'], $query_params );
		}
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new MCMS_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param MCMS_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
