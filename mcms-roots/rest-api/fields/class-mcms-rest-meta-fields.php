<?php
/**
 * REST API: MCMS_REST_Meta_Fields class
 *
 * @package MandarinCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 */
abstract class MCMS_REST_Meta_Fields {
	 ...
	 * @return MCMS_Error|object Object containing the meta values by name, otherwise MCMS_Error object.
	 */
	 ...
	 * @param MCMS_REST_Request $request Current request object.
	 ...
	 * @param int             $object_id Object ID to fetch meta for.
	 * @return MCMS_Error|null MCMS_Error if one occurs, null on success.
	 */
	 ...
	 * @return bool|MCMS_Error True if meta field is deleted, MCMS_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! delete_metadata( $meta_type, $object_id, mcms_slash( $meta_key ) ) ) {
			return new MCMS_Error(
				'rest_meta_database_error',
				__( 'Could not delete meta value from database.' ),
				array( 'key' => $name, 'status' => MCMS_Http::INTERNAL_SERVER_ERROR )
			);
		}
	 ...
	 * @return bool|MCMS_Error True if meta fields are updated, MCMS_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, mcms_slash( $meta_key ), mcms_slash( $value ) ) ) {
				return new MCMS_Error(
					'rest_meta_database_error',
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, mcms_slash( $meta_key ), mcms_slash( $value ) ) ) {
				return new MCMS_Error(
					'rest_meta_database_error',
					__( 'Could not update meta value in database.' ),
					array( 'key' => $name, 'status' => MCMS_Http::INTERNAL_SERVER_ERROR )
	 ...
	 * @return bool|MCMS_Error True if the meta field was updated, MCMS_Error otherwise.
	 */
	protected function update_meta_value( $object_id, $meta_key, $name, $value ) {
		...
		$meta_key   = mcms_slash( $meta_key );
		$meta_value = mcms_slash( $value );
	 ...
	 * @return MCMS_Error|string The meta array, if valid, otherwise an error.
	 */
