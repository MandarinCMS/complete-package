 ...
 *
 * MCMS_Date_Query is a helper that allows primary query classes, such as MCMS_Query, to filter
 * their results by date columns, by generating `WHERE` subclauses to be attached to the
 * primary SQL query string.
 *
 * Attempting to filter by an invalid date value (eg month=13) will generate SQL that will
 * return no results. In these cases, a _doing_it_wrong() error notice is also thrown.
 * See MCMS_Date_Query::validate_date_values().
 *
 * @link https://dev.mandarincms.com/Function_Reference/MCMS_Query Codex page.
 *
 * @since 3.7.0
 */
class MCMS_Date_Query {
	/**
	 * Array of date queries.
	 *
	 * See MCMS_Date_Query::__construct() for information on date query arguments.
	 *
	 * @since 3.7.0
	 * @var array
	 */
	...
	public function validate_date_values( $date_query = array() ) {
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! mcms_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! mcms_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
	 ...
	 * known tables, and then, if found, have a table prefix (such as 'mcms_posts.') prepended.
	 * Prefixed column names (such as 'mcms_posts.post_date') bypass this whitelist check,
	...
	public function get_sql() {
		 ...
		 * @param MCMS_Date_Query $this  The MCMS_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public MCMS_Date_Query::get_sql(), this method is abstracted
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) ) {
			$where_parts[] = $mcmsdb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );
		}
		if ( ! empty( $query['before'] ) ) {
			$where_parts[] = $mcmsdb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		}
		...
		if ( isset( $query['week'] ) && false !== ( $value = $this->build_value( $compare, $query['week'] ) ) ) {
			$where_parts[] = _mcms_mysql_week( $column ) . " $compare $value";
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $mcmsdb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}
}
