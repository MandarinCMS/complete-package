<?php
/**
 * Comment API: MCMS_Comment_Query class
 *
 * @package MandarinCMS
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see MCMS_Comment_Query::__construct() for accepted arguments.
 */
class MCMS_Comment_Query {
	 ...
	 *     @type array        $date_query                Date query clauses to limit comments by. See MCMS_Date_Query.
	 *                                                   Default null.
	 ...
	 *                                                   See MCMS_Meta_Query. Default empty.
	 ...
	 *                                                   stored in a `children` property on the `MCMS_Comment`
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value' => '',
			'meta_query' => '',
			'date_query' => null, // See MCMS_Date_Query
			'hierarchical' => false,
			'cache_domain' => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from MCMS_Comment_Query::query().
	 *
	 *
	 * @param string|array $query MCMS_Comment_Query arguments. See MCMS_Comment_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = mcms_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.2.0
		 *
		 * @param MCMS_Comment_Query $this The MCMS_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_comment_query', array( &$this ) );
	}

	/**
	 * Sets up the MandarinCMS query for retrieving comments.
	 *
	 * @since 3.1.0
	...
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param MCMS_Comment_Query $this Current instance of MCMS_Comment_Query (passed by reference).
		 */
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_sql( 'comment', $mcmsdb->comments, 'comment_ID', $this );
		}
		 ...
		 */
		$_args = mcms_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) );
		unset( $_args['fields'] );

		$key = md5( serialize( $_args ) );
		$last_changed = mcms_cache_get_last_changed( 'comment' );

		$cache_key   = "get_comments:$key:$last_changed";
		$cache_value = mcms_cache_get( $cache_key, 'comment' );
		if ( false === $cache_value ) {
			...
			mcms_cache_add( $cache_key, $cache_value, 'comment' );
		} else {
		...
		// Convert to MCMS_Comment instances
	...
	protected function get_comment_ids() {
		...
		if ( ! in_array( 'any', $statuses ) ) {
			foreach ( $statuses as $status ) {
				switch ( $status ) {
					...
					default :
						$status_clauses[] = $mcmsdb->prepare( "comment_approved = %s", $status );
						break;
				}
			}
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			...
			foreach ( $include_unapproved as $unapproved_identifier ) {
				...
				if ( is_numeric( $unapproved_identifier ) ) {
					$approved_clauses[] = $mcmsdb->prepare( "( user_id = %d AND comment_approved = '0' )", $unapproved_identifier );

				// Otherwise we match against email addresses.
				} else {
					$approved_clauses[] = $mcmsdb->prepare( "( comment_author_email = %s AND comment_approved = '0' )", $unapproved_identifier );
				}
			}
		}
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			if ( empty( $orderby_array ) ) {
				$orderby_array[] = "$mcmsdb->comments.comment_date_gmt $order";
			}
			...
			if ( ! $found_orderby_comment_ID ) {
				...
				$orderby_array[] = "$mcmsdb->comments.comment_ID $comment_ID_order";
			}

			$orderby = implode( ', ', $orderby_array );
		} else {
			$orderby = "$mcmsdb->comments.comment_date_gmt $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$mcmsdb->comments.comment_ID";
		}

		$post_id = absint( $this->query_vars['post_id'] );
		if ( ! empty( $post_id ) ) {
			$this->sql_clauses['where']['post_id'] = $mcmsdb->prepare( 'comment_post_ID = %d', $post_id );
		}

		// Parse comment IDs for an IN clause.
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$mcmsdb->comments.comment_ID IN ( " . implode( ',', mcms_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$mcmsdb->comments.comment_ID NOT IN ( " . implode( ',', mcms_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}

		if ( '' !== $this->query_vars['author_email'] ) {
			$this->sql_clauses['where']['author_email'] = $mcmsdb->prepare( 'comment_author_email = %s', $this->query_vars['author_email'] );
		}

		if ( '' !== $this->query_vars['author_url'] ) {
			$this->sql_clauses['where']['author_url'] = $mcmsdb->prepare( 'comment_author_url = %s', $this->query_vars['author_url'] );
		}

		if ( '' !== $this->query_vars['karma'] ) {
			$this->sql_clauses['where']['karma'] = $mcmsdb->prepare( 'comment_karma = %d', $this->query_vars['karma'] );
		}
		...
		foreach ( $raw_types as $operator => $_raw_types ) {
			...
			foreach ( $_raw_types as $type ) {
				switch ( $type ) {
					...
					default:
						$comment_types[ $operator ][] = $mcmsdb->prepare( '%s', $type );
						break;
				}
			}
		...
		if ( '' !== $parent ) {
			$this->sql_clauses['where']['parent'] = $mcmsdb->prepare( 'comment_parent = %d', $parent );
		}

		if ( is_array( $this->query_vars['user_id'] ) ) {
			$this->sql_clauses['where']['user_id'] = 'user_id IN (' . implode( ',', array_map( 'absint', $this->query_vars['user_id'] ) ) . ')';
		} elseif ( '' !== $this->query_vars['user_id'] ) {
			$this->sql_clauses['where']['user_id'] = $mcmsdb->prepare( 'user_id = %d', $this->query_vars['user_id'] );
		}
		...
		$plucked = mcms_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $post_fields ) ) {
			...
			foreach ( $post_fields as $field_name => $field_value ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $mcmsdb->prepare( " {$mcmsdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $field_value );
			}
		}
		...
		foreach ( array( 'post_status', 'post_type' ) as $field_name ) {
			...
			if ( ! empty( $this->query_vars[ $field_name ] ) ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $mcmsdb->prepare( " {$mcmsdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ")", $q_values );
			}
		}

		// Comment author IDs for an IN clause.
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', mcms_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}

		$join = '';

		if ( $join_posts_table ) {
			$join .= "JOIN $mcmsdb->posts ON $mcmsdb->posts.ID = $mcmsdb->comments.comment_post_ID";
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			if ( ! $this->query_vars['count'] ) {
				$groupby = "{$mcmsdb->comments}.comment_ID";
			}
		}

		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query = new MCMS_Date_Query( $this->query_vars['date_query'], 'comment_date' );
		...
		$this->sql_clauses['from']    = "FROM $mcmsdb->comments $join";
		...
		if ( $this->query_vars['count'] ) {
			return intval( $mcmsdb->get_var( $this->request ) );
		} else {
			$comment_ids = $mcmsdb->get_col( $this->request );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param MCMS_Comment_Query $comment_query        The `MCMS_Comment_Query` instance.
			 */
			$found_comments_query = apply_filters( 'found_comments_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_comments = (int) $mcmsdb->get_var( $found_comments_query );
		}
	}
	...
	protected function fill_descendants( $comments ) {
		...
		$levels = array(
			0 => mcms_list_pluck( $comments, 'comment_ID' ),
		);

		$key = md5( serialize( mcms_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = mcms_cache_get_last_changed( 'comment' );
		...
		do {
			...
			foreach ( $_parent_ids as $parent_id ) {
				...
				$parent_child_ids = mcms_cache_get( $cache_key, 'comment' );
			...
			if ( $uncached_parent_ids ) {
				...
				foreach ( $parent_map as $parent_id => $children ) {
					$cache_key = "get_comment_child_ids:$parent_id:$key:$last_changed";
					mcms_cache_set( $cache_key, $children, 'comment' );
				}
			}
	...
	protected function get_search_sql( $string, $cols ) {
		...
		$like = '%' . $mcmsdb->esc_like( $string ) . '%';

		$searches = array();
		foreach ( $cols as $col ) {
			$searches[] = $mcmsdb->prepare( "$col LIKE %s", $like );
		}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $orderby == $this->query_vars['meta_key'] || $orderby == 'meta_value' ) {
			$parsed = "$mcmsdb->commentmeta.meta_value";
		} elseif ( $orderby == 'meta_value_num' ) {
			$parsed = "$mcmsdb->commentmeta.meta_value+0";
		} elseif ( $orderby == 'comment__in' ) {
			$comment__in = implode( ',', array_map( 'absint', $this->query_vars['comment__in'] ) );
			$parsed = "FIELD( {$mcmsdb->comments}.comment_ID, $comment__in )";
			...
			if ( isset( $meta_query_clauses[ $orderby ] ) ) {
				...
				$parsed = "$mcmsdb->comments.$orderby";
			}
		}
