 ...
 * @since 3.9.0
 */
function mcms_underscore_audio_template() {
	$audio_types = mcms_get_audio_extensions();
?>
...
>
	...
	<source src="{{ data.model.src }}" type="{{ mcms.media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>

	<?php foreach ( $audio_types as $type ):
	?><# if ( ! _.isEmpty( data.model.<?php echo $type ?> ) ) { #>
	<source src="{{ data.model.<?php echo $type ?> }}" type="{{ mcms.media.view.settings.embedMimes[ '<?php echo $type ?>' ] }}" />
	<# } #>
	<?php endforeach;
?></audio>
<?php
 ...
 * @since 3.9.0
 */
function mcms_underscore_video_template() {
	$video_types = mcms_get_video_extensions();
?>
<#  var w_rule = '', classes = [],
		w, h, settings = mcms.media.view.settings,
...
#>
<div style="{{ w_rule }}" class="mcms-video">
<video controls
	class="mcms-video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
 ...
 * @global bool $is_IE
 */
function mcms_print_media_templates() {
	global $is_IE;
	$class = 'media-modal mcms-core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<?php if ( ! _device_can_upload() ) : ?>
			<h2 class="upload-instructions"><?php printf( __( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.' ), 'https://apps.mandarincms.com/' ); ?></h2>
		<?php elseif ( is_multisite() && ! is_upload_space_available() ) : ?>
			<h2 class="upload-instructions"><?php _e( 'Upload Limit Exceeded' ); ?></h2>
			<?php
			/** This action is documented in mcms-admin/includes/media.php */
	...
	<script type="text/html" id="tmpl-attachment-details-two-column">
		<div class="attachment-media-view {{ data.orientation }}">
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<# if ( 'audio' === data.type ) { #>
				<div class="mcms-media-wrapper">
					<audio style="visibility: hidden" controls class="mcms-audio-shortcode" width="100%" preload="none">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( mcms.media.view.settings.contentWidth ) {
						w_rule = 'width: ' + mcms.media.view.settings.contentWidth + 'px;';
					}
				#>
				<div style="{{ w_rule }}" class="mcms-media-wrapper mcms-video">
					<video controls="controls" class="mcms-video-shortcode" preload="metadata"
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<label class="setting">
			...
			<select class="link-to"
				...
				<option value="post" <# if ( ! mcms.media.galleryDefaults.link || 'post' == mcms.media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_html_e( 'Attachment Page' ); ?>
				</option>
				<option value="file" <# if ( 'file' == mcms.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_html_e( 'Media File' ); ?>
				</option>
				<option value="none" <# if ( 'none' == mcms.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_html_e( 'None' ); ?>
				</option>
			</select>
		</label>
			...
			<select class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i ?> == mcms.media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: mcms.media.view.settings.embedMimes.mp3,
			ogg: mcms.media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = mcms_get_audio_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-audio-settings">
				<?php mcms_underscore_audio_template() ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: mcms.media.view.settings.embedMimes.mp4,
			ogv: mcms.media.view.settings.embedMimes.ogv,
			webm: mcms.media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = mcms_get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-video-settings">
				<div class="mcms-video-holder">
				<#
				...
				<?php mcms_underscore_video_template() ?>
				...
				<# _.each( data.attachments, function( attachment, index ) { #>
					<dl class="gallery-item">
						...
						<# if ( attachment.caption ) { #>
							<dd class="mcms-caption-text gallery-caption">
