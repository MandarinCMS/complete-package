...
class MCMS_Embed {
	...
	public function __construct() {
		// Hack to get the [embed] shortcode to run before mcmsautop()
	 ...
	 * an Ajax request that will call MCMS_Embed::cache_oembed().
	 */
...
<?php
	 ...
	 *
	 * Do not use this function directly, use mcms_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use mcms_embed_unregister_handler() instead.
	 *
	 ...
	 * will be given to the MCMS_oEmbed class.
	 *
	 * @param array $attr {
	...
	public function shortcode( $attr, $url = '' ) {
		...
		$rawattr = $attr;
		$attr = mcms_parse_args( $attr, mcms_embed_defaults( $url ) );

		$this->last_attr = $attr;

		// kses converts & into &amp; and we need to undo this
		// See https://core.trac.mandarincms.com/ticket/11311
		$url = str_replace( '&amp;', '&', $url );
		...
		// Potentially set by MCMS_Embed::cache_oembed().
		...
		if ( $post_ID ) {
			...
			if ( $html ) {
				if ( $cached_post_id ) {
					mcms_update_post( mcms_slash( array(
						'ID' => $cached_post_id,
						'post_content' => $html,
					) ) );
				} else {
					mcms_insert_post( mcms_slash( array_merge(
						$insert_post_args,
						array(
							'post_content' => $html,
						)
		...
		if ( $html ) {
			/** This filter is documented in mcms-roots/class-mcms-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to MCMS_Embed::shortcode() for potential embedding.
	 *
	 * @see MCMS_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = mcms_replace_in_html_tags( $content, array( "\n" => '<!-- mcms-line-break -->' ) );
		...
		return str_replace( '<!-- mcms-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for MCMS_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = mcms_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new MCMS_Query( array(
			'post_type'              => 'oembed_cache',
			'post_status'            => 'publish',
			'name'                   => $cache_key,
			'posts_per_page'         => 1,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			mcms_cache_set( $cache_key, $oembed_post_id, $cache_group );
