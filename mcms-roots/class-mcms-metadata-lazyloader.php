<?php
/**
 * Meta API: MCMS_Metadata_Lazyloader class
 *
 * @package MandarinCMS
 * @subpackage Meta
 * @since 4.5.0
 ...
 *
 * When loading many objects of a given type, such as posts in a MCMS_Query loop, it often makes
 ...
 *
 * MCMS uses the MCMS_Metadata_Lazyloader class to queue objects for metadata cache priming. The class
 * then detects the relevant get_*_meta() function call, and queries the metadata of all queued objects.
 *
 * Do not access this class directly. Use the mcms_metadata_lazyloader() function.
 *
 * @since 4.5.0
 */
class MCMS_Metadata_Lazyloader {
	/**
	 ...
	 * @return bool|MCMS_Error True on success, MCMS_Error on failure.
	 */
	public function queue_objects( $object_type, $object_ids ) {
		if ( ! isset( $this->settings[ $object_type ] ) ) {
			return new MCMS_Error( 'invalid_object_type', __( 'Invalid object type' ) );
		}
		 ...
		 * @param MCMS_Metadata_Lazyloader $lazyloader  The lazy-loader object.
		 */
	 ...
	 *               another value if filtered by a module.
	 */
	 ...
	 * directly, from either inside or outside the `MCMS_Query` object.
	 *
	 * @since 4.5.0
	 *
