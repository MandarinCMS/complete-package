<?php
/**
 * User API: MCMS_User_Query class
 *
 * @package MandarinCMS
 * @subpackage Users
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see MCMS_User_Query::prepare_query() for information on accepted arguments.
 */
class MCMS_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param array $args Query vars, as passed to `MCMS_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return mcms_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the MCMS_User_Query has been parsed.
		 *
		 * The passed MCMS_User_Query object contains the query variables, not
		 * yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param MCMS_User_Query $this The current MCMS_User_Query instance,
		 *                            passed by reference.
		 */
		...
		if ( is_array( $qv['fields'] ) ) {
			...
			foreach ( $qv['fields'] as $field ) {
				...
				$this->query_fields[] = "$mcmsdb->users.$field";
			}
			$this->query_fields = implode( ',', $this->query_fields );
		} elseif ( 'all' == $qv['fields'] ) {
			$this->query_fields = "$mcmsdb->users.*";
		} else {
			$this->query_fields = "$mcmsdb->users.ID";
		}
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = mcms_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $qv['has_published_posts'] && $blog_id ) {
			...
			foreach ( $post_types as &$post_type ) {
				$post_type = $mcmsdb->prepare( '%s', $post_type );
			}

			$posts_table = $mcmsdb->get_blog_prefix( $blog_id ) . 'posts';
			$this->query_where .= " AND $mcmsdb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . join( ", ", $post_types ) . " ) )";
		}

		// nicename
		if ( '' !== $qv['nicename']) {
			$this->query_where .= $mcmsdb->prepare( ' AND user_nicename = %s', $qv['nicename'] );
		}
		...
		// login
		if ( '' !== $qv['login']) {
			$this->query_where .= $mcmsdb->prepare( ' AND user_login = %s', $qv['login'] );
		}
		...
		if ( isset( $qv['who'] ) && 'authors' == $qv['who'] && $blog_id ) {
			$who_query = array(
				'key' => $mcmsdb->get_blog_prefix( $blog_id ) . 'user_level',
				'value' => 0,
				'compare' => '!=',
			);
		...
		if ( $blog_id && ( ! empty( $roles ) || ! empty( $role__in ) || ! empty( $role__not_in ) || is_multisite() ) ) {
			...
			if ( ! empty( $roles ) ) {
				foreach ( $roles as $role ) {
					$roles_clauses[] = array(
						'key'     => $mcmsdb->get_blog_prefix( $blog_id ) . 'capabilities',
						'value'   => '"' . $role . '"',
						'compare' => 'LIKE',
					);
				}
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'user', $mcmsdb->users, 'ID', $this );
		...
		if ( isset( $qv['number'] ) && $qv['number'] > 0 ) {
			if ( $qv['offset'] ) {
				$this->query_limit = $mcmsdb->prepare("LIMIT %d, %d", $qv['offset'], $qv['number']);
			} else {
				$this->query_limit = $mcmsdb->prepare( "LIMIT %d, %d", $qv['number'] * ( $qv['paged'] - 1 ), $qv['number'] );
			}
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				elseif ( preg_match('|^https?://|', $search) && ! ( is_multisite() && mcms_is_large_network( 'users' ) ) )
			...
			/**
			 * Filters the columns to search in a MCMS_User_Query search.
			 *
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND $mcmsdb->users.ID IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids = implode( ',', mcms_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $mcmsdb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query = new MCMS_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the MCMS_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed MCMS_User_Query object contains SQL parts formed
	...
	public function query() {
		...
		if ( is_array( $qv['fields'] ) || 'all' == $qv['fields'] ) {
			$this->results = $mcmsdb->get_results( $this->request );
		} else {
			$this->results = $mcmsdb->get_col( $this->request );
		}

		/**
		 * Filters SELECT FOUND_ROWS() query for the current MCMS_User_Query instance.
		 *
		 * @since 3.2.0
		 *
		 * @global mcmsdb $mcmsdb MandarinCMS database abstraction object.
		 *
		 * @param string $sql The SELECT FOUND_ROWS() query for the current MCMS_User_Query.
		 */
		if ( isset( $qv['count_total'] ) && $qv['count_total'] )
			$this->total_users = (int) $mcmsdb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
		...
		if ( 'all_with_meta' == $qv['fields'] ) {
			...
			foreach ( $this->results as $userid )
				$r[ $userid ] = new MCMS_User( $userid, '', $qv['blog_id'] );

			$this->results = $r;
		} elseif ( 'all' == $qv['fields'] ) {
			foreach ( $this->results as $key => $user ) {
				$this->results[ $key ] = new MCMS_User( $user, '', $qv['blog_id'] );
			}
		}
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) {
		...
		$like = $leading_wild . $mcmsdb->esc_like( $string ) . $trailing_wild;

		foreach ( $cols as $col ) {
			if ( 'ID' == $col ) {
				$searches[] = $mcmsdb->prepare( "$col = %s", $string );
			} else {
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ) ) ) {
			...
			) p ON ({$mcmsdb->users}.ID = p.post_author)
			";
			$_orderby = 'post_count';
		} elseif ( 'ID' == $orderby || 'id' == $orderby ) {
			$_orderby = 'ID';
		} elseif ( 'meta_value' == $orderby || $this->get( 'meta_key' ) == $orderby ) {
			$_orderby = "$mcmsdb->usermeta.meta_value";
		} elseif ( 'meta_value_num' == $orderby ) {
			$_orderby = "$mcmsdb->usermeta.meta_value+0";
		} elseif ( 'include' === $orderby && ! empty( $this->query_vars['include'] ) ) {
			$include = mcms_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( $mcmsdb->users.ID, $include_sql )";
