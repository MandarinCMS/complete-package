<?php
/**
 * XML-RPC protocol support for MandarinCMS
 *
 * @package MandarinCMS
...
/**
 * MandarinCMS XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional MandarinCMS API for managing comments, pages, posts,
 * options, etc.
 *
 * As of MandarinCMS 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in mcms_xmlrpc_server::login().
 *
 * @since 1.5.0
 *
 * @see IXR_Server
 */
class mcms_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @var array
	...
	/**
	 * Flags that the user authentication has failed in this instance of mcms_xmlrpc_server.
	 *
	 * @var bool
	 */
	 ...
	 * methods through the {@see 'xmlrpc_methods'} filter to allow modules to extend
	...
	public function __construct() {
		$this->methods = array(
			// MandarinCMS API
			'mcms.getUsersBlogs'		=> 'this:mcms_getUsersBlogs',
			'mcms.newPost'			=> 'this:mcms_newPost',
			'mcms.editPost'			=> 'this:mcms_editPost',
			'mcms.deletePost'			=> 'this:mcms_deletePost',
			'mcms.getPost'			=> 'this:mcms_getPost',
			'mcms.getPosts'			=> 'this:mcms_getPosts',
			'mcms.newTerm'			=> 'this:mcms_newTerm',
			'mcms.editTerm'			=> 'this:mcms_editTerm',
			'mcms.deleteTerm'			=> 'this:mcms_deleteTerm',
			'mcms.getTerm'			=> 'this:mcms_getTerm',
			'mcms.getTerms'			=> 'this:mcms_getTerms',
			'mcms.getTaxonomy'		=> 'this:mcms_getTaxonomy',
			'mcms.getTaxonomies'		=> 'this:mcms_getTaxonomies',
			'mcms.getUser'			=> 'this:mcms_getUser',
			'mcms.getUsers'			=> 'this:mcms_getUsers',
			'mcms.getProfile'			=> 'this:mcms_getProfile',
			'mcms.editProfile'		=> 'this:mcms_editProfile',
			'mcms.getPage'			=> 'this:mcms_getPage',
			'mcms.getPages'			=> 'this:mcms_getPages',
			'mcms.newPage'			=> 'this:mcms_newPage',
			'mcms.deletePage'			=> 'this:mcms_deletePage',
			'mcms.editPage'			=> 'this:mcms_editPage',
			'mcms.getPageList'		=> 'this:mcms_getPageList',
			'mcms.getAuthors'			=> 'this:mcms_getAuthors',
			'mcms.getCategories'		=> 'this:mw_getCategories',		// Alias
			'mcms.getTags'			=> 'this:mcms_getTags',
			'mcms.newCategory'		=> 'this:mcms_newCategory',
			'mcms.deleteCategory'		=> 'this:mcms_deleteCategory',
			'mcms.suggestCategories'	=> 'this:mcms_suggestCategories',
			'mcms.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'mcms.deleteFile'			=> 'this:mcms_deletePost',		// Alias
			'mcms.getCommentCount'	=> 'this:mcms_getCommentCount',
			'mcms.getPostStatusList'	=> 'this:mcms_getPostStatusList',
			'mcms.getPageStatusList'	=> 'this:mcms_getPageStatusList',
			'mcms.getPageTemplates'	=> 'this:mcms_getPageTemplates',
			'mcms.getOptions'			=> 'this:mcms_getOptions',
			'mcms.setOptions'			=> 'this:mcms_setOptions',
			'mcms.getComment'			=> 'this:mcms_getComment',
			'mcms.getComments'		=> 'this:mcms_getComments',
			'mcms.deleteComment'		=> 'this:mcms_deleteComment',
			'mcms.editComment'		=> 'this:mcms_editComment',
			'mcms.newComment'			=> 'this:mcms_newComment',
			'mcms.getCommentStatusList' => 'this:mcms_getCommentStatusList',
			'mcms.getMediaItem'		=> 'this:mcms_getMediaItem',
			'mcms.getMediaLibrary'	=> 'this:mcms_getMediaLibrary',
			'mcms.getPostFormats'     => 'this:mcms_getPostFormats',
			'mcms.getPostType'		=> 'this:mcms_getPostType',
			'mcms.getPostTypes'		=> 'this:mcms_getPostTypes',
			'mcms.getRevisions'		=> 'this:mcms_getRevisions',
			'mcms.restoreRevision'	=> 'this:mcms_restoreRevision',
	 ...
	 * @return MCMS_User|bool MCMS_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new MCMS_Error( 'login_prevented' );
		} else {
		...
		if ( is_mcms_error( $user ) ) {
			...
			// Flag that authentication has failed once on this mcms_xmlrpc_server instance
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use mcms_xmlrpc_server::login()
	 * @see mcms_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields($post_id, $fields) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset($meta['id']) ) {
				...
				if ( isset($meta['key']) ) {
					$meta['key'] = mcms_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key )
						continue;
					$meta['value'] = mcms_unslash( $meta['value'] );
					if ( current_user_can( 'edit_post_meta', $post_id, $meta['key'] ) )
						update_metadata_by_mid( 'post', $meta['id'], $meta['value'] );
				} elseif ( current_user_can( 'delete_post_meta', $post_id, $pmeta->meta_key ) ) {
					delete_metadata_by_mid( 'post', $meta['id'] );
				}
			} elseif ( current_user_can( 'add_post_meta', $post_id, mcms_unslash( $meta['key'] ) ) ) {
	...
	public function initialise_blog_option_info() {
		$this->blog_options = array(
			...
			'blog_url'          => array(
				'desc'          => __( 'MandarinCMS Address (URL)' ),
				'readonly'      => true,
				'option'        => 'siteurl'
			),
			'home_url'          => array(
			...
			'post_thumbnail'    => array(
				...
				'readonly'      => true,
				'value'         => current_myskin_supports( 'post-thumbnails' )
			),
	 ...
	 */
	public function mcms_getUsersBlogs( $args ) {
		if ( ! $this->minimum_args( $args, 2 ) ) {
			return $this->error;
		}

		// If this isn't on MCMSMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., mcms.getUsersBlogs, mcms.newPost, etc.
		 *
		 * @since 2.5.0
		 *
		 * @param string $name The method name.
		 */
		do_action( 'xmlrpc_call', 'mcms.getUsersBlogs' );
		...
		foreach ( $blogs as $blog ) {
			...
			$struct[] = array(
				...
				'xmlrpc'    => site_url( 'xmlrequest.php', 'rpc' ),
			);
	...
	protected function _prepare_taxonomy( $taxonomy, $fields ) {
		 ...
		 * @param MCMS_Taxonomy $taxonomy  Taxonomy object.
		 * @param array       $fields    The subset of taxonomy fields to return.
		 */
	...
	/**
	 * Convert a MandarinCMS date string to an IXR_Date object.
	 *
	 *
	...
	/**
	 * Convert a MandarinCMS GMT date string to an IXR_Date object.
	 *
	 *
	 * @param string $date_gmt MandarinCMS GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $thumbnail_id ) {
			$thumbnail_size = current_myskin_supports('post-thumbnail') ? 'post-thumbnail' : 'thumbnail';
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields ) ) {
			...
			$terms = mcms_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms'] = array();
	 ...
	 *
	 * @param MCMS_Post_Type $post_type Post type object.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => mcms_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => mcms_get_attachment_metadata( $media_item->ID ),
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( mcms_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'mcms_slug'                => $page->post_name,
			'mcms_password'            => $page->post_password,
			'mcms_author'              => $author->display_name,
			'mcms_page_parent_id'      => $page->post_parent,
			'mcms_page_parent_title'   => $parent_title,
			'mcms_page_order'          => $page->menu_order,
			'mcms_author_id'           => (string) $author->ID,
			'mcms_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'mcms_page_template'       => $page_template
		);
	 ...
	 *
	 *
	 * @param MCMS_User $user   The unprepared user object.
	 ...
	 *         Content struct for adding a new post. See mcms_insert_post() for information on
	 ...
	 */
	public function mcms_newPost( $args ) {
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.newPost' );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = mcms_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for mcms_newPost() and mcms_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 *
	 * @see mcms_insert_post()
	 *
	 * @param MCMS_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = mcms_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = mcms_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_mcms_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? mcms_update_post( $post_data, true ) : mcms_insert_post( $post_data, true );
	 ...
	 */
	public function mcms_editPost( $args ) {
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.editPost' );
	 ...
	 */
	public function mcms_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.deletePost' );
		...
		$result = mcms_delete_post( $post_id );
	 ...
	 *  - 'enclosure'
	 */
	public function mcms_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mcms.getPost' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPost' );
	 ...
	 * @since 3.4.0
	 *
	 * @see mcms_get_recent_posts()
	 * @see mcms_getPost() for more on `$fields`
	 ...
	 */
	public function mcms_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mcms.getPosts' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPosts' );
		...
		$posts_list = mcms_get_recent_posts( $query );
	 ...
	 */
	public function mcms_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.newTerm' );
		...
		$term = mcms_insert_term( $term_data['name'] , $taxonomy['name'] , $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function mcms_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.editTerm' );
		...
		$term = mcms_update_term( $term_id , $taxonomy['name'] , $term_data );
	 ...
	 */
	public function mcms_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.deleteTerm' );
		...
		$result = mcms_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function mcms_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getTerm' );
	 ...
	 */
	public function mcms_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getTerms' );
	 ...
	 */
	public function mcms_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mcms.getTaxonomy' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getTaxonomy' );
	 ...
	 */
	public function mcms_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mcms.getTaxonomies' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getTaxonomies' );
	 ...
	 *  - 'roles'
	 */
	public function mcms_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mcms.getUser' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getUser' );
	 ...
	 *
	 * @uses get_users()
	 * @see mcms_getUser() for more on $fields and return values
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mcms_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mcms.getUsers' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getUsers' );
	 ...
	 *     @type array  $fields (optional)
	 * }
	 * @return array|IXR_Error (@see mcms_getUser)
	 */
	public function mcms_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mcms.getProfile' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getProfile' );
	 ...
	 */
	public function mcms_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.editProfile' );
		...
		$result = mcms_update_user( $user_data );
	 ...
	 */
	public function mcms_getPage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPages( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPages' );
	 ...
	 * @since 2.2.0
	 *
	 * @see mcms_xmlrpc_server::mw_newPost()
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mcms_newPage( $args ) {
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.newPage' );
	 ...
	 */
	public function mcms_deletePage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.deletePage' );
		...
		$result = mcms_delete_post($page_id);
		if ( !$result )
		 ...
		 */
		do_action( 'xmlrpc_call_success_mcms_deletePage', $page_id, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_editPage( $args ) {
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.editPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPageList( $args ) {
		global $mcmsdb;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPageList' );

		// Get list of pages ids and titles
		$page_list = $mcmsdb->get_results("
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getAuthors( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getAuthors' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getKeywords' );
	 ...
	 */
	public function mcms_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.newCategory' );
		...
		$cat_id = mcms_insert_category($new_category, true);
		if ( is_mcms_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_mcms_newCategory', $cat_id, $args );
	 ...
	 * }
	 * @return bool|IXR_Error See mcms_delete_term() for return info.
	 */
	public function mcms_deleteCategory( $args ) {
		$this->escape( $args );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mcms_deleteCategory', $category_id, $args );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.suggestCategories' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getComment($args) {
		$this->escape($args);
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getComment' );
	 ...
	 * - 'offset' - Default is 0. See MCMS_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of comments. See mcms_xmlrpc_server::mcms_getComment() for a description of each item contents
	 */
	public function mcms_getComments( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getComments' );
	 ...
	 * See mcms_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type int    $comment_ID
	 * }
	 * @return bool|IXR_Error See mcms_delete_comment().
	 */
	public function mcms_deleteComment( $args ) {
		$this->escape($args);
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.deleteComment' );

		$status = mcms_delete_comment( $comment_ID );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mcms_deleteComment', $comment_ID, $args );
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function mcms_editComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.editComment' );
		...
		$result = mcms_update_comment($comment);
		if ( is_mcms_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_mcms_editComment', $comment_ID, $args );
	 ...
	 *     @type array      $content_struct
	 * }
	 * @return int|IXR_Error See mcms_new_comment().
	 */
	public function mcms_newComment($args) {
		$this->escape($args);
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.newComment' );

		$comment_ID = mcms_new_comment( $comment, true );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mcms_newComment', $comment_ID, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getCommentStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getCommentStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getCommentCount( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getCommentCount' );

		$count = mcms_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPostStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPageStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[$o_name]['option'], mcms_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function mcms_getMediaItem( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getMediaItem' );
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of media items. See mcms_xmlrpc_server::mcms_getMediaItem() for a description of each item contents
	 */
	public function mcms_getMediaLibrary($args) {
		$this->escape($args);
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getMediaLibrary' );
	 ...
	 */
	public function mcms_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPostFormats' );
		...
		if ( isset( $args[3] ) && is_array( $args[3] ) ) {
			if ( $args[3]['show-supported'] ) {
				if ( current_myskin_supports( 'post-formats' ) ) {
					$supported = get_myskin_support( 'post-formats' );
	 ...
	 *  - 'supports'
	 */
	public function mcms_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mcms.getPostType' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPostType' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mcms_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mcms.getPostTypes' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getPostTypes' );
	 ...
	 */
	public function mcms_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'mcms.getRevisions' );
		}
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.getRevisions' );
		...
		if ( ! mcms_revisions_enabled( $post ) )
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );

		$revisions = mcms_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves
			if ( mcms_is_post_autosave( $revision ) )
				continue;
	 ...
	 * @since 3.5.0
	 *
	 * @uses mcms_restore_post_revision()
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mcms_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		/** This action is documented in mcms-roots/class-mcms-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mcms.restoreRevision' );

		if ( ! $revision = mcms_get_post_revision( $revision_id ) )
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		...
		$post = mcms_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs( $args ) {
		...
		$path = $current_blog->path . 'xmlrequest.php';

		$rpc = new IXR_Client( set_url_scheme( "http://{$domain}{$path}" ) );
		$rpc->query('mcms.getUsersBlogs', $args[1], $args[2]);
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode(',', mcms_get_post_categories($post_ID));

		$content  = '<title>'.mcms_unslash($post_data['post_title']).'</title>';
		$content .= '<category>'.$categories.'</category>';
		$content .= mcms_unslash($post_data['post_content']);
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', mcms_get_post_categories($entry['ID']));

			$content  = '<title>'.mcms_unslash($entry['post_title']).'</title>';
			$content .= '<category>'.$categories.'</category>';
			$content .= mcms_unslash($entry['post_content']);
	...
	public function blogger_newPost( $args ) {
		...
		$post_ID = mcms_insert_post($post_data);
		if ( is_mcms_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function blogger_editPost( $args ) {
		...
		$result = mcms_update_post( $postdata );
	...
	public function mw_newPost($args) {
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type = 'page';
				if ( !empty( $content_struct['mcms_page_template'] ) )
					$page_template = $content_struct['mcms_page_template'];
		...
		if ( isset( $content_struct['mcms_post_format'] ) ) {
			$content_struct['mcms_post_format'] = sanitize_key( $content_struct['mcms_post_format'] );
			if ( !array_key_exists( $content_struct['mcms_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format.' ) );
			}
		}

		// Let MandarinCMS generate the post_name (slug) unless
		// one has been provided.
		$post_name = "";
		if ( isset($content_struct['mcms_slug']) )
			$post_name = $content_struct['mcms_slug'];

		// Only use a password if one was given.
		if ( isset($content_struct['mcms_password']) )
			$post_password = $content_struct['mcms_password'];

		// Only set a post parent if one was provided.
		if ( isset($content_struct['mcms_page_parent_id']) )
			$post_parent = $content_struct['mcms_page_parent_id'];

		// Only set the menu_order if it was provided.
		if ( isset($content_struct['mcms_page_order']) )
			$menu_order = $content_struct['mcms_page_order'];

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['mcms_author_id'] ) && ( $user->ID != $content_struct['mcms_author_id'] ) ) {
			switch ( $post_type ) {
				case "post":
			...
			$author = get_userdata( $content_struct['mcms_author_id'] );
			if ( ! $author )
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			$post_author = $content_struct['mcms_author_id'];
		}
		...
		if ( isset ( $content_struct['mcms_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['mcms_post_thumbnail'] ) === false )
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );

			unset( $content_struct['mcms_post_thumbnail'] );
		}
		...
		// in this function
		if ( isset( $content_struct['mcms_post_format'] ) )
			set_post_format( $post_ID, $content_struct['mcms_post_format'] );

		$post_ID = mcms_insert_post( $postdata, true );
		if ( is_mcms_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $mcmsdb->get_results( "SELECT ID, guid FROM {$mcmsdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && strpos( $post_content, $file->guid ) !== false )
					$mcmsdb->update($mcmsdb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	public function mw_editPost( $args ) {
		...
		// Use mcms.editPost to edit post types other than post and page.
		...
		// Let MandarinCMS manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['mcms_page_template'] ) && 'page' == $post_type )
			$page_template = $content_struct['mcms_page_template'];

		$post_author = $postdata['post_author'];

		// Only set the post_author if one is set.
		if ( isset( $content_struct['mcms_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['mcms_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		// We've got all the data -- post it.
		$nemcmsost = compact('ID', 'post_content', 'post_title', 'post_category', 'post_status', 'post_excerpt', 'comment_status', 'ping_status', 'edit_date', 'post_date', 'post_date_gmt', 'to_ping', 'post_name', 'post_password', 'post_parent', 'menu_order', 'post_author', 'tags_input', 'page_template');

		$result = mcms_update_post($nemcmsost, true);
		if ( is_mcms_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		...
		if ( isset ( $content_struct['mcms_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['mcms_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['mcms_post_thumbnail'] ) === false )
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
	...
	public function mw_getPost( $args ) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = mcms_get_post_categories($post_ID);
			foreach ($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = mcms_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'mcms_more_text' => $post['more_text'],
				'mt_allow_comments' => $allow_comments,
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'mcms_slug' => $postdata['post_name'],
				'mcms_password' => $postdata['post_password'],
				'mcms_author_id' => (string) $author->ID,
				'mcms_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'mcms_post_format' => $post_format,
				'sticky' => $sticky,
			...
			$resp['mcms_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = array();
			$catids = mcms_get_post_categories($entry['ID']);
			foreach ( $catids as $catid )
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = mcms_get_post_tags( $entry['ID'] );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'mcms_slug' => $entry['post_name'],
				'mcms_password' => $entry['post_password'],
				'mcms_author_id' => (string) $author->ID,
				'mcms_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				...
				'mcms_post_thumbnail' => get_post_thumbnail_id( $entry['ID'] )
			);
		}
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-mandarincms-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @global mcmsdb $mcmsdb MandarinCMS database abstraction object.
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = mcms_upload_bits($name, null, $bits);
		...
		// Save the data
		$id = mcms_insert_attachment( $attachment, $upload[ 'file' ], $post_id );
		mcms_update_attachment_metadata( $id, mcms_generate_attachment_metadata( $id, $upload['file'] ) );
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids = mcms_get_post_categories(intval($post_ID));
	...
	public function mt_setPostCategories( $args ) {
		...
		mcms_set_post_categories($post_ID, $catids);
	...
	public function mt_getTrackbackPings( $post_ID ) {
		...
		$comments = $mcmsdb->get_results( $mcmsdb->prepare("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $mcmsdb->comments WHERE comment_post_ID = %d", $post_ID) );
	...
	public function mt_publishPost( $args ) {
		...
		// retain old cats
		$cats = mcms_get_post_categories($post_ID);
		$postdata['post_category'] = $cats;
		$this->escape($postdata);

		return mcms_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			if ( intval($urltest['fragment']) ) {
				...
				$sql = $mcmsdb->prepare("SELECT ID FROM $mcmsdb->posts WHERE post_title RLIKE %s", $title );
				if (! ($post_ID = $mcmsdb->get_var($sql)) ) {
		...
		if ( $mcmsdb->get_results( $mcmsdb->prepare("SELECT * FROM $mcmsdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
		...
		/** This filter is documented in mcms-roots/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'MandarinCMS/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ) );
		...
		$request = mcms_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source = $remote_source_original = mcms_remote_retrieve_body( $request );
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$excerpt = preg_replace('|\</?mcmscontext\>|', '', $para);

				// prevent really long link text
				if ( strlen($context[1]) > 100 )
					$context[1] = substr($context[1], 0, 100) . '&#8230;';

				$marker = '<mcmscontext>'.$context[1].'</mcmscontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<mcmscontext>');        // strip all tags but our context marker
