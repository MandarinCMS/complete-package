<?php
/**
 * MandarinCMS implementation for PHP functions either missing from older PHP versions or not included by default.
 *
 * @package PHP
 * @access private
 */
 ...
 */
function _mcms_can_use_pcre_u( $set = null ) {
...
if ( !function_exists('json_encode') ) {
	function json_encode( $string ) {
		...
		if ( ! ( $mcms_json instanceof Services_JSON ) ) {
			require_once( BASED_TREE_URI . MCMSINC . '/class-json.php' );
			$mcms_json = new Services_JSON();
		}

		return $mcms_json->encodeUnsafe( $string );
	}
}

if ( !function_exists('json_decode') ) {
	/**
	 * @global Services_JSON $mcms_json
	 * @param string $string
...
// Defined here to prevent a notice when using it with mcms_json_encode()
...
if ( ! interface_exists( 'JsonSerializable' ) ) {
	define( 'MCMS_JSON_SERIALIZE_COMPATIBLE', true );
	/**
...
if ( ! function_exists( 'random_int' ) ) {
	require BASED_TREE_URI . MCMSINC . '/random_compat/random.php';
}
...
if ( ! function_exists( 'array_replace_recursive' ) ) :
	 ...
	 *
	 * The array_replace_recursive() function is a PHP 5.3 function. MandarinCMS
	 ...
	 *
	 * Subject to removal once MandarinCMS makes PHP 5.3.0 the minimum requirement.
	 *
	 * @since 4.5.3
	 *
...
if ( ! function_exists( 'spl_autoload_register' ) ) {
	require_once BASED_TREE_URI . MCMSINC . '/spl-autoload-compat.php';
}
