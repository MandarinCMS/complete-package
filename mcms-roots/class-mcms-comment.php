<?php
/**
 * Comment API: MCMS_Comment class
 *
 * @package MandarinCMS
 * @subpackage Comments
 * @since 4.4.0
...
final class MCMS_Comment {
	...
	/**
	 * Retrieves a MCMS_Comment instance.
	 *
	 * @since 4.4.0
	 * @static
	 *
	 ...
	 * @return MCMS_Comment|false Comment object, otherwise false.
	 */
	public static function get_instance( $id ) {
		...
		$_comment = mcms_cache_get( $comment_id, 'comment' );

		if ( ! $_comment ) {
			$_comment = $mcmsdb->get_row( $mcmsdb->prepare( "SELECT * FROM $mcmsdb->comments WHERE comment_ID = %d LIMIT 1", $comment_id ) );

			if ( ! $_comment ) {
				return false;
			}

			mcms_cache_add( $_comment->comment_ID, $_comment, 'comment' );
		}

		return new MCMS_Comment( $_comment );
	}
	 ...
	 *                                 stored in a `children` property on the `MCMS_Comment` object.
	 ...
	 * }
	 * @return array Array of `MCMS_Comment` objects.
	 */
	public function get_children( $args = array() ) {
		...
		$_args = mcms_parse_args( $args, $defaults );
	 ...
	 *
	 * Used by `MCMS_Comment_Query` when bulk-filling descendants.
	 *
	 * @since 4.4.0
	 *
	 * @param MCMS_Comment $child Child comment.
	 */
	public function add_child( MCMS_Comment $child ) {
	 ...
	 * @return MCMS_Comment|bool Returns the comment object if found, otherwise false.
	 */
