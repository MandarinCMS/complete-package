...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$mcms_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	global $mcms_rewrite, $mcms;

	if ( empty( $query ) ) {
		$qv = trim( $tag, '%' );
		$mcms->add_query_var( $qv );
		$query = $qv . '=';
	}

	$mcms_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $mcms_rewrite;
	$mcms_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$mcms_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see MCMS_Rewrite::remove_permastruct()
 * @global MCMS_Rewrite $mcms_rewrite MandarinCMS rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$mcms_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $function ) {
	...
	if ( ! in_array( $feedname, $mcms_rewrite->feeds ) ) {
		$mcms_rewrite->feeds[] = $feedname;
	}
 ...
 *
 * Be sure to flush the rewrite rules - see flush_rewrite_rules() - when your module gets
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $mcms_rewrite;
	$mcms_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _mcms_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to MCMS_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, mcms_unique_post_slug() prevents the creation of post slugs
 ...
 *                          MCMS::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function mcms_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' == get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof MCMS_Post ) {
	...
	if ( !$mcms_rewrite->using_index_permalinks() )
		$url = str_replace( $mcms_rewrite->index . '/', '', $url );
	...
	foreach ( (array)$rewrite as $match => $query) {
		...
		if ( preg_match("#^$match#", $request_match, $matches) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( $key, $mcms->public_query_vars ) ){
					$query[$key] = $value;
			...
			$query = mcms_resolve_numeric_slug_conflicts( $query );
