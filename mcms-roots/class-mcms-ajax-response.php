...
class MCMS_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to MCMS_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 * @see MCMS_Ajax_Response::add()
	 *
	 ...
	 *     @type int|MCMS_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `MCMS_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|MCMS_Error $data         The response content/message. Also accepts a MCMS_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_mcms_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response .= "<mcms_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . "]]></mcms_error>";
				...
				$response .= "<mcms_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><mcms_ajax>";
