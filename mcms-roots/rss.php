 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', MCMSINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'MandarinCMS/' . $GLOBALS['mcms_version']);
...
/**
 * Retrieve URL headers and content using MCMS HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = mcms_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_mcms_error($resp) ) {
	...
	// Also note, MCMS_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( mcms_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = mcms_remote_retrieve_response_code( $resp );
	$response->response_code = mcms_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = mcms_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'MandarinCMS/' . $GLOBALS['mcms_version'];
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = MCMS_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function mcms_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with mcms_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
