<?php
/**
 * MandarinCMS Customize Nav Menus classes
 *
 * @package MandarinCMS
 ...
 * @since 4.3.0
 *
 * @see MCMS_Customize_Manager
 */
final class MCMS_Customize_Nav_Menus {

	/**
	 * MCMS_Customize_Manager instance.
	 *
	 * @since 4.3.0
	...
	/**
	 * Original nav menu locations before the myskin was switched.
	 *
	 * @since 4.9.0
	 * @var array
	 */
	 ...
	 * @since 4.3.0
	 *
	 * @param object $manager An instance of the MCMS_Customize_Manager class.
	 */
	public function __construct( $manager ) {
		$this->manager = $manager;
		$this->original_nav_menu_locations = get_nav_menu_locations();

		// See https://github.com/xmcms/mcms-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L469-L499
		...
		add_action( 'mcms_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'mcms_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
		add_action( 'mcms_ajax_customize-nav-menus-insert-auto-draft', array( $this, 'ajax_insert_auto_draft_post' ) );
	...
	public function filter_nonces( $nonces ) {
		$nonces['customize-menus'] = mcms_create_nonce( 'customize-menus' );
		return $nonces;
	}
	...
	public function ajax_load_available_items() {
		...
		if ( isset( $_POST['item_types'] ) && is_array( $_POST['item_types'] ) ) {
			$item_types = mcms_unslash( $_POST['item_types'] );
		} elseif ( isset( $_POST['type'] ) && isset( $_POST['object'] ) ) { // Back compat.
			$item_types[] = array(
				'type' => mcms_unslash( $_POST['type'] ),
				'object' => mcms_unslash( $_POST['object'] ),
				'page' => empty( $_POST['page'] ) ? 0 : absint( $_POST['page'] ),
			);
		} else {
			mcms_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		foreach ( $item_types as $item_type ) {
			...
			if ( is_mcms_error( $items ) ) {
				mcms_send_json_error( $items->get_error_code() );
			}
			$all_items[ $item_type['type'] . ':' . $item_type['object'] ] = $items;
		}

		mcms_send_json_success( array( 'items' => $all_items ) );
	}
	 ...
	 * @return MCMS_Error|array Returns either a MCMS_Error object or an array of menu items.
	 */
	public function load_available_items_query( $type = 'post_type', $object = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $type ) {
			...
			if ( ! $post_type ) {
				return new MCMS_Error( 'nav_menus_invalid_post_type' );
			}
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			mcms_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s = sanitize_text_field( mcms_unslash( $_POST['search'] ) );
		$items = $this->search_available_items_query( array( 'pagenum' => $p, 's' => $s ) );

		if ( empty( $items ) ) {
			mcms_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			mcms_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on MCMS_Editor::mcms_link_query().
	 *
	 * @since 4.3.0
	 *
	...
	public function search_available_items_query( $args = array() ) {
		...
		if ( 1 === $args['pagenum'] && $nav_menus_created_posts_setting && count( $nav_menus_created_posts_setting ) > 0 ) {
			$stub_post_query = new MCMS_Query( array_merge(
				$query,
				array(
		...
		// Query posts.
		$get_posts = new MCMS_Query( $query );
	...
	public function enqueue_scripts() {
		mcms_enqueue_style( 'customize-nav-menus' );
		mcms_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new MCMS_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new MCMS_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );

		$num_locations = count( get_registered_nav_menus() );
		if ( 1 === $num_locations ) {
			$locations_description = __( 'Your myskin can display menus in one location.' );
		} else {
			/* translators: %s: number of menu locations */
			$locations_description = sprintf( _n( 'Your myskin can display menus in %s location.', 'Your myskin can display menus in %s locations.', $num_locations ), number_format_i18n( $num_locations ) );
		}

		// Pass data to JS.
		$settings = array(
			'allMenus'             => mcms_get_nav_menus(),
			...
			'l10n'                 => array(
				...
				'newMenuNameDescription' => __( 'If your myskin has multiple menus, giving them clear names will help you manage them.' ),
		...
		$data = sprintf( 'var _mcmsCustomizeNavMenusSettings = %s;', mcms_json_encode( $settings ) );
		mcms_scripts()->add_data( 'customize-nav-menus', 'data', $data );

		// This is copied from nav-menus.php, and it has an unfortunate object name of `menus`.
		$nav_menus_l10n = array(
			'oneMySkinLocationNoMenus' => null,
		...
		mcms_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( MCMS_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MCMS_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( MCMS_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MCMS_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && MCMS_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MCMS_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && MCMS_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MCMS_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'MCMS_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'MCMS_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'MCMS_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'MCMS_Customize_Nav_Menu_Locations_Control' );
		$this->manager->register_control_type( 'MCMS_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'MCMS_Customize_Nav_Menu_Item_Control' );
		...
		if ( current_myskin_supports( 'widgets' ) ) {
			...
			$description .= '<p>' . sprintf( __( 'Menus can be displayed in locations defined by your myskin or in <a href="%s">widget areas</a> by adding a &#8220;Navigation Menu&#8221; widget.' ), "javascript:mcms.customize.panel( 'widgets' ).focus();" ) . '</p>';
		} else {
			$description .= '<p>' . __( 'Menus can be displayed in locations defined by your myskin.' ) . '</p>';
		}
		$this->manager->add_panel( new MCMS_Customize_Nav_Menus_Panel( $this->manager, 'nav_menus', array(
			'title'       => __( 'Menus' ),
			'description' => $description,
			'priority'    => 100,
			// 'myskin_supports' => 'menus|widgets', @todo allow multiple myskin supports
		) ) );
		...
		if ( 1 == $num_locations ) {
			$description = '<p>' . __( 'Your myskin can display menus in one location. Select which menu you would like to use.' ) . '</p>';
		} else {
			/* translators: %s: number of menu locations */
			$description = '<p>' . sprintf( _n( 'Your myskin can display menus in %s location. Select which menu you would like to use.', 'Your myskin can display menus in %s locations. Select which menu appears in each location.', $num_locations ), number_format_i18n( $num_locations ) ) . '</p>';
		}

		if ( current_myskin_supports( 'widgets' ) ) {
			/* translators: URL to the widgets panel of the customizer */
			$description .= '<p>' . sprintf( __( 'If your myskin has widget areas, you can also add menus there. Visit the <a href="%s">Widgets panel</a> and add a &#8220;Navigation Menu widget&#8221; to display a menu in a sidebar or footer.' ), "javascript:mcms.customize.panel( 'widgets' ).focus();" ) . '</p>';
		}
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = mcms_html_excerpt( $menu->name, 40, '&hellip;' );
		}

		// Attempt to re-map the nav menu location assignments when previewing a myskin switch.
		$mapped_nav_menu_locations = array();
		if ( ! $this->manager->is_myskin_active() ) {
			$myskin_mods = get_option( 'myskin_mods_' . $this->manager->get_stylesheet(), array() );

			// If there is no data from a previous activation, start fresh.
			if ( empty( $myskin_mods['nav_menu_locations'] ) ) {
				$myskin_mods['nav_menu_locations'] = array();
			}

			$mapped_nav_menu_locations = mcms_map_nav_menu_locations( $myskin_mods['nav_menu_locations'], $this->original_nav_menu_locations );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			// Override the assigned nav menu location if mapped during previewed myskin switch.
			...
			$this->manager->add_control( new MCMS_Customize_Nav_Menu_Location_Control( $this->manager, $setting_id, array(
				'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section( new MCMS_Customize_Nav_Menu_Section( $this->manager, $section_id, array(
			...
			$this->manager->add_setting( new MCMS_Customize_Nav_Menu_Setting( $this->manager, $nav_menu_setting_id, array(
				'transport' => 'postMessage',
			) ) );

			// Add the menu contents.
			$menu_items = (array) mcms_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting( new MCMS_Customize_Nav_Menu_Item_Setting( $this->manager, $menu_item_setting_id, array(
					'value'     => $value,
					'transport' => 'postMessage',
				) ) );

				// Create a control for each menu item.
				$this->manager->add_control( new MCMS_Customize_Nav_Menu_Item_Control( $this->manager, $menu_item_setting_id, array(
					'label'    => $item->title,
					'section'  => $section_id,
					'priority' => 10 + $i,
				) ) );
		...
		$this->manager->add_setting( new MCMS_Customize_Filter_Setting( $this->manager, 'nav_menus_created_posts', array(
			'transport' => 'postMessage',
			'type' => 'option', // To prevent myskin prefix in changeset.
			'default' => array(),
	...
	public function available_item_types() {
		...
		if ( $taxonomies ) {
			foreach ( $taxonomies as $slug => $taxonomy ) {
				if ( 'post_format' === $taxonomy && ! current_myskin_supports( 'post-formats' ) ) {
					continue;
				}
				$item_types[] = array(
	 ...
	 * }
	 * @return MCMS_Post|MCMS_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) ) {
			return new MCMS_Error( 'unknown_post_type', __( 'Invalid post type.' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new MCMS_Error( 'empty_title', __( 'Empty title' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new MCMS_Error( 'status_forbidden', __( 'Status is forbidden' ) );
		}
		...
		add_filter( 'mcms_insert_post_empty_content', '__return_false', 1000 );
		$r = mcms_insert_post( mcms_slash( $postarr ), true );
		remove_filter( 'mcms_insert_post_empty_content', '__return_false', 1000 );
	...
	public function ajax_insert_auto_draft_post() {
		if ( ! check_ajax_referer( 'customize-menus', 'customize-menus-nonce', false ) ) {
			mcms_send_json_error( 'bad_nonce', 400 );
		}

		if ( ! current_user_can( 'customize' ) ) {
			mcms_send_json_error( 'customize_not_allowed', 403 );
		}

		if ( empty( $_POST['params'] ) || ! is_array( $_POST['params'] ) ) {
			mcms_send_json_error( 'missing_params', 400 );
		}

		$params = mcms_unslash( $_POST['params'] );
		$illegal_params = array_diff( array_keys( $params ), array( 'post_type', 'post_title' ) );
		if ( ! empty( $illegal_params ) ) {
			mcms_send_json_error( 'illegal_params', 400 );
		}
	 ...
	 *
	 * Templates are imported into the JS use mcms.template.
	 *
	 * @since 4.3.0
	 */
	public function print_templates() {
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<span class="item-title" aria-hidden="true">
							<span class="menu-item-title<# if ( ! data.title ) { #> no-title<# } #>">{{ data.title || mcms.customize.Menus.data.l10n.untitled }}</span>
						</span>
						<button type="button" class="button-link item-add">
							<span class="screen-reader-text"><?php
								/* translators: 1: Title of a menu item, 2: Type of a menu item */
								printf( __( 'Add to menu: %1$s (%2$s)' ), '{{ data.title || mcms.customize.Menus.data.l10n.untitled }}', '{{ data.type_label }}' );
							?></span>
						</button>
					</div>
				</div>
		...
		<script type="text/html" id="tmpl-nav-menu-create-menu-section-title">
			...
			<p class="add-new-menu-notice">
				<?php _e( 'You&#8217;ll create a menu, assign it a location, and add menu items like links to pages and categories. If your myskin has multiple menu areas, you might need to create more than one.' ); ?>
			</p>
			<h3>
	...
	protected function print_custom_links_available_menu_item() {
		...
		<div id="new-custom-menu-item" class="accordion-section">
			...
			<div class="accordion-section-content customlinkdiv">
				...
				<p id="menu-item-url-wrap" class="mcms-clearfix">
				...
				<p id="menu-item-name-wrap" class="mcms-clearfix">
	...
	public function customize_preview_init() {
		add_action( 'mcms_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );
		add_filter( 'mcms_nav_menu_args', array( $this, 'filter_mcms_nav_menu_args' ), 1000 );
		add_filter( 'mcms_nav_menu', array( $this, 'filter_mcms_nav_menu' ), 10, 2 );
		add_filter( 'mcms_footer', array( $this, 'export_preview_data' ), 1 );
	...
	public function make_auto_draft_status_previewable() {
		...
		$mcms_post_statuses['auto-draft']->protected = true;
	}
	...
	public function sanitize_nav_menus_created_posts( $value ) {
		$post_ids = array();
		foreach ( mcms_parse_id_list( $value ) as $post_id ) {
			if ( empty( $post_id ) ) {
				continue;
			}
	 ...
	 * `MCMS_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 * remove any post IDs for which the user cannot publish or for which the
	 * post is not an auto-draft.
	 *
	 * @since 4.7.0
	 *
	 * @param MCMS_Customize_Setting $setting Customizer setting object.
	 */
	public function save_nav_menus_created_posts( $setting ) {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				// Note that mcms_publish_post() cannot be used because unique slugs need to be assigned.
				mcms_update_post( mcms_slash( $args ) );
	...
	/**
	 * Keep track of the arguments that are being passed to mcms_nav_menu().
	 *
	 * @since 4.3.0
	 * @see mcms_nav_menu()
	 * @see MCMS_Customize_Widgets_Partial_Refresh::filter_dynamic_sidebar_params()
	 *
	 * @param array $args An array containing mcms_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_mcms_nav_menu_args( $args ) {
		/*
		 * The following conditions determine whether or not this instance of
		 * mcms_nav_menu() can use selective refreshed. A mcms_nav_menu() can be
		 * selective refreshed if...
		 */
		$can_partial_refresh = (
			// ...if mcms_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
			! empty( $args['echo'] )
			&&
			...
			// ...and if it has a myskin location assigned or an assigned menu to display,
			&& (
				! empty( $args['myskin_location'] )
				||
	...
	/**
	 * Prepares mcms_nav_menu() calls for partial refresh.
	 *
	 ...
	 * @param object $args             An object containing mcms_nav_menu() arguments.
	 * @return string Nav menu HTML with selective refresh attributes added if partial can be refreshed.
	 */
	public function filter_mcms_nav_menu( $nav_menu_content, $args ) {
		if ( isset( $args->customize_preview_nav_menus_args['can_partial_refresh'] ) && $args->customize_preview_nav_menus_args['can_partial_refresh'] ) {
			...
			$attributes .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mcms_json_encode( $args->customize_preview_nav_menus_args ) ) );
	...
	public function hash_nav_menu_args( $args ) {
		return mcms_hash( serialize( $args ) );
	}
	...
	public function customize_preview_enqueue_deps() {
		mcms_enqueue_script( 'customize-preview-nav-menus' ); // Note that we have overridden this.
	}
	...
	public function export_preview_data() {
		...
		// Why not mcms_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'navMenuInstanceArgs' => $this->preview_nav_menu_instance_args,
		);
		printf( '<script>var _mcmsCustomizePreviewNavMenusExports = %s;</script>', mcms_json_encode( $exports ) );
	}

	/**
	 * Export any mcms_nav_menu() calls during the rendering of any partials.
	 *
	 * @since 4.5.0
	 *
	...
	/**
	 * Render a specific menu via mcms_nav_menu() using the supplied arguments.
	 *
	 * @since 4.3.0
	 *
	 * @see mcms_nav_menu()
	 *
	 * @param MCMS_Customize_Partial $partial       Partial.
