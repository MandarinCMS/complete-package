/* global tinymce, mcmsCookies, autosaveL10n, switchEditors */
// Back-compat
	...
	function autosave() {
		...
		function autosaveLocal() {
			...
			function checkStorage() {
				...
				try {
					window.sessionStorage.setItem( 'mcms-test', test );
					result = window.sessionStorage.getItem( 'mcms-test' ) === test;
					window.sessionStorage.removeItem( 'mcms-test' );
				} catch(e) {}
			...
			function getStorage() {
				...
				if ( hasStorage && blog_id ) {
					stored_obj = sessionStorage.getItem( 'mcms-autosave-' + blog_id );
			...
			function run() {
				...
				if ( $( '#mcms-plugins-wrap' ).hasClass( 'tmce-active' ) ) {
				...
				$( 'form#post' ).on( 'submit.autosave-local', function() {
					...
					mcmsCookies.set( 'mcms-saving-post', post_id + '-check', 24 * 60 * 60, false, false, secure );
				});
			}
			...
			function checkPost() {
				var content, post_title, excerpt, $notice,
					postData = getSavedPostData(),
					cookie = mcmsCookies.get( 'mcms-saving-post' ),
					$newerAutosaveNotice = $( '#has-newer-autosave' ).parent( '.notice' ),
					$headerEnd = $( '.mcms-header-end' );

				if ( cookie === post_id + '-saved' ) {
					mcmsCookies.remove( 'mcms-saving-post' );
			...
			function restorePost( postData ) {
				...
				if ( postData ) {
					...
					if ( editor && ! editor.isHidden() && typeof switchEditors !== 'undefined' ) {
						if ( editor.settings.mcmsautop && postData.content ) {
							postData.content = switchEditors.mcmsautop( postData.content );
						}
		...
		function autosaveServer() {
			...
			function save() {
				...
				$document.trigger( 'mcmscountwords', [ postData.content ] )
					.trigger( 'before-autosave', [ postData ] );

				postData._mcmsnonce = $( '#_mcmsnonce' ).val() || '';
			...
			$document.on( 'heartbeat-send.autosave', function( event, data ) {
				...
				if ( autosaveData ) {
					data.mcms_autosave = autosaveData;
				}
				...
				if ( data.mcms_autosave ) {
					response( data.mcms_autosave );
				}
				/**
				...
				if ( 'timeout' === error || 603 === status ) {
					...
					if ( ! mcms.autosave.local.hasStorage ) {
