...
(function( window, settings ) {
	...
	CustomHeader.prototype = {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'mcms-custom-header-video-loaded' );
						break;
					}
				}
			}
		 ...
		 *
		 * MySkins and modules can override this method to change the criteria.
		 *
		 * @return {boolean}
		 */
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'mcms-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'mcms-custom-header-video-button' );
			button.setAttribute( 'class', 'mcms-custom-header-video-button mcms-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'mcms-custom-header-video-button mcms-custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window.mcms ) {
					window.mcms.a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'mcms-custom-header-video-button mcms-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.mcms ) {
					window.mcms.a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	 ...
	 * @class
	 */
	NativeHandler = BaseHandler.extend(/** @lends mcms.NativeHandler.prototype */{
		/**
		...
		ready: function() {
			...
			video.id = 'mcms-custom-header-video';
	 ...
	 * @memberOf mcms
	 *
	 * @class mcms.YouTubeHandler
	 */
	YouTubeHandler = BaseHandler.extend(/** @lends mcms.YouTubeHandler.prototype */{
		/**
	...
	window.mcms.customHeader = new CustomHeader();
	document.addEventListener( 'DOMContentLoaded', window.mcms.customHeader.initialize.bind( window.mcms.customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window.mcms ) {
		window.mcms.customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window.mcms.customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window.mcms.customHeader.initialize();
			}
		});
	}

})( window, window._mcmsCustomHeaderSettings || {} );
