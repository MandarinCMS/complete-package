/* global _mcmsWidgetCustomizerPreviewSettings */

/** @namespace mcms.customize.widgetsPreview */
mcms.customize.widgetsPreview = mcms.customize.WidgetCustomizerPreview = (function( $, _, mcms, api ) {
	...
	self.init = function() {
		...
		api.preview.bind( 'refresh-widget-partial', function( widgetId ) {
			...
			if ( api.selectiveRefresh.partial.has( partialId ) ) {
				...
				api.preview.send( 'refresh' ); // Fallback in case myskin does not support 'customize-selective-refresh-widgets'.
			}
		} );
	};
	 ...
	 *
	 * @memberOf mcms.customize.widgetsPreview
	 * @alias mcms.customize.widgetsPreview.WidgetPartial
	 *
	 * @class
	 * @augments mcms.customize.selectiveRefresh.Partial
	 * @since 4.5.0
	 */
	self.WidgetPartial = api.selectiveRefresh.Partial.extend(/** @lends mcms.customize.widgetsPreview.WidgetPartial.prototype */{
	 ...
	 *
	 * @memberOf mcms.customize.widgetsPreview
	 * @alias mcms.customize.widgetsPreview.SidebarPartial
	 *
	 * @class
	 * @augments mcms.customize.selectiveRefresh.Partial
	 * @since 4.5.0
	 */
	self.SidebarPartial = api.selectiveRefresh.Partial.extend(/** @lends mcms.customize.widgetsPreview.SidebarPartial.prototype */{
		 ...
		 * @since 4.5.0
		 *
		 * @return {Array.<mcms.customize.selectiveRefresh.Placement>} List of placements that were reflowed.
		 */
		 ...
		 * @returns {mcms.customize.selectiveRefresh.Partial} Widget instance partial.
		 */
	...
	api.bind( 'preview-ready', function() {
		$.extend( self, _mcmsWidgetCustomizerPreviewSettings );
		self.init();
	});
