...
(function( exports, $ ){
	...
	( function( history ) {
		...
		injectUrlWithState = function( url ) {
			...
			if ( oldQueryParams.customize_myskin ) {
				newQueryParams.customize_myskin = oldQueryParams.customize_myskin;
			}
	...
	/**
	 * @memberOf mcms.customize
	 * @alias mcms.customize.Preview
	 *
	 * @constructor
	 ...
	 */
	api.Preview = api.Messenger.extend(/** @lends mcms.customize.Preview.prototype */{
		/**
		...
		handleLinkClick: function( event ) {
			...
			if ( ! api.isLinkPreviewable( link[0] ) ) {
				mcms.a11y.speak( api.settings.l10n.linkUnpreviewable );
		...
		handleFormSubmit: function( event ) {
			...
			if ( 'GET' !== form.prop( 'method' ).toUpperCase() || ! api.isLinkPreviewable( urlParser ) ) {
				mcms.a11y.speak( api.settings.l10n.formUnpreviewable );
				event.preventDefault();
				return;
			}
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		// Skip mcms login and signup pages.
		if ( /\/mcms-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/mcms-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/mcms-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		if ( $element.closest( '#mcmsadminbar' ).length ) {
			return;
		}
		...
		// Ignore links with class mcms-playlist-caption
		if ( $element.hasClass( 'mcms-playlist-caption' ) ) {
			return;
		}
		...
		if ( ! api.settings.myskin.active ) {
			queryParams.customize_myskin = api.settings.myskin.stylesheet;
		}
		if ( api.settings.channel ) {
	...
	api.prepareFormPreview = function prepareFormPreview( form ) {
		...
		if ( ! api.settings.myskin.active ) {
			stateParams.customize_myskin = api.settings.myskin.stylesheet;
		}
		if ( api.settings.channel ) {
	...
	api.keepAliveCurrentUrl = ( function() {
		var previousPathName = location.pathname,
			...
			stateQueryParams = [ 'customize_myskin', 'customize_changeset_uuid', 'customize_messenger_channel', 'customize_autosaved' ];
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'mcms-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /mcms-roots/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'mcms-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'mcms-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the mcms-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
		...
		api( 'custom_css[' + api.settings.myskin.stylesheet + ']', function( setting ) {
