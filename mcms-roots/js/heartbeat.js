 ...
 * @see mcms_ajax_nopriv_heartbeat(), mcms_ajax_heartbeat()
 *
...
( function( $, window, undefined ) {
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.mcms-heartbeat', function() {
			...
			$(window).on( 'unload.mcms-heartbeat', function() {
		...
		function userIsActive() {
			...
			$document.off( '.mcms-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.mcms-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.mcms-heartbeat-active keyup.mcms-heartbeat-active touchend.mcms-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.mcms-heartbeat-active keyup.mcms-heartbeat-active touchend.mcms-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 * Use mcms.heartbeat.isQueued('handle') to see if any data is already queued for that handle.
		 *
	...
	window.mcms.heartbeat = new Heartbeat();
