...
/** @namespace mcms.customize.selectiveRefresh */
mcms.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @memberOf mcms.customize.selectiveRefresh
	 *
	 * @see PHP class MCMS_Customize_Partial.
	 *
	 * @class
	 * @augments mcms.customize.Class
	 * @since 4.5.0
	 */
	Partial = self.Partial = api.Class.extend(/** @lends mcms.customize.SelectiveRefresh.Partial.prototype */{
		 ...
		 * @since 4.5.0
		 *
		 * @param {mcms.customize.Value|string} setting  ID or object for setting.
		...
		showControl: function() {
			...
			if ( partial.getType() === 'nav_menu' ) {
				if ( partial.params.navMenuArgs.myskin_location ) {
					settingId = 'nav_menu_locations[' + partial.params.navMenuArgs.myskin_location + ']';
		 ...
		 * @since 4.5.0
		 *
		 * @this {mcms.customize.selectiveRefresh.Partial}
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( mcms.emoji && mcms.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = mcms.emoji.parse( content );
				}
			...
			/*
			 * Note that the 'mcms_audio_shortcode_library' and 'mcms_video_shortcode_library' filters
			 * will determine whether or not mcms.mediaelement is loaded and whether it will
			 * initialize audio and video respectively. See also https://core.trac.mandarincms.com/ticket/40144
			 */
			if ( mcms.mediaelement ) {
				mcms.mediaelement.initialize();
			}

			if ( mcms.playlist ) {
				mcms.playlist.initialize();
			}
	 ...
	 * @since 4.5.0
	 */
	self.Placement = Placement = api.Class.extend(/** @lends mcms.customize.selectiveRefresh.prototype */{

		/**
		 * The partial with which the container is associated.
		 *
		 * @param {mcms.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	 ...
	 * @since 4.5.0
	 *
	 * @type {Object.<string, mcms.customize.selectiveRefresh.Partial>}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @see mcms.customize.previewer.query()
	 *
	 * @return {object}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @type {Object<string, { deferred: jQuery.Promise, partial: mcms.customize.selectiveRefresh.Partial }>}
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param {mcms.customize.selectiveRefresh.Partial} partial
	 * @return {jQuery.Promise}
	 */
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = mcms.ajax.send( null, {
					data: data,
				...
				request.done( function( data ) {
					 ...
					 * so modules can inject data from the server to be utilized
					 * on the client via this event. Modules may use this filter
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to mcms.customize
		 * will only include the changed setting object as an argument, not including the
		 * new value or the old value.
		 *
		 * @since 4.5.0
		 * @this {mcms.customize.Setting}
		 *
		 ...
		 *
		 * @since 4.5.0
		 * @this {mcms.customize.Values}
		 *
		 * @param {mcms.customize.Setting} setting
		 */
