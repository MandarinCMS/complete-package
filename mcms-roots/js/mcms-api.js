(function( window, undefined ) {
	...
	/**
	 * Initialise the MCMS_API.
	 */
	function MCMS_API() {
		/** @namespace mcms.api.models */
		this.models = {};
		/** @namespace mcms.api.collections */
		this.collections = {};
		/** @namespace mcms.api.views */
		this.views = {};
	}
	...
	/** @namespace mcms.api */
	mcms.api               = mcms.api || new MCMS_API();
	mcms.api.versionString = mcms.api.versionString || 'mcms/v2/';
...
(function( window, undefined ) {
	...
	/** @namespace mcms.api */
	mcms.api = mcms.api || {};
	/** @namespace mcms.api.utils */
	mcms.api.utils = mcms.api.utils || {};
	 ...
	 */
	mcms.api.getModelByRoute = function( route ) {
		return _.find( mcms.api.models, function( model ) {
	 ...
	 */
	mcms.api.getCollectionByRoute = function( route ) {
		return _.find( mcms.api.collections, function( collection ) {
	 ...
	 * @param {Date} date.
	 */
	mcms.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	mcms.api.utils.getRootUrl = function() {
	 ...
	 */
	mcms.api.utils.capitalize = function( str ) {
	 ...
	 */
	mcms.api.utils.capitalizeAndCamelCaseDashes = function( str ) {
		if ( _.isUndefined( str ) ) {
			return str;
		}
		str = mcms.api.utils.capitalize( str );

		return mcms.api.utils.camelCaseDashes( str );
	};

	/**
	 * Helper function to camel case the letter after dashes, removing the dashes.
	 */
	mcms.api.utils.camelCaseDashes = function( str ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `mcms.api.versionString`.
	 * @param {boolean} [reverse]       Whether to reverse the order when extracting the route part. Optional, default false.
	 */
	mcms.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		var routeParts;

		part = part || 1;
		versionString = versionString || mcms.api.versionString;
	 ...
	 */
	mcms.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	mcms.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	mcms.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. MandarinCMS expects dates in UTC.
				 ...
				 *
				 * MandarinCMS returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				getDate: function( field ) {
					...
					return new Date( mcms.api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new mcms.api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a mcms.api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new mcms.api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				getTags: function() {
					var tagIds = this.get( 'tags' ),
						tags  = new mcms.api.collections.Tags();
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new mcms.api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new mcms.api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new mcms.api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new mcms.api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new mcms.api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new mcms.api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new mcms.api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', 'mcms:featuredmedia', 'source_url' );
				}
			};
...
(function() {
	...
	var mcmsApiSettings = window.mcmsApiSettings || {},
	trashableTypes    = [ 'Comment', 'Media', 'Comment', 'Post', 'Page', 'Status', 'Taxonomy', 'Type' ];

	/**
	 * Backbone base model for all models.
	 */
	mcms.api.MCMSApiBaseModel = Backbone.Model.extend(
		/** @lends MCMSApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-MCMS-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-MCMS-Nonce' );
	 ...
	 */
	mcms.api.models.Schema = mcms.api.MCMSApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				mcms.api.MCMSApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || mcmsApiSettings.root;
				model.versionString = options.versionString || mcmsApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	mcms.api.MCMSApiBaseCollection = Backbone.Collection.extend(
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-mcms-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-mcms-total' ), 10 );
						}
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		mcmsApiSettings = window.mcmsApiSettings || {};
	...
	// If mcmsApiSettings is unavailable, try the default.
	if ( _.isEmpty( mcmsApiSettings ) ) {
		mcmsApiSettings.root = window.location.origin + '/mcms-json/';
	}

	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		defaults: {
			apiRoot: mcmsApiSettings.root,
			versionString: mcms.api.versionString,
			nonce: null,
			schema: null,
			models: {},
			collections: {}
		...
		initialize: function() {
			...
			model.schemaModel = new mcms.api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( mcmsApiSettings.cacheSchema ) || mcmsApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'mcms-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'mcms-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( mcmsApiSettings.cacheSchema ) || mcmsApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'mcms-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable mcmsApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = mcmsApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( mcms.api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
					routeName  = mcms.api.utils.extractRoutePart( modelRoute.index, 2, routeModel.get( 'versionString' ), true ),
					parentName = mcms.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), false ),
					routeEnd   = mcms.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), true );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = mcms.api.utils.capitalizeAndCamelCaseDashes( parentName ) + mcms.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mcms.api.MCMSApiBaseModel.extend( {
					...
					modelClassName = mcms.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mcms.api.MCMSApiBaseModel.extend( {
				...
				mcms.api.utils.decorateFromRoute(
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = mcms.api.utils.extractRoutePart( collectionRoute.index, 1, routeModel.get( 'versionString' ), false );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {

					collectionClassName = mcms.api.utils.capitalizeAndCamelCaseDashes( parentName ) + mcms.api.utils.capitalizeAndCamelCaseDashes( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = mcms.api.MCMSApiBaseCollection.extend( {
					...
					collectionClassName = mcms.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = mcms.api.MCMSApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				mcms.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = mcms.api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	mcms.api.endpoints = new Backbone.Collection();

	/**
	 * Initialize the mcms-api, optionally passing the API root.
	 *
	 * @param {object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to mcmsApiSettings.nonce.
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to mcmsApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to mcmsApiSettings.root.
	 * @param {object} [args.schema] The schema. Optional, will be fetched from API if not provided.
	 */
	mcms.api.init = function( args ) {
		var endpoint, attributes = {}, deferred, promise;

		args                      = args || {};
		attributes.nonce          = _.isString( args.nonce ) ? args.nonce : ( mcmsApiSettings.nonce || '' );
		attributes.apiRoot        = args.apiRoot || mcmsApiSettings.root || '/mcms-json';
		attributes.versionString  = args.versionString || mcmsApiSettings.versionString || 'mcms/v2/';
		attributes.schema         = args.schema || null;
		attributes.modelEndpoints = args.modelEndpoints || [ 'me', 'settings' ];
		if ( ! attributes.schema && attributes.apiRoot === mcmsApiSettings.root && attributes.versionString === mcmsApiSettings.versionString ) {
			attributes.schema = mcmsApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {

			// Look for an existing copy of this endpoint
			endpoint = mcms.api.endpoints.findWhere( { 'apiRoot': attributes.apiRoot, 'versionString': attributes.versionString } );
			if ( ! endpoint ) {
			...
			endpoint.schemaConstructed.done( function( resolvedEndpoint ) {
				mcms.api.endpoints.add( resolvedEndpoint );

				// Map the default endpoints, extending any already present items (including Schema model).
				mcms.api.models      = _.extend( mcms.api.models, resolvedEndpoint.get( 'models' ) );
				mcms.api.collections = _.extend( mcms.api.collections, resolvedEndpoint.get( 'collections' ) );
	...
	// The mcms.api.init function returns a promise that will resolve with the endpoint once it is ready.
	mcms.api.loadPromise = mcms.api.init();
