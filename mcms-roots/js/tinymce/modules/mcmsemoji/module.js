( function( tinymce, mcms, settings ) {
	tinymce.ModuleManager.add( 'mcmsemoji', function( editor ) {
		var typing,
			env = tinymce.Env,
		...
		if ( ! mcms || ! mcms.emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-mcms-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			mcms.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after mcms.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-mcms-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-mcms-emoji' ) ) {
				event.preventDefault();
			}
		} );
	} );
} )( window.tinymce, window.mcms, window._mcmsemojiSettings );
