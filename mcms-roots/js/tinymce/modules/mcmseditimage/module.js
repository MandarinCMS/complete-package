/* global tinymce */
tinymce.ModuleManager.add( 'mcmseditimage', function( editor ) {
	...
	editor.addButton( 'mcms_img_remove', {
		tooltip: 'Remove',
	...
	editor.addButton( 'mcms_img_edit', {
	...
	each( {
		...
		editor.addButton( 'mcms_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'mcmsAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.mcms-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:mcms_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:mcms_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'mcmseditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="mcms-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="mcms-caption-dt">'+ img +'</dt><dd class="mcms-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+mcms-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /mcms-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^mcms-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'mcms-image-', '' ), 10 );
		...
		// Extract caption
		captionBlock = dom.getParents( imageNode, '.mcms-caption' );
		...
		if ( captionBlock.length ) {
			...
			tinymce.each( classes, function( name ) {
				if ( /^align/.test( name ) ) {
					metadata.align = name.replace( 'align', '' );
				} else if ( name && name !== 'mcms-caption' ) {
			...
			caption = dom.select( 'dd.mcms-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'mcms-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.mcms-caption', captionNode );
				...
				dd = dom.select( '.mcms-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="mcms-caption-dt"></dt><dd class="mcms-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.mcms-caption-dt' ).append( node );
		...
		if ( mcms.media.events ) {
			mcms.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		if ( typeof mcms === 'undefined' || ! mcms.media ) {
		...
		mcms.media.events.trigger( 'editor:image-edit', {
			editor: editor,
			metadata: metadata,
			image: img
		} );
		...
		mcms.media.events.trigger( 'editor:frame-create', { frame: frame } );
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'mcmseditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Prevent IE11 from making dl.mcms-caption resizable
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.mcms-caption' ) ) {
					// Hide the thick border with resize handles around dl.mcms-caption
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'mcms-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.mcms-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.mcms-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'mcmsAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.mcms-caption' );
	...
	editor.mcmsSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.mcmsGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__mcms-temp-img-id"]' ).attr( 'id', null );
		}
	});

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.mcmsSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.mcmsGetImgCaption( event.content );
		}
	});
	...
	editor.mcms.isPlaceholder = isPlaceholder;
