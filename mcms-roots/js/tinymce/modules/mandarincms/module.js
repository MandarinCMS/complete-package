...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #mcmsadminbar (100000)
if ( ! tinymce.ui.FloatPanel.zIndex || tinymce.ui.FloatPanel.zIndex < 100100 ) {
	tinymce.ui.FloatPanel.zIndex = 100100;
}

tinymce.ModuleManager.add( 'mandarincms', function( editor ) {
	var mcmsAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
		__ = editor.editorManager.i18n.translate,
		$ = window.jQuery,
		mcms = window.mcms,
		hasWpautop = ( mcms && mcms.editor && mcms.editor.autop && editor.getParam( 'mcmsautop', true ) );
	...
	function toggleToolbars( state ) {
		...
		if ( editor.myskin.panel ) {
			toolbars = editor.myskin.panel.find('.toolbar:not(.menubar)');
		}
		...
		if ( pixels && ! initial ) {
			...
			if ( state === 'hide' ) {
				...
				mcmsAdvButton && mcmsAdvButton.active( false );
			} else {
				setUserSetting('hidetb', '1');
				mcmsAdvButton && mcmsAdvButton.active( true );
			}
		}

		editor.fire( 'mcms-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'mcms_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'MCMS_Adv',
		onPostRender: function() {
			mcmsAdvButton = this;
			mcmsAdvButton.active( getUserSetting( 'hidetb' ) === '1' ? true : false );
		}
	});

	// Hide the toolbars after loading
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'mandarincms_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		}
	});

	editor.addCommand( 'MCMS_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.mcmsActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-mcms-more="more" data-mcms-more-text="' + moretext + '" ' +
						'class="mcms-more-tag mce-mcms-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-mcms-more="nextpage" class="mcms-more-tag mce-mcms-nextpage" ' +
						'alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />' );
			}

			if ( event.load && event.format !== 'raw' && hasWpautop ) {
				event.content = mcms.editor.autop( event.content );
			}
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-mcms-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-mcms-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-mcms-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-mcms-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-mcms-preserve' ) !== -1 ) {
					if ( match = image.match( / data-mcms-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-mcms-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands
	editor.addCommand( 'MCMS_More', function( tag ) {
		var parent, html, title,
			classname = 'mcms-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-mcms-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-mcms-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'MCMS_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'MCMS_Page', function() {
		editor.execCommand( 'MCMS_More', 'nextpage' );
	});

	editor.addCommand( 'MCMS_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl + Alt + letter:' ) : __( 'Shift + Alt + letter:' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.mcms_shortcut_labels,
		...
		if ( editor.modules.mcmstextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-mcms-help' );
	...
	editor.addCommand( 'MCMS_Medialib', function() {
		if ( mcms && mcms.media && mcms.media.editor ) {
			mcms.media.editor.open( editor.id );
		}
	});

	// Register buttons
	editor.addButton( 'mcms_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'MCMS_More', 'more' );
		}
	});

	editor.addButton( 'mcms_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'MCMS_More', 'nextpage' );
		}
	...
	editor.addButton( 'mcms_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'MCMS_Help'
	});

	editor.addButton( 'mcms_code', {
		tooltip: 'Code',
		cmd: 'MCMS_Code',
		stateSelector: 'code'
	});
	...
	// Menubar
	// Insert->Upload&nbsp;Attachment
	if ( mcms && mcms.media && mcms.media.editor ) {
		editor.addMenuItem( 'add_media', {
			text: 'Upload&nbsp;Attachment',
			icon: 'mcms-media-library',
			context: 'insert',
			cmd: 'MCMS_Medialib'
		});
	}
	...
	editor.on( 'init', function() {
		var env = tinymce.Env,
			bodyClass = ['mceContentBody'], // back-compat for myskins that use this in editor-style.css...
			doc = editor.getDoc(),
			dom = editor.dom;
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'mcms_lang_attr' ) );
		...
		if ( editor.getParam( 'mcms_paste_filters', true ) ) {
		...
		if ( editor.settings.mcms_shortcut_labels && editor.myskin.panel ) {
			var labels = {};
			...
			each( editor.settings.mcms_shortcut_labels, function( value, name ) {
				labels[ name ] = value.replace( 'access', access ).replace( 'meta', meta );
			} );

			each( editor.myskin.panel.find('button'), function( button ) {
			...
			each( editor.myskin.panel.find('listbox'), function( listbox ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			event.content = mcms.editor.removep( event.content );
		}
	});
	...
	editor.on( 'preInit', function() {
		...
		editor.addShortcut( 'meta+s', '', function() {
			if ( mcms && mcms.autosave ) {
				mcms.autosave.server.triggerSave();
			}
		} );
	 ...
	 * This functionality will change in the next releases. Not recommended for use by modules.
	 */
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			mcmsAdminbar = document.getElementById( 'mcmsadminbar' ),
			...
			if ( editor.id === 'content' ) {
				mcmsStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					mcmsAdminbarBottom = mcmsAdminbar ? mcmsAdminbar.getBoundingClientRect().bottom : 0,
					...
					mcmsStatusbarTop = mcmsStatusbar ? windowHeight - mcmsStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, mcmsAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, mcmsStatusbarTop, windowHeight - iframeRect.bottom ),
			...
			toolbar.on( 'show', function() {
				...
				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-tooltip' ).addClass( 'mcms-hide-mce-tooltip' );
				}
			} );

			toolbar.on( 'hide', function() {
				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-tooltip' ).removeClass( 'mcms-hide-mce-tooltip' );
				}
			} );
		...
		editor.mcms._createToolbar = create;
	}, true );
