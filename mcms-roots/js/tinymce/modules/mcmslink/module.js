( function( tinymce ) {
	tinymce.ui.Factory.add( 'MCMSLinkPreview', tinymce.ui.Control.extend( {
		url: '#',
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="mcms-link-preview">' +
	...
	tinymce.ui.Factory.add( 'MCMSLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="mcms-link-input">' +
	...
	tinymce.ModuleManager.add( 'mcmslink', function( editor ) {
		var toolbar;
		var editToolbar;
		var previewInstance;
		var inputInstance;
		...
		var speak = ( typeof window.mcms !== 'undefined' && window.mcms.a11y && window.mcms.a11y.speak ) ? window.mcms.a11y.speak : function() {};
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '_mcms_link_placeholder' ) {
					editor.dom.remove( element, true );
				} else if ( $element.attr( 'data-mcmslink-edit' ) ) {
					$element.attr( 'data-mcmslink-edit', null );
				}
			});
		}
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_mcms_link_placeholder"' ) > -1 ) {
					return text;
				}

				if ( dataAttr ) {
					tag = tag.replace( / data-mcmslink-edit="true"/g, '' );
				}

				tag = tag.replace( / data-mcmslink-url-error="true"/g, '' );
		...
		editor.on( 'preinit', function() {
			if ( editor.mcms && editor.mcms._createToolbar ) {
				...
				if ( typeof window.mcmsLink !== 'undefined' ) {
					editButtons.push( 'mcms_link_advanced' );
				}

				editToolbar = editor.mcms._createToolbar( editButtons, true );

				editToolbar.on( 'show', function() {
					if ( typeof window.mcmsLink === 'undefined' || ! window.mcmsLink.modalOpen ) {
						window.setTimeout( function() {
							...
							if ( element ) {
								if ( ! element.value && selection && typeof window.mcmsLink !== 'undefined' ) {
									element.value = window.mcmsLink.getUrlFromSelection( selection );
								}
				...
				editToolbar.on( 'hide', function() {
					if ( ! editToolbar.scrolling ) {
						editor.execCommand( 'mcms_link_cancel' );
					}
				} );
			}
		} );

		editor.addCommand( 'MCMS_Link', function() {
			if ( tinymce.Env.ie && tinymce.Env.ie < 10 && typeof window.mcmsLink !== 'undefined' ) {
				window.mcmsLink.open( editor.id );
				return;
			}
			...
			if ( linkNode ) {
				editor.dom.setAttribs( linkNode, { 'data-mcmslink-edit': true } );
			} else {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_mcms_link_placeholder' } );

				linkNode = editor.$( 'a[href="_mcms_link_placeholder"]' )[0];
				editor.nodeChanged();
			}
		} );

		editor.addCommand( 'mcms_link_apply', function() {
			if ( editToolbar.scrolling ) {
				return;
			}
			...
			if ( linkNode ) {
				...
				editor.dom.setAttribs( linkNode, { href: href, 'data-mcmslink-edit': null } );
			...
			if ( typeof window.mcmsLinkL10n !== 'undefined' && ! hasLinkError ) {
				speak( window.mcmsLinkL10n.linkInserted );
			}
		} );

		editor.addCommand( 'mcms_link_cancel', function() {
		...
		editor.addCommand( 'mcms_unlink', function() {
			editor.execCommand( 'unlink' );
			editToolbar.tempHide = false;
			editor.execCommand( 'mcms_link_cancel' );
		} );

		// MCMS default shortcuts
		editor.addShortcut( 'access+a', '', 'MCMS_Link' );
		editor.addShortcut( 'access+s', '', 'mcms_unlink' );
		// The "de-facto standard" shortcut, see #27305
		editor.addShortcut( 'meta+k', '', 'MCMS_Link' );
		...
		editor.addButton( 'mcms_link_preview', {
			type: 'MCMSLinkPreview',
			onPostRender: function() {
				previewInstance = this;
			}
		} );

		editor.addButton( 'mcms_link_input', {
			type: 'MCMSLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-mcms-autocomplete-' + ui.item.ID );
							/*
						...
						select: function( event, ui ) {
							...
							if ( 9 === event.keyCode && typeof window.mcmsLinkL10n !== 'undefined' ) {
								// Audible confirmation message when a link has been selected.
								speak( window.mcmsLinkL10n.linkSelected );
							}
						...
						var fallbackTitle = ( typeof window.mcmsLinkL10n !== 'undefined' ) ? window.mcmsLinkL10n.noTitle : '',
							title = item.title ? item.title : fallbackTitle;

						return $( '<li role="option" id="mce-mcms-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="mcms-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
					...
					.autocomplete( 'widget' )
						.addClass( 'mcmslink-autocomplete' )
						.attr( 'role', 'listbox' )
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'mcms_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.on( 'mcmstoolbar', function( event ) {
			...
			if ( linkNode ) {
				...
				edit = $linkNode.attr( 'data-mcmslink-edit' );

				if ( href === '_mcms_link_placeholder' || edit ) {
					if ( href !== '_mcms_link_placeholder' && ! inputInstance.getURL() ) {
					...
					if ( $linkNode.attr( 'data-mcmslink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.mcms-link-preview a' ).addClass( 'mcmslink-url-error' );
					} else {
						toolbar.$el.find( '.mcms-link-preview a' ).removeClass( 'mcmslink-url-error' );
						hasLinkError = false;
					}
				}
		...
		editor.addButton( 'mcms_link_edit', {
			tooltip: 'Edit ', // trailing space is needed, used for context
			icon: 'dashicon dashicons-edit',
			cmd: 'MCMS_Link'
		} );

		editor.addButton( 'mcms_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'mcms_unlink'
		} );

		editor.addButton( 'mcms_link_advanced', {
			...
			onclick: function() {
				if ( typeof window.mcmsLink !== 'undefined' ) {
					...
					editToolbar.tempHide = true;
					window.mcmsLink.open( editor.id, url, text, linkNode );
		...
		editor.addButton( 'mcms_link_apply', {
			tooltip: 'Apply',
