/* global adminpage, mcmsActiveEditor, quicktagsL10n, mcmsLink, prompt */
/*
 * Quicktags
 *
 * This is the HTML editor in MandarinCMS. It can be attached to any textarea and will
 ...
 * Back-compat
 *
 * Define all former global functions so modules that hack quicktags.js directly don't cause fatal errors.
 */
...
(function(){
	...
	qt = QTags = function(settings) {
		...
		wrap = document.getElementById( 'mcms-' + id + '-wrap' );
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'mcms_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(mcmsActiveEditor), event;
	...
	qt.Button.prototype.html = function(idPrefix) {
		var active, on, mcms,
			...
			dfw = ( mcms = window.mcms ) && mcms.editor && mcms.editor.dfw;
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( mcms = window.mcms ) || ! mcms.editor || ! mcms.editor.dfw ) {
			return;
		}

		window.mcms.editor.dfw.toggle();
	};
