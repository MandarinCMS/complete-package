...
(function($){
	...
	/**
	 * @class $.widget.mcms.pointer
	 */
	$.widget('mcms.pointer',/** @lends $.widget.mcms.pointer.prototype */{
		options: {
			pointerClass: 'mcms-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( mcmsPointerL10n ) ? mcmsPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="mcms-pointer-content"></div>');
			this.arrow   = $('<div class="mcms-pointer-arrow"><div class="mcms-pointer-arrow-inner"></div></div>');
			...
			this.pointer = $('<div />')
				.append( this.content )
				.append( this.arrow )
				.attr('id', 'mcms-pointer-' + identifier++)
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="mcms-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /mcms-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'mcms-pointer-' + edge );
		},
