 ...
 * |  |- ...
 */
( function( window, mcms, shortcode, $ ) {
	'use strict';
	...
	mcms.mce.views = {
		 ...
		 * @param {Object} extend An object to extend mcms.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = mcms.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content, editor ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-mcmsview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-mcmsview-marker=/g, '<p data-mcmsview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {mcms.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {mcms.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-mcmsview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-mcmsview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	mcms.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	mcms.mce.View.extend = Backbone.View.extend;

	_.extend( mcms.mce.View.prototype, /** @lends mcms.mce.View.prototype */{
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mcmsview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mcmsview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== tinymce.DOM.decode( this.text ) ) {
					editor.dom.setAttrib( node, 'data-mcmsview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="mcmsview mcmsview-wrap" data-mcmsview-text="' + this.encodedText + '" data-mcmsview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			if ( _.isObject( content ) && ( content.sandbox || content.head || content.body.indexOf( '<script' ) !== -1 ) ) {
				...
				this.getNodes( function( editor, node ) {
					...
					editor.undoManager.transact( function() {
						...
						editor.dom.add( node, 'span', { 'class': 'mcmsview-end' } );
					} );
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/mandarincms/mcms-plugins.css' ) === -1 ) {
				...
				editor.undoManager.transact( function() {
					...
					dom.add( node, 'span', { 'class': 'mcmsview-end' } );
				} );
						...
						'<head>' +
							...
							'<style>' +
								'html {' +
									...
									'margin: 0;' +
								'}' +
								'body#mcmsview-iframe-sandbox {' +
									'background: transparent;' +
									'padding: 1px 0 !important;' +
									'margin: -1px 0 0 !important;' +
								'}' +
								'body#mcmsview-iframe-sandbox:before,' +
								'body#mcmsview-iframe-sandbox:after {' +
									'display: none;' +
									'content: "";' +
								'}' +
								'iframe {' +
							...
							'</style>' +
						'</head>' +
						'<body id="mcmsview-iframe-sandbox" class="' + bodyClasses + '">' +
							body +
						'</body>' +
					'</html>'
				);
		...
		setLoader: function( dashicon ) {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="mcmsview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {String} dashicon A dashicon ID. Optional. {@link https://developer.mandarincms.com/resource/dashicons/}
		 */
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-mcmsview-text', encodeURIComponent( text ) );
					mcms.mce.views.createInstance( type, text, match.options, force ).render();
...
} )( window, window.mcms, window.mcms.shortcode, window.jQuery );

/*
 * The MandarinCMS core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'mcmsview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.mcmsview-sandbox', content ).get( 0 );
...
} )( window, window.mcms.mce.views, window.mcms.media, window.jQuery );
