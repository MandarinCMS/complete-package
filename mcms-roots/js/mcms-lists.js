...
mcmsList = {
	 ...
	 * 4. `_mcmsnonce` value in element's href attribute.
	 * 5. `_mcmsnonce` input field that is a descendant of element.
	...
	nonce: function( element, settings ) {
		var url      = mcmsAjax.unserialize( element.attr( 'href' ) ),
			$element = $( '#' + settings.element );

		return settings.nonce || url._ajax_nonce || $element.find( 'input[name="_ajax_nonce"]' ).val() || url._mcmsnonce || $element.find( 'input[name="_mcmsnonce"]' ).val() || 0;
	},

	/**
	 * Extract list item data from a DOM element.
	 *
	 * Example 1: data-mcms-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-mcms-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		var data = [], mcmsListsData;

		try {
			mcmsListsData = $( element ).data( 'mcms-lists' ) || '';
			mcmsListsData = mcmsListsData.match( new RegExp( type + ':[\\S]+' ) );

			if ( mcmsListsData ) {
				data = mcmsListsData[0].split( ':' );
			}
		} catch ( error ) {}
	...
	pre: function( list, settings, action ) {
		...
		settings = $.extend( {}, this.mcmsList.settings, {
			element: null,
			nonce:   0,
			target:  list.get( 0 )
		}, settings || {} );
	...
	ajaxAdd: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mcmsList.parseData( $element, 'add' ),
			formValues, formData, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mcmsList.pre.call( list, $element, settings, 'add' );
		...
		if ( ! $element.is( '[id="' + settings.element + '-submit"]' ) ) {
			return ! mcmsList.add.call( list, $element, settings );
		}
		...
		settings.nonce  = mcmsList.nonce( $element, settings );

		if ( ! mcmsAjax.validateForm( '#' + settings.element ) ) {
			return false;
		}
		...
		}, mcmsAjax.unserialize( data[4] || '' ) ) );

		formValues = $( '#' + settings.element + ' :input' ).not( '[name="_ajax_nonce"], [name="_mcmsnonce"], [name="action"]' );
		...
		settings.success = function( response ) {
			parsedResponse   = mcmsAjax.parseAjaxResponse( response, settings.response, settings.element );
			...
			$.each( parsedResponse.responses, function() {
				mcmsList.add.call( list, this.data, $.extend( {}, settings, { // this.firstChild.nodevalue
			...
			list.mcmsList.recolor();
			$( list ).trigger( 'mcmsListAddEnd', [ settings, list.mcmsList ] );
			mcmsList.clear.call( list, '#' + settings.element );
		};
	...
	ajaxDel: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mcmsList.parseData( $element, 'delete' ),
			$eventTarget, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mcmsList.pre.call( list, $element, settings, 'delete' );
		...
		if ( 'none' !== settings.delColor ) {
			$eventTarget.css( 'backgroundColor', settings.delColor ).fadeOut( 350, function() {
				list.mcmsList.recolor();
				$( list ).trigger( 'mcmsListDelEnd', [ settings, list.mcmsList ] );
			} );
		} else {
			list.mcmsList.recolor();
			$( list ).trigger( 'mcmsListDelEnd', [ settings, list.mcmsList ] );
	...
	ajaxDim: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mcmsList.parseData( $element, 'dim' ),
		...
		settings = settings || {};
		settings = mcmsList.pre.call( list, $element, settings, 'dim' );
		...
		color        = mcmsList.getColor( $eventTarget );
			...
			$eventTarget
				...
				.animate( { backgroundColor: color }, {
					complete: function() {
						...
						$( list ).trigger( 'mcmsListDimEnd', [ settings, list.mcmsList ] );
					}
				} );
		} else {
			$( list ).trigger( 'mcmsListDimEnd', [ settings, list.mcmsList ] );
	...
	add: function( element, settings ) {
		...
		settings = $.extend( { position: 0, id: 0, oldId: null }, this.mcmsList.settings, settings );
		...
		if ( 'none' !== settings.addColor ) {
			$element.css( 'backgroundColor', settings.addColor ).animate( { backgroundColor: mcmsList.getColor( $element ) }, {
				complete: function() {
					$( this ).css( 'backgroundColor', '' );
				}
			} );
	...
	clear: function( elementId ) {
		...
		if ( list.mcmsList && $element.parents( '#' + list.id ).length ) {
			return;
		}
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-mcms-lists^="add:' + list.id + ':"]', function() {
			return list.mcmsList.add( this );
		} );

		$element.on( 'click', 'a[data-mcms-lists^="add:' + list.id + ':"], input[data-mcms-lists^="add:' + list.id + ':"]', function() {
			return list.mcmsList.add( this );
		} );

		$element.on( 'click', '[data-mcms-lists^="delete:' + list.id + ':"]', function() {
			return list.mcmsList.del( this );
		} );

		$element.on( 'click', '[data-mcms-lists^="dim:' + list.id + ':"]', function() {
			return list.mcmsList.dim( this );
		} );
	},
	...
	recolor: function() {
		...
		if ( list.mcmsList.settings.altOffset % 2 ) {
			evenOdd.reverse();
		}

		items.filter( evenOdd[0] ).addClass( list.mcmsList.settings.alt ).end();
		items.filter( evenOdd[1] ).removeClass( list.mcmsList.settings.alt );
	},
	...
	init: function() {
		...
		$list.mcmsList.process = function( element ) {
			$list.each( function() {
				this.mcmsList.process( element );
			} );
		};

		$list.mcmsList.recolor = function() {
			$list.each( function() {
				this.mcmsList.recolor();
			} );
		};
	}
};
 ...
 *
 * @param {mcmsList~confirm}   settings.confirm     Callback that's run before a request is made. Default: null.
 * @param {mcmsList~addBefore} settings.addBefore   Callback that's run before an item gets added to the list.
 *                                                Default: null.
 * @param {mcmsList~addAfter}  settings.addAfter    Callback that's run after an item got added to the list.
 *                                                Default: null.
 * @param {mcmsList~delBefore} settings.delBefore   Callback that's run before an item gets deleted from the list.
 *                                                Default: null.
 * @param {mcmsList~delAfter}  settings.delAfter    Callback that's run after an item got deleted from the list.
 *                                                Default: null.
 * @param {mcmsList~dimBefore} settings.dimBefore   Callback that's run before an item gets dim'd. Default: null.
 * @param {mcmsList~dimAfter}  settings.dimAfter    Callback that's run after an item got dim'd. Default: null.
 * @returns {$.fn} mcmsList API function.
 */
$.fn.mcmsList = function( settings ) {
	this.each( function( index, list ) {
		list.mcmsList = {
			settings: $.extend( {}, mcmsList.settings, { what: mcmsList.parseData( list, 'list' )[1] || '' }, settings )
		};

		$.each( functions, function( func, callback ) {
			list.mcmsList[func] = function( element, setting ) {
				return mcmsList[callback].call( list, element, setting );
			};
		} );
	} );

	mcmsList.init.call( this );
	this.mcmsList.process();
