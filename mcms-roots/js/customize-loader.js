/* global _mcmsCustomizeLoaderSettings */
/**
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo mcms_customize_url(); ?>">Open Customizer</a>
	 *
	 * @memberOf mcms.customize
	 *
	 * @class
	 * @augments mcms.customize.Events
	 */
	Loader = $.extend( {}, api.Events,/** @lends mcms.customize.Loader.prototype */{
		/**
		...
		initialize: function() {
			...
			// the Customizer.
			$('#mcmsbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'mcms_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'mcms_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _mcmsCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.mcms.customize.Loader
	api.Loader = Loader;
})( mcms, jQuery );
