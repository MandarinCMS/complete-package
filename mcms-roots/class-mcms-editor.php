<?php
/**
 * Facilitates adding of the MandarinCMS editor as used on the Write and Edit screens.
 *
 * @package MandarinCMS
 * @since 3.3.0
 *
 * Private, not included by default. See mcms_editor() in mcms-roots/general-template.php.
 */

final class _MCMS_Editors {
	 ...
	 *
	 *     @type bool       $mcmsautop           Whether to use mcmsautop(). Default true.
	 *     @type bool       $media_buttons     Whether to show the Upload&nbsp;Attachment/other media buttons.
	 ...
	 *     @type bool       $dfw               Deprecated in 4.1. Since 4.3 used only to enqueue mcms-fullscreen-stub.js
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the mcms_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 * @param string $editor_id ID for the current editor instance.
		 */
		$settings = apply_filters( 'mcms_editor_settings', $settings, $editor_id );

		$set = mcms_parse_args( $settings, array(
			'mcmsautop'             => true,
			'media_buttons'       => true,
			'default_editor'      => '',
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'mcms_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['mcms_autoresize_on'] ) ) {
	 ...
	 * @param array $settings See _MCMS_Editors()::parse_settings() for description.
	 */
	public static function editor( $content, $editor_id, $settings = array() ) {
		$set = self::parse_settings( $editor_id, $settings );
		$editor_class = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' mcms-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : mcms_default_editor();
				// 'html' is used for the "Text" editor tab.
				if ( 'html' !== $default_editor ) {
					$default_editor = 'tinymce';
				}

				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="mcms-switch-editor switch-tmce"' .
					' data-mcms-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="mcms-switch-editor switch-html"' .
					' data-mcms-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class = 'mcms-core-ui mcms-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="mcms-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			mcms_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="mcms-' . $editor_id_attr . '-editor-tools" class="mcms-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) )
					include( BASED_TREE_URI . 'mcms-admin/includes/media.php' );

				echo '<div id="mcms-' . $editor_id_attr . '-media-buttons" class="mcms-media-buttons">';
			...
			echo '<div class="mcms-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		 ...
		 */
		$the_editor = apply_filters( 'the_editor', '<div id="mcms-' . $editor_id_attr . '-editor-container" class="mcms-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings($editor_id, $set) {
		...
		if ( empty(self::$first_init) ) {
			if ( is_admin() ) {
				...
				add_action( 'mcms_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'mcms_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					...
					/**
					 * Filters the list of teenyMCE modules.
					 *
					 * @since 2.7.0
					 *
					 * @param array  $modules   An array of teenyMCE modules.
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					$modules = apply_filters( 'teeny_mce_modules', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'mandarincms', 'mcmseditimage', 'mcmslink' ), $editor_id );
				} else {

					/**
					 * Filters the list of TinyMCE external modules.
					 *
					 * The filter takes an associative array of external modules for
					 * TinyMCE in the form 'module_name' => 'url'.
					 *
					 * The url should be absolute, and should include the js filename
					 * to be loaded. For example:
					 * 'mymodule' => 'http://mysite.com/mcms-plugins/modules/myfolder/mce_module.js'.
					 *
					 * If the external module adds a button, it should be added with
					 * one of the 'mce_buttons' filters.
					 *
					 * @since 2.5.0
					 *
					 * @param array $external_modules An array of external TinyMCE modules.
					 */
					$mce_external_modules = apply_filters( 'mce_external_modules', array() );
					...
					/**
					 * Filters the list of default TinyMCE modules.
					 *
					 * The filter specifies which of the default modules included
					 * in MandarinCMS should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
					 * @param array $modules An array of default TinyMCE modules.
					 */
					$modules = array_unique( apply_filters( 'tiny_mce_modules', $modules ) );

					if ( ( $key = array_search( 'spellchecker', $modules ) ) !== false ) {
						// Remove 'spellchecker' from the internal modules if added with 'tiny_mce_modules' filter to prevent errors.
						// It can be added with 'mce_external_modules'.
						unset( $modules[$key] );
					}

					if ( ! empty( $mce_external_modules ) ) {

						/**
						 * Filters the translations loaded for external TinyMCE 3.x modules.
						 *
						 * The filter takes an associative array ('module_name' => 'path')
						 * where 'path' is the include path to the file.
						 *
						 * The language file should follow the same format as mcms_mce_translation(),
						 * and should define a variable ($strings) that holds all translated strings.
						 *
						 * @since 2.5.0
						 *
						 * @param array $translations Translations for external TinyMCE modules.
						 */
						...
						foreach ( $mce_external_modules as $name => $url ) {
							if ( in_array( $name, $modules, true ) ) {
								unset( $mce_external_modules[ $name ] );
								continue;
							}

							$url = set_url_scheme( $url );
							$mce_external_modules[ $name ] = $url;
							$plugurl = dirname( $url );
							$strings = '';
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = MCMS_CONTENT_DIR . $path . '/langs/';
							...
							$ext_modules .= 'tinyMCEPreInit.load_ext("' . $plugurl . '", "' . $mce_locale . '");' . "\n";
							$ext_modules .= 'tinymce.ModuleManager.load("' . $name . '", "' . $url . '");' . "\n";
						}
					}
				}
				...
				$settings['modules'] = implode( ',', $modules );

				if ( ! empty( $mce_external_modules ) ) {
					$settings['external_modules'] = mcms_json_encode( $mce_external_modules );
				}

				/** This filter is documented in mcms-admin/includes/media.php */
				if ( apply_filters( 'disable_captions', '' ) ) {
					$settings['mcmseditimage_disable_captions'] = true;
				}
			...
			if ( $set['teeny'] ) {
				...
				$mce_buttons = array( 'formatselect', 'bold', 'italic', 'bullist', 'numlist', 'blockquote', 'alignleft', 'aligncenter', 'alignright', 'link', 'mcms_more', 'spellchecker' );
			...
			if ( $post = get_post() ) {
				...
				if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
					...
					if ( $post_format && ! is_mcms_error( $post_format ) )
			...
			$mceInit = array (
				'selector' => "#$editor_id",
				'mcmsautop' => (bool) $set['mcmsautop'],
				'indent' => ! $set['mcmsautop'],
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			mcms_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			mcms_enqueue_script( 'quicktags' );
			mcms_enqueue_style( 'buttons' );
		}

		if ( $default_scripts || in_array( 'mcmslink', self::$modules, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			mcms_enqueue_script( 'mcmslink' );
			mcms_enqueue_script( 'jquery-ui-autocomplete' );
		}

		if ( self::$old_dfw_compat ) {
			mcms_enqueue_script( 'mcms-fullscreen-stub' );
		}
		 ...
		 */
		do_action( 'mcms_enqueue_editor', array(
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'mcms_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add mcms-roots/css/editor.css
		mcms_enqueue_style( 'editor-buttons' );

		if ( is_admin() ) {
			add_action( 'admin_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		} else {
			add_action( 'mcms_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			// In production all modules are loaded (they are in mcms-editor.js.gz).
			// The 'mcmsview', 'mcmsdialogs', and 'media' TinyMCE modules are not initialized by default.
			// Can be added from js by using the 'mcms-before-tinymce-init' event.
			$settings['modules'] = implode( ',', array(
				'charmap',
				'colorpicker',
				'hr',
				'lists',
		...
		window.mcms.editor.getDefaultSettings = function() {
			return {
	 ...
	 * Doesn't include modules, buttons, editor selector.
	 *
	 * @global string $tinymce_version
	 *
	 * @return array
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats' => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.mcms-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.mcms-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.mcms-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding' => 'raw',
			'keep_styles' => false,
			'cache_suffix' => 'mcms-mce-' . $tinymce_version,
			'resize' => 'vertical',
			'menubar' => false,
			'branding' => false,
			...
			'mcmseditimage_html5_captions' => true,
			'mcms_lang_attr' => get_bloginfo( 'language' ),
			'mcms_keep_scroll_position' => false,
			'mcms_shortcut_labels' => mcms_json_encode( $shortcut_labels ),
		);
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/mandarincms/mcms-plugins.css?$version" );
	...
	private static function get_translation() {
		if ( empty( self::$translation ) ) {
			...
			// Each of these have a corresponding module
			...
			// Spelling, search/replace modules
		...
		/*
		Imagetools module (not included):
	 ...
	 */
	public static function mcms_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'mcms_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return mcms_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . mcms_json_encode( $mce_translation ) . ");\n" .
	...
	public static function print_tinymce_scripts() {
		...
		$has_custom_myskin = false;
		foreach ( self::$mce_settings as $init ) {
			if ( ! empty( $init['myskin_url'] ) ) {
				$has_custom_myskin = true;
				break;
			}
		}

		$compressed = $compress_scripts && $concatenate_scripts && isset( $_SERVER['HTTP_ACCEPT_ENCODING'] )
			&& false !== stripos( $_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip' ) && ! $has_custom_myskin;

		// Load tinymce.js when running from /src, else load mcms-tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( get_bloginfo( 'version' ), '-src' ) ? '' : '.min';

		if ( $compressed ) {
			echo "<script type='text/javascript' src='{$baseurl}/mcms-tinymce.php?c=1&amp;$version'></script>\n";
		} else {
			echo "<script type='text/javascript' src='{$baseurl}/tinymce{$mce_suffix}.js?$version'></script>\n";
			echo "<script type='text/javascript' src='{$baseurl}/modules/compat3x/module{$suffix}.js?$version'></script>\n";
		}

		echo "<script type='text/javascript'>\n" . self::mcms_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_mcms_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_modules ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/mcms-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'mcms_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.mcms-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
					return;
				}

				for ( id in tinyMCEPreInit.mceInit ) {
					init = tinyMCEPreInit.mceInit[id];
					$wrap = tinymce.$( '#mcms-' + id + '-wrap' );

					if ( ( $wrap.hasClass( 'tmce-active' ) || ! tinyMCEPreInit.qtInit.hasOwnProperty( id ) ) && ! init.mcms_skip_init ) {
						tinymce.init( init );

						if ( ! window.mcmsActiveEditor ) {
							window.mcmsActiveEditor = id;
						}
					}
				}
			}
		...
		if ( in_array( 'mcmslink', self::$modules, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			self::mcms_link_dialog();
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_mcms_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @static
	 */
	public static function mcms_fullscreen_html() {
	 ...
	 */
	public static function mcms_link_query( $args = array() ) {
		 ...
		 *
		 * @see MCMS_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of MCMS_Query arguments.
		 */
		$query = apply_filters( 'mcms_link_query_args', $query );
		 ...
		 * @since 3.7.0
		 *
		 * @see 'mcms_link_query_args' filter
		 *
		 * @param array $results {
		 ...
		 * @param array $query  An array of MCMS_Query arguments.
		 */
		$results = apply_filters( 'mcms_link_query', $results, $query );
	 ...
	 * @static
	 */
	public static function mcms_link_dialog() {
		// Run once
		...
		// display: none is required here, see #MCMS27605
		?>
		<div id="mcms-link-backdrop" style="display: none"></div>
		<div id="mcms-link-wrap" class="mcms-core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
		<form id="mcms-link" tabindex="-1">
		<?php mcms_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ) ?></h1>
		<button type="button" id="mcms-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="mcmslink-enter-url"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span>
					<input id="mcms-link-url" type="text" aria-describedby="mcmslink-enter-url" /></label>
				</div>
				<div class="mcms-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="mcms-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="mcms-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			<p class="howto" id="mcmslink-link-existing-content"><?php _e( 'Or link to existing content' ); ?></p>
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						<span class="search-label"><?php _e( 'Search' ); ?></span>
						<input type="search" id="mcms-link-search" class="link-search-field" autocomplete="off" aria-describedby="mcmslink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="mcms-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="mcms-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="mcms-link-submit" name="mcms-link-submit">
			</div>
		</div>
		</form>
		</div>
