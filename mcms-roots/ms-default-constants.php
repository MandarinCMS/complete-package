 ...
 * mcms-roots/ms-files.php (mcms-plugins/blogs.php in MU).
 *
 * @since 3.0.0
 */
function ms_upload_constants() {
	...
	// Base uploads dir relative to BASED_TREE_URI
	if ( !defined( 'UPLOADBLOGSDIR' ) )
		define( 'UPLOADBLOGSDIR', 'mcms-plugins/blogs.dir' );

	// Note, the main site in a post-MU network uses mcms-plugins/uploads.
	// This is handled in mcms_upload_dir() by ignoring UPLOADS for this case.
	if ( ! defined( 'UPLOADS' ) ) {
		$site_id = get_current_blog_id();

		define( 'UPLOADS', UPLOADBLOGSDIR . '/' . $site_id . '/files/' );

		// Uploads dir relative to BASED_TREE_URI
		if ( 'mcms-plugins/blogs.dir' == UPLOADBLOGSDIR && ! defined( 'BLOGUPLOADDIR' ) )
			define( 'BLOGUPLOADDIR', MCMS_CONTENT_DIR . '/blogs.dir/' . $site_id . '/files/' );
	}
}
...
function ms_file_constants() {
	 ...
	 * @since 3.0.0
	 */
	if ( !defined( 'MCMSMU_SENDFILE' ) )
		define( 'MCMSMU_SENDFILE', false );

	/**
	 * Optional support for X-Accel-Redirect header
	 * @since 3.0.0
	 */
	if ( !defined( 'MCMSMU_ACCEL_REDIRECT' ) )
		define( 'MCMSMU_ACCEL_REDIRECT', false );
}
...
function ms_subdomain_constants() {
	...
	if ( $subdomain_error ) {
		$vhost_deprecated = sprintf(
			/* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: database-settings.php, 4: is_subdomain_install() */
