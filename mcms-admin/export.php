<?php
/**
 * MandarinCMS Export Administration Screen
 *
 * @package MandarinCMS
...
if ( !current_user_can('export') )
	mcms_die(__('Sorry, you are not allowed to export the content of this site.'));

/** Load MandarinCMS export API */
require_once( BASED_TREE_URI . 'mcms-admin/includes/export.php' );
$title = __('Export');
...
get_current_screen()->add_help_tab( array(
	...
	'content' => '<p>' . __('You can export a file of your site&#8217;s content in order to import it into another installation or platform. The export file will be an XML file format called WXR. Posts, pages, comments, custom fields, categories, and tags can be included. You can choose for the WXR file to include only certain posts or pages by setting the dropdown filters to limit the export by category, author, date range by month, or publishing status.') . '</p>' .
		'<p>' . __('Once generated, your WXR file can be imported by another MandarinCMS site or by another blogging platform able to access this format.') . '</p>',
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="https://dev.mandarincms.com/Tools_Export_Screen">Documentation on Export</a>') . '</p>' .
 ...
 * @global MCMS_Locale $mcms_locale Date and Time Locale object.
 *
 * @since 3.1.0
 *
...
function export_date_options( $post_type = 'post' ) {
	...
	foreach ( $months as $date ) {
		...
		echo '<option value="' . $date->year . '-' . $month . '">' . $mcms_locale->get_month( $month ) . ' ' . $date->year . '</option>';
	}
}
?>
...
<p><?php _e('When you click the button below MandarinCMS will create an XML file for you to save to your computer.'); ?></p>
<p><?php _e('This format, which we call MandarinCMS eXtended RSS or WXR, will contain your posts, pages, comments, custom fields, categories, and tags.'); ?></p>
<p><?php _e('Once you&#8217;ve saved the download file, you can use the Import function in another MandarinCMS installation to import the content from this site.'); ?></p>
...
<p><label><input type="radio" name="content" value="posts" /> <?php _e( 'News' ); ?></label></p>
<ul id="post-filters" class="export-filters">
	<li>
		<label><span class="label-responsive"><?php _e( 'Categories:' ); ?></span>
		<?php mcms_dropdown_categories( array( 'show_option_all' => __('All') ) ); ?>
		</label>
	</li>
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $mcmsdb->get_col( "SELECT DISTINCT post_author FROM {$mcmsdb->posts} WHERE post_type = 'post'" );
...
<ul id="page-filters" class="export-filters">
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $mcmsdb->get_col( "SELECT DISTINCT post_author FROM {$mcmsdb->posts} WHERE post_type = 'page'" );
		mcms_dropdown_users( array(
			'include' => $authors,
			'name' => 'page_author',
			'multi' => true,
