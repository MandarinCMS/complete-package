 ...
 */
global $mcms_scripts, $mcms_customize;

if ( $mcms_customize->changeset_post_id() ) {
	$changeset_post = get_post( $mcms_customize->changeset_post_id() );
	...
	if ( $missed_schedule ) {
		/*
		 * Note that an Ajax request spawns here instead of just calling `mcms_publish_post( $changeset_post->ID )`.
		 *
		 * Because MCMS_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		 ...
		 */
		$nonces = $mcms_customize->get_nonces();
		$request_args = array(
			'nonce' => $nonces['save'],
			'customize_changeset_uuid' => $mcms_customize->changeset_uuid(),
			'mcms_customize' => 'on',
			'customize_changeset_status' => 'publish',
		);
		ob_start();
		?>
		<?php mcms_print_scripts( array( 'mcms-util' ) ); ?>
		<script>
			mcms.ajax.post( 'customize_save', <?php echo mcms_json_encode( $request_args ); ?> );
		</script>
		<?php
		$script = ob_get_clean();
...
mcms_reset_vars( array( 'url', 'return', 'autofocus' ) );
if ( ! empty( $url ) ) {
	$mcms_customize->set_preview_url( mcms_unslash( $url ) );
}
if ( ! empty( $return ) ) {
	$mcms_customize->set_return_url( mcms_unslash( $return ) );
}
if ( ! empty( $autofocus ) && is_array( $autofocus ) ) {
	$mcms_customize->set_autofocus( mcms_unslash( $autofocus ) );
}

$registered = $mcms_scripts->registered;
$mcms_scripts = new MCMS_Scripts;
$mcms_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts',        'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_mcms_footer_scripts'     );
...
mcms_enqueue_script( 'customize-controls' );
mcms_enqueue_style( 'customize-controls' );
...
$body_class = 'mcms-core-ui mcms-customizer js';
...
$admin_title = sprintf( $mcms_customize->get_document_title_template(), __( 'Loading&hellip;' ) );

?><title><?php echo $admin_title; ?></title>

<script type="text/javascript">
var ajaxurl = <?php echo mcms_json_encode( admin_url( 'admin-ajax.php', 'relative' ) ); ?>,
	pagenow = 'customize';
</script>
...
<div class="mcms-full-overlay expanded">
	<form id="customize-controls" class="wrap mcms-full-overlay-sidebar">
		<div id="customize-header-actions" class="mcms-full-overlay-header">
			<?php $save_text = $mcms_customize->is_myskin_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url( $mcms_customize->get_return_url() ); ?>">
		...
		<div id="customize-sidebar-outer-content">
			<div id="customize-outer-myskin-controls">
		...
		<div id="widgets-right" class="mcms-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="mcms-full-overlay-sidebar-content" tabindex="-1">
				...
				<div id="customize-myskin-controls">
					<ul class="customize-pane-parent"><?php // Panels and sections are managed here via JavaScript ?></ul>
				</div>
			</div>
		</div>

		<div id="customize-footer-actions" class="mcms-full-overlay-footer">
			...
			<?php $previewable_devices = $mcms_customize->get_previewable_devices(); ?>
	...
	<div id="customize-preview" class="mcms-full-overlay-main"></div>
	<?php
