<?php
/**
 * Modules may load this file to gain access to special helper functions for
 * module installation. This file is not included by MandarinCMS and it is
 ...
 *
 *     check_column( 'mcms_links', 'link_description', 'mediumtext' );
 *     if ( check_column( $mcmsdb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     $error_count = 0;
 *     $tablename = $mcmsdb->links;
 *     // Check the column.
 *     if ( ! check_column($mcmsdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $mcmsdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $mcmsdb->query( $ddl );
 *     }
 *
 *     if ( check_column( $mcmsdb->links, 'link_description', 'varchar( 255 )' ) ) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $mcmsdb;
	foreach ($mcmsdb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $mcmsdb;
	$diffs = 0;
	$results = $mcmsdb->get_results("DESC $table_name");
