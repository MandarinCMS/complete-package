...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://dev.mandarincms.com/Settings_Permalinks_Screen">Documentation on Permalinks Settings</a>') . '</p>' .
	'<p>' . __('<a href="https://dev.mandarincms.com/Using_Permalinks">Documentation on Using Permalinks</a>') . '</p>' .
 ...
 * base prefix, MandarinCMS core can no longer account for the possible collision.
 */
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess' ) ) {
		...
		$writable = false;
		$existing_rules  = array_filter( extract_from_markers( $home_path . '.htaccess', 'MandarinCMS' ) );
		$new_rules       = array_filter( explode( "\n", $mcms_rewrite->mod_rewrite_rules() ) );
		$update_required = ( $new_rules !== $existing_rules );
	}
}

$using_index_permalinks = $mcms_rewrite->using_index_permalinks();
...
if ( isset($_POST['permalink_structure']) || isset($_POST['category_base']) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$mcms_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$mcms_rewrite->set_tag_base( $tag_base );
	}
	...
	mcms_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}
...
<?php mcms_nonce_field('update-permalink') ?>

	<p><?php
		printf(
			/* translators: %s: Codex URL */
			__( 'MandarinCMS offers you the ability to create a custom URL structure for your permalinks and archives. Custom URL structures can improve the aesthetics, usability, and forward-compatibility of your links. A <a href="%s">number of tags are available</a>, and here are some examples to get you started.' ),
			__( 'https://dev.mandarincms.com/Using_Permalinks' )
		);
	?></p>
...
<p><?php
	printf(
		...
		__( 'https://dev.mandarincms.com/Changing_File_Permissions' ),
		'<kbd>CTRL + a</kbd>',
...
<?php mcms_nonce_field('update-permalink') ?>
	<p><textarea rows="9" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mcms_rewrite->iis7_url_rewrite_rules() ); ?></textarea></p>
</form>
<p><?php
	printf(
...
<form action="options-permalink.php" method="post">
<?php mcms_nonce_field('update-permalink') ?>
	<p><textarea rows="18" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mcms_rewrite->iis7_url_rewrite_rules(true) ); ?></textarea></p>
</form>
<p><?php
	printf(
		/* translators: %s: web.config */
...
?></p>
	...
	<?php endif; ?>
<?php elseif ( $is_nginx ) : ?>
	<p><?php _e( '<a href="https://dev.mandarincms.com/Nginx">Documentation on Nginx configuration</a>.' ); ?></p>
<?php else:
...
<form action="options-permalink.php" method="post">
<?php mcms_nonce_field('update-permalink') ?>
	<p><textarea rows="6" class="large-text readonly" name="rules" id="rules" readonly="readonly"><?php echo esc_textarea( $mcms_rewrite->mod_rewrite_rules() ); ?></textarea></p>
</form>
	<?php endif; ?>
<?php endif; ?>
<?php } // multisite ?>
...
<?php require( BASED_TREE_URI . 'mcms-admin/admin-footer.php' ); ?>
