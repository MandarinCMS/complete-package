...
( function( $ ) {
	 ...
	 *
	 * Example: `$( element ).mcmsTagsSuggest( options )`.
	 *
	 * The taxonomy can be passed in a `data-mcms-taxonomy` attribute on the element or
	 ...
	 */
	$.fn.mcmsTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-mcms-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'mcms-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.mcms.a11y.speak( window.tagsSuggestL10n.termSelected, 'assertive' );
					event.preventDefault();
		...
		.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="mcms-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
		...
		.autocomplete( 'widget' )
			.addClass( 'mcms-tags-autocomplete' )
			.attr( 'role', 'listbox' )
