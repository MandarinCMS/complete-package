if ( 'undefined' === typeof window.mcms ) {
	window.mcms = {};
}
if ( 'undefined' === typeof window.mcms.codeEditor ) {
	window.mcms.codeEditor = {};
}
...
( function( $, mcms ) {
	 ...
	 * @type {object}
	 */
	mcms.codeEditor.defaultSettings = {
		codemirror: {},
		csslint: {},
		htmlhint: {},
		jshint: {},
	 ...
	 */
	mcms.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, mcms.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = mcms.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = mcms.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
