...
(function($) {
	...
	function bindToggleButton() {
		$toggleButton = $pass1Row.find('.mcms-hide-pw');
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #mcms-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.mcms-pwd' );
		$generateButton  = $pass1Row.find( 'button.mcms-generate-pw' );
		...
		$cancelButton = $pass1Row.find( 'button.mcms-cancel-pw' );
		$cancelButton.on( 'click', function () {
			...
			mcms.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
	...
	function check_pass_strength() {
		...
		strength = mcms.passwordStrength.meter( pass1, mcms.passwordStrength.userInputBlacklist(), pass1 );
	...
	$(document).ready( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#mcms-admin-bar-my-account' ).find( '.display-name' );
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// repaint icons
				if ( typeof mcms !== 'undefined' && mcms.svgPainter ) {
					try {
						colors = $.parseJSON( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						mcms.svgPainter.setColors( colors );
						mcms.svgPainter.paint();
					}
				}
	...
	$( '#destroy-sessions' ).on( 'click', function( e ) {
		...
		mcms.ajax.post( 'destroy-sessions', {
			nonce: $( '#_mcmsnonce' ).val(),
			user_id: $( '#user_id' ).val()
		}).done( function( response ) {
