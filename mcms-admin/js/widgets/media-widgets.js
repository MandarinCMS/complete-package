...
mcms.mediaWidgets = ( function( $ ) {
	 ...
	 *
	 * @type {Object.<string, mcms.mediaWidgets.MediaWidgetModel>}
	 */
	 ...
	 * @constructor
	 */
	component.PersistentDisplaySettingsLibrary = mcms.media.controller.Library.extend({
		...
		initialize: function initialize( options ) {
			...
			mcms.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = mcms.media.controller.Library.prototype.display.call( this, model );
	 ...
	 * @constructor
	 */
	component.MediaEmbedView = mcms.media.view.Embed.extend({
		...
		initialize: function( options ) {
			...
			mcms.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {mcms.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @returns {void}
		 */
		refresh: function refresh() {
			...
			if ( 'image' === this.controller.options.mimeType ) {
				...
				Constructor = mcms.media.view.EmbedLink.extend({
					...
					updateoEmbed: function() {
						...
						mcms.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! mcms.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== mcms.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = mcms.apiRequest({
	 ...
	 * @constructor
	 */
	component.MediaFrameSelect = mcms.media.view.MediaFrame.Post.extend({
		...
		createStates: function createStates() {
			...
			_.each( mcms.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new mcms.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new mcms.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {mcms.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {mcms.Backbone.View} view - Toolbar view.
		 * @this {mcms.media.controller.Library}
		 * @returns {void}
		 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {mcms.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {mcms.Backbone.View} toolbar - Toolbar view.
		 * @this {mcms.media.controller.Library}
		 * @returns {void}
		 */
		 ...
		 * Embed content.
		 *
		 * Forked override of {mcms.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @returns {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend({
		 ...
		 * exported from PHP to JS such as is done in MCMS_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in MCMS_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in MCMS_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new mcms.media.model.Attachment();
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.mandarincms.com/ticket/33507>.
			 */
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( mcms.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new mcms.media.model.Attachment({
		...
		template: function template() {
			...
			return mcms.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new mcms.media.model.Selection( selectionModels, { multiple: false } );
			...
			mcms.media.frame = mediaFrame; // See mcms.media().
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.mandarincms.com/ticket/40403>.
			defaultSync = mcms.media.model.Attachment.prototype.sync;
			mcms.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				mcms.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {mcms.media.view.MediaFrame.Select} mediaFrame - Select frame.
	...
	component.MediaWidgetModel = Backbone.Model.extend({
		 ...
		 * @returns {mcms.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 *
	 * @type {Object.<string, mcms.mediaWidgets.MediaWidgetControl>}
	 */
	 ...
	 * attached which calls mcms.mediaWidgets.init().
	 *
	 * @returns {void}
	 */
