...
mcms.customHtmlWidgets = ( function( $ ) {
	'use strict';
	...
	component.CustomHtmlWidgetControl = Backbone.View.extend({
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( mcms.template( 'widget-custom-html-control-fields' )( { codeEditorDisabled: component.codeEditorSettings.disabled } ) );
		...
		updateErrorNotice: function( errorAnnotations ) {
			...
			if ( mcms.customize && mcms.customize.has( control.customizeSettingId ) ) {
				customizeSetting = mcms.customize( control.customizeSettingId );
				customizeSetting.notifications.remove( 'htmlhint_error' );
				if ( 0 !== errorAnnotations.length ) {
					customizeSetting.notifications.add( 'htmlhint_error', new mcms.customize.Notification( 'htmlhint_error', {
						message: message,
						type: 'error'
					} ) );
				}
		...
		initializeEditor: function initializeEditor() {
			...
			control.editor = mcms.codeEditor.initialize( control.fields.content, settings );
	 ...
	 *
	 * @type {Object.<string, mcms.textWidgets.CustomHtmlWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		...
		renderWhenAnimationDone = function() {
			if ( ! ( mcms.customize ? widgetContainer.parent().hasClass( 'expanded' ) : widgetContainer.hasClass( 'open' ) ) ) { // Core merge: The mcms.customize condition can be eliminated with this change being in core: https://github.com/xmcms/mandarincms-develop/pull/247/commits/5322387d
