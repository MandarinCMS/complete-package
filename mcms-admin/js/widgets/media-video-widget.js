...
(function( component ) {
	 ...
	 * @constructor
	 */
	VideoDetailsMediaFrame = mcms.media.view.MediaFrame.VideoDetails.extend({
		...
		createStates: function createStates() {
			this.states.add([
				...
				new mcms.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: mcms.media.view.l10n.videoAddSourceTitle,
				...
				new mcms.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: mcms.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See MCMS_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	...
	VideoWidgetControl = component.MediaWidgetControl.extend({
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = mcms.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( mcms.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( mcms.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = mcms.template( 'mcms-media-widget-video-preview' );
