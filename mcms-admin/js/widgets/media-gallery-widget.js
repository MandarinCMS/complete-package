...
(function( component ) {
	 ...
	 * @constructor
	 */
	GalleryDetailsMediaFrame = mcms.media.view.MediaFrame.Post.extend( {
		...
		createStates: function createStates() {
			this.states.add([
				new mcms.media.controller.Library({
					id:         'gallery',
					title:      mcms.media.view.l10n.createGalleryTitle,
					priority:   40,
	 ...
	 *
	 * See MCMS_Widget_Gallery::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @since 4.9.0
	...
	GalleryWidgetControl = component.MediaWidgetControl.extend( {
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachments = new mcms.media.model.Attachments();
			 ...
			 */
			if ( mcms.customize && mcms.customize.previewer ) {
				control.selectedAttachments.on( 'change', function() {
					mcms.customize.previewer.send( 'refresh-widget-partial', control.model.get( 'widget_id' ) );
				} );
			}
		},
		...
		renderPreview: function renderPreview() {
			...
			previewTemplate = mcms.template( 'mcms-media-widget-gallery-preview' );
		...
		editMedia: function editMedia() {
			...
			selection = new mcms.media.model.Selection( control.selectedAttachments.models, {
				multiple: true
			});
		 ...
		 *
		 * @since 4.9.0
		 * @param {mcms.media.models.Attachment} attachment - Attachment.
		 * @returns {void}
		 */
