/* global postL10n, ajaxurl, mcmsAjax, setPostThumbnailL10n, postboxes, pagenow, tinymce, alert, deleteUserSetting */
...
var commentsBox, MCMSSetThumbnailHTML, MCMSSetThumbnailID, MCMSRemoveThumbnail, mcmstitlehint, makeSlugeditClickable, editPermalink;
// Backwards compatibility: prevent fatal errors.
makeSlugeditClickable = editPermalink = function(){};

// Make sure the mcms object exists.
...
( function( $ ) {
	...
	commentsBox = {
		...
		get : function(total, num) {
			...
			$.post(
				...
				data,
				function(r) {
					r = mcmsAjax.parseAjaxResponse(r);
					...
					$('#the-comment-list').append('<tr><td colspan="2">'+mcmsAjax.broken+'</td></tr>');
				}
			);
	 ...
	 * @global
	 */
	MCMSSetThumbnailHTML = function(html){
	 ...
	 * @global
	 */
	MCMSSetThumbnailID = function(id){
	 ...
	 * @global
	 */
	MCMSRemoveThumbnail = function(nonce){
		$.post(ajaxurl, {
		...
		if ( data['mcms-refresh-post-lock'] ) {
			...
			if ( received.lock_error ) {
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					...
					wrap.find('.mcms-tab-first').focus();
				}
...
(function($) {
	...
	$(document).on( 'heartbeat-send.mcms-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#mcms-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_mcmsnonce').val() ) {
				data['mcms-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.mcms-refresh-nonces', function( e, data ) {
		var nonces = data['mcms-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('mcms-tab-first') && e.shiftKey ) {
			$(this).find('.mcms-tab-last').focus();
			e.preventDefault();
		// [tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('mcms-tab-last') && ! e.shiftKey ) {
			$(this).find('.mcms-tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.mcms-tab-first').focus();

	// Set the heartbeat interval to 15 sec. if post lock dialogs are enabled.
	if ( mcms.heartbeat && $('#post-lock-dialog').length ) {
		mcms.heartbeat.interval( 15 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#mcms-preview'),
			target = $this.attr('target') || 'mcms-preview',
		...
		if ( mcms.autosave ) {
			mcms.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! mcms.heartbeat || ! mcms.heartbeat.hasConnectionError() ) {
	...
	$(window).on( 'beforeunload.edit-post', function() {
		...
		if ( ( editor && ! editor.isHidden() && editor.isDirty() ) ||
			( mcms.autosave && mcms.autosave.server.postChanged() ) ) {
		...
		var data = {
			action: 'mcms-remove-post-lock',
			_mcmsnonce: $('#_mcmsnonce').val(),
			post_ID: postID,
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see mcms-lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'mcms-hidden-children' );
		...
		buttons.children( '.save' ).click( function() {
			...
			$.post(
				...
				function(data) {
					...
					$( '.edit-slug' ).focus();
					mcms.a11y.speak( postL10n.permalinkSaved );
				}
			);
		});
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'mcms-editor-expand' ) ) {
				return;
			}

			if ( mce ) {
				editor.myskin.resizeTo( null, offset + event.pageY );
			} else {
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#mcms-plugins-editor-container .mce-toolbar-grp' ).height(), 10 );
			...
			$document.off( '.mcms-editor-resize' );
		...
		$handle.on( 'mousedown.mcms-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.mcms-editor-resize', dragging )
				.on( 'mouseup.mcms-editor-resize mouseleave.mcms-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.mcms-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.mcms-autosave', function( event ) {
		// Key [s] has code 83.
		if ( event.which === 83 ) {
			...
			mcms.autosave && mcms.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'mcms-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#mcms-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new mcms.utils.WordCounter() );
