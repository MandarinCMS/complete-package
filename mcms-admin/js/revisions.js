...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.mcms.revisions.
	 */
	revisions = mcms.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._mcmsRevisionsSettings || {};
	...
	/**
	 * mcms.revisions.model.Revisions
	 *
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			mcms.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				mcms.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = mcms.ajax.send( options ),
	...
	/**
	 * mcms.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see mcms.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = mcms.Backbone.View.extend({
		className: 'revisions',
		template: mcms.template('revisions-frame'),
		...
		render: function() {
			mcms.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#mcmsbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = mcms.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.mcms.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.mcms.revisions.pinning click.mcms.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.mcms.revisions.pinning');
	...
	// The tickmarks view
	revisions.view.Tickmarks = mcms.Backbone.View.extend({
	...
	// The metabox view
	revisions.view.Metabox = mcms.Backbone.View.extend({
	...
	revisions.view.Meta = mcms.Backbone.View.extend({
		template: mcms.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = mcms.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: mcms.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = mcms.Backbone.View.extend({
	...
	revisions.view.Buttons = mcms.Backbone.View.extend({
		className: 'revisions-buttons',
		template: mcms.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = mcms.Backbone.View.extend({
		className: 'mcms-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.mcms.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.mcms.revisions');
	...
	revisions.view.Diff = mcms.Backbone.View.extend({
		className: 'revisions-diff',
		template:  mcms.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// mcms_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// mcms_localize_script does not allow for top-level booleans so do a comparator here.
