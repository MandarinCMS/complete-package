...
mcmsWidgets = {
	 ...
	 * Translations.
	 *
	 * Exported from PHP in mcms_default_scripts().
	 *
	 * @var {object}
	 */
	l10n: {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.click( function() {
				...
				$document.triggerHandler( 'mcms-pin-menu' );
			})
			/*
		...
		$( window ).on( 'beforeunload.widgets', function( event ) {
			...
			if ( 0 !== dirtyWidgetIds.length ) {
				...
				event.returnValue = mcmsWidgets.l10n.saveAlert;
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				if ( ! widget.data( 'dirty-state-initialized' ) ) {
					...
					saveButton.prop( 'disabled', true ).val( mcmsWidgets.l10n.saved );
					inside.on( 'input change', function() {
						self.dirtyWidgets[ widgetId ] = true;
						widget.addClass( 'widget-dirty' );
						saveButton.prop( 'disabled', false ).val( mcmsWidgets.l10n.save );
					});
				...
				mcmsWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				mcmsWidgets.save( target.closest('div.widget'), 1, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				toggleBtn.attr( 'aria-expanded', 'false' );
				mcmsWidgets.close( widget );
				e.preventDefault();
			} else if ( target.attr( 'id' ) === 'inactive-widgets-control-remove' ) {
				mcmsWidgets.removeInactiveWidgets();
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#mcmsbody-content' ).append( chooser.hide() );
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( mcmsWidgets.hoveredSidebar && ! $wrap.is( mcmsWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					mcmsWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					mcmsWidgets.hoveredSidebar = $wrap;
					$wrap
						.removeClass( 'closed' )
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				mcmsWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					mcmsWidgets.save( $widget, 1, 0, 1 ); // delete widget
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					mcmsWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_mcmsnonce_widgets').val(),
			sidebars: []
		};
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#mcms_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'mcms_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#mcms_inactive_widgets .widget' ).length );
					}
				}
			} else {
				...
				if ( r && r.length > 2 ) {
					...
					mcmsWidgets.appendTitle( widget );

					// Re-disable the save button.
					widget.find( '.widget-control-save' ).prop( 'disabled', true ).val( mcmsWidgets.l10n.saved );
	...
	removeInactiveWidgets : function() {
		...
		a = {
			...
			removeinactivewidgets : $( '#_mcmsnonce_remove_inactive_widgets' ).val()
		};

		data = $.param( a );

		$.post( ajaxurl, data, function() {
			$( '#mcms_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
...
$document.ready( function(){ mcmsWidgets.init(); } );
