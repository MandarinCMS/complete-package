/* global _mcmsCustomizeNavMenusSettings, mcmsNavMenu, console */
( function( api, mcms, $ ) {
	'use strict';

	/**
	 * Set up mcmsNavMenu for drag and drop.
	 */
	mcmsNavMenu.originalInit = mcmsNavMenu.init;
	mcmsNavMenu.options.menuItemDepthPerLevel = 20;
	mcmsNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	mcmsNavMenu.options.targetTolerance       = 10;
	mcmsNavMenu.init = function() {
	...
	if ( 'undefined' !== typeof _mcmsCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _mcmsCustomizeNavMenusSettings );
	}
	...
	/**
	 * mcms.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's MCMS_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @constructor
	...
	/**
	 * mcms.customize.Menus.AvailableItemCollection
	 *
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = mcms.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	/**
	 * mcms.customize.Menus.AvailableMenuItemsPanelView
	 *
	 ...
	 */
	api.Menus.AvailableMenuItemsPanelView = mcms.Backbone.View.extend({
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				mcms.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = mcms.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					mcms.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					mcms.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = mcms.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( mcms.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * mcms.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the MCMS_Customize_Menu_Panel::$type.
	 *
	 * @constructor
	...
	api.Menus.MenusPanel = api.Panel.extend({
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					mcms.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = mcms.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * mcms.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the MCMS_Customize_Menu_Section::$type.
	 *
	 * @constructor
	...
	api.Menus.MenuSection = api.Section.extend({
		...
		refreshAssignedLocations: function() {
			...
			_.each( section.navMenuLocationSettings, function( setting, myskinLocation ) {
				if ( setting() === menuTermId ) {
					currentAssignedLocations.push( myskinLocation );
				}
			});
		...
		/**
		 * @param {Array} myskinLocationSlugs MySkin location slugs.
		 */
		updateAssignedLocationsInSectionTitle: function( myskinLocationSlugs ) {
			var section = this,
				$title;

			$title = section.container.find( '.accordion-section-title:first' );
			$title.find( '.menu-in-location' ).remove();
			_.each( myskinLocationSlugs, function( myskinLocationSlug ) {
				var $label, locationName;
				$label = $( '<span class="menu-in-location"></span>' );
				locationName = api.Menus.data.locationSlugMappedToName[ myskinLocationSlug ];
				$label.text( api.Menus.data.l10n.menuLocation.replace( '%s', locationName ) );
				$title.append( $label );
			});

			section.container.toggleClass( 'assigned-to-menu-location', 0 !== myskinLocationSlugs.length );
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				mcmsNavMenu.menuList = section.contentContainer;
				mcmsNavMenu.targetList = mcmsNavMenu.menuList;

				// Add attributes needed by mcmsNavMenu
				$( '#menu-to-edit' ).removeAttr( 'id' );
				mcmsNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						mcmsNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
						section.deferred.initSortables.resolve( mcmsNavMenu.menuList ); // Now MenuControl can extend the sortable.

						// @todo Note that mcms.customize.reflowPaneContents() is debounced, so this immediate change will show a slight flicker while priorities get updated.
	 ...
	 * @returns {mcms.customize.Menus.MenuSection} Added nav menu.
	 */
	...
	/**
	 * mcms.customize.Menus.NewMenuSection
	 *
	...
	api.Menus.NewMenuSection = api.Section.extend({
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				mcms.template( 'nav-menu-create-menu-section-title' )
			);
			 ...
			 *
			 * @since 4.9.0
			 * @param {mcms.customize.Setting} setting - Added setting.
			 * @returns {void}
			 */
			 ...
			 *
			 * @since 4.9.0
			 * @param {mcms.customize.Setting} setting - Removed setting.
			 * @returns {void}
			 */
		...
		submit: function() {
			...
			mcms.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * mcms.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the MCMS_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @constructor
	...
	api.Menus.MenuLocationControl = api.Control.extend({
		initialize: function( id, options ) {
			...
			control.myskinLocation = matches[1];
	...
	/**
	 * mcms.customize.Menus.MenuItemControl
	 *
	 * Customizer control for menu items.
	 * Note that 'menu_item' must match the MCMS_Customize_Menu_Item_Control::$type.
	 *
	 * @constructor
	 * @augments mcms.customize.Control
	 */
	api.Menus.MenuItemControl = api.Control.extend({
		...
		_setupReorderUI: function() {
			...
			template = mcms.template( 'menu-item-reorder-nav' );
		...
		_setupUpdateUI: function() {
			...
			// @todo allow other elements, added by modules, to be automatically picked up here; allow additional values to be added to setting array.
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					mcms.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {mcms.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			mcms.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			mcms.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			mcms.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			mcms.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * mcms.customize.Menus.MenuNameControl
	 *
	...
	/**
	 * mcms.customize.Menus.MenuLocationsControl
	 *
				...
				var container = $( this ),
					...
					updateSelectedMenuLabel = function( selectedMenuId ) {
						...
						if ( ! selectedMenuId || ! menuSetting || ! menuSetting() ) {
							container.find( '.myskin-location-set' ).hide();
						} else {
							container.find( '.myskin-location-set' ).show().find( 'span' ).text( displayNavMenuName( menuSetting().name ) );
						}
					};
	...
	/**
	 * mcms.customize.Menus.MenuAutoAddControl
	 *
	...
	/**
	 * mcms.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the MCMS_Menu_Customize_Control::$type
	 *
	 * @constructor
	 * @augments mcms.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend({
		 ...
		 *
		 * Notice that the UI aspects here are handled by mcmsNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that mcmsNavMenu will be setting this .menu-item-data-parent-id input's value.
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						mcms.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				mcms.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				mcms.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {mcms.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @returns {mcms.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			mcms.a11y.speak( api.Menus.data.l10n.itemAdded );
		 ...
		 * @since 4.9.0
		 *
		 * @param {mcms.customize.controlConstructor.nav_menu_item[]} optionalMenuItemControls
		 */
	...
	/**
	 * mcms.customize.Menus.NewMenuControl
	 *
	 * Customizer control for creating new menus and handling deletion of existing menus.
	 * Note that 'new_menu' must match the MCMS_Customize_New_Menu_Control::$type.
	 *
	 * @constructor
	 * @augments mcms.customize.Control
	...
	api.Menus.NewMenuControl = api.Control.extend({
		...
		initialize: function() {
			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] mcms.customize.NewMenuControl will be removed. Please use mcms.customize.Menus.createNavMenu() instead.' );
			}
	...
	/**
	 * Extends mcms.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends mcms.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends mcms.customize.sectionConstructor with section constructor for menu.
	 */
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in MCMS_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @param menuId
	 * @return {mcms.customize.controlConstructor.menus[]}
	 */
	...
	function displayNavMenuName( name ) {
		name = name || '';
		name = $( '<div>' ).text( name ).html(); // Emulate esc_html() which is used in mcms-admin/nav-menus.php.
		name = $.trim( name );
		return name || api.Menus.data.l10n.unnamed;
	}

})( mcms.customize, mcms, jQuery );
