...
( function( $, mcms ) {
	...
	setBulk : function(){
		...
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-mcms-taxonomy]' ).each( function ( i, element ) {
				/*
				...
				if (r) {
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							mcms.a11y.speak( inlineEditL10n.saved );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
						$errorNotice.removeClass( 'hidden' );
						$error.html( r );
						mcms.a11y.speak( $error.text() );
					}
				} else {
					$errorNotice.removeClass( 'hidden' );
					$error.html( inlineEditL10n.error );
					mcms.a11y.speak( inlineEditL10n.error );
				}
			},
		'html');
...
$( document ).on( 'heartbeat-tick.mcms-check-locked-posts', function( e, data ) {
	var locked = data['mcms-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('mcms-locked') ) {
				...
				row.addClass('mcms-locked');
			}
		} else if ( row.hasClass('mcms-locked') ) {
			// Make room for the CSS animation
			row.removeClass('mcms-locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.mcms-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['mcms-check-locked-posts'] = check;
	}
}).ready( function() {

	// Set the heartbeat interval to 15 sec.
	if ( typeof mcms !== 'undefined' && mcms.heartbeat ) {
