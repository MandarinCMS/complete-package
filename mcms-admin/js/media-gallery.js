...
jQuery(function($) {
	/**
	 * Adds a click event handler to the element with a 'mcms-gallery' class.
	 */
	$( 'body' ).bind( 'click.mcms-gallery', function(e) {
		var target = $( e.target ), id, img_size;

		if ( target.hasClass( 'mcms-set-header' ) ) {
			// Opens the image to preview it full size.
			( window.dialogArguments || opener || parent || top ).location.href = target.data( 'location' );
			e.preventDefault();
		} else if ( target.hasClass( 'mcms-set-background' ) ) {
			// Sets the image as background of the myskin.
