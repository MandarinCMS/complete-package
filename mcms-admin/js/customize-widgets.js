/* global _mcmsCustomizeWidgetsSettings */
(function( mcms, $ ){
	...
	// Link settings
	api.Widgets.data = _mcmsCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;

	/**
	 * mcms.customize.Widgets.WidgetModel
	 *
	...
	/**
	 * mcms.customize.Widgets.WidgetCollection
	 *
	...
	/**
	 * mcms.customize.Widgets.SidebarModel
	 *
	...
	/**
	 * mcms.customize.Widgets.SidebarCollection
	 *
	...
	/**
	 * mcms.customize.Widgets.AvailableWidgetsPanelView
	 *
	 ...
	 */
	api.Widgets.AvailableWidgetsPanelView = mcms.Backbone.View.extend({
	...
	/**
	 * mcms.customize.Widgets.WidgetControl
	 *
	 * Customizer control for widgets.
	 * Note that 'widget_form' must match the MCMS_Widget_Form_Customize_Control::$type
	 *
	 * @constructor
	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		embedWidgetContent: function() {
			...
			/*
			 * Trigger widget-added event so that modules can attach any event
		...
		_setupWideWidget: function() {
			var self = this, $widgetInside, $widgetForm, $customizeSidebar,
				$myskinControlsContainer, positionWidget;
			...
			$customizeSidebar = $( '.mcms-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						mcms.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						mcms.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // mcms_widget_control() outputs the button as "Delete"
		...
		/**
		 * @return {mcms.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( mcms.ajax.settings.url, data );
	...
	/**
	 * mcms.customize.Widgets.WidgetsPanel
	 *
	...
	api.Widgets.WidgetsPanel = api.Panel.extend({
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.mandarincms.com/ticket/38794>.
	...
	/**
	 * mcms.customize.Widgets.SidebarSection
	 *
	...
	/**
	 * mcms.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the MCMS_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @constructor
	...
	api.Widgets.SidebarControl = api.Control.extend({
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[mcms_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[mcms_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[mcms_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				mcms.a11y.speak( l10n.reorderModeOn );
				...
				mcms.a11y.speak( l10n.reorderModeOff );
		 ...
		 *
		 * @since 3.9.0
		 * @return {mcms.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo this can use widgetIdToSettingId(), then pass into mcms.customize.control( x ).getSidebarWidgetsControl()
	 ...
	 *
	 * @param {mcms.customize.Section|mcms.customize.Panel|mcms.customize.Control} focusConstruct - The object to initially focus.
	 * @param {mcms.customize.Section|mcms.customize.Panel|mcms.customize.Control} returnConstruct - The object to return focus.
	 */
