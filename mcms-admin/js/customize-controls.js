/* global _mcmsCustomizeHeader, _mcmsCustomizeBackground, _mcmsMediaViewsL10n, MediaElementPlayer, console, confirm */
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = mcms.customize;
	 ...
	 * @since 4.9.0
	 * @class
	 * @augments mcms.customize.Notification
	 */
	 ...
	 * @since 4.9.0
	 * @class
	 * @augments mcms.customize.Values
	 */
	api.Notifications = api.Values.extend({
		 ...
		 * @returns {void}
		 * @this {mcms.customize.Notifications}
		 */
		 ...
		 * @since 4.9.0
		 *
		 * @param {string|mcms.customize.Notification} notification - Notification object to add. Alternatively code may be supplied, and in that case the second notificationObject argument must be supplied.
		 * @param {mcms.customize.Notification} [notificationObject] - Notification to add when first argument is the code string.
		 * @returns {mcms.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		 ...
		 * @return {Array.<mcms.customize.Notification>} Notifications.
		 * @this {mcms.customize.Notifications}
		 */
		get: function( args ) {
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( mcms.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					mcms.a11y.speak( notification.message, 'assertive' );
				}
	 ...
	 *
	 * A setting is MandarinCMS data (myskin mod, option, menu, etc.) that the user can
	 * draft changes to in the Customizer.
	 *
	 * @see PHP class MCMS_Customize_Setting.
	 *
	 * @since 3.4.0
	 * @class
	 * @augments mcms.customize.Value
	 * @augments mcms.customize.Class
	 */
	api.Setting = api.Value.extend({
		 ...
		 * @param {object}  [options.previewer]         - The Previewer instance to sync with. Defaults to mcms.customize.previewer.
		 */
		 ...
		 *
		 * @since 4.6.0
		 * @returns {mcms.customize.Control[]} Controls associated with setting.
		 */
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// Allow modules to attach additional params to the settings.
		...
		// A status would cause a revision to be made, and for this mcms.customize.previewer.save() should be used. Status is also disallowed for revisions regardless.
		...
		// Ensure that if any modules add data to save requests by extending query() that they get included here.
		...
		_.extend( data, {
			...
			customize_myskin: api.settings.myskin.stylesheet,
		...
		// Allow modules to modify the params included with the save request.
		api.trigger( 'save-request-params', data );

		request = mcms.ajax.post( 'customize_save', data );
	 ...
	 * @since 4.1.0
	 *
	 * @param {mcms.customize.Class} instance
	 ...
	 * @since 4.1.0
	 *
	 * @param {(mcms.customize.Panel|mcms.customize.Section|mcms.customize.Control)} a
	 * @param {(mcms.customize.Panel|mcms.customize.Section|mcms.customize.Control)} b
	 * @returns {Number}
	 */
	...
	Container = api.Class.extend({
		 ...
		 * @param {string=default} [options.type] - The type of the panel. See mcms.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.mcms-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.mcms-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = mcms.template( container.templateSelector );
			} else {
				template = mcms.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend({
		...
		containerParent: '#customize-myskin-controls',
		 ...
		 * @param {string=default} [options.type] - The type of the section. See mcms.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.mcms-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.mcms-full-overlay' ),
	...
	/**
	 * mcms.customize.MySkinsSection
	 *
	 * Custom section for myskins that loads myskins by category, and also
	 * handles the myskin-details view rendering and navigation.
	 *
	 * @constructor
	 * @augments mcms.customize.Section
	 * @augments mcms.customize.Container
	 */
	api.MySkinsSection = api.Section.extend({
		currentMySkin: '',
		overlay: '',
		template: '',
		...
		/**
		 * Embed the section in the DOM when the myskins panel is ready.
		 *
		 * Insert the section before the myskins container. Assume that a myskins section is within a panel, but not necessarily the myskins panel.
		 *
		 * @since 4.9.0
		 */
		embed: function() {
			...
			inject = function( panelId ) {
				...
				api.panel( panelId, function( panel ) {
					...
					panel.deferred.embedded.done( function() {
						parentContainer = panel.contentContainer;
						if ( ! section.headContainer.parent().is( parentContainer ) ) {
							parentContainer.find( '.customize-myskins-full-container-container' ).before( section.headContainer );
						}
						if ( ! section.contentContainer.parent().is( section.headContainer ) ) {
							section.containerParent.append( section.contentContainer );
						}
		...
		ready: function() {
			var section = this;
			section.overlay = section.container.find( '.myskin-overlay' );
			section.template = mcms.template( 'customize-myskins-details-view' );

			// Bind global keyboard events.
			section.container.on( 'keydown', function( event ) {
				if ( ! section.overlay.find( '.myskin-wrap' ).is( ':visible' ) ) {
					return;
				}
				...
				if ( 27 === event.keyCode ) {
					if ( section.$body.hasClass( 'modal-open' ) ) {
						...
						section.headerContainer.find( '.customize-myskins-section-title' ).focus();
					}
		 ...
		 *
		 * Ignore the active states' of the contained myskin controls, and just
		 * use the section's own active state instead. This prevents empty search
		 * results for myskin sections from causing the section to become inactive.
		 *
		 * @since 4.2.0
		 *
		 * @returns {Boolean}
		...
		attachEvents: function () {
			...
			section.headerContainer.on( 'click', '.customize-myskins-section-title', function() {

				// Toggle accordion filters under section headers.
				if ( section.headerContainer.find( '.filter-details' ).length ) {
					section.headerContainer.find( '.customize-myskins-section-title' )
			...
			// Preview installed myskins.
			section.container.on( 'click', '.myskin-actions .preview-myskin', function() {
				api.panel( 'myskins' ).loadMySkinPreview( $( this ).data( 'slug' ) );
			});

			// MySkin navigation in details view.
			...
			section.container.on( 'click', '.myskin-backdrop, .close', function() {
				section.closeDetails();
			});

			if ( 'local' === section.params.filter_type ) {

				// Filter-search all myskin objects loaded in the section.
				section.container.on( 'input', '.mcms-filter-search-myskins', function( event ) {
				...
				section.contentContainer.on( 'input', '.mcms-filter-search', function() {
					if ( ! api.panel( 'myskins' ).expanded() ) {
						return;
					}
					debounced( section );
					if ( ! section.expanded() ) {
			...
			section.contentContainer.on( 'click', '.feature-filter-toggle', function( e ) {
				var $myskinContainer = $( '.customize-myskins-full-container' ),
					$filterToggle = $( e.currentTarget );
				section.filtersHeight = $filterToggle.parent().next( '.filter-drawer' ).height();

				if ( 0 < $myskinContainer.scrollTop() ) {
					$myskinContainer.animate( { scrollTop: 0 }, 400 );
				...
				if ( $filterToggle.hasClass( 'open' ) ) {
					...
					section.contentContainer.find( '.myskins' ).css( 'margin-top', section.filtersHeight + marginOffset );
				} else {
					section.contentContainer.find( '.myskins' ).css( 'margin-top', 0 );
				}
			});

			// Setup section cross-linking.
			section.contentContainer.on( 'click', '.no-myskins-local .search-dotorg-myskins', function() {
				api.section( 'mcmsorg_myskins' ).focus();
			});

			function updateSelectedState() {
				var el = section.headerContainer.find( '.customize-myskins-section-title' );
			...
			// Move section controls to the myskins area.
			api.bind( 'ready', function () {
				section.contentContainer = section.container.find( '.customize-myskins-section' );
				section.contentContainer.appendTo( $( '.customize-myskins-full-container' ) );
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var section = this,
				container = section.contentContainer.closest( '.customize-myskins-full-container' );
			...
			function expand() {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'myskins' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.mcms-filter-search' ).val();
							section.contentContainer.find( '.mcms-filter-search' ).val( searchTerm );
		...
		/**
		 * Load myskin data via Ajax and add myskins to the section as controls.
		 *
		 * @since 4.9.0
		 *
		 * @returns {void}
		 */
		loadMySkins: function() {
			var section = this, params, page, request;

			if ( section.loading ) {
				return; // We're already loading a batch of myskins.
			}

			// Parameters for every API query. Additional params are set in PHP.
			page = Math.ceil( section.loaded / 100 ) + 1;
			params = {
				'nonce': api.settings.nonce.switch_myskins,
				'mcms_customize': 'on',
				'myskin_action': section.params.action,
				'customized_myskin': api.settings.myskin.stylesheet,
				'page': page
			};
			...
			// Load myskins.
			section.headContainer.closest( '.mcms-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-myskins' ).hide();
			request = mcms.ajax.post( 'customize_load_myskins', params );
			request.done(function( data ) {
				var myskins = data.myskins;
				...
				if ( 0 !== myskins.length ) {
					...
					section.loadControls( myskins, page );

					if ( 1 === page ) {

						// Pre-load the first 3 myskin screenshots.
						_.each( section.controls().slice( 0, 3 ), function( control ) {
							var img, src = control.params.myskin.screenshot[0];
							if ( src ) {
								img = new Image();
								img.src = src;
							}
						...
						if ( 'local' !== section.params.filter_type ) {
							mcms.a11y.speak( api.settings.l10n.myskinSearchResults.replace( '%d', data.info.results ) );
						}
					}

					_.delay( section.renderScreenshots, 100 ); // Wait for the controls to become visible.

					if ( 'local' === section.params.filter_type || 100 > myskins.length ) { // If we have less than the requested 100 myskins, it's the end of the list.
						section.fullyLoaded = true;
					}
				} else {
					if ( 0 === section.loaded ) {
						section.container.find( '.no-myskins' ).show();
						mcms.a11y.speak( section.container.find( '.no-myskins' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				if ( 'local' === section.params.filter_type ) {
					section.updateCount(); // Count of visible myskin controls.
				} else {
				...
				section.headContainer.closest( '.mcms-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					mcms.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		/**
		 * Loads controls into the section from data received from loadMySkins().
		 *
		 * @since 4.9.0
		 * @param {Array} myskins - Array of myskin data to create controls with.
		 * @param {integer} page - Page of results being loaded.
		 * @returns {void}
		 */
		loadControls: function( myskins, page ) {
			var newMySkinControls = [],
				section = this;

			// Add controls for each myskin.
			_.each( myskins, function( myskin ) {
				var myskinControl = new api.controlConstructor.myskin( section.params.action + '_myskin_' + myskin.id, {
					type: 'myskin',
				...
				api.control.add( myskinControl );
				newMySkinControls.push( myskinControl );
				section.loaded = section.loaded + 1;
			});

			if ( 1 !== page ) {
				Array.prototype.push.apply( section.screenshotQueue, newMySkinControls ); // Add new myskins to the screenshot queue.
			}
		},

		/**
		 * Determines whether more myskins should be loaded, and loads them.
		 *
		 * @since 4.9.0
		 * @returns {void}
		 */
		loadMore: function() {
			...
			if ( ! section.fullyLoaded && ! section.loading ) {
				container = section.container.closest( '.customize-myskins-full-container' );
		...
		filterSearch: function( term ) {
			var count = 0,
				visible = false,
				section = this,
				noFilter = ( api.section.has( 'mcmsorg_myskins' ) && 'remote' !== section.params.filter_type ) ? '.no-myskins-local' : '.no-myskins',
				controls = section.controls(),
				terms;
			...
			if ( 0 === count ) {
				...
				mcms.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		 ...
		 * @since 4.9.0
		 *
		 * @param {mcms.customize.MySkinsSection} section - The current myskin section, passed through the debouncer.
		 * @returns {void}
		 */
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.mcms-filter-search' ).val();
		...
		filtersChecked: function() {
			...
			if ( 0 === tags.length ) {
				...
				section.contentContainer.find( '.feature-filter-toggle .myskin-filter-count' ).text( tags.length );
		...
		initializeNewQuery: function( newTerm, newTags ) {
			...
			// Run a new query, with loadMySkins handling paging, etc.
			if ( ! section.loading ) {
				section.term = newTerm;
				section.tags = newTags;
				section.loadMySkins();
			} else {
				section.nextTerm = newTerm; // This will reload from loadMySkins() with the newest term once the current batch is loaded.
				section.nextTags = newTags; // This will reload from loadMySkins() with the newest tags once the current batch is loaded.
			}
			if ( ! section.expanded() ) {
		...
		renderScreenshots: function() {
			...
			section.screenshotQueue = _.filter( section.screenshotQueue, function( control ) {
				var $imageWrapper = control.container.find( '.myskin-screenshot' ),
		...
		/**
		 * Update the number of myskins in the section.
		 *
		 * @since 4.9.0
		 *
		 * @returns {void}
		...
		updateCount: function( count ) {
			...
			displayed = section.contentContainer.find( '.myskins-displayed' );
			countEl = section.contentContainer.find( '.myskin-count' );
			...
			if ( 0 === count ) {
				...
				mcms.a11y.speak( api.settings.l10n.announceMySkinCount.replace( '%d', count ) );
			}
		},

		/**
		 * Advance the modal to the next myskin.
		 *
		 * @since 4.2.0
		 *
		 * @returns {void}
		...
		nextMySkin: function () {
			var section = this;
			if ( section.getNextMySkin() ) {
				section.showDetails( section.getNextMySkin(), function() {
		...
		/**
		 * Get the next myskin model.
		 *
		 * @since 4.2.0
		 *
		 * @returns {mcms.customize.MySkinControl|boolean} Next myskin.
		 */
		getNextMySkin: function () {
			var section = this, control, nextControl, sectionControls, i;
			control = api.control( section.params.action + '_myskin_' + section.currentMySkin );
			...
			return nextControl.params.myskin;
		},

		/**
		 * Advance the modal to the previous myskin.
		 *
		 * @since 4.2.0
		 * @returns {void}
		 */
		previousMySkin: function () {
			var section = this;
			if ( section.getPreviousMySkin() ) {
				section.showDetails( section.getPreviousMySkin(), function() {
		...
		/**
		 * Get the previous myskin model.
		 *
		 * @since 4.2.0
		 * @returns {mcms.customize.MySkinControl|boolean} Previous myskin.
		 */
		getPreviousMySkin: function () {
			var section = this, control, nextControl, sectionControls, i;
			control = api.control( section.params.action + '_myskin_' + section.currentMySkin );
			sectionControls = section.controls();
			i = _.indexOf( sectionControls, control );
		...
		/**
		 * Disable buttons when we're viewing the first or last myskin.
		 *
		 * @since 4.2.0
		 *
		 * @returns {void}
		...
		updateLimits: function () {
			if ( ! this.getNextMySkin() ) {
				this.overlay.find( '.right' ).addClass( 'disabled' );
			}
			if ( ! this.getPreviousMySkin() ) {
		 ...
		 *
		 * @deprecated
		 * @param {string} myskinId MySkin ID.
		 * @returns {jQuery.promise} Promise.
		 */
		loadMySkinPreview: function( myskinId ) {
			return api.MySkinsPanel.prototype.loadMySkinPreview.call( this, myskinId );
		},

		/**
		 * Render & show the myskin details for a given myskin model.
		 *
		 * @since 4.2.0
		 *
		 * @param {object} myskin - MySkin.
		 * @param {Function} [callback] - Callback once the details have been shown.
		 * @returns {void}
		 */
		showDetails: function ( myskin, callback ) {
			var section = this, panel = api.panel( 'myskins' );
			section.currentMySkin = myskin.id;
			section.overlay.html( section.template( myskin ) )
				.fadeIn( 'fast' )
				.focus();

			function disableSwitchButtons() {
				return ! panel.canSwitchMySkin( myskin.id );
			}

			// Temporary special function since supplying SFTP credentials does not work yet. See #42184.
			function disableInstallButtons() {
				return disableSwitchButtons() || false === api.settings.myskin._canInstall || true === api.settings.myskin._filesystemCredentialsNeeded;
			}

			section.overlay.find( 'button.preview, button.preview-myskin' ).toggleClass( 'disabled', disableSwitchButtons() );
			section.overlay.find( 'button.myskin-install' ).toggleClass( 'disabled', disableInstallButtons() );

			section.$body.addClass( 'modal-open' );
			section.containFocus( section.overlay );
			section.updateLimits();
			mcms.a11y.speak( api.settings.l10n.announceMySkinDetails.replace( '%s', myskin.name ) );
			if ( callback ) {
				callback();
			}
		},
		...
		/**
		 * Close the myskin details modal.
		 *
		 * @since 4.2.0
		 *
		 * @returns {void}
		...
		closeDetails: function () {
			...
			api.control( section.params.action + '_myskin_' + section.currentMySkin ).container.find( '.myskin' ).focus();
		},

		/**
		 * Keep tab focus within the myskin details modal.
		 *
		 * @since 4.2.0
		 *
	...
	/**
	 * Class mcms.customize.OuterSection.
	 *
	...
	api.OuterSection = api.Section.extend({
		...
		initialize: function() {
			var section = this;
			section.containerParent = '#customize-outer-myskin-controls';
	...
	api.Panel = Container.extend({
		...
		embed: function () {
			var panel = this,
				container = $( '#customize-myskin-controls' ),
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.mcms-full-overlay' ),
				container = accordionSection.closest( '.mcms-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = mcms.template( panel.templateSelector + '-content' );
			} else {
				template = mcms.template( 'customize-panel-default-content' );
			}
	...
	/**
	 * Class mcms.customize.MySkinsPanel.
	 *
	 * Custom section for myskins that displays without the customize preview.
	 *
	 * @constructor
	 * @augments mcms.customize.Panel
	 * @augments mcms.customize.Container
	 */
	api.MySkinsPanel = api.Panel.extend({
		...
		initialize: function( id, options ) {
			var panel = this;
			panel.installingMySkins = [];
			api.Panel.prototype.initialize.call( panel, id, options );
		},

		/**
		 * Determine whether a given myskin can be switched to, or in general.
		 *
		 * @since 4.9.0
		 *
		 * @param {string} [slug] - MySkin slug.
		 * @returns {boolean} Whether the myskin can be switched to.
		 */
		canSwitchMySkin: function canSwitchMySkin( slug ) {
			if ( slug && slug === api.settings.myskin.stylesheet ) {
				return true;
			}
		...
		attachEvents: function() {
			...
			if ( api.settings.myskin._canInstall && api.settings.myskin._filesystemCredentialsNeeded ) {
				panel.notifications.add( new api.Notification( 'myskin_install_unavailable', {
					message: api.l10n.myskinInstallUnavailable,
					type: 'info',
					dismissible: true
				} ) );
			}
			...
			function toggleDisabledNotifications() {
				if ( panel.canSwitchMySkin() ) {
					panel.notifications.remove( 'myskin_switch_unavailable' );
				} else {
					panel.notifications.add( new api.Notification( 'myskin_switch_unavailable', {
						message: api.l10n.myskinPreviewUnavailable,
						type: 'warning'
					} ) );
				}
			}
			...
			// Collapse panel to customize the current myskin.
			panel.contentContainer.on( 'click', '.customize-myskin', function() {
				panel.collapse();
			});

			// Toggle between filtering and browsing myskins on mobile.
			panel.contentContainer.on( 'click', '.customize-myskins-section-title, .customize-myskins-mobile-back', function() {
				$( '.mcms-full-overlay' ).toggleClass( 'showing-myskins' );
			});

			// Install (and maybe preview) a myskin.
			panel.contentContainer.on( 'click', '.myskin-install', function( event ) {
				panel.installMySkin( event );
			});

			// Update a myskin. MySkin cards have the class, the details modal has the id.
			panel.contentContainer.on( 'click', '.update-myskin, #update-myskin', function( event ) {

				// #update-myskin is a link.
				event.preventDefault();
				event.stopPropagation();

				panel.updateMySkin( event );
			});

			// Delete a myskin.
			panel.contentContainer.on( 'click', '.delete-myskin', function( event ) {
				panel.deleteMySkin( event );
			});

			_.bindAll( panel, 'installMySkin', 'updateMySkin' );
		},
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.mcms-full-overlay' );

			if ( expanded ) {
				overlay
					.addClass( 'in-myskins-panel' )
					.delay( 200 ).find( '.customize-myskins-full-container' ).addClass( 'animate' );

				_.delay( function() {
					overlay.addClass( 'myskins-panel-expanded' );
				}, 200 );
				...
				overlay
					.removeClass( 'in-myskins-panel myskins-panel-expanded' )
					.find( '.customize-myskins-full-container' ).removeClass( 'animate' );
			}
		},

		/**
		 * Install a myskin via mcms.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		installMySkin: function( event ) {
			...
			// Temporary since supplying SFTP credentials does not work yet. See #42184.
			if ( api.settings.myskin._filesystemCredentialsNeeded ) {
				deferred.reject({
					errorCode: 'myskin_install_unavailable'
				});
				return deferred.promise();
			}

			// Prevent loading a non-active myskin preview when there is a drafted/scheduled changeset.
			if ( ! panel.canSwitchMySkin( slug ) ) {
				deferred.reject({
					errorCode: 'myskin_switch_unavailable'
				});
				return deferred.promise();
			}

			// MySkin is already being installed.
			if ( _.contains( panel.installingMySkins, slug ) ) {
				deferred.reject({
					errorCode: 'myskin_already_installing'
				});
				return deferred.promise();
			}
			...
			onInstallSuccess = function( response ) {
				var myskin = false, myskinControl;
				if ( preview ) {
					api.notifications.remove( 'myskin_installing' );

					panel.loadMySkinPreview( slug );

				} else {
					api.control.each( function( control ) {
						if ( 'myskin' === control.params.type && control.params.myskin.id === response.slug ) {
							myskin = control.params.myskin; // Used below to add myskin control.
							control.rerenderAsInstalled( true );
						}
					});

					// Don't add the same myskin more than once.
					if ( ! myskin || api.control.has( 'installed_myskin_' + myskin.id ) ) {
						deferred.resolve( response );
						return;
					}

					// Add myskin control to installed section.
					myskin.type = 'installed';
					myskinControl = new api.controlConstructor.myskin( 'installed_myskin_' + myskin.id, {
						type: 'myskin',
						section: 'installed_myskins',
						myskin: myskin,
						priority: 0 // Add all newly-installed myskins to the top.
					} );

					api.control.add( myskinControl );
					api.control( myskinControl.id ).container.trigger( 'render-screenshot' );

					// Close the details modal if it's open to the installed myskin.
					api.section.each( function( section ) {
						if ( 'myskins' === section.params.type ) {
							if ( myskin.id === section.currentMySkin ) { // Don't close the modal if the user has navigated elsewhere.
								section.closeDetails();
							}
						}
					});
			...
			panel.installingMySkins.push( slug ); // Note: we don't remove elements from installingMySkins, since they shouldn't be installed again.
			request = mcms.updates.installMySkin( {
				slug: slug
			} );

			// Also preview the myskin as the event is triggered on Install & Preview.
			if ( preview ) {
				api.notifications.add( new api.OverlayNotification( 'myskin_installing', {
					message: api.l10n.myskinDownloading,
					type: 'info',
					loading: true
				} ) );
			}
		...
		loadMySkinPreview: function( myskinId ) {
			...
			// Prevent loading a non-active myskin preview when there is a drafted/scheduled changeset.
			if ( ! panel.canSwitchMySkin( myskinId ) ) {
				deferred.reject({
					errorCode: 'myskin_switch_unavailable'
				});
				return deferred.promise();
			...
			api.notifications.add( new api.OverlayNotification( 'myskin_previewing', {
				message: api.l10n.myskinPreviewWait,
				type: 'info',
				loading: true
			} ) );
			...
			onceProcessingComplete = function() {
				...
				request.fail( function() {
					...
					api.notifications.remove( 'myskin_previewing' );
		...
		/**
		 * Update a myskin via mcms.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		updateMySkin: function( event ) {
			...
			$( document ).one( 'mcms-myskin-update-success', function( e, response ) {

				// Rerender the control to reflect the update.
				api.control.each( function( control ) {
					if ( 'myskin' === control.params.type && control.params.myskin.id === response.slug ) {
						control.params.myskin.hasUpdate = false;
						control.params.myskin.version = response.newVersion;
						setTimeout( function() {
							control.rerenderAsInstalled( true );
						}, 2000 );
					}
		...
		/**
		 * Delete a myskin via mcms.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		deleteMySkin: function( event ) {
			var myskin, section;
			myskin = $( event.target ).data( 'slug' );
			section = api.section( 'installed_myskins' );
			...
			if ( ! window.confirm( api.settings.l10n.confirmDeleteMySkin ) ) {
				return;
			}

			mcms.updates.maybeRequestFilesystemCredentials( event );

			$( document ).one( 'mcms-myskin-delete-success', function() {
				var control = api.control( 'installed_myskin_' + myskin );

				// Remove myskin control.
				control.container.remove();
				api.control.remove( control.id );
				...
				// Rerender any other myskin controls as uninstalled.
				api.control.each( function( control ) {
					if ( 'myskin' === control.params.type && control.params.myskin.id === myskin ) {
	 ...
	 *
	 * @see PHP class MCMS_Customize_Control.
	 *
	 * @class
	 * @augments mcms.customize.Class
	 */
	api.Control = api.Class.extend({
		...
		renderNotifications: function() {
			...
			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] mcms.customize.Control.prototype.renderNotifications() is deprecated in favor of instantating a mcms.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = mcms.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = mcms.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = mcms.template( 'available-menu-item' );
	 ...
	 *
	 * @class
	 * @augments mcms.customize.Control
	 * @augments mcms.customize.Class
	 */
	api.ColorControl = api.Control.extend({
		ready: function() {
			...
			if ( isHueSlider ) {
				...
				picker.val( control.setting() ).mcmsColorPicker({
				...
				picker.val( control.setting() ).mcmsColorPicker({
					change: function() {
						updating = true;
						control.setting.set( picker.mcmsColorPicker( 'color' ) );
						updating = false;
					},
					clear: function() {
						updating = true;
			...
			control.setting.bind( function ( value ) {
				...
				picker.val( value );
				picker.mcmsColorPicker( 'color', value );
			} );
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.mcms-picker-container' );
				if ( pickerContainer.hasClass( 'mcms-picker-active' ) ) {
					picker.mcmsColorPicker( 'close' );
					control.container.find( '.mcms-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	...
	api.MediaControl = api.Control.extend({
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					mcms.media.attachment( value ).fetch().done( function() {
						...
						mcms.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && mcms.media.mixin.removePlayer( this.player );
		},
		...
		initFrame: function() {
			this.frame = mcms.media({
				...
				states: [
					new mcms.media.controller.Library({
						title:     this.params.button_labels.frame_title,
						library:   mcms.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._mcmsmejsSettings || {};
	 ...
	 *
	 * @class
	 * @augments mcms.customize.MediaControl
	 * @augments mcms.customize.Control
	 * @augments mcms.customize.Class
	 */
	 ...
	 *
	 * @class
	 * @augments mcms.customize.UploadControl
	 * @augments mcms.customize.MediaControl
	 * @augments mcms.customize.Control
	 * @augments mcms.customize.Class
	 */
	...
	api.BackgroundControl = api.UploadControl.extend({
		...
		select: function() {
			...
			mcms.ajax.post( 'custom-background-add', {
				nonce: _mcmsCustomizeBackground.nonces.add,
				mcms_customize: 'on',
				customize_myskin: api.settings.myskin.stylesheet,
	...
	api.CroppedImageControl = api.MediaControl.extend({
		...
		initFrame: function() {
			...
			this.frame = mcms.media({
				...
				states: [
					new mcms.media.controller.Library({
						title: this.params.button_labels.frame_title,
						library: mcms.media.query({ type: 'image' }),
						multiple: false,
						date: false,
						priority: 20,
					...
					new mcms.media.controller.CustomizeImageCropper({
		 ...
		 * control-specific data, to be fed to the imgAreaSelect module in
		 * mcms.media.view.Cropper.
		 *
		 * @param {mcms.media.model.Attachment} attachment
		 * @param {mcms.media.controller.Cropper} controller
	 ...
	 *
	 * @class
	 * @augments mcms.customize.CroppedImageControl
	 * @augments mcms.customize.MediaControl
	 * @augments mcms.customize.Control
	 * @augments mcms.customize.Class
	 */
		...
		initFrame: function() {
			...
			this.frame = mcms.media({
				...
				states: [
					...
					new mcms.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	api.HeaderControl = api.Control.extend({
		ready: function() {
			...
			// Ensure custom-header-crop Ajax requests bootstrap the Customizer to activate the previewed myskin.
			mcms.media.controller.Cropper.prototype.defaults.doCropArgs.mcms_customize = 'on';
			mcms.media.controller.Cropper.prototype.defaults.doCropArgs.customize_myskin = api.settings.myskin.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _mcmsCustomizeHeader.uploads, function( imageObj ) {
		...
		/**
		 * Returns a set of options, computed from the attached image data and
		 * myskin-specific data, to be fed to the imgAreaSelect module in
		 * mcms.media.view.Cropper.
		 *
		 * @param {mcms.media.model.Attachment} attachment
		 * @param {mcms.media.controller.Cropper} controller
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_mcmsCustomizeHeader.data.width, 10),
				yInit = parseInt(_mcmsCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_mcmsCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_mcmsCustomizeHeader.data['flex-height'], 10),
			...
			this.headerImage.set({
				myskinWidth: xInit,
				myskinHeight: yInit,
				myskinFlexWidth: flexWidth,
				myskinFlexHeight: flexHeight,
				imageWidth: realWidth,
				imageHeight: realHeight
			});
		 ...
		 * current myskin, a cropping step after selection may be required or
		 * skippable.
		 *
		 * @param {event} event
		 */
			...
			this.frame = mcms.media({
				...
				states: [
					new mcms.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _mcmsCustomizeHeader.data.width,
						suggestedHeight: _mcmsCustomizeHeader.data.height
					}),
					new mcms.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new mcms.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * mcms.customize.MySkinControl
	 *
	 * @constructor
	 * @augments mcms.customize.Control
	 * @augments mcms.customize.Class
	 */
	api.MySkinControl = api.Control.extend({
		 ...
		 */
		ready: function() {
			var control = this, panel = api.panel( 'myskins' );

			function disableSwitchButtons() {
				return ! panel.canSwitchMySkin( control.params.myskin.id );
			}
			...
			function updateButtons() {
				control.container.find( 'button.preview, button.preview-myskin' ).toggleClass( 'disabled', disableSwitchButtons() );
				control.container.find( 'button.myskin-install' ).toggleClass( 'disabled', disableInstallButtons() );
			}
			...
			control.container.on( 'touchmove', '.myskin', function() {
				control.touchDrag = true;
			});

			// Bind details view trigger.
			control.container.on( 'click keydown touchend', '.myskin', function( event ) {
				var section;
				if ( api.utils.isKeydownButNotEnterEvent( event ) ) {
					return;
				}
				...
				if ( $( event.target ).is( '.myskin-actions .button, .update-myskin' ) ) {
					return;
				}

				event.preventDefault(); // Keep this AFTER the key filter above
				section = api.section( control.section() );
				section.showDetails( control.params.myskin, function() {

					// Temporary special function since supplying SFTP credentials does not work yet. See #42184.
					if ( api.settings.myskin._filesystemCredentialsNeeded ) {
						section.overlay.find( '.myskin-actions .delete-myskin' ).remove();
					}
				} );
			});
		...
		/**
		 * Show or hide the myskin based on the presence of the term in the title, description, tags, and author.
		 *
		 * @since 4.2.0
		 * @param {Array} terms - An array of terms to search for.
		 * @returns {boolean} Whether a myskin control was activated or not.
		 */
		filter: function( terms ) {
			var control = this,
				matchCount = 0,
				haystack = control.params.myskin.name + ' ' +
					control.params.myskin.description + ' ' +
					control.params.myskin.tags + ' ' +
					control.params.myskin.author + ' ';
			haystack = haystack.toLowerCase().replace( '-', ' ' );

			// Back-compat for behavior in MandarinCMS 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}

			// Always give exact name matches highest ranking.
			if ( control.params.myskin.name.toLowerCase() === terms.join( ' ' ) ) {
				matchCount = 100;
			} else {
		...
		/**
		 * Rerender the myskin from its JS template with the installed type.
		 *
		 * @since 4.9.0
		 *
		 * @returns {void}
		...
		rerenderAsInstalled: function( installed ) {
			...
			if ( installed ) {
				control.params.myskin.type = 'installed';
			} else {
				section = api.section( control.params.section );
				control.params.myskin.type = section.params.action;
			}
	...
	/**
	 * Class mcms.customize.CodeEditorControl
	 *
	 * @since 4.9.0
	 *
	 * @constructor
	...
	api.CodeEditorControl = api.Control.extend({
		...
		initEditor: function() {
			...
			if ( mcms.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = mcms.codeEditor.defaultSettings ? _.clone( mcms.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = mcms.codeEditor.initialize( $textarea, settings );
			...
			// Update CodeMirror when the setting is changed by another module.
			control.setting.bind( function( value ) {
	...
	/**
	 * Class mcms.customize.DateTimeControl.
	 *
	 * @since 4.9.0
	 * @constructor
	 * @augments mcms.customize.Control
	...
	api.DateTimeControl = api.Control.extend({
		 ...
		 * @return {mcms.customize.DateTimeControl}
		 */
	 ...
	 * @param {mcms.customize.Control[]} Resolved controls.
	 */
	 ...
	 * @returns {mcms.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param), or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * mcms.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = mcms.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = mcms.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * mcms.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = mcms.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * mcms.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = mcms.customize.control( 'background_color ').setting.get();
	 * value = mcms.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * mcms.customize.control.add( new mcms.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * mcms.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * mcms.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * mcms.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 * @param {mcms.customize.Section[]} Resolved sections.
	 */
	 ...
	 * @returns {mcms.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param), or promise resolving to requested sections.
	 *
	 ...
	 * section = mcms.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * mcms.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link mcms.customize.control} for further examples of how to interact with {@link mcms.customize.Values} instances.
	 */
	 ...
	 * @param {mcms.customize.Panel[]} Resolved panels.
	 */
	 ...
	 * @returns {mcms.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param), or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * mcms.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = mcms.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * mcms.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link mcms.customize.control} for further examples of how to interact with {@link mcms.customize.Values} instances.
	 ...
	 * @param {mcms.customize.Notification[]} Resolved notifications.
	 */
	 ...
	 * @returns {mcms.customize.Notification|undefined|jQuery.promise} notification instance or undefined (if function called with one code param), or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = mcms.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = mcms.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * mcms.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * mcms.customize.notifications.add( new mcms.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * mcms.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link mcms.customize.control} for further examples of how to interact with {@link mcms.customize.Values} instances.
	 */
	 ...
	 *
	 * @class
	 * @augments mcms.customize.Messenger
	 * @augments mcms.customize.Class
	 * @mixes mcms.customize.Events
	 */
		...
		run: function( deferred ) {
			...
			params = _.extend(
				api.utils.parseQueryString( urlParser.search.substr( 1 ) ),
				{
					...
					customize_myskin: previewFrame.query.customize_myskin,
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See MCMS Trac #38149.
	...
	api.Previewer = api.Messenger.extend({
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in mcms-roots or mcms-plugins.
				if ( /\/mcms-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
				...
				if ( urlParser.search.length > 1 ) {
					...
					delete queryParams.customize_myskin;
		...
		keepPreviewAlive: function keepPreviewAlive() {
			 ...
			 *
			 * This is most likely to happen in the case of a connectivity error, or if the myskin causes the browser
		...
		refreshNonces: function() {
			...
			request = mcms.ajax.post( 'customize_refresh_nonces', {
				mcms_customize: 'on',
				customize_myskin: api.settings.myskin.stylesheet
			});
	...
	api.controlConstructor = {
		...
		myskin:               api.MySkinControl,
		date_time:           api.DateTimeControl,
		code_editor:         api.CodeEditorControl
	};
	api.panelConstructor = {
		myskins: api.MySkinsPanel
	};
	api.sectionConstructor = {
		myskins: api.MySkinsSection,
	 ...
	 * @returns {object<string, mcms.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	...
	api.reflowPaneContents = _.bind( function () {
		...
		api.panel.each( function ( panel ) {
			if ( 'myskins' === panel.id ) {
				return; // Don't reflow myskin sections, as doing so moves them after the myskins container.
			}
		...
		appendContainer = $( '#customize-myskin-controls .customize-pane-parent' ); // @todo This should be defined elsewhere, and to be configurable
	...
	$( function() {
		api.settings = window._mcmsCustomizeSettings;
		api.l10n = window._mcmsCustomizeControlsL10n;
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.mcms-full-overlay' ),
		...
		api.bind( 'ready', function() {
			api.section.add( new api.OuterSection( 'publish_settings', {
				...
				priority: 0,
				active: api.settings.myskin.active
			} ) );
		} );
		...
		api.section( 'publish_settings', function( section ) {
			...
			trashControl.deferred.embedded.done( function() {
				trashControl.container.find( '.button-link' ).on( 'click', function() {
					if ( confirm( api.l10n.trashConfirm ) ) {
						mcms.customize.previewer.trash();
					}
				} );
			} );
			...
			// Make sure publish settings are not available while the myskin is not active and the customizer is in a published state.
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = mcms.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = mcms.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
			...
			getFrontendPreviewUrl: function() {
				...
				if ( ! api.state( 'activated' ).get() ) {
					params.customize_myskin = api.settings.myskin.stylesheet;
				}
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /mcms_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.mcms-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			state.bind( 'change', function() {
				 ...
				 * and if the myskin is not active or the changeset exists but is not published.
				 */
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via mcms.customizer.previewer.save() will update selection.
			saved( true );
			...
			saving( false );
			activated( api.settings.myskin.active );
			processing( 0 );
			paneVisible( true );
			expandedPanel( false );
		...
		( function checkAndDisplayLockNotice() {
			 ...
			 * @class
			 * @augments mcms.customize.Notification
			 * @augments mcms.customize.OverlayNotification
			 */
			var LockedNotification = api.OverlayNotification.extend({
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = mcms.ajax.post( 'customize_override_changeset_lock', {
							mcms_customize: 'on',
							customize_myskin: api.settings.myskin.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				mcms.ajax.post( 'customize_dismiss_autosave_or_lock', {
					mcms_customize: 'on',
					customize_myskin: api.settings.myskin.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		body.on( 'keydown', function( event ) {
			...
			if ( collapsedObject ) {
				if ( 'myskins' === collapsedObject.params.type ) {

					// MySkins panel or section.
					if ( body.hasClass( 'modal-open' ) ) {
						collapsedObject.closeDetails();
					} else if ( api.panel.has( 'myskins' ) ) {

						// If we're collapsing a section, collapse the panel also.
						api.panel( 'myskins' ).collapse();
					}
					return;
				}
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.mcms-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {mcms.customize.Panel|mcms.customize.Section} container Construct.
			 * @returns {void}
			 */
		...
		(function() {
			...
			function isCleanState() {
				...
				/*
				 * Handle special case of previewing myskin switch since some settings (for nav menus and widgets)
				 * are pre-dirty and non-active myskins can only ever be auto-drafts.
				 */
				if ( ! api.state( 'activated' ).get() ) {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.mcms-customize-changeset-update' );
					$( window ).off( 'beforeunload.mcms-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					mcms.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_mcmsCustomizeBackground.defaults['default-position-x'],
				_mcmsCustomizeBackground.defaults['default-position-y'],
				_mcmsCustomizeBackground.defaults['default-size'],
				_mcmsCustomizeBackground.defaults['default-repeat'],
				_mcmsCustomizeBackground.defaults['default-attachment']
			];
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.mcms-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.mcms-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
