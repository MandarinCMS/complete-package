( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#mcms-plugins-wrap' ),
			$tools = $( '#mcms-plugins-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.mcms.editor && window.mcms.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.mcms.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.mcms && editor.mcms.getView && ( view = editor.mcms.getView( node ) ) ) {
			 ...
			 * the window when the cursor moves out of the viewport to a mcmsview.
			 ...
			 * Executes the mcmsAutoResize on the active editor.
			 *
			 * @since 4.0.0
			 *
			 * @returns {void}
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'mcmsAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'mcms-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent mcms-autoresize mcms-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'mcms-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent mcms-autoresize mcms-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'mcms-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .mcms-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'mcms-editor-expand' );
			 ...
			 */
			$document.on( 'mcms-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'mcms-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'mcmsAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.mcms_autoresize_on = true;
				mceEditor.execCommand( 'mcmsAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'mcms-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.mcms_autoresize_on = false;
				mceEditor.execCommand( 'mcmsAutoResizeOff' );

				if ( ! mceEditor.isHidden() ) {
					$textEditor.hide();

					if ( height ) {
						mceEditor.myskin.resizeTo( null, height );
					}
				}
			}
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'mcmsToggleDFW', toggle );
			editor.addShortcut( 'access+w', '', 'mcmsToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'mcms-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'mcms-autoresize', recalcEditorRect );
				};
