/* global pagenow, ajaxurl, postboxes, mcmsActiveEditor:true */
var ajaxWidgets, ajaxPopulateWidgets, quickPressLoad;
window.mcms = window.mcms || {};

jQuery(document).ready( function($) {
	var welcomePanel = $( '#welcome-panel' ),
		welcomePanelHide = $('#mcms_welcome_panel-hide'),
		updateWelcomePanel;
	...
	$('.welcome-panel-close, .welcome-panel-dismiss a', welcomePanel).click( function(e) {
		...
		$('#mcms_welcome_panel-hide').prop('checked', false);
	});
	...
	$( '.meta-box-sortables' ).sortable( 'option', 'containment', '#mcmswrap' );
...
jQuery( function( $ ) {
	...
	app = window.mcms.communityEvents = {
		initialized: false,
		model: null,

		/**
		 * Initializes the mcms.communityEvents object.
		 *
		 * @since 4.8.0
		 */
		init: function() {
		...
		getEvents: function( requestParams ) {
			...
			requestParams._mcmsnonce = communityEventsData.nonce;
			...
			mcms.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		 ...
		 * @since 4.8.0
		 *
		 * @param {Object} templateParams The various parameters that will get passed to mcms.template.
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					template = mcms.template( 'community-events-event-list' );
					$results.html( template( templateParams ) );
				} else {
					template = mcms.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = mcms.template( 'community-events-attend-event-near' );
				...
				if ( 'user' === initiatedBy ) {
					mcms.a11y.speak( communityEventsData.l10n.city_updated.replace( l10nPlaceholder, templateParams.location.description ), 'assertive' );
				}
				...
				template = mcms.template( 'community-events-could-not-locate' );
				$( '.community-events-could-not-locate' ).html( template( templateParams ) );
				mcms.a11y.speak( communityEventsData.l10n.could_not_locate_city.replace( l10nPlaceholder, templateParams.unknownCity ) );
				 ...
				 */
				mcms.a11y.speak( communityEventsData.l10n.error_occurred_please_try_again );
