...
jQuery(document).ready(function($) {
	...
	if ( $('#media-items>*').length > 1 ) {
		w = mcmsgallery.getWin();

		$('#save-all, #gallery-settings').show();
		if ( typeof w.tinyMCE !== 'undefined' && w.tinyMCE.activeEditor && ! w.tinyMCE.activeEditor.isHidden() ) {
			mcmsgallery.mcemode = true;
			mcmsgallery.init();
		} else {
			$('#insert-gallery').show();
		}
...
jQuery(window).unload( function () { tinymce = tinyMCE = mcmsgallery = null; } ); // Cleanup

/* gallery settings */
var tinymce = null, tinyMCE, mcmsgallery;
...
mcmsgallery = {
	...
	setup : function() {
		...
		if ( t.el.nodeName !== 'IMG' || ! ed.dom.hasClass(t.el, 'mcmsGallery') ) {
			if ( ( g = ed.dom.select('img.mcmsGallery') ) && g[0] ) {
				t.el = g[0];
			} else {
