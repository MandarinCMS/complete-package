...
mcms.myskinModuleEditor = (function( $ ) {
	'use strict';
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = mcms.template( 'mcms-file-editor-notice' );
		...
		if ( false !== component.codeEditor ) {
			/*
			 * Defer adding notices until after DOM ready as workaround for MCMS Admin injecting
	...
	component.showWarning = function() {
		...
		// Hide all the #mcmswrap content from assistive technologies.
		$( '#mcmswrap' ).attr( 'aria-hidden', 'true' );
		...
		setTimeout( function() {
			mcms.a11y.speak( mcms.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.dismissWarning = function() {
		...
		mcms.ajax.post( 'dismiss-mcms-pointer', {
			pointer: component.myskinOrModule + '_editor_notice'
		});

		// Hide modal.
		component.warning.remove();
		$( '#mcmswrap' ).removeAttr( 'aria-hidden' );
	...
	component.submit = function( event ) {
		...
		request = mcms.ajax.post( 'edit-myskin-module-file', data );
	...
	component.addNotice = function( notice ) {
		...
		mcms.a11y.speak( notice.message );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = mcms.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
		...
		$( editor.codemirror.display.lineDiv )
			.attr({
				role: 'textbox',
				'aria-multiline': 'true',
				'aria-labelledby': 'myskin-module-editor-label',
				'aria-describedby': 'editor-keyboard-trap-help-1 editor-keyboard-trap-help-2 editor-keyboard-trap-help-3 editor-keyboard-trap-help-4'
			});

		// Focus the editor when clicking on its label.
		$( '#myskin-module-editor-label' ).on( 'click', function() {
