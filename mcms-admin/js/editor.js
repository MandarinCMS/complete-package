...
( function( $, mcms ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'mcms-switch-editor' ) ) {
						id = target.attr( 'data-mcms-editor-id' );
		...
		function switchEditor( id, mode ) {
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				if ( editor ) {
					keepSelection = editor.getParam( 'mcms_keep_scroll_position' );
				} else {
					keepSelection = window.tinyMCEPreInit.mceInit[ id ] &&
									window.tinyMCEPreInit.mceInit[ id ].mcms_keep_scroll_position;
				}
				...
				if ( editor ) {
					...
					if ( ! tinymce.Env.iOS && editorHeight ) {
						...
						if ( editorHeight > 50 && editorHeight < 5000 ) {
							editor.myskin.resizeTo( null, editorHeight );
						}
					}

					if ( editor.getParam( 'mcms_keep_scroll_position' ) ) {
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = mcms.shortcode.regexp( allShortcodes.join( '|' ) ),
		 ...
		 * The method directly manipulates the `textarea` content, to allow TinyMCE modules
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#mcms-plugins-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#mcms-plugins-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viewport (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.mcms_autoresize_on ) {
		 ...
		 * `mcms-roots/js/tinymce/modules/mandarincms/module.js`.
		 *
		...
		function findBookmarkedPosition( editor ) {
			 ...
			 * TinyMCE does some serialization of the content, to fix things like shortcodes, run modules, prettify
		...
		/**
		 * @summary Replaces <p> tags with two line breaks. "Opposite" of mcmsautop().
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<mcms-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<mcms-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<mcms-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<mcms-preserve>/g, function() {
		 ...
		 *
		 * Similar to `mcmsautop()` in formatting.php.
		 *
		 * @since 2.5.0
		 *
		 * @memberof switchEditors
		...
		function autop( text ) {
			...
			if ( text.indexOf( '[caption' ) !== -1 ) {
				...
				text = text.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					a = a.replace( /<br([^>]*)>/g, '<mcms-temp-br$1>' );
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<mcms-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<mcms-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		mcms.editor.autop = mcmsautop;
		mcms.editor.removep = pre_mcmsautop;

		exports = {
			go: switchEditor,
			mcmsautop: mcmsautop,
			pre_mcmsautop: pre_mcmsautop,
			_mcms_Autop: autop,
			_mcms_Nop: removep
		};
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with mcms_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Text editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, mcms-before-tinymce-init and mcms-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * }
	 */
	mcms.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! mcms.editor.getDefaultSettings ) {
			return;
		}

		defaults = mcms.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'mcms-core-ui mcms-editor-wrap tmce-active',
					id: 'mcms-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="mcms-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-mcms-editor-id': id
				} );

			var $editorTools = $( '<div class="mcms-editor-tools">' );

			if ( settings.mediaButtons ) {
				var buttonText = 'Add Media';

				if ( window._mcmsMediaViewsL10n && window._mcmsMediaViewsL10n.addMedia ) {
					buttonText = window._mcmsMediaViewsL10n.addMedia;
				}

				var $addMediaButton = $( '<button type="button" class="button insert-media add_media">' );

				$addMediaButton.append( '<span class="mcms-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="mcms-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="mcms-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'mcms-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'mcms-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'mcms-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'mcms-before-quicktags-init', init );
			window.quicktags( init );

			if ( ! window.mcmsActiveEditor ) {
				window.mcmsActiveEditor = init.id;
			}
		}
	};
	 ...
	 *
	 * Intended for use with editors that were initialized with mcms.editor.initialize().
	 *
	 * @since 4.8.0
	 *
	 * @param {string} id The HTML id of the editor textarea.
	 */
	mcms.editor.remove = function( id ) {
	 ...
	 */
	mcms.editor.getContent = function( id ) {
		var editor;
