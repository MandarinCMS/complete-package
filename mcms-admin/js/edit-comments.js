/* global adminCommentsL10n, thousandsSeparator, list_args, QTags, ajaxurl, mcmsAjax */
...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			mcmsListsData = $( settings.target ).attr( 'data-mcms-lists' );
		...
		if ( mcmsListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( mcmsListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = mcmsListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_mcmsnonce=' + settings.data._ajax_nonce);
			a.attr('data-mcms-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).mcmsList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function () {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).mcmsList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see MCMS_Comments_List_Table::prepare_items() @ class-mcms-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).mcmsList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').mcmsList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').mcmsList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('mcmsListDelEnd', function(e, s){
			var mcmsListsData = $(s.target).attr('data-mcms-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( mcmsListsData.indexOf(':trash=1') != -1 || mcmsListsData.indexOf(':spam=1') != -1 )
...
commentReply = {
	...
	show : function(xml) {
		...
		r = mcmsAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': mcmsAjax.broken});
			return false;
		}
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		toggle_all = function() {
			$('#cb-select-all-1').data( 'mcms-toggle', 1 ).trigger( 'click' ).removeData( 'mcms-toggle' );
		};
