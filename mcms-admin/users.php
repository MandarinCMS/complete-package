...
if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="mcms_http_referer" value="'. esc_attr( mcms_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset($_REQUEST['mcms_http_referer']) ) {
	$redirect = remove_query_arg(array('mcms_http_referer', 'updated', 'delete_count'), mcms_unslash( $_REQUEST['mcms_http_referer'] ) );
	$referer = '<input type="hidden" name="mcms_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		mcms_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && ! $mcms_roles->role_objects[ $role ]->has_cap('promote_users')
	...
	mcms_redirect(add_query_arg('update', $update, $redirect));
	exit();

case 'dodelete':
	if ( is_multisite() )
		mcms_die( __('User deletion is not allowed from this screen.'), 400 );
	...
	if ( empty( $_REQUEST['delete_option'] ) ) {
		...
		$url = str_replace( '&amp;', '&', mcms_nonce_url( $url, 'bulk-users' ) );
		mcms_redirect( $url );
		exit;
	}

	if ( ! current_user_can( 'delete_users' ) )
		mcms_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
	...
	foreach ( $userids as $id ) {
		if ( ! current_user_can( 'delete_user', $id ) )
			mcms_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
		...
		case 'delete':
			...
			break;
		case 'reassign':
			mcms_delete_user( $id, $_REQUEST['reassign_user'] );
			break;
		}
		++$delete_count;
	}
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new MCMS_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
	...
	if ( $mcmsdb->get_var( "SELECT ID FROM {$mcmsdb->posts} WHERE post_author IN( " . implode( ',', $userids ) . " ) LIMIT 1" ) ) {
		$users_have_content = true;
	} elseif ( $mcmsdb->get_var( "SELECT link_id FROM {$mcmsdb->links} WHERE link_owner IN( " . implode( ',', $userids ) . " ) LIMIT 1" ) ) {
		$users_have_content = true;
	}
...
<?php mcms_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) :
	 ...
	 *
	 * @param MCMS_User $current_user MCMS_User object for the current user.
...
case 'doremove':
	...
	if ( ! is_multisite() )
		mcms_die( __( 'You can&#8217;t remove users.' ), 400 );
	...
	if ( ! current_user_can( 'remove_users' ) )
		mcms_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new MCMS_Error('edit_users', __('Sorry, you are not allowed to remove users.'));
...
?>
<form method="post" name="updateusers" id="updateusers">
<?php mcms_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_mcms_http_referer']) ) {
		mcms_redirect( remove_query_arg( array( '_mcms_http_referer', '_mcmsnonce'), mcms_unslash( $_SERVER['REQUEST_URI'] ) ) );
		exit;
	}

	if ( $mcms_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
		$userids = $_REQUEST['users'];
		$sendback = mcms_get_referer();

		/** This action is documented in mcms-admin/edit-comments.php */
		$sendback = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $sendback, $mcms_list_table->current_action(), $userids );
		...
		case 'add':
			if ( isset( $_GET['id'] ) && ( $user_id = $_GET['id'] ) && current_user_can( 'edit_user', $user_id ) ) {
				...
				$messages[] = '<div id="message" class="updated notice is-dismissible"><p>' . sprintf( __( 'New user created. <a href="%s">Edit user</a>' ),
					esc_url( add_query_arg( 'mcms_http_referer', urlencode( mcms_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<?php #$mcms_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
