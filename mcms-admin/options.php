...
mcms_reset_vars(array('action', 'option_page'));
...
if ( ! empty( $_GET[ 'adminhash' ] ) ) {
	...
	mcms_redirect( admin_url( $redirect ) );
	exit;
} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
	check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
	delete_option( 'adminhash' );
	delete_option( 'new_admin_email' );
	mcms_redirect( admin_url( 'options-general.php?updated=true' ) );
	exit;
}
...
$whitelist_options = array(
	'general' => array( 'blogname', 'blogdescription', 'gmt_offset', 'date_format', 'time_format', 'start_of_week', 'timezone_string', 'MCMSLANG', 'new_admin_email' ),
...
if ( !is_multisite() ) {
	...
	if ( get_option( 'upload_url_path' ) || ( get_option('upload_path') != 'mcms-plugins/uploads' && get_option('upload_path') ) ) {
...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		mcms_die( __( '<strong>ERROR</strong>: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! current_user_can( 'manage_network_options' ) ) {
			mcms_die( __( 'Sorry, you are not allowed to modify unregistered settings for this site.' ) );
		}
		$options = explode( ',', mcms_unslash( $_POST[ 'page_options' ] ) );
	} else {
	...
	if ( 'general' == $option_page ) {
		...
		if ( !empty($_POST['date_format']) && isset($_POST['date_format_custom']) && '\c\u\s\t\o\m' == mcms_unslash( $_POST['date_format'] ) )
			$_POST['date_format'] = $_POST['date_format_custom'];
		if ( !empty($_POST['time_format']) && isset($_POST['time_format_custom']) && '\c\u\s\t\o\m' == mcms_unslash( $_POST['time_format'] ) )
		...
		if ( ! empty( $_POST['MCMSLANG'] ) && current_user_can( 'install_languages' ) ) {
			require_once( BASED_TREE_URI . 'mcms-admin/includes/translation-install.php' );

			if ( mcms_can_install_language_pack() ) {
	...
	if ( $options ) {
		...
		foreach ( $options as $option ) {
			if ( $unregistered ) {
				_deprecated_argument( 'options.php', '2.7.0',
					sprintf(
						...
						__( 'The %s setting is unregistered. Unregistered settings are deprecated. See https://dev.mandarincms.com/Settings_API' ),
						'<code>' . $option . '</code>'
					)
				);
			}
			...
			if ( isset( $_POST[ $option ] ) ) {
				...
				$value = mcms_unslash( $value );
			}
			update_option( $option, $value );
		}

		/*
		 * Switch translation in case MCMSLANG was changed.
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true',  mcms_get_referer() );
	mcms_redirect( $goback );
	exit;
}
...
<div class="wrap">
  ...
  <?php mcms_nonce_field('options-options') ?>
  <input type="hidden" name="action" value="update" />
  <input type="hidden" name="option_page" value="options" />
  <table class="form-table">
<?php
$options = $mcmsdb->get_results( "SELECT * FROM $mcmsdb->options ORDER BY option_name" );
