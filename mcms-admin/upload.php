...
if ( !current_user_can('upload_files') )
	mcms_die( __( 'Sorry, you are not allowed to upload files.' ) );
...
if ( 'grid' === $mode ) {
	mcms_enqueue_media();
	mcms_enqueue_script( 'media-grid' );
	mcms_enqueue_script( 'media' );

	remove_action( 'admin_head', 'mcms_admin_canonical_url' );

	$q = $_GET;
	// let JS handle this
	unset( $q['s'] );
	$vars = mcms_edit_attachments_query_vars( $q );
	...
	mcms_localize_script( 'media-grid', '_mcmsMediaGridSettings', array(
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://dev.mandarincms.com/Media_Library_Screen">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="mcms-media-grid" data-search="<?php _admin_search_query() ?>">
...
$mcms_list_table = _get_list_table('MCMS_Media_List_Table');
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		$post_ids = $mcmsdb->get_col( "SELECT ID FROM $mcmsdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			mcms_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			mcms_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				...
				if ( !mcms_delete_attachment( $post_id_del ) )
...
if ( ! empty( $_GET['trashed'] ) && $trashed = absint( $_GET['trashed'] ) ) {
	...
	$message .= ' <a href="' . esc_url( mcms_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __('Undo') . '</a>';
...
$messages[4] = __( 'Media file moved to the trash.' ) . ' <a href="' . esc_url( mcms_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __( 'Undo' ) . '</a>';
