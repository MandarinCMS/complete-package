...
/** MandarinCMS Administration Widgets API */
...
if ( 'on' == $widgets_access ) {
	add_filter( 'admin_body_class', 'mcms_widgets_access_body_class' );
} else {
	mcms_enqueue_script('admin-widgets');
...
'content'	=>
	'<p>' . __('If you want to remove the widget but save its setting for possible future use, just drag it into the Inactive Widgets area. You can add them back anytime from there. This is especially helpful when you switch to a myskin with fewer or different widget areas.') . '</p>
...
'content'	=>
	'<p>' . __('Many myskins show some sidebar widgets by default until you edit your sidebars, but they are not automatically displayed in your sidebar management tool. After you make your first widget change, you can re-add the default widgets by adding them from the Available Widgets area.') . '</p>' .
		'<p>' . __('When changing myskins, there is often some variation in the number and setup of widget areas/sidebars and sometimes these conflicts make the transition a bit less smooth. If you changed myskins and seem to be missing widgets, scroll down on this screen to the Inactive Widgets area, where all of your widgets and their settings will have been saved.') . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="https://dev.mandarincms.com/Dexign_Widgets_Screen">Documentation on Widgets</a>') . '</p>' .
	'<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
);

if ( ! current_myskin_supports( 'widgets' ) ) {
	mcms_die( __( 'The myskin you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your myskin widget-aware, please <a href="https://dev.mandarincms.com/Widgetizing_MySkins">follow these instructions</a>.' ) );
}
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = mcms_get_widget_defaults();

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'mcms_inactive_widgets' == $sidebar_id )
		continue;
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			mcms_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
	...
	mcms_redirect( admin_url('widgets.php?message=0') );
	exit;
}

// Remove inactive widgets without js
if ( isset( $_POST['removeinactivewidgets'] ) ) {
	check_admin_referer( 'remove-inactive-widgets', '_mcmsnonce_remove_inactive_widgets' );
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	if ( isset($_GET['addnew']) ) {
		...
		$keys = array_keys( $mcms_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			// Copy minimal info from an existing instance of this widget to a new instance
			foreach ( $mcms_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					$control_callback = $control['callback'];
					$multi_number = (int) $_GET['num'];
					$control['params'][0]['number'] = -1;
					$widget_id = $control['id'] = $control['id_base'] . '-' . $multi_number;
					$mcms_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($mcms_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $mcms_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($mcms_registered_widget_controls[$widget_id]) && isset($mcms_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($mcms_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'mcms_inactive_widgets';
...
<?php
	foreach ( $mcms_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'mcms_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
...
<?php	mcms_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
...
$myskin_sidebars = array();
foreach ( $mcms_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( false !== strpos( $registered_sidebar['class'], 'inactive-sidebar' ) || 'orphaned_widgets' == substr( $sidebar, 0, 16 ) ) {
		$wrap_class = 'widgets-holder-wrap';
		if ( !empty( $registered_sidebar['class'] ) )
			$wrap_class .= ' ' . $registered_sidebar['class'];

		$is_inactive_widgets = 'mcms_inactive_widgets' == $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php mcms_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form action="" method="post">
						<p>
							...
							if ( empty($sidebars_widgets['mcms_inactive_widgets']) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php mcms_nonce_field( 'remove-inactive-widgets', '_mcmsnonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
		...
		$myskin_sidebars[$sidebar] = $registered_sidebar;
	}
}
...
$sidebars_count = count( $myskin_sidebars );
...
<div id="widgets-right" class="mcms-clearfix<?php echo $single_sidebar_class; ?>">
<div class="sidebars-column-1">
<?php

foreach ( $myskin_sidebars as $sidebar => $registered_sidebar ) {
	$wrap_class = 'widgets-holder-wrap';
	if ( !empty( $registered_sidebar['class'] ) )
	...
	?>
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		<?php mcms_list_widget_controls( $sidebar, $registered_sidebar['name'] ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
	<?php
...
<form method="post">
<?php mcms_nonce_field( 'save-sidebar-widgets', '_mcmsnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
