...
if ( is_multisite() ) {
	add_filter( 'mcmsmu_signup_user_notification_email', 'admin_created_user_email' );
}
...
if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	...
	$user_details = null;
	$user_email = mcms_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			mcms_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		mcms_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
	...
	if ( $username != null && array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST[ 'noconfirmation' ] ) && current_user_can( 'manage_network_users' ) ) {
			...
			$newuser_key = mcms_generate_password( 20, false );
...
%4$s' );
			mcms_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), mcms_specialchars_decode( get_option( 'blogname' ) ) ), sprintf( $message, get_option( 'blogname' ), home_url(), mcms_specialchars_decode( translate_user_role( $role['name'] ) ), home_url( "/newbloguser/$newuser_key/" ) ) );
	...
	check_admin_referer( 'create-user', '_mcmsnonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = mcms_unslash( $_REQUEST['email'] );
		$user_details = mcmsmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_mcms_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			 ...
			 */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( mcms_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'mcmsmu_signup_user_notification', '__return_false' ); // Disable confirmation email
				add_filter( 'mcmsmu_welcome_user_notification', '__return_false' ); // Disable welcome email
			}
			mcmsmu_signup_user( $new_user_login, $new_user_email, array( 'add_to_blog' => get_current_blog_id(), 'new_role' => $_REQUEST['role'] ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && current_user_can( 'manage_network_users' ) ) {
				$key = $mcmsdb->get_var( $mcmsdb->prepare( "SELECT activation_key FROM {$mcmsdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				$new_user = mcmsmu_activate_signup( $key );
				if ( is_mcms_error( $new_user ) ) {
...
'content' => '<p>' . __('Here is a basic overview of the different user roles and the permissions associated with each one:') . '</p>' .
				'<ul>' .
				'<li>' . __('Seeders can read comments/comment/receive newsletters, etc. but cannot create regular site content.') . '</li>' .
				'<li>' . __('Feeders can write and manage their posts but not publish posts or upload media files.') . '</li>' .
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://dev.mandarincms.com/Users_Add_New_Screen">Documentation on Adding New Users</a>') . '</p>' .
    '<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
);

mcms_enqueue_script('mcms-ajax-response');
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! mcms_is_large_network( 'users' )
	...
	mcms_enqueue_script( 'user-suggest' );
}
	...
	if ( is_multisite() ) {
		...
		switch ( $_GET['update'] ) {
			...
			case "addexisting":
				...
				break;
			case "could_not_add":
				$add_user_errors = new MCMS_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case "created_could_not_add":
				$add_user_errors = new MCMS_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case "does_not_exist":
				$add_user_errors = new MCMS_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case "enter_email":
				$add_user_errors = new MCMS_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' == $_GET['update'] )
...
<?php if ( isset($add_user_errors) && is_mcms_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<?php mcms_nonce_field( 'add-user', '_mcmsnonce_add-user' ) ?>

<table class="form-table">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo $label; ?></label></th>
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="mcms-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e('Role'); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php mcms_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"<?php
	/** This action is documented in mcms-admin/user-new.php */
	do_action( 'user_new_form_tag' );
?>>
<input name="action" type="hidden" value="createuser" />
<?php mcms_nonce_field( 'create-user', '_mcmsnonce_create-user' ); ?>
<?php
// Load up the passed data, else set to a default.
$creating = isset( $_POST['createuser'] );

$new_user_login = $creating && isset( $_POST['user_login'] ) ? mcms_unslash( $_POST['user_login'] ) : '';
$new_user_firstname = $creating && isset( $_POST['first_name'] ) ? mcms_unslash( $_POST['first_name'] ) : '';
$new_user_lastname = $creating && isset( $_POST['last_name'] ) ? mcms_unslash( $_POST['last_name'] ) : '';
$new_user_email = $creating && isset( $_POST['email'] ) ? mcms_unslash( $_POST['email'] ) : '';
$new_user_uri = $creating && isset( $_POST['url'] ) ? mcms_unslash( $_POST['url'] ) : '';
$new_user_role = $creating && isset( $_POST['role'] ) ? mcms_unslash( $_POST['role'] ) : '';
$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
$new_user_ignore_pass = $creating && isset( $_POST['noconfirmation'] ) ? mcms_unslash( $_POST['noconfirmation'] ) : '';
...
<?php if ( !is_multisite() ) { ?>
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button mcms-generate-pw hide-if-no-js"><?php _e( 'Show password' ); ?></button>
			<div class="mcms-pwd hide-if-js">
				<?php $initial_password = mcms_generate_password( 24 ); ?>
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			mcms_dropdown_roles($new_user_role);
			?>
			</select>
		</td>
	</tr>
