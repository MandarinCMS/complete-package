 ...
 * @since 2.0.0
 *
 * @return int|MCMS_Error MCMS_Error or User ID.
 */
function add_user() {
	return edit_user();
}
 ...
 * @return int|MCMS_Error user id of the updated user
 */
function edit_user( $user_id = 0 ) {
	$mcms_roles = mcms_roles();
	$user = new stdClass;
	if ( $user_id ) {
		$update = true;
		$user->ID = (int) $user_id;
		$userdata = get_userdata( $user_id );
		$user->user_login = mcms_slash( $userdata->user_login );
	} else {
		$update = false;
	}
	...
	if ( isset( $_POST['role'] ) && current_user_can( 'edit_users' ) ) {
		...
		$potential_role = isset($mcms_roles->role_objects[$new_role]) ? $mcms_roles->role_objects[$new_role] : false;
		...
		if ( ! empty( $new_role ) && empty( $editable_roles[$new_role] ) )
			mcms_die( __( 'Sorry, you are not allowed to give users that role.' ), 403 );
	}

	if ( isset( $_POST['email'] ))
		$user->user_email = sanitize_text_field( mcms_unslash( $_POST['email'] ) );
	if ( isset( $_POST['url'] ) ) {
		if ( empty ( $_POST['url'] ) || $_POST['url'] == 'http://' ) {
			$user->user_url = '';
		} else {
			$user->user_url = esc_url_raw( $_POST['url'] );
			$protocols = implode( '|', array_map( 'preg_quote', mcms_allowed_protocols() ) );
	...
	foreach ( mcms_get_user_contact_methods( $user ) as $method => $name ) {
	...
	if ( false !== strpos( mcms_unslash( $pass1 ), "\\" ) ) {
	 ...
	 * @since 2.8.0
	 *
	 * @param MCMS_Error $errors MCMS_Error object (passed by reference).
	...
	if ( $update ) {
		$user_id = mcms_update_user( $user );
	} else {
		$user_id = mcms_insert_user( $user );
		...
		/**
		  ...
		  * @param string $notify  Type of notification that should happen. See mcms_send_new_user_notifications()
 ...
 * list of roles in the $mcms_roles object so that modules can remove
 ...
 * @return MCMS_User|bool MCMS_User object on success, false on failure.
 */
...
function get_users_drafts( $user_id ) {
	global $mcmsdb;
	$query = $mcmsdb->prepare("SELECT ID, post_title FROM $mcmsdb->posts WHERE post_type = 'post' AND post_status = 'draft' AND post_author = %d ORDER BY post_modified DESC", $user_id);
	...
	return $mcmsdb->get_results( $query );
}
 ...
 */
function mcms_delete_user( $id, $reassign = null ) {
	global $mcmsdb;
	...
	if ( null === $reassign ) {
		...
		$post_ids = $mcmsdb->get_col( $mcmsdb->prepare( "SELECT ID FROM $mcmsdb->posts WHERE post_author = %d AND post_type IN ('$post_types_to_delete')", $id ) );
		if ( $post_ids ) {
		...
		// Clean links
		$link_ids = $mcmsdb->get_col( $mcmsdb->prepare("SELECT link_id FROM $mcmsdb->links WHERE link_owner = %d", $id) );
		...
		$post_ids = $mcmsdb->get_col( $mcmsdb->prepare( "SELECT ID FROM $mcmsdb->posts WHERE post_author = %d", $id ) );
		$mcmsdb->update( $mcmsdb->posts, array('post_author' => $reassign), array('post_author' => $id) );
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id )
				clean_post_cache( $post_id );
		}
		$link_ids = $mcmsdb->get_col( $mcmsdb->prepare("SELECT link_id FROM $mcmsdb->links WHERE link_owner = %d", $id) );
		$mcmsdb->update( $mcmsdb->links, array('link_owner' => $reassign), array('link_owner' => $id) );
		if ( ! empty( $link_ids ) ) {
			foreach ( $link_ids as $link_id )
	...
	if ( is_multisite() ) {
		...
		$meta = $mcmsdb->get_col( $mcmsdb->prepare( "SELECT umeta_id FROM $mcmsdb->usermeta WHERE user_id = %d", $id ) );
		foreach ( $meta as $mid )
			delete_metadata_by_mid( 'user', $mid );

		$mcmsdb->delete( $mcmsdb->users, array( 'ID' => $id ) );
	}
...
%%s' ), mcms_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), home_url(), mcms_specialchars_decode( translate_user_role( $role['name'] ) ) );
}
 ...
 * @return bool|MCMS_Error Returns true/false based on the success of sending the email, or a MCMS_Error object.
 */
function _mcms_privacy_resend_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = get_post( $request_id );

	if ( ! $request || 'user_request' !== $request->post_type ) {
		return new MCMS_Error( 'privacy_request_error', __( 'Invalid request.' ) );
	}

	$result = mcms_send_user_request( $request_id );

	if ( is_mcms_error( $result ) ) {
		return $result;
	} elseif ( ! $result ) {
		return new MCMS_Error( 'privacy_request_error', __( 'Unable to initiate confirmation request.' ) );
	}
 ...
 *
 * @param  int          $request_id Request ID.
 * @return int|MCMS_Error $request    Request ID on success or MCMS_Error.
 */
function _mcms_privacy_completed_request( $request_id ) {
	$request_id   = absint( $request_id );
	...
	update_post_meta( $request_id, '_mcms_user_request_completed_timestamp', time() );

	$request = mcms_update_post( array(
		'ID'          => $request_id,
 ...
 * @access private
 */
function _mcms_personal_data_handle_actions() {
	if ( isset( $_POST['privacy_action_email_retry'] ) ) { // MCMSCS: input var ok.
		check_admin_referer( 'bulk-privacy_requests' );

		$request_id = absint( current( array_keys( (array) mcms_unslash( $_POST['privacy_action_email_retry'] ) ) ) ); // MCMSCS: input var ok, sanitization ok.
		$result     = _mcms_privacy_resend_request( $request_id );
		...
		$action = isset( $_POST['action'] ) ? sanitize_key( mcms_unslash( $_POST['action'] ) ) : ''; // MCMSCS: input var ok, CSRF ok.
		...
		switch ( $action ) {
			...
			case 'add_remove_personal_data_request':
				...
				if ( ! isset( $_POST['type_of_action'], $_POST['username_or_email_to_export'] ) ) { // MCMSCS: input var ok.
					add_settings_error(
						'action_type',
						'action_type',
						__( 'Invalid action.' ),
				...
				$action_type               = sanitize_text_field( mcms_unslash( $_POST['type_of_action'] ) ); // MCMSCS: input var ok.
				$username_or_email_address = sanitize_text_field( mcms_unslash( $_POST['username_or_email_to_export'] ) ); // MCMSCS: input var ok.
				$email_address             = '';

				if ( ! in_array( $action_type, _mcms_privacy_action_request_types(), true ) ) {
					add_settings_error(
						'action_type',
						'action_type',
						__( 'Invalid action.' ),
				...
				$request_id = mcms_create_user_request( $email_address, $action_type );

				if ( is_mcms_error( $request_id ) ) {
					add_settings_error(
						'username_or_email_to_export',
						'username_or_email_to_export',
				...
				mcms_send_user_request( $request_id );
 ...
 * @access private
 */
function _mcms_personal_data_cleanup_requests() {
	/** This filter is documented in mcms-roots/user.php */
 ...
 * @access private
 */
function _mcms_personal_data_export_page() {
	if ( ! current_user_can( 'export_others_personal_data' ) ) {
		mcms_die( __( 'Sorry, you are not allowed to export personal data on this site.' ) );
	}

	_mcms_personal_data_handle_actions();
	_mcms_personal_data_cleanup_requests();

	// "Borrow" xfn.js for now so we don't have to create new files.
	mcms_enqueue_script( 'xfn' );

	$requests_table = new MCMS_Privacy_Data_Export_Requests_Table( array(
	...
	<div class="wrap nosubsub">
		...
		<form method="post" class="mcms-privacy-request-form">
			...
			<div class="mcms-privacy-request-form-field">
			...
			<?php mcms_nonce_field( 'personal-data-request' ); ?>
		...
		<form class="search-form mcms-clearfix">
 ...
 * @access private
 */
function _mcms_personal_data_removal_page() {
	/*
	...
	if ( ! current_user_can( 'erase_others_personal_data' ) || ! current_user_can( 'delete_users' ) ) {
		mcms_die( __( 'Sorry, you are not allowed to erase data on this site.' ) );
	}
	...
	$requests_table = new MCMS_Privacy_Data_Removal_Requests_Table( array(
		'plural'   => 'privacy_requests',
		'singular' => 'privacy_request',
	) );
 ...
 * @since 4.9.6
 *
 * @see mcms_privacy_personal_data_erasure_page
 *
 ...
 */
function mcms_privacy_process_personal_data_erasure_page( $response, $eraser_index, $email_address, $page, $request_id ) {
	/*
	...
	if ( ! $request || 'remove_personal_data' !== $request->action_name ) {
		mcms_send_json_error( __( 'Invalid request ID when processing eraser data.' ) );
	}
	 ...
	 */
	do_action( 'mcms_privacy_personal_data_erased', $request_id );
 ...
 * @access private
 */
function _mcms_privacy_hook_requests_page() {
	add_submenu_page( 'tools.php', __( 'Export Personal Data' ), __( 'Export Personal Data' ), 'export_others_personal_data', 'export_personal_data', '_mcms_personal_data_export_page' );
	add_submenu_page( 'tools.php', __( 'Erase Personal Data' ), __( 'Erase Personal Data' ), 'erase_others_personal_data', 'remove_personal_data', '_mcms_personal_data_removal_page' );
}
...
/**
 * MCMS_Privacy_Requests_Table class.
 *
 * @since 4.9.6
 */
abstract class MCMS_Privacy_Requests_Table extends MCMS_List_Table {

	/**
	 * Action name for the requests this table will work with. Classes
	 * which inherit from MCMS_Privacy_Requests_Table should define this.
	 *
	...
	protected function get_request_counts() {
		...
		$results = (array) $mcmsdb->get_results( $mcmsdb->prepare( $query, $this->post_type, $this->request_type ), ARRAY_A );
	...
	protected function get_views() {
		...
		$statuses       = _mcms_privacy_statuses();
		$views          = array();
	...
	public function process_bulk_action() {
		...
		$request_ids = isset( $_REQUEST['request_id'] ) ? mcms_parse_id_list( mcms_unslash( $_REQUEST['request_id'] ) ) : array(); // MCMSCS: input var ok, CSRF ok.
		$count       = 0;
		...
		switch ( $action ) {
			case 'delete':
				foreach ( $request_ids as $request_id ) {
					if ( mcms_delete_post( $request_id, true ) ) {
						$count ++;
					}
				}
			...
			case 'resend':
				foreach ( $request_ids as $request_id ) {
					$resend = _mcms_privacy_resend_request( $request_id );

					if ( $resend && ! is_mcms_error( $resend ) ) {
						$count++;
					}
				}
	...
	public function prepare_items() {
		...
		$requests_query = new MCMS_Query( $args );
		$requests       = $requests_query->posts;

		foreach ( $requests as $request ) {
			$this->items[] = mcms_get_user_request_data( $request->ID );
		}
	 ...
	 * @since 4.9.6
	 *
	 * @param MCMS_User_Request $item Item being shown.
	 ...
	 * @since 4.9.6
	 *
	 * @param MCMS_User_Request $item The current item.
	 */
...
/**
 * MCMS_Privacy_Data_Export_Requests_Table class.
 *
 * @since 4.9.6
 */
class MCMS_Privacy_Data_Export_Requests_Table extends MCMS_Privacy_Requests_Table {
	/**
	...
	public function column_email( $item ) {
		...
		$request_id      = $item->ID;
		$nonce           = mcms_create_nonce( 'mcms-privacy-export-personal-data-' . $request_id );
	...
	public function column_next_steps( $item ) {
		...
		switch ( $status ) {
			...
			case 'request-failed':
				...
				break;
			case 'request-completed':
				echo '<a href="' . esc_url( mcms_nonce_url( add_query_arg( array(
					'action'     => 'delete',
...
/**
 * MCMS_Privacy_Data_Removal_Requests_Table class.
 *
 * @since 4.9.6
 */
class MCMS_Privacy_Data_Removal_Requests_Table extends MCMS_Privacy_Requests_Table {
	/**
	 * Action name for the requests this table will work with.
	 *
	 * @since 4.9.6
	...
	public function column_email( $item ) {
		...
		if ( 'request-confirmed' !== $status ) {
			...
			$request_id    = $item->ID;
			$nonce         = mcms_create_nonce( 'mcms-privacy-erase-personal-data-' . $request_id );
