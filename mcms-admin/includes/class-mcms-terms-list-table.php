<?php
/**
 * List Table API: MCMS_Terms_List_Table class
 *
 * @package MandarinCMS
 ...
 * @see MCMS_List_Table
 */
class MCMS_Terms_List_Table extends MCMS_List_Table {
	...
	public function prepare_items() {
		...
		$search = !empty( $_REQUEST['s'] ) ? trim( mcms_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( !empty( $_REQUEST['orderby'] ) )
			$args['orderby'] = trim( mcms_unslash( $_REQUEST['orderby'] ) );

		if ( !empty( $_REQUEST['order'] ) )
			$args['order'] = trim( mcms_unslash( $_REQUEST['order'] ) );

		$this->callback_args = $args;

		$this->set_pagination_args( array(
			'total_items' => mcms_count_terms( $this->screen->taxonomy, compact( 'search' ) ),
	...
	public function display_rows_or_placeholder() {
		...
		$args = mcms_parse_args( $this->callback_args, array(
			'page' => 1,
			'number' => 20,
			'search' => '',
			'hide_empty' => 0
	...
	/**
	 * @global string $taxonomy
	 * @param MCMS_Term $tag Term object.
	 * @param int $level
	 */
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see MCMS_Terms_List_Table::column_name()
		 *
		...
		$uri = mcms_doing_ajax() ? mcms_get_referer() : $_SERVER['REQUEST_URI'];

		$edit_link = add_query_arg(
			'mcms_http_referer',
			urlencode( mcms_unslash( $uri ) ),
	 ...
	 * @since 4.3.0
	 *
	 * @param MCMS_Term $tag         Tag being acted upon.
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_term', $tag->term_id ) ) {
			$actions['delete'] = sprintf(
				...
				mcms_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ),
				/* translators: %s: taxonomy term name */
...
?>
	...
	?>
		...
		<div class="inline-edit-save submit">
			...
			<?php mcms_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
