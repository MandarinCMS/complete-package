<?php
/**
 * MandarinCMS Administration Media API.
 *
 * @package MandarinCMS
...
function update_gallery_tab($tabs) {
	...
	if ( $post_id )
		$attachments = intval( $mcmsdb->get_var( $mcmsdb->prepare( "SELECT count(*) FROM $mcmsdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) ) );
...
function the_media_upload_tabs() {
	...
	if ( !empty($tabs) ) {
		...
		if ( isset($redir_tab) && array_key_exists($redir_tab, $tabs) ) {
			...
			/** This filter is documented in mcms-admin/media-upload.php */
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	...
	if ( $rel ) {
		if ( is_string( $rel ) ) {
			...
			$rel = ' rel="attachment mcms-att-' . intval( $id ) . '"';
		}
	} else {
		$rel = '';
	}
...
win.send_to_editor( <?php echo mcms_json_encode( $html ); ?> );
</script>
<?php
	exit;
}
 ...
 * @param array  $overrides Override the mcms_handle_upload() behavior. Optional.
 * @return int|MCMS_Error ID of the attachment or a MCMS_Error object on failure.
 */
function media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	$file = mcms_handle_upload($_FILES[$file_id], $overrides, $time);

	if ( isset($file['error']) )
		return new MCMS_Error( 'upload_error', $file['error'] );

	$name = $_FILES[$file_id]['name'];
	$ext  = pathinfo( $name, PATHINFO_EXTENSION );
	$name = mcms_basename( $name, ".$ext" );
	...
	if ( preg_match( '#^audio#', $type ) ) {
		$meta = mcms_read_audio_metadata( $file );
	...
	} elseif ( 0 === strpos( $type, 'image/' ) && $image_meta = mcms_read_image_metadata( $file ) ) {
	...
	// Save the data
	$id = mcms_insert_attachment( $attachment, $file, $post_id, true );
	if ( !is_mcms_error($id) ) {
		mcms_update_attachment_metadata( $id, mcms_generate_attachment_metadata( $id, $file ) );
	}
 ...
 * @return int|object The ID of the attachment or a MCMS_Error on failure.
 */
function media_handle_sideload( $file_array, $post_id, $desc = null, $post_data = array() ) {
	...
	$file = mcms_handle_sideload( $file_array, $overrides, $time );
	if ( isset($file['error']) )
		return new MCMS_Error( 'upload_error', $file['error'] );
	...
	// Use image exif/iptc data for title and caption defaults if possible.
	if ( $image_meta = mcms_read_image_metadata( $file ) ) {
		if ( trim( $image_meta['title'] ) && ! is_numeric( sanitize_title( $image_meta['title'] ) ) )
			$title = $image_meta['title'];
		if ( trim( $image_meta['caption'] ) )
	...
	$id = mcms_insert_attachment($attachment, $file, $post_id);
	if ( !is_mcms_error($id) )
		mcms_update_attachment_metadata( $id, mcms_generate_attachment_metadata( $id, $file ) );
 ...
 */
function mcms_iframe($content_func /* ... */) {
	_mcms_admin_html_begin();
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('MandarinCMS'); ?></title>
<?php

mcms_enqueue_style( 'colors' );
// Check callback name for 'media'
if ( ( is_array( $content_func ) && ! empty( $content_func[1] ) && 0 === strpos( (string) $content_func[1], 'media' ) )
	|| ( ! is_array( $content_func ) && 0 === strpos( $content_func, 'media' ) ) )
	mcms_enqueue_style( 'deprecated-media' );
mcms_enqueue_style( 'ie' );
?>
<script type="text/javascript">
addLoadEvent = function(func){if(typeof jQuery!="undefined")jQuery(document).ready(func);else if(typeof mcmsOnload!='function'){mcmsOnload=func;}else{var oldonload=mcmsOnload;mcmsOnload=function(){oldonload();func();}}};
var ajaxurl = '<?php echo admin_url( 'admin-ajax.php', 'relative' ); ?>', pagenow = 'media-upload-popup', adminpage = 'media-upload-popup',
isRtl = <?php echo (int) is_rtl(); ?>;
</script>
<?php
	/** This action is documented in mcms-admin/admin-header.php */
...
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?> class="mcms-core-ui no-js">
<script type="text/javascript">
...
<?php
	...
	/** This action is documented in mcms-admin/admin-footer.php */
...
function media_buttons($editor_id = 'content') {
	...
	mcms_enqueue_media( array(
		'post' => $post
	) );

	$img = '<span class="mcms-media-buttons-icon"></span> ';
	...
	if ( $legacy_filter ) {
		// #MCMS22559. Close <a> if a module started by closing <a> to open their own <a> tag.
 ...
 * @since 2.5.0
 *
 * @return mixed void|object MCMS_Error on failure
 */
function media_upload_form_handler() {
	...
	if ( !empty($_POST['attachments']) ) foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see mcms_get_attachment_metadata()
		 *
		...
		if ( isset($attachment['image_alt']) ) {
			$image_alt = mcms_unslash( $attachment['image_alt'] );
			if ( $image_alt != get_post_meta($attachment_id, '_mcms_attachment_image_alt', true) ) {
				$image_alt = mcms_strip_all_tags( $image_alt, true );

				// Update_meta expects slashed.
				update_post_meta( $attachment_id, '_mcms_attachment_image_alt', mcms_slash( $image_alt ) );
			}
		}
		...
		foreach ( get_attachment_taxonomies($post) as $t ) {
			if ( isset($attachment[$t]) )
				mcms_set_object_terms($attachment_id, array_map('trim', preg_split('/,+/', $attachment[$t])), $t, false);
		}
	}
	...
	if ( isset($send_id) ) {
		$attachment = mcms_unslash( $_POST['attachments'][$send_id] );

		$html = isset( $attachment['post_title'] ) ? $attachment['post_title'] : '';
		if ( !empty($attachment['url']) ) {
			$rel = '';
			if ( strpos($attachment['url'], 'attachment_id') || get_attachment_link($send_id) == $attachment['url'] )
				$rel = " rel='attachment mcms-att-" . esc_attr($send_id) . "'";
 ...
 * @return null|string
 */
function mcms_media_upload_handler() {
	$errors = array();
	$id = 0;
	...
	if ( !empty($_POST['insertonlybutton']) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' != $_POST['media_type'] ) {
			$title = esc_html( mcms_unslash( $_POST['title'] ) );
			if ( empty( $title ) )
			...
			$align = '';
			$alt = esc_attr( mcms_unslash( $_POST['alt'] ) );
			if ( isset($_POST['align']) ) {
				$align = esc_attr( mcms_unslash( $_POST['align'] ) );
	...
	if ( isset( $_POST['save'] ) ) {
		...
		mcms_enqueue_script( 'admin-gallery' );
 		return mcms_iframe( 'media_upload_gallery_form', $errors );
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' ) {
		...
		if ( isset( $_GET['type'] ) && in_array( $_GET['type'], array( 'video', 'audio', 'file' ) ) )
			$type = $_GET['type'];
		return mcms_iframe( 'media_upload_type_url_form', $type, $errors, $id );
	}

	return mcms_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|MCMS_Error Populated HTML img tag on success, MCMS_Error object otherwise.
 */
function media_sideload_image( $file, $post_id, $desc = null, $return = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( ! $matches ) {
			return new MCMS_Error( 'image_sideload_failed', __( 'Invalid image URL' ) );
		}
		...
		if ( is_mcms_error( $file_array['tmp_name'] ) ) {
		...
		$src = mcms_get_attachment_url( $id );
	}
	...
	if ( ! empty( $src ) ) {
		...
		return new MCMS_Error( 'image_sideload_failed' );
	}
}
...
function media_upload_library() {
	...
	return mcms_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields($post, $url_type = '') {
	...
	$file = mcms_get_attachment_url($post->ID);
 ...
 * @since 3.4.0
 *
 * @param MCMS_Post $edit_post Attachment MCMS_Post object.
 * @return string HTML markup for the textarea element.
 */
function mcms_caption_input_textarea($edit_post) {
 ...
 * @param MCMS_Post $post        The MCMS_Post attachment object.
 ...
 * @since 2.5.0
 *
 * @param array $post       The MCMS_Post attachment object converted to an array.
 * @param array $attachment An array of attachment metadata.
...
function image_attachment_fields_to_save( $post, $attachment ) {
	if ( substr( $post['post_mime_type'], 0, 5 ) == 'image' ) {
		if ( strlen( trim( $post['post_title'] ) ) == 0 ) {
			...
			$post['post_title'] = preg_replace( '/\.\w+$/', '', mcms_basename( $attachment_url ) );
...
function get_attachment_fields_to_edit($post, $errors = null) {
	if ( is_int($post) )
		$post = get_post($post);
	if ( is_array($post) )
		$post = new MCMS_Post( (object) $post );

	$image_url = mcms_get_attachment_url($post->ID);
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label'      => __('Caption'),
			'input'      => 'html',
			'html'       => mcms_caption_input_textarea($edit_post)
		),
		'post_content' => array(
		...
		'image_url'	=> array(
			...
			'value'      => mcms_get_attachment_url($post->ID),
	...
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		$alt = get_post_meta($post->ID, '_mcms_attachment_image_alt', true);
		if ( empty($alt) )
			$alt = '';
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array($GLOBALS['mcms_the_query']->posts) )
			foreach ( $GLOBALS['mcms_the_query']->posts as $attachment )
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( ( $attachment_id = intval( $attachment_id ) ) && $thumb_url = mcms_get_attachment_image_src( $attachment_id, 'thumbnail', true ) )
	...
	$filename = esc_html( mcms_basename( $file ) );
	$title = esc_attr( $post->post_title );

	$post_mime_types = get_post_mime_types();
	$keys = array_keys( mcms_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	$type = reset( $keys );
	...
	$display_title = $r['show_title'] ? "<div class='filename new'><span class='title'>" . mcms_html_excerpt( $display_title, 60, '&hellip;' ) . "</span></div>" : '';
	...
	$media_dims = '';
	$meta = mcms_get_attachment_metadata( $post->ID );
	...
	$image_edit_button = '';
	if ( mcms_attachment_is_image( $post->ID ) && mcms_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
		$nonce = mcms_create_nonce( "image_editor-$post->ID" );
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . mcms_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete-permanently'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( !MEDIA_TRASH ) {
			...
			'<p>' . sprintf( __( 'You are about to delete %s.' ), '<strong>' . $filename . '</strong>' ) . "</p>
			 <a href='" . mcms_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
			 <a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . "</a>
			 </div>";
		} else {
			$delete = "<a href='" . mcms_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . mcms_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-post_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . "</a>";
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' == $type && $calling_post_id && current_myskin_supports( 'post-thumbnails', get_post_type( $calling_post_id ) )
		...
		$ajax_nonce = mcms_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail = "<a class='mcms-post-thumbnail' id='mcms-post-thumbnail-" . $attachment_id . "' href='#' onclick='MCMSSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . "</a>";
	}
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf( __('The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.'), 'https://apps.mandarincms.com/' ) . '</p>';
		return;
	}
...
<div id="media-upload-error"><?php
	...
	if (isset($errors['upload_error']) && is_mcms_error($errors['upload_error']))
...
$post_params = array(
	"post_id" => $post_id,
	"_mcmsnonce" => mcms_create_nonce('media-form'),
	"type" => $_type,
	"tab" => $_tab,
	"short" => "1",
);
...
mcmsUploaderInit = <?php echo mcms_json_encode( $plupload_init ); ?>;
</script>
...
<?php mcms_nonce_field('media-form'); ?>
 ...
 */
echo apply_filters( 'type_url_form_media', mcms_media_insert_url_form( $type ) );
?>
</div>
</div>
</form>
...
<p class="ml-submit">
<input type="button" class="button" style="display:none;" onMouseDown="mcmsgallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e( 'Insert gallery' ); ?>" />
<input type="button" class="button" style="display:none;" onMouseDown="mcmsgallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e( 'Update gallery settings' ); ?>" />
</p>
</div>
</form>
<?php
...
function media_upload_library_form($errors) {
	global $mcmsdb, $mcms_query, $mcms_locale, $type, $tab, $post_mime_types;
	...
	list($post_mime_types, $avail_post_mime_types) = mcms_edit_attachments_query( $q );
...
<?php
$type_links = array();
$_num_posts = (array) mcms_count_attachments();
$matches = mcms_match_mime_types(array_keys($post_mime_types), array_keys($_num_posts));
...
if ( empty($_GET['post_mime_type']) && !empty($num_posts[$type]) ) {
	...
	list($post_mime_types, $avail_post_mime_types) = mcms_edit_attachments_query();
}
...
foreach ( $post_mime_types as $mime_type => $label ) {
	...
	if ( !mcms_match_mime_types($mime_type, $avail_post_mime_types) )
		continue;

	if ( isset($_GET['post_mime_type']) && mcms_match_mime_types($mime_type, $_GET['post_mime_type']) )
		$class = ' class="current"';
...
$page_links = paginate_links( array(
	...
	'prev_text' => __('&laquo;'),
	'next_text' => __('&raquo;'),
	'total' => ceil($mcms_query->found_posts / 10),
	'current' => $q['paged'],
));
...
$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $mcmsdb->posts WHERE post_type = 'attachment' ORDER BY post_date DESC";

$arc_result = $mcmsdb->get_results( $arc_query );
...
foreach ($arc_result as $arc_row) {
	...
	echo esc_html( $mcms_locale->get_month($arc_row->mmonth) . " $arc_row->yyear" );
	echo "</option>\n";
}
?>
</select>
 ...
 */
function mcms_media_insert_url_form( $default_view = 'image' ) {
	/** This filter is documented in mcms-admin/includes/media.php */
	if ( ! apply_filters( 'disable_captions', '' ) ) {
		$caption = '
		<tr class="image-only">
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
	   <?php _e('You are using the browser&#8217;s built-in file uploader. The MandarinCMS uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.'); ?>
	</p>
	<?php
}
 ...
 * @since 3.5.0
 *
 * @param MCMS_Post $post A post object.
 */
function edit_form_image_editor( $post ) {
	$open = isset( $_GET['image-editor'] );
	if ( $open )
		require_once BASED_TREE_URI . 'mcms-admin/includes/image-edit.php';

	$thumb_url = false;
	if ( $attachment_id = intval( $post->ID ) )
		$thumb_url = mcms_get_attachment_image_src( $attachment_id, array( 900, 450 ), true );

	$alt_text = get_post_meta( $post->ID, '_mcms_attachment_image_alt', true );

	$att_url = mcms_get_attachment_url( $post->ID ); ?>
	<div class="mcms_attachment_holder mcms-clearfix">
	<?php
	if ( mcms_attachment_is_image( $post->ID ) ) :
		$image_edit_button = '';
		if ( mcms_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
			$nonce = mcms_create_nonce( "image_editor-$post->ID" );
			$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <span class='spinner'></span>";
		}
	?>
		...
		<div<?php if ( $open ) echo ' style="display:none"'; ?> class="mcms_attachment_image mcms-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		...
		<div<?php if ( ! $open ) echo ' style="display:none"'; ?> class="image-editor" id="image-editor-<?php echo $attachment_id; ?>">
			<?php if ( $open ) mcms_image_editor( $attachment_id ); ?>
		</div>
	<?php
	elseif ( $attachment_id && mcms_attachment_is( 'audio', $post ) ):

		mcms_maybe_generate_attachment_metadata( $post );

		echo mcms_audio_shortcode( array( 'src' => $att_url ) );

	elseif ( $attachment_id && mcms_attachment_is( 'video', $post ) ):
		...
		if ( ! empty( $thumb_id ) ) {
			$attr['poster'] = mcms_get_attachment_url( $thumb_id );
		}

		echo mcms_video_shortcode( $attr );

	elseif ( isset( $thumb_url[0] ) ):

		?>
		<div class="mcms_attachment_image mcms-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		 ...
		 * @param MCMS_Post $post A post object.
		 */
		do_action( 'mcms_edit_form_attachment_display', $post );

	endif; ?>
	</div>
	<div class="mcms_attachment_details edit-form-section">
		<p>
	...
	<?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?>
		<p>
			...
			<input type="text" class="widefat" name="_mcms_attachment_image_alt" id="attachment_alt" value="<?php echo esc_attr( $alt_text ); ?>" />
		</p>
	<?php endif; ?>
	...
	<?php mcms_editor( $post->post_content, 'attachment_content', $editor_args ); ?>
...
?>
	...
	if ( preg_match( '#^(audio|video)/#', $post->post_mime_type ) ) {
		 ...
		 * @param MCMS_Post $post   MCMS_Post object for the current attachment.
		 */
 ...
 */
function mcms_add_id3_tag_data( &$metadata, $data ) {
	foreach ( array( 'id3v2', 'id3v1' ) as $version ) {
		if ( ! empty( $data[$version]['comments'] ) ) {
			foreach ( $data[$version]['comments'] as $key => $list ) {
				if ( 'length' !== $key && ! empty( $list ) ) {
					$metadata[$key] = mcms_kses_post( reset( $list ) );
 ...
 */
function mcms_read_video_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	...
	if ( ! class_exists( 'getID3', false ) ) {
		require( BASED_TREE_URI . MCMSINC . '/ID3/getid3.php' );
	}
	$id3 = new getID3();
	...
	if ( empty( $metadata['created_timestamp'] ) ) {
		$created_timestamp = mcms_get_media_creation_timestamp( $data );
	...
	mcms_add_id3_tag_data( $metadata, $data );
	 ...
	 */
	return apply_filters( 'mcms_read_video_metadata', $metadata, $file, $file_format, $data );
}
 ...
 * @return array|bool Returns array of metadata, if found.
 */
function mcms_read_audio_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	$metadata = array();
 ...
 */
function mcms_get_media_creation_timestamp( $metadata ) {
	$creation_date = false;
 ...
 *                          Default 'attach'.
 */
function mcms_media_attach_action( $parent_id, $action = 'attach' ) {
	global $mcmsdb;
	...
	if ( ! empty( $ids ) ) {
		...
		if ( 'attach' === $action ) {
			$result = $mcmsdb->query( $mcmsdb->prepare( "UPDATE $mcmsdb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $ids_string )", $parent_id ) );
		} else {
			$result = $mcmsdb->query( "UPDATE $mcmsdb->posts SET post_parent = 0 WHERE post_type = 'attachment' AND ID IN ( $ids_string )" );
		}
