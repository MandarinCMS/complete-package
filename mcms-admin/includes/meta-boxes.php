...
?>
<a class="preview button" href="<?php echo $preview_link; ?>" target="mcms-preview-<?php echo (int) $post->ID; ?>" id="post-preview"><?php echo $preview_button; ?></a>
<input type="hidden" name="mcms-preview" id="mcms-preview" value="" />
</div>
 ...
 * @since 4.4.0
 *
 * @param MCMS_Post $post MCMS_Post object for the current post.
 */
...
<div hidden class="misc-pub-section misc-pub-post-status">
...
<?php _e('Privacy:'); ?> <span id="post-visibility-display"><?php
...
<div hidden class="misc-pub-section misc-pub-revisions">
	<?php
...
if ( $can_publish ) : // Feeders don't get to choose the date of publish ?>
<div hidden class="misc-pub-section curtime misc-pub-curtime">
	<span id="timestamp">
 ...
 *
 * @param MCMS_Post|null $post MCMS_Post object for the current post on Edit Post screen,
...
function post_format_meta_box( $post, $box ) {
	if ( current_myskin_supports( 'post-formats' ) && post_type_supports( $post->post_type, 'post-formats' ) ) :
	$post_formats = get_myskin_support( 'post-formats' );

	if ( is_array( $post_formats[0] ) ) :
		$post_format = get_post_format( $post->ID );
		if ( !$post_format )
			$post_format = '0';
		// Add in the current one if it isn't there yet, in case the current myskin doesn't support it
...
<div class="tagsdiv" id="<?php echo $tax_name; ?>">
	...
	<div class="ajaxtag hide-if-no-js">
		...
		<p><input data-mcms-taxonomy="<?php echo $tax_name; ?>" type="text" id="new-tag-<?php echo $tax_name; ?>" name="newtag[<?php echo $tax_name; ?>]" class="newtag form-input-tip" size="16" autocomplete="off" aria-describedby="new-tag-<?php echo $tax_name; ?>-desc" value="" />
	...
	<div id="taxonomy-<?php echo $tax_name; ?>" class="categorydiv">
		...
		<div id="<?php echo $tax_name; ?>-pop" class="tabs-panel" style="display: none;">
			<ul id="<?php echo $tax_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				<?php $popular_ids = mcms_popular_terms_checklist( $tax_name ); ?>
			</ul>
		</div>
		...
		<div id="<?php echo $tax_name; ?>-all" class="tabs-panel">
			...
			?>
			<ul id="<?php echo $tax_name; ?>checklist" data-mcms-lists="list:<?php echo $tax_name; ?>" class="categorychecklist form-no-clear">
				<?php mcms_terms_checklist( $post->ID, array( 'taxonomy' => $tax_name, 'popular_cats' => $popular_ids ) ); ?>
			</ul>
		</div>
	<?php if ( current_user_can( $taxonomy->cap->edit_terms ) ) : ?>
			<div id="<?php echo $tax_name; ?>-adder" class="mcms-hidden-children">
				...
				<p id="<?php echo $tax_name; ?>-add" class="category-add mcms-hidden-child">
					...
					mcms_dropdown_categories( $parent_dropdown_args );
					?>
					<input type="button" id="<?php echo $tax_name; ?>-add-submit" data-mcms-lists="add:<?php echo $tax_name; ?>checklist:<?php echo $tax_name; ?>-add" class="button category-add-submit" value="<?php echo esc_attr( $taxonomy->labels->add_new_item ); ?>" />
					<?php mcms_nonce_field( 'add-' . $tax_name, '_ajax_nonce-add-' . $tax_name, false ); ?>
...
<p><?php
	printf(
		...
		__( 'Excerpts are optional hand-crafted summaries of your content that can be used in your myskin. <a href="%s">Learn more about manual excerpts</a>.' ),
		__( 'https://dev.mandarincms.com/Excerpt' )
	);
?></p>
<?php
}
...
<p><?php
	printf(
		/* translators: %s: Codex URL */
		__( 'Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. If you link other MandarinCMS sites, they&#8217;ll be notified automatically using <a href="%s">pingbacks</a>, no other action necessary.' ),
		__( 'https://dev.mandarincms.com/Introduction_to_Blogging#Managing_Comments' )
	);
?></p>
<?php
if ( ! empty($pings) )
...
<p><?php
	printf(
		/* translators: %s: Codex URL */
		__( 'Custom fields can be used to add extra metadata to a post that you can <a href="%s">use in your myskin</a>.' ),
		__( 'https://dev.mandarincms.com/Using_Custom_Fields' )
	);
?></p>
<?php
}
...
<p class="meta-options">
	 ...
	 * @since 3.1.0
	 *
	 * @param MCMS_Post $post MCMS_Post object of the current post.
	 */
...
function post_comment_meta_box( $post ) {
	mcms_nonce_field( 'get-comments', 'add_comment_nonce', false );
	?>
...
function page_attributes_meta_box($post) {
	if ( is_post_type_hierarchical( $post->post_type ) ) :
		 ...
		 * @since 3.3.0
		 *
		 * @see mcms_dropdown_pages()
		 *
		 * @param array   $dropdown_args Array of arguments used to generate the pages drop-down.
		 * @param MCMS_Post $post          The current post.
		 */
		$dropdown_args = apply_filters( 'page_attributes_dropdown_pages_args', $dropdown_args, $post );
		$pages = mcms_dropdown_pages( $dropdown_args );
		if ( ! empty($pages) ) :
?>
...
<div id="major-publishing-actions">
<?php
/** This action is documented in mcms-admin/includes/meta-boxes.php */
do_action( 'post_submitbox_start', null );
?>
<div id="delete-action">
<?php
if ( !empty($_GET['action']) && 'edit' == $_GET['action'] && current_user_can('manage_links') ) { ?>
	<a class="submitdelete deletion" href="<?php echo mcms_nonce_url("link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id); ?>" onclick="if ( confirm('<?php echo esc_js(sprintf(__("You are about to delete this link '%s'\n  'Cancel' to stop, 'OK' to delete."), $link->link_name )); ?>') ) {return true;}return false;"><?php _e('Delete'); ?></a>
<?php } ?>
</div>
...
<div id="taxonomy-linkcategory" class="categorydiv">
	...
	<div id="categories-all" class="tabs-panel">
		<ul id="categorychecklist" data-mcms-lists="list:category" class="categorychecklist form-no-clear">
			<?php
			if ( isset($link->link_id) )
				mcms_link_category_checklist($link->link_id);
			else
				mcms_link_category_checklist();
			?>
		</ul>
	</div>
	...
	<div id="categories-pop" class="tabs-panel" style="display: none;">
		<ul id="categorychecklist-pop" class="categorychecklist form-no-clear">
			<?php mcms_popular_terms_checklist('link_category'); ?>
		</ul>
	</div>

	<div id="category-adder" class="mcms-hidden-children">
		<a id="category-add-toggle" href="#category-add" class="taxonomy-add-new"><?php _e( '+ Add New Category' ); ?></a>
		<p id="link-category-add" class="mcms-hidden-child">
			<label class="screen-reader-text" for="newcat"><?php _e( '+ Add New Category' ); ?></label>
			<input type="text" name="newcat" id="newcat" class="form-required form-input-tip" value="<?php esc_attr_e( 'New category name' ); ?>" aria-required="true" />
			<input type="button" id="link-category-add-submit" data-mcms-lists="add:categorychecklist:link-category-add" class="button" value="<?php esc_attr_e( 'Add' ); ?>" />
			<?php mcms_nonce_field( 'add-link-category', '_ajax_nonce', false ); ?>
...
function post_thumbnail_meta_box( $post ) {
	...
	echo _mcms_post_thumbnail_html( $thumbnail_id, $post->ID );
}
