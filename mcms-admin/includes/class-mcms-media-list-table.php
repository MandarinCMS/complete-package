<?php
/**
 * List Table API: MCMS_Media_List_Table class
 *
 * @package MandarinCMS
 ...
 * @see MCMS_List_Table
 */
class MCMS_Media_List_Table extends MCMS_List_Table {
	/**
	...
	public function prepare_items() {
		global $mcms_query, $post_mime_types, $avail_post_mime_types, $mode;

		list( $post_mime_types, $avail_post_mime_types ) = mcms_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args( array(
			...
			'total_pages' => $mcms_query->max_num_pages,
			'per_page' => $mcms_query->query_vars['posts_per_page'],
		) );
	}
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = selected(
				$filter && 0 === strpos( $filter, 'post_mime_type:' ) &&
					mcms_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $filter ) ),
				true,
				false
			);
...
<?php
		if ( ! is_singular() ) {
			...
			/** This action is documented in mcms-admin/includes/class-mcms-posts-list-table.php */
...
?>
	...
	public function column_title( $post ) {
		...
		$thumb = mcms_get_attachment_image( $post->ID, array( 60, 60 ), true, array( 'alt' => '' ) );
		$link_start = $link_end = '';
		...
		<p class="filename">
			...
			echo esc_html( mcms_basename( $file ) );
			?>
		</p>
		<?php
	}
...
?>
			...
			if ( $user_can_edit ):
				$detach_url = add_query_arg( array(
					...
					'media[]' => $post->ID,
					'_mcmsnonce' => mcms_create_nonce( 'bulk-' . $this->_args['plural'] )
				), 'upload.php' );
				printf(
	...
	public function display_rows() {
		...
		$post_ids = mcms_list_pluck( $mcms_query->posts, 'ID' );
		reset( $mcms_query->posts );
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = sprintf(
						...
						mcms_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ),
						/* translators: %s: attachment title */
					...
					$actions['delete'] = sprintf(
						...
						mcms_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ),
						$delete_ays,
						/* translators: %s: attachment title */
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash ) {
					$actions['untrash'] = sprintf(
						'<a href="%s" class="submitdelete aria-button-if-js" aria-label="%s">%s</a>',
						mcms_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ),
						/* translators: %s: attachment title */
