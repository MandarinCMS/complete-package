<?php
/**
 * Administration API: MCMS_Site_Icon class
 *
 * @package MandarinCMS
...
class MCMS_Site_Icon {
	...
	public $site_icon_sizes = array(
		 ...
		 *
		 * @link https://developers.google.com/web/updates/2014/11/Support-for-myskin-color-in-Chrome-39-for-Android
	...
	public function insert_attachment( $object, $file ) {
		...
		$metadata      = mcms_generate_attachment_metadata( $attachment_id, $file );
	...
	public function intermediate_image_sizes( $sizes = array() ) {
		/** This filter is documented in mcms-admin/includes/class-mcms-site-icon.php */
		$this->site_icon_sizes = apply_filters( 'site_icon_image_sizes', $this->site_icon_sizes );
		foreach ( $this->site_icon_sizes as $size ) {
	...
	public function get_post_metadata( $value, $post_id, $meta_key, $single ) {
		if ( $single && '_mcms_attachment_backup_sizes' === $meta_key ) {
			$site_icon_id = get_option( 'site_icon' );
