 ...
 * @see MCMS_List_Table
 */
class MCMS_Posts_List_Table extends MCMS_List_Table {
	...
	public function __construct( $args = array() ) {
		global $post_type_object, $mcmsdb;
		...
		$this->user_posts_count = intval( $mcmsdb->get_var( $mcmsdb->prepare( "
			SELECT COUNT( 1 )
			FROM $mcmsdb->posts
			WHERE post_type = %s
		...
		if ( 'post' === $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $mcmsdb->get_var( $mcmsdb->prepare( "SELECT COUNT( 1 ) FROM $mcmsdb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts)", $post_type ) );
		}
	}
	...
	public function prepare_items() {
		global $avail_post_stati, $mcms_query, $per_page, $mode;

		// is going to call mcms()
		$avail_post_stati = mcms_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' === $mcms_query->query['orderby'] );

		$post_type = $this->screen->post_type;
		$per_page = $this->get_items_per_page( 'edit_' . $post_type . '_per_page' );

		/** This filter is documented in mcms-admin/includes/post.php */
 		$per_page = apply_filters( 'edit_posts_per_page', $per_page, $post_type );

		if ( $this->hierarchical_display ) {
			$total_items = $mcms_query->post_count;
		} elseif ( $mcms_query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $mcms_query->found_posts;
		} else {
			$post_counts = (array) mcms_count_posts( $post_type, 'readable' );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts = mcms_count_posts( $post_type, 'readable' );
	...
	protected function categories_dropdown( $post_type ) {
		...
		if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
			...
			mcms_dropdown_categories( $dropdown_options );
		}
	}
...
<?php
		if ( 'top' === $which && !is_singular() ) {
			 ...
			 *                          'top' or 'bottom' for MCMS_Posts_List_Table,
			 *                          'bar' for MCMS_Media_List_Table.
			 */
...
<?php
	...
	public function get_columns() {
		...
		$taxonomies = mcms_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $mcms_query, $per_page;

		if ( empty( $posts ) )
			$posts = $mcms_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$mcmsdb->update( $mcmsdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	 ...
	 * @since 4.3.0
	 *
	 * @param MCMS_Post $post The current MCMS_Post object.
	 */
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && $post->post_status != 'trash' ) {
			$lock_holder = mcms_check_post_lock( $post->ID );
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' === $post->post_status ) {
				$actions['untrash'] = sprintf(
					'<a href="%s" aria-label="%s">%s</a>',
					mcms_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ),
					/* translators: %s: post title */
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $authors = mcms_dropdown_users( $users_opt ) ) :
	...
	?>
			...
			<div class="inline-edit-group mcms-clearfix">
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php mcms_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
	...
	<?php
		...
		mcms_dropdown_pages( $dropdown_args );
	?>
			</label>
...
<?php	endif; // $bulk ?>
                <?php
				/** This filter is documented in mcms-admin/includes/meta-boxes.php */
	...
	<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
		<?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) :
			...
			<label class="inline-edit-tags">
				...
				<textarea data-mcms-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo $taxonomy_name; ?>]" class="tax_input_<?php echo $taxonomy_name; ?>"></textarea>
			</label>
		<?php endif; ?>
	...
	<?php if ( $bulk ) : ?>
						<option value="-1"><?php _e( '&mdash; No Change &mdash;' ); ?></option>
	<?php endif; // $bulk ?>
					<?php if ( $can_publish ) : // Feeders only get "Unpublished" and "Pending Review" ?>
	...
	if ( $bulk && current_myskin_supports( 'post-formats' ) && post_type_supports( $screen->post_type, 'post-formats' ) ) {
	...
	<?php
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( $bulk ) {
				 ...
				 * @param MCMS_Post $post_type   The post type slug.
				 */
	...
	?>
		<div class="submit inline-edit-save">
			...
			<?php if ( ! $bulk ) {
				mcms_nonce_field( 'inlineeditnonce', '_inline_edit', false );
				?>
