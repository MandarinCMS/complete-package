<?php
/**
 * List Table API: MCMS_MS_Users_List_Table class
 *
 * @package MandarinCMS
 ...
 * @see MCMS_List_Table
 */
class MCMS_MS_Users_List_Table extends MCMS_List_Table {
	/**
	 *
	 * @return bool
	 */
	...
	public function prepare_items() {
		global $usersearch, $role, $mcmsdb, $mode;
		...
		if ( mcms_is_large_network( 'users' ) ) {
		...
		if ( $role === 'super' ) {
			...
			$args['include'] = $mcmsdb->get_col( "SELECT ID FROM $mcmsdb->users WHERE user_login IN ('$logins')" );
		}
		 ...
		 */
		if ( !$usersearch && mcms_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in mcms-admin/includes/class-mcms-users-list-table.php */
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'mcmsmu_users_columns', $users_columns );
	}
	 ...
	 * @since 4.3.0
	 *
	 * @param MCMS_User $user The current MCMS_User object.
	 */
	...
	public function column_username( $user ) {
		...
		$edit_link = esc_url( add_query_arg( 'mcms_http_referer', urlencode( mcms_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	protected function handle_row_actions( $user, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins ) ) {
			$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_mcms_http_referer', urlencode( mcms_unslash( $_SERVER['REQUEST_URI'] ) ), mcms_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
