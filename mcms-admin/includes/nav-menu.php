...
require_once( BASED_TREE_URI . 'mcms-admin/includes/class-walker-nav-menu-edit.php' );

/** Walker_Nav_Menu_Checklist class */
require_once( BASED_TREE_URI . 'mcms-admin/includes/class-walker-nav-menu-checklist.php' );
 ...
 */
function _mcms_ajax_menu_quick_search( $request = array() ) {
	$args = array();
	...
	if ( 'get-post-item' == $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', array( get_post( $object_id ) ) ), 0, (object) $args );
			...
			if ( isset( $request['ID'] ) ) {
				$object_id = (int) $request['ID'];
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
		...
		if ( 'posttype' == $matches[1] && get_post_type_object( $matches[2] ) ) {
			$post_type_obj = _mcms_nav_menu_meta_box_object( get_post_type_object( $matches[2] ) );
			$args = array_merge(
				$args,
				array(
					'no_found_rows'          => true,
			...
			$search_results_query = new MCMS_Query( $args );
			...
			while ( $search_results_query->have_posts() ) {
				...
				if ( 'markup' == $response_format ) {
					$var_by_ref = $post->ID;
					echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', array( get_post( $var_by_ref ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo mcms_json_encode(
						array(
							'ID' => $post->ID,
			...
			if ( empty( $terms ) || is_mcms_error( $terms ) )
				return;
			foreach ( (array) $terms as $term ) {
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', array( $term ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo mcms_json_encode(
						array(
							'ID' => $term->term_id,
 ...
 * @since 3.0.0
 **/
function mcms_nav_menu_setup() {
	// Register meta boxes
	mcms_nav_menu_post_type_meta_boxes();
	add_meta_box( 'add-custom-links', __( 'Custom Links' ), 'mcms_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
	mcms_nav_menu_taxonomy_meta_boxes();

	// Register advanced menu items (columns)
	add_filter( 'manage_nav-menus_columns', 'mcms_nav_menu_manage_columns' );
 ...
 **/
function mcms_initial_nav_menu_meta_boxes() {
	global $mcms_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array($mcms_meta_boxes) )
		return;

	$initial_meta_boxes = array( 'add-post-type-page', 'add-post-type-post', 'add-custom-links', 'add-category' );
	$hidden_meta_boxes = array();

	foreach ( array_keys($mcms_meta_boxes['nav-menus']) as $context ) {
		foreach ( array_keys($mcms_meta_boxes['nav-menus'][$context]) as $priority ) {
			foreach ( $mcms_meta_boxes['nav-menus'][$context][$priority] as $box ) {
 ...
 * @since 3.0.0
 */
function mcms_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( $post_type ) {
			...
			add_meta_box( "add-post-type-{$id}", $post_type->labels->name, 'mcms_nav_menu_item_post_type_meta_box', 'nav-menus', 'side', $priority, $post_type );
		}
	}
}
 ...
 * @since 3.0.0
 */
function mcms_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		...
		if ( $tax ) {
			$id = $tax->name;
			add_meta_box( "add-{$id}", $tax->labels->name, 'mcms_nav_menu_item_taxonomy_meta_box', 'nav-menus', 'side', 'default', $tax );
		}
	}
}
 ...
 * @since 3.6.0
 *
 * @global bool $one_myskin_location_no_menus to determine if no menus exist
 *
 * @param int|string $nav_menu_selected_id (id, name or slug) of the currently-selected menu
 * @return string Disabled attribute if at least one menu exists, false if not
 */
function mcms_nav_menu_disabled_check( $nav_menu_selected_id ) {
	global $one_myskin_location_no_menus;
 ...
 */
function mcms_nav_menu_item_link_meta_box() {
	...
	<div class="customlinkdiv" id="customlinkdiv">
		...
		<p class="button-controls mcms-clearfix">
			<span class="add-to-menu">
				<input type="submit"<?php mcms_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button submit-add-to-menu right" value="<?php esc_attr_e('Add to Menu'); ?>" name="add-custom-menu-item" id="submit-customlinkdiv" />
 ...
 *     @type MCMS_Post_Type $args     Extra meta box arguments (the post type object for this meta box).
 * }
 */
function mcms_nav_menu_item_post_type_meta_box( $object, $box ) {
	global $_nav_menu_placeholder, $nav_menu_selected_id;
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>">
			<ul id="<?php echo $post_type_name; ?>checklist-most-recent" class="categorychecklist form-no-clear">
				 ...
				 * @param array $args        An array of MCMS_Query arguments for the meta box.
				 * @param array $box         Arguments passed to mcms_nav_menu_item_post_type_meta_box().
				 * @param array $recent_args An array of MCMS_Query arguments for 'Most Recent' tab.
				 */
				$most_recent = apply_filters( "nav_menu_items_{$post_type_name}_recent", $most_recent, $args, $box, $recent_args );

				echo walk_nav_menu_tree( array_map( 'mcms_setup_nav_menu_item', $most_recent ), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<ul id="<?php echo $post_type_name; ?>-search-checklist" data-mcms-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_mcms_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $search_results), 0, (object) $args );
				?>
			<?php elseif ( is_mcms_error( $search_results ) ) : ?>
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $post_type_name; ?>checklist" data-mcms-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				 ...
				 *
				 * @see MCMS_Query::query()
				 *
				 * @param array        $posts     The posts for the current post type.
				 * @param array        $args      An array of MCMS_Query arguments.
				 * @param MCMS_Post_Type $post_type The current post type object for this menu item meta box.
				 */
				$posts = apply_filters( "nav_menu_items_{$post_type_name}", $posts, $args, $post_type );

				$checkbox_items = walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $posts), 0, (object) $args );
		...
		<p class="button-controls mcms-clearfix">
			...
			<span class="add-to-menu">
				<input type="submit"<?php mcms_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-post-type-menu-item" id="<?php echo esc_attr( 'submit-posttype-' . $post_type_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 * }
 */
function mcms_nav_menu_item_taxonomy_meta_box( $object, $box ) {
	...
	if ( ! $terms || is_mcms_error($terms) ) {
		echo '<p>' . __( 'No items.' ) . '</p>';
		return;
	}

	$num_pages = ceil( mcms_count_terms( $taxonomy_name , array_merge( $args, array('number' => '', 'offset' => '') ) ) / $per_page );
	...
	<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
		...
		?>">
			<ul id="<?php echo $taxonomy_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $popular_terms), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $taxonomy_name; ?>checklist" data-mcms-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $terms), 0, (object) $args );
				?>
			</ul>
			<?php if ( ! empty( $page_links ) ) : ?>
				<div class="add-menu-item-pagelinks">
		...
		?>" id="tabs-panel-search-taxonomy-<?php echo $taxonomy_name; ?>">
			...
			<ul id="<?php echo $taxonomy_name; ?>-search-checklist" data-mcms-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_mcms_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $search_results), 0, (object) $args );
		...
		<p class="button-controls mcms-clearfix">
			...
			<span class="add-to-menu">
				<input type="submit"<?php mcms_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-taxonomy-menu-item" id="<?php echo esc_attr( 'submit-taxonomy-' . $taxonomy_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 */
function mcms_save_nav_menu_items( $menu_id = 0, $menu_data = array() ) {
	$menu_id = (int) $menu_id;
	$items_saved = array();
	...
	if ( 0 == $menu_id || is_nav_menu( $menu_id ) ) {
		...
		foreach ( (array) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			$items_saved[] = mcms_update_nav_menu_item( $menu_id, $_actual_db_id, $args );
 ...
 */
function _mcms_nav_menu_meta_box_object( $object = null ) {
 ...
 * @return string|MCMS_Error $output The menu formatted to edit or error object on failure.
 */
function mcms_get_nav_menu_to_edit( $menu_id = 0 ) {
	$menu = mcms_get_nav_menu_object( $menu_id );

	// If the menu exists, get its items.
	if ( is_nav_menu( $menu ) ) {
		$menu_items = mcms_get_nav_menu_items( $menu->term_id, array('post_status' => 'any') );
		 ...
		 */
		$walker_class_name = apply_filters( 'mcms_edit_nav_menu_walker', 'Walker_Nav_Menu_Edit', $menu_id );

		if ( class_exists( $walker_class_name ) ) {
			$walker = new $walker_class_name;
		} else {
			return new MCMS_Error( 'menu_walker_not_exist',
		...
		$result .= walk_nav_menu_tree( array_map('mcms_setup_nav_menu_item', $menu_items), 0, (object) array('walker' => $walker ) );
		$result .= ' </ul> ';
		return $result;
	} elseif ( is_mcms_error( $menu ) ) {
		return $menu;
	}
 ...
 */
function mcms_nav_menu_manage_columns() {
	return array(
 ...
 */
function _mcms_delete_orphaned_draft_menu_items() {
	global $mcmsdb;
	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	// Delete orphaned draft menu items.
	$menu_items_to_delete = $mcmsdb->get_col($mcmsdb->prepare("SELECT ID FROM $mcmsdb->posts AS p LEFT JOIN $mcmsdb->postmeta AS m ON p.ID = m.post_id WHERE post_type = 'nav_menu_item' AND post_status = 'draft' AND meta_key = '_menu_item_orphaned' AND meta_value < %d", $delete_timestamp ) );

	foreach ( (array) $menu_items_to_delete as $menu_item_id )
		mcms_delete_post( $menu_item_id, true );
}
 ...
 */
function mcms_nav_menu_update_menu_items ( $nav_menu_selected_id, $nav_menu_selected_title ) {
	$unsorted_menu_items = mcms_get_nav_menu_items( $nav_menu_selected_id, array( 'orderby' => 'ID', 'output' => ARRAY_A, 'output_key' => 'ID', 'post_status' => 'draft,publish' ) );
	$messages = array();
	$menu_items = array();
	...
	if ( ! empty( $_POST['menu-item-db-id'] ) ) {
		foreach ( (array) $_POST['menu-item-db-id'] as $_key => $k ) {
			...
			$menu_item_db_id = mcms_update_nav_menu_item( $nav_menu_selected_id, ( $_POST['menu-item-db-id'][$_key] != $_key ? 0 : $_key ), $args );

			if ( is_mcms_error( $menu_item_db_id ) ) {
	...
	$nav_menu_option['auto_add'] = array_intersect( $nav_menu_option['auto_add'], mcms_get_nav_menus( array( 'fields' => 'ids' ) ) );
	update_option( 'nav_menu_options', $nav_menu_option );

	mcms_defer_term_counting( false );

	/** This action is documented in mcms-roots/nav-menu.php */
	do_action( 'mcms_update_nav_menu', $nav_menu_selected_id );
 ...
 * @access private
 */
function _mcms_expand_nav_menu_post_data() {
		...
		foreach ( $data as $post_input_data ) {
			...
			for ( $i = count( $array_bits ) - 1; $i >= 0; $i -- ) {
				if ( $i == count( $array_bits ) - 1 ) {
					$new_post_data[ $array_bits[ $i ] ] = mcms_slash( $post_input_data->value );
				} else {
