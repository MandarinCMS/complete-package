...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && ! isset( $_POST['html-upload'] ) && ! mcms_doing_ajax() ) {
		mcms_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
 ...
 */
function mcmsmu_delete_blog( $blog_id, $drop = false ) {
	global $mcmsdb;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, mcms_upload_dir is not reliable.
	...
	if ( $drop ) {
		 ...
		 */
		$drop_tables = apply_filters( 'mcmsmu_drop_tables', $tables, $blog_id );

		foreach ( (array) $drop_tables as $table ) {
			$mcmsdb->query( "DROP TABLE IF EXISTS `$table`" );
		}

		$mcmsdb->delete( $mcmsdb->blogs, array( 'blog_id' => $blog_id ) );
		 ...
		 * @since MU (3.0.0)
		 *
		 * @param string $uploads['basedir'] Uploads path without subdirectory. @see mcms_upload_dir()
		 * @param int    $blog_id            The site ID.
		 */
		$dir = apply_filters( 'mcmsmu_delete_blog_upload_dir', $uploads['basedir'], $blog_id );
 ...
 * @since 3.0.0
 *
 * @todo Merge with mcms_delete_user() ?
 *
 * @global mcmsdb $mcmsdb MandarinCMS database abstraction object.
 *
 ...
 */
function mcmsmu_delete_user( $id ) {
	global $mcmsdb;
	 ...
	 */
	do_action( 'mcmsmu_delete_user', $id );
	...
	/** This action is documented in mcms-admin/includes/user.php */
 ...
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the mcms_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$mcmsdb->update( $mcmsdb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
...
function _access_denied_splash() {
	...
	if ( mcms_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) )
		return;

	$blog_name = get_bloginfo( 'name' );

	if ( empty( $blogs ) )
		mcms_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ), 403 );
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) // MandarinCMS english
 ...
 * @since 3.0.0
 *
 * @global int    $mcms_db_version The version number of the database.
 * @global string $pagenow
 *
 * @return false False if the current user is not a super admin.
 */
function site_admin_notice() {
	global $mcms_db_version, $pagenow;
	...
	if ( get_site_option( 'mcmsmu_upgrade_site' ) != $mcms_db_version ) {
...
function confirm_delete_users( $users ) {
	...
	<?php
	mcms_nonce_field( 'ms-users-delete' );
	...
	<?php foreach ( ( $allusers = (array) $_POST['allusers'] ) as $user_id ) {
		if ( $user_id != '' && $user_id != '0' ) {
			...
			if ( ! current_user_can( 'delete_user', $delete_user->ID ) ) {
				mcms_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );
			}

			if ( in_array( $delete_user->user_login, $site_admins ) ) {
				mcms_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network administrator.' ), '<em>' . $delete_user->user_login . '</em>' ) );
			}
			?>
			<tr>
	...
	<?php
	/** This action is documented in mcms-admin/users.php */
...
function network_edit_site_nav( $args = array() ) {
	 ...
	 *
	 * Default links: 'site-info', 'site-users', 'site-myskins', and 'site-settings'.
	 *
	 * @since 4.6.0
	 *
	 * @param array $links {
	...
	$links = apply_filters( 'network_edit_site_nav_links', array(
		...
		'site-myskins'   => array( 'label' => __( 'MySkins' ),   'url' => 'site-myskins.php',   'cap' => 'manage_sites' ),
		'site-settings' => array( 'label' => __( 'Settings' ), 'url' => 'site-settings.php', 'cap' => 'manage_sites' )
	) );

	// Parse arguments
	$r = mcms_parse_args( $args, array(
	...
	// All done!
	echo '<h2 class="nav-tab-wrapper mcms-clearfix">';
...
function get_site_screen_help_tab_args() {
	return array(
		...
		'content' =>
			...
			'<p>' . sprintf( __('<strong>MySkins</strong> &mdash; This area shows myskins that are not already enabled across the network. Enabling a myskin in this menu makes it accessible to this site. It does not activate the myskin, but allows it to show in the site&#8217;s Dexign menu. To enable a myskin for the entire network, see the <a href="%s">Network MySkins</a> screen.' ), network_admin_url( 'myskins.php' ) ) . '</p>' .
			'<p>' . __('<strong>Settings</strong> &mdash; This page shows a list of all settings associated with this site. Some are created by MandarinCMS and others are created by modules you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.') . '</p>'
	);
}
...
function get_site_screen_help_sidebar_content() {
	return '<p><strong>' . __('For more information:') . '</strong></p>' .
		'<p>' . __('<a href="https://dev.mandarincms.com/Network_Admin_Sites_Screen">Documentation on Site Management</a>') . '</p>' .
