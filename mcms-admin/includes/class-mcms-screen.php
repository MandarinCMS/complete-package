...
final class MCMS_Screen {
	 ...
	 *
	 * @param string|MCMS_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 * 	                                  Defaults to the current $hook_suffix global.
	 * @return MCMS_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof MCMS_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'MCMS_NETWORK_ADMIN' ) && MCMS_NETWORK_ADMIN )
				$in_admin = 'network';
			elseif ( defined( 'MCMS_USER_ADMIN' ) && MCMS_USER_ADMIN )
				$in_admin = 'user';
			else
				$in_admin = 'site';
		}
	...
	public function set_current_screen() {
		 ...
		 * @since 3.0.0
		 *
		 * @param MCMS_Screen $current_screen Current MCMS_Screen object.
		 */
	 ...
	 * @static
	 *
	 * @param MCMS_Screen $screen A screen object.
	...
	public function set_screen_reader_content( $content = array() ) {
		...
		$content = mcms_parse_args( $content, $defaults );
	...
	public function render_screen_meta() {
		 ...
		 * @param MCMS_Screen $this            Current MCMS_Screen instance.
		 */
		 ...
		 *
		 * This hook provides back-compat for modules using the back-compat
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $mcms_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );

		switch ( $this->base ) {
			case 'widgets':
				$nonce = mcms_create_nonce( 'widgets-access' );
				$this->_screen_settings = '<p><a id="access-on" href="widgets.php?widgets-access=on&_mcmsnonce=' . urlencode( $nonce ) . '">' . __('Enable accessibility mode') . '</a><a id="access-off" href="widgets.php?widgets-access=off&_mcmsnonce=' . urlencode( $nonce ) . '">' . __('Disable accessibility mode') . "</a></p>\n";
				break;
			case 'post' :
		 ...
		 * @param MCMS_Screen $this            MCMS_Screen object.
		 */
	...
	public function render_screen_options( $options = array() ) {
		...
		if ( 'widgets' !== $this->base ) {
			...
			$form_end = "\n" . mcms_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false, false ) . "\n</form>\n";
		}
	...
	public function render_meta_boxes_preferences() {
		...
		if ( ! isset( $mcms_meta_boxes[ $this->id ] ) ) {
			return;
		}
		?>
		...
		<?php
			...
			if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_myskin_options' ) ) {
				if ( isset( $_GET['welcome'] ) ) {
					$welcome_checked = empty( $_GET['welcome'] ) ? 0 : 1;
					update_user_meta( get_current_user_id(), 'show_welcome_panel', $welcome_checked );
				} else {
					$welcome_checked = get_user_meta( get_current_user_id(), 'show_welcome_panel', true );
					if ( 2 == $welcome_checked && mcms_get_current_user()->user_email != get_option( 'admin_email' ) ) {
						$welcome_checked = false;
					}
				}
				echo '<label for="mcms_welcome_panel-hide">';
				echo '<input type="checkbox" id="mcms_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
	...
	public function render_list_table_columns_preferences() {
		...
		foreach ( $columns as $column => $title ) {
			 ...
			 * title and any other custom column title modules might add.
			 */
			$title = mcms_strip_all_tags( $title );
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' == $option ) {
			...
			/** This filter is documented in mcms-admin/includes/class-mcms-comments-list-table.php */
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );
		} elseif ( 'categories_per_page' == $option ) {
			/** This filter is documented in mcms-admin/includes/class-mcms-terms-list-table.php */
			$per_page = apply_filters( 'edit_categories_per_page', $per_page );
		} else {
			/** This filter is documented in mcms-admin/includes/class-mcms-list-table.php */
		...
		<legend><?php _e( 'Pagination' ); ?></legend>
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="mcms_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>" maxlength="3"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif; ?>
				<input type="hidden" name="mcms_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</fieldset>
		<?php
	}
