<?php
/**
 * MandarinCMS Administration Revisions API
 *
 * @package MandarinCMS
 ...
 *                    Or, false on failure.
 */
function mcms_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	...
	foreach ( _mcms_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 * @param MCMS_Post $compare_from         The revision post object to compare to or from.
		 * @param string  null                  The context of whether the current revision is the old
		 *                                      or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_mcms_post_revision_field_{$field}", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in mcms-admin/includes/revision.php */
		$content_to = apply_filters( "_mcms_post_revision_field_{$field}", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filters the options passed to mcms_text_diff() when viewing a post revision.
		 *
		 * @since 4.1.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to mcms_text_diff().
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param MCMS_Post $compare_from The revision post to compare from.
		 * @param MCMS_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = mcms_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param MCMS_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'mcms_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function mcms_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post = get_post( $post );
	$authors = array();
	$now_gmt = time();

	$revisions = mcms_get_post_revisions( $post->ID, array( 'order' => 'ASC', 'check_enabled' => false ) );
	...
	cache_users( mcms_list_pluck( $revisions, 'post_author' ) );
	...
	foreach ( $revisions as $revision ) {
		...
		if ( $can_restore ) {
			$restore_link = str_replace( '&amp;', '&', mcms_nonce_url(
				add_query_arg(
		...
		$autosave = (bool) mcms_is_post_autosave( $revision );
		 ...
		 *     @type string     $title      Title for the revision's parent MCMS_Post object.
		 ...
		 * }
		 * @param MCMS_Post $revision       The revision's MCMS_Post object.
		 * @param MCMS_Post $post           The revision's parent MCMS_Post object.
		 */
		$revisions[ $revision->ID ] = apply_filters( 'mcms_prepare_revision_for_js', $revisions_data, $revision, $post );
	}
	...
	$diffs = array( array(
		...
		'fields' => mcms_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
	));

	return array(
		'postId'           => $post->ID,
		'nonce'            => mcms_create_nonce( 'revisions-ajax-nonce' ),
 ...
 * @since 4.1.0
 *
 * @global MCMS_Post $post The global `$post` object.
 */
function mcms_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( mcms_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
