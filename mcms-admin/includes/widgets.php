<?php
/**
 * MandarinCMS Widgets Administration API
 *
 * @package MandarinCMS
...
function mcms_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset($mcms_registered_widget_controls[$widget['id']]['id_base']) && isset($widget['params'][0]['number']) ) {
 ...
 */
function mcms_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'mcms_list_widget_controls_dynamic_sidebar' );

	$description = mcms_sidebar_description( $sidebar );
 ...
 * @return array
 */
function mcms_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $mcms_registered_widgets[$widget_id]['callback'] ) ) {
		$mcms_registered_widgets[$widget_id]['_callback'] = $mcms_registered_widgets[$widget_id]['callback'];
		$mcms_registered_widgets[$widget_id]['callback'] = 'mcms_widget_control';
	}
 ...
 * @return array
 */
function mcms_widget_control( $sidebar_args ) {
	global $mcms_registered_widgets, $mcms_registered_widget_controls, $sidebars_widgets;

	$widget_id = $sidebar_args['widget_id'];
	$sidebar_id = isset($sidebar_args['id']) ? $sidebar_args['id'] : false;
	$key = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[$sidebar_id] ) : '-1'; // position of widget in sidebar
	$control = isset($mcms_registered_widget_controls[$widget_id]) ? $mcms_registered_widget_controls[$widget_id] : array();
	$widget = $mcms_registered_widgets[$widget_id];
	...
	$mcms_registered_widgets[$widget_id]['callback'] = $mcms_registered_widgets[$widget_id]['_callback'];
	unset($mcms_registered_widgets[$widget_id]['_callback']);
...
<?php echo ( $widget_description = mcms_widget_description($widget_id) ) ? "$widget_description\n" : "$widget_title\n"; ?>
	</div>
<?php
 ...
 * @return string
 */
function mcms_widgets_access_body_class($classes) {
