...
/**
 * Class MCMS_Community_Events.
 *
 * A client for api.mandarincms.com/events.
 *
 * @since 4.8.0
 */
class MCMS_Community_Events {
	/**
	 * ID for a MandarinCMS user account.
	 *
	 * @since 4.8.0
	 *
	 * @var int
	...
	/**
	 * Constructor for MCMS_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 * @param int        $user_id       MCMS user ID.
	 ...
	 * than having the browser make the request directly to api.mandarincms.com,
	 * because it allows results to be cached server-side and shared with other
	 * users and sites in the network. This makes the process more efficient,
	 * since increasing the number of visits that get cached data means users
	 * don't have to wait as often; if the user's browser made the request
	 * directly, it would also need to make a second request to MCMS in order to
	 * pass the data for caching. Having MCMS make the request also introduces
	 ...
	 * @return array|MCMS_Error A MCMS_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$api_url      = 'http://api.mandarincms.com/events/1.0/';
		$request_args = $this->get_request_args( $location_search, $timezone );
		$request_args['user-agent'] = 'MandarinCMS/' . $mcms_version . '; ' . home_url( '/' );

		if ( mcms_http_supports( array( 'ssl' ) ) ) {
			$api_url = set_url_scheme( $api_url, 'https' );
		}

		$response       = mcms_remote_get( $api_url, $request_args );
		$response_code  = mcms_remote_retrieve_response_code( $response );
		$response_body  = json_decode( mcms_remote_retrieve_body( $response ), true );
		$response_error = null;

		if ( is_mcms_error( $response ) ) {
			$response_error = $response;
		} elseif ( 200 !== $response_code ) {
			$response_error = new MCMS_Error(
				'api-error',
		...
		if ( is_mcms_error( $response_error ) ) {
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `mcms_remote_get()`.
		return array(
			'body' => $args
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = mcms_privacy_anonymize_ip( $client_ip, true );
	 ...
	 *
	 * @since 4.8.0
	 * @deprecated 4.9.0 Use a module instead. See #41217 for an example.
	 *
