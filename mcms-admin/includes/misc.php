<?php
/**
 * Misc MandarinCMS Administration API.
 *
 * @package MandarinCMS
...
function save_mod_rewrite_rules() {
	 ...
	 */
	if ((!file_exists($htaccess_file) && is_writable($home_path) && $mcms_rewrite->using_mod_rewrite_permalinks()) || is_writable($htaccess_file)) {
		if ( got_mod_rewrite() ) {
			$rules = explode( "\n", $mcms_rewrite->mod_rewrite_rules() );
			return insert_with_markers( $htaccess_file, 'MandarinCMS', $rules );
		}
	}
...
function iis7_save_url_rewrite_rules(){
	...
	if ( iis7_supports_permalinks() && ( ( ! file_exists($web_config_file) && win_is_writable($home_path) && $mcms_rewrite->using_mod_rewrite_permalinks() ) || win_is_writable($web_config_file) ) ) {
		$rule = $mcms_rewrite->iis7_url_rewrite_rules(false, '', '');
		if ( ! empty($rule) ) {
...
/**
 * Update the "recently-edited" file for the module or myskin editor.
 *
 * @since 1.5.0
 *
 * @param string $file
...
/**
 * Makes a tree structure for the MySkin Editor's file list.
 *
 * @since 4.9.0
 * @access private
 *
 * @param array $allowed_files List of myskin file paths.
 * @return array Tree structure for listing myskin files.
 */
function mcms_make_myskin_file_tree( $allowed_files ) {
	$tree_list = array();
...
/**
 * Outputs the formatted file list for the MySkin Editor.
 *
 * @since 4.9.0
 * @access private
 *
 ...
 */
function mcms_print_myskin_file_tree( $tree, $level = 2, $size = 1, $index = 1 ) {
	global $relative_file, $stylesheet;

	if ( is_array( $tree ) ) {
		$index = 0;
		$size = count( $tree );
		foreach ( $tree as $label => $myskin_file ) :
			$index++;
			if ( ! is_array( $myskin_file ) ) {
				mcms_print_myskin_file_tree( $myskin_file, $level, $index, $size );
				continue;
			}
			?>
			<li role="treeitem" aria-expanded="true" tabindex="-1"
				...
				<ul role="group" class="tree-folder"><?php mcms_print_myskin_file_tree( $myskin_file, $level + 1, $index, $size ); ?></ul>
			</li>
			<?php
		endforeach;
	} else {
		...
		$url = add_query_arg(
			array(
				...
				'myskin' => rawurlencode( $stylesheet ),
			),
			self_admin_url( 'myskin-editor.php' )
		);
		?>
...
/**
 * Makes a tree structure for the Module Editor's file list.
 *
 * @since 4.9.0
 * @access private
 *
 * @param string $module_editable_files List of module file paths.
 * @return array Tree structure for listing module files.
 */
function mcms_make_module_file_tree( $module_editable_files ) {
	$tree_list = array();
	foreach ( $module_editable_files as $module_file ) {
		$list = explode( '/', preg_replace( '#^.+?/#', '', $module_file ) );
		$last_dir = &$tree_list;
		foreach ( $list as $dir ) {
			$last_dir =& $last_dir[ $dir ];
		}
		$last_dir = $module_file;
	}
	return $tree_list;
}

/**
 * Outputs the formatted file list for the Module Editor.
 *
 * @since 4.9.0
 * @access private
 *
 ...
 * @param int          $index The aria-posinset for the current iteration.
 */
function mcms_print_module_file_tree( $tree, $label = '', $level = 2, $size = 1, $index = 1 ) {
	global $file, $module;
	if ( is_array( $tree ) ) {
		$index = 0;
		$size = count( $tree );
		foreach ( $tree as $label => $module_file ) :
			$index++;
			if ( ! is_array( $module_file ) ) {
				mcms_print_module_file_tree( $module_file, $label, $level, $index, $size );
				continue;
			}
			?>
			<li role="treeitem" aria-expanded="true" tabindex="-1"
				...
				aria-posinset="<?php echo esc_attr( $index ); ?>">
				<span class="folder-label"><?php echo esc_html( $label ); ?> <span class="screen-reader-text"><?php _e( 'folder' ); ?></span><span aria-hidden="true" class="icon"></span></span>
				<ul role="group" class="tree-folder"><?php mcms_print_module_file_tree( $module_file, '', $level + 1, $index, $size ); ?></ul>
			</li>
			<?php
		endforeach;
	} else {
		$url = add_query_arg(
			...
			self_admin_url( 'module-editor.php' )
		);
		?>
 ...
 * @since 2.1.0
 *
 * @param string|MCMS_Error $message
 */
function show_message($message) {
	if ( is_mcms_error($message) ){
 ...
 * @return array
 */
function mcms_doc_link_parse( $content ) {
...
function set_screen_options() {
	...
	if ( isset($_POST['mcms_screen_options']) && is_array($_POST['mcms_screen_options']) ) {
		check_admin_referer( 'screen-options-nonce', 'screenoptionnonce' );

		if ( !$user = mcms_get_current_user() )
			return;
		$option = $_POST['mcms_screen_options']['option'];
		$value = $_POST['mcms_screen_options']['value'];
		...
		switch ( $map_option ) {
			...
			case 'modules_per_page':
			// Network admin
			case 'sites_network_per_page':
			case 'users_network_per_page':
			case 'site_users_network_per_page':
			case 'modules_network_per_page':
			case 'myskins_network_per_page':
			case 'site_myskins_network_per_page':
				$value = (int) $value;
		...
		$url = remove_query_arg( array( 'pagenum', 'apage', 'paged' ), mcms_get_referer() );
		if ( isset( $_POST['mode'] ) ) {
...
/**
 * Check if rewrite rule for MandarinCMS already exists in the IIS 7+ configuration file
 *
 * @since 2.8.0
 *
 * @return bool
...
function iis7_rewrite_rule_exists($filename) {
	...
	$rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'mandarincms\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'MandarinCMS\')]');
	if ( $rules->length == 0 )
		return false;
	else
		return true;
...
/**
 * Delete MandarinCMS rewrite rule from web.config file if it exists there
 *
 * @since 2.8.0
 *
...
/**
 * Add MandarinCMS rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename The file path to the configuration file
...
function iis7_add_rewrite_rule($filename, $rewrite_rule) {
	...
	$mandarincms_rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'mandarincms\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'MandarinCMS\')]');
	if ( $mandarincms_rules->length > 0 )
		return true;
...
function admin_color_scheme_picker( $user_id ) {
	...
	ksort( $_mcms_admin_css_colors );

	if ( isset( $_mcms_admin_css_colors['fresh'] ) ) {
		// Set Default ('fresh') and Light should go first.
		$_mcms_admin_css_colors = array_filter( array_merge( array( 'fresh' => '', 'light' => '' ), $_mcms_admin_css_colors ) );
	}

	$current_color = get_user_option( 'admin_color', $user_id );

	if ( empty( $current_color ) || ! isset( $_mcms_admin_css_colors[ $current_color ] ) ) {
		$current_color = 'fresh';
	}
	...
	<fieldset id="color-picker" class="scheme-list">
		...
		<?php
		mcms_nonce_field( 'save-color-scheme', 'color-nonce', false );
		foreach ( $_mcms_admin_css_colors as $color => $color_info ) :

			?>
			<div class="color-option <?php echo ( $color == $current_color ) ? 'selected' : ''; ?>">
				<input name="admin_color" id="admin_color_<?php echo esc_attr( $color ); ?>" type="radio" value="<?php echo esc_attr( $color ); ?>" class="tog" <?php checked( $color, $current_color ); ?> />
				<input type="hidden" class="css_url" value="<?php echo esc_url( $color_info->url ); ?>" />
				<input type="hidden" class="icon_colors" value="<?php echo esc_attr( mcms_json_encode( array( 'icons' => $color_info->icon_colors ) ) ); ?>" />
 ...
 * @global array $_mcms_admin_css_colors
 */
function mcms_color_scheme_settings() {
	global $_mcms_admin_css_colors;

	$color_scheme = get_user_option( 'admin_color' );

	// It's possible to have a color scheme set that is no longer registered.
	if ( empty( $_mcms_admin_css_colors[ $color_scheme ] ) ) {
		$color_scheme = 'fresh';
	}

	if ( ! empty( $_mcms_admin_css_colors[ $color_scheme ]->icon_colors ) ) {
		$icon_colors = $_mcms_admin_css_colors[ $color_scheme ]->icon_colors;
	} elseif ( ! empty( $_mcms_admin_css_colors['fresh']->icon_colors ) ) {
		$icon_colors = $_mcms_admin_css_colors['fresh']->icon_colors;
	} else {
		// Fall back to the default set of icon colors if the default scheme is missing.
		$icon_colors = array( 'base' => '#82878c', 'focus' => '#00a0d2', 'current' => '#fff' );
	}

	echo '<script type="text/javascript">var _mcmsColorScheme = ' . mcms_json_encode( array( 'icons' => $icon_colors ) ) . ";</script>\n";
}
 ...
 */
function mcms_check_locked_posts( $response, $data, $screen_id ) {
	$checked = array();

	if ( array_key_exists( 'mcms-check-locked-posts', $data ) && is_array( $data['mcms-check-locked-posts'] ) ) {
		foreach ( $data['mcms-check-locked-posts'] as $key ) {
			if ( ! $post_id = absint( substr( $key, 5 ) ) )
				continue;

			if ( ( $user_id = mcms_check_post_lock( $post_id ) ) && ( $user = get_userdata( $user_id ) ) && current_user_can( 'edit_post', $post_id ) ) {
	...
	if ( ! empty( $checked ) )
		$response['mcms-check-locked-posts'] = $checked;
 ...
 * @return array The Heartbeat response.
 */
function mcms_refresh_post_lock( $response, $data, $screen_id ) {
	if ( array_key_exists( 'mcms-refresh-post-lock', $data ) ) {
		$received = $data['mcms-refresh-post-lock'];
		$send = array();
		...
		if ( ( $user_id = mcms_check_post_lock( $post_id ) ) && ( $user = get_userdata( $user_id ) ) ) {
			$error = array(
			...
			if ( $new_lock = mcms_set_post_lock( $post_id ) )
				$send['new_lock'] = implode( ':', $new_lock );
		}

		$response['mcms-refresh-post-lock'] = $send;
	}
 ...
 * @return array The Heartbeat response.
 */
function mcms_refresh_post_nonces( $response, $data, $screen_id ) {
	if ( array_key_exists( 'mcms-refresh-post-nonces', $data ) ) {
		$received = $data['mcms-refresh-post-nonces'];
		$response['mcms-refresh-post-nonces'] = array( 'check' => 1 );
		...
		$response['mcms-refresh-post-nonces'] = array(
			'replace' => array(
				'getpermalinknonce' => mcms_create_nonce('getpermalink'),
				'samplepermalinknonce' => mcms_create_nonce('samplepermalink'),
				'closedpostboxesnonce' => mcms_create_nonce('closedpostboxes'),
				'_ajax_linking_nonce' => mcms_create_nonce( 'internal-linking' ),
				'_mcmsnonce' => mcms_create_nonce( 'update-post_' . $post_id ),
			),
			'heartbeatNonce' => mcms_create_nonce( 'heartbeat-nonce' ),
		);
	}
 ...
 */
function mcms_heartbeat_set_suspension( $settings ) {
	global $pagenow;
...
function heartbeat_autosave( $response, $data ) {
	if ( ! empty( $data['mcms_autosave'] ) ) {
		$saved = mcms_autosave( $data['mcms_autosave'] );

		if ( is_mcms_error( $saved ) ) {
			$response['mcms_autosave'] = array( 'success' => false, 'message' => $saved->get_error_message() );
		} elseif ( empty( $saved ) ) {
			$response['mcms_autosave'] = array( 'success' => false, 'message' => __( 'Error while saving.' ) );
		} else {
			/* translators: draft saved date format, see https://secure.php.net/date */
			$draft_saved_date_format = __( 'g:i:s a' );
			/* translators: %s: date and time */
			$response['mcms_autosave'] = array( 'success' => true, 'message' => sprintf( __( 'Draft saved at %s.' ), date_i18n( $draft_saved_date_format ) ) );
		}
	}
 ...
 * @since 4.2.0
 */
function mcms_admin_canonical_url() {
	$removable_query_args = mcms_removable_query_args();
	...
	?>
	<link id="mcms-admin-canonical" rel="canonical" href="<?php echo esc_url( $filtered_url ); ?>" />
	<script>
		if ( window.history.replaceState ) {
			window.history.replaceState( null, null, document.getElementById( 'mcms-admin-canonical' ).href + window.location.hash );
		}
	</script>
<?php
}
 ...
 * @since 4.6.0
 */
function mcms_page_reload_on_back_button_js() {
	?>
	<script>
...
###SITEURL###' );
	...
	$content = str_replace( '###SITENAME###', mcms_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $content );
	$content = str_replace( '###SITEURL###', home_url(), $content );

	mcms_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), mcms_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) ), $content );
...
/**
 * MCMS_Privacy_Policy_Content class.
 * TODO: move this to a new file.
 *
 * @since 4.9.6
 */
final class MCMS_Privacy_Policy_Content {
	 ...
	 *
	 * Intended for use from `mcms_add_privacy_policy_content()`.
	 *
	 * $since 4.9.6
	 *
	 ...
	 */
	public static function add( $module_name, $policy_text ) {
		if ( empty( $module_name ) || empty( $policy_text ) ) {
			return;
		}
	...
	public static function text_change_check() {
		...
		$policy_page_id = (int) get_option( 'mcms_page_for_privacy_policy' );
		...
		$old = (array) get_post_meta( $policy_page_id, '_mcms_suggested_privacy_policy_content' );
		...
		$cached = get_option( '_mcms_suggested_policy_text_has_changed' );
		...
		if ( $new != $old ) {
			// A module was activated or deactivated, or some policy text has changed.
			// Show a notice on the relevant screens to inform the admin.
			add_action( 'admin_notices', array( 'MCMS_Privacy_Policy_Content', 'policy_text_changed_notice' ) );
			$state = 'changed';
		} else {
			$state = 'not-changed';
		}
		...
		if ( $cached !== $state ) {
			update_option( '_mcms_suggested_policy_text_has_changed', $state );
		}
		...
		<div class="policy-text-updated notice notice-warning is-dismissible">
			<p><?php
				printf(
					...
					esc_url( admin_url( 'tools.php?mcms-privacy-policy-guide=1' ) )
				);
			?></p>
		</div>
		<?php
	...
	public static function _policy_page_updated( $post_id ) {
		...
		foreach ( $old as $old_key => $old_data ) {
			...
			if ( ! empty( $old_data['updated'] ) ) {
				...
				$done[] = array(
					'module_name' => $old_data['module_name'],
		...
		if ( $update_cache ) {
			delete_post_meta( $policy_page_id, '_mcms_suggested_privacy_policy_content' );
			// Update the cache.
			foreach ( $done as $data ) {
				add_post_meta( $policy_page_id, '_mcms_suggested_privacy_policy_content', $data );
			}
		}
	}

	/**
	 * Check for updated, added or removed privacy policy information from modules.
	 *
	 * Caches the current info in post_meta of the policy page.
	 *
	 * @since 4.9.6
	 *
	 * @return array The privacy policy text/informtion added by core and modules.
	 */
	public static function get_suggested_policy_text() {
		...
		foreach ( $new as $new_key => $new_data ) {
			foreach ( $old as $old_key => $old_data ) {
				...
				if ( $new_data['policy_text'] === $old_data['policy_text'] ) {
					// Use the new module name in case it was changed, translated, etc.
					if ( $old_data['module_name'] !== $new_data['module_name'] ) {
						$old_data['module_name'] = $new_data['module_name'];
						$update_cache = true;
					}

					// A module was re-activated.
					if ( ! empty( $old_data['removed'] ) ) {
					...
					$checked[] = $old_data;
					$found = true;
				} elseif ( $new_data['module_name'] === $old_data['module_name'] ) {
		...
		if ( ! empty( $new ) ) {
			// A module was activated.
			foreach ( $new as $new_data ) {
				if ( ! empty( $new_data['module_name'] ) && ! empty( $new_data['policy_text'] ) ) {
					$new_data['added'] = $time;
					$checked[]         = $new_data;
				}
			}
		...
		if ( ! empty( $old ) ) {
			// A module was deactivated.
			foreach ( $old as $old_data ) {
				if ( ! empty( $old_data['module_name'] ) && ! empty( $old_data['policy_text'] ) ) {
					$data = array(
						'module_name' => $old_data['module_name'],
						'policy_text' => $old_data['policy_text'],
						'removed'     => $time,
	 ...
	 * @since 4.9.6
	 *
	 * @param $post MCMS_Post The currently edited post.
	 */
	public static function notice( $post ) {
		...
		?>
		<div class="notice notice-warning inline mcms-pp-notice">
			<p>
			<?php
			/* translators: 1: Privacy Policy guide URL, 2: additional link attributes, 3: accessibility text */
			printf(
				__( 'Need help putting together your new Privacy Policy page? <a href="%1$s" %2$s>Check out our guide%3$s</a> for recommendations on what content to include, along with policies suggested by your modules and myskin.' ),
				admin_url( 'tools.php?mcms-privacy-policy-guide=1' ),
				'target="_blank"',
				sprintf(
	...
	/**
	 * Output the privacy policy guide together with content from the myskin and modules.
	 *
	 * @since 4.9.6
	 */
	public static function privacy_policy_guide() {
		...
		$content = '';
		$toc = array( '<li><a href="#mcms-privacy-policy-guide-introduction">' . __( 'Introduction' ) . '</a></li>' );
		...
		foreach ( $content_array as $section ) {
			...
			if ( ! empty( $section['removed'] ) ) {
				...
				$removed = __( 'You deactivated this module on %s and may no longer need this policy.' );
			...
			$module_name = esc_html( $section['module_name'] );
			$toc_id = 'mcms-privacy-policy-guide-' . sanitize_title( $module_name );
			$toc[] = sprintf( '<li><a href="#%1$s">%2$s</a>' . $meta . '</li>', $toc_id, $module_name );

			$content .= '<div class="privacy-text-section' . $class . '">';
			$content .= '<a id="' . $toc_id . '">&nbsp;</a>';
			/* translators: %s: module name */
			$content .= '<h2>' . sprintf( __( 'Source: %s' ), $module_name ) . '</h2>';
			$content .= $removed;
			...
			if ( empty( $section['removed'] ) ) {
				$content .= '<div class="privacy-text-actions">';
					$content .= '<button type="button" class="privacy-text-copy button">';
						$content .= $copy;
						$content .= '<span class="screen-reader-text">' . sprintf( __( 'Copy suggested policy text from %s.' ), $module_name ) . '</span>';
					$content .= '</button>';
				$content .= '</div>';
			}
		...
		<div class="privacy-text-box">
			<div class="privacy-text-box-head">
				<a id="mcms-privacy-policy-guide-introduction">&nbsp;</a>
				<h2><?php _e( 'Introduction' ); ?></h2>
				<p><?php _e( 'Hello,' ); ?></p>
				<p><?php _e( 'This text template will help you to create your web site&#8217;s privacy policy.' ); ?></p>
				<p><?php _e( 'We have suggested the sections you will need. Under each section heading you will find a short summary of what information you should provide, which will help you to get started. Some sections include suggested policy content, others will have to be completed with information from your myskin and modules.' ); ?></p>
				<p><?php _e( 'Please edit your privacy policy content, making sure to delete the summaries, and adding any information from your myskin and modules. Once you publish your policy page, remember to add it to your navigation menu.' ); ?></p>
	...
	public static function get_default_content( $descr = false ) {
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default MandarinCMS does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your modules may collect personal data. You should add the relevant information below.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Comments' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which MandarinCMS collects by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'By default, MandarinCMS does not include a contact form. If you use a contact form module, use this subsection to note what personal data is captured when someone submits a contact form, and how long you keep it. For example, you may note that you keep contact form submissions for a certain period for customer service purposes, but you do not use the information submitted through them for marketing purposes.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Cookies' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your web site uses, including those set by your modules, social media, and analytics. We have provided the cookies which MandarinCMS installs by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default MandarinCMS does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a MandarinCMS module that provides analytics services. In that case, add information from that module here.' ) . '</p>';
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default MandarinCMS does not share any personal data with anyone.' ) . '</p>';
		 ...
		 */
		return apply_filters( 'mcms_get_default_privacy_policy_content', $content );
	}
	...
	public static function add_suggested_content() {
		...
		mcms_add_privacy_policy_content( __( 'MandarinCMS' ), $content );
	}
}
