<?php
/**
 * List Table API: MCMS_Users_List_Table class
 *
 * @package MandarinCMS
 ...
 * @see MCMS_List_Table
 */
class MCMS_Users_List_Table extends MCMS_List_Table {
	...
	public function prepare_items() {
		...
		$usersearch = isset( $_REQUEST['s'] ) ? mcms_unslash( trim( $_REQUEST['s'] ) ) : '';
		...
		if ( 'none' === $role ) {
			$args = array(
				...
				'include' => mcms_get_users_with_no_role( $this->site_id ),
				'search' => $usersearch,
		 ...
		 * @since 4.4.0
		 *
		 * @param array $args Arguments passed to MCMS_User_Query to retrieve items for the current
		 *                    users list table.
		 */
		$args = apply_filters( 'users_list_table_query_args', $args );

		// Query the user IDs for this page
		$mcms_user_search = new MCMS_User_Query( $args );

		$this->items = $mcms_user_search->get_results();

		$this->set_pagination_args( array(
			'total_items' => $mcms_user_search->get_total(),
	...
	protected function get_views() {
		...
		foreach ( $mcms_roles->get_names() as $this_role => $name ) {
	...
	<div class="alignleft actions">
		...
		<label class="screen-reader-text" for="<?php echo $id ?>"><?php _e( 'Select type&hellip;' ) ?></label>
		<select name="<?php echo $id ?>" id="<?php echo $id ?>">
			<option value=""><?php _e( 'Select type&hellip;' ) ?></option>
			<?php mcms_dropdown_roles(); ?>
		</select>
	<?php
			submit_button( __( 'Select' ), '', $button_id, false );
		endif;
	...
	public function get_columns() {
		$c = array(
			'cb'       => '<input type="checkbox" class="form-control" />',
			'username' => __( '<strong>Username' ),
			#'name'     => __( 'Name' ),
			'email'    => __( '<strong>Email' ),
			'role'     => __( '<strong>Role' ),
			'posts'    => __( '<strong>News' )
		);
	 ...
	 *
	 * @param MCMS_User $user_object The current user object.
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		if ( ! ( $user_object instanceof MCMS_User ) ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			$edit_link = esc_url( add_query_arg( 'mcms_http_referer', urlencode( mcms_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_object->ID ) ) );
			...
			if ( !is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'delete_user', $user_object->ID ) )
				$actions['delete'] = "<a class='submitdelete' href='" . mcms_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Delete' ) . "</a>";
			if ( is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'remove_user', $user_object->ID ) )
				$actions['remove'] = "<a class='submitdelete' href='" . mcms_nonce_url( $url."action=remove&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Remove' ) . "</a>";
			 ...
			 * @param MCMS_User $user_object MCMS_User object for the currently-listed user.
			 */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			$data = 'data-colname="' . mcms_strip_all_tags( $column_display_name ) . '"';
	 ...
	 * @since 4.4.0
	 *
	 * @param MCMS_User $user_object The MCMS_User object.
	...
	protected function get_role_list( $user_object ) {
		...
		foreach ( $user_object->roles as $role ) {
			if ( isset( $mcms_roles->role_names[ $role ] ) ) {
				$role_list[ $role ] = translate_user_role( $mcms_roles->role_names[ $role ] );
			}
		}
		 ...
		 * @param MCMS_User $user_object A MCMS_User object.
		 */
