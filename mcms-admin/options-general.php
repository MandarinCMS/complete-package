...
/** MandarinCMS Translation Installation API */
require_once( BASED_TREE_URI . 'mcms-admin/includes/translation-install.php' );

if ( ! current_user_can( 'manage_options' ) )
	mcms_die( __( 'Sorry, you are not allowed to manage options for this site.' ) );
...
$options_help = '<p>' . __('The fields on this screen determine some of the basics of your site setup.') . '</p>' .
	'<p>' . __('Most myskins display the site title at the top of every page, in the title bar of the browser, and as the identifying name for syndicated feeds. The tagline is also displayed by many myskins.') . '</p>';

if ( ! is_multisite() ) {
	$options_help .= '<p>' . __('The MandarinCMS URL and the Site URL can be the same (example.com) or different; for example, having the MandarinCMS core files (example.com/mandarincms) in a subdirectory instead of the root directory.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://dev.mandarincms.com/Settings_General_Screen">Documentation on General Settings</a>') . '</p>' .
...
<tr>
<th scope="row"><label for="siteurl"><?php _e('MandarinCMS Address (URL)') ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'MCMS_SITEURL' ) ); ?> class="regular-text code<?php if ( defined( 'MCMS_SITEURL' ) ) echo ' disabled' ?>" /></td>
</tr>

<tr>
<th scope="row"><label for="home"><?php _e('Site Address (URL)') ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'MCMS_HOME' ) ); ?> class="regular-text code<?php if ( defined( 'MCMS_HOME' ) ) echo ' disabled' ?>" />
<?php if ( ! defined( 'MCMS_HOME' ) ) : ?>
<p class="description" id="home-description"><?php
	printf(
		/* translators: %s: Codex URL */
		__( 'Enter the address here if you <a href="%s">want your site home page to be different from your MandarinCMS installation directory</a>.' ),
		__( 'https://dev.mandarincms.com/Giving_MandarinCMS_Its_Own_Directory' )
	);
?></p>
<?php endif; ?>
</td>
...
if ( $new_admin_email && $new_admin_email != get_option( 'admin_email' ) ) : ?>
	...
	<p><?php
		...
		printf(
			' <a href="%1$s">%2$s</a>',
			esc_url( mcms_nonce_url( admin_url( 'options.php?dismiss=new_admin_email' ), 'dismiss-' . get_current_blog_id() . '-new_admin_email' ) ),
			__( 'Cancel' )
		);
	?></p>
	</div>
...
<td>
<select name="default_role" id="default_role"><?php mcms_dropdown_roles( get_option('default_role') ); ?></select>
</td>
</tr>
...
if ( ! is_multisite() && defined( 'MCMSLANG' ) && '' !== MCMSLANG && 'en_US' !== MCMSLANG && ! in_array( MCMSLANG, $languages ) ) {
	$languages[] = MCMSLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th scope="row"><label for="MCMSLANG"><?php _e( 'Site Language' ); ?></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			mcms_dropdown_languages( array(
				...
				'show_available_translations' => current_user_can( 'install_languages' ) && mcms_can_install_language_pack(),
			) );

			// Add note about deprecated MCMSLANG constant.
			if ( defined( 'MCMSLANG' ) && ( '' !== MCMSLANG ) && $locale !== MCMSLANG ) {
				if ( is_multisite() && current_user_can( 'manage_network_options' )
					|| ! is_multisite() && current_user_can( 'manage_options' ) ) {
					?>
					<p class="description">
						<strong><?php _e( 'Note:' ); ?></strong> <?php printf( __( 'The %s constant in your %s file is no longer needed.' ), '<code>MCMSLANG</code>', '<code>database-settings.php</code>' ); ?>
					</p>
					<?php
				}
				_deprecated_argument( 'define()', '4.0.0', sprintf( __( 'The %s constant in your %s file is no longer needed.' ), 'MCMSLANG', 'database-settings.php' ) );
			}
			?>
		</td>
	</tr>
...
<select id="timezone_string" name="timezone_string" aria-describedby="timezone-description">
	<?php echo mcms_timezone_choice( $tzstring, get_user_locale() ); ?>
</select>
...
<?php
	...
	*/
	...
	echo "\t<p class='date-time-doc'>" . __('<a href="https://dev.mandarincms.com/Formatting_Date_and_Time">Documentation on date and time formatting</a>.') . "</p>\n";
?>
	</fieldset>
</td>
</tr>
...
for ($day_index = 0; $day_index <= 6; $day_index++) :
	...
	echo "\n\t<option value='" . esc_attr($day_index) . "' $selected>" . $mcms_locale->get_weekday($day_index) . '</option>';
endfor;
?>
</select></td>
</tr>
