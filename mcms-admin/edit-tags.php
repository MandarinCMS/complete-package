...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ) ) ) {
   mcms_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
/**
 * $post_type is set when the MCMS_Terms_List_Table instance is created
 *
 * @global string $post_type
 */
global $post_type;

$mcms_list_table = _get_list_table('MCMS_Terms_List_Table');
...
if ( ! $referer ) { // For POST requests.
	$referer = mcms_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_mcms_http_referer', '_mcmsnonce', 'error', 'message', 'paged' ), $referer );
switch ( $mcms_list_table->current_action() ) {

case 'add-tag':
	check_admin_referer( 'add-tag', '_mcmsnonce_add-tag' );
	...
	$ret = mcms_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
...
case 'delete':
	...
	mcms_delete_term( $tag_ID, $taxonomy );
...
case 'edit':
	...
	if ( ! $term instanceof MCMS_Term ) {
		mcms_die( __( 'You attempted to edit an item that doesn&#8217;t exist. Perhaps it was deleted?' ) );
	}

	mcms_redirect( esc_url_raw( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
	exit;
...
case 'editedtag':
	...
	$ret = mcms_update_term( $tag_ID, $taxonomy, $_POST );
	...
	break;
default:
	if ( ! $mcms_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
		break;
	}
	check_admin_referer( 'bulk-tags' );
	$tags = (array) $_REQUEST['delete_tags'];
	/** This action is documented in mcms-admin/edit-comments.php */
	$location = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $location, $mcms_list_table->current_action(), $tags );
	break;
}

if ( ! $location && ! empty( $_REQUEST['_mcms_http_referer'] ) ) {
	$location = remove_query_arg( array( '_mcms_http_referer', '_mcmsnonce' ), mcms_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( $location ) {
	 ...
	 */
	mcms_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}
...
mcms_enqueue_script('admin-tags');
if ( current_user_can($tax->cap->edit_terms) )
	mcms_enqueue_script('inline-edit-tax');

if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy  ) {
	$help ='';
	if ( 'category' == $taxonomy )
		$help = '<p>' . sprintf(__( 'You can use categories to define sections of your site and group related posts. The default category is &#8220;Default&#8221; until you change it in your <a href="%s">writing settings</a>.' ) , 'options-writing.php' ) . '</p>';
	...
	if ( 'category' == $taxonomy || 'post_tag' == $taxonomy ) {
		...
		$help .= '<li>' . __( '<strong>Description</strong> &mdash; The description is not prominent by default; however, some myskins may display it.' ) . '</li>' .
		'</ul>' .
	...
	if ( 'category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://dev.mandarincms.com/Posts_Categories_Screen">Documentation on Categories</a>' ) . '</p>';
	elseif ( 'link_category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://dev.mandarincms.com/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	else
		$help .= '<p>' . __( '<a href="https://dev.mandarincms.com/Posts_Tags_Screen">Documentation on Tags</a>' ) . '</p>';

	$help .= '<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>';
...
require_once( BASED_TREE_URI . 'mcms-admin/includes/edit-tag-messages.php' );

$class = ( isset( $_REQUEST['error'] ) ) ? 'error' : 'updated';

if ( is_module_active( 'mcmscat2tag-importer/mcmscat2tag-importer.php' ) ) {
	$import_link = admin_url( 'admin.php?import=mcmscat2tag' );
} else {
...
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	...
	printf( '<span class="subtitle">' . __( 'Search results for &#8220;%s&#8221;' ) . '</span>', esc_html( mcms_unslash( $_REQUEST['s'] ) ) );
}
?>
...
<form class="search-form mcms-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />

<?php $mcms_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>

</form>

<div id="col-container" class="mcms-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />
<?php mcms_nonce_field('add-tag', '_mcmsnonce_add-tag'); ?>
...
<div class="form-field term-parent-wrap">
	...
	mcms_dropdown_categories( $dropdown_args );
	?>
...
<div class="form-field term-description-wrap">
	...
	<p><?php _e('The description is not prominent by default; however, some myskins may show it.'); ?></p>
</div>
