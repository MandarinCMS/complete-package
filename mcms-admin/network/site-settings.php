...
if ( ! current_user_can( 'manage_sites' ) )
	mcms_die( __( 'Sorry, you are not allowed to edit this site.' ) );
...
if ( isset($_REQUEST['action']) && 'update-site' == $_REQUEST['action'] && is_array( $_POST['option'] ) ) {
	...
	$skip_options = array( 'allowedmyskins' ); // Don't update these options since they are handled elsewhere in the form.
	foreach ( (array) $_POST['option'] as $key => $val ) {
		$key = mcms_unslash( $key );
		$val = mcms_unslash( $val );
		if ( $key === 0 || is_array( $val ) || in_array($key, $skip_options) )
			continue; // Avoids "0 is a protected MCMS option and may not be modified" error when edit blog options
	 ...
	 */
	do_action( 'mcmsmu_update_blog_options', $id );

	restore_current_blog();
	mcms_redirect( add_query_arg( array( 'update' => 'updated', 'id' => $id ), 'site-settings.php') );
	exit;
}
...
<form method="post" action="site-settings.php?action=update-site">
	<?php mcms_nonce_field( 'edit-site' ); ?>
	...
	<table class="form-table">
		...
		$query = $mcmsdb->prepare( $sql,
			$mcmsdb->esc_like( '_' ) . '%',
			'%' . $mcmsdb->esc_like( 'user_roles' )
		);
		$options = $mcmsdb->get_results( $query );
		 ...
		 */
		do_action( 'mcmsmueditblogaction', $id );
		?>
	</table>
	<?php submit_button(); ?>
</form>
