...
get_current_screen()->add_help_tab( array(
	...
	'title'   => __('Overview'),
	'content' =>
		'<p>' . __('Only use this screen once you have updated to a new version of MandarinCMS through Updates/Available Updates (via the Network Administration navigation menu or the Toolbar). Clicking the Upgrade Network button will step through each site in the network, five at a time, and make sure any database updates are applied.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://dev.mandarincms.com/Network_Admin_Updates_Screen">Documentation on Upgrade Network</a>') . '</p>' .
...
switch ( $action ) {
	case "upgrade":
		...
		if ( $n < 5 ) {
			/**
			 * @global string $mcms_db_version
			 */
			global $mcms_db_version;
			update_site_option( 'mcmsmu_upgrade_site', $mcms_db_version );
		}
		...
		foreach ( (array) $site_ids as $site_id ) {
			...
			$response = mcms_remote_get( $upgrade_url, array(
				'timeout'     => 120,
				'httpversion' => '1.1',
				'sslverify'   => false,
			) );
			 ...
			 * @since MU (3.0.0)
			 *
			 * @param array|MCMS_Error $response The upgrade response array or MCMS_Error on failure.
			 */
			 ...
			 */
			do_action( 'mcmsmu_upgrade_site', $site_id );
		}
		echo "</ul>";
	...
	case 'show':
	default:
		if ( get_site_option( 'mcmsmu_upgrade_site' ) != $GLOBALS['mcms_db_version'] ) :
		?>
		<h2><?php _e( 'Database Update Required' ); ?></h2>
		<p><?php _e( 'MandarinCMS has been updated! Before we send you on your way, we need to individually upgrade the sites in your network.' ); ?></p>
		<?php endif; ?>
