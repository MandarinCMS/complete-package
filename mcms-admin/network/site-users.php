...
if ( ! current_user_can('manage_sites') )
	mcms_die( __( 'Sorry, you are not allowed to edit this site.' ), 403 );
...
$referer = remove_query_arg( 'update', mcms_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_mcmsnonce_add-new-user' );
			$user = $_POST['user'];
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = mcms_generate_password( 12, false);
				$user_id = mcmsmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
	...
	mcms_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit();
}
 ...
 */
if ( ! mcms_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) )
...
/** This filter is documented in mcms-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table">
		<tr>
			...
			<td><input type="text" class="regular-text mcms-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
			...
			<?php
			switch_to_blog( $id );
			mcms_dropdown_roles( get_option( 'default_role' ) );
			restore_current_blog();
			?>
			</select></td>
		</tr>
...
<form action="<?php echo network_admin_url('site-users.php?action=newuser'); ?>" id="newuser" method="post">
	...
	<?php mcms_nonce_field( 'add-user', '_mcmsnonce_add-new-user' ) ?>
