...
get_current_screen()->add_help_tab( array(
		...
		'content' =>
			...
			'<p>' . __('Registration settings can disable/enable public signups. If you let others sign up for a site, install spam modules. Spaces, not commas, should separate names banned as sites for this network.') . '</p>' .
			...
			'<p>' . __('Menu setting enables/disables the module menus from appearing for non super admins, so that only super admins, not site admins, have access to activate modules.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://dev.mandarincms.com/Network_Admin_Settings_Screen">Documentation on Network Settings</a>') . '</p>' .
	'<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
);

if ( $_POST ) {
	/** This action is documented in mcms-admin/network/edit.php */
	do_action( 'mcmsmuadminedit' );
	...
	$options = array(
		...
		'illegal_names', 'limited_email_domains', 'banned_email_domains', 'MCMSLANG', 'new_admin_email',
		'first_comment_email',
	);

	// Handle translation installation.
	if ( ! empty( $_POST['MCMSLANG'] ) && current_user_can( 'install_languages' ) && mcms_can_install_language_pack() ) {
		$language = mcms_download_language_pack( $_POST['MCMSLANG'] );
		if ( $language ) {
			$_POST['MCMSLANG'] = $language;
		}
	}
	...
	foreach ( $options as $option_name ) {
		if ( ! isset($_POST[$option_name]) )
			continue;
		$value = mcms_unslash( $_POST[$option_name] );
	 ...
	 * @since MU (3.0.0)
	 */
	do_action( 'update_mcmsmu_options' );

	mcms_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php mcms_nonce_field( 'siteoptions' ); ?>
				...
				<td>
					...
					if ( $new_admin_email && $new_admin_email != get_site_option( 'admin_email' ) ) : ?>
						...
						<p><?php
							...
							printf(
								' <a href="%1$s">%2$s</a>',
								esc_url( mcms_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
								__( 'Cancel' )
							);
						?></p>
						</div>
			...
			<tr>
				...
				<td>
					...
					<?php if ( is_subdomain_install() ) {
						...
						/* translators: 1: NOBLOGREDIRECT 2: database-settings.php */
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ) ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table">
				<tr>
					<th><label for="MCMSLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<?php
		...
		<table id="menu" class="form-table">
			 ...
			 * default option, 'modules' is enabled, site administrators are granted access to the Modules
			 ...
			 */
			$menu_items = apply_filters( 'mu_menu_items', array( 'modules' => __( 'Modules' ) ) );
			$fieldset_end = '';
