	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id != '0' && $id != '1' ) {
				...
				mcms_redirect( network_admin_url( 'users.php' ) );
			}
			exit();
		...
		case 'allusers':
			...
			if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
						...
						switch ( $doaction ) {
							...
							case 'spam':
								...
								if ( is_super_admin( $user->ID ) )
									mcms_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
				...
				mcms_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $userfunction ), mcms_get_referer() ) );
			} else {
		...
		case 'dodelete':
			...
			mcms_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $deletefunction ), network_admin_url( 'users.php' ) ) );
			exit();
	}
}

$mcms_list_table = _get_list_table('MCMS_MS_Users_List_Table');
...
if ( isset( $_REQUEST['updated'] ) && $_REQUEST['updated'] == 'true' && ! empty( $_REQUEST['action'] ) ) {
	...
	?>
<div class="wrap">
	<h1 class="mcms-heading-inline"><?php esc_html_e( 'Users' ); ?></h1>
	...
	<form method="get" class="search-form">
		<?php $mcms_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>
...
<?php require_once( BASED_TREE_URI . 'mcms-admin/admin-footer.php' ); ?>
