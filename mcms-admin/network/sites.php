...
$mcms_list_table = _get_list_table( 'MCMS_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( ! array_key_exists( $site_action, $manage_actions ) ) {
			mcms_die( __( 'The requested action is not valid.' ) );
		}
		...
		if ( get_network()->site_id == $id ) {
			mcms_die( __( 'Sorry, you are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $site_action ); ?>" method="post">
					...
					<input type="hidden" name="_mcms_http_referer" value="<?php echo esc_attr( mcms_get_referer() ); ?>" />
					<?php mcms_nonce_field( $site_action . '_' . $id, '_mcmsnonce', false ); ?>
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			if ( ! current_user_can( 'delete_sites' ) )
				mcms_die( __( 'Sorry, you are not allowed to access this page.' ), '', array( 'response' => 403 ) );

			$updated_action = 'not_deleted';
			if ( $id != '0' && $id != get_network()->site_id && current_user_can( 'delete_site', $id ) ) {
				mcmsmu_delete_blog( $id, true );
		...
		case 'delete_sites':
			...
			foreach ( (array) $_POST['site_ids'] as $site_id ) {
				...
				if ( ! current_user_can( 'delete_site', $site_id ) ) {
					...
					mcms_die( sprintf( __( 'Sorry, you are not allowed to delete the site %s.' ), $site_address ), 403 );
				}

				$updated_action = 'all_delete';
				mcmsmu_delete_blog( $site_id, true );
			}
			break;
							...
							case 'delete':
								...
								<div class="wrap">
									...
									<form action="sites.php?action=delete_sites" method="post">
										...
										<input type="hidden" name="_mcms_http_referer" value="<?php echo esc_attr( mcms_get_referer() ); ?>" />
										<?php mcms_nonce_field( 'ms-delete-sites', '_mcmsnonce', false ); ?>
	...
	if ( ! empty( $updated_action ) ) {
		mcms_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), mcms_get_referer() ) );
		exit();
	}
}
...
<div class="wrap">
<h1 class="mcms-heading-inline"><?php _e( 'Sites' ); ?></h1>
...
<?php $mcms_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
