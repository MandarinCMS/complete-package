...
if ( ! current_user_can( 'create_sites' ) ) {
	mcms_die( __( 'Sorry, you are not allowed to add sites to this network.' ) );
}
...
if ( isset($_REQUEST['action']) && 'add-site' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-blog', '_mcmsnonce_add-blog' );

	if ( ! is_array( $_POST['blog'] ) )
		mcms_die( __( 'Can&#8217;t create an empty site.' ) );
	...
	if ( ! is_subdomain_install() ) {
		...
		if ( in_array( $domain, $subdirectory_reserved_names ) ) {
			mcms_die(
				...
				sprintf( __( 'The following words are reserved for use by MandarinCMS functions and cannot be used as blog names: %s' ),
	...
	if ( isset( $_POST['MCMSLANG'] ) ) {
		if ( '' === $_POST['MCMSLANG'] ) {
			$meta['MCMSLANG'] = ''; // en_US
		} elseif ( in_array( $_POST['MCMSLANG'], get_available_languages() ) ) {
			$meta['MCMSLANG'] = $_POST['MCMSLANG'];
		} elseif ( current_user_can( 'install_languages' ) && mcms_can_install_language_pack() ) {
			$language = mcms_download_language_pack( mcms_unslash( $_POST['MCMSLANG'] ) );
			if ( $language ) {
				$meta['MCMSLANG'] = $language;
			}
		}
	}

	if ( empty( $domain ) )
		mcms_die( __( 'Missing or invalid site address.' ) );

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		mcms_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		mcms_die( __( 'Invalid email address.' ) );
	}
	...
	if ( !$user_id ) { // Create a new user with a random password
		...
		if ( $user_id ) {
			mcms_die( __( 'The domain or path entered conflicts with an existing username.' ) );
		}
		$password = mcms_generate_password( 12, false );
		$user_id = mcmsmu_create_user( $domain, $password, $email );
		if ( false === $user_id ) {
			mcms_die( __( 'There was an error creating the user.' ) );
		}
	...
	$mcmsdb->hide_errors();
	$id = mcmsmu_create_blog( $newdomain, $path, $title, $user_id, $meta, get_current_network_id() );
	$mcmsdb->show_errors();
	if ( ! is_mcms_error( $id ) ) {
...
Name: %3$s' ),
		...
		mcmsmu_welcome_notification( $id, $user_id, $password, $title, array( 'public' => 1 ) );
		mcms_redirect( add_query_arg( array( 'update' => 'added', 'id' => $id ), 'site-new.php' ) );
		exit;
	} else {
		mcms_die( $id->get_error_message() );
	}
}
...
<?php mcms_nonce_field( 'add-blog', '_mcmsnonce_add-blog' ) ?>
	<table class="form-table">
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text mcms-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" /></td>
		</tr>
		<tr class="form-field">
