<?php
/**
 * MandarinCMS Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_myskin_supports( 'menus' ) && ! current_myskin_supports( 'widgets' ) )
	mcms_die( __( 'Your myskin does not support navigation menus or widgets.' ) );
 ...
 */
_mcms_expand_nav_menu_post_data();

switch ( $action ) {
	case 'add-menu-item':
		check_admin_referer( 'add-menu_item', 'menu-settings-column-nonce' );
		if ( isset( $_REQUEST['nav-menu-locations'] ) )
			set_myskin_mod( 'nav_menu_locations', array_map( 'absint', $_REQUEST['menu-locations'] ) );
		elseif ( isset( $_REQUEST['menu-item'] ) )
			mcms_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		break;
	case 'move-down-menu-item' :
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : mcms_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			if ( ! is_mcms_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id = (int) $menus[0];
				$ordered_menu_items = mcms_get_nav_menu_items( $menu_id );
				$menu_item_data = (array) mcms_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if (
					...
					$next_item_data = (array) mcms_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if (
						...
						$parent_object = mcms_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_mcms_error( $parent_object ) ) {
					...
					// Make menu item a child of its next sibling.
						...
						mcms_update_post($menu_item_data);
						mcms_update_post($next_item_data);
					}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && mcms_delete_post( $menu_item_id, true ) )
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deletion = mcms_delete_nav_menu( $nav_menu_selected_id );
		} else {
	...
	case 'delete_menus':
		...
		foreach ( $_REQUEST['delete_menus'] as $menu_id_to_delete ) {
			...
			$deletion = mcms_delete_nav_menu( $menu_id_to_delete );
			if ( is_mcms_error( $deletion ) ) {
				$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $deletion->get_error_message() . '</p></div>';
				$deletion_error = true;
			}
	...
	case 'update':
		...
		// Set menu locations.
		set_myskin_mod( 'nav_menu_locations', $menu_locations );
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = mcms_update_nav_menu_object( 0, array('menu-name' => $new_menu_title) );

				if ( is_mcms_error( $_nav_menu_selected_id ) ) {
					$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $_nav_menu_selected_id->get_error_message() . '</p></div>';
				} else {
					$_menu_object = mcms_get_nav_menu_object( $_nav_menu_selected_id );
					$nav_menu_selected_id = $_nav_menu_selected_id;
					$nav_menu_selected_title = $_menu_object->name;
					if ( isset( $_REQUEST['menu-item'] ) )
						mcms_save_nav_menu_items( $nav_menu_selected_id, absint( $_REQUEST['menu-item'] ) );
					if ( isset( $_REQUEST['zero-menu-state'] ) ) {
						// If there are menu items, add them
						mcms_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title );
					...
					mcms_redirect( admin_url( 'nav-menus.php?menu=' . $_nav_menu_selected_id ) );
					exit();
				}
			} else {
		...
		// Update existing menu.
			...
			$_menu_object = mcms_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_mcms_error( $_menu_object ) ) {
				$_nav_menu_selected_id = mcms_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
				if ( is_mcms_error( $_nav_menu_selected_id ) ) {
			...
			// Update menu items.
			if ( ! is_mcms_error( $_menu_object ) ) {
				$messages = array_merge( $messages, mcms_nav_menu_update_menu_items( $_nav_menu_selected_id, $nav_menu_selected_title ) );

				// If the menu ID changed, redirect to the new URL.
				if ( $nav_menu_selected_id != $_nav_menu_selected_id ) {
					mcms_redirect( admin_url( 'nav-menus.php?menu=' . intval( $_nav_menu_selected_id ) ) );
					exit();
				}
			}
		}
	...
	case 'locations':
		if ( ! $num_locations ) {
			mcms_redirect( admin_url( 'nav-menus.php' ) );
			exit();
		}
...
/*
 * If we have one myskin location, and zero menus, we take them right
 * into editing their first menu.
 */
$page_count = mcms_count_posts( 'page' );
$one_myskin_location_no_menus = ( 1 == count( get_registered_nav_menus() ) && ! $add_new_screen && empty( $nav_menus ) && ! empty( $page_count->publish ) ) ? true : false;

$nav_menus_l10n = array(
	'oneMySkinLocationNoMenus' => $one_myskin_location_no_menus,
 ...
 * or more than 1 myskin locations.
 */
if ( 0 == $menu_count && ! $add_new_screen && ! $one_myskin_location_no_menus )
	mcms_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
...
if ( $one_myskin_location_no_menus ) {
	$nav_menu_selected_id = 0;
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object = mcms_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_mcms_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names.
foreach ( (array) $nav_menus as $key => $_nav_menu ) {
	$nav_menus[$key]->truncated_name = mcms_html_excerpt( $_nav_menu->name, 40, '&hellip;' );
}

// Retrieve menu locations.
if ( current_myskin_supports( 'menus' ) ) {
	$locations = get_registered_nav_menus();
	$menu_locations = get_nav_menu_locations();
}
 ...
 *
 * @global int $_mcms_nav_menu_max_depth
 */
global $_mcms_nav_menu_max_depth;
$_mcms_nav_menu_max_depth = 0;

// Calling mcms_get_nav_menu_to_edit generates $_mcms_nav_menu_max_depth.
if ( is_nav_menu( $nav_menu_selected_id ) ) {
	$menu_items = mcms_get_nav_menu_items( $nav_menu_selected_id, array( 'post_status' => 'any' ) );
	$edit_markup = mcms_get_nav_menu_to_edit( $nav_menu_selected_id );
}
 ...
 * @return string
 */
function mcms_nav_menu_max_depth( $classes ) {
	global $_mcms_nav_menu_max_depth;
	return "$classes menu-max-depth-$_mcms_nav_menu_max_depth";
}

add_filter('admin_body_class', 'mcms_nav_menu_max_depth');

mcms_nav_menu_setup();
mcms_initial_nav_menu_meta_boxes();

if ( ! current_myskin_supports( 'menus' ) && ! $num_locations )
	$messages[] = '<div id="message" class="updated"><p>' . sprintf( __( 'Your myskin does not natively support menus, but you can use them in sidebars by adding a &#8220;Navigation Menu&#8221; widget on the <a href="%s">Widgets</a> screen.' ), admin_url( 'widgets.php' ) ) . '</p></div>';

if ( ! $locations_screen ) : // Main tab
	$overview  = '<p>' . __( 'This screen is used for managing your navigation menus.' ) . '</p>';
	/* translators: 1: Widgets admin screen URL, 2 and 3: The name of the default myskins */
	$overview .= '<p>' . sprintf( __( 'Menus can be displayed in locations defined by your myskin, even used in sidebars by adding a &#8220;Navigation Menu&#8221; widget on the <a href="%1$s">Widgets</a> screen. If your myskin does not support the navigation menus feature (the default myskins, %2$s and %3$s, do), you can learn about adding this support by following the Documentation link to the side.' ), admin_url( 'widgets.php' ), 'Twenty Sixteen', 'Twenty Seventeen' ) . '</p>';
	...
	$menu_management .= '<p>' . __( 'You can assign myskin locations to individual menus by <strong>selecting the desired settings</strong> at the bottom of the menu editor. To assign menus to all myskin locations at once, <strong>visit the Manage Locations tab</strong> at the top of the screen.' ) . '</p>';
	...
	$locations_overview  = '<p>' . __( 'This screen is used for globally assigning menus to locations defined by your myskin.' ) . '</p>';
	$locations_overview .= '<ul><li>' . __( 'To assign menus to one or more myskin locations, <strong>select a menu from each location&#8217;s drop down.</strong> When you&#8217;re finished, <strong>click Save Changes</strong>' ) . '</li>';
	$locations_overview .= '<li>' . __( 'To edit a menu currently assigned to a myskin location, <strong>click the adjacent &#8217;Edit&#8217; link</strong>' ) . '</li>';
	$locations_overview .= '<li>' . __( 'To add a new menu instead of assigning an existing one, <strong>click the &#8217;Use new menu&#8217; link</strong>. Your new menu will be automatically assigned to that myskin location' ) . '</li></ul>';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://dev.mandarincms.com/Dexign_Menus_Screen">Documentation on Menus</a>') . '</p>' .
...
?>
<div class="wrap">
	<h1 class="mcms-heading-inline"><?php echo esc_html( __( 'Menus' ) ); ?></h1>
	<?php
	if ( current_user_can( 'customize' ) ) :
		...
		printf(
			...
			esc_url( add_query_arg( array(
				...
				'return' => urlencode( remove_query_arg( mcms_removable_query_args(), mcms_unslash( $_SERVER['REQUEST_URI'] ) ) ),
	...
	if ( $locations_screen ) :
		if ( 1 == $num_locations ) {
			echo '<p>' . __( 'Your myskin supports one menu. Select which menu you would like to use.' ) . '</p>';
		} else {
			echo '<p>' .  sprintf( _n( 'Your myskin supports %s menu. Select which menu appears in each location.', 'Your myskin supports %s menus. Select which menu appears in each location.', $num_locations ), number_format_i18n( $num_locations ) ) . '</p>';
		}
	?>
	<div id="menu-locations-wrap">
		<form method="post" action="<?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?>">
			<table class="widefat fixed" id="menu-locations-table">
				<thead>
				<tr>
					<th scope="col" class="manage-column column-locations"><?php _e( 'MySkin Location' ); ?></th>
							...
							<select name="menu-locations[<?php echo $_location; ?>]" id="locations-<?php echo $_location; ?>">
								...
								<?php foreach ( $nav_menus as $menu ) : ?>
									...
									<option <?php if ( $selected ) echo 'data-orig="true"'; ?> <?php selected( $selected ); ?> value="<?php echo $menu->term_id; ?>">
										<?php echo mcms_html_excerpt( $menu->name, 40, '&hellip;' ); ?>
									</option>
								<?php endforeach; ?>
							</select>
			...
			<p class="button-controls mcms-clearfix"><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?></p>
			<?php mcms_nonce_field( 'save-menu-locations' ); ?>
				...
				<?php foreach ( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations ) ) {
							 ...
							 */
							$assigned_locations = array_slice( $locations_assigned_to_this_menu, 0, absint( apply_filters( 'mcms_nav_locations_listed_per_menu', 3 ) ) );
	...
	<div id="nav-menus-frame" class="mcms-clearfix">
	<div id="menu-settings-column" class="metabox-holder<?php if ( isset( $_GET['menu'] ) && '0' == $_GET['menu'] ) { echo ' metabox-holder-disabled'; } ?>">
		...
		<form id="nav-menu-meta" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php mcms_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
		...
		<div id="menu-management">
			<form id="update-nav-menu" method="post" enctype="multipart/form-data">
				<div class="menu-edit <?php if ( $add_new_screen ) echo 'blank-slate'; ?>">
					...
					mcms_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
					...
					<div id="nav-menu-header">
						<div class="major-publishing-actions mcms-clearfix">
					...
					<div id="post-body">
						<div id="post-body-content" class="mcms-clearfix">
							<?php if ( ! $add_new_screen ) : ?>
							<h3><?php _e( 'Menu Structure' ); ?></h3>
							<?php $starter_copy = ( $one_myskin_location_no_menus ) ? __( 'Edit your default menu by adding or removing items. Drag each item into the order you prefer. Click Create Menu to save your changes.' ) : __( 'Drag each item into the order you prefer. Click the arrow on the right of the item to reveal additional configuration options.' ); ?>
							<div class="drag-instructions post-body-plain" <?php if ( isset( $menu_items ) && 0 == count( $menu_items ) ) { ?>style="display: none;"<?php } ?>>
								<p><?php echo $starter_copy; ?></p>
							</div>
							<?php
							if ( isset( $edit_markup ) && ! is_mcms_error( $edit_markup ) ) {
								echo $edit_markup;
							} else {
							?>
							...
							<?php if ( $add_new_screen ) : ?>
								...
								<?php endif; ?>
							<?php endif; ?>
							<div class="menu-settings" <?php if ( $one_myskin_location_no_menus ) { ?>style="display: none;"<?php } ?>>
								...
								<?php if ( current_myskin_supports( 'menus' ) ) : ?>

									<fieldset class="menu-settings-group menu-myskin-locations">
										...
										<div class="menu-settings-input checkbox-input">
											...
											<?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] != $nav_menu_selected_id ) : ?>
												<span class="myskin-location-set"><?php
													/* translators: %s: menu name */
													printf( _x( '(Currently set to: %s)', 'menu location' ),
														mcms_get_nav_menu_object( $menu_locations[ $location ] )->name
													);
												?></span>
											<?php endif; ?>
										</div>
					...
					<div id="nav-menu-footer">
						<div class="major-publishing-actions mcms-clearfix">
							...
							<span class="delete-action">
								<a class="submitdelete deletion menu-delete" href="<?php echo esc_url( mcms_nonce_url( add_query_arg( array( 'action' => 'delete', 'menu' => $nav_menu_selected_id ), admin_url( 'nav-menus.php' ) ), 'delete-nav_menu-' . $nav_menu_selected_id) ); ?>"><?php _e('Delete Menu'); ?></a>
