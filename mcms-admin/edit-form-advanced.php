...
mcms_enqueue_script('post');
$_mcms_editor_expand = $_content_editor_dfw = false;
 ...
 */
if ( post_type_supports( $post_type, 'editor' ) && ! mcms_is_mobile() &&
	 ! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	 apply_filters( 'mcms_editor_expand', true, $post_type ) ) {

	mcms_enqueue_script('editor-expand');
	$_content_editor_dfw = true;
	$_mcms_editor_expand = ( get_user_setting( 'editor_expand', 'on' ) === 'on' );
}
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_mcms_posts_page_notice' );
	remove_post_type_support( $post_type, 'editor' );
}

$thumbnail_support = current_myskin_supports( 'post-thumbnails', $post_type ) && post_type_supports( $post_type, 'thumbnail' );
...
if ( $thumbnail_support ) {
	add_thickbox();
	mcms_enqueue_media( array( 'post' => $post_ID ) );
}
...
$messages['post'] = array(
	...
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __( 'Post restored to revision from %s.' ), mcms_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	 ...
	 4 => __( 'Page updated.' ),
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __( 'Page restored to revision from %s.' ), mcms_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = mcms_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _mcms_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice )
		mcms_delete_post_revision( $autosave->ID );
...
require_once( BASED_TREE_URI . 'mcms-admin/includes/meta-boxes.php' );
...
if ( current_myskin_supports( 'post-formats' ) && post_type_supports( $post_type, 'post-formats' ) )
...
if ( 'post' == $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
			...
			'<p>' . __('<a href="https://dev.mandarincms.com/Posts_Add_New_Screen">Documentation on Writing and Editing Posts</a>') . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('For more information:') . '</strong></p>' .
			'<p>' . __('<a href="https://dev.mandarincms.com/Pages_Add_New_Screen">Documentation on Adding New Pages</a>') . '</p>' .
			'<p>' . __('<a href="https://dev.mandarincms.com/Pages_Screen#Editing_Individual_Pages">Documentation on Editing Pages</a>') . '</p>' .
			'<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
	);
	...
	get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="https://dev.mandarincms.com/Media_Add_New_Screen#Edit_Media">Documentation on Edit Media</a>') . '</p>' .
	'<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
	);
}
...
if ( 'post' == $post_type || 'page' == $post_type ) {
	...
	$inserting_media .= '<p>' . __( 'You can also embed media from many popular websites including Twitter, YouTube, Flickr and others by pasting the media URL on its own line into the content of your post/page. Please refer to the Codex to <a href="https://dev.mandarincms.com/Embeds">learn more about embeds</a>.' ) . '</p>';
...
if ( 'post' == $post_type ) {
	...
	if ( current_myskin_supports( 'post-formats' ) && post_type_supports( 'post', 'post-formats' ) ) {
		$publish_box .= '<li>' . __( '<strong>Format</strong> &mdash; Post Formats designate how your myskin will display a specific post. For example, you could have a <em>standard</em> blog post with a title and paragraphs, or a short <em>aside</em> that omits the title and contains a short text blurb. Please refer to the Codex for <a href="https://dev.mandarincms.com/Post_Formats#Supported_Formats">descriptions of each post format</a>. Your myskin could enable all or some of 10 possible formats.' ) . '</li>';
	}

	if ( current_myskin_supports( 'post-thumbnails' ) && post_type_supports( 'post', 'thumbnail' ) ) {
		/* translators: %s: Featured Image */
		$publish_box .= '<li>' . sprintf( __( '<strong>%s</strong> &mdash; This allows you to associate an image with your post without inserting it. This is usually useful only if your myskin makes use of the image as a post thumbnail on the home page, a custom header, etc.' ), esc_html( $post_type_object->labels->featured_image ) ) . '</li>';
	}
	...
	$discussion_settings  = '<p>' . __('<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other MandarinCMS sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.') . '</p>';
	...
	$page_attributes = '<p>' . __('<strong>Parent</strong> &mdash; You can arrange your pages in hierarchies. For example, you could have an &#8220;About&#8221; page that has &#8220;Life Story&#8221; and &#8220;My Dog&#8221; pages under it. There are no limits to how many levels you can nest pages.') . '</p>' .
		'<p>' . __('<strong>Template</strong> &mdash; Some myskins have custom templates you can use for certain pages that might have additional features or custom layouts. If so, you&#8217;ll see them in this dropdown menu.') . '</p>' .
...
?>>
<?php mcms_nonce_field($nonce_action); ?>
...
if ( 'draft' != get_post_status( $post ) )
	mcms_original_referer_field(true, 'previous');
...
if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
	$shortlink = mcms_get_shortlink($post->ID, 'post');
...
<?php
mcms_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div><!-- /titlediv -->
<?php
}
...
?>
<div id="postdivrich" class="postarea<?php if ( $_mcms_editor_expand ) { echo ' mcms-editor-expand'; } ?>">

<?php mcms_editor( $post->post_content, 'content', array(
	...
	'tinymce' => array(
		'resize' => false,
		'mcms_autoresize_on' => $_mcms_editor_expand,
		'add_unload_trigger' => false,
		'mcms_keep_scroll_position' => ! $is_IE,
	),
) ); ?>
<table id="post-status-info"><tbody><tr>
	<td id="mcms-word-count" class="hide-if-no-js"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
...
<?php if ( ! mcms_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
