...
if ( !is_multisite() )
	mcms_die( __( 'Multisite support is not enabled.' ) );

if ( ! current_user_can( 'delete_site' ) )
	mcms_die(__( 'Sorry, you are not allowed to delete this site.'));

if ( isset( $_GET['h'] ) && $_GET['h'] != '' && get_option( 'delete_blog_hash' ) != false ) {
	if ( hash_equals( get_option( 'delete_blog_hash' ), $_GET['h'] ) ) {
		mcmsmu_delete_blog( get_current_blog_id() );
		mcms_die( sprintf( __( 'Thank you for using %s, your site has been deleted. Happy trails to you until we meet again.' ), get_network()->site_name ) );
	} else {
		mcms_die( __( 'Sorry, the link you clicked is stale. Please select another option.' ) );
	}
}
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'deleteblog' && isset( $_POST['confirmdelete'] ) && $_POST['confirmdelete'] == '1' ) {
	...
	$hash = mcms_generate_password( 20, false );
...
###SITE_NAME###" );
	...
	mcms_mail( get_option( 'admin_email' ), "[ " . mcms_specialchars_decode( get_option( 'blogname' ) ) . " ] ".__( 'Delete My Site' ), $content );
...
<?php } else {
	...
	<form method="post" name="deletedirect">
		<?php mcms_nonce_field( 'delete-blog' ) ?>
