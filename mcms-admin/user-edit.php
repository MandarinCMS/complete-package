...
mcms_reset_vars( array( 'action', 'user_id', 'mcms_http_referer' ) );
...
$profile_help = '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using MandarinCMS.') . '</p>' .
	'<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your MandarinCMS administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.') . '</p>' .
	'<p>' . __( 'You can select the language you wish to use while using the MandarinCMS administration screen without affecting the language site visitors see.' ) . '</p>' .
	...
	'<p>' . __('Required fields are indicated; the rest are optional. Profile information will only be displayed if your myskin is set up to do so.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://dev.mandarincms.com/Users_Your_Profile_Screen">Documentation on User Profiles</a>') . '</p>' .
    '<p>' . __('<a href="https://mandarincms.com/support/">Support Forums</a>') . '</p>'
);

$mcms_http_referer = remove_query_arg( array( 'update', 'delete_count', 'user_id' ), $mcms_http_referer );
...
if ( is_multisite()
	...
	mcms_die( __( 'Sorry, you are not allowed to edit this user.' ) );
}
...
if ( IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email && hash_equals( $new_email[ 'hash' ], $_GET[ 'newuseremail' ] ) ) {
		...
		if ( is_multisite() && $mcmsdb->get_var( $mcmsdb->prepare( "SELECT user_login FROM {$mcmsdb->signups} WHERE user_login = %s", $current_user->user_login ) ) ) {
			$mcmsdb->query( $mcmsdb->prepare( "UPDATE {$mcmsdb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		}
		mcms_update_user( $user );
		delete_user_meta( $current_user->ID, '_new_email' );
		mcms_redirect( add_query_arg( array( 'updated' => 'true' ), self_admin_url( 'profile.php' ) ) );
		die();
	} else {
		mcms_redirect( add_query_arg( array( 'error' => 'new-email' ), self_admin_url( 'profile.php' ) ) );
	}
...
if ( is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $mcmsdb->get_var( $mcmsdb->prepare( "SELECT user_login FROM {$mcmsdb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
		$mcmsdb->query( $mcmsdb->prepare( "UPDATE {$mcmsdb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );
	}
}
...
if ( !is_mcms_error( $errors ) ) {
	...
	if ( $mcms_http_referer )
		$redirect = add_query_arg('mcms_http_referer', urlencode($mcms_http_referer), $redirect);
	mcms_redirect($redirect);
	exit;
}
...
$sessions = MCMS_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated notice is-dismissible">
	...
	<?php endif; ?>
	<?php if ( $mcms_http_referer && false === strpos( $mcms_http_referer, 'user-new.php' ) && ! IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( mcms_validate_redirect( esc_url_raw( $mcms_http_referer ), self_admin_url( 'users.php' ) ) ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
...
<?php endif; ?>
<?php if ( isset( $errors ) && is_mcms_error( $errors ) ) : ?>
...
?>>
<?php mcms_nonce_field('update-user_' . $user_id) ?>
<?php if ( $mcms_http_referer ) : ?>
	<input type="hidden" name="mcms_http_referer" value="<?php echo esc_url($mcms_http_referer); ?>" />
<?php endif; ?>
<p>
...
$show_syntax_highlighting_preference = (
	...
	user_can( $profileuser, 'edit_myskin_options' )
	||
	// Edit modules.
	user_can( $profileuser, 'edit_modules' )
	||
	// Edit myskins.
	user_can( $profileuser, 'edit_myskins' )
);
?>

<?php if ( count($_mcms_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>

<?php
endif; // $_mcms_admin_css_colors
 ...
 * @since 2.7.0
 *
 * @param MCMS_User $profileuser The current MCMS_User object.
 */
...
<tr class="user-email-wrap">
	...
	<p><?php
		...
		printf(
			' <a href="%1$s">%2$s</a>',
			esc_url( mcms_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ),
			__( 'Cancel' )
		);
	?></p>
	</div>
...
<?php
	foreach ( mcms_get_user_contact_methods( $profileuser ) as $name => $desc ) {
?>
 ...
 * @param MCMS_User $profileuser User object for the current user to edit.
 */
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button mcms-generate-pw hide-if-no-js"><?php _e( 'Generate Password' ); ?></button>
		<div class="mcms-pwd hide-if-js">
			<span class="password-input-wrapper">
				<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="off" data-pw="<?php echo esc_attr( mcms_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
			</span>
			<button type="button" class="button mcms-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
				<span class="dashicons dashicons-hidden"></span>
				<span class="text"><?php _e( 'Hide' ); ?></span>
			</button>
			<button type="button" class="button mcms-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
