...
if ( ! current_user_can( 'manage_privacy_options' ) ) {
	mcms_die( __( 'Sorry, you are not allowed to manage privacy on this site.' ) );
}
...
if ( ! empty( $action ) ) {
	...
	if ( 'set-privacy-page' === $action ) {
		...
		update_option( 'mcms_page_for_privacy_policy', $privacy_policy_page_id );
		...
		$privacy_policy_page_content = MCMS_Privacy_Policy_Content::get_default_content();
		$privacy_policy_page_id = mcms_insert_post(
			array(
		...
		if ( is_mcms_error( $privacy_policy_page_id ) ) {
			add_settings_error(
				'page_for_privacy_policy',
				'page_for_privacy_policy',
			...
			mcms_redirect( admin_url( 'post.php?post=' . $privacy_policy_page_id . '&action=edit' ) );
			exit;
		}
	}
}
...
$privacy_policy_page_id     = (int) get_option( 'mcms_page_for_privacy_policy' );

if ( ! empty( $privacy_policy_page_id ) ) {

	$privacy_policy_page = get_post( $privacy_policy_page_id );

	if ( ! $privacy_policy_page instanceof MCMS_Post ) {
		add_settings_error(
			'page_for_privacy_policy',
			'page_for_privacy_policy',
...
<div class="wrap">
	...
	<p>
		...
		<?php _e( 'We would also suggest reviewing your privacy policy from time to time, especially after installing or updating any myskins or modules. There may be changes or new suggested information for you to consider adding to your policy.' ); ?>
	</p>
	<?php
	...
	if ( $privacy_policy_page_exists ) {
		...
		<p>
			...
			printf(
				...
				admin_url( 'tools.php?mcms-privacy-policy-guide' ),
				'',
				''
			);
			...
			<td>
				...
				if ( $has_pages ) : ?>
					<form method="post" action="">
						...
						mcms_nonce_field( 'set-privacy-page' );
				...
				<form class="mcms-create-privacy-page" method="post" action="">
					...
					<?php
					mcms_nonce_field( 'create-privacy-page' );
