 ...
 *
 * Requires mcms-admin/includes/revision.php.
 *
 * @package MandarinCMS
...
mcms_reset_vars( array( 'revision', 'action', 'from', 'to' ) );
...
case 'restore' :
	...
	if ( ! mcms_revisions_enabled( $post ) && ! mcms_is_post_autosave( $revision ) ) {
	...
	mcms_restore_post_revision( $revision->ID );
...
mcms_enqueue_script( 'revisions' );
mcms_localize_script( 'revisions', '_mcmsRevisionsSettings', mcms_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://dev.mandarincms.com/Revision_Management">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://mandarincms.com/support/">Support Forums</a>' ) . '</p>';
