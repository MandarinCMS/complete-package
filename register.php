...
if ( is_array( get_site_option( 'illegal_names' )) && isset( $_GET[ 'new' ] ) && in_array( $_GET[ 'new' ], get_site_option( 'illegal_names' ) ) ) {
	mcms_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via mcms_head
 *
 * @since MU (3.0.0)
 */
...
add_action( 'mcms_head', 'do_signup_header' );

if ( !is_multisite() ) {
	mcms_redirect( mcms_registration_url() );
	die();
}

if ( !is_main_site() ) {
	mcms_redirect( network_site_url( 'register.php' ) );
	die();
}

// Fix for page title
$mcms_query->is_404 = false;
 ...
 * @since MU (3.0.0)
 */
function mcmsmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'mcms_head', 'mcmsmu_signup_stylesheet' );
get_header( 'mcms-signup' );
...
<div class="mu_register mcms-signup-container">
<?php
/**
 ...
 * @param MCMS_Error|string $errors     A MCMS_Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( ! empty( $languages ) ) :
		...
		<p>
			...
			<?php
			// Network default.
			$lang = get_site_option( 'MCMSLANG' );

			if ( isset( $_POST['MCMSLANG'] ) ) {
				$lang = $_POST['MCMSLANG'];
			}
	 ...
	 * @since 3.0.0
	 *
	 * @param MCMS_Error $errors A MCMS_Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
...
function validate_blog_form() {
	...
	return mcmsmu_validate_blog_signup($_POST['blogname'], $_POST['blog_title'], $user);
}
...
function show_user_form($user_name = '', $user_email = '', $errors = '') {
	 ...
	 * @since 3.0.0
	 *
	 * @param MCMS_Error $errors A MCMS_Error object containing 'user_name' or 'user_email' errors.
	 */
...
function validate_user_form() {
	return mcmsmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type MCMS_Error $errors     A MCMS_Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
...
function validate_another_blog_signup() {
	...
	if ( ! empty( $_POST['MCMSLANG'] ) ) {

		$languages = signup_get_available_languages();

		if ( in_array( $_POST['MCMSLANG'], $languages ) ) {
			$language = mcms_unslash( sanitize_text_field( $_POST['MCMSLANG'] ) );

			if ( $language ) {
				$blog_meta_defaults['MCMSLANG'] = $language;
			}
		}
	...
	$blog_id = mcmsmu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, get_current_network_id() );
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array(), $blog_id = 0 ) {
	...
	if ( $blog_id ) {
		...
		$login_url = 'http://' . $domain . $path . 'mcms-login.php';
	}
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type MCMS_Error $errors     A MCMS_Error object with possible errors relevant to the sign-up user.
	 * }
	 */
...
function validate_user_signup() {
	...
	mcmsmu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function signup_blog($user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '') {
	 ...
	 *     @type MCMS_Error $errors     A MCMS_Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
function validate_blog_signup() {
	...
	$user_result = mcmsmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result = mcmsmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain = $result['domain'];
	$path = $result['path'];
	$blogname = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	mcmsmu_signup_blog($domain, $path, $blog_title, $user_name, $user_email, $meta);
 ...
 */
$active_signup = apply_filters( 'mcmsmu_active_signup', $active_signup );
...
if ( $active_signup == 'none' ) {
	...
	$login_url = mcms_login_url( network_site_url( 'register.php' ) );
	/* translators: %s: login URL */
	printf( __( 'You must first <a href="%s">sign in</a>, and then you can create a new site.' ), $login_url );
} else {
...
<?php
/**
 * Fires after the sign-up forms, before mcms_footer.
 *
 * @since 3.0.0
 */
do_action( 'after_signup_form' ); ?>

<?php get_footer( 'mcms-signup' );
